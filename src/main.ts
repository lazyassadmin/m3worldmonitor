...
Sentry.init({
	...
	beforeSend(event) {
		...
		const vendorChunk =
			/\/(maplibre|deck-stack|d3|topojson|i18n|sentry|transformers|onnxruntime)-[A-Za-z0-9_-]+\.js/;
		const firstPartyFile = (filename: string) => {
			if (/\.(ts|tsx)$/.test(filename) || /^src\//.test(filename)) return true;
			if (/\/assets\/[A-Za-z0-9_-]+(-[A-Za-z0-9_-]+)*\.js/.test(filename))
		...
		if (
			...
			if (
				frames.some((f) => /\/(map|maplibre|deck-stack)-[A-Za-z0-9_-]+\.js/.test(f.filename ?? ""))
			)
				return null;
		}
		...
		if ((excType === "TypeError" || /^TypeError:/.test(msg)) && frames.length > 0) {
			if (
				...
				nonInfraFrames.every((f) =>
					/\/(map|maplibre|deck-stack)-[A-Za-z0-9_-]+\.js/.test(f.filename ?? ""),
				)
			)
				return null;
		}
		...
		if (
			/null is not an object \(evaluating '\w{1,3}\.(id|type|style)'\)/.test(msg) &&
			frames.length === 0
		)
			return null;
		...
		if (
			...
			/insertBefore/.test(msg) &&
			frames.every((f) => !f.lineno && !f.function)
		)
			return null;
		...
		if (
			frames.some(
				(f) => /\/uv\/service\//.test(f.filename ?? "") || /uv\.handler/.test(f.filename ?? ""),
			)
		)
			return null;
		...
		if (
			frames.length > 0 &&
			frames.every((f) => !f.filename || /\/x-plugin-script\//.test(f.filename))
		)
			return null;
		...
		if (
			/^options is not defined$/.test(msg) &&
			frames.every(
		...
		if (
			(/TransactionInactiveError/.test(msg) || excType === "TransactionInactiveError") &&
			!hasFirstParty
		)
			return null;
		...
		if (
			...
			(/\.(?:toLowerCase|trim|indexOf|findIndex) is not a function/.test(msg) ||
				...
				/(?:Failed to fetch|Importing a module script failed|error loading) dynamically imported module/i.test(
					msg,
				) ||
...
function shouldSuppressCspViolation(
	...
	if (directive === "connect-src" && cspConnectSrcAllowsHttps) {
		try {
			...
			/* scheme-only values like "blob" fall through */
		}
	}
	// Browser extensions or injected scripts.
	if (
		/^(?:chrome|moz|safari(?:-web)?)-extension/.test(sourceFile) ||
		/^(?:chrome|moz|safari(?:-web)?)-extension/.test(blockedURI)
	)
		return true;
	...
	if (/gstatic\.com\/_\/translate/.test(blockedURI) || /facebook\.net/.test(blockedURI))
		return true;
	...
	navigator.serviceWorker
		...
		.then((registration) => {
			...
			const maybeCheckForSwUpdate = async (
				...
				const interval =
					lastOk >= lastCheck ? SW_UPDATE_SUCCESS_INTERVAL_MS : SW_UPDATE_FAILURE_INTERVAL_MS;
