...
const commodityBreaker = createCircuitBreaker<ListCommodityQuotesResponse>({
	name: "Commodity Quotes",
	cacheTtlMs: MARKET_QUOTES_CACHE_TTL_MS,
	persistCache: true,
});
const sectorBreaker = createCircuitBreaker<GetSectorSummaryResponse>({
	name: "Sector Summary",
	cacheTtlMs: MARKET_QUOTES_CACHE_TTL_MS,
	persistCache: true,
});
...
const cryptoSectorsBreaker = createCircuitBreaker<ListCryptoSectorsResponse>({
	name: "Crypto Sectors",
	persistCache: true,
});
const defiBreaker = createCircuitBreaker<ListDefiTokensResponse>({
	name: "DeFi Tokens",
	persistCache: true,
});
const aiBreaker = createCircuitBreaker<ListAiTokensResponse>({
	name: "AI Tokens",
	persistCache: true,
});
const otherBreaker = createCircuitBreaker<ListOtherTokensResponse>({
	name: "Other Tokens",
	persistCache: true,
});
...
export async function fetchMultipleStocks(
	...
	const resp = await stockBreaker.execute(
		async () => {
	...
	const results = resp.quotes.map((q) => {
		...
		const meta =
			symbolMetaMap.get(trimmed) ?? uppercaseMetaMap.get(trimmed.toUpperCase()) ?? undefined;
...
export async function fetchCommodityQuotes(
	...
	const resp = await commodityBreaker.execute(
		async () => {
