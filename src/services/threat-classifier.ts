...
const ESCALATION_ACTIONS =
	/\b(attack|attacks|attacked|strike|strikes|struck|bomb|bombs|bombed|bombing|shell|shelled|shelling|missile|missiles|intercept|intercepted|retaliates|retaliating|retaliation|killed|casualties|offensive|invaded|invades)\b/;
const ESCALATION_TARGETS =
	/\b(iran|tehran|isfahan|tabriz|russia|moscow|china|beijing|taiwan|taipei|north korea|pyongyang|nato|us base|us forces|american forces|us military)\b/;
			...
			for (let i = 0; i < batch.length; i++) {
				...
				try {
					...
					if (
						...
						console.warn(
							`[Classify] ${err.statusCode} — pausing AI classification for ${delay / 1000}s (backoff #${consecutive429s})`,
						);
...
function classifyWithAIUncached(
	...
	return new Promise((resolve) => {
		if (batchQueue.length >= MAX_QUEUE_LENGTH) {
			console.warn(
				`[Classify] Queue full (${MAX_QUEUE_LENGTH}), dropping classification for: ${title.slice(0, 60)}`,
			);
			resolve(null);
			return;
		}
...
export function classifyWithAI(
	...
	return classifyBreaker.execute(() => classifyWithAIUncached(title, variant), null, {
		cacheKey,
