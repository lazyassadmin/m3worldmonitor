...
class SignalAggregator {
	...
	ingestRadiationObservations(observations: RadiationObservation[]): void {
		...
		for (const observation of observations) {
			...
			const code =
				normalizeCountryCode(observation.country) ||
				this.coordsToCountry(observation.lat, observation.lon);
	...
	ingestTemporalAnomalies(
		...
		this.signals = this.signals.filter(
			(s) =>
				s.type !== "temporal_anomaly" || !typesToClear.has(this.temporalSourceMap.get(s) || ""),
		);
