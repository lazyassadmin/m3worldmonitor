...
function getWbBreaker(indicatorCode: string) {
	if (!wbBreakers.has(indicatorCode)) {
		if (wbBreakers.size >= WB_BREAKERS_WARN_THRESHOLD) {
			console.warn(
				`[wb] breaker pool at ${wbBreakers.size} — unexpected growth, investigate getWbBreaker callers`,
			);
		}
		wbBreakers.set(
			indicatorCode,
...
const blsBreaker = createCircuitBreaker<FredSeries[]>({
	name: "BLS Batch",
	cacheTtlMs: 15 * 60 * 1000,
	persistCache: true,
});
...
const crudeBreaker = createCircuitBreaker<GetCrudeInventoriesResponse>({
	name: "EIA Crude Inventories",
	cacheTtlMs: 60 * 60 * 1000,
	persistCache: true,
});
const emptyEuGasFallback: GetEuGasStorageResponse = {
	fillPct: 0,
	fillPctChange1d: 0,
...
const euGasBreaker = createCircuitBreaker<GetEuGasStorageResponse>({
	name: "EU Gas Storage",
...
const eurostatBreaker = createCircuitBreaker<GetEurostatCountryDataResponse>({
	name: "Eurostat Country Data",
	cacheTtlMs: 4 * 60 * 60 * 1000,
	persistCache: true,
});
const emptyNatGasFallback: GetNatGasStorageResponse = { weeks: [], latestPeriod: "" };
const natGasBreaker = createCircuitBreaker<GetNatGasStorageResponse>({
	name: "EIA Nat Gas Storage",
	cacheTtlMs: 60 * 60 * 1000,
	persistCache: true,
});
...
const oilStocksAnalysisBreaker = createCircuitBreaker<GetOilStocksAnalysisResponse>({
	name: "IEA Oil Stocks Analysis",
	cacheTtlMs: 4 * 60 * 60 * 1000,
	persistCache: true,
});
...
export async function fetchFredData(): Promise<FredSeries[]> {
	...
	const resp = await fredBatchBreaker.execute(
		async () => {
			try {
	...
	for (const config of FRED_SERIES) {
		...
		if (obs.length >= 2) {
			...
			const changePercent =
				previous.value !== 0 ? ((latest.value - previous.value) / previous.value) * 100 : null;
			...
			out.push({
				...
				observations: obs
					.slice(-30)
					.map((o) => ({ date: o.date, value: toDisplayValue(o.value, config) })),
			});
		} else {
			const latest = obs[0]!;
...
export async function fetchBlsData(): Promise<FredSeries[]> {
	return blsBreaker.execute(
		async () => {
			const results = await Promise.allSettled(
				BLS_SERIES.map((cfg) =>
					client
						.getBlsSeries({ seriesId: cfg.id, limit: 60 }, { signal: AbortSignal.timeout(15_000) })
			...
			for (let i = 0; i < BLS_SERIES.length; i++) {
				...
				const change = previous
					? Number((latest.value - previous.value).toFixed(cfg.precision))
					: null;
				const changePercent =
				...
				const displayDate = lastObs.periodName
					? `${lastObs.periodName} ${lastObs.year}`
					: latest.date;
...
export async function fetchNatGasStorageRpc(): Promise<GetNatGasStorageResponse> {
	...
	if (hydrated?.weeks?.length) return hydrated;
	try {
		return await natGasBreaker.execute(
			async () => {
...
export async function fetchEnergyCapacityRpc(
	...
	try {
		return await capacityBreaker.execute(
			async () => {
			...
			{ shouldCache: (r) => r.series.length > 0 },
		);
	} catch {
...
function buildWorldBankResponse(
	...
	timeSeries.sort(
		(a, b) => b.year.localeCompare(a.year) || a.countryCode.localeCompare(b.countryCode),
	);
...
export const INDICATOR_PRESETS = {
	digitalInfrastructure: ["IT.NET.USER.ZS", "IT.CEL.SETS.P2", "IT.NET.BBND.P2", "IT.NET.SECR.P6"],
...
const nationalDebtBreaker = createCircuitBreaker<GetNationalDebtResponse>({
	name: "National Debt",
	cacheTtlMs: 6 * 60 * 60 * 1000,
});
const emptyNationalDebtFallback: GetNationalDebtResponse = {
	entries: [],
	seededAt: "",
	unavailable: true,
};
		...
		const [policy, eer, credit] = await Promise.all([
			hPolicy?.rates?.length
				...
				: bisPolicyBreaker.execute(
						() => client.getBisPolicyRates({}, { signal: AbortSignal.timeout(20_000) }),
			...
			hEer?.rates?.length
				...
				: bisEerBreaker.execute(
						() => client.getBisExchangeRates({}, { signal: AbortSignal.timeout(20_000) }),
						emptyBisEerFallback,
						{ shouldCache: (r) => (r.rates?.length ?? 0) > 0 },
					),
...
const ecbFxRatesBreaker = createCircuitBreaker<GetEcbFxRatesResponse>({
	name: "ECB FX Rates",
	cacheTtlMs: 4 * 60 * 60 * 1000,
});
const emptyEcbFxRatesFallback: GetEcbFxRatesResponse = {
	rates: [],
	updatedAt: "",
	seededAt: "0",
	unavailable: true,
...
export async function getEurostatCountryData(): Promise<GetEurostatCountryDataResponse> {
	const hydrated = getHydratedData("eurostatCountryData") as
