...
import {
	...
	setGlobeVisualPreset,
	GLOBE_VISUAL_PRESET_OPTIONS,
...
function renderMapThemeDropdown(container: HTMLElement, provider: MapProvider): void {
	...
	select.innerHTML = MAP_THEME_OPTIONS[provider]
		.map(
			(opt) =>
				`<option value="${opt.value}"${opt.value === currentTheme ? " selected" : ""}>${escapeHtml(opt.label)}</option>`,
		)
		.join("");
}
...
export function renderPreferences(host: PreferencesHost): PreferencesResult {
	...
	html += toggleRowHtml(
		"us-badge-anim",
		t("components.insights.badgeAnimLabel"),
		t("components.insights.badgeAnimDesc"),
		settings.badgeAnimation,
	);
	html += `</div></details>`;
		...
		attach(container: HTMLElement): () => void {
					...
					if (target.id === "usImportInput") {
						...
						importSettings(file)
							.then((result: ImportResult) => {
								showToast(
									container,
									t("components.settings.importSuccess", { count: String(result.keysImported) }),
									true,
								);
							})
							.catch(() => {
			...
			container.addEventListener(
				...
				(e) => {
					...
					if (tab?.dataset.fwTab) {
						...
						container
							.querySelectorAll(".fw-import-tab")
							.forEach((el) =>
								el.classList.toggle("active", (el as HTMLElement).dataset.fwTab === tabId),
							);
						container.querySelectorAll(".fw-import-tab-panel").forEach((el) => {
							const panelEl = el as HTMLElement;
							panelEl.classList.toggle(
								"active",
								panelEl.id === `fwTabPanel${tabId.charAt(0).toUpperCase() + tabId.slice(1)}`,
							);
						});
						return;
					}
						...
						fetch("/api/skills/fetch-agentskills", {
							...
							.then((data) => {
								if (!data.instructions) {
									showImportError(
										errEl,
										"This skill has no instructions — it may use tools only (not supported).",
									);
									return;
								}
								...
								if (descEl)
									descEl.textContent =
										data.instructions.slice(0, 200) + (data.instructions.length > 200 ? "…" : "");
								if (preview) {
									preview.style.display = "block";
									(
										preview as HTMLElement & {
											_fwData?: { name: string; description: string; instructions: string };
										}
									)._fwData = {
					...
					if (target.closest("#fwAgentskillsSaveBtn")) {
						...
						try {
							saveImportedFramework({
								...
								systemPromptAppend: fwData.instructions,
							});
					...
					if (renameBtn?.dataset.fwId) {
						...
						const current = renameBtn
							.closest(".fw-library-item")
							?.querySelector(".fw-library-item-name");
			...
			if (!host.isDesktopApp && host.isSignedIn) {
				...
				function channelIcon(type: ChannelType): string {
					if (type === "telegram")
						return `<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>`;
					if (type === "email")
						return `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>`;
					if (type === "discord")
						return `<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/></svg>`;
				...
				function renderChannelRow(channel: NotificationChannel | null, type: ChannelType): string {
					...
					if (channel?.verified) {
						...
						if (type === "telegram") {
							...
							const ch = rawCh
								? `#${escapeHtml(rawCh.startsWith("#") ? rawCh.slice(1) : rawCh)}`
								: "connected";
            ...
            <select class="unified-settings-select" id="usSharedTimezone" style="width:100%">${makeTzOptions(sharedTz)}</select>`;
					...
					(e) => {
						...
						if (target.id === "usNotifEnabled" || target.id === "usNotifSensitivity") {
							...
							alertRuleDebounceTimer = setTimeout(() => {
								const enabledEl = container.querySelector<HTMLInputElement>("#usNotifEnabled");
								const sensitivityEl =
									container.querySelector<HTMLSelectElement>("#usNotifSensitivity");
              ...
              `;
									...
									notifPollInterval = setInterval(() => {
										...
										if (countdownEl)
											countdownEl.textContent = `Waiting… ${Math.max(0, remaining)}s`;
							...
							if (!email) {
								...
								if (rowEl) {
									...
									rowEl.insertAdjacentHTML(
										"beforeend",
										'<span class="us-notif-error">No email found on your account</span>',
									);
								}
								return;
							}
								...
								.then((oauthUrl) => {
									...
									if (!popup) {
										...
										if (rowEl) {
											...
											rowEl.insertAdjacentHTML(
												"beforeend",
												'<span class="us-notif-error">Popup blocked — please allow popups for this site, then try again.</span>',
											);
										}
									} else {
				...
				const onMessage = (e: MessageEvent): void => {
					...
					if (e.data?.type === "wm:slack_connected") {
						...
						if (rowEl) {
							...
							rowEl.insertAdjacentHTML(
								"beforeend",
								`<span class="us-notif-error">Slack connection failed: ${escapeHtml(String(e.data.error ?? "unknown"))}</span>`,
							);
						...
						if (!signal.aborted) {
							saveRuleWithNewChannel("discord");
							reloadNotifSection();
						}
						...
						if (rowEl) {
							...
							rowEl.insertAdjacentHTML(
								"beforeend",
								`<span class="us-notif-error">Discord connection failed: ${escapeHtml(String(e.data.error ?? "unknown"))}</span>`,
							);
...
function renderFrameworkLibraryHtml(): string {
	...
	if (frameworks.length === 0)
		return '<div class="fw-library-empty">No frameworks in library.</div>';
	return frameworks
		.map(
			(fw) => `
