...
const newsClient = new NewsServiceClient(getRpcBaseUrl(), {
...
async function tryBrowserT5(
	...
	try {
		...
		if (
			!summary ||
			summary.length < 20 ||
			summary.toLowerCase().includes("summarize") ||
			summary.toLowerCase().includes("résumez")
		) {
			return null;
		}
...
export async function generateSummary(
	...
	const optionsSuffix =
		options?.skipCloudProviders || options?.skipBrowserFallback
			? `:opts${options.skipCloudProviders ? "C" : ""}${options.skipBrowserFallback ? "B" : ""}`
			: "";
	const cacheKey =
		buildSummaryCacheKey(headlines, "brief", geoContext, SITE_VARIANT, lang) + optionsSuffix;
...
async function generateSummaryInternal(
	...
	if (!options?.skipCloudProviders) {
		try {
			...
			/* cache lookup failed — proceed to provider chain */
		}
	}
