...
async function ensureLocationMapLoaded(): Promise<void> {
	...
	if (locationMapPromise) {
		await locationMapPromise;
		return;
	}
	locationMapPromise = import("./oref-locations")
		.then((m) => {
			locationTranslator = m.translateLocation;
		})
		.catch(() => {
			locationMapPromise = null;
			console.warn("[OREF] Failed to load location translations, will retry");
		});
	await locationMapPromise;
}
...
function buildTranslationPrompt(alerts: OrefAlert[]): string {
	...
	return (
		"Translate each line from Hebrew to English. Keep the ALERT/AREAS/DESC labels and IDs exactly as-is. Only translate the text after the colon.\n" +
		lines.join("\n")
	);
}
...
function parseTranslationResponse(raw: string, alerts: OrefAlert[]): void {
	...
	for (const alert of alerts) {
		...
		const entry = {
			...
			data: (areas && !areas.some(hasHebrew)
				? areas
				: alert.data.map((d) =>
						locationTranslator ? locationTranslator(staticTranslate(d)) : staticTranslate(d),
					)
			).map(stripOrefLabels),
...
async function translateAlerts(alerts: OrefAlert[]): Promise<boolean> {
	const untranslated = alerts.filter(
		(a) => !translationCache.has(a.id) && alertNeedsTranslation(a),
	);
...
export function startOrefPolling(): void {
	...
	pollingLoop = startSmartPollLoop(
		async ({ signal }) => {
