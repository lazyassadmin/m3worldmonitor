...
function getPriorityFromRadiation(
	observation: RadiationObservation,
	spikeCount: number,
): AlertPriority {
	let score = 0;
...
function buildConvergenceAlert(convergence: GeoConvergenceAlert, alertId: string): UnifiedAlert {
	const location =
		getCountriesNearLocation(convergence.lat, convergence.lon).join(", ") || "Unknown";
	return {
		id: alertId,
		type: "convergence",
		priority: getPriorityFromConvergence(convergence.score, convergence.types.length),
		title: t("alerts.geoAlert", { location }),
		summary: t("alerts.eventsDetected", {
			count: convergence.totalEvents,
			lat: convergence.lat.toFixed(1),
			lon: convergence.lon.toFixed(1),
		}),
...
export function createCIIAlert(
	...
	const alert: UnifiedAlert = {
		...
		title: t(change > 0 ? "alerts.instabilityRising" : "alerts.instabilityFalling", {
			country: countryName,
		}),
...
export function createCascadeAlert(cascade: CascadeResult): UnifiedAlert | null {
	...
	const alert: UnifiedAlert = {
		...
		summary: t("alerts.countriesAffected", {
			count: cascade.countriesAffected.length,
...
function createSanctionsAlert(): UnifiedAlert | null {
	...
	const leadCountry = [...pressure.countries].sort(
		(a, b) => b.newEntryCount - a.newEntryCount || b.entryCount - a.entryCount,
	)[0];
	if (!leadCountry) return null;
	if (pressure.newEntryCount === 0 && leadCountry.entryCount < 25) return null;

	const leadProgram = [...pressure.programs].sort(
		(a, b) => b.newEntryCount - a.newEntryCount || b.entryCount - a.entryCount,
	)[0];
...
function getRadiationRank(observation: RadiationObservation): number {
	const severityRank =
		observation.severity === "spike" ? 2 : observation.severity === "elevated" ? 1 : 0;
	const confidenceRank =
		observation.confidence === "high" ? 2 : observation.confidence === "medium" ? 1 : 0;
...
function createRadiationAlert(): UnifiedAlert | null {
	...
	const countries = strongest.country
		? [strongest.country]
		: getCountriesNearLocation(strongest.lat, strongest.lon);
...
function shouldMergeAlerts(a: UnifiedAlert, b: UnifiedAlert): boolean {
	...
	const sameTime = Math.abs(a.timestamp.getTime() - b.timestamp.getTime()) < ALERT_MERGE_WINDOW_MS;
	const sameLocation = !!(
		a.location &&
		b.location &&
...
function generateCompositeTitle(a: UnifiedAlert, b: UnifiedAlert): string {
	...
	if (ciiChange) {
		return t(ciiChange.change > 0 ? "alerts.instabilityRising" : "alerts.instabilityFalling", {
			country: ciiChange.countryName,
		});
	}
...
function generateCompositeSummary(a: UnifiedAlert, b: UnifiedAlert): string {
	...
	if (ciiA && ciiB && ciiA.country === ciiB.country) {
		...
		return t(summaryKey, {
			from: earliest.previousScore,
			to: latest.currentScore,
			change: changeStr,
...
function identifyTopRisks(
	...
	if (leadSanctions && (sanctions.newEntryCount > 0 || leadSanctions.entryCount >= 25)) {
		...
		risks.push(
			`${label}: ${leadSanctions.countryName} (${leadSanctions.entryCount}, +${leadSanctions.newEntryCount} new)`,
		);
	}
	...
	if (strongestRadiation) {
		const status = strongestRadiation.corroborated
			? strongestRadiation.severity === "spike"
				? "Confirmed radiation spike"
				: "Confirmed radiation anomaly"
			: strongestRadiation.conflictingSources
				...
				: strongestRadiation.severity === "spike"
					...
					: "Elevated radiation";
		risks.push(
			`${status}: ${strongestRadiation.location} (+${strongestRadiation.delta.toFixed(1)} ${strongestRadiation.unit})`,
		);
	}
