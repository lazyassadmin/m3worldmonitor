...
function dispatchAlert(alert: BreakingAlert): void {
	console.log(
		"[breaking-news-alerts] dispatching:",
		alert.origin,
		alert.threatLevel,
		alert.headline.slice(0, 60),
	);
	pruneDedupeMap();
	...
	if (!RELAY_GATES_READY) {
		void (async () => {
			...
			if (!token) {
				console.warn("[breaking-news-alerts] no Clerk token, skipping notify");
				return;
			}
			...
			if (isDesktopRuntime()) {
				...
				fetch("/api/notify", {
					...
					.then((res) => {
						if (!res.ok)
							console.warn("[breaking-news-alerts] notify returned", res.status, alert.origin);
						else
							console.log("[breaking-news-alerts] notify queued:", alert.origin, alert.threatLevel);
					})
					.catch((err) => {
						console.warn("[breaking-news-alerts] notify network error:", err);
					});
			}
		})();
	}
