			...
			switch (event.event_type) {
				...
				case "checkout.error":
					...
					Sentry.captureMessage(
						`Dodo checkout overlay error: ${event.data?.message || "unknown"}`,
						{ level: "error", tags: { component: "dodo-checkout" } },
					);
					break;
			}
		},
...
export async function resumePendingCheckout(options?: { openAuth?: () => void }): Promise<boolean> {
	...
	console.log(
		`[checkout] resumePendingCheckout: intent=${intent.productId}, clerkUser=${clerkUser?.id ?? "null"}, hasOpenAuth=${!!options?.openAuth}`,
	);
...
export async function startCheckout(
	...
	try {
		...
		Sentry.captureException(err, {
			tags: { component: "dodo-checkout", action: "createCheckout" },
			extra: { productId },
		});
		if (fallbackToPricingPage) window.open("https://worldmonitor.app/pro", "_blank");
		return false;
