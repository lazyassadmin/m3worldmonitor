...
function getSymbolTokens(item: Pick<MarketData, "symbol" | "display" | "name">): string[] {
	const raw = [item.symbol, item.display, ...item.name.toLowerCase().split(/[^a-z0-9]+/gi)];
	const out: string[] = [];
...
function resolveTargets(
	...
	const targetEntries =
		explicitEntries || (watchlistEntries && watchlistEntries.length > 0 ? watchlistEntries : []);
...
function buildItemNote(change: number | null, relatedHeadline?: string): string {
	...
	return relatedHeadline ? `${moveNote} Headline driver: ${relatedHeadline}` : moveNote;
}
...
function buildSummaryInputs(
	items: DailyMarketBriefItem[],
	headlines: NewsItem[],
): { headlines: string[]; marketContext: string } {
	const marketContext = items
		.map((item) => {
...
function buildExtendedMarketContext(
	...
	if (regime && regime.compositeScore > 0) {
		const lines = [`Fear & Greed: ${regime.compositeScore.toFixed(0)} (${regime.compositeLabel})`];
...
export async function buildDailyMarketBrief(
	...
	const { headlines: summaryHeadlines, marketContext } = buildSummaryInputs(
		items,
		relevantHeadlines,
	);
	let extendedContext = buildExtendedMarketContext(
		marketContext,
		options.regimeContext,
		options.yieldCurveContext,
		options.sectorContext,
	);
