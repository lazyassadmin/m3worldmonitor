...
const co2Breaker = createCircuitBreaker<GetCo2MonitoringResponse>({
	name: "CO2 Monitoring",
	cacheTtlMs: 6 * 60 * 60 * 1000,
	persistCache: true,
});
const oceanIceBreaker = createCircuitBreaker<GetOceanIceDataResponse>({
	name: "Ocean Ice",
	cacheTtlMs: 26 * 60 * 60 * 1000,
	persistCache: true,
});
...
export async function fetchClimateAnomalies(): Promise<ClimateFetchResult> {
	...
	if (hydrated && (hydrated.anomalies ?? []).length > 0) {
		const anomalies = hydrated.anomalies
	...
	const response = await breaker.execute(
		async () => {
			return client.listClimateAnomalies({
				minSeverity: "ANOMALY_SEVERITY_UNSPECIFIED",
				pageSize: 0,
				cursor: "",
			});
		},
...
export async function fetchCo2Monitoring(): Promise<Co2Monitoring | null> {
	...
	const response = await co2Breaker.execute(
		async () => {
			return client.getCo2Monitoring({});
		},
		emptyCo2Fallback,
		{ shouldCache: (result) => Boolean(result.monitoring?.currentPpm) },
	);
...
export async function fetchOceanIceData(): Promise<OceanIceIndicators | null> {
	const hydrated = getHydratedData("oceanIce") as
		| GetOceanIceDataResponse
		| OceanIceSeedSnakeShape
		| undefined;
	...
	const response = await oceanIceBreaker.execute(
		async () => {
			return client.getOceanIceData({});
		},
		emptyOceanIceFallback,
		{ shouldCache: (result) => Boolean(result.data) },
	);
