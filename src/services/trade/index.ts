...
const client = new TradeServiceClient(getRpcBaseUrl(), {
	fetch: (...args) => globalThis.fetch(...args),
});

const restrictionsBreaker = createCircuitBreaker<GetTradeRestrictionsResponse>({
	name: "WTO Restrictions",
	cacheTtlMs: 30 * 60 * 1000,
	persistCache: true,
});
const tariffsBreaker = createCircuitBreaker<GetTariffTrendsResponse>({
	name: "WTO Tariffs",
	cacheTtlMs: 30 * 60 * 1000,
	persistCache: true,
});
const flowsBreaker = createCircuitBreaker<GetTradeFlowsResponse>({
	name: "WTO Flows",
	cacheTtlMs: 30 * 60 * 1000,
	persistCache: true,
});
const barriersBreaker = createCircuitBreaker<GetTradeBarriersResponse>({
	name: "WTO Barriers",
	cacheTtlMs: 30 * 60 * 1000,
	persistCache: true,
});
const revenueBreaker = createCircuitBreaker<GetCustomsRevenueResponse>({
	name: "Treasury Revenue",
	cacheTtlMs: 30 * 60 * 1000,
	persistCache: true,
});
const comtradeBreaker = createCircuitBreaker<ListComtradeFlowsResponse>({
	name: "Comtrade Flows",
	cacheTtlMs: 6 * 60 * 60 * 1000,
	persistCache: true,
});

const emptyRestrictions: GetTradeRestrictionsResponse = {
	restrictions: [],
	fetchedAt: "",
...
const emptyBarriers: GetTradeBarriersResponse = {
	barriers: [],
	fetchedAt: "",
	upstreamUnavailable: false,
};
const emptyRevenue: GetCustomsRevenueResponse = {
	months: [],
	fetchedAt: "",
	upstreamUnavailable: false,
};
const emptyComtrade: ListComtradeFlowsResponse = {
	flows: [],
	fetchedAt: "",
	upstreamUnavailable: false,
};

export async function fetchTradeRestrictions(
	countries: string[] = [],
	limit = 50,
): Promise<GetTradeRestrictionsResponse> {
	if (!isFeatureAvailable("wtoTrade")) return emptyRestrictions;
	try {
		return await restrictionsBreaker.execute(
			async () => {
				return client.getTradeRestrictions({ countries, limit });
			},
			emptyRestrictions,
			{ shouldCache: (r) => (r.restrictions?.length ?? 0) > 0 },
		);
	} catch {
...
export async function fetchTradeFlows(
	...
	try {
		return await flowsBreaker.execute(
			async () => {
				return client.getTradeFlows({ reportingCountry, partnerCountry, years });
			},
			emptyFlows,
			{ shouldCache: (r) => (r.flows?.length ?? 0) > 0 },
		);
	} catch {
		return emptyFlows;
	}
...
export async function fetchTradeBarriers(
	countries: string[] = [],
	measureType = "",
	limit = 50,
	...
	try {
		return await barriersBreaker.execute(
			async () => {
				return client.getTradeBarriers({ countries, measureType, limit });
			},
			emptyBarriers,
			{ shouldCache: (r) => (r.barriers?.length ?? 0) > 0 },
		);
	} catch {
...
export async function fetchCustomsRevenue(): Promise<GetCustomsRevenueResponse> {
	...
	try {
		return await revenueBreaker.execute(
			async () => {
				return client.getCustomsRevenue({});
			},
			emptyRevenue,
			{ shouldCache: (r) => (r.months?.length ?? 0) > 0 },
		);
	} catch {
		return emptyRevenue;
	}
...
export async function fetchComtradeFlows(): Promise<ListComtradeFlowsResponse> {
	try {
		return await comtradeBreaker.execute(
			async () => {
