...
function msToDt(ms: number): Date | null {
...
function toDisplayOps(p: ProtoOpsSummary): AirportOpsSummary {
	return {
		...
		totalFlights: p.totalFlights,
		closureStatus: p.closureStatus,
		notamFlags: p.notamFlags ?? [],
		severity: SEVERITY_MAP[p.severity] ?? "normal",
		topDelayReasons: p.topDelayReasons ?? [],
		source: p.source,
		updatedAt: new Date(p.updatedAt),
	};
}
...
function toDisplayFlight(p: ProtoFlight): FlightInstance {
	return {
		...
		scheduledDeparture: msToDt(p.scheduledDeparture),
		scheduledArrival: msToDt(p.scheduledArrival),
...
function toDisplayCarrierOps(p: ProtoCarrierOps): CarrierOps {
	return {
		carrierIata: p.carrier?.iataCode ?? "",
		carrierName: p.carrier?.name ?? p.carrier?.iataCode ?? "",
		airport: p.airport,
		totalFlights: p.totalFlights,
...
const breakerFlights = createCircuitBreaker<FlightInstance[]>({
	name: "Airport Flights",
	cacheTtlMs: 5 * 60 * 1000,
	persistCache: false,
});
const breakerCarrier = createCircuitBreaker<CarrierOps[]>({
	name: "Carrier Ops",
	cacheTtlMs: 5 * 60 * 1000,
	persistCache: false,
});
const breakerStatus = createCircuitBreaker<FlightInstance[]>({
	name: "Flight Status",
	cacheTtlMs: 6 * 60 * 1000,
	persistCache: false,
});
const breakerTrack = createCircuitBreaker<PositionSample[]>({
	name: "Track Aircraft",
	cacheTtlMs: 15 * 1000,
	persistCache: false,
});
const breakerPrices = createCircuitBreaker<{ quotes: PriceQuote[]; isDemoMode: boolean }>({
	name: "Flight Prices",
...
const breakerGoogleFlights = createCircuitBreaker<GoogleFlightsResult>({
	name: "Google Flights",
	cacheTtlMs: 0,
	persistCache: false,
});
// 5-min client cache (server has 10-min Redis + medium gateway cache)
const breakerGoogleDates = createCircuitBreaker<GoogleDatesResult>({
	name: "Google Dates",
	cacheTtlMs: 5 * 60 * 1000,
	persistCache: false,
});
...
export async function fetchFlightDelays(): Promise<AirportDelayAlert[]> {
	...
	return breakerDelays.execute(
		async () => {
			const r = await client.listAirportDelays({
...
export async function fetchAirportFlights(
	...
	const dirMap = {
		departure: "FLIGHT_DIRECTION_DEPARTURE",
		arrival: "FLIGHT_DIRECTION_ARRIVAL",
		both: "FLIGHT_DIRECTION_BOTH",
	} as const;
	return breakerFlights.execute(
		async () => {
...
export async function fetchFlightStatus(
	...
	return breakerStatus.execute(
		async () => {
			const r = await client.getFlightStatus({
				flightNumber,
				date: date ?? "",
				origin: origin ?? "",
			});
	...
	return breakerPrices.execute(
		async () => {
			const r = await client.searchFlightPrices({
				...
				cabin: opts.cabin ?? "CABIN_CLASS_ECONOMY",
				nonstopOnly: opts.nonstopOnly ?? false,
...
export async function fetchGoogleFlights(opts: {
	...
	return breakerGoogleFlights.execute(
		async () => {
			...
			return {
				flights: r.flights.map(toDisplayGoogleFlight),
				degraded: r.degraded ?? false,
				error: r.error ?? "",
			};
		},
		{ flights: [], degraded: true, error: "Request failed" },
		{ cacheKey },
	);
}
...
export async function fetchGoogleDates(opts: {
	...
	return breakerGoogleDates.execute(
		async () => {
			const r = await client.searchGoogleDates({
				...
				tripDuration: opts.tripDuration ?? 0,
			...
			return {
				dates: r.dates.map(toDisplayDatePrice),
				degraded: r.degraded ?? false,
				error: r.error ?? "",
			};
		},
		{ dates: [], degraded: true, error: "Request failed" },
		{ cacheKey },
	);
}
