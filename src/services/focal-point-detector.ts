...
class FocalPointDetector {
			...
			for (const entity of context.entities) {
				...
				if (existing) {
					...
					existing.avgConfidence =
						(existing.avgConfidence * (existing.mentionCount - 1) + entity.confidence) /
						existing.mentionCount;
		...
		if (
			...
			mention.topHeadlines.some((h) => {
				...
				return (
					...
					(signals.signalTypes.has("sanctions_pressure") &&
						/sanction|designation|ofac|treasury|embargo|blacklist/.test(lower)) ||
					(signals.signalTypes.has("radiation_anomaly") &&
						/nuclear|radiation|reactor|contamination|radnet/.test(lower)) ||
					(signals.signalTypes.has("active_strike") &&
						/strike|attack|bomb|missile|target|hit/.test(lower))
				);
			})
		) {
			bonus += 5;
	...
	getNewsCorrelationContext(countryCodes: string[]): string | null {
		...
		const relevantFPs = this.lastSummary.focalPoints.filter(
			(fp) =>
				fp.entityType === "country" && countryCodes.includes(fp.entityId) && fp.newsMentions > 0,
		);
