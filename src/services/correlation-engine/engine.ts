...
export class CorrelationEngine {
	...
	async run(ctx: AppContext): Promise<void> {
		...
		try {
			...
			document.dispatchEvent(
				new CustomEvent("wm:correlation-updated", {
	...
	private scoreClusters(clusters: SignalCluster[], adapter: DomainAdapter): ScoredCluster[] {
		return clusters.map((cluster) => {
			...
			const countries = [
				...new Set(cluster.signals.map((s) => s.country).filter(Boolean) as string[]),
			];
		...
		return scored.map((sc) => {
			...
			const match = previous.find((prev) => {
				...
				if (
					sc.centroidLat != null &&
					sc.centroidLon != null &&
					prev.centroidLat != null &&
					prev.centroidLon != null
				) {
					return (
						haversineKm(sc.centroidLat, sc.centroidLon, prev.centroidLat, prev.centroidLon) <=
						halfRadius
					);
				}
				return false;
	...
	private queueLlmAssessments(cards: ConvergenceCard[], adapter: DomainAdapter): void {
		...
		for (const { card, cacheKey } of pending) {
			...
			this.llmInFlight++;
			void this.fetchAssessment(card, adapter, cacheKey).finally(() => {
				this.llmInFlight--;
			});
		}
	}
	...
	private async fetchAssessment(
		...
		try {
			...
			const query =
				`Analyze this ${domainLabels[adapter.domain] ?? adapter.domain} convergence pattern. ` +
			...
			const resp = await this.intelligenceClient.deductSituation({
				query,
				geoContext,
				framework: "",
			});
