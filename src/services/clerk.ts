...
const PUBLISHABLE_KEY = (typeof import.meta !== "undefined" &&
	...
	_tokenInflight = (async () => {
		if (!clerkInstance && PUBLISHABLE_KEY) {
			try {
				...
				/* Clerk load failed, proceed with null */
			}
		}
		const session = clerkInstance?.session;
		if (!session) {
			console.warn(
				`[clerk] getClerkToken: no session (clerkInstance=${!!clerkInstance}, user=${!!clerkInstance?.user})`,
			);
			_tokenInflight = null;
			return null;
		}
