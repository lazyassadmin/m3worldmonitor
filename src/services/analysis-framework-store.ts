...
export function saveImportedFramework(
	fw: Omit<AnalysisFramework, "isBuiltIn" | "createdAt">,
): void {
	const imported = loadFromStorage<AnalysisFramework[]>(LIBRARY_KEY, []);
	...
	if (fw.systemPromptAppend.length > MAX_INSTRUCTIONS_LEN) {
		throw new Error(
			`Instructions exceed the ${MAX_INSTRUCTIONS_LEN}-character limit (${fw.systemPromptAppend.length} chars). Trim and retry.`,
		);
	}
	const existing = loadFrameworkLibrary();
	if (existing.some((f) => f.name === fw.name)) {
		throw new Error(
			`A framework named '${fw.name}' already exists. Rename the existing one first.`,
		);
	}
...
export function renameImportedFramework(id: string, name: string): void {
	...
	saveToStorage(
		LIBRARY_KEY,
		imported.map((f) => (f.id === id ? { ...f, name } : f)),
	);
	_activeCache.clear();
}
...
export function setActiveFrameworkForPanel(
	...
	window.dispatchEvent(
		new CustomEvent(FRAMEWORK_CHANGED_EVENT, {
			detail: { panelId, frameworkId },
		}),
	);
}

export function subscribeFrameworkChange(panelId: AnalysisPanelId, cb: () => void): () => void {
