...
export function initSettingsWindow(): void {
	...
	function render(): void {
		const panelEntries = Object.entries(panelSettings).filter(
			([key]) =>
				(key !== "runtime-config" || isDesktopApp) && (!key.startsWith("cw-") || isProUser()),
		);
			...
			grid.querySelectorAll(".panel-toggle-item").forEach((item) => {
				item.addEventListener("click", () => {
					...
					if (config) {
						if (
							!config.enabled &&
							!isPanelEntitled(panelKey, ALL_PANELS[panelKey] ?? config, isProUser())
						)
							return;
						if (!config.enabled && !isProUser()) {
							const enabledCount = Object.entries(panelSettings).filter(
