...
} from "@/services/breaking-news-alerts";
...
export class DataLoaderManager implements AppModule {
		...
		if (SITE_VARIANT === "full") {
			try {
				...
				if (cached && cached.cii.length > 0) {
					...
					this.ctx.map?.setCIIScores(
						cached.cii.map((s) => ({ code: s.code, score: s.score, level: s.level })),
					);
		...
		if (SITE_VARIANT !== "happy") {
			tasks.push({
				...
				task: runGuarded("techReadiness", () =>
					(this.ctx.panels["tech-readiness"] as TechReadinessPanel)?.refresh(),
				),
			});
		}
	...
	filterItemsByTimeRange(
		...
		return items.filter((item) => {
			const ts =
				item.pubDate instanceof Date ? item.pubDate.getTime() : new Date(item.pubDate).getTime();
	...
	private async loadNewsCategory(
		...
		try {
			...
			const fallbackFeeds = this.selectLimitedFeeds(
				enabledFeeds,
	...
	async loadNews(): Promise<void> {
		...
		if (SITE_VARIANT === "full") {
			const enabledIntelSources = INTEL_SOURCES.filter(
				(f) => !this.ctx.disabledSources.has(f.name),
			);
			...
			if (enabledIntelSources.length === 0) {
				...
				if (staleIntel.length > 0) {
					console.warn(
						`[News] Intel digest missing, serving stale headlines (${staleIntel.length})`,
					);
		...
		try {
			...
			(this.ctx.panels["geo-hubs"] as GeoHubsPanel | undefined)?.setActivities(
				getTopActiveGeoHubs(this.ctx.latestClusters),
			);
			(this.ctx.panels["tech-hubs"] as TechHubsPanel | undefined)?.setActivities(
				getTopActiveHubs(this.ctx.latestClusters),
			);
	...
	async loadMarkets(): Promise<void> {
		try {
			...
			const mapCommodity = (c: MarketData) => ({
				symbol: c.symbol,
				display: c.display,
				price: c.price,
				change: c.change,
			...
			const filterCommodityTape = (data: MarketData[]) =>
				data.filter((item) => item.symbol !== "^VIX" && !energySymbols.has(item.symbol));
			...
			if (commoditiesPanel) {
				try {
					...
					if (!fxResp.unavailable && fxResp.rates?.length) {
						...
						const orderedRates = EUR_FX_ORDER.map((ccy) =>
							fxResp.rates.find((r) => r.pair === `EUR${ccy}`),
						).filter((r): r is NonNullable<typeof r> => r != null);
						commoditiesPanel.updateFxRates(
							orderedRates.map((r) => ({
	...
	async loadDailyMarketBrief(force = false): Promise<void> {
		...
		try {
			...
			this.callPanel(
				"daily-market-brief",
				"showError",
				"Failed to build daily market brief. Retrying later.",
			);
		} finally {
	...
	async loadTechEvents(): Promise<void> {
		...
		try {
			...
			if (SITE_VARIANT === "tech" && this.ctx.searchModal) {
				this.ctx.searchModal.registerSource(
					"techevent",
					mapEvents.map(
						(e: { id: string; title: string; location: string; startDate: string }) => ({
							id: e.id,
							title: e.title,
		...
		tasks.push(
			(async () => {
				try {
					...
					(this.ctx.panels["internet-disruptions"] as InternetDisruptionsPanel)?.setOutages(
						outages,
					);
					fetchTrafficAnomalies()
						.then((r) => {
							this.ctx.map?.setTrafficAnomalies(r.anomalies);
							(this.ctx.panels["internet-disruptions"] as InternetDisruptionsPanel)?.setAnomalies(
								r.anomalies,
							);
						})
						.catch(() => {});
					...
					if (!isInLearningMode()) {
						...
						if (surgeAlerts.length > 0) {
							...
							if (this.shouldShowIntelligenceNotifications())
								this.ctx.signalModal?.show(surgeSignals);
						}
						...
						if (foreignAlerts.length > 0) {
							...
							if (this.shouldShowIntelligenceNotifications())
								this.ctx.signalModal?.show(foreignSignals);
						}
					}
				} catch (error) {
	...
	async hydrateHappyPanelsFromCache(): Promise<void> {
		try {
			...
			this.callPanel(
				...
				"setItems",
				items.filter(
					(item) => scienceSources.includes(item.source) || item.happyCategory === "science-health",
				),
			);
			this.callPanel(
				"spotlight",
	...
	private async loadHappySupplementaryAndRender(): Promise<void> {
		...
		const scienceItems = this.ctx.happyAllItems.filter(
			(item) => scienceSources.includes(item.source) || item.happyCategory === "science-health",
		);
