...
export class SearchManager implements AppModule {
	...
	private setupSearchModal(): void {
		...
		// so mid-session sign-ins get the feature without a page reload.
		{
			this.ctx.searchModal.setOnFlightSearch((callsign) => {
				...
				fetchAircraftPositions({ callsign })
					.then((positions) => {
	...
	updateSearchIndex(): void {
		...
		console.log(
			`[Search] Indexing ${newsItems.length} news items (allNews total: ${this.ctx.allNews.length})`,
		);
		...
		if (this.ctx.latestPredictions.length > 0) {
			this.ctx.searchModal.registerSource(
				"prediction",
				this.ctx.latestPredictions.map((p) => ({
					id: p.title,
					title: p.title,
