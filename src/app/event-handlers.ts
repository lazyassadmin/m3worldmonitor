...
export class EventHandlerManager implements AppModule {
	...
	private performUndo(): void {
		...
		if (!config) return;
		if (!isProUser()) {
			const enabledCount = Object.entries(this.ctx.panelSettings).filter(
				([k, p]) => p.enabled && !k.startsWith("cw-"),
			).length;
	...
	private setupEventListeners(): void {
		...
		this.ctx.container.addEventListener("wm:mcp-configure", ((
			e: CustomEvent<{ panelId: string }>,
		) => {
	...
	private setupMobileMenu(): void {
		...
		sheet?.querySelectorAll<HTMLButtonElement>(".region-sheet-option").forEach((opt) => {
			opt.addEventListener("click", () => {
				...
				const menuRegionLabel = document
					.getElementById("mobileMenuRegion")
					?.querySelector(".mobile-menu-item-label");
				if (menuRegionLabel)
					menuRegionLabel.textContent = opt.querySelector("span")?.textContent ?? "";
	...
	private initDownloadDropdown(): void {
		...
		const renderDropdown = () => {
			const primaryHtml = primary
				.map((b) => `<a class="dl-dd-btn ${b.cls} primary" href="${b.href}">${b.label}</a>`)
				.join("");
			const othersHtml = others
				.map((b) => `<a class="dl-dd-btn ${b.cls}" href="${b.href}">${b.label}</a>`)
				.join("");
        ...
        ${
					others.length
						? `<button class="dl-dd-toggle" id="dlDdToggle">${t("modals.downloadBanner.showAllPlatforms")}</button>
	...
	setupExportPanel(): void {
		...
		this.proGateUnsubscribers.push(
			subscribeAuthState((state) => applyProGate(state.user?.role === "pro")),
		);
	}
		...
		this.ctx.unifiedSettings = new UnifiedSettings({
			...
			toggleSource: (name: string) => {
				...
				if (reenabling && !isProUser()) {
					...
					const currentlyEnabled = allSources.filter(
						(n) => !this.ctx.disabledSources.has(n),
					).length;
			...
			setSourcesEnabled: (names: string[], enabled: boolean) => {
				if (enabled && !isProUser()) {
					...
					const wouldEnable = names.filter(
						(n) => this.ctx.disabledSources.has(n) && allSources.includes(n),
					).length;
