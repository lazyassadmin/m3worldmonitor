				...
				row.addEventListener("click", (e) => {
					...
					if (
						(e.target as HTMLElement).closest(
							"input, button, textarea, select, .live-news-manage-row-remove-x",
						)
					)
						return;
					e.preventDefault();
		...
		if (term) {
			const activeHasMatch = filteredRegions.some((r) => {
				...
				return r.channelIds.some((id) => {
					...
					return (
						ch && (ch.name.toLowerCase().includes(term) || ch.handle?.toLowerCase().includes(term))
					);
				});
			});
			if (!activeHasMatch) {
				const firstMatch = filteredRegions.find((r) =>
					r.channelIds.some((id) => {
						...
						return (
							ch &&
							(ch.name.toLowerCase().includes(term) || ch.handle?.toLowerCase().includes(term))
						);
					}),
				);
		...
		for (const region of filteredRegions) {
			...
			for (const chId of region.channelIds) {
				...
				if (
					...
					!ch.handle?.toLowerCase().includes(term)
				)
					continue;
			...
			if (matchCount === 0 && term) {
				...
				empty.textContent = (
					t("components.liveNews.noResults") ?? 'No channels found matching "{{term}}"'
				).replace("{{term}}", term);
		...
		if (hlsUrl) {
			if (!isHlsUrl(hlsUrl)) {
				if (hlsInput) {
					...
					hlsInput.setAttribute(
						"title",
						t("components.liveNews.invalidHlsUrl") ?? "Enter a valid HLS stream URL (.m3u8)",
					);
				}
				return;
			}
			...
			if (!resolvedName) {
				try {
					const res = await fetch(
						toApiUrl(`/api/youtube/live?videoId=${encodeURIComponent(videoId)}`),
					);
					if (res.ok) {
