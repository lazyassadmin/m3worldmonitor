...
function barChart(
	...
	if (!series.length)
		return `<div style="height:${height}px;display:flex;align-items:center;color:var(--text-dim);font-size:10px">No data</div>`;
...
export class HormuzPanel extends Panel {
	...
	private renderPanel(): void {
		...
		const dateStr = d.updatedDate
			? `<span style="font-size:10px;color:var(--text-dim)">${escapeHtml(d.updatedDate)}</span>`
			: "";
