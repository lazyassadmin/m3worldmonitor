...
export class EnergyComplexPanel extends Panel {
	...
	private render(): void {
		...
		const tapeCoveredSymbols = new Set(
			this.tape.filter((d) => d.price !== null).map((d) => d.symbol),
		);
		...
		const wowClass =
			wowChange === null ? "" : wowChange > 0 ? "change-negative" : "change-positive";
		const crudeSparklineValues = this.crudeWeeks
			.slice()
			.reverse()
			.map((w) => w.stocksMb);
		...
		const ngSparklineValues = this.natGasWeeks
			.slice()
			.reverse()
			.map((w) => w.storBcf);
		...
		const euClass =
			euChange1d === null ? "" : euChange1d > 0 ? "change-positive" : "change-negative";
		const euTrend = this.euGas?.trend ?? "";
		const euSparklineValues = (this.euGas?.history ?? [])
			.slice()
			.reverse()
			.map((h) => h.fillPct);
