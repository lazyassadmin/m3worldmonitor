...
export class SecurityAdvisoriesPanel extends Panel {
		...
		switch (this.activeFilter) {
			case "critical":
				return this.advisories.filter(
					(a) => a.level === "do-not-travel" || a.level === "reconsider",
				);
			case "health":
	...
	private getLevelClass(level?: SecurityAdvisory["level"]): string {
		switch (level) {
			case "do-not-travel":
				return "sa-level-dnt";
			case "reconsider":
				return "sa-level-reconsider";
			case "caution":
				return "sa-level-caution";
			case "normal":
				return "sa-level-normal";
			default:
				return "sa-level-info";
		}
	}
	...
	private getLevelLabel(level?: SecurityAdvisory["level"]): string {
		switch (level) {
			case "do-not-travel":
				return t("components.securityAdvisories.levels.doNotTravel");
			case "reconsider":
				return t("components.securityAdvisories.levels.reconsider");
			case "caution":
				return t("components.securityAdvisories.levels.caution");
			case "normal":
				return t("components.securityAdvisories.levels.normal");
			default:
				return t("components.securityAdvisories.levels.info");
		}
	}
