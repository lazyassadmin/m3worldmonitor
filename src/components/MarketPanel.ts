...
const XAU_CURRENCY_CONFIG: Array<{
	symbol: string;
	label: string;
	flag: string;
	multiply: boolean;
...
export class CommoditiesPanel extends Panel {
	...
	private _buildTabBar(hasFx: boolean, hasXau: boolean): string {
		...
		if (hasXau)
			tabs.push(
				`<button class="panel-tab${this._tab === "xau" ? " active" : ""}" data-tab="xau" style="font-size:11px;padding:3px 10px">XAU/FX</button>`,
			);
		return tabs.length > 1
			? `<div style="display:flex;gap:4px;margin-bottom:8px">${tabs.join("")}</div>`
			: "";
	}
	...
	private _renderXau(): string {
		...
		const fxMap = new Map(
			this._commodityData.filter((d) => d.symbol?.endsWith("=X")).map((d) => [d.symbol!, d]),
		);
		...
		if (rows.length === 0) {
			const placeholders = XAU_CURRENCY_CONFIG.map(
				(cfg) =>
	...
	private _render(): void {
		...
		if (this._tab === "fx" && hasFx) {
          ...
          ${changeStr ? `<div class="commodity-change ${escapeHtml(changeClass)}">${escapeHtml(changeStr)}</div>` : ""}
			...
			this.setContent(
				tabBar +
					`<div class="commodities-grid">${items}</div><div style="margin-top:6px;font-size:9px;color:var(--text-dim)">Source: ECB</div>`,
			);
			return;
		}
		...
		if (validData.length === 0) {
			...
			this.setContent(
				tabBar +
					`<div style="padding:8px;color:var(--text-dim);font-size:12px">${t("common.failedCommodities")}</div>`,
			);
			return;
		}
...
export class AiTokensPanel extends TokenListPanel {
	constructor() {
		super({
			id: "ai-tokens",
			title: "AI Tokens",
			infoTooltip: t("components.aiTokens.infoTooltip"),
		});
	}
}
...
export class OtherTokensPanel extends TokenListPanel {
	constructor() {
		super({
			id: "other-tokens",
			title: "Alt Tokens",
			infoTooltip: t("components.altTokens.infoTooltip"),
		});
	}
}
