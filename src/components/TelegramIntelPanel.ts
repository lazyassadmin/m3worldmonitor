...
export class TelegramIntelPanel extends Panel {
	...
	private renderItems(): void {
		...
		replaceChildren(
			...
			h(
				"div",
				{ className: "telegram-intel-items" },
	...
	private buildItem(item: TelegramItem): HTMLElement {
		...
		return h(
			"div",
			{ className: `telegram-intel-item ${isLive ? "is-live" : ""}` },
			h(
				"div",
				{ className: "telegram-intel-item-header" },
				h(
					"div",
					{ className: "telegram-intel-channel-wrapper" },
					h("span", { className: "telegram-intel-channel" }, item.channelTitle || item.channel),
					isLive
						? h("span", { className: "live-indicator" }, t("components.telegramIntel.live"))
						: null,
				),
				h(
					"div",
					{ className: "telegram-intel-meta" },
			...
			item.mediaUrls && item.mediaUrls.length > 0
				? h(
						"div",
						{ className: "telegram-intel-media-grid" },
			...
			h(
				"div",
				{ className: "telegram-intel-item-actions" },
				h(
					"a",
					{
