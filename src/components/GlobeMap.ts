...
import {
	getGlobeRenderScale,
	resolveGlobePixelRatio,
	resolvePerformanceProfile,
	subscribeGlobeRenderScaleChange,
	getGlobeTexture,
	GLOBE_TEXTURE_URLS,
	subscribeGlobeTextureChange,
	getGlobeVisualPreset,
	subscribeGlobeVisualPresetChange,
	type GlobeRenderScale,
	type GlobePerformanceProfile,
	type GlobeVisualPreset,
...
const SAT_TYPE_EMOJI: Record<string, string> = {
	sar: "\u{1F4E1}",
	optical: "\u{1F4F7}",
	military: "\u{1F396}",
	sigint: "\u{1F4FB}",
...
export class GlobeMap {
	...
	private onLayerChangeCb:
		...
		| null = null;
	private onMapContextMenuCb?: (payload: {
		lat: number;
		lon: number;
		screenX: number;
		screenY: number;
	...
	private async initGlobe(): Promise<void> {
		...
		attribution.innerHTML =
			'© <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a> © <a href="https://www.naturalearthdata.com" target="_blank" rel="noopener">Natural Earth</a>';
		...
		(globe as any)
			...
			.arcColor((d: TradeRouteSegment) => {
				if (d.status === "disrupted")
					return ["rgba(255,32,32,0.1)", "rgba(255,32,32,0.8)", "rgba(255,32,32,0.1)"];
				if (d.status === "high_risk")
					return ["rgba(255,180,0,0.1)", "rgba(255,180,0,0.7)", "rgba(255,180,0,0.1)"];
				if (d.category === "energy")
					return ["rgba(255,140,0,0.05)", "rgba(255,140,0,0.6)", "rgba(255,140,0,0.05)"];
				if (d.category === "container")
					return ["rgba(68,136,255,0.05)", "rgba(68,136,255,0.6)", "rgba(68,136,255,0.05)"];
			...
			.pathColor((d: GlobePath) => {
				...
				if (d.pathType === "orbit") {
					const colors: Record<string, string> = {
						CN: "rgba(255,32,32,0.4)",
						RU: "rgba(255,136,0,0.4)",
						US: "rgba(68,136,255,0.4)",
						EU: "rgba(68,204,68,0.4)",
					};
		...
		(globe as any)
			...
			.polygonCapColor((d: GlobePolygon) => {
				...
				if (d._kind === "conflict")
					return GlobeMap.CONFLICT_CAP[d.intensity!] ?? GlobeMap.CONFLICT_CAP.low;
			...
			.polygonSideColor((d: GlobePolygon) => {
				...
				if (d._kind === "conflict")
					return GlobeMap.CONFLICT_SIDE[d.intensity!] ?? GlobeMap.CONFLICT_SIDE.low;
				if (d._kind === "imageryFootprint") return "rgba(0,0,0,0)";
			...
			.polygonStrokeColor((d: GlobePolygon) => {
				...
				if (d._kind === "conflict")
					return GlobeMap.CONFLICT_STROKE[d.intensity!] ?? GlobeMap.CONFLICT_STROKE.low;
			...
			.polygonAltitude((d: GlobePolygon) => {
				...
				if (d._kind === "conflict")
					return GlobeMap.CONFLICT_ALT[d.intensity!] ?? GlobeMap.CONFLICT_ALT.low;
				return 0.005;
			})
			.polygonLabel((d: GlobePolygon) => {
				if (d._kind === "cii")
					return `<b>${escapeHtml(d.name)}</b><br/>CII: ${d.score}/100 (${escapeHtml(d.level ?? "")})`;
				if (d._kind === "conflict") {
					let label = `<b>${escapeHtml(d.name)}</b>`;
					if (d.parties?.length)
						label += `<br/>Parties: ${d.parties.map((p) => escapeHtml(p)).join(", ")}`;
		...
		getCountriesGeoJson()
			...
			.catch((err) => {
				if (import.meta.env.DEV) console.warn("[GlobeMap] Failed to load countries GeoJSON", err);
			});
	}
	...
	private pulseStyle(duration: string): string {
		return this._pulseEnabled
			? `animation:globe-pulse ${duration} ease-out infinite;`
			: "animation:none;";
	}
	...
	private buildMarkerElement(d: GlobeMarker): HTMLElement {
        ...
        <div style="transform:rotate(${heading}deg);font-size:11px;color:${color};text-shadow:0 0 4px ${color}88;line-height:1;">
          ✈
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:9px;color:${c};text-shadow:0 0 4px ${c}88;font-weight:bold;">⚡</div>`,
			);
			el.title = d.headline;
        ...
        <div style="position:relative;width:9px;height:9px;">
          ...
          <div style="position:absolute;inset:-4px;border-radius:50%;background:${sc}33;${this.pulseStyle("2s")}"></div>
			...
			const sc =
				d.severity === "total" ? "#ff2020" : d.severity === "major" ? "#ff8800" : "#ffcc00";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:12px;color:${sc};text-shadow:0 0 4px ${sc}88;">📡</div>`,
			);
			el.title = `${d.country}: ${d.title}`;
		} else if (d._kind === "trafficAnomaly") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#ffa000;text-shadow:0 0 4px #ffa00088;font-weight:bold;">⚡</div>`,
			);
			el.title = `${d.type || "Traffic Anomaly"}: ${d.locationName}`;
		} else if (d._kind === "ddosHit") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#b400ff;text-shadow:0 0 4px #b400ff88;font-weight:bold;">⚔</div>`,
			);
				...
				d.severity === "critical"
					...
					: d.severity === "high"
						...
						: d.severity === "medium"
							...
							: "#44aaff";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:${sc};text-shadow:0 0 4px ${sc}88;font-weight:bold;">🛡</div>`,
			);
			el.title = `${d.type}: ${d.indicator}`;
		} else if (d._kind === "fire") {
			const intensity = d.brightness > 400 ? "#ff2020" : d.brightness > 330 ? "#ff6600" : "#ffaa00";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:${intensity};text-shadow:0 0 4px ${intensity}88;">🔥</div>`,
			);
			...
			const typeColors: Record<string, string> = {
				...
				protest: "#ffaa00",
				strike: "#44aaff",
				demonstration: "#88ff44",
				civil_unrest: "#ff6600",
			};
			const c = typeColors[d.eventType] ?? "#ffaa00";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:${c};text-shadow:0 0 4px ${c}88;">📢</div>`,
			);
			el.title = d.title;
        ...
        <div style="position:relative;width:${size}px;height:${size}px;">
          <div style="position:absolute;inset:0;border-radius:50%;background:rgba(255,100,0,0.85);border:1.5px solid rgba(255,160,80,0.9);box-shadow:0 0 5px 2px rgba(255,100,0,0.5);"></div>
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:#88bbff;text-shadow:0 0 4px #88bbff88;">👥</div>`,
			);
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:${c};text-shadow:0 0 4px ${c}88;">🌡</div>`,
			);
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#44aaff;text-shadow:0 0 4px #44aaff88;">💻</div>`,
			);
			el.title = d.title;
		} else if (d._kind === "conflictZone") {
			const intColor =
				d.intensity === "high" ? "#ff2020" : d.intensity === "medium" ? "#ff8800" : "#ffcc00";
			el.innerHTML = `
        ...
        "></div>`);
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:#ffd700;text-shadow:0 0 4px #ffd70088;">☢</div>`,
			);
			el.title = `${d.name} (${d.type})`;
		} else if (d._kind === "irradiator") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#ff8800;text-shadow:0 0 3px #ff880088;">⚠</div>`,
			);
			el.title = `${d.city}, ${d.country}`;
		} else if (d._kind === "spaceport") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:#88ddff;text-shadow:0 0 4px #88ddff88;">🚀</div>`,
			);
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="width:${sz}px;height:${sz}px;border-radius:50%;background:${mc}44;border:2px solid ${mc};box-shadow:0 0 6px 2px ${mc}55;"></div>`,
			);
			el.title = `M${d.magnitude.toFixed(1)} — ${d.place}`;
		} else if (d._kind === "economic") {
			const ec =
				d.type === "exchange" ? "#ffd700" : d.type === "central-bank" ? "#4488ff" : "#44cc88";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:${ec};text-shadow:0 0 4px ${ec}88;">💰</div>`,
			);
			el.title = `${d.name} · ${d.country}`;
		} else if (d._kind === "datacenter") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#88aaff;text-shadow:0 0 3px #88aaff88;">🖥</div>`,
			);
			el.title = `${d.name} (${d.owner})`;
		} else if (d._kind === "waterway") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#44aadd;text-shadow:0 0 3px #44aadd88;">⚓</div>`,
			);
			el.title = d.name;
		} else if (d._kind === "mineral") {
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:10px;color:#cc88ff;text-shadow:0 0 3px #cc88ff88;">💎</div>`,
			);
				...
				d.severity === "severe"
					...
					: d.severity === "major"
						...
						: d.severity === "moderate"
							...
							: "#ffee44";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:${sc};text-shadow:0 0 4px ${sc}88;">✈</div>`,
			);
			...
			const tc =
				d.threatLevel === "critical"
					...
					: d.threatLevel === "high"
						? "#ff6600"
						: d.threatLevel === "elevated" || d.threatLevel === "medium"
							? "#ffaa00"
							: "#44aaff";
			el.innerHTML = `
        <div style="position:relative;width:16px;height:16px;">
          ...
          <div style="position:absolute;inset:-5px;border-radius:50%;background:${tc}22;${this.pulseStyle("1.8s")}"></div>
			...
			const sc =
				d.severity === "high" ? "#ff2020" : d.severity === "elevated" ? "#ff8800" : "#44aaff";
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:${sc};text-shadow:0 0 4px ${sc}88;">⛴</div>`,
			);
			el.title = d.name;
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<div style="font-size:11px;color:#00b4ff;text-shadow:0 0 4px #00b4ff88;">&#128752;</div>`,
			);
			...
			el.innerHTML = GlobeMap.wrapHit(
				`<span style="background:${style.color}33;border:1px solid ${style.color}88;border-radius:10px;padding:1px 5px;font-size:12px;">${emoji}</span>`,
			);
			el.title = d.title;
		} else if (d._kind === "webcam-cluster") {
			el.innerHTML = GlobeMap.wrapHit(
				`<span style="background:#00d4ff33;border:1px solid #00d4ff88;border-radius:12px;padding:2px 7px;font-size:11px;font-weight:bold;color:#00d4ff;">${d.count}</span>`,
			);
	...
	private showMarkerTooltip(d: GlobeMarker, anchor: HTMLElement): void {
		...
		if (d._kind === "conflict") {
			html =
				`<span style="color:#ff5050;font-weight:bold;">⚔ ${esc(d.location)}</span>` +
			...
			const sc =
				["", "#88ff44", "#ffdd00", "#ffaa00", "#ff6600", "#ff2020"][d.escalationScore] ?? "#ffaa00";
			html =
				`<span style="color:${sc};font-weight:bold;">🎯 ${esc(d.name)}</span>` +
			...
			const portLine =
				inPort && d.usniHomePort
					? `<br><span style="color:#44aaff;font-size:10px;">🏠 ${esc(d.usniHomePort)}</span>`
					: "";
			html =
				`<span style="font-weight:bold;">⛴ ${esc(d.name)}${hullLine}${deployStatus}</span>` +
				darkWarning +
				`<br><span style="opacity:.7;">${esc(d.typeLabel)}</span>` +
				operatorLine +
				portLine +
				(!inPort && d.usniStrikeGroup
					? `<br><span style="opacity:.85;">⚓ ${esc(d.usniStrikeGroup)}</span>`
					: "") +
				(d.usniRegion
					? `<br><span style="opacity:.6;font-size:10px;">${esc(d.usniRegion)}</span>`
					: "") +
				(d.usniActivityDescription
					? `<br><span style="opacity:.6;font-size:10px;white-space:normal;display:block;max-width:200px;">${esc(d.usniActivityDescription.slice(0, 120))}</span>`
					: "") +
				(d.usniSource
			...
			const cc = GlobeMap.CLUSTER_ACTIVITY_COLORS[d.activityType ?? "unknown"] ?? "#6688aa";
			const actLabel =
				d.activityType && d.activityType !== "unknown"
					? d.activityType.charAt(0).toUpperCase() + d.activityType.slice(1)
					: "";
			html =
				`<span style="color:${cc};font-weight:bold;">⚓ ${esc(d.name)}</span>` +
				`<br><span style="opacity:.7;">${d.vesselCount} vessel${d.vesselCount !== 1 ? "s" : ""}</span>` +
				(actLabel
					? `<br><span style="opacity:.6;font-size:10px;">Activity: ${esc(actLabel)}</span>`
					: "") +
				(d.region ? `<br><span style="opacity:.6;font-size:10px;">${esc(d.region)}</span>` : "");
		} else if (d._kind === "weather") {
			const wc =
				d.severity === "Extreme" ? "#ff0044" : d.severity === "Severe" ? "#ff6600" : "#88aaff";
			html =
				`<span style="color:${wc};font-weight:bold;">⚡ ${esc(d.severity)}</span>` +
			...
			html =
				`<span style="color:${rc};font-weight:bold;">☢ ${esc(d.severity.toUpperCase())}</span>` +
			...
			html =
				`<span style="font-weight:bold;">${esc(d.title.slice(0, 60))}</span>` +
				`<br><span style="opacity:.7;">${esc(d.category)}</span>`;
		} else if (d._kind === "iran") {
			const sc = getIranEventHexColor(d);
			html =
				`<span style="color:${sc};font-weight:bold;">🎯 ${esc(d.title.slice(0, 60))}</span>` +
				`<br><span style="opacity:.7;">${esc(d.category)}${d.location ? " · " + esc(d.location) : ""}</span>`;
		} else if (d._kind === "outage") {
			const sc =
				d.severity === "total" ? "#ff2020" : d.severity === "major" ? "#ff8800" : "#ffcc00";
			html =
				`<span style="color:${sc};font-weight:bold;">📡 ${d.severity.toUpperCase()} Outage</span>` +
			...
			html =
				`<span style="color:#ffa000;font-weight:bold;">⚡ ${esc(d.type || "Traffic Anomaly")}</span>` +
				`<br><span style="opacity:.7;">${esc(d.locationName)}</span>`;
		} else if (d._kind === "ddosHit") {
			html =
				`<span style="color:#b400ff;font-weight:bold;">⚔ DDoS Target: ${esc(d.countryName)}</span>` +
				`<br><span style="opacity:.7;">${d.percentage.toFixed(1)}% of attack traffic</span>`;
		} else if (d._kind === "cyber") {
			const sc =
				d.severity === "critical" ? "#ff0044" : d.severity === "high" ? "#ff4400" : "#ffaa00";
			html =
				`<span style="color:${sc};font-weight:bold;">🛡 ${d.severity.toUpperCase()}</span>` +
				`<br><span style="opacity:.7;">${esc(d.type)}</span>` +
				`<br><span style="opacity:.5;font-size:10px;">${esc(d.indicator.slice(0, 40))}</span>`;
		} else if (d._kind === "fire") {
			html =
				`<span style="color:#ff6600;font-weight:bold;">🔥 Wildfire</span>` +
			...
			const c = typeColors[d.eventType] ?? "#ffaa00";
			html =
				`<span style="color:${c};font-weight:bold;">📢 ${esc(d.eventType)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.country)}</span>` +
				`<br><span style="opacity:.7;white-space:normal;display:block;">${esc(d.title.slice(0, 70))}</span>`;
		} else if (d._kind === "ucdp") {
			html =
				`<span style="color:#ff6400;font-weight:bold;">⚔ ${esc(d.country)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.sideA)} vs ${esc(d.sideB)}</span>` +
				(d.deaths ? `<br><span style="opacity:.5;">Deaths: ${d.deaths}</span>` : "");
		} else if (d._kind === "displacement") {
			html =
				`<span style="color:#88bbff;font-weight:bold;">👥 Displacement</span>` +
			...
			html =
				`<span style="color:${tc};font-weight:bold;">🌡 ${esc(d.type.toUpperCase())}</span>` +
			...
			html =
				`<span style="color:${gc};font-weight:bold;">📡 GPS Jamming</span>` +
				`<br><span style="opacity:.7;">Level: ${esc(d.level)}</span>` +
				`<br><span style="opacity:.5;">Avg satellites visible: ${d.npAvg.toFixed(1)}</span>`;
		} else if (d._kind === "tech") {
			html =
				`<span style="color:#44aaff;font-weight:bold;">💻 ${esc(d.title.slice(0, 50))}</span>` +
				`<br><span style="opacity:.7;">${esc(d.country)}</span>` +
				(d.daysUntil >= 0 ? `<br><span style="opacity:.5;">In ${d.daysUntil} days</span>` : "");
		} else if (d._kind === "conflictZone") {
			const ic =
				d.intensity === "high" ? "#ff3030" : d.intensity === "medium" ? "#ff8800" : "#ffcc00";
			html =
				`<span style="color:${ic};font-weight:bold;">⚔ ${esc(d.name)}</span>` +
				(d.parties.length
					? `<br><span style="opacity:.7;">${d.parties.map(esc).join(", ")}</span>`
					: "") +
				(d.casualties
					? `<br><span style="opacity:.5;">Casualties: ${esc(d.casualties)}</span>`
					: "");
		} else if (d._kind === "milbase") {
			html =
				`<span style="color:#4488ff;font-weight:bold;">🏛 ${esc(d.name)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.type)}${d.country ? " · " + esc(d.country) : ""}</span>`;
		} else if (d._kind === "nuclearSite") {
			const nc =
				d.status === "active" ? "#ffd700" : d.status === "construction" ? "#ff8800" : "#888888";
			html =
				`<span style="color:${nc};font-weight:bold;">☢ ${esc(d.name)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.type)} · ${esc(d.status)}</span>`;
		} else if (d._kind === "irradiator") {
			html =
				`<span style="color:#ff8800;font-weight:bold;">⚠ Gamma Irradiator</span>` +
				`<br><span style="opacity:.7;">${esc(d.city)}, ${esc(d.country)}</span>`;
		} else if (d._kind === "spaceport") {
			const lc =
				d.launches === "High" ? "#88ddff" : d.launches === "Medium" ? "#44aaff" : "#aaaaaa";
			html =
				`<span style="color:${lc};font-weight:bold;">🚀 ${esc(d.name)}</span>` +
			...
			html =
				`<span style="color:${mc};font-weight:bold;">🌍 M${d.magnitude.toFixed(1)}</span>` +
				`<br><span style="opacity:.7;white-space:normal;display:block;">${esc(d.place.slice(0, 70))}</span>`;
		} else if (d._kind === "economic") {
			const ec =
				d.type === "exchange" ? "#ffd700" : d.type === "central-bank" ? "#4488ff" : "#44cc88";
			html =
				`<span style="color:${ec};font-weight:bold;">💰 ${esc(d.name)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.type)} · ${esc(d.country)}</span>` +
				(d.description
					? `<br><span style="opacity:.5;white-space:normal;display:block;">${esc(d.description.slice(0, 70))}</span>`
					: "");
		} else if (d._kind === "datacenter") {
			html =
				`<span style="color:#88aaff;font-weight:bold;">🖥 ${esc(d.name)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.owner)} · ${esc(d.country)}</span>` +
				`<br><span style="opacity:.5;">${esc(d.chipType)}</span>`;
		} else if (d._kind === "waterway") {
			html =
				`<span style="color:#44aadd;font-weight:bold;">⚓ ${esc(d.name)}</span>` +
				(d.description
					? `<br><span style="opacity:.7;white-space:normal;display:block;">${esc(d.description.slice(0, 80))}</span>`
					: "");
		} else if (d._kind === "mineral") {
			const mc2 = d.status === "producing" ? "#cc88ff" : "#8866bb";
			html =
				`<span style="color:${mc2};font-weight:bold;">💎 ${esc(d.mineral)}</span>` +
				...
				d.severity === "severe"
					...
					: d.severity === "major"
						...
						: d.severity === "moderate"
							...
							: "#ffee44";
			html =
				`<span style="color:${sc};font-weight:bold;">✈ ${esc(d.iata)} — ${esc(d.severity.toUpperCase())}</span>` +
				`<br><span style="opacity:.7;">${esc(d.name)}, ${esc(d.country)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.delayType.replace(/_/g, " "))}` +
				(d.avgDelayMinutes > 0 ? ` · avg ${d.avgDelayMinutes}min` : "") +
				`</span>` +
				(d.reason
					? `<br><span style="opacity:.5;white-space:normal;display:block;">${esc(d.reason.slice(0, 70))}</span>`
					: "");
		} else if (d._kind === "notamRing") {
			html =
				`<span style="color:#ff2828;font-weight:bold;">⚠ NOTAM CLOSURE</span>` +
				`<br><span style="opacity:.7;">${esc(d.name)}</span>` +
				(d.reason
					? `<br><span style="opacity:.5;white-space:normal;display:block;">${esc(d.reason.slice(0, 100))}</span>`
					: "");
		} else if (d._kind === "cableAdvisory") {
			const sc = d.severity === "fault" ? "#ff2020" : "#ff8800";
			html =
				`<span style="color:${sc};font-weight:bold;">🔌 ${esc(d.severity.toUpperCase())} — ${esc(d.title.slice(0, 50))}</span>` +
				(d.impact
					? `<br><span style="opacity:.7;white-space:normal;display:block;">${esc(d.impact.slice(0, 70))}</span>`
					: "") +
			...
			html =
				`<span style="color:${sc};font-weight:bold;">⛴ ${esc(typeLabel)}</span>` +
				`<br><span style="opacity:.7;">${esc(d.name)}</span>` +
				...
				d.threatLevel === "critical"
					...
					: d.threatLevel === "high"
						...
						: d.threatLevel === "elevated" || d.threatLevel === "medium"
							...
							: "#44aaff";
			html =
				`<span style="color:${tc};font-weight:bold;">📰 ${esc(d.title.slice(0, 60))}</span>` +
			...
			const operatorName =
				SAT_OPERATOR_NAME[d.country] || getCountryNameByCode(d.country) || d.country;
			...
			html =
				`<span style="color:#00b4ff;font-weight:bold;">&#128752; ${esc(d.satellite)}</span>` +
		...
		const left = Math.max(
			4,
			Math.min(ar.left - cr.left + (anchor.offsetWidth ?? 14) + 6, cr.width - el.offsetWidth - 4),
		);
		const top = Math.max(4, Math.min(ar.top - cr.top - 8, cr.height - el.offsetHeight - 4));
		...
		const hideDelay =
			d._kind === "webcam"
				? 8000
				: d._kind === "webcam-cluster"
					? 12000
					: richKinds.has(d._kind)
						? 6000
						: 3500;
			...
			import("@/services/webcams").then(({ fetchWebcams, getClusterCellSize }) => {
				...
				fetchWebcams(10, {
					...
					closeBtn2.style.cssText =
						"position:absolute;top:4px;right:4px;background:none;border:none;color:#888;cursor:pointer;font-size:14px;line-height:1;padding:2px 4px;";
					...
					for (const webcam of webcams) {
						...
						item.style.cssText =
							"padding:2px 0;cursor:pointer;color:#aaa;border-bottom:1px solid rgba(255,255,255,0.08);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;";
	...
	private createLayerToggles(): void {
      ...
      <div class="toggle-list" style="max-height:32vh;overflow-y:auto;scrollbar-width:thin;">
		...
		el.querySelectorAll(".layer-toggle input").forEach((input) => {
			input.addEventListener("change", () => {
				const layer = (input as HTMLInputElement)
					...
					?.getAttribute("data-layer") as keyof MapLayers | null;
				if (layer) {
		...
		const webcamToggleEl = el.querySelector(
			'.layer-toggle[data-layer="webcams"]',
		) as HTMLElement | null;
		if (webcamToggleEl) {
			...
			const renderModeLabel = (): string =>
				currentMode() === "emoji" ? "&#128247; icon mode" : "&#128512; emoji mode";
			const modeBtn = document.createElement("button");
			modeBtn.style.cssText =
				"background:rgba(0,212,255,0.1);border:1px solid rgba(0,212,255,0.3);color:#00d4ff;font-size:10px;padding:1px 6px;border-radius:3px;cursor:pointer;margin-left:2px;";
	...
	private static readonly CONFLICT_CAP: Record<string, string> = {
		high: "rgba(255,40,40,0.25)",
		medium: "rgba(255,120,0,0.20)",
		low: "rgba(255,200,0,0.15)",
	};
	private static readonly CONFLICT_SIDE: Record<string, string> = {
		high: "rgba(255,40,40,0.12)",
		medium: "rgba(255,120,0,0.08)",
		low: "rgba(255,200,0,0.06)",
	};
	private static readonly CONFLICT_STROKE: Record<string, string> = {
		high: "#ff3030",
		medium: "#ff8800",
		low: "#ffcc00",
	};
	private static readonly CONFLICT_ALT: Record<string, number> = {
		high: 0.006,
		medium: 0.004,
		low: 0.003,
	};

	private getReversedRing(
		zoneId: string,
		countryIso: string,
		ringIdx: number,
		ring: number[][][],
		...
		switch (layer) {
			...
			case "nuclear":
				if (!this.nuclearSiteMarkers.length) {
					this.nuclearSiteMarkers = NUCLEAR_FACILITIES.filter(
		...
		for (const e of events ?? []) {
			if (e.forecastTrack?.length) {
				trackPaths.push({
					...
					points: [[e.lon, e.lat, 0], ...e.forecastTrack.map((p) => [p.lon, p.lat, 0])],
					pathType: "stormTrack",
					status: "active",
				});
			}
	...
	private static readonly LAYER_CHANNELS: Map<
		string,
		{ markers: boolean; arcs: boolean; paths: boolean; polygons: boolean }
	> = new Map([
	...
	public setLayers(layers: MapLayers): void {
		...
		if (prev.satellites !== layers.satellites) {
			...
			if (layers.satellites) {
				...
				if (this.imageryFetchTimer) {
					clearTimeout(this.imageryFetchTimer);
					this.imageryFetchTimer = null;
				}
	...
	public enableLayer(layer: keyof MapLayers): void {
		...
		const toggle = this.layerTogglesEl?.querySelector(
	...
	private enforceLayerLimit(): void {
		...
		const activeCount = Array.from(
			this.layerTogglesEl.querySelectorAll<HTMLInputElement>(".layer-toggle input"),
	...
	private static readonly VIEW_POVS: Record<
		MapView,
	...
	public setOnMapContextMenu(
		cb: (payload: { lat: number; lon: number; screenX: number; screenY: number }) => void,
	): void {
	...
	public getHotspotDynamicScore(_id: string) {
		return undefined;
	}
	public getHotspotLevels() {
	...
	public setCableActivity(advisories: CableAdvisory[], repairShips: RepairShip[]): void {
		...
		this.cableFaultIds = new Set(
			(advisories ?? []).filter((a) => a.severity === "fault").map((a) => a.cableId),
		);
		this.cableDegradedIds = new Set(
			(advisories ?? []).filter((a) => a.severity === "degraded").map((a) => a.cableId),
		);
		this.flushMarkers();
		this.flushPaths();
	}
	...
	private async rebuildSatBeams(positions: SatellitePosition[]): Promise<void> {
		...
		for (const s of positions) {
			...
			const beamTop = new THREE.Vector3()
				.copy(groundCenter)
				.normalize()
				.multiplyScalar(GLOBE_R + BEAM_HEIGHT);
			...
			for (let i = 0; i < RAY_COUNT; i++) {
				...
				allConePositions.push(beamTop.x, beamTop.y, beamTop.z, gi.x, gi.y, gi.z, gn.x, gn.y, gn.z);
	...
	public onHotspotClicked(cb: (h: Hotspot) => void): void {
		this.onHotspotClickCb = cb;
	}
	...
	private wakeGlobe(): void {
		...
		if (!this.isGlobeAnimating) {
			...
			try {
				(this.globe as any).resumeAnimation?.();
			} catch {
				/* best-effort */
			}
		}
	...
	private scheduleIdlePause(): void {
		...
		this.idleTimer = setTimeout(() => {
			...
			try {
				(this.globe as any).pauseAnimation?.();
			} catch {
				/* best-effort */
			}
		}, 3000);
	...
	public destroy(): void {
		...
		this.reversedRingCache.clear();
		this.hideTooltip();
		if (this.satHoverStyle) {
			this.satHoverStyle.remove();
			this.satHoverStyle = null;
		}
		this.controls = null;
		this.controlsAutoRotateBeforePause = null;
		this.controlsDampingBeforePause = null;
