...
export class DefensePatentsPanel extends Panel {
	...
	constructor() {
		super({
			...
			showCount: true,
			infoTooltip:
				"Weekly defense and dual-use patent filings by Raytheon, Lockheed, Huawei, DARPA, and other strategic organizations. Categories: H04B (comms), H01L (semiconductors), F42B (ammunition), G06N (AI), C12N (biotech). Source: USPTO PatentsView.",
		});
	...
	private async fetchPatents(): Promise<void> {
		...
		try {
			const data = await militaryClient.listDefensePatents({
				cpcCode: "",
				assignee: "",
				limit: 100,
			});
	...
	protected render(): void {
		if (this.loading) {
			replaceChildren(
				...
				h(
					"div",
					{ className: "defense-patents-loading" },
		...
		const tabs: [ViewMode, string][] = [
			["all", "All"],
			...Object.entries(CPC_LABELS).map(([code, label]): [ViewMode, string] => [
				code as ViewMode,
				label,
			]),
		];
		...
		replaceChildren(
			...
			h(
				"div",
				{ className: "defense-patents-panel" },
				h(
					"div",
				...
				h(
					"div",
					{ className: "defense-patents-list" },
					...(filtered.length > 0
	...
	private buildRow(p: DefensePatentFiling): HTMLElement {
		...
		return h(
			"div",
			{ className: "defense-patent-row" },
			h("div", { className: "patent-icon", title: p.cpcDesc || p.cpcCode }, icon),
			h(
				"div",
				{ className: "patent-body" },
				h(
					"div",
					{ className: "patent-header" },
