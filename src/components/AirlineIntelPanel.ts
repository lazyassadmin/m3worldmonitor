...
export class AirlineIntelPanel extends Panel {
	...
	constructor() {
		...
		this.liveIndicator.style.cssText =
			"display:none;color:#22c55e;font-size:10px;font-weight:700;margin-left:8px;letter-spacing:0.5px;";
	...
	private handleFlightSearch(): void {
		const origin = (
			(this.content.querySelector("#priceFromInput") as HTMLInputElement)?.value || ""
		)
			.toUpperCase()
			.trim();
		const dest = ((this.content.querySelector("#priceToInput") as HTMLInputElement)?.value || "")
			.toUpperCase()
			.trim();
		const dep = (this.content.querySelector("#priceDepInput") as HTMLInputElement)?.value || "";
		const cabin =
			(this.content.querySelector("#priceCabinSelect") as HTMLSelectElement)?.value || "ECONOMY";
	...
	private handleDatesSearch(): void {
		const origin = (
			(this.content.querySelector("#datesFromInput") as HTMLInputElement)?.value || ""
		)
			.toUpperCase()
			.trim();
		const dest = ((this.content.querySelector("#datesToInput") as HTMLInputElement)?.value || "")
			.toUpperCase()
			.trim();
		...
		const rt =
			(this.content.querySelector("#datesRoundTripCheck") as HTMLInputElement)?.checked ?? true;
		const dur = parseInt(
			(this.content.querySelector("#datesTripDurInput") as HTMLInputElement)?.value || "7",
			10,
		);
		const cabin =
			(this.content.querySelector("#datesCabinSelect") as HTMLSelectElement)?.value || "ECONOMY";
	...
	private handleTrackSearch(): void {
		const q = ((this.content.querySelector("#trackQueryInput") as HTMLInputElement)?.value || "")
			.trim()
			.toUpperCase();
					...
					if (this.trackingQuery) {
						if (/^[A-Z]{2}\d{1,4}$/.test(this.trackingQuery)) {
							...
							this.trackingData = await fetchAircraftPositions({
								icao24: this.trackingQuery.toLowerCase(),
							});
						} else {
				...
				case "prices": {
					if (this.pricesMode === "dates") {
						const r = await fetchGoogleDates({
							origin: this.pricesOrigin,
							destination: this.pricesDest,
							startDate: this.datesStart,
							endDate: this.datesEnd,
							tripDuration: this.datesTripDuration,
							isRoundTrip: this.datesRoundTrip,
						...
						const dep =
							this.pricesDep || new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10);
	...
	private renderTracking(): void {
      ...
      <div class="track-search" style="display:flex;gap:6px;padding:8px 0 6px">
		...
		if (this.trackingFlightData.length) {
			const rows = this.trackingFlightData
				.map((f) => {
					const depStr = f.estimatedDeparture ? `Dep ${fmtTime(f.estimatedDeparture)}` : "";
					const arrStr = f.estimatedArrival ? ` · Arr ${fmtTime(f.estimatedArrival)}` : "";
					const color = STATUS_BADGE[f.status] ?? "#6b7280";
					return `
      ...
      <div class="price-mode-toggle">
        ...
        <div id="priceInlineErr" style="color:#ef4444;font-size:11px;min-height:14px"></div>`;
			...
			if (this.googleFlightsData.length) {
				const cards = this.googleFlightsData
					.map((it) => {
						const stops =
							it.stops === 0 ? t("components.airlineIntel.nonstop") : `${it.stops} stop`;
						const legs = it.legs
							.map(
								(leg) => `
              <div class="gf-leg">
        ...
        <div id="datesInlineErr" style="color:#ef4444;font-size:11px;min-height:14px"></div>`;
			...
			if (this.datesData.length) {
				...
				const rows = sorted
					.map((d) => {
						const cls =
							d.price <= cheapThreshold
								? "dp-cheap"
								: d.price >= expThreshold
									? "dp-expensive"
									: "";
						return `
            <div class="dp-row">
