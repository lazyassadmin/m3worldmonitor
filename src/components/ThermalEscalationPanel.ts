...
export class ThermalEscalationPanel extends Panel {
	...
	constructor() {
		super({
			...
			trackActivity: true,
			infoTooltip:
				"Seeded FIRMS/VIIRS thermal anomaly clusters with baseline comparison, persistence tracking, and strategic context. This panel answers where thermal activity is abnormal and which clusters may signal conflict, industrial disruption, or escalation.",
		});
	...
	private render(): void {
		...
		const footer =
			this.fetchedAt && this.fetchedAt.getTime() > 0
	...
	private renderCard(c: ThermalEscalationCluster): string {
		...
		const persistence =
			c.persistenceHours >= 24
				? `${Math.round(c.persistenceHours / 24)}d`
				: `${Math.round(c.persistenceHours)}h`;
		const frpDisplay =
			c.totalFrp >= 1000 ? `${(c.totalFrp / 1000).toFixed(1)}k` : c.totalFrp.toFixed(0);
		...
		const contextBadge =
			c.context === "conflict_adjacent"
				? '<span class="te-badge te-badge-conflict">conflict-adj</span>'
				: c.context === "energy_adjacent"
					? '<span class="te-badge te-badge-energy">energy-adj</span>'
					: c.context === "industrial"
						? '<span class="te-badge te-badge-industrial">industrial</span>'
						: "";
		const badges = [
			`<span class="te-badge te-badge-${statusClass}">${escapeHtml(c.status)}</span>`,
			contextBadge,
			c.strategicRelevance === "high"
				? '<span class="te-badge te-badge-strategic">strategic</span>'
				: "",
		]
			.filter(Boolean)
			.join("");
