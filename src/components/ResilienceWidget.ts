...
export class ResilienceWidget {
	...
	constructor(countryCode?: string | null) {
		...
		this.unsubscribeAuth = subscribeAuthState((state) => {
			...
			if (
				gateReason === PanelGateReason.NONE &&
				this.currentCountryCode &&
				!this.loading &&
				this.currentData?.countryCode !== this.currentCountryCode
			) {
				void this.refresh();
				return;
			}
	...
	public async refresh(): Promise<void> {
		...
		try {
			...
			this.currentData = null;
			this.errorMessage =
				error instanceof Error ? error.message : "Unable to load resilience score.";
			this.render();
		}
	}
			...
			h(
				...
				h(
					"span",
					{
						...
						title:
							"Composite resilience score derived from economic, infrastructure, energy, social/governance, and health/food domains.",
	...
	private renderBody(gateReason: PanelGateReason): HTMLElement {
		if (!this.currentCountryCode) {
			return h(
				"div",
				{ className: "cdp-card-body" },
				this.makeEmpty("Resilience data loads when a country is selected."),
			);
		}
		...
		if (this.loading) {
			return h(
				"div",
				{ className: "cdp-card-body" },
				this.makeLoading("Loading resilience score…"),
			);
		}
		...
		if (!this.currentData) {
			return h(
				"div",
				{ className: "cdp-card-body" },
				this.makeEmpty("Resilience score unavailable."),
			);
		}
			...
			h(
				...
				this.renderBarBlock(
					...
					h(
						...
						h(
							"span",
							{ className: "resilience-widget__overall-score" },
							String(Math.round(clampScore(data.overallScore))),
						),
						h(
							"span",
							{ className: "resilience-widget__overall-level", style: { color: levelColor } },
							levelLabel,
						),
						h(
							"span",
							{ className: "resilience-widget__overall-trend" },
							`${getResilienceTrendArrow(data.trend)} ${data.trend}`,
						),
					),
				),
			),
			...
			h(
				...
				h(
					"span",
					{
						...
						title: preview
							? "Preview only"
							: "Cronbach alpha and coverage-based confidence signal.",
					},
					formatResilienceConfidence(data),
				),
				h(
					"span",
					{ className: "resilience-widget__delta" },
					formatResilienceChange30d(data.change30d),
				),
			),
		);
	}
	...
	private renderDomainRow(domain: ResilienceDomain, preview = false): HTMLElement {
		...
		return h(
			...
			h(
				"span",
				{ className: "resilience-widget__domain-label" },
				getResilienceDomainLabel(domain.id),
			),
