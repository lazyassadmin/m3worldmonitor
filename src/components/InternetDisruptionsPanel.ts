...
export class InternetDisruptionsPanel extends Panel {
	...
	private buildOutageRow(o: InternetOutage): HTMLElement {
		const severityColor =
			o.severity === "total" ? "#ff2020" : o.severity === "major" ? "#ff8800" : "#ffcc00";
		const badge =
			o.severity === "total" ? "NATIONWIDE" : o.severity === "major" ? "REGIONAL" : "PARTIAL";
			...
			d.protocol.length > 0
				? h(
						...
						h(
							...
							t("components.internetDisruptions.byProtocol"),
						),
			...
			d.vector.length > 0
				? h(
						...
						h(
							"div",
							{ className: "id-section-title" },
							t("components.internetDisruptions.byVector"),
						),
			...
			d.topTargetLocations.length > 0
				? h(
						...
						h(
							"div",
							{ className: "id-section-title" },
							t("components.internetDisruptions.topTargets"),
						),
						...d.topTargetLocations
							.slice(0, 8)
							.map((loc) =>
	...
	private buildAnomalyRow(a: TrafficAnomaly): HTMLElement {
		...
		return h(
			...
			a.startDate ? h("div", { className: "id-row-meta" }, this.formatEpoch(a.startDate)) : false,
		);
	}
