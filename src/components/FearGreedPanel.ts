...
function getDivergenceWarnings(d: FearGreedData): string[] {
	...
	if (cnn > 0 && Math.abs(comp - cnn) > 20)
		warnings.push(
			`CNN F&G ${Math.round(cnn)} diverges ${Math.abs(Math.round(comp - cnn))}pts from composite — sentiment/structural disconnect`,
		);
...
export class FearGreedPanel extends Panel {
	...
	private async refreshFromRpc(): Promise<boolean> {
		try {
			...
			if (!this.data)
				this.showError(
					e instanceof Error ? e.message : t("common.failedToLoad"),
					() => void this.fetchData(),
				);
			return false;
		}
