...
const QUICK_ACTIONS: QuickAction[] = [
	{ label: "Situation", icon: "🌍", query: "Summarize today's geopolitical situation" },
	{
		label: "Markets",
		icon: "📈",
		query: "Key market moves, macro signals, and commodity moves today",
	},
...
export class ChatAnalystPanel extends Panel {
	...
	private buildUI(): void {
		...
		const sendBtn = h(
			"button",
			{ className: "chat-analyst-send", dataset: { action: "send" } },
			"▶",
		);
		const clearBtn = h(
			"button",
			{ className: "chat-analyst-clear", dataset: { action: "clear" } },
			"✕",
		);
		const exportBtn = h(
			"button",
			{ className: "chat-analyst-export", dataset: { action: "export" } },
			"↓",
		);
	...
	private renderActionChip(bubble: HTMLElement, action: ActionEvent): void {
		...
		chip.addEventListener("click", () => {
			this.element.dispatchEvent(
				new CustomEvent("wm:open-widget-creator", {
					bubbles: true,
	...
	async send(query: string): Promise<void> {
		...
		try {
			...
			const finished = await this.readStream(reader, bubble, streamingBody, (text) => {
			...
			for (const line of lines) {
				...
				try {
					...
					if (payload.error) {
						this.finalizeStreamingBubble(
							bodyEl,
							"⚠ Analyst unavailable. Try again shortly.",
							false,
						);
						return "error";
					}
