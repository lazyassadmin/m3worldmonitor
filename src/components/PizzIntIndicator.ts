...
export class PizzIntIndicator {
	...
	constructor() {
		const panel = h(
			"div",
			{ className: "pizzint-panel hidden" },
			h(
				"div",
				{ className: "pizzint-header" },
	...
	private render(): void {
		...
		defconEl.textContent = t("components.pizzint.defcon", {
			level: String(this.status.defconLevel),
		});
		...
		replaceChildren(
			...
			...this.status.locations.map((loc) =>
				h(
					"div",
					{ className: "pizzint-location" },
					h("span", { className: "pizzint-location-name" }, loc.name),
					h(
						"span",
						{ className: `pizzint-location-status ${this.getStatusClass(loc)}` },
						this.getStatusLabel(loc),
					),
				),
			),
		);
