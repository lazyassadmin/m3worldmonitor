...
function parseIntent(raw: string): Intent {
	...
	if (
		/^(FLIGHT|FLT)\s+[A-Z]{2,3}\d{1,4}/.test(q) ||
		/[A-Z]{2,3}\d{1,4}\s+(STATUS|FLIGHT|FLT)/.test(q) ||
		/^STATUS\s+[A-Z]{2,3}\d{1,4}/.test(q)
	) {
	...
	if (words.some((w) => /^(FLY|FLIGHTS?|FARES?|BOOK)$/.test(w))) {
		const nonKeywords = words.filter(
			(w) =>
				!/^(FLY|FLIGHTS?|FARES?|BOOK|TO|FROM|ON|FOR)$/.test(w) && !/^\d{4}-\d{2}-\d{2}$/.test(w),
		);
		...
		if (priceAirports.length >= 2) {
			...
			return {
				type: "PRICE_WATCH",
				origin: priceAirports[0]!,
				destination: priceAirports[1]!,
				date,
			};
		}
	}
...
async function executeIntent(intent: Intent): Promise<CommandResult> {
	...
	if (intent.type === "FLIGHT_STATUS") {
		...
		if (!flights.length)
			return {
				html: `<div class="cmd-empty">No results for ${escapeHtml(intent.flightNumber)}.</div>`,
			};
		const f = flights[0]!;
	...
	if (intent.type === "PRICE_WATCH") {
		...
		const dateChips = ([1, 3, 7, 14, 30] as const)
			.map((days) => {
		...
		const gfResult = await fetchGoogleFlights({
			origin: intent.origin,
			destination: intent.destination,
			departureDate: date,
		});
		if (gfResult.flights.length) {
			const sorted = [...gfResult.flights].sort((a, b) =>
				a.stops !== b.stops ? a.stops - b.stops : a.price - b.price,
			);
			const rows = sorted
				.slice(0, 5)
				.map((f) => {
					const leg = f.legs[0];
					const carrier = leg
						? `${escapeHtml(leg.airlineCode)} ${escapeHtml(leg.flightNumber)}`
						: "";
					...
					const rowUrl = sanitizeUrl(
						`https://www.google.com/travel/flights/search?q=Flights+from+${encodeURIComponent(intent.origin)}+to+${encodeURIComponent(intent.destination)}+on+${encodeURIComponent(date)}`,
					);
		...
		const rows = [...quotes]
			.sort((a, b) => (a.stops !== b.stops ? a.stops - b.stops : a.priceAmount - b.priceAmount))
			.slice(0, 5)
			.map((q) => {
	...
	if (intent.type === "NEWS_BRIEF") {
		...
		const rows = items
			.map(
				(n) =>
					`<div class="cmd-news-item"><a href="${sanitizeUrl(n.url)}" target="_blank" rel="noopener">${escapeHtml(n.title)}</a></div>`,
			)
			.join("");
	...
	if (intent.type === "TRACK") {
		return {
			html: `<div class="cmd-section">🛰️ Tracking <strong>${escapeHtml(intent.callsign ?? intent.icao24 ?? "?")}</strong> — open Tracking tab in Airline Intel panel for live positions.</div>`,
		};
	}
...
export class AviationCommandBar {
	...
	open(): void {
      ...
      <div id="aviation-cmd-box">
		...
		this.overlay
			.querySelector("#aviation-cmd-close")
	...
	private renderHistory(): void {
		...
		el.innerHTML = `<div style="font-size:11px;color:#6b7280;margin-top:4px">${h
			.map(
				(c) =>
