...
function cpiYoY(obs: { date: string; value: number }[]): {
	value: number | null;
	prior: number | null;
	date: string;
} {
...
function tileHtml(tile: MacroTile): string {
	...
	const color =
		delta !== null
			? deltaColor(delta, tile.lowerIsBetter, tile.neutral ?? false)
			: "var(--text-dim)";
...
function euAvg(
	...
	const priorAvg =
		priorValues.length === values.length
		...
		try {
			...
			this._usTiles = [
				...
				{ id: "unrate", label: "Unemployment", ...unrate, lowerIsBetter: true, format: pctFmt },
				{
					...
					lowerIsBetter: false,
					format: gdpFmt,
					deltaFormat: (v) => `${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}B`,
				},
				{
					id: "fed",
