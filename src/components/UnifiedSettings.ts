...
export class UnifiedSettings {
	...
	private renderUpgradeSection(): string {
		if (isEntitled()) {
			...
			const statusColor =
				sub?.status === "active" ? "#22c55e" : sub?.status === "on_hold" ? "#eab308" : "#ef4444";
	...
	private renderPanelsTab(): void {
		...
		container.innerHTML = entries
			.map(([key, panel]) => {
				...
				const displayName = this.config.getLocalizedPanelName(
					key,
					getEffectivePanelConfig(key, SITE_VARIANT).name ?? panel.name,
				);
				return `
	...
	private clonePanelSettings(
		source: Record<string, PanelConfig> = this.config.getPanelSettings(),
		...
		for (const key of Object.keys(ALL_PANELS)) {
			if (!(key in cloned)) {
				cloned[key] = {
					...
					enabled: variantDefaults.has(key),
				};
			}
		}
		return cloned;
	...
	private hasPendingPanelChanges(): boolean {
		...
		return Object.entries(this.draftPanelSettings).some(
			([key, panel]) => savedSettings[key]?.enabled !== panel.enabled,
		);
	}
	...
	private toggleDraftPanel(key: string): void {
		...
		if (!panel.enabled && !isProUser()) {
			const enabledCount = Object.entries(this.draftPanelSettings).filter(
	...
	private savePanelChanges(): void {
		...
		this.config.savePanelSettings(
			Object.fromEntries(Object.entries(this.draftPanelSettings).map(([k, v]) => [k, { ...v }])),
		);
		this.draftPanelSettings = this.clonePanelSettings();
