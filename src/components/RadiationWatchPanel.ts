...
export class RadiationWatchPanel extends Panel {
	...
	constructor() {
		super({
			...
			trackActivity: true,
			infoTooltip:
				"Seeded EPA RadNet and Safecast readings with anomaly scoring and source-confidence synthesis. This panel answers what is normal, what is elevated, and which anomalies are confirmed versus tentative.",
		});
		...
		const rows = this.observations
			.map((obs) => {
				...
				const flags = [
					...
					obs.corroborated
						? '<span class="radiation-badge radiation-flag-confirmed">confirmed</span>'
						: "",
					obs.conflictingSources
						? '<span class="radiation-badge radiation-flag-conflict">conflict</span>'
						: "",
					obs.convertedFromCpm
						? '<span class="radiation-badge radiation-flag-converted">CPM-derived</span>'
						: "",
