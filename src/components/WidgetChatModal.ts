...
export function openWidgetChatModal(options: WidgetChatOptions): void {
	...
	const proBadgeHtml = isPro
		? `<span class="widget-pro-badge">${escapeHtml(t("widgets.proBadge"))}</span>`
		: "";
  ...
  `;
	...
	if (isModify && options.existingSpec) {
		...
		if (currentSessionHtml) {
			renderPreviewHtml(
				previewEl,
				currentSessionHtml,
				options.existingSpec.title,
				t("widgets.phaseReadyToPrompt"),
				t("widgets.modifyHint"),
				isPro,
			);
		}
	...
	overlay.addEventListener("click", (e) => {
		if (e.target === overlay) closeWidgetChatModal();
	});

	const escHandler = (e: KeyboardEvent) => {
		if (e.key === "Escape") closeWidgetChatModal();
	};
	...
	async function runPreflight(): Promise<void> {
		...
		try {
			...
			try {
				payload = (await res.json()) as WidgetAgentHealth;
			} catch {
				/* ignore */
			}
			...
			setFooterStatus(
				footerStatusEl,
				currentSessionHtml ? t("widgets.modifyHint") : t("widgets.readyToGenerate"),
			);
			syncComposerState();
		} catch {
			preflightReady = false;
	...
	const submit = async () => {
		...
		const body = JSON.stringify({
			...
			conversationHistory: sessionHistory.map((m) => ({
				role: m.role,
		...
		try {
			...
			radarEl.innerHTML =
				'<span class="panel-loading-radar"><span class="panel-radar-sweep"></span><span class="panel-radar-dot"></span></span>';
				...
				for (const line of lines) {
					...
					if (event.type === "tool_call") {
						...
						renderPreviewHtml(
							...
							resultHtml,
							resultTitle,
							t("widgets.phaseComposing"),
							t("widgets.previewComposingCopy"),
							isPro,
						);
						...
						renderPreviewHtml(
							...
							resultHtml,
							resultTitle,
							t("widgets.phaseComplete"),
							t("widgets.previewReadyCopy"),
							isPro,
						);
						setFooterStatus(footerStatusEl, t("widgets.readyToApply", { title: resultTitle }));
						actionBtn.textContent = isModify
							? t("widgets.applyChanges")
							: t("widgets.addToDashboard");
						requestInFlight = false;
						syncComposerState();
...
function resolvePreflightMessage(
	...
	if (status === 503 && payload?.proKeyConfigured === false)
		return t("widgets.preflightProUnavailable");
...
function getPhaseLabel(phase: PreviewPhase): string {
	switch (phase) {
		case "checking":
			return t("widgets.phaseChecking");
		case "ready_to_prompt":
			return t("widgets.phaseReadyToPrompt");
		case "fetching":
			return t("widgets.phaseFetching");
		case "composing":
			return t("widgets.phaseComposing");
		case "complete":
			return t("widgets.phaseComplete");
		case "error":
			return t("widgets.phaseError");
	}
}
...
function getPreviewHeading(phase: PreviewPhase): string {
	switch (phase) {
		case "checking":
			return t("widgets.previewCheckingHeading");
		case "ready_to_prompt":
			return t("widgets.previewReadyHeading");
		case "fetching":
			return t("widgets.previewFetchingHeading");
		case "composing":
			return t("widgets.previewComposingHeading");
		case "complete":
			return t("widgets.previewReadyHeading");
		case "error":
			return t("widgets.previewErrorHeading");
	}
}
...
function getPreviewCopy(phase: PreviewPhase): string {
	switch (phase) {
		case "checking":
			return t("widgets.previewCheckingCopy");
		case "ready_to_prompt":
			return t("widgets.previewReadyCopy");
		case "fetching":
			return t("widgets.previewFetchingCopy");
		case "composing":
			return t("widgets.previewComposingCopy");
		case "complete":
			return t("widgets.previewReadyCopy");
		case "error":
			return t("widgets.previewErrorCopy");
	}
}
