...
function deltaToColSpan(startSpan: number, deltaX: number, maxSpan = 3): number {
	const spanDelta =
		deltaX > 0 ? Math.floor(deltaX / COL_RESIZE_STEP_PX) : Math.ceil(deltaX / COL_RESIZE_STEP_PX);
...
function deltaToRowSpan(startSpan: number, deltaY: number): number {
	const spanDelta =
		deltaY > 0 ? Math.floor(deltaY / ROW_RESIZE_STEP_PX) : Math.ceil(deltaY / ROW_RESIZE_STEP_PX);
...
export class Panel {
	...
	public showGatedCta(reason: PanelGateReason, onAction: () => void): void {
		...
		replaceChildren(
			this.content,
			h("div", { className: "panel-locked-state" }, iconEl, descEl, ctaBtn),
		);
	}
