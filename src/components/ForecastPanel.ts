	...
	try {
		...
		return arr.filter(
			(v): v is SimulationTheater =>
				v &&
				typeof v === "object" &&
				typeof v.theaterId === "string" &&
				typeof v.theaterLabel === "string",
		);
	} catch {
		return [];
	}
...
export class ForecastPanel extends Panel {
	...
	constructor() {
		...
		this.content.addEventListener("click", (e) => {
			...
			if (toggle) {
				...
				const detail = panelId
					? (item?.querySelector(`[data-fc-panel="${panelId}"]`) as HTMLElement | null)
					: null;
	...
	private renderNexus(): string {
		...
		const detail = this.expandedTheaterId
			? this.renderTheaterDetail(
					this.theaters.find((t) => t.theaterId === this.expandedTheaterId) ?? null,
				)
			: "";
		return `
	...
	private renderTheaterDetail(t: SimulationTheater | null): string {
		...
		const pathsHtml = t.topPaths
			.map((p) => {
				const pctColor =
					p.confidence >= 0.65 ? "#3fb950" : p.confidence >= 0.45 ? "#d29922" : "#e05252";
				const actors = p.keyActors
					.map((a) => `<span class="fc-actor-chip">${escapeHtml(a)}</span>`)
					.join("");
				const typeTag = p.pathId
					? `<span class="fc-path-type fc-path-type-${escapeHtml(p.pathId)}">${escapeHtml(PATH_ID_LABELS[p.pathId] ?? p.pathId)}</span>`
					: "";
      ...
      `;
		...
		const reactions = t.dominantReactions
			.map((r) => `<div class="fc-footer-item fc-react-item">${escapeHtml(r)}</div>`)
			.join("");
		const stabilizers = t.stabilizers
			.map((s) => `<div class="fc-footer-item fc-stab-item">${escapeHtml(s)}</div>`)
			.join("");
		const invalidators = t.invalidators
			.map((s) => `<div class="fc-footer-item fc-inval-item">${escapeHtml(s)}</div>`)
			.join("");
	...
	private renderProbRow(f: Forecast): string {
		...
		const trendText =
			f.trend === "rising" ? "↑ rising" : f.trend === "falling" ? "↓ falling" : "→ stable";
		const trendColor =
			f.trend === "rising" ? "#3fb950" : f.trend === "falling" ? "#e05252" : "#7d8590";
	...
	private renderSimBar(f: Forecast): string {
		...
		if (demoted) {
			...
			barColor = conf >= 0.7 ? "#3fb950" : "#d29922";
			labelText = conf < 0.7 ? `AI signal (moderate) · +${adjPct}%` : `AI signal · +${adjPct}%`;
		} else {
			barColor = "#ea580c";
	...
	private renderDetailBody(f: Forecast): string {
      ...
      `);
		...
		const chips = [
			f.calibration?.marketTitle
				? `Market: ${f.calibration.marketTitle} (${Math.round((f.calibration.marketPrice || 0) * 100)}%)`
				: "",
			typeof f.priorProbability === "number"
				? `Prior: ${Math.round(f.priorProbability * 100)}%`
				: "",
			f.cascades?.length ? `Cascades: ${f.cascades.length}` : "",
		].filter(Boolean);
		if (chips.length > 0) {
			sections.push(
				`<div class="fc-section"><div class="fc-section-title">Context</div><div class="fc-chip-row">${chips.map((c) => `<span class="fc-chip">${escapeHtml(c)}</span>`).join("")}</div></div>`,
			);
		}
	...
	private renderEvidence(items: Array<{ summary?: string; weight?: number }> | undefined): string {
		if (!items || items.length === 0) return "";
		return `<div class="fc-list-block">${items
			.map((item) => {
				const suffix =
					typeof item.weight === "number" ? ` (${Math.round(item.weight * 100)}%)` : "";
		...
		return `<div class="fc-list-block">${items
			.map((actor) => {
				const chips = [
					...
					typeof actor.influenceScore === "number"
						? `Influence ${Math.round(actor.influenceScore * 100)}%`
						: "",
				]
					.filter(Boolean)
					.map((chip) => `<span class="fc-chip">${escapeHtml(chip)}</span>`)
					.join("");
				return `
	...
	private renderBranches(
		...
		return `<div class="fc-list-block">${items
			.map((branch) => {
				const projected =
					typeof branch.projectedProbability === "number"
				...
				const rounds = (branch.rounds || [])
					...
					.map((round) => {
						const copy = [
							(round.developments || []).slice(0, 2).join(" "),
							(round.actorMoves || []).slice(0, 1).join(" "),
						]
							.filter(Boolean)
							.join(" ");
