...
export class CorrelationPanel extends Panel {
	...
	private buildCard(card: ConvergenceCard): HTMLElement {
		...
		return h(
			"div",
			{
				...
				style:
					"border:1px solid rgba(255,255,255,0.08);border-radius:6px;margin-bottom:4px;background:rgba(255,255,255,0.02);",
			},
			header,
			detailEl,
		);
	...
	private populateDetail(el: HTMLElement, card: ConvergenceCard): void {
		const signalList = card.signals.slice(0, 10).map((s) =>
			h(
				"div",
				{ style: "padding:2px 0;display:flex;gap:6px;align-items:baseline;" },
				h(
					"span",
					{
						style:
							"font-size:8px;padding:1px 4px;border-radius:3px;background:rgba(255,255,255,0.1);white-space:nowrap;",
					},
					s.type,
				),
		...
		const children: HTMLElement[] = [h("div", { style: "padding:6px 0;" }, ...signalList)];
		...
		if (card.assessment) {
			children.push(
				h(
					...
					{
						style:
							"padding:6px 8px;margin:4px 0;border-radius:4px;background:rgba(100,150,255,0.08);border-left:2px solid rgba(100,150,255,0.3);font-size:10px;line-height:1.4;",
					},
					card.assessment,
				),
			);
		...
		if (card.location) {
			const mapBtn = h(
				...
				{
					style:
						"margin-top:4px;padding:3px 8px;font-size:9px;border:1px solid rgba(255,255,255,0.15);border-radius:3px;background:transparent;color:inherit;cursor:pointer;",
				},
				"View on map",
			);
