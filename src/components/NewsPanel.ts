...
export class NewsPanel extends Panel {
	...
	private saveSortMode(): void {
		try {
			localStorage.setItem(this.sortStorageKey, this.sortMode);
		} catch {
			/* storage full */
		}
	}
		...
		if (this.lastRawClusters && this.windowedList) {
			const found = this.windowedList.scrollToItem(
				(p: { cluster: ClusteredEvent }) =>
					p.cluster.primaryLink === link ||
					p.cluster.allItems.some((i: { link: string }) => i.link === link),
			);
			if (found) {
				setTimeout(() => {
					const el =
						this.content.querySelector(`[data-news-id="${CSS.escape(link)}"]`) ??
