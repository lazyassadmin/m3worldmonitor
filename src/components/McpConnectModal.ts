...
export function openMcpConnectModal(options: McpConnectOptions): void {
	...
	const presetsHtml = MCP_PRESETS.map(
		(p) => `
  ...
  `;
	...
	let activeApiKeyHeader =
		matchingPreset?.apiKeyHeader ?? (initialSimpleMode ? DEFAULT_API_KEY_HEADER : "");
	...
	function showAdvancedMode(prefillFromKey = true): void {
		if (prefillFromKey && activeApiKeyHeader && apiKeyInput.value.trim()) {
			authInput.value = _headersToLine(
				buildHeadersFromTemplate(activeApiKeyHeader, apiKeyInput.value.trim()),
			);
		}
	...
	function renderTools(list: McpToolDef[]): void {
      ...
      `;
			item.addEventListener("click", () => {
				toolsList
					.querySelectorAll(".mcp-tool-item")
	...
	addBtn.addEventListener("click", () => {
		...
		const spec: McpPanelSpec = {
			...
			toolArgs,
			refreshIntervalMs:
				Math.max(MIN_MCP_REFRESH_S, parseInt(refreshInput.value, 10) || MIN_MCP_REFRESH_S) * 1000,
