...
export class WorldClockPanel extends Panel {
	...
	private setupDragHandlers(): void {
		...
		document.addEventListener("mousemove", (e: MouseEvent) => {
			...
			for (const row of rows) {
				...
				if (e.clientY >= rect.top && e.clientY <= rect.bottom) {
					row.classList.add(
						e.clientY < rect.top + rect.height / 2 ? "wc-drag-over-above" : "wc-drag-over-below",
					);
				}
			}
		});
		...
		document.addEventListener("mouseup", (e: MouseEvent) => {
			...
			const rows = content.querySelectorAll(".wc-row[data-city-id]");
			rows.forEach((r) =>
				r.classList.remove("wc-dragging", "wc-drag-over-above", "wc-drag-over-below"),
			);
	...
	private renderClocks(): void {
		...
		if (sorted.length === 0) {
			this.setContent(
				'<div class="wc-empty">No cities selected. Click \u2699 to add cities.</div>',
			);
			return;
		}
