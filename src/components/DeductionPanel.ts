...
export class DeductionPanel extends Panel {
	...
	constructor(getLatestNews?: () => NewsItem[]) {
		super({
			...
			infoTooltip:
				"Use AI intelligence to deduct the timeline and impact of a hypothetical or current event.",
		});
		...
		const formRow = h("div", { className: "deduction-form-row" }, this.geoInputEl, this.submitBtn);
		...
		const container = h(
			"div",
			{ className: "deduction-panel-content" },
			this.formEl,
