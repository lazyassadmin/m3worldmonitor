...
export class MapContainer {
	...
	constructor(container: HTMLElement, initialState: MapContainerState, preferGlobe = false) {
		...
		if (!this.useDeckGL && this.initialState.layers?.resilienceScore) {
			this.initialState = {
				...this.initialState,
				layers: { ...this.initialState.layers, resilienceScore: false },
			};
		}
	...
	public setView(view: MapView, zoom?: number): void {
		...
		if (this.useDeckGL) {
			this.deckGLMap?.setView(view as DeckMapView, zoom);
		} else {
	...
	public setLayers(layers: MapLayers): void {
		const sanitized =
			!this.useDeckGL && layers.resilienceScore ? { ...layers, resilienceScore: false } : layers;
		if (this.useGlobe) {
