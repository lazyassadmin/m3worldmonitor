...
export class NationalDebtPanel extends Panel {
	...
	constructor() {
		super({
			...
			showCount: true,
			infoTooltip:
				"Live national debt estimates for 150+ countries. Data anchored at 2024-01-01 and accruing using IMF deficit projections.",
		});
	...
	private applyFilters(): void {
		...
		const base = q
			? this.entries.filter(
					(e) =>
						e.iso3.toLowerCase().includes(q) || getCountryName(e.iso3).toLowerCase().includes(q),
				)
			: this.entries;
      ...
      <div class="debt-panel-container">
        ...
        <div class="debt-list">
          ${this.filteredEntries
						...
						.map((entry, idx) => this.renderRow(entry, idx + 1))
						.join("")}
        </div>
        ${
	...
	private renderRow(entry: NationalDebtEntry, rank: number): string {
		...
		const ratioStr =
			Number.isFinite(entry.debtToGdp) && entry.debtToGdp > 0
				? `${entry.debtToGdp.toFixed(1)}%`
				: "—";
		const growthStr =
			Number.isFinite(entry.annualGrowth) && entry.annualGrowth !== 0
				? `${entry.annualGrowth > 0 ? "+" : ""}${entry.annualGrowth.toFixed(1)}%`
				: "—";
		const growthClass =
			entry.annualGrowth > 5 ? "debt-growth-high" : entry.annualGrowth > 0 ? "debt-growth-mid" : "";
