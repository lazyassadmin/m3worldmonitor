...
export class SearchModal {
	...
	private createModal(): void {
      ...
      `;
			...
			this.overlay
				.querySelector(".search-sheet-cancel")
	...
	private renderEmpty(): void {
		...
		const tips: { icon: string; key: string; exampleKey: string }[] = [
			...
			{
				icon: "\u{1F6E1}\uFE0F",
				key: "commands.tips.layers",
				exampleKey: "commands.tips.layersExample",
			},
			{ icon: "\u23F1\uFE0F", key: "commands.tips.time", exampleKey: "commands.tips.timeExample" },
			{
				icon: "\u2699\uFE0F",
				key: "commands.tips.settings",
				exampleKey: "commands.tips.settingsExample",
			},
		];
		if (this.sources.some((s) => s.type === "flight")) {
			tips.push({
				icon: "\u2708\uFE0F",
				key: "commands.tips.flight",
				exampleKey: "commands.tips.flightExample",
			});
		}
