...
function countdown(dateStr: string): string {
	...
	if (days < 0)
		return Math.abs(days) < 14
			? `${Math.abs(days)}d ago`
			: `${Math.round(Math.abs(days) / 7)}w ago`;
...
export class EconomicCalendarPanel extends Panel {
	...
	public async fetchData(): Promise<boolean> {
		...
		try {
			...
			if (!this._hasData)
				this.showError("Failed to load economic calendar.", () => void this.fetchData());
			return false;
		}
	}
	...
	private _render(): void {
		...
		const emptyMsgText =
			this._region === "all" ? "No upcoming economic events" : "No events for selected region";
		const emptyMsg =
			filtered.length === 0
