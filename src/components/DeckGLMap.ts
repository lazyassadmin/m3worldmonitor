...
import {
	fetchMilitaryBases,
	type MilitaryBaseCluster as ServerBaseCluster,
} from "@/services/military-bases";
...
import type {
	TrafficAnomaly as ProtoTrafficAnomaly,
	DdosLocationHit,
...
import {
	...
	TRADE_ROUTES as TRADE_ROUTES_LIST,
...
const MARKER_ICONS = {
	...
	square:
		...
		btoa(
			`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><rect x="2" y="2" width="28" height="28" rx="3" fill="white"/></svg>`,
		),
	...
	plane:
		"data:image/svg+xml;base64," +
		btoa(
			`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path d="M16 2 L17.5 10 L17 12 L27 17 L27 19 L17 16 L17 24 L20 26.5 L20 28 L16 27 L12 28 L12 26.5 L15 24 L15 16 L5 19 L5 17 L15 12 L14.5 10 Z" fill="white"/></svg>`,
		),
};
...
export class DeckGLMap {
	...
	private speciesRecoveryZones: Array<
		SpeciesRecovery & { recoveryZone: { name: string; lat: number; lon: number } }
	> = [];
	...
	private applyRadarLayer(): void {
		...
		try {
			const existing = this.maplibreMap.getSource("weather-radar") as
			...
			const beforeId = this.maplibreMap.getLayer("country-interactive")
				? "country-interactive"
				: undefined;
			...
			console.warn("[DeckGLMap] radar layer apply failed:", (err as Error)?.message);
		}
	}
	...
	private initMapLibre(): void {
		...
		const recreateWithFallback = () => {
			...
			this.usedFallbackStyle = true;
			const fallback = isLightMapTheme(initialMapTheme)
				? FALLBACK_LIGHT_STYLE
				: FALLBACK_DARK_STYLE;
		...
		this.deckOverlay = new MapboxOverlay({
			...
			onError: (error: Error) => {
				...
				if (error.message.includes("satellite-imagery-layer")) {
					...
					console.warn(
						"[DeckGLMap] Satellite imagery layer failed (likely Intel GPU driver incompatibility) — rebuilding layer stack without it",
					);
					try {
						this.deckOverlay?.setProps({ layers: this.buildLayers() });
					} catch {
	...
	private _timeFilterCache = new WeakMap<
		object,
		{ min: number; range: TimeRange; result: unknown[] }
	>();
	...
	private filterMilitaryFlightClustersByTimeCached(
	...
	private filterMilitaryVesselClustersByTimeCached(
	...
	private filterMilitaryFlightClustersByTime(
		clusters: MilitaryFlightCluster[],
	): MilitaryFlightCluster[] {
		return clusters
			.map((cluster) => {
	...
	private rebuildProtestSupercluster(
		source: SocialUnrestEvent[] = this.getFilteredProtests(),
	): void {
		...
		this.protestSC = new Supercluster({
			...
			map: (props: Record<string, unknown>) => ({
				...
				riotTimeMs:
					props.eventType === "riot" &&
					props.sourceType !== "gdelt" &&
					Number.isFinite(Number(props.timeMs))
						? Number(props.timeMs)
						: 0,
			}),
			reduce: (acc: Record<string, unknown>, props: Record<string, unknown>) => {
				acc.maxSeverityRank = Math.max(
					Number(acc.maxSeverityRank ?? 0),
					Number(props.maxSeverityRank ?? 0),
				);
				acc.riotCount = Number(acc.riotCount ?? 0) + Number(props.riotCount ?? 0);
				acc.highSeverityCount =
					Number(acc.highSeverityCount ?? 0) + Number(props.highSeverityCount ?? 0);
	...
	private updateClusterData(): void {
		...
		const useTechEvents =
			SITE_VARIANT === "tech" && layers.techEvents && this.techEvents.length > 0;
		...
		if (
			zoom === this.lastSCZoom &&
			boundsKey === this.lastSCBoundsKey &&
			layerMask === this.lastSCMask
		)
			return;
		...
		if (useTechHQ && this.techHQSC) {
			this.techHQClusters = this.techHQSC.getClusters(bbox, zoom).map((f) => {
				...
				if (f.properties.cluster) {
					...
					const clusterCount = Number(f.properties.point_count ?? 0);
					const primaryType =
						faangCount >= unicornCount && faangCount >= publicCount
							? "faang"
			...
			this.techEventClusters = this.techEventSC.getClusters(bbox, zoom).map((f) => {
				...
				if (f.properties.cluster) {
					...
					return {
						...
						location: String(props.location ?? ""),
						country: String(props.country ?? ""),
						soonestDaysUntil: Number.isFinite(soonestDaysUntil)
							? soonestDaysUntil
		...
		if (useDatacenterClusters && this.datacenterSC) {
			...
			this.datacenterClusters = this.datacenterSC.getClusters(bbox, zoom).map((f) => {
				...
				return {
					...
					region: item.country,
					country: item.country,
					totalChips: item.chipCount,
					totalPowerMW: item.powerMW ?? 0,
	...
	private buildLayers(): LayersList {
		...
		const filteredEarthquakes = mapLayers.natural
			? this.filterByTimeCached(this.earthquakes, (eq) => eq.occurredAt)
			: [];
		const filteredNaturalEvents = mapLayers.natural
			? this.filterByTimeCached(this.naturalEvents, (event) => event.date)
			: [];
		const filteredDiseaseOutbreaks = mapLayers.diseaseOutbreaks
			? this.filterByTimeCached(this.diseaseOutbreaks, (item) => item.publishedAt)
			: [];
		const filteredRadiationObservations = mapLayers.radiationWatch
			? this.filterByTimeCached(this.radiationObservations, (obs) => obs.observedAt)
			: [];
		const filteredPositiveEvents = mapLayers.positiveEvents
			? this.filterByTimeCached(this.positiveEvents, (e) => e.timestamp)
			: [];
		const filteredIranEvents = mapLayers.iranAttacks
			? this.filterByTimeCached(this.iranEvents, (e) => e.timestamp)
			: [];
		const filteredFirmsFireData = mapLayers.fires
			? this.filterByTimeCached(this.firmsFireData, (d) => d.acq_date)
			: [];
		const filteredTrafficAnomalies = mapLayers.outages
			? this.filterByTimeCached(this.trafficAnomalies, (a) => a.startDate)
			: [];
		const filteredKindnessPoints = mapLayers.kindness
			? this.filterByTimeCached(this.kindnessPoints, (p) => p.timestamp)
			: [];
		const filteredImageryScenes = mapLayers.satellites
			? this.filterByTimeCached(this.imageryScenes, (s) => s.datetime)
			: [];
		const filteredWeatherAlerts = mapLayers.weather
			? this.filterByTimeCached(this.weatherAlerts, (alert) => alert.onset)
			: [];
		const filteredOutages = mapLayers.outages
			? this.filterByTimeCached(this.outages, (outage) => outage.pubDate)
			: [];
		const filteredCableAdvisories = mapLayers.cables
			? this.filterByTimeCached(this.cableAdvisories, (advisory) => advisory.reported)
			: [];
		const filteredFlightDelays = mapLayers.flights
			? this.filterByTimeCached(this.flightDelays, (delay) => delay.updatedAt)
			: [];
		const filteredMilitaryFlights = mapLayers.military
			? this.filterByTimeCached(this.militaryFlights, (flight) => flight.lastSeen)
			: [];
		const filteredMilitaryVessels = mapLayers.military
			? this.filterByTimeCached(this.militaryVessels, (vessel) => vessel.lastAisUpdate)
			: [];
		const filteredMilitaryFlightClusters = mapLayers.military
			? this.filterMilitaryFlightClustersByTimeCached(this.militaryFlightClusters)
			: [];
		const filteredMilitaryVesselClusters = mapLayers.military
			? this.filterMilitaryVesselClustersByTimeCached(this.militaryVesselClusters)
			: [];
		...
		if (
			...
			SITE_VARIANT !== "tech" &&
			SITE_VARIANT !== "happy" &&
			this.aptGroups.length > 0 &&
			!this.aptGroupsLayerFailed
		) {
		...
		if (
			mapLayers.satellites &&
			filteredImageryScenes.length > 0 &&
			!this.satelliteImageryLayerFailed
		) {
		...
		if (mapLayers.webcams && this.webcamData.length > 0) {
			layers.push(
				new ScatterplotLayer<WebcamEntry | WebcamCluster>({
					id: "webcam-layer",
					data: this.webcamData,
					getPosition: (d) => [d.lng, d.lat],
					getRadius: (d) => ("count" in d ? Math.min(8 + d.count * 0.5, 24) : 6),
					getFillColor: (d) =>
						("count" in d ? [0, 212, 255, 180] : [255, 215, 0, 200]) as [
							number,
							number,
							number,
							number,
	...
	private createCablesLayer(): PathLayer {
		...
		if (
			cached &&
			highlightSignature === this.lastCableHighlightSignature &&
			healthSignature === this.lastCableHealthSignature
		)
			return cached;
	...
	private createNaturalEventsLayers(events: NaturalEvent[]): Layer[] {
			...
			layers.push(
				new ScatterplotLayer({
					...
					getRadius: (d: NaturalEvent) =>
						d.title.startsWith("🔴") ? 20000 : d.title.startsWith("🟠") ? 15000 : 8000,
		...
		for (const e of cyclones) {
			...
			forecastPaths.push({
				path: [
					[e.lon, e.lat] as [number, number],
					...e.forecastTrack.map((p) => [p.lon, p.lat] as [number, number]),
				],
				stormName: e.stormName || e.title,
				_event: e,
			});
	...
	private createCyberThreatsLayer(): ScatterplotLayer<CyberThreat> {
		return new ScatterplotLayer<CyberThreat>({
			...
			getFillColor: (d) => {
				switch (d.severity) {
					case "critical":
						return [255, 61, 0, 225] as [number, number, number, number];
					case "high":
						return [255, 102, 0, 205] as [number, number, number, number];
					case "medium":
						return [255, 176, 0, 185] as [number, number, number, number];
					default:
						return [255, 235, 59, 170] as [number, number, number, number];
				}
			},
			radiusMinPixels: 6,
			radiusMaxPixels: 18,
	...
	private createRadiationLayer(
		items: RadiationObservation[],
	): ScatterplotLayer<RadiationObservation> {
		return new ScatterplotLayer<RadiationObservation>({
			...
			getFillColor: (d) =>
				(d.severity === "spike"
					...
					: d.confidence === "low"
						? [255, 174, 0, 150]
						: [255, 174, 0, 200]) as [number, number, number, number],
			getLineColor: [255, 255, 255, 200],
			stroked: true,
			lineWidthMinPixels: 2,
			radiusMinPixels: 6,
	...
	private createDiseaseOutbreaksLayer(
		items: DiseaseOutbreakItem[],
	): ScatterplotLayer<{ lon: number; lat: number; item: DiseaseOutbreakItem }> {
		...
		return new ScatterplotLayer<Point>({
			...
			getPosition: (d) => [d.lon, d.lat],
			getRadius: (d) =>
				d.item.alertLevel === "alert" ? 180000 : d.item.alertLevel === "warning" ? 130000 : 90000,
			getFillColor: (d) =>
				(d.item.alertLevel === "alert"
					? [231, 76, 60, 200]
					: d.item.alertLevel === "warning"
						? [230, 126, 34, 190]
						: [241, 196, 15, 170]) as [number, number, number, number],
		...
		return new ScatterplotLayer({
			...
			getFillColor: (d) => {
				...
				if (d.type === "supranational")
					return [255, 235, 140, 220] as [number, number, number, number];
	...
	private createMiningSitesLayer(): ScatterplotLayer {
		return new ScatterplotLayer({
			...
			getPosition: (d) => [d.lon, d.lat],
			getRadius: (d) =>
				d.status === "producing" ? 10000 : d.status === "development" ? 8000 : 6000,
	...
	private createProcessingPlantsLayer(): ScatterplotLayer {
		return new ScatterplotLayer({
			...
			getFillColor: (d) => {
				switch (d.type) {
					case "smelter":
						return [255, 80, 30, 210] as [number, number, number, number];
					case "refinery":
						return [255, 160, 50, 200] as [number, number, number, number];
					case "separation":
						return [160, 100, 255, 200] as [number, number, number, number];
					case "processing":
						return [100, 200, 150, 200] as [number, number, number, number];
					default:
						return [200, 150, 100, 200] as [number, number, number, number];
				}
			},
			radiusMinPixels: 5,
			radiusMaxPixels: 12,
	...
	private createTechHQClusterLayers(): Layer[] {
		...
		layers.push(
			new ScatterplotLayer<MapTechHQCluster>({
				...
				getFillColor: (d) => {
					if (d.primaryType === "faang")
						return [0, 220, 120, 200] as [number, number, number, number];
					if (d.primaryType === "unicorn")
						return [255, 100, 200, 180] as [number, number, number, number];
		...
		if (multiClusters.length > 0) {
			layers.push(
				new TextLayer<MapTechHQCluster>({
	...
	private createTechEventClusterLayers(): Layer[] {
		...
		layers.push(
			new ScatterplotLayer<MapTechEventCluster>({
				...
				getFillColor: (d) => {
					if (d.soonestDaysUntil <= 14)
						return [255, 220, 50, 200] as [number, number, number, number];
		...
		if (multiClusters.length > 0) {
			layers.push(
				new TextLayer<MapTechEventCluster>({
	...
	private createDatacenterClusterLayers(): Layer[] {
		...
		layers.push(
			new ScatterplotLayer<MapDatacenterCluster>({
		...
		if (multiClusters.length > 0) {
			layers.push(
				new TextLayer<MapDatacenterCluster>({
	...
	private hasRecentRiot(now = Date.now(), windowMs = 2 * 60 * 60 * 1000): boolean {
		const hasRecentClusterRiot = this.protestClusters.some(
			(c) =>
	...
	private createNewsLocationsLayer(): ScatterplotLayer[] {
		...
		const filteredNewsLocations = this.filterByTime(
			this.newsLocations,
	...
	private createKindnessLayers(items: KindnessPoint[]): Layer[] {
		...
		// Dot layer (tooltip on hover via getTooltip)
		layers.push(
			new ScatterplotLayer<KindnessPoint>({
				id: "kindness-layer",
				data: items,
	...
	private createSpeciesRecoveryLayer(): ScatterplotLayer {
		return new ScatterplotLayer({
			...
			getPosition: (d: (typeof this.speciesRecoveryZones)[number]) => [
				d.recoveryZone.lon,
				d.recoveryZone.lat,
			],
			getRadius: 50000,
	...
	private createRenewableInstallationsLayer(): ScatterplotLayer {
		...
		return new ScatterplotLayer({
			...
			radiusMinPixels: 5,
			radiusMaxPixels: 18,
			getFillColor: (d: RenewableInstallation) =>
				typeColors[d.type] ?? ([200, 200, 200, 200] as [number, number, number, number]),
			stroked: true,
			getLineColor: (d: RenewableInstallation) =>
				typeLineColors[d.type] ?? ([200, 200, 200, 255] as [number, number, number, number]),
			lineWidthMinPixels: 1,
			pickable: true,
		});
	}
	...
	private async fetchImageryForViewport(): Promise<void> {
		...
		try {
			...
			/* viewport fetch failed silently */
		}
	}
	...
	private getTooltip(info: PickingInfo): { html: string } | null {
		...
		switch (layerId) {
			...
			case "earthquakes-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>M${(obj.magnitude || 0).toFixed(1)} ${t("components.deckgl.tooltip.earthquake")}</strong><br/>${text(obj.place)}</div>`,
				};
			...
			case "military-flights-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.callsign || obj.registration || t("components.deckgl.tooltip.militaryAircraft"))}</strong><br/>${text(obj.type)}</div>`,
				};
			case "military-vessel-clusters-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name || t("components.deckgl.tooltip.vesselCluster"))}</strong><br/>${obj.vesselCount || 0} ${t("components.deckgl.tooltip.vessels")}<br/>${text(obj.activityType)}</div>`,
				};
			case "military-flight-clusters-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name || t("components.deckgl.tooltip.flightCluster"))}</strong><br/>${obj.flightCount || 0} ${t("components.deckgl.tooltip.aircraft")}<br/>${text(obj.activityType)}</div>`,
				};
			case "protests-layer":
				return {
			...
			case "protest-clusters-layer":
				if (obj.count === 1) {
					...
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(item?.title || t("components.deckgl.tooltip.protest"))}</strong><br/>${text(item?.city || item?.country || "")}</div>`,
					};
				}
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("components.deckgl.tooltip.protestsCount", { count: String(obj.count) })}</strong><br/>${text(obj.country)}</div>`,
				};
			case "tech-hq-clusters-layer":
				if (obj.count === 1) {
					const hq = obj.items?.[0];
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(hq?.company || "")}</strong><br/>${text(hq?.city || "")}</div>`,
					};
				}
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("components.deckgl.tooltip.techHQsCount", { count: String(obj.count) })}</strong><br/>${text(obj.city)}</div>`,
				};
			case "tech-event-clusters-layer":
				if (obj.count === 1) {
					const ev = obj.items?.[0];
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(ev?.title || "")}</strong><br/>${text(ev?.location || "")}</div>`,
					};
				}
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("components.deckgl.tooltip.techEventsCount", { count: String(obj.count) })}</strong><br/>${text(obj.location)}</div>`,
				};
			case "datacenter-clusters-layer":
				if (obj.count === 1) {
					const dc = obj.items?.[0];
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(dc?.name || "")}</strong><br/>${text(dc?.owner || "")}</div>`,
					};
				}
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("components.deckgl.tooltip.dataCentersCount", { count: String(obj.count) })}</strong><br/>${text(obj.country)}</div>`,
				};
			case "bases-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.country)}${obj.kind ? ` · ${text(obj.kind)}` : ""}</div>`,
				};
			...
			case "datacenters-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.owner)}</div>`,
				};
			case "cables-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${t("components.deckgl.tooltip.underseaCable")}</div>`,
				};
			case "pipelines-layer": {
				const pipelineType = String(obj.type || "").toLowerCase();
				const pipelineTypeLabel =
					pipelineType === "oil"
				...
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${pipelineTypeLabel}</div>`,
				};
			}
			case "conflict-zones-layer": {
				const props = obj.properties || obj;
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(props.name)}</strong><br/>${t("components.deckgl.tooltip.conflictZone")}</div>`,
				};
			}

			case "natural-events-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.title)}</strong><br/>${text(obj.category || t("components.deckgl.tooltip.naturalEvent"))}</div>`,
				};
			case "storm-centers-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.stormName || obj.title)}</strong><br/>${text(obj.classification || "")} ${obj.windKt ? obj.windKt + " kt" : ""}</div>`,
				};
			case "storm-forecast-track-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.stormName)}</strong><br/>${t("popups.naturalEvent.classification")}: Forecast Track</div>`,
				};
			case "storm-past-track-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.stormName)}</strong><br/>Past Track (${obj.windKt} kt)</div>`,
				};
			case "storm-cone-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.stormName)}</strong><br/>Forecast Cone</div>`,
				};
			case "ais-density-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("components.deckgl.layers.shipTraffic")}</strong><br/>${t("popups.intensity")}: ${text(obj.intensity)}</div>`,
				};
			case "waterways-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${t("components.deckgl.layers.strategicWaterways")}</div>`,
				};
			...
			case "stock-exchanges-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.shortName)}</strong><br/>${text(obj.city)}, ${text(obj.country)}</div>`,
				};
			case "financial-centers-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.type)} ${t("components.deckgl.tooltip.financialCenter")}</div>`,
				};
			...
			case "commodity-hubs-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.type)} · ${text(obj.city)}</div>`,
				};
			...
			case "tech-hqs-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.company)}</strong><br/>${text(obj.city)}</div>`,
				};
			case "accelerators-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.city)}</div>`,
				};
			case "cloud-regions-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.provider)}</strong><br/>${text(obj.region)}</div>`,
				};
			case "tech-events-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.title)}</strong><br/>${text(obj.location)}</div>`,
				};
			case "irradiators-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.type || t("components.deckgl.layers.gammaIrradiators"))}</div>`,
				};
			case "disease-outbreaks-layer": {
				...
				const summaryHtml = item.summary
					? `<br/><span style="opacity:.75">${text(item.summary.slice(0, 100))}${item.summary.length > 100 ? "…" : ""}</span>`
					: "";
				return {
					html: `<div class="deckgl-tooltip"><strong style="color:${lvlColor}">${text(item.alertLevel.toUpperCase())}</strong> ${text(item.disease)}<br/>${text(item.location)}${summaryHtml}${metaHtml}</div>`,
				};
			}
			case "radiation-watch-layer": {
				const severityLabel =
					obj.severity === "spike"
						? t("components.deckgl.layers.radiationSpike")
						: t("components.deckgl.layers.radiationElevated");
				const delta = Number(obj.delta || 0);
				const confidence = String(obj.confidence || "low").toUpperCase();
				const corroboration = obj.corroborated
					? "CONFIRMED"
					: obj.conflictingSources
						? "CONFLICTING"
						: confidence;
				return {
					html: `<div class="deckgl-tooltip"><strong>${severityLabel}</strong><br/>${text(obj.location)}<br/>${Number(obj.value).toFixed(1)} ${text(obj.unit)} · ${delta >= 0 ? "+" : ""}${delta.toFixed(1)} vs baseline<br/>${text(corroboration)}</div>`,
				};
			}
			case "spaceports-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.country || t("components.deckgl.layers.spaceports"))}</div>`,
				};
			case "ports-layer": {
				const typeIcon =
					obj.type === "naval" ? "⚓" : obj.type === "oil" || obj.type === "lng" ? "🛢️" : "🏭";
				return {
					html: `<div class="deckgl-tooltip"><strong>${typeIcon} ${text(obj.name)}</strong><br/>${text(obj.type || t("components.deckgl.tooltip.port"))} - ${text(obj.country)}</div>`,
				};
			}
			case "flight-delays-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)} (${text(obj.iata)})</strong><br/>${text(obj.severity)}: ${text(obj.reason)}</div>`,
				};
			case "notam-overlay-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong style="color:#ff2828;">&#9888; NOTAM CLOSURE</strong><br/>${text(obj.name)} (${text(obj.iata)})<br/><span style="opacity:.7">${text((obj.reason || "").slice(0, 100))}</span></div>`,
				};
			case "aircraft-positions-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.callsign || obj.icao24)}</strong><br/>${obj.altitudeFt?.toLocaleString() ?? 0} ft · ${obj.groundSpeedKts ?? 0} kts · ${Math.round(obj.trackDeg ?? 0)}°</div>`,
				};
			case "apt-groups-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.aka)}<br/>${t("popups.sponsor")}: ${text(obj.sponsor)}</div>`,
				};
			case "minerals-layer":
				return {
			...
			case "mining-sites-layer": {
				const statusLabel =
					obj.status === "producing"
						? "⛏️ Producing"
						: obj.status === "development"
							? "🔧 Development"
							: "🔍 Exploration";
				const outputStr = obj.annualOutput
					? `<br/><span style="opacity:.75">${text(obj.annualOutput)}</span>`
					: "";
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.mineral)} · ${text(obj.country)}<br/>${statusLabel}${outputStr}</div>`,
				};
			}
					...
					obj.type === "smelter"
						...
						: obj.type === "refinery"
							...
							: obj.type === "separation"
								? "🧪 Separation"
								: "🏗️ Processing";
				const capacityStr = obj.capacityTpa
					? `<br/><span style="opacity:.75">${text(String((obj.capacityTpa / 1000).toFixed(0)))}k t/yr</span>`
					: "";
				const mineralLabel =
					obj.mineral ?? (Array.isArray(obj.materials) ? obj.materials.join(", ") : "");
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(mineralLabel)} · ${text(obj.country)}<br/>${typeLabel}${capacityStr}</div>`,
				};
			}
			case "commodity-ports-layer": {
				const commoditiesStr = Array.isArray(obj.commodities) ? obj.commodities.join(", ") : "";
				const volumeStr = obj.annualVolumeMt
					? `<br/><span style="opacity:.75">${text(String(obj.annualVolumeMt))}Mt/yr</span>`
					: "";
				return {
					html: `<div class="deckgl-tooltip"><strong>⚓ ${text(obj.name)}</strong><br/>${text(obj.country)}<br/>${text(commoditiesStr)}${volumeStr}</div>`,
				};
			}
			case "ais-disruptions-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>AIS ${text(obj.type || t("components.deckgl.tooltip.disruption"))}</strong><br/>${text(obj.severity)} ${t("popups.severity")}<br/>${text(obj.description)}</div>`,
				};
			case "gps-jamming-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>GPS Jamming</strong><br/>${text(obj.level)} · NP avg: ${Number(obj.npAvg).toFixed(2)}<br/>H3: ${text(obj.h3)}</div>`,
				};
			case "cable-advisories-layer": {
				const cableName = UNDERSEA_CABLES.find((c) => c.id === obj.cableId)?.name || obj.cableId;
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(cableName)}</strong><br/>${text(obj.severity || t("components.deckgl.tooltip.advisory"))}<br/>${text(obj.description)}</div>`,
				};
			}
			case "repair-ships-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name || t("components.deckgl.tooltip.repairShip"))}</strong><br/>${text(obj.status)}</div>`,
				};
			case "weather-layer": {
				...
				const area = areaDesc
					...
					: "";
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.event || t("components.deckgl.layers.weatherAlerts"))}</strong><br/>${text(obj.severity)}${area}</div>`,
				};
			}
			case "outages-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.title || t("components.deckgl.tooltip.internetOutage"))}</strong><br/>${text(obj.country)}</div>`,
				};
			case "traffic-anomalies-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.type || "Traffic Anomaly")}</strong><br/>${text(obj.locationName || obj.asnName || "")}</div>`,
				};
			case "ddos-locations-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>DDoS: ${text(obj.countryName)}</strong><br/>${text(obj.percentage ? obj.percentage.toFixed(1) + "%" : "")}</div>`,
				};
			case "cyber-threats-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("popups.cyberThreat.title")}</strong><br/>${text(obj.severity || t("components.deckgl.tooltip.medium"))} · ${text(obj.country || t("popups.unknown"))}</div>`,
				};
			case "iran-events-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>${t("components.deckgl.layers.iranAttacks")}: ${text(obj.category || "")}</strong><br/>${text((obj.title || "").slice(0, 80))}</div>`,
				};
			case "news-locations-layer":
				return {
					html: `<div class="deckgl-tooltip"><strong>📰 ${t("components.deckgl.tooltip.news")}</strong><br/>${text(obj.title?.slice(0, 80) || "")}</div>`,
				};
			case "positive-events-layer": {
				const catLabel = obj.category ? obj.category.replace(/-/g, " & ") : "Positive Event";
				const countInfo =
					obj.count > 1
						? `<br/><span style="opacity:.7">${obj.count} sources reporting</span>`
						: "";
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/><span style="text-transform:capitalize">${text(catLabel)}</span>${countInfo}</div>`,
				};
			}
			case "kindness-layer":
			...
			case "happiness-choropleth-layer": {
				...
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(hcName)}</strong><br/>Happiness: ${hcScoreStr}/10${hcScore != null ? `<br/><span style="opacity:.7">${text(this.happinessSource)} (${this.happinessYear})</span>` : ""}</div>`,
				};
			}
			case "cii-choropleth-layer": {
				...
				if (!ciiEntry)
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(ciiName)}</strong><br/><span style="opacity:.7">No CII data</span></div>`,
					};
				const levelColor = DeckGLMap.CII_LEVEL_HEX[ciiEntry.level] ?? "#888";
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(ciiName)}</strong><br/>CII: <span style="color:${levelColor};font-weight:600">${ciiEntry.score}/100</span><br/><span style="text-transform:capitalize;opacity:.7">${text(ciiEntry.level)}</span></div>`,
				};
			}
			case "resilience-choropleth-layer": {
				...
				const resilienceEntry = resilienceCode
					? this.resilienceScoresMap.get(resilienceCode as string)
					: undefined;
				if (!resilienceEntry) {
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(resilienceName)}</strong><br/><span style="opacity:.7">No resilience data</span></div>`,
					};
				}
				if (resilienceEntry.level === "insufficient_data") {
					return {
						html: `<div class="deckgl-tooltip"><strong>${text(resilienceName)}</strong><br/><span style="opacity:.7">Insufficient data</span></div>`,
					};
				}
			...
			case "species-recovery-layer": {
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.commonName)}</strong><br/>${text(obj.recoveryZone?.name ?? obj.region)}<br/><span style="opacity:.7">Status: ${text(obj.recoveryStatus)}</span></div>`,
				};
			}
			case "renewable-installations-layer": {
				const riTypeLabel = obj.type
					? String(obj.type).charAt(0).toUpperCase() + String(obj.type).slice(1)
					: "Renewable";
				return {
					html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${riTypeLabel} &middot; ${obj.capacityMW?.toLocaleString() ?? "?"} MW<br/><span style="opacity:.7">${text(obj.country)} &middot; ${obj.year}</span></div>`,
				};
			}
			case "gulf-investments-layer": {
				...
				const usd =
					inv.investmentUSD != null
						? inv.investmentUSD >= 1000
							? `$${(inv.investmentUSD / 1000).toFixed(1)}B`
							: `$${inv.investmentUSD}M`
						: t("components.deckgl.tooltip.undisclosed");
				const stake =
					inv.stakePercent != null
						? `<br/>${text(String(inv.stakePercent))}% ${t("components.deckgl.tooltip.stake")}`
						: "";
				return {
			...
			case "webcam-layer": {
				const label = "count" in obj ? `${obj.count} webcams` : obj.title || obj.name || "Webcam";
	...
	private handleClick(info: PickingInfo): void {
		...
		if (!info.object || isChoropleth) {
			if (info.coordinate && this.onCountryClick) {
				...
				if (isChoropleth && info.object?.properties) {
					country = {
						code: info.object.properties["ISO3166-1-Alpha-2"] as string,
			...
			if (cluster.items.length === 0 && cluster._clusterId != null && this.protestSC) {
				try {
					...
					cluster.items = leaves
						.map((l) => this.protestSuperclusterSource[l.properties.index])
		...
		if (layerId === "tech-hq-clusters-layer") {
			...
			if (cluster.items.length === 0 && cluster._clusterId != null && this.techHQSC) {
				try {
					...
					cluster.items = leaves
						.map((l) => TECH_HQS[l.properties.index])
						.filter(Boolean) as typeof TECH_HQS;
					cluster.sampled = cluster.items.length < cluster.count;
				} catch (e) {
		...
		if (layerId === "tech-event-clusters-layer") {
			...
			if (cluster.items.length === 0 && cluster._clusterId != null && this.techEventSC) {
				try {
					const leaves = this.techEventSC.getLeaves(
						cluster._clusterId,
						DeckGLMap.MAX_CLUSTER_LEAVES,
					);
					cluster.items = leaves
						.map((l) => this.techEvents[l.properties.index])
						.filter((x): x is TechEventMarker => !!x);
					cluster.sampled = cluster.items.length < cluster.count;
				} catch (e) {
		...
		if (layerId === "datacenter-clusters-layer") {
			...
			if (cluster.items.length === 0 && cluster._clusterId != null && this.datacenterSC) {
				try {
					const leaves = this.datacenterSC.getLeaves(
						cluster._clusterId,
						DeckGLMap.MAX_CLUSTER_LEAVES,
					);
					cluster.items = leaves
			...
			const related = this.iranEvents
				.filter(
					(e) =>
						e.id !== clickedId &&
						e.locationName &&
						e.locationName.trim().toLowerCase() === normalizedLoc,
				)
				...
				if (layer) {
					...
					if (enabled && (layer === "resilienceScore" || layer === "ciiChoropleth")) {
						const conflictingLayer =
							layer === "resilienceScore" ? "ciiChoropleth" : "resilienceScore";
						if (this.state.layers[conflictingLayer]) {
							this.state.layers[conflictingLayer] = false;
							const conflictingToggle = this.container.querySelector(
								`.layer-toggle[data-layer="${conflictingLayer}"] input`,
	...
	private showLayerHelp(): void {
		...
		const label = (layerKey: string): string =>
			t(`components.deckgl.layers.${layerKey}`).toUpperCase();
		const staticLabel = (labelKey: string): string =>
			t(`components.deckgl.layerHelp.labels.${labelKey}`).toUpperCase();
	...
	private createLegend(): void {
		...
		const resilienceLegendItems: { shape: string; label: string; layerKey: keyof MapLayers }[] = [
			{
				shape: shapes.square("rgb(239, 68, 68)"),
				label: "Resilience: Very Low",
				layerKey: "resilienceScore",
			},
			{
				shape: shapes.square("rgb(249, 115, 22)"),
				label: "Resilience: Low",
				layerKey: "resilienceScore",
			},
			{
				shape: shapes.square("rgb(234, 179, 8)"),
				label: "Resilience: Moderate",
				layerKey: "resilienceScore",
			},
			{
				shape: shapes.square("rgb(132, 204, 22)"),
				label: "Resilience: High",
				layerKey: "resilienceScore",
			},
			{
				shape: shapes.square("rgb(34, 197, 94)"),
				label: "Resilience: Very High",
				layerKey: "resilienceScore",
			},
		];
		const legendItems: { shape: string; label: string; layerKey: keyof MapLayers }[] =
			SITE_VARIANT === "tech"
				? [
						{
							shape: shapes.circle(isLight ? "rgb(22, 163, 74)" : "rgb(0, 255, 150)"),
							label: t("components.deckgl.legend.startupHub"),
							layerKey: "startupHubs",
						},
						{
							shape: shapes.circle("rgb(100, 200, 255)"),
							label: t("components.deckgl.legend.techHQ"),
							layerKey: "techHQs",
						},
						{
							shape: shapes.circle(isLight ? "rgb(180, 120, 0)" : "rgb(255, 200, 0)"),
							label: t("components.deckgl.legend.accelerator"),
							layerKey: "accelerators",
						},
						{
							shape: shapes.circle("rgb(150, 100, 255)"),
							label: t("components.deckgl.legend.cloudRegion"),
							layerKey: "cloudRegions",
						},
						{
							shape: shapes.square("rgb(136, 68, 255)"),
							label: t("components.deckgl.legend.datacenter"),
							layerKey: "datacenters",
						},
						{
							shape: shapes.circle("rgb(231, 76, 60)"),
							label: t("components.deckgl.legend.diseaseAlert"),
							layerKey: "diseaseOutbreaks",
						},
						{
							shape: shapes.circle("rgb(230, 126, 34)"),
							label: t("components.deckgl.legend.diseaseWarning"),
							layerKey: "diseaseOutbreaks",
						},
						{
							shape: shapes.circle("rgb(241, 196, 15)"),
							label: t("components.deckgl.legend.diseaseWatch"),
							layerKey: "diseaseOutbreaks",
						},
				...
				: SITE_VARIANT === "finance"
					? [
							{
								shape: shapes.circle("rgb(255, 215, 80)"),
								label: t("components.deckgl.legend.stockExchange"),
								layerKey: "stockExchanges",
							},
							{
								shape: shapes.circle("rgb(0, 220, 150)"),
								label: t("components.deckgl.legend.financialCenter"),
								layerKey: "financialCenters",
							},
							{
								shape: shapes.hexagon("rgb(255, 210, 80)"),
								label: t("components.deckgl.legend.centralBank"),
								layerKey: "centralBanks",
							},
							{
								shape: shapes.square("rgb(255, 150, 80)"),
								label: t("components.deckgl.legend.commodityHub"),
								layerKey: "commodityHubs",
							},
							{
								shape: shapes.triangle("rgb(80, 170, 255)"),
								label: t("components.deckgl.legend.waterway"),
								layerKey: "waterways",
							},
							{
								shape: shapes.circle("rgb(231, 76, 60)"),
					...
					: SITE_VARIANT === "happy"
						? [
								{
									shape: shapes.circle("rgb(34, 197, 94)"),
									label: "Positive Event",
									layerKey: "positiveEvents",
								},
								{
									shape: shapes.circle("rgb(234, 179, 8)"),
									label: "Breakthrough",
									layerKey: "positiveEvents",
								},
								{
									shape: shapes.circle("rgb(74, 222, 128)"),
									label: "Act of Kindness",
									layerKey: "kindness",
								},
								{
									shape: shapes.circle("rgb(255, 100, 50)"),
									label: "Natural Event",
									layerKey: "natural",
								},
								{
									shape: shapes.square("rgb(34, 180, 100)"),
									label: "Happy Country",
									layerKey: "happiness",
								},
								{
									shape: shapes.circle("rgb(74, 222, 128)"),
									label: "Species Recovery Zone",
									layerKey: "speciesRecovery",
								},
								{
									shape: shapes.circle("rgb(255, 200, 50)"),
									label: "Renewable Installation",
									layerKey: "renewableInstallations",
								},
								{
									shape: shapes.circle("rgb(160, 100, 255)"),
									label: t("components.deckgl.legend.aircraft"),
									layerKey: "flights",
								},
								{
									shape: shapes.circle("rgb(231, 76, 60)"),
						...
						: SITE_VARIANT === "commodity"
							? [
									{
										shape: shapes.hexagon(isLight ? "rgb(180, 120, 0)" : "rgb(255, 200, 0)"),
										label: t("components.deckgl.legend.commodityHub"),
										layerKey: "commodityHubs",
									},
									{
										shape: shapes.circle("rgb(180, 80, 80)"),
										label: t("components.deckgl.legend.miningSite"),
										layerKey: "miningSites",
									},
									{
										shape: shapes.square("rgb(80, 160, 220)"),
										label: t("components.deckgl.legend.commodityPort"),
										layerKey: "commodityPorts",
									},
									{
										shape: shapes.circle("rgb(255, 150, 50)"),
										label: t("components.deckgl.legend.pipeline"),
										layerKey: "pipelines",
									},
									{
										shape: shapes.triangle("rgb(80, 170, 255)"),
									...
									{
										shape: shapes.circle("rgb(200, 100, 255)"),
										label: t("components.deckgl.legend.processingPlant"),
										layerKey: "processingPlants",
									},
									{
										shape: shapes.circle("rgb(231, 76, 60)"),
										label: t("components.deckgl.legend.diseaseAlert"),
							...
							: [
									{
										shape: shapes.circle("rgb(255, 68, 68)"),
										label: t("components.deckgl.legend.highAlert"),
										layerKey: "hotspots",
									},
									{
										shape: shapes.circle("rgb(255, 165, 0)"),
										label: t("components.deckgl.legend.elevated"),
										layerKey: "hotspots",
									},
									{
										shape: shapes.circle(isLight ? "rgb(180, 120, 0)" : "rgb(255, 255, 0)"),
										label: t("components.deckgl.legend.monitoring"),
										layerKey: "hotspots",
									},
									{
										shape: shapes.circle("rgb(255, 100, 100)"),
										label: t("components.deckgl.legend.conflict"),
										layerKey: "conflicts",
									},
									{
										shape: shapes.triangle("rgb(68, 136, 255)"),
										label: t("components.deckgl.legend.base"),
										layerKey: "bases",
									},
									{
										shape: shapes.hexagon(isLight ? "rgb(180, 120, 0)" : "rgb(255, 220, 0)"),
										label: t("components.deckgl.legend.nuclear"),
										layerKey: "nuclear",
									},
									{
										shape: shapes.square("rgb(136, 68, 255)"),
	...
	private updateLayers(): void {
		...
		try {
			...
			/* map may be mid-teardown (null.getProjection) */
		}
		this.maplibreMap.triggerRepaint();
		const elapsed = performance.now() - startTime;
		if (import.meta.env.DEV && elapsed > 16) {
	...
	private updateZoomHints(): void {
		...
		for (const [key, enabled] of Object.entries(this.state.layers)) {
			const toggle = toggleList.querySelector(
				`.layer-toggle[data-layer="${key}"]`,
			) as HTMLElement | null;
			if (!toggle) continue;
		...
		return new ScatterplotLayer<UcdpGeoEvent>({
			...
			getFillColor: (d) => {
				switch (d.type_of_violence) {
					case "state-based":
						return COLORS.ucdpStateBased;
					case "non-state":
						return COLORS.ucdpNonState;
					case "one-sided":
	...
	private createDisplacementArcsLayer(): ArcLayer<DisplacementFlow> {
		const withCoords = this.displacementFlows.filter(
			(f) => f.originLat != null && f.asylumLat != null,
		);
	...
	private createTradeRoutesLayer(): ArcLayer<TradeRouteSegment> {
		const active: [number, number, number, number] =
			getCurrentTheme() === "light" ? [30, 100, 180, 200] : [100, 200, 255, 160];
		const disrupted: [number, number, number, number] =
			getCurrentTheme() === "light" ? [200, 40, 40, 220] : [255, 80, 80, 200];
		const highRisk: [number, number, number, number] =
			getCurrentTheme() === "light" ? [200, 140, 20, 200] : [255, 180, 50, 180];
	...
	private createDayNightLayer(): PolygonLayer {
		const nightPolygon =
			this.cachedNightPolygon ?? (this.cachedNightPolygon = this.computeNightPolygon());
		const isLight = getCurrentTheme() === "light";
	...
	private fetchServerBases(): void {
		...
		fetchMilitaryBases(sw.lat, sw.lng, ne.lat, ne.lng, zoom)
			.then((result) => {
				if (!result) return;
	...
	private enforceLayerLimit(): void {
		...
		const activeCount = Array.from(
			togglesEl.querySelectorAll<HTMLInputElement>(".layer-toggle input"),
		)
	...
	public enableLayer(layer: keyof MapLayers): void {
		if (!this.state.layers[layer]) {
			if (layer === "resilienceScore" && this.state.layers.ciiChoropleth) {
				...
				const ciiToggle = this.container.querySelector(
					`.layer-toggle[data-layer="ciiChoropleth"] input`,
				) as HTMLInputElement | null;
				...
				const resilienceToggle = this.container.querySelector(
					`.layer-toggle[data-layer="resilienceScore"] input`,
				) as HTMLInputElement | null;
	...
	public triggerBaseClick(id: string): void {
		const base =
			this.serverBases.find((b) => b.id === id) || MILITARY_BASES.find((b) => b.id === id);
		if (base) {
	...
	private resolveCountryFromCoordinate(
		lon: number,
		lat: number,
		...
		try {
			...
			const code =
				typeof properties["ISO3166-1-Alpha-2"] === "string"
	...
	private setupCountryHover(): void {
		...
		map.on("mousemove", (e) => {
			...
			try {
				...
				/* style not done loading during theme switch */
			}
		});
	...
	public clearCountryHighlight(): void {
		...
		try {
			...
			/* style unloaded or map torn down between panel close and highlight clear */
		}
	}
