...
export class TradePolicyPanel extends Panel {
	...
	private render(): void {
		...
		if (!wtoAvailable && !hasRevenue && !hasComtrade) {
			this.setContent(
				`<div class="economic-empty">${t("components.tradePolicy.apiKeyMissing")}</div>`,
			);
			return;
		}
      ...
      <div class="panel-tabs">
        ...
        ${
					hasTariffs
						? `<button class="panel-tab ${this.activeTab === "tariffs" ? "active" : ""}" data-tab="tariffs">
        ...
        ${
					hasFlows
						? `<button class="panel-tab ${this.activeTab === "flows" ? "active" : ""}" data-tab="flows">
        ...
        ${
					hasBarriers
						? `<button class="panel-tab ${this.activeTab === "barriers" ? "active" : ""}" data-tab="barriers">
        ...
        ${
					hasRevenue
						? `<button class="panel-tab ${this.activeTab === "revenue" ? "active" : ""}" data-tab="revenue">
        ...
        ${
					hasComtrade
						? `<button class="panel-tab ${this.activeTab === "comtrade" ? "active" : ""}" data-tab="comtrade">
    ...
    `;
		...
		const unavailableBanner =
			!activeHasData && activeData?.upstreamUnavailable
		...
		switch (this.activeTab) {
			...
			case "barriers":
				...
				break;
			case "revenue":
				contentHtml = this.renderRevenue();
				break;
			case "comtrade":
				contentHtml = this.renderComtradeFlows();
				break;
		}

		const source =
			this.activeTab === "comtrade"
				? t("components.tradePolicy.sourceComtrade")
				: this.activeTab === "revenue"
					? t("components.tradePolicy.sourceTreasury")
					: (this.activeTab === "tariffs" || this.activeTab === "restrictions") &&
							this.tariffsData?.effectiveTariffRate?.sourceName
    ...
    <div class="trade-restrictions-list">
      ${this.restrictionsData.restrictions
				.map((r) => {
					...
					const statusLabel =
						r.status === "high"
							? t("components.tradePolicy.highTariff")
							: r.status === "moderate"
								? t("components.tradePolicy.moderateTariff")
								: t("components.tradePolicy.lowTariff");
	...
	private renderTariffSummary(
		latestBaseline: TariffDataPoint | null,
		effectiveRate: EffectiveTariffRate | null,
	): string {
		if (!latestBaseline) return "";

		const baselineMeta = t("components.tradePolicy.wtoBaselineMeta", {
			year: String(latestBaseline.year),
		});
		const baselineCard = `
	...
	private getEffectiveTariffGapSummary(): {
		baseline: TariffDataPoint;
		effectiveRate: EffectiveTariffRate;
		gap: number;
	} | null {
    ...
    `;
		...
		const rows = [...months]
			...
			.map((m) => {
				const highlight =
					m.monthlyAmountBillions > priorAvg * 2 ? ' class="trade-revenue-spike"' : "";
				return `<tr${highlight}>
        <td>${m.recordDate}</td>
