...
export class MapPopup {
	...
	private renderConflictPopup(conflict: ConflictZone): string {
		const severityClass =
			conflict.intensity === "high" ? "high" : conflict.intensity === "medium" ? "medium" : "low";
		const severityLabel = escapeHtml(
			conflict.intensity?.toUpperCase() || t("popups.unknown").toUpperCase(),
		);
	...
	private renderHotspotPopup(hotspot: Hotspot, relatedNews?: NewsItem[]): string {
		...
		const localizedSubtext = hotspot.subtext
			? this.getLocalizedHotspotSubtext(hotspot.subtext)
			: "";
		...
		const trendColors: Record<string, string> = {
			escalating: getCSSColor("--semantic-critical"),
			stable: getCSSColor("--semantic-elevated"),
			"de-escalating": getCSSColor("--semantic-normal"),
		};
	...
	public async loadHotspotGdeltContext(hotspot: Hotspot): Promise<void> {
        ...
        <div class="hotspot-gdelt-articles">
          ${articles
						.slice(0, 5)
						.map((article) => this.renderGdeltArticle(article))
						.join("")}
        </div>
      `;
		} catch (error) {
		...
		try {
			...
			if (live.photoUrl) {
				...
				if (photoSrc) {
					...
					const credit = live.photoCredit
						? `<span class="flight-photo-credit">\u00a9 ${escapeHtml(live.photoCredit)}</span>`
						: "";
			...
			if (live.callsignIata) {
				const name = live.airlineName
					? ` <span style="font-size:12px;opacity:0.6;font-weight:400">${escapeHtml(live.airlineName)}</span>`
					: "";
				parts.push(
					`<div style="font-weight:700;font-size:15px;margin:4px 0">${escapeHtml(live.callsignIata)}${name}</div>`,
				);
			}

			// Route (FROM → TO)
			if (live.depIata && live.arrIata) {
				const terminal = live.arrTerminal
					? `<span style="font-size:10px;opacity:0.5;margin-left:4px">T${escapeHtml(live.arrTerminal)}</span>`
					: "";
				const duration = live.flightDurationMin
					? `<span style="font-size:11px;opacity:0.6">${Math.floor(live.flightDurationMin / 60)}h${live.flightDurationMin % 60 > 0 ? ` ${live.flightDurationMin % 60}m` : ""}</span>`
					: "";
				parts.push(`
          ...
          <div class="flight-times" style="font-size:11px;display:grid;grid-template-columns:1fr auto 1fr;gap:2px 8px;margin-bottom:6px;opacity:0.85">
            ...
            <span>${depEst}${fmtDelayMin(live.depDelayedMin)}</span><span style="opacity:0.3;text-align:center">\u2194</span><span style="text-align:right">${arrEst}${fmtDelayMin(live.arrDelayedMin)}</span>`
				...
				const bookUrl = sanitizeUrl(
					`https://www.google.com/travel/flights/search?q=Flights+from+${encodeURIComponent(live.depIata)}+to+${encodeURIComponent(live.arrIata)}+on+${encodeURIComponent(todayStr)}`,
				);
				parts.push(
					`<a href="${bookUrl}" target="_blank" rel="noopener" style="display:block;margin-top:8px;padding:7px 12px;background:rgba(68,255,136,.06);border:1px solid rgba(68,255,136,.18);border-radius:6px;color:var(--green,#44ff88);text-decoration:none;font-size:12px;text-align:center">Book this route &rarr;</a>`,
				);
			}
			...
			if (live.registration)
				rows.push(
					`<div class="popup-stat"><span class="stat-label">Reg</span><span class="stat-value">${escapeHtml(live.registration)}</span></div>`,
				);
			if (live.model)
				rows.push(
					`<div class="popup-stat"><span class="stat-label">Model</span><span class="stat-value">${escapeHtml(live.model)}</span></div>`,
				);
			if (live.operator)
				rows.push(
					`<div class="popup-stat"><span class="stat-label">Operator</span><span class="stat-value">${escapeHtml(live.operator)}</span></div>`,
				);
			if (live.verticalRate !== 0)
				rows.push(
					`<div class="popup-stat"><span class="stat-label">Climb</span><span class="stat-value">${live.verticalRate > 0 ? "+" : ""}${Math.round(live.verticalRate)} fpm</span></div>`,
				);
	...
	private renderEarthquakePopup(earthquake: Earthquake): string {
		const severity =
			earthquake.magnitude >= 6 ? "high" : earthquake.magnitude >= 5 ? "medium" : "low";
		const severityLabel =
			earthquake.magnitude >= 6
				? t("popups.earthquake.levels.major")
				: earthquake.magnitude >= 5
					? t("popups.earthquake.levels.moderate")
					: t("popups.earthquake.levels.minor");
	...
	private renderBasePopup(base: MilitaryBase): string {
		...
		const enriched = base as MilitaryBase & {
			kind?: string;
			catAirforce?: boolean;
			catNaval?: boolean;
			catNuclear?: boolean;
	...
	private renderAisPopup(event: AisDisruptionEvent): string {
		...
		const typeLabel =
			event.type === "gap_spike" ? t("popups.aisGapSpike") : t("popups.chokepointCongestion");
	...
	private renderProtestPopup(event: SocialUnrestEvent): string {
		...
		const sourceLabel =
			event.sourceType === "acled" ? t("popups.protest.acledVerified") : t("popups.protest.gdelt");
		const validatedBadge = event.validated
			? `<span class="popup-badge verified">${t("popups.verified")}</span>`
			: "";
	...
	private renderProtestClusterPopup(data: ProtestClusterData): string {
		...
		const highSeverity =
			data.highSeverityCount ?? data.items.filter((e) => e.severity === "high").length;
		const verified = data.verifiedCount ?? data.items.filter((e) => e.validated).length;
		const totalFatalities =
			data.totalFatalities ?? data.items.reduce((sum, e) => sum + (e.fatalities || 0), 0);
		...
		const listItems = sortedItems
			...
			.map((event) => {
				...
				const dateStr = event.time.toLocaleDateString(undefined, {
					month: "short",
					day: "numeric",
				});
		...
		const moreCount =
			remainingCount > 0
				? `<li class="cluster-more">+${remainingCount} ${t("popups.moreEvents")}</li>`
				: "";
	...
	private renderFlightPopup(delay: AirportDelayAlert): string {
		...
		const avgDelaySection =
			delay.avgDelayMinutes > 0
...
${isFeatureAvailable("wingbitsEnrichment") ? '<div class="wingbits-live-section"><div class="wingbits-live-loading" style="font-size:11px;opacity:0.5;padding:4px 0">Loading Wingbits live data…</div></div>' : ""}
	...
	private renderCablePopup(cable: UnderseaCable): string {
		...
		if (healthRecord?.status === "fault") {
			...
			statusLabel =
				advisory.severity === "fault" ? t("popups.cable.fault") : t("popups.cable.degraded");
		...
		const repairShipNote = repairShip
			? escapeHtml(repairShip.note || t("popups.repairShip.note"))
			: "";
        ...
        ${
          ...
          <div class="popup-section">
            ...
            <div class="popup-tags">
              ${outage.categories
								.slice(0, 5)
								.map((c) => `<span class="popup-tag">${escapeHtml(c)}</span>`)
								.join("")}
            </div>
          </div>
        `
	...
	private renderDatacenterClusterPopup(data: DatacenterClusterData): string {
		...
		const totalPower =
			data.totalPowerMW ?? data.items.reduce((sum, dc) => sum + (dc.powerMW || 0), 0);
		const existingCount =
			data.existingCount ?? data.items.filter((dc) => dc.status === "existing").length;
		const plannedCount =
			data.plannedCount ?? data.items.filter((dc) => dc.status === "planned").length;
	...
	private renderMilitaryFlightPopup(flight: MilitaryFlight): string {
		...
		const climbRateStat =
			flight.verticalRate !== undefined && flight.verticalRate !== 0
				? `
          <div class="popup-stat">
		...
		const enrichedStats = flight.enriched
			? [
					flight.enriched.manufacturer
						? `<div class="popup-stat"><span class="stat-label">${t("popups.militaryFlight.manufacturer")}</span><span class="stat-value">${escapeHtml(flight.enriched.manufacturer)}</span></div>`
						: "",
					flight.enriched.owner
						? `<div class="popup-stat"><span class="stat-label">${t("popups.militaryFlight.owner")}</span><span class="stat-value">${escapeHtml(flight.enriched.owner)}</span></div>`
						: "",
					flight.enriched.builtYear
						? `<div class="popup-stat"><span class="stat-label">${t("popups.militaryFlight.builtYear")}</span><span class="stat-value">${escapeHtml(flight.enriched.builtYear)}</span></div>`
						: "",
				].join("")
			: "";
...
${isFeatureAvailable("wingbitsEnrichment") ? '<div class="wingbits-live-section"><div class="wingbits-live-loading" style="font-size:11px;opacity:0.5;padding:4px 0">Loading Wingbits live data…</div></div>' : ""}
	...
	private getOperatorCountryCode(vessel: { operator: string; operatorCountry?: string }): string {
		return (
			(vessel.operatorCountry ? nameToCountryCode(vessel.operatorCountry) : null) ||
	...
	private renderMilitaryVesselPopup(vessel: MilitaryVessel): string {
		...
		const deploymentBadge =
			vessel.usniDeploymentStatus && vessel.usniDeploymentStatus !== "unknown"
		...
		const displayType =
			vessel.vesselType === "unknown" && vessel.aisShipType
				? vessel.aisShipType
		...
		const vesselName = escapeHtml(
			vessel.name || `${t("popups.militaryVessel.vessel")} ${vessel.mmsi}`,
		);
		const vesselOperator = escapeHtml(
			operatorLabels[vessel.operator] || vessel.operatorCountry || t("popups.unknown"),
		);
		...
		const usniIntel =
			vessel.usniActivityDescription || vessel.usniRegion || vessel.usniStrikeGroup
				? `
	...
	private renderMilitaryFlightClusterPopup(cluster: MilitaryFlightCluster): string {
		...
		const dominantOperator = cluster.dominantOperator
			? escapeHtml(cluster.dominantOperator.toUpperCase())
			: "";
		const flightSummary = cluster.flights
			.slice(0, 5)
			.map(
				(f) =>
					`<div class="cluster-flight-item">${escapeHtml(f.callsign)} - ${escapeHtml(f.aircraftType)}</div>`,
			)
			.join("");
		const moreFlights =
			cluster.flightCount > 5
	...
	private renderMilitaryVesselClusterPopup(cluster: MilitaryVesselCluster): string {
		...
		cluster.vessels.forEach((v) => {
			opCounts[v.operator] = (opCounts[v.operator] || 0) + 1;
		});
		...
		const moreLabel = escapeHtml(
			t("popups.militaryCluster.moreVessels", { count: String(hiddenCount) }),
		);
	...
	private renderTcDetails(event: NaturalEvent): string {
		...
		const catLabel =
			...
			(cat > 0 ? `Category ${cat}` : t("popups.naturalEvent.tropicalSystem"));
	...
	private renderStockExchangePopup(exchange: StockExchangePopupData): string {
		...
		const tierClass =
			exchange.tier === "mega" ? "high" : exchange.tier === "major" ? "medium" : "low";
