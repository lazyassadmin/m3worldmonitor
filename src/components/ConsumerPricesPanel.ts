...
export class ConsumerPricesPanel extends Panel {
	...
	private renderOverview(): string {
		...
		if (!d || !d.asOf || d.asOf === "0")
			return this.renderEmptyState("No price data available yet");
      ...
      ${
        ...
        <div class="cp-category-mini">
          ${d.topCategories
						.slice(0, 5)
						.map((c) => this.renderCategoryMini(c))
						.join("")}
        </div>
      `
					: ""
	...
	private renderMovers(): string {
		...
		if (!risers.length && !fallers.length)
			return this.renderEmptyState("No movers for this selection");
	...
	private renderSpread(): string {
		...
		if (!d?.retailers?.length)
			return this.renderEmptyState("Retailer comparison starts once data is collected");
