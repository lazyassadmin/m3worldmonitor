...
import type {
	...
	MarketImplicationsData,
	TransmissionNode,
} from "@/services/market-implications";
import { FrameworkSelector } from "./FrameworkSelector";
import { hasPremiumAccess } from "@/services/panel-gating";

const DISCLAIMER =
	"AI-generated trade signals for informational purposes only. Not investment advice. Always do your own research.";
...
export class MarketImplicationsPanel extends Panel {
	...
	constructor() {
		...
		this.fwSelector = new FrameworkSelector({
			...
			panel: this,
			note: "Applies to next AI regeneration",
		});
	...
	public renderImplications(
		data: MarketImplicationsData,
		...
		const freshness = data.generatedAt
			? describeFreshness(new Date(data.generatedAt).getTime())
			: "";
	...
	public showUnavailable(
		message = "AI market implications are generated after each forecast run. Check back shortly.",
	): void {
