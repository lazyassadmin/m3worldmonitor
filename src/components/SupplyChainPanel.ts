...
export class SupplyChainPanel extends Panel {
	...
	private clearTransitChart(): void {
		if (this.chartMountTimer) {
			clearTimeout(this.chartMountTimer);
    ...
    `;
		...
		const activeData =
			this.activeTab === "chokepoints"
				? this.chokepointData
				: this.activeTab === "shipping" || this.activeTab === "indicators"
					? this.shippingData
					: this.activeTab === "stress"
						? this.stressData
						: this.mineralsData;
		...
		if (this.activeTab === "chokepoints" && this.expandedChokepoint) {
			const mountTransitChart = (): boolean => {
				const el = this.content.querySelector(
					`[data-chart-cp="${this.expandedChokepoint}"]`,
				) as HTMLElement | null;
				if (!el) return false;
				const cp = this.chokepointData?.chokepoints?.find(
					(c) => c.name === this.expandedChokepoint,
				);
	...
	private renderChokepoints(): string {
      ...
      ${[...this.chokepointData.chokepoints]
					...
					const disruptClass =
						disruptPct > 10
							...
							: disruptPct > 3
								...
								: "sc-disrupt-green";
					const riskClass =
						ts?.riskLevel === "critical" || ts?.riskLevel === "high"
							? "sc-disrupt-red"
							: ts?.riskLevel === "elevated" || ts?.riskLevel === "moderate"
								? "sc-disrupt-yellow"
								: "sc-disrupt-green";

					const expanded = this.expandedChokepoint === cp.name;
					const actionRow =
						expanded && ts?.riskReportAction
							? `<div class="sc-routing-advisory">${escapeHtml(ts.riskReportAction)}</div>`
							: "";
					const chartPlaceholder =
						expanded && ts?.history?.length
          ...
          <div class="trade-restriction-body">
            ...
            ${
							ts && (ts.todayTotal > 0 || hasWow || disruptPct > 0)
								? `<div class="sc-metric-row">
            ...
            <div class="trade-affected">${cp.affectedRoutes
							.slice(0, 3)
							.map((r) => escapeHtml(r))
							.join(", ")}</div>
            ${actionRow}
			...
			const cards = indices
				.map((idx) => {
					...
					const sparkline = this.renderSparkline(
						idx.history.map((h) => h.value),
						idx.history.map((h) => h.date),
					);
	...
	private renderIndicators(): string {
		...
		const container = new Set(["SCFI", "CCFI"]);
		const bulk = new Set(["BDI", "BCI", "BPI", "BSI", "BHSI"]);
		const econIndices = this.shippingData.indices.filter(
			(i) => !container.has(i.indexId) && !bulk.has(i.indexId),
		);
		if (!econIndices.length) {
			return `<div class="economic-empty">${t("components.supplyChain.noShipping")}</div>`;
		}
		const cards = econIndices
			.map((idx) => {
				const changeClass = idx.changePct >= 0 ? "change-positive" : "change-negative";
				const changeArrow = idx.changePct >= 0 ? "\u25B2" : "\u25BC";
				const sparkline = this.renderSparkline(
      ...
      <div style="text-align:right;font-size:10px;color:var(--text-dim);margin-top:2px">${stressScore.toFixed(1)}/100</div>
		...
		const rows = carriers
			.map((c) => {
				...
				const typeLabel =
					c.carrierType === "etf" ? "ETF" : c.carrierType === "index" ? "IDX" : "CARR";
	...
	private renderMinerals(): string {
		...
		const rows = this.mineralsData.minerals
			.map((m) => {
				const riskClass =
					m.riskRating === "critical"
						? "sc-risk-critical"
						: m.riskRating === "high"
							? "sc-risk-high"
							: m.riskRating === "moderate"
								? "sc-risk-moderate"
								: "sc-risk-low";
				const top3 = m.topProducers
					.slice(0, 3)
					.map((p) => `${escapeHtml(p.country)} ${p.sharePct.toFixed(0)}%`)
					.join(", ");
				return `<tr>
