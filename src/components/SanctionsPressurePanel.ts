...
export class SanctionsPressurePanel extends Panel {
	...
	constructor() {
		super({
			...
			defaultRowSpan: 2,
			infoTooltip:
				"OFAC sanctions designations from the SDN and Consolidated Lists. Shows which countries face the highest designation pressure, what programs are driving it, and what has been newly added since the last update.",
		});
    ...
    `;
		...
		const entriesHtml =
			data.entries.length > 0
				? data.entries
						.slice(0, 10)
						.map((entry) => this.renderEntryRow(entry))
						.join("")
		...
		const programsHtml =
			data.programs.length > 0
				? data.programs
						.slice(0, 6)
						.map((program) => this.renderProgramRow(program))
						.join("")
	...
	private renderCountryRow(country: CountrySanctionsPressure): string {
		const flags: string[] = [];
		if (country.newEntryCount > 0)
			flags.push(
				`<span class="sanctions-pill sanctions-pill-new">+${country.newEntryCount} new</span>`,
			);
		if (country.vesselCount > 0)
			flags.push(`<span class="sanctions-pill">🚢 ${country.vesselCount}</span>`);
		if (country.aircraftCount > 0)
			flags.push(`<span class="sanctions-pill">✈ ${country.aircraftCount}</span>`);
	...
	private renderEntryRow(entry: SanctionsEntry): string {
		...
		const note = entry.note
			? `<div class="sanctions-entry-note">${escapeHtml(entry.note)}</div>`
			: "";
