...
export class CountryDeepDivePanel implements CountryBriefPanel {
	...
	public showGeoError(onRetry: () => void): void {
		...
		const retryBtn = this.el(
			"button",
			"cdp-geo-error-retry",
			t("countryBrief.retryBtn"),
	...
	public updateNews(headlines: NewsItem[]): void {
		...
		for (let i = 0; i < items.length; i++) {
			...
			top.append(
				this.badge(`Tier ${tier}`, `cdp-tier-badge tier-${Math.max(1, Math.min(4, tier))}`),
			);
	...
	public updateMilitaryActivity(summary: CountryDeepDiveMilitarySummary): void {
		...
		stats.append(
			...
			this.metric(
				t("countryBrief.foreignFlights"),
				String(summary.foreignFlights),
				summary.foreignFlights > 0 ? "cdp-chip-danger" : "cdp-chip-neutral",
			),
			this.metric(
				t("countryBrief.navalVessels"),
				String(summary.nearbyVessels),
				"cdp-chip-neutral",
			),
			this.metric(
				t("countryBrief.foreignPresence"),
				summary.foreignPresence ? t("countryBrief.detected") : t("countryBrief.notDetected"),
				summary.foreignPresence ? "cdp-chip-danger" : "cdp-chip-success",
			),
		);
	...
	public updateInfrastructure(countryCode: string): void {
		...
		for (const type of INFRA_TYPES) {
			const list = this.infrastructureByType.get(type) ?? [];
			if (list.length === 0) continue;
			const typeLabel = this.el(
				"div",
				"cdp-subtitle",
				`${INFRA_ICONS[type]} ${t(`countryBrief.infra.${type}`)}`,
			);
		...
		const nearbyPorts = PORTS.map((port) => ({
			...port,
	...
	public updateCountryFacts(data: CountryFactsData): void {
		...
		if (data.wikipediaSummary) {
			const summaryText =
				data.wikipediaSummary.length > 300
		...
		grid.append(
			this.factItem(t("countryBrief.facts.area"), `${data.areaSqKm.toLocaleString()} km\u00B2`),
		);

		const rawTitle = data.headOfStateTitle || "";
		const hosLabel =
			rawTitle.length > 30
				? t("countryBrief.facts.headOfState")
				: rawTitle || t("countryBrief.facts.headOfState");
	...
	private renderEnergyProfile(data: CountryEnergyProfileData): void {
		...
		const hasAny =
			data.mixAvailable ||
			data.jodiOilAvailable ||
			data.ieaStocksAvailable ||
			data.jodiGasAvailable ||
			data.gasStorageAvailable ||
		...
		if (data.mixAvailable) {
			const segments: Array<{ label: string; color: string; value: number }> = [
				...
				{ label: "Solar", color: "#FFD700", value: data.solarShare },
				{
					label: "Other renew",
					color: "#32CD32",
					value: Math.max(0, data.renewShare - data.windShare - data.solarShare - data.hydroShare),
				},
			];
			...
			bar.style.cssText =
				"display:flex;width:100%;height:12px;border-radius:4px;overflow:hidden;margin-bottom:8px";
		...
		if (data.jodiOilAvailable) {
			...
			section.append(
				this.el("div", "cdp-subtitle", `Oil Product Supply (${data.jodiOilDataMonth})`),
			);
		...
		if (data.jodiGasAvailable) {
			...
			pipeBadge.style.cssText =
				"background:#6b7280;color:#fff;padding:1px 5px;border-radius:3px;font-size:11px";
		...
		if (data.ieaStocksAvailable) {
			...
			if (data.ieaNetExporter) {
				...
				coverLabel.style.cssText =
					"font-size:12px;margin-bottom:4px;display:flex;align-items:center;gap:6px";
				...
				if (data.ieaBelowObligation) {
					...
					warn.style.cssText =
						"background:#ef4444;color:#fff;padding:1px 5px;border-radius:3px;font-size:11px";
				...
				barOuter.style.cssText =
					"position:relative;width:100%;height:8px;border-radius:4px;background:#374151;overflow:visible";
				...
				marker.style.cssText =
					"position:absolute;top:-2px;left:50%;width:2px;height:12px;background:#f59e0b;transform:translateX(-50%)";
	...
	private renderShockScenarioWidget(): HTMLElement {
		...
		chokepointSelect.style.cssText =
			"background:#1f2937;color:#e5e7eb;border:1px solid #374151;border-radius:4px;padding:3px 6px;font-size:11px";
		const chopkpts: Array<[string, string]> = [
			["hormuz", "Strait of Hormuz"],
			["malacca", "Strait of Malacca"],
			["suez", "Suez Canal"],
			["babelm", "Bab el-Mandeb"],
		];
		...
		disruptionSelect.style.cssText =
			"background:#1f2937;color:#e5e7eb;border:1px solid #374151;border-radius:4px;padding:3px 6px;font-size:11px";
		...
		computeBtn.addEventListener("click", () => {
			...
			const url = toApiUrl(
				`/api/intelligence/v1/compute-energy-shock?country_code=${encodeURIComponent(code)}&chokepoint_id=${encodeURIComponent(chokepoint)}&disruption_pct=${disruption}`,
			);
			globalThis
				.fetch(url)
	...
	public updateMarkets(markets: PredictionMarket[]): void {
		...
		for (const market of markets.slice(0, 5)) {
			...
			const prob = this.el(
				"div",
				"cdp-market-prob",
				`Probability: ${Math.round(market.yesPrice)}%`,
			);
			const meta = this.el(
				"div",
				"cdp-market-meta",
				market.endDate ? `Ends ${this.shortDate(market.endDate)}` : "Active",
			);
			...
			if (market.volume != null) {
				expanded.append(
					this.el("div", "cdp-market-volume", `Volume: $${market.volume.toLocaleString()}`),
				);
			}
	...
	public updateBrief(data: CountryIntelData): void {
		...
		if (data.error || data.skipped || !data.brief) {
			this.briefBody.append(
				this.makeEmpty(data.error || data.reason || t("countryBrief.assessmentUnavailable")),
			);
			return;
		}
	...
	private renderSkeleton(
		...
		const subtitle = this.el(
			...
			`${code.toUpperCase()} • Country Intelligence`,
		);
	...
	private renderInitialSignals(signals: CountryBriefSignals): void {
		...
		this.addSignalChip(
			chips,
			signals.criticalNews,
			t("countryBrief.chips.criticalNews"),
			"🚨",
			"conflict",
		);
		...
		this.addSignalChip(
			chips,
			signals.militaryFlights,
			t("countryBrief.chips.militaryAir"),
			"✈️",
			"military",
		);
		this.addSignalChip(
		...
		this.addSignalChip(
			chips,
			signals.aisDisruptions,
			t("countryBrief.chips.aisDisruptions"),
			"🚢",
			"outage",
		);
		this.addSignalChip(
			...
			t("countryBrief.chips.satelliteFires"),
			"🔥",
			"climate",
		);
		...
		this.addSignalChip(
			chips,
			signals.temporalAnomalies,
			t("countryBrief.chips.temporalAnomalies"),
			"⏱️",
			"outage",
		);
		this.addSignalChip(
			chips,
			signals.cyberThreats,
			t("countryBrief.chips.cyberThreats"),
			"🛡️",
			"conflict",
		);
		this.addSignalChip(
			chips,
			signals.earthquakes,
			t("countryBrief.chips.earthquakes"),
			"🌍",
			"quake",
		);
		if (signals.displacementOutflow > 0) {
			...
			chips.append(
				this.makeSignalChip(`🌊 ${fmt} ${t("countryBrief.chips.displaced")}`, "displacement"),
			);
		}
		this.addSignalChip(
			chips,
			signals.climateStress,
			t("countryBrief.chips.climateStress"),
			"🌡️",
			"climate",
		);
		this.addSignalChip(
			...
			t("countryBrief.chips.conflictEvents"),
			"⚔️",
			"conflict",
		);
		this.addSignalChip(
			chips,
			signals.activeStrikes,
			t("countryBrief.chips.activeStrikes"),
			"💥",
			"conflict",
		);
		if (signals.travelAdvisories > 0 && signals.travelAdvisoryMaxLevel) {
			const advLabel =
				signals.travelAdvisoryMaxLevel === "do-not-travel"
					? t("countryBrief.chips.doNotTravel")
					: signals.travelAdvisoryMaxLevel === "reconsider"
						? t("countryBrief.chips.reconsiderTravel")
						: t("countryBrief.chips.exerciseCaution");
			chips.append(
				this.makeSignalChip(
					`⚠️ ${signals.travelAdvisories} ${t("countryBrief.chips.advisory")}: ${advLabel}`,
					"advisory",
				),
			);
		}
		this.addSignalChip(
			chips,
			signals.orefSirens,
			t("countryBrief.chips.activeSirens"),
			"🚨",
			"conflict",
		);
		this.addSignalChip(
			chips,
			signals.orefHistory24h,
			t("countryBrief.chips.sirens24h"),
			"🕓",
			"conflict",
		);
		this.addSignalChip(
			chips,
			signals.aviationDisruptions,
			t("countryBrief.chips.aviationDisruptions"),
			"🚫",
			"outage",
		);
		this.addSignalChip(
			chips,
			signals.gpsJammingHexes,
			t("countryBrief.chips.gpsJammingZones"),
			"📡",
			"outage",
		);
	...
	private renderComponentBars(components: CountryScore["components"]): HTMLElement {
		...
		const items = [
			...
			{
				label: t("countryBrief.components.information"),
	...
	private renderEconomicIndicators(): void {
		...
		for (const indicator of this.economicIndicators.slice(0, 3)) {
			...
			const isMarketRow =
				indicator.label === "Stock Index" || indicator.label === "Weekly Momentum";
			const trendClass = isMarketRow
				? `trend-market-${indicator.trend}`
				: `trend-${indicator.trend}`;
			top.append(
				this.el("span", "cdp-economic-label", indicator.label),
				this.el(
					"span",
					`cdp-trend-token ${trendClass}`,
					this.trendArrowFromDirection(indicator.trend),
				),
			);
	...
	private getFocusableElements(): HTMLElement[] {
		...
		return Array.from(this.panel.querySelectorAll<HTMLElement>(selectors)).filter(
			(el) =>
	...
	private formatBrief(text: string, headlineCount = 0): string {
		return formatIntelBrief(
			text,
			headlineCount > 0 ? { count: headlineCount, hrefPrefix: "#cdp-news-" } : undefined,
		);
	}
