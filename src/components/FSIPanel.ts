...
function fsiInterpretation(label: string): string {
	if (label === "Low Stress")
		return "Credit markets functioning normally, equity/bond ratio healthy.";
	if (label === "Moderate Stress")
		return "Some deterioration in credit conditions, monitor closely.";
	if (label === "Elevated Stress")
		return "Significant credit market stress, defensive positioning warranted.";
	...
	public async fetchData(): Promise<boolean> {
		...
		try {
			...
			try {
				...
				if (
					hydratedEuFsi &&
					!hydratedEuFsi.unavailable &&
					Number.isFinite(hydratedEuFsi.latestValue)
				) {
					euFsi = hydratedEuFsi;
				} else {
