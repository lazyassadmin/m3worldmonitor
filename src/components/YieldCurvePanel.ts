...
async function getEconomicClient(): Promise<EconomicServiceClient> {
	if (!_client) {
		...
		_client = new EconomicServiceClient(getRpcBaseUrl(), {
...
function buildXAxisLabels(count: number): string {
	return TENOR_LABELS.slice(0, count)
		.map((label, i) => {
...
function buildEcbPolyline(ecbRates: Record<string, number>, yMin: number, yMax: number): string {
	const points = ECB_TENOR_ORDER.map((tenor) => {
...
function renderChart(
	...
	if (allValues.length === 0)
		return '<div style="padding:16px;color:var(--text-dim);font-size:12px">No yield data available.</div>';
...
function renderTable(points: YieldPoint[]): string {
	const headers = points
		.map(
			(p) =>
				`<th style="font-size:9px;font-weight:600;color:var(--text-dim);padding:4px 6px;text-align:center">${escapeHtml(p.tenor)}</th>`,
		)
		.join("");
	const cells = points
		.map((p) => {
...
function renderRatesTab(rows: RateRow[]): string {
	...
	if (!hasAny)
		return '<div style="padding:16px;color:var(--text-dim);font-size:12px">ECB rate data unavailable</div>';
...
export class YieldCurvePanel extends Panel {
	...
	public async fetchData(): Promise<boolean> {
		...
		try {
			...
			this._current = SERIES_IDS.map((id, i) => {
				...
				return {
					tenor: TENOR_LABELS[i] ?? id,
					value: obs.length > 0 ? (obs[obs.length - 1]?.value ?? null) : null,
				};
			});
			this._prior = SERIES_IDS.map((id, i) => {
				...
				return {
					tenor: TENOR_LABELS[i] ?? id,
					value: obs.length > 1 ? (obs[obs.length - 2]?.value ?? null) : null,
				};
			});
			...
			this._rateRows = [
				{ id: "ESTR", label: "€STR", obs: results["ESTR"]?.observations ?? [], color: "#2ecc71" },
				{
					id: "EURIBOR3M",
					label: "EURIBOR 3M",
					obs: results["EURIBOR3M"]?.observations ?? [],
					color: "#3498db",
				},
				{
					id: "EURIBOR6M",
					label: "EURIBOR 6M",
					obs: results["EURIBOR6M"]?.observations ?? [],
					color: "#9b59b6",
				},
				{
					id: "EURIBOR1Y",
					label: "EURIBOR 1Y",
					obs: results["EURIBOR1Y"]?.observations ?? [],
					color: "#e67e22",
				},
			];
			...
			if (!this._hasData)
				this.showError(
					e instanceof Error ? e.message : "Failed to load yield curve",
					() => void this.fetchData(),
				);
			return false;
		}
	...
	private _render(): void {
      ...
      <button class="panel-tab${this._tab === "rates" ? " active" : ""}" data-tab="rates" style="font-size:11px;padding:3px 10px">ECB Rates</button>
		...
		if (this._tab === "rates") {
			this.setContent(
				`<div style="padding:10px 14px 6px">${tabBar}${renderRatesTab(this._rateRows)}</div>`,
			);
			return;
		}
