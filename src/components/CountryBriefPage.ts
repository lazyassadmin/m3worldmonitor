...
export class CountryBriefPage implements CountryBriefPanel {
			...
			if (linkShareBtn) {
				...
				navigator.clipboard
					...
					.then(() => {
						...
						linkShareBtn.innerHTML =
							'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>';
						setTimeout(() => {
							linkShareBtn.innerHTML = orig;
						}, 1500);
					})
					.catch(() => {});
				return;
	...
	private signalChips(signals: CountryBriefSignals): string {
		...
		if (signals.criticalNews > 0)
			chips.push(
				`<span class="signal-chip conflict">🚨 ${signals.criticalNews} Critical News</span>`,
			);
		...
		if (signals.aisDisruptions > 0)
			chips.push(
				`<span class="signal-chip outage">🚢 ${signals.aisDisruptions} AIS Disruptions</span>`,
			);
		if (signals.satelliteFires > 0)
			chips.push(
				`<span class="signal-chip climate">🔥 ${signals.satelliteFires} Satellite Fires</span>`,
			);
		if (signals.radiationAnomalies > 0)
			chips.push(
				`<span class="signal-chip outage">☢️ ${signals.radiationAnomalies} Radiation Anomalies</span>`,
			);
		if (signals.temporalAnomalies > 0)
			chips.push(
				`<span class="signal-chip outage">⏱️ ${signals.temporalAnomalies} Temporal Anomalies</span>`,
			);
		if (signals.cyberThreats > 0)
			chips.push(
				`<span class="signal-chip conflict">🛡️ ${signals.cyberThreats} Cyber Threats</span>`,
			);
		...
		if (signals.activeStrikes > 0)
			chips.push(
				`<span class="signal-chip conflict">\u{1F4A5} ${signals.activeStrikes} ${t("modals.countryBrief.signals.activeStrikes")}</span>`,
			);
			...
			const advisoryLabel =
				signals.travelAdvisoryMaxLevel === "do-not-travel"
					...
					: signals.travelAdvisoryMaxLevel === "reconsider"
						...
						: "Exercise Caution";
			chips.push(
				`<span class="signal-chip ${advisoryClass}">\u26A0\uFE0F ${signals.travelAdvisories} Advisory: ${advisoryLabel}</span>`,
			);
		}
		if (signals.orefSirens > 0)
			chips.push(
				`<span class="signal-chip conflict">\u{1F6A8} ${signals.orefSirens} Active Sirens</span>`,
			);
		if (signals.orefHistory24h > 0)
			chips.push(
				`<span class="signal-chip conflict">\u{1F553} ${signals.orefHistory24h} Sirens / 24h</span>`,
			);
		if (signals.aviationDisruptions > 0)
			chips.push(
				`<span class="signal-chip outage">\u{1F6AB} ${signals.aviationDisruptions} ${t("modals.countryBrief.signals.aviationDisruptions")}</span>`,
			);
		if (signals.gpsJammingHexes > 0)
			chips.push(
				`<span class="signal-chip outage">\u{1F4E1} ${signals.gpsJammingHexes} ${t("modals.countryBrief.signals.gpsJammingZones")}</span>`,
			);
		chips.push(
	...
	private formatBrief(text: string, headlineCount = 0): string {
		return formatIntelBrief(
			text,
			headlineCount > 0 ? { count: headlineCount, hrefPrefix: "#cb-news-" } : undefined,
		);
	}
