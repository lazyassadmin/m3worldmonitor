	...
	private renderIndicators(): string {
		if (this.fredData.length === 0) {
			...
			if (this.fredState === "error" || this.fredState === "retrying") {
				...
				const mainMsg = raw.includes("\u2014")
					? raw.slice(0, raw.indexOf("\u2014")).trimEnd()
					: raw;
				const countdownLine = isRetrying
					? `<div class="panel-error-countdown">${escapeHtml(this.fredErrorMsg)}</div>`
					: "";
				return `
