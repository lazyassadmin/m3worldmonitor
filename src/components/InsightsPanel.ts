...
export class InsightsPanel extends Panel {
	...
	private getTheaterPostureContext(): string {
		...
		const postures = cachedPostures?.length
			? cachedPostures
			: this.lastMilitaryFlights.length > 0
				? getTheaterPostureSummaries(this.lastMilitaryFlights)
				: [];
				...
				const isqA = computeISQ(
					{
						...
						isAlert: a.isAlert,
						threatLevel: a.threatLevel ?? undefined,
				...
				const isqB = computeISQ(
					{
						...
						isAlert: b.isAlert,
						threatLevel: b.threatLevel ?? undefined,
	...
	private async updateFromClient(
		...
		try {
			...
			const importantItems = this.selectTopStories(
				clusters,
				8,
				focalFn,
				getCountryScore,
		...
		return stories
			.map((story, i) => {
				...
				if (story.sourceCount >= 3) {
					badges.push(
						`<span class="insight-badge confirmed">✓ ${story.sourceCount} sources</span>`,
					);
				...
				if (story.threatLevel === "critical" || story.threatLevel === "high") {
					const safeThreat = VALID_THREAT_LEVELS.includes(story.threatLevel)
						? story.threatLevel
						: "moderate";
					badges.push(
						`<span class="insight-badge velocity ${safeThreat}">${escapeHtml(story.category)}</span>`,
					);
				}
	...
	private get showMlDetected(): boolean {
		try {
			return localStorage.getItem("wm:debug-ml") === "1";
		} catch {
			return false;
		}
	}
	...
	private renderFocalPoints(): string {
		...
		const correlatedFPs = this.lastFocalPoints
			.filter(
				(fp) =>
					(fp.newsMentions > 0 && fp.signalCount > 0) || fp.signalTypes.includes("active_strike"),
			)
			.slice(0, 5);
