	...
	public async fetchData(): Promise<boolean> {
		...
		try {
			...
			if (!data.outbreaks?.length) {
				if (!this._hasData)
					this.showError("No outbreak data available", () => void this.fetchData());
				return false;
			}
		...
		const rows = filtered
			.map((o) => {
				...
				const sourceLink = o.sourceUrl
					...
					: o.sourceName
						? `<span style="font-size:9px;color:var(--text-dim)">${escapeHtml(o.sourceName)}</span>`
						: "";
