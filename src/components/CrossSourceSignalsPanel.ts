...
const SEVERITY_BADGE_STYLE: Record<string, string> = {
	CROSS_SOURCE_SIGNAL_SEVERITY_CRITICAL:
		"background:var(--semantic-critical);color:#fff;border:1px solid var(--semantic-critical)",
	CROSS_SOURCE_SIGNAL_SEVERITY_HIGH:
		"background:rgba(255,140,140,0.15);color:#ff8c8c;border:1px solid rgba(255,140,140,0.4)",
	CROSS_SOURCE_SIGNAL_SEVERITY_MEDIUM:
		"background:rgba(245,197,66,0.08);color:var(--yellow);border:1px solid rgba(245,197,66,0.35)",
...
export class CrossSourceSignalsPanel extends Panel {
	...
	constructor() {
		super({
			...
			trackActivity: true,
			infoTooltip:
				"Aggregates 15+ real-time data streams every 15 minutes. Ranks cross-domain signals by severity and detects composite escalation when 3 or more signal categories co-fire in the same theater.",
		});
	...
	public showFetchError(): void {
		this.showError("Signal data unavailable — upstream feeds unreachable.", () => {
	...
	private renderSignal(sig: CrossSourceSignal, index: number): string {
		...
		const sevBadgeStyle =
			SEVERITY_BADGE_STYLE[sig.severity] ?? SEVERITY_BADGE_STYLE.CROSS_SOURCE_SIGNAL_SEVERITY_LOW;
		...
		const contributors =
			isComposite && sig.contributingTypes.length > 0
				? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:5px">${sig.contributingTypes
						.slice(0, 5)
						.map(
							(t) =>
	...
	private render(): void {
		if (this.signals.length === 0) {
			if (!this.evaluatedAt) {
				...
				this.setContent(
					'<div style="padding:16px 0;text-align:center;font-size:12px;color:var(--text-dim)">No cross-source signals detected.</div>',
				);
			}
			return;
		}
