...
export class App {
	...
	private isPanelNearViewport(panelId: string, marginPx = 400): boolean {
		const panel = this.state.panels[panelId] as
	...
	private shouldRefreshIntelligence(): boolean {
		return (
			this.isAnyPanelNearViewport(["cii", "strategic-risk", "strategic-posture"]) ||
	...
	private shouldRefreshCorrelation(): boolean {
		return this.isAnyPanelNearViewport([
	...
	private updateConnectivityUi(): void {
		...
		if (!online) {
			...
			const hasAnyCached =
				this.bootstrapHydrationState.source === "cached" ||
				this.bootstrapHydrationState.source === "mixed";
			if (hasAnyCached) {
				statusMode = "cached";
				const offlineCachedAt = this.bootstrapHydrationState.tiers
					? Math.min(
							...Object.values(this.bootstrapHydrationState.tiers)
								.filter((tier) => tier.source === "cached" || tier.source === "mixed")
								.map((tier) => tier.updatedAt)
								.filter((v): v is number => typeof v === "number" && Number.isFinite(v)),
						)
					: NaN;
				const freshness = Number.isFinite(offlineCachedAt)
					? describeFreshness(offlineCachedAt)
					: t("common.cached").toLowerCase();
			...
			statusMode = "cached";
			const freshness = cachedUpdatedAt
				? describeFreshness(cachedUpdatedAt)
				: t("common.cached").toLowerCase();
		...
		if (bannerMessage) {
			...
			this.cachedModeBannerEl.classList.toggle(
	...
	constructor(containerId: string) {
		...
		const isDynamicPanel = (k: string) =>
			k === "runtime-config" || k.startsWith("cw-") || k.startsWith("mcp-");
		...
		if (storedVariant !== currentVariant) {
			...
			mapLayers = normalizeExclusiveChoropleths(
				sanitizeLayersForVariant({ ...defaultLayers }, currentVariant as MapVariant),
				null,
			);
		...
		if (!localStorage.getItem(PANEL_PRUNE_KEY)) {
			...
			for (const orderKey of [
				PANEL_ORDER_KEY,
				PANEL_ORDER_KEY + "-bottom-set",
				PANEL_ORDER_KEY + "-bottom",
			]) {
				try {
		...
		if (initialUrlState.layers) {
			mapLayers = normalizeExclusiveChoropleths(
				sanitizeLayersForVariant(initialUrlState.layers, currentVariant as MapVariant),
				null,
			);
	...
	public async init(): Promise<void> {
			...
			if (userId !== null && userId !== _prevUserId) {
				...
				if (anonId) {
					void (async () => {
						...
						const totalClaimed =
							claimed.subscriptions + claimed.entitlements + claimed.customers + claimed.payments;
						if (totalClaimed > 0) {
		...
		await Promise.all([this.dataLoader.loadAllData(true), this.primeVisiblePanelData(true)]);
	...
	private enforceFreeTierLimits(): void {
		...
		if (needsTrim) {
			...
			console.log(
				`[App] Free tier: trimmed ${enabledKeys.length - FREE_MAX_PANELS} panel(s) to enforce ${FREE_MAX_PANELS}-panel limit`,
			);
		}
		...
		if (enabledCount > FREE_MAX_SOURCES) {
			...
			console.log(
				`[App] Free tier: disabled ${toDisable} source(s) to enforce ${FREE_MAX_SOURCES}-source limit`,
			);
		}
	}
	...
	private setupRefreshIntervals(): void {
		...
		if (SITE_VARIANT !== "happy") {
			this.refreshScheduler.registerAll([
				...
				},
				{
					...
					condition: () => this.isPanelNearViewport("economic"),
				},
				{
					name: "spending",
				...
				},
				{
					...
					condition: () => this.isPanelNearViewport("energy-complex"),
				},
				{
					name: "firms",
					fn: () => this.dataLoader.loadFirmsData(),
					intervalMs: REFRESH_INTERVALS.firms,
					condition: () => this.shouldRefreshFirms(),
				},
				{
					name: "ais",
		...
		this.refreshScheduler.scheduleRefresh(
			"defense-patents",
			() => {
				(this.state.panels["defense-patents"] as DefensePatentsPanel).refresh();
				return Promise.resolve();
			},
		...
		if (SITE_VARIANT !== "happy") {
			this.refreshScheduler.scheduleRefresh(
				"temporalBaseline",
				() => this.dataLoader.refreshTemporalBaseline(),
				REFRESH_INTERVALS.temporalBaseline,
				() => this.shouldRefreshIntelligence(),
			);
		}
		...
		if (SITE_VARIANT === "full" || SITE_VARIANT === "finance" || SITE_VARIANT === "commodity") {
			this.refreshScheduler.scheduleRefresh(
				...
				() => this.isPanelNearViewport("trade-policy"),
			);
			this.refreshScheduler.scheduleRefresh(
				"supplyChain",
				() => this.dataLoader.loadSupplyChain(),
				REFRESH_INTERVALS.supplyChain,
				() => this.isPanelNearViewport("supply-chain"),
			);
		}
