...
export function exportToCSV(data: ExportData, filename = "worldmonitor-export"): void {
	...
	if (intel) {
			...
			intel.earthquakes.forEach((e) => {
				lines.push(
					csvRow([
						...
						String(e.magnitude),
						String(e.depthKm),
						new Date(e.occurredAt * 1000).toISOString(),
						e.sourceUrl,
					]),
				);
			});
		...
		if (intel.military?.vessels && intel.military.vessels.length > 0) {
			...
			intel.military.vessels.forEach((v) => {
				lines.push(
					csvRow([v.name, v.mmsi, v.operatorCountry, v.vesselType, String(v.lat), String(v.lon)]),
				);
			});
			lines.push("");
		}
		...
		if (intel.orefAlerts) {
			...
			lines.push(
				csvRow([String(intel.orefAlerts.alertCount), String(intel.orefAlerts.historyCount24h)]),
			);
			lines.push("");
		}
		...
		if (intel.radiation?.observations && intel.radiation.observations.length > 0) {
			...
			intel.radiation.observations.forEach((s) => {
				lines.push(
					csvRow([s.location, s.country, String(s.value), s.unit, s.observedAt.toISOString()]),
				);
			});
			lines.push("");
		}
	...
	if (clean.gpsJamming) {
		...
		lines.push(
			csvRow([
				clean.gpsJamming.fetchedAt,
				String(s.totalHexes),
				String(s.highCount),
				String(s.mediumCount),
			]),
		);
