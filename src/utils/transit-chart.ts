...
const VESSEL_KEYS: Array<keyof TransitDayCount & string> = [
	"container",
	"dryBulk",
	"generalCargo",
	"roro",
...
const CAP_KEYS: Array<keyof TransitDayCount & string> = [
	"capContainer",
	"capDryBulk",
	"capGeneralCargo",
	"capRoro",
...
export class TransitChart {
	...
	private buildLegend(): void {
		...
		leg.innerHTML = VESSEL_LABELS.map((label, i) => {
			...
			return (
				`<span style="display:flex;align-items:center;gap:4px;font-size:10px;color:${textDim}">` +
	...
	private draw = (): void => {
		...
		const stacks: number[][] = data.map((d) =>
			(this.tab === "calls" ? VESSEL_KEYS : CAP_KEYS).map(
				(k) => d[k as keyof TransitDayCount] as number,
			),
		);
	...
	private onMouseMove = (e: MouseEvent): void => {
		...
		const d = data[idx]!;
		const stack = (this.tab === "calls" ? VESSEL_KEYS : CAP_KEYS).map(
			(k) => d[k as keyof TransitDayCount] as number,
		);
		const total = stack.reduce((a, b) => a + b, 0);

		const maValues = compute7dMA(
			data.map((r) =>
				(this.tab === "calls" ? VESSEL_KEYS : CAP_KEYS).reduce(
					(s, k) => s + (r[k as keyof TransitDayCount] as number),
					0,
				),
			),
		);
