...
test.describe("RAG vector store (worker-side)", () => {
	...
	test("ingest → count → search round-trip", async () => {
		...
		const result = await sharedPage.evaluate(async () => {
			...
			const items = [
				{
					text: "Iran sanctions debate intensifies in Washington",
					pubDate: Date.now() - 86400000,
					source: "Reuters",
					url: "https://example.com/1",
				},
				{
					text: "Ukraine frontline positions shift near Bakhmut",
					pubDate: Date.now() - 172800000,
					source: "AP",
					url: "https://example.com/2",
				},
				{
					text: "China trade talks resume with EU delegation",
					pubDate: Date.now() - 259200000,
					source: "BBC",
	...
	test("minScore filtering excludes dissimilar results", async () => {
		...
		const result = await sharedPage.evaluate(async () => {
			...
			await mlWorker.vectorStoreIngest([
				{
					text: "Weather forecast sunny skies tomorrow morning",
					pubDate: Date.now(),
					source: "Weather",
					url: "",
				},
			...
			const results = await mlWorker.vectorStoreSearch(
				["Iran nuclear weapons program sanctions"],
				5,
				0.8,
			);
	...
	test("deduplicates across multi-query matches keeping max score", async () => {
		...
		const result = await sharedPage.evaluate(async () => {
			...
			await mlWorker.vectorStoreIngest([
				{
					text: "Military operations expand in eastern regions",
					pubDate: Date.now(),
					source: "Reuters",
					url: "https://example.com/1",
				},
	...
	test("queue resilience after IDB error", async () => {
		...
		const result = await sharedPage.evaluate(async () => {
			...
			await mlWorker.vectorStoreIngest([
				{
					text: "Valid headline about economic policy",
					pubDate: Date.now(),
					source: "Reuters",
					url: "https://example.com/1",
				},
			...
			await mlWorker.vectorStoreIngest([
				{
					text: "Recovery headline after IDB reset",
					pubDate: Date.now(),
					source: "AP",
					url: "https://example.com/3",
				},
