...
test.describe("AI widget builder", () => {
	...
	test("creates a widget through the live modal flow and persists it after reload", async ({
		page,
	}) => {
		...
		expect(bannerPosition.bannerRect!.left).toBeGreaterThanOrEqual(
			bannerPosition.panelRect.left - 1,
		);

		await page.reload();
		await expect(
			page.locator(".custom-widget-panel", {
				has: page.locator(".panel-title", { hasText: "Oil vs Gold" }),
			}),
		).toBeVisible({ timeout: 20000 });

		const storedWidgets = await page.evaluate(() => {
			return JSON.parse(localStorage.getItem("wm-custom-widgets") || "[]") as Array<{
				title: string;
			}>;
		});
	...
	test("supports modify, keeps session history, exposes touch-sized controls, and cleans storage on delete", async ({
		page,
	}) => {
		...
		const initialAccent = await colorButton.evaluate(
			(button) => getComputedStyle(button).backgroundColor,
		);
		await colorButton.click();
		const updatedAccent = await colorButton.evaluate(
			(button) => getComputedStyle(button).backgroundColor,
		);
		...
		await expect(modifyModal.locator(".widget-chat-messages")).toContainText(
			"Generated widget: Oil vs Gold",
		);
		...
		expect(
			secondRequest?.conversationHistory?.some((entry) => entry.content.includes(createPrompt)),
		).toBe(true);
		expect(
			secondRequest?.conversationHistory?.some((entry) =>
				entry.content.includes("Generated widget: Oil vs Gold"),
			),
		).toBe(true);
...
test.describe("AI widget builder — PRO tier", () => {
	...
	test("creates a PRO widget: iframe renders with allow-scripts sandbox and PRO badge visible", async ({
		...
		await expect(modal.locator(".widget-chat-readiness")).toContainText("Connected", {
			timeout: 15000,
		});
		await modal
			.locator(".widget-chat-input")
			.fill("Interactive chart comparing oil and gold prices");
		await modal.locator(".widget-chat-send").click();
	...
	test("modify PRO widget: tier preserved, history passed to server", async ({ page }) => {
		...
		await modifyModal
			.locator(".widget-chat-input")
			.fill("Turn into flight delay interactive chart");
		await modifyModal.locator(".widget-chat-send").click();
		await expect(modifyModal.locator(".widget-chat-action-btn")).toBeEnabled({ timeout: 30000 });
		await modifyModal.locator(".widget-chat-action-btn").click();
		...
		expect(
			secondRequest?.conversationHistory?.some((e) =>
				e.content.includes("Interactive oil gold chart"),
			),
		).toBe(true);
