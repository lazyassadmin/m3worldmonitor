...
export function buildRelayResponse(response, body, headers) {
	...
	if (isNonJsonError) {
		console.warn(
			`[relay] Wrapping non-JSON ${response.status} upstream error (ct: ${ct || "none"}); body preview: ${String(body).slice(0, 120)}`,
		);
	}
	return new Response(
		isNonJsonError
...
export function createRelayHandler(cfg) {
	return async function handler(req) {
		...
		try {
			...
			const path =
				typeof cfg.buildRelayPath === "function"
