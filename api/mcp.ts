...
const TOOL_REGISTRY: ToolDef[] = [
	...
	},
	{
		...
		inputSchema: { type: "object", properties: {}, required: [] },
		_cacheKeys: ["seismology:earthquakes:v1", "wildfire:fires:v1", "natural:events:v1"],
	...
	},
	{
		...
		description:
			"Macro economic indicators: Fed Funds rate (FRED), economic calendar events, fuel prices, ECB FX rates, EU yield curve, earnings calendar, COT positioning, and energy storage data.",
		inputSchema: { type: "object", properties: {}, required: [] },
		_cacheKeys: [
	...
	// -------------------------------------------------------------------------
	{
		...
		description:
			"AI-generated world intelligence brief. Fetches the latest geopolitical headlines and produces an LLM-summarized brief. Supply an optional geo_context to focus on a region or topic.",
		inputSchema: {
			type: "object",
			properties: {
				geo_context: {
	...
	},
	{
		...
		_execute: async (params, base, apiKey) => {
			const UA = "worldmonitor-mcp-edge/1.0";
			const countryCode = String(params.country_code ?? "")
				.toUpperCase()
				.slice(0, 2);
			...
			try {
				...
				/* proceed without context — better than failing */
			}
	...
	},
	{
		...
		_execute: async (params, base, apiKey) => {
			const code = String(params.country_code ?? "")
				.toUpperCase()
				.slice(0, 2);
	...
	},
	{
		...
		_execute: async (params, base, apiKey) => {
			...
			if (!bbox)
				return {
					error: `Unknown country code: ${code}. Use ISO 3166-1 alpha-2 (e.g. "AE", "US", "GB").`,
				};
			...
			const [civResult, milResult] = await Promise.allSettled([
				type === "military"
					...
					: fetch(`${base}/api/aviation/v1/track-aircraft?${bboxQ}`, {
							headers,
							signal: AbortSignal.timeout(8_000),
						}).then((r) =>
							r.ok
								? (r.json() as Promise<CivilianResp>)
								: Promise.reject(new Error(`HTTP ${r.status}`)),
						),
				type === "civilian"
					? Promise.resolve(null)
					: fetch(`${base}/api/military/v1/list-military-flights?${bboxQ}&page_size=100`, {
							headers,
							signal: AbortSignal.timeout(8_000),
						}).then((r) =>
							r.ok ? (r.json() as Promise<MilResp>) : Promise.reject(new Error(`HTTP ${r.status}`)),
						),
			]);
			...
			if (!civOk && !milOk)
				throw new Error("Airspace data unavailable: both civilian and military sources failed");
			...
			return {
				...
				updated_at: civ?.updated_at
					? new Date(civ.updated_at).toISOString()
	...
	},
	{
		...
		_execute: async (params, base, apiKey) => {
			...
			if (!bbox)
				return {
					error: `Unknown country code: ${code}. Use ISO 3166-1 alpha-2 (e.g. "AE", "SA", "JP").`,
				};
			const [sw_lat, sw_lon, ne_lat, ne_lon] = bbox;
			const bboxQ = `sw_lat=${sw_lat}&sw_lon=${sw_lon}&ne_lat=${ne_lat}&ne_lon=${ne_lon}`;
			...
			return {
				...
				snapshot_at: snap.snapshot_at
					? new Date(snap.snapshot_at).toISOString()
					: new Date().toISOString(),
		...
		_execute: async (params, base, apiKey) => {
			const res = await fetch(`${base}/api/intelligence/v1/deduct-situation`, {
				...
				body: JSON.stringify({
					...
					geoContext: String(params.context ?? ""),
					framework: String(params.framework ?? ""),
				}),
		...
		inputSchema: {
			...
			properties: {
				...
				trip_duration: {
					type: "number",
					description:
						"Trip duration in days — required when is_round_trip is true (e.g. 7 for a one-week trip)",
				},
				cabin_class: {
					type: "string",
					description:
						'Cabin class: "economy", "premium_economy", "business", or "first" (optional)',
				},
				passengers: { type: "number", description: "Number of passengers (1-9, default 1)" },
				sort_by_price: {
					type: "boolean",
...
export function evaluateFreshness(
	...
	for (const [i, check] of checks.entries()) {
		const meta = metas[i];
		const fetchedAt =
			meta && typeof meta === "object" && "fetchedAt" in meta
...
export default async function handler(req: Request): Promise<Response> {
	...
	if (req.method !== "POST") {
		return new Response(null, {
			status: 405,
			headers: { Allow: "POST, HEAD, OPTIONS", ...corsHeaders },
		});
	}
			...
			return new Response(
				...
				}),
				{
					...
					headers: {
						"Content-Type": "application/json",
						"WWW-Authenticate":
							'Bearer realm="worldmonitor", error="invalid_token", resource_metadata="https://api.worldmonitor.app/.well-known/oauth-protected-resource"',
						...corsHeaders,
					},
				},
			);
			...
			return new Response(
				...
				}),
				{
					...
					headers: {
						"Content-Type": "application/json",
						"WWW-Authenticate":
							'Bearer realm="worldmonitor", resource_metadata="https://api.worldmonitor.app/.well-known/oauth-protected-resource"',
						...corsHeaders,
					},
				},
			);
