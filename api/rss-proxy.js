...
export default async function handler(req) {
	...
	try {
		...
		if (isRelayOnly) {
			...
			usedRelay = !!response;
			if (!response)
				throw new Error(`Railway relay unavailable for relay-only domain: ${hostname}`);
		} else {
			try {
		...
		return new Response(data, {
			...
			headers: {
				...
				...(isSuccess && {
					"CDN-Cache-Control": `public, s-maxage=${cdnTtl}, stale-while-revalidate=${swr}, stale-if-error=${sie}`,
				}),
				...corsHeaders,
			},
		});
