...
export default async function handler(req: Request): Promise<Response> {
		...
		return new Response(null, {
			...
			headers: {
				...
				"Access-Control-Allow-Headers":
					"Content-Type, Authorization, X-WorldMonitor-Key, X-Widget-Key, X-Pro-Key",
			},
		});
	}
	...
	if (!WIDGET_AGENT_KEY && !PRO_WIDGET_KEY) {
		return json(
			{ error: "Widget agent unavailable", ok: false, widgetKeyConfigured: false },
			503,
			corsHeaders,
		);
	}
	...
	try {
		...
		/* malformed body — relay will return 400 */
	}
