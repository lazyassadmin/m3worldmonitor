...
async function timingSafeEqual(a, b) {
	...
	const key = await crypto.subtle.importKey("raw", aBuf, { name: "HMAC", hash: "SHA-256" }, false, [
		"sign",
	]);
...
export default async function handler(req) {
	...
	if (!hasKeys && !hasPatterns) {
		return jsonResponse(
			{ error: 'At least one of "keys" or "patterns" required' },
			422,
			corsHeaders,
		);
	}
	...
	if (hasPatterns) {
		for (const p of patterns) {
			if (typeof p !== "string" || !p.endsWith("*") || p === "*") {
				return jsonResponse(
					{ error: `Invalid pattern "${p}": must end with "*" and cannot be bare "*"` },
					422,
					corsHeaders,
				);
			}
	...
	if (hasPatterns) {
		for (const p of patterns) {
			...
			for (const rawKey of scan.keys) {
				const unprefixed =
					prefix && rawKey.startsWith(prefix) ? rawKey.slice(prefix.length) : rawKey;
	...
	if (dryRun) {
		...
		return jsonResponse(
			{ matched: keyList.length, deleted: 0, keys: keyList, dryRun: true, truncated },
			200,
			corsHeaders,
		);
	}
	...
	if (keyList.length === 0) {
		...
		return jsonResponse(
			{ matched: 0, deleted: 0, keys: [], dryRun: false, truncated },
			200,
			corsHeaders,
		);
	}
	...
	return jsonResponse(
		{ matched: keyList.length, deleted, keys: keyList, dryRun: false, truncated },
		200,
		corsHeaders,
	);
}
