...
const SEED_META = {
	...
	consumerPricesCategories: {
		key: "seed-meta:consumer-prices:categories:ae:30d",
		maxStaleMin: 1500,
	},
	consumerPricesMovers: { key: "seed-meta:consumer-prices:movers:ae:30d", maxStaleMin: 1500 },
	consumerPricesSpread: {
		key: "seed-meta:consumer-prices:retailer-spread:ae:essentials-ae",
		maxStaleMin: 1500,
	},
...
export default async function handler(req) {
	...
	const allDataKeys = [...Object.values(BOOTSTRAP_KEYS), ...Object.values(STANDALONE_KEYS)];
	...
	const onDemandWarnCount = Object.values(checks).filter(
		(c) => c.status === "EMPTY_ON_DEMAND",
	).length;
	...
	if (overall !== "HEALTHY" && overall !== "WARNING") {
		const problemKeys = Object.entries(checks)
			.filter(
				([, c]) => c.status === "EMPTY" || c.status === "EMPTY_DATA" || c.status === "STALE_SEED",
			)
