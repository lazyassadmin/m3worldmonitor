...
async function publishWelcome(userId: string, channelType: string): Promise<void> {
	if (!UPSTASH_URL || !UPSTASH_TOKEN) {
		console.error(
			"[notification-channels] publishWelcome: UPSTASH env vars missing — welcome not queued",
		);
		return;
	}
	...
	try {
		...
		console.log(
			`[notification-channels] publishWelcome LPUSH: status=${res.status} result=${JSON.stringify(data?.result)}`,
		);
	} catch (err) {
	...
	if (req.method === "POST") {
		...
		try {
			if (action === "create-pairing-token") {
				...
				if (!resp.ok) {
					console.error(
						"[notification-channels] POST create-pairing-token relay error:",
						resp.status,
					);
			...
			if (action === "set-digest-settings") {
				...
				if (!resp.ok) {
					console.error(
						"[notification-channels] POST set-digest-settings relay error:",
						resp.status,
					);
					return json({ error: "Operation failed" }, 500, corsHeaders);
				}
