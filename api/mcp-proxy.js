						...
						if (line.startsWith("event: ")) {
							...
							if (eventType === "endpoint") {
								...
								try {
									...
									self._endpointDeferred.reject(
										new Error("SSE endpoint event contains invalid URL"),
									);
									return;
								}
...
async function mcpCallToolSse(serverUrl, toolName, toolArgs, customHeaders) {
	...
	try {
		...
		if (initResp.error) throw new Error(`Initialize error: ${initResp.error.message}`);
		await session.notify("notifications/initialized", {});
		const callResp = await session.send(2, "tools/call", {
			name: toolName,
			arguments: toolArgs || {},
		});
...
export default async function handler(req) {
	...
	try {
		...
		return jsonResponse(
			{ error: isTimeout ? "MCP server timed out" : msg },
			isTimeout ? 504 : 422,
			cors,
		);
	}
