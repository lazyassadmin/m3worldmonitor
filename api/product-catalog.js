...
function buildTiers(dodoPrices) {
	...
	for (const group of PUBLIC_TIER_GROUPS) {
		...
		const monthlyEntry = Object.entries(CATALOG).find(
			([, v]) => v.tierGroup === group && v.billingPeriod === "monthly",
		);
		const annualEntry = Object.entries(CATALOG).find(
		...
		if (monthlyEntry) {
			...
			if (monthlyPrice) {
				...
				console.warn(
					`[product-catalog] FALLBACK price for ${monthlyId} ($${tier.monthlyPrice}) — Dodo fetch failed`,
				);
			}
		...
		if (annualEntry) {
			...
			if (annualPrice) {
				...
				console.warn(
					`[product-catalog] FALLBACK price for ${annualId} ($${tier.annualPrice}) — Dodo fetch failed`,
				);
			}
...
export default async function handler(req) {
	...
	if (DODO_API_KEY) {
		...
		const pricedPublicIds = Object.entries(CATALOG)
			.filter(
				([, v]) =>
					PUBLIC_TIER_GROUPS.includes(v.tierGroup) &&
					v.tierGroup !== "free" &&
					v.tierGroup !== "enterprise",
			)
			.map(([id]) => id);
	...
	const now = Date.now();
	return json(
		{ tiers, fetchedAt: now, cachedUntil: now + 60_000, priceSource: "fallback" },
		200,
		cors,
