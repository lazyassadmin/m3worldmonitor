...
async function fetchEonet(days) {
	...
	for (const event of data.events || []) {
		...
		if (normalizedCategory === "wildfires" && now - eventDate.getTime() > WILDFIRE_MAX_AGE_MS)
			continue;
...
function parseGdacsTcFields(props) {
	...
	const desc =
		String(props.description || "") + " " + String(props.severitydata?.severitytext || "");
...
async function fetchGdacs() {
	...
	for (const f of features) {
		...
		const alertPrefix =
			props.alertlevel === "Red" ? "\u{1F534} " : props.alertlevel === "Orange" ? "\u{1F7E0} " : "";
...
async function fetchNhc() {
	...
	for (const { slot, points, cone, pastPts } of stormData) {
		...
		const forecastTrack = points.features
			...
			.sort(
				(a, b) =>
					(a.properties.tau || a.properties.fcstprd) - (b.properties.tau || b.properties.fcstprd),
			)
			.map((f) => ({
