...
const CCY =
	"USD|GBP|EUR|JPY|CHF|CNY|INR|AUD|CAD|NZD|BRL|MXN|ZAR|TRY|KRW|SGD|HKD|TWD|THB|IDR|NOK|SEK|DKK|PLN|CZK|HUF|RON|PHP|VND|MYR|PKR|ILS|ARS|COP|CLP|UAH|NGN|KES|AED|SAR|QAR|KWD|BHD|OMR|EGP|JOD|LBP";
...
async function searchExa(query, includeDomains = null) {
	...
	const body = {
		...
		type: "auto",
		contents: {
			summary: { query: "What is the current Big Mac price in local currency and USD?" },
		},
	};
...
async function fetchBigMacPrices(prevSnapshot) {
	...
	for (const country of COUNTRIES) {
		...
		if (usdPrice !== null && (usdPrice < USD_MIN || usdPrice > USD_MAX)) {
			console.warn(
				`  [PRICE] ANOMALY ${country.flag} ${country.name}: $${usdPrice} out of range [$${USD_MIN}-$${USD_MAX}] — dropping price`,
			);
			usdPrice = null;
			localPrice = null;
		}
	...
	const cheapest = withData.length
		? withData.reduce((a, b) => (a.usdPrice < b.usdPrice ? a : b)).code
		: "";
	...
	if (wowAvailable) {
		...
		for (const r of results) {
			if (r.usdPrice != null && prevMap[r.code] != null && prevMap[r.code] > 0) {
				...
				if (Math.abs(raw) > WOW_ANOMALY_THRESHOLD) {
					console.warn(
						`  [WoW] ANOMALY ${r.flag} ${r.name}: ${raw}% (prev=$${prevMap[r.code]} now=$${r.usdPrice}) — hiding WoW for this country`,
					);
					suspiciousCount++;
		...
		if (suspiciousCount > 0) {
			console.error(
				`  [WoW] ADMIN ALERT: ${suspiciousCount} country/ies had anomalous WoW (>±${WOW_ANOMALY_THRESHOLD}%): ${suspiciousNames}`,
			);
		}
		...
		const rawAvg =
			rawWowValues.length > 0
				? +(rawWowValues.reduce((s, v) => s + v, 0) / rawWowValues.length).toFixed(2)
				: 0;
		if (Math.abs(rawAvg) > WOW_ANOMALY_THRESHOLD) {
			console.error(
				`  [WoW] ADMIN ALERT: Global WoW raw avg ${rawAvg}% exceeds ±${WOW_ANOMALY_THRESHOLD}% — disabling WoW entirely, likely systematic data bug`,
			);
			wowAvailable = false;
		}
	}
