...
async function main() {
	...
	if (resumedCount > 0) {
		console.log(
			`Resuming from checkpoint: ${resumedCount} pages already fetched (${allRows.length} rows)`,
		);
	}
	...
	for (let page = 0; page < TOTAL_PAGES; page++) {
		...
		try {
			...
			console.log(
				`  Page ${page}: fetched ${data.length} rows (range ${rangeStart}-${rangeEnd}, total so far: ${allRows.length})`,
			);
	...
	if (validRows.length < MIN_EXPECTED_ROWS) {
		console.warn(
			`  WARNING: Valid row count (${validRows.length}) is below expected minimum (${MIN_EXPECTED_ROWS}).`,
		);
	} else {
