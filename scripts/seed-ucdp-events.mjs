...
async function main() {
	...
	if (capped.length === 0) {
		console.warn(
			`  0 events after processing — extending existing key TTL (preserving last good data)`,
		);
		try {
	...
	if (mapped[0]) {
		console.log(
			`  Newest: ${new Date(mapped[0].dateStart).toISOString().slice(0, 10)} — ${mapped[0].country}`,
		);
	}
	console.log();
	...
	if (getResp.ok) {
		...
		if (getData.result) {
			...
			console.log(
				`  Version: ${parsed.version} | fetchedAt: ${new Date(parsed.fetchedAt).toISOString()}`,
			);
		}
	}
