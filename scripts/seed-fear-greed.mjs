...
async function fetchCBOE() {
	try {
		...
		if (!resp.ok) {
			console.warn(`  Barchart $CPC: HTTP ${resp.status}`);
			return {};
		}
		...
		console.warn(`  Barchart $CPC: ${e.message}`);
		return {};
	}
}
...
async function fetchBarchartS5TH() {
	try {
		...
		if (!resp.ok) {
			console.warn(`  Barchart $S5TH: HTTP ${resp.status}`);
			return null;
		}
		const html = await resp.text();
		const block = html.match(/<script id="__NEXT_DATA__"[^>]*>([\s\S]*?)<\/script>/)?.[1] ?? html;
		...
		console.warn("  Barchart $S5TH fetch failed:", e.message);
		return null;
	}
}
...
async function fetchCNN() {
	try {
		...
		if (!resp.ok) {
			console.warn(`  CNN F&G: HTTP ${resp.status}`);
			return null;
		}
		...
		const rating =
			typeof rawRating === "string" && VALID_CNN_LABELS.has(rawRating) ? rawRating : null;
		return score != null
			? { score: Math.round(score), label: rating ?? labelFromScore(Math.round(score)) }
			: null;
	} catch (e) {
		console.warn(`  CNN F&G: ${e.message}`);
		return null;
	}
}
...
async function fetchAAII() {
	try {
		...
		const pcts = [...html.matchAll(/<td[^>]*class="tableTxt"[^>]*>([\d.]+)%/g)].map((m) =>
			parseFloat(m[1]),
		);
	...
	switch (name) {
		case "sentiment": {
			...
			return {
				...
				inputs: {
					cnnFearGreed: cnnFg,
					aaiBull: aaiBull ?? null,
					aaiBear: aaiBear ?? null,
					cryptoFg,
				},
			};
		}
		case "volatility": {
			...
			const termStructure =
				vix3m != null ? (vix / vix3m < 1 ? "contango" : "backwardation") : "unknown";
			return {
				score: Math.round(vixScore * 0.7 + termScore * 0.3),
				inputs: { vix, vix9d, vix3m, termStructure },
			};
		}
		case "positioning": {
			...
			return {
				score: Math.round(pcScore * w[0] + skewScore * w[1]),
				inputs: { putCallRatio: pc, skew },
			};
		}
		case "trend": {
			const { prices } = inputs;
			...
			return {
				...
				inputs: { spxPrice: price, sma20: s20, sma50: s50, sma200: s200, aboveMaCount: aboveCount },
			};
		}
		case "breadth": {
			...
			const rspRoc =
				rspCloses?.length && spyCloses?.length
					? (roc(rspCloses, 30) ?? 0) - (roc(spyCloses, 30) ?? 0)
					: null;
			...
			return {
				score: Math.round(clamp(score, 0, 100)),
				degraded: mmthPrice == null,
				inputs: { pctAbove200d: mmthPrice, rspSpyRatio: rspRoc, advDecRatio: advDecRatio ?? null },
			};
		}
		case "momentum": {
			...
			const sectorRsiValues = sectorCloses
				? Object.values(sectorCloses)
						.filter(Boolean)
						.map((c) => rsi(c))
				: [];
			const avgRsi = sectorRsiValues.length
				? sectorRsiValues.reduce((a, b) => a + b, 0) / sectorRsiValues.length
				: 50;
			const rsiScore = clamp(((avgRsi - 30) / 40) * 100, 0, 100);
			return {
				score: Math.round(rsiScore * 0.5 + rocScore * 0.5),
				inputs: { spxRoc20d: spxRoc, sectorRsiAvg: Math.round(avgRsi) },
			};
		}
		case "liquidity": {
			...
			const m2Latest = fredLatest(m2Obs),
				m2Ago = fredNMonthsAgo(m2Obs, 52);
			...
			const walclLatest = fredLatest(walclObs),
				walclAgo = fredNMonthsAgo(walclObs, 4);
			const fedBsMom =
				walclLatest && walclAgo && walclAgo !== 0
					? ((walclLatest - walclAgo) / walclAgo) * 100
					: null;
			...
			return {
				score: Math.round(m2Score * 0.4 + fedScore * 0.3 + sofrScore * 0.3),
				inputs: { m2Yoy, fedBsMom, sofr },
			};
		}
		case "credit": {
			const { hyObs, igObs } = inputs;
			const hySpread = fredLatest(hyObs),
				igSpread = fredLatest(igObs);
			...
			return {
				score: Math.round(hyScore * 0.4 + igScore * 0.3 + trendScore * 0.3),
				inputs: { hySpread, igSpread, hyTrend30d: hyTrend },
			};
		}
		case "macro": {
			const { fedObs, curveObs, unrateObs } = inputs;
			const fedRate = fredLatest(fedObs),
				t10y2y = fredLatest(curveObs),
				unrate = fredLatest(unrateObs);
			const rateScore = fedRate != null ? clamp(100 - fedRate * 15, 0, 100) : 50;
			const curveScore =
				t10y2y != null
					? t10y2y > 0
						? clamp(60 + t10y2y * 20, 0, 100)
						: clamp(40 + t10y2y * 40, 0, 100)
					: 50;
			const unempScore = unrate != null ? clamp(100 - (unrate - 3.5) * 20, 0, 100) : 50;
			return {
				score: Math.round(rateScore * 0.3 + curveScore * 0.4 + unempScore * 0.3),
				inputs: { fedRate, t10y2y, unrate },
			};
		}
		case "crossAsset": {
			...
			return {
				score: Math.round((goldSignal + bondSignal + dxySignal) / 3),
				inputs: {
					goldReturn30d: goldRoc,
					tltReturn30d: tltRoc,
					spyReturn30d: spyRoc,
					dxyChange30d: dxyRoc,
				},
...
async function fetchAll() {
	...
	const [yahooResults, cboeResult, cnnResult, aaiiResult, macroSignals, barchartResult] =
	...
	console.log(
		`  Sources: Yahoo=${yahooCount}/${YAHOO_SYMBOLS.length} | putCall=${cboe.totalPc ?? "null"} | CNN=${cnn ? cnn.score : "null"} | AAII bull=${aaii ? aaii.bull : "null"} | Barchart=$S5TH=${barchartResult.status === "fulfilled" ? (barchartResult.value ?? "null") : "err"} | proxy=${_proxyAuth ? "yes" : "no"}`,
	);

	if (yahooResults.status === "rejected")
		console.warn("  Yahoo batch failed:", yahooResults.reason?.message);
	...
	if (barchartResult.status === "fulfilled" && barchartResult.value == null)
		console.warn("  Barchart $S5TH: unavailable (using RSP/SPY proxy if possible)");

	const [hyObs, igObs, m2Obs, walclObs, sofrObs, fedObs, curveObs, unrateObs, vixObs, dgs10Obs] =
		await Promise.all([
			readFred("BAMLH0A0HYM2"),
			readFred("BAMLC0A0CM"),
			readFred("M2SL"),
			readFred("WALCL"),
			readFred("SOFR"),
	...
	const cats = {
		sentiment: scoreCategory("sentiment", {
			cnnFg: cnn?.score ?? null,
			aaiBull: aaii?.bull ?? null,
			aaiBear: aaii?.bear ?? null,
			cryptoFg,
		}),
		volatility: scoreCategory("volatility", { vix: vixLive, vix9d: vix9dPrice, vix3m: vix3mPrice }),
		positioning: scoreCategory("positioning", {
			totalPc: cboe.totalPc,
			equityPc: cboe.equityPc,
			skew: skewPrice,
		}),
		trend: scoreCategory("trend", { prices: gspc?.closes ?? [] }),
		breadth: scoreCategory("breadth", {
			mmthPrice: pctAbove200d,
			rspCloses: rsp?.closes,
			spyCloses: spy?.closes,
			advDecRatio: null,
		}),
		momentum: scoreCategory("momentum", {
			spxCloses: gspc?.closes,
			sectorCloses: {
				XLK: xlk?.closes,
				XLF: xlf?.closes,
				XLE: xle?.closes,
		...
		crossAsset: scoreCategory("crossAsset", {
			gldCloses: gld?.closes,
			tltCloses: tlt?.closes,
			spyCloses: spy?.closes,
			dxyCloses: dxy?.closes,
		}),
	};
	...
	const sectorPerformance = Object.entries(SECTOR_ETF_NAMES)
		.map(([sym, name]) => {
			const d = yahoo[sym];
			if (!d?.closes || d.closes.length < 2) return null;
			const prev = d.closes.at(-2),
				curr = d.closes.at(-1);
	...
	const payload = {
		...
		categories: {
			sentiment: {
				score: cats.sentiment.score,
				weight: WEIGHTS.sentiment,
				contribution: Math.round(cats.sentiment.score * WEIGHTS.sentiment * 10) / 10,
				inputs: cats.sentiment.inputs,
				degraded: cats.sentiment.degraded ?? false,
			},
			volatility: {
				score: cats.volatility.score,
				weight: WEIGHTS.volatility,
				contribution: Math.round(cats.volatility.score * WEIGHTS.volatility * 10) / 10,
				inputs: cats.volatility.inputs,
			},
			positioning: {
				score: cats.positioning.score,
				weight: WEIGHTS.positioning,
				contribution: Math.round(cats.positioning.score * WEIGHTS.positioning * 10) / 10,
				inputs: cats.positioning.inputs,
			},
			trend: {
				score: cats.trend.score,
				weight: WEIGHTS.trend,
				contribution: Math.round(cats.trend.score * WEIGHTS.trend * 10) / 10,
				inputs: cats.trend.inputs,
			},
			breadth: {
				score: cats.breadth.score,
				weight: WEIGHTS.breadth,
				contribution: Math.round(cats.breadth.score * WEIGHTS.breadth * 10) / 10,
				inputs: cats.breadth.inputs,
				degraded: cats.breadth.degraded ?? false,
			},
			momentum: {
				score: cats.momentum.score,
				weight: WEIGHTS.momentum,
				contribution: Math.round(cats.momentum.score * WEIGHTS.momentum * 10) / 10,
				inputs: cats.momentum.inputs,
			},
			liquidity: {
				score: cats.liquidity.score,
				weight: WEIGHTS.liquidity,
				contribution: Math.round(cats.liquidity.score * WEIGHTS.liquidity * 10) / 10,
				inputs: cats.liquidity.inputs,
			},
			credit: {
				score: cats.credit.score,
				weight: WEIGHTS.credit,
				contribution: Math.round(cats.credit.score * WEIGHTS.credit * 10) / 10,
				inputs: cats.credit.inputs,
			},
			macro: {
				score: cats.macro.score,
				weight: WEIGHTS.macro,
				contribution: Math.round(cats.macro.score * WEIGHTS.macro * 10) / 10,
				inputs: cats.macro.inputs,
			},
			crossAsset: {
				score: cats.crossAsset.score,
				weight: WEIGHTS.crossAsset,
				contribution: Math.round(cats.crossAsset.score * WEIGHTS.crossAsset * 10) / 10,
				inputs: cats.crossAsset.inputs,
			},
		},
		headerMetrics: {
