...
async function fetchEcbCiss() {
	...
	if (!obsMap || typeof obsMap !== "object")
		throw new Error("ECB CISS: no observations in response");

	const timeDim = json?.structure?.dimensions?.observation?.[0]?.values;
	if (!Array.isArray(timeDim) || timeDim.length === 0)
		throw new Error("ECB CISS: missing time dimension values");
	...
	console.log(
		`  ECB CISS: latest=${latestValue.toFixed(4)} (${latestDate}) label=${label} points=${history.length}`,
	);
