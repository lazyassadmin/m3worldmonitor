...
async function fetchEcbSeries(def) {
	...
	console.log(
		`  ${def.id}: ${observations.length} observations (latest: ${observations.at(-1)?.date} = ${observations.at(-1)?.value}%)`,
	);
	return observations;
}
...
async function main() {
	...
	const lockResult = await acquireLockSafely(`${domain}:${resource}`, runId, 300_000, {
