...
const CONVEX_SITE_URL =
	process.env.CONVEX_SITE_URL ?? CONVEX_URL.replace(".convex.cloud", ".convex.site");
...
if (!UPSTASH_URL || !UPSTASH_TOKEN) {
	console.error("[relay] UPSTASH_REDIS_REST_URL/TOKEN not set");
	process.exit(1);
}
if (!CONVEX_URL) {
	console.error("[relay] CONVEX_URL not set");
	process.exit(1);
}
if (!RELAY_SECRET) {
	console.error("[relay] RELAY_SHARED_SECRET not set");
	process.exit(1);
}
...
async function drainHeldForUser(userId, variant, allowedChannelTypes) {
	...
	const lines = [
		`WorldMonitor — ${events.length} held alert${events.length !== 1 ? "s" : ""} from quiet hours`,
		"",
	];
	...
	for (const ch of verifiedChannels) {
		try {
			...
			if (ch.channelType === "telegram" && ch.chatId)
				ok = await sendTelegram(userId, ch.chatId, text);
			else if (ch.channelType === "slack" && ch.webhookEnvelope)
				ok = await sendSlack(userId, ch.webhookEnvelope, text);
			else if (ch.channelType === "discord" && ch.webhookEnvelope)
				ok = await sendDiscord(userId, ch.webhookEnvelope, text);
			else if (ch.channelType === "email" && ch.email)
				ok = await sendEmail(ch.email, subject, text);
			if (ok) anyDelivered = true;
		} catch (err) {
			console.warn(
				`[relay] drainHeldForUser: delivery error for ${userId}/${ch.channelType}:`,
				err.message,
			);
		}
	}
	if (anyDelivered) {
		await upstashRest("DEL", key);
		console.log(
			`[relay] drainHeldForUser: delivered ${events.length} held events to ${userId} (${variant})`,
		);
	}
}
...
async function drainBatchOnWake() {
	...
	const batchRules = allRules.filter(
		(r) => r.quietHoursEnabled && r.quietHoursOverride === "batch_on_wake" && !isInQuietHours(r),
	);
...
async function processFlushQuietHeld(event) {
	...
	try {
		...
		console.warn(
			`[relay] flush_quiet_held: could not fetch rule for ${userId} — held alerts preserved until drain:`,
			err.message,
		);
		return;
	}
	...
	if (!allowedChannels) {
		console.log(
			`[relay] flush_quiet_held: no active rule with channels for ${userId} (${variant}) — held alerts preserved`,
		);
		return;
	}
...
async function sendTelegram(userId, chatId, text) {
	...
	if (res.status === 403 || res.status === 400) {
		...
		console.warn(
			`[relay] Telegram ${res.status} for ${userId}: ${body.description ?? "(no description)"}`,
		);
	...
	if (res.status === 401) {
		console.error(
			"[relay] Telegram 401 Unauthorized — TELEGRAM_BOT_TOKEN is invalid or belongs to a different bot; correct the Railway env var to restore Telegram delivery",
		);
		return false;
	}
	if (!res.ok) {
...
async function sendDiscord(userId, webhookEnvelope, text, retryCount = 0) {
	...
	const content =
		text.length > DISCORD_MAX_CONTENT ? text.slice(0, DISCORD_MAX_CONTENT - 1) + "…" : text;
	const res = await fetch(webhookUrl, {
		method: "POST",
		headers: { "Content-Type": "application/json", "User-Agent": "worldmonitor-relay/1.0" },
...
async function sendEmail(email, subject, text) {
	if (!resend) {
		console.warn("[relay] RESEND_API_KEY not set — skipping email");
		return false;
	}
	try {
...
function shouldNotify(rule, event) {
	...
	if (process.env.IMPORTANCE_SCORE_LIVE === "1" && event.payload?.importanceScore != null) {
		const threshold = rule.sensitivity === "critical" ? 85 : rule.sensitivity === "high" ? 65 : 40; // 'all'
...
function formatMessage(event) {
	const parts = [
		`[${(event.severity ?? "high").toUpperCase()}] ${event.payload?.title ?? event.eventType}`,
	];
...
async function processEvent(event) {
	if (event.eventType === "channel_welcome") {
		await processWelcome(event);
		return;
	}
	if (event.eventType === "flush_quiet_held") {
		await processFlushQuietHeld(event);
		return;
	}
	...
	if (IMPORTANCE_SCORE_LIVE && event.eventType === "rss_alert") {
		...
		if (score < IMPORTANCE_SCORE_MIN) {
			console.log(
				`[relay] Score gate: dropped ${event.eventType} score=${score} < ${IMPORTANCE_SCORE_MIN}`,
			);
			return;
		}
	}
	...
	const matching = enabledRules.filter(
		(r) =>
	...
	for (const rule of matching) {
		...
		if (quietAction === "suppress") {
			console.log(
				`[relay] Quiet hours suppress for ${rule.userId} (severity=${eventSeverity}, override=${rule.quietHoursOverride ?? "critical_only"})`,
			);
			continue;
		}
		...
		if (quietAction === "hold") {
			...
			if (!isNew) {
				console.log(`[relay] Dedup hit (held) for ${rule.userId}`);
				continue;
			}
		...
		const isNew = await checkDedup(rule.userId, event.eventType, event.payload?.title ?? "");
		if (!isNew) {
			console.log(`[relay] Dedup hit for ${rule.userId}`);
			continue;
		}
		...
		const verifiedChannels = channels.filter(
			(c) => c.verified && rule.channels.includes(c.channelType),
		);
		...
		for (const ch of verifiedChannels) {
			try {
				...
				console.warn(
					`[relay] Delivery error for ${rule.userId}/${ch.channelType}:`,
	...
	while (true) {
		try {
			...
			if (nowMs - lastDrainMs >= DRAIN_INTERVAL_MS) {
				lastDrainMs = nowMs;
				drainBatchOnWake().catch((err) =>
					console.warn("[relay] drainBatchOnWake error:", err.message),
				);
			}
