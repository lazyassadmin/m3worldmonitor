...
const R2_BUCKET_URL =
	"https://api.cloudflare.com/client/v4/accounts/{acct}/r2/buckets/worldmonitor-data/objects/seed-data/military-bases-final.json";
const MAX_RETRIES = 3;
...
async function pipelineRequest(url, token, commands, attempt = 1) {
	...
	if (!resp.ok) {
		...
		if (attempt < MAX_RETRIES) {
			...
			console.warn(
				`  Pipeline failed (HTTP ${resp.status}), retry ${attempt}/${MAX_RETRIES} in ${delay}ms...`,
			);
			await sleep(delay);
			return pipelineRequest(url, token, commands, attempt + 1);
		}
		throw new Error(
			`Pipeline failed after ${MAX_RETRIES} attempts: HTTP ${resp.status} — ${text.slice(0, 200)}`,
		);
	}
...
async function validate(url, token, prefix, version, expectedCount) {
	...
	const hmgetResult = await pipelineRequest(url, token, [["HMGET", metaKey, ...sampleIds]]);
...
async function main() {
	...
	if (invalid.length > 0) {
		console.error(
			`Found ${invalid.length} entries missing id/lat/lon. First: ${JSON.stringify(invalid[0])}`,
		);
		process.exit(1);
	}
