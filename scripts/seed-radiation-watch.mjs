...
const EPA_SITES = [
	...
	{
		anchorId: "us-san-francisco",
		state: "CA",
		slug: "SAN%20FRANCISCO",
		name: "San Francisco",
		country: "United States",
		lat: 37.7749,
		lon: -122.4194,
...
const SAFECAST_SITES = [
	...EPA_SITES.map(({ anchorId, name, country, lat, lon }) => ({
		anchorId,
		name,
		country,
		lat,
...
function toEpaObservation(site, readings) {
	...
	const sigma =
		baselineValues.length >= BASELINE_MIN_SAMPLES ? stdDev(baselineValues, baselineValue) : 0;
...
function toSafecastObservation(site, measurements) {
	...
	const baselineValue = baselineValues.length > 0 ? average(baselineValues) : latest.value;
	const sigma =
		baselineValues.length >= SAFECAST_MIN_SAMPLES ? stdDev(baselineValues, baselineValue) : 0;
	const delta = latest.value - baselineValue;
	const zScore = sigma > 0 ? delta / sigma : 0;
	const severity = classifySeverity(delta, zScore, freshness);
...
function baseConfidence(observation) {
	...
	if (observation._directUnit && observation._baselineSamples >= SAFECAST_MIN_SAMPLES)
		return "RADIATION_CONFIDENCE_MEDIUM";
	return "RADIATION_CONFIDENCE_LOW";
}
...
function supportsSameSignal(primary, secondary) {
	if (
		primary.severity === "RADIATION_SEVERITY_NORMAL" &&
		secondary.severity === "RADIATION_SEVERITY_NORMAL"
	) {
...
function finalizeObservationGroup(group) {
	...
	const corroborated = alternateSources.some((observation) =>
		supportsSameSignal(primary, observation),
	);
	const conflictingSources = alternateSources.some((observation) =>
		materiallyConflicts(primary, observation),
	);
...
function summarizeObservations(observations) {
	...
	return {
		observations: sorted,
		fetchedAt: Date.now(),
		epaCount: sorted.filter((item) =>
			item.contributingSources.includes("RADIATION_SOURCE_EPA_RADNET"),
		).length,
		safecastCount: sorted.filter((item) =>
			item.contributingSources.includes("RADIATION_SOURCE_SAFECAST"),
		).length,
		...
		lowConfidenceCount: sorted.filter((item) => item.confidence === "RADIATION_CONFIDENCE_LOW")
			.length,
	...
	const normalized = (Array.isArray(measurements) ? measurements : [])
		.map((measurement) => {
			...
			return {
				...
				locationName:
					typeof measurement?.location_name === "string" ? measurement.location_name.trim() : "",
				observedAt,
				lat,
				lon,
...
async function fetchRadiationWatch() {
	...
	const capturedAfter = new Date(Date.now() - SAFECAST_LOOKBACK_DAYS * 24 * 60 * 60 * 1000)
		.toISOString()
		.slice(0, 10);
