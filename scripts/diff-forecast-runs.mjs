...
function diffForecastRuns(baselineArtifacts = {}, candidateArtifacts = {}) {
	...
	const candidateTopTitles = new Set(
		(candidateSummary.topForecasts || []).map((item) => item.title),
	);
	...
	return {
		...
		tracedForecastCountDelta: Number(
			(candidateSummary.tracedForecastCount || 0) - (baselineSummary.tracedForecastCount || 0),
		),
		impactExpansionDelta: {
			candidateCount: Number(
				(candidateSummary.worldStateSummary?.impactExpansionCandidateCount || 0) -
					(baselineSummary.worldStateSummary?.impactExpansionCandidateCount || 0),
			),
			mappedSignalCount: Number(
				(candidateSummary.worldStateSummary?.impactExpansionMappedSignalCount || 0) -
					(baselineSummary.worldStateSummary?.impactExpansionMappedSignalCount || 0),
			),
		},
		interactionDelta: {
			simulation: Number(
				(candidateSummary.worldStateSummary?.simulationInteractionCount || 0) -
					(baselineSummary.worldStateSummary?.simulationInteractionCount || 0),
			),
			reportable: Number(
				(candidateSummary.worldStateSummary?.reportableInteractionCount || 0) -
					(baselineSummary.worldStateSummary?.reportableInteractionCount || 0),
			),
		},
		...
		addedTopForecastTitles: [...candidateTopTitles]
			.filter((title) => !baselineTopTitles.has(title))
			.sort(),
		removedTopForecastTitles: [...baselineTopTitles]
			.filter((title) => !candidateTopTitles.has(title))
			.sort(),
...
export { parseArgs, diffNumberMap, diffForecastRuns, diffForecastRunIds };
