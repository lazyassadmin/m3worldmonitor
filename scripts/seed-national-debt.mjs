...
const TREASURY_URL =
	"https://api.fiscaldata.treasury.gov/services/api/v1/accounting/od/debt_to_penny?fields=record_date,tot_pub_debt_out_amt&sort=-record_date&page[size]=1";
...
export function computeEntries(
	...
	for (const [iso3, debtByYear] of Object.entries(debtPctByCountry)) {
		...
		entries.push({
			...
			source:
				iso3 === "USA" && treasuryOverride ? "IMF WEO + US Treasury FiscalData" : "IMF WEO 2024",
		});
	}
