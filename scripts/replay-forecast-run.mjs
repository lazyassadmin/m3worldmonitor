...
function buildEmptyImpactExpansionBundle(snapshot, failureReason) {
	const candidatePackets = Array.isArray(snapshot?.impactExpansionCandidates)
...
async function resolveReplayBundle({ providerMode, artifacts, snapshot, priorWorldState }) {
	if (providerMode === "recorded") {
		return (
			artifacts.impactExpansionDebug?.impactExpansionBundle ||
	...
	const payload = {
		...
		evaluationSummary: evaluation
			? {
					...
					expandedPathCount: (evaluation.selectedPaths || []).filter(
						(path) => path.type === "expanded",
					).length,
					rejectedPathCount: (evaluation.rejectedPaths || []).length,
					mappedSignalCount: Number(
						evaluation.deepWorldState?.impactExpansion?.mappedSignalCount || 0,
					),
				}
			: null,
...
export { parseArgs, buildEmptyImpactExpansionBundle, replayForecastRun };
