...
async function searchExa(query, sites, locationCode) {
	const apiKey = (process.env.EXA_API_KEYS || process.env.EXA_API_KEY || "")
		.split(/[\n,]+/)[0]
		.trim();
	...
	const body = {
		...
		contents: {
			summary: {
				...
				query:
					"What is the retail price of this product? State amount and ISO currency code (e.g. GBP 1.50, EUR 2.99, JPY 193).",
			},
		},
	};
				...
				extract: {
					...
					schema: {
						...
						properties: {
							...
							currency: {
								type: "string",
								description: "ISO 4217 currency code, e.g. SAR, KRW, USD",
							},
						},
...
async function tryDirectFetch(url, expectedCurrency, itemId, fxRate) {
	try {
		...
		if (fxRate && ITEM_USD_MAX[itemId] && hit.price * fxRate > ITEM_USD_MAX[itemId]) {
			console.warn(
				`    [learned bulk] ${itemId}: ${hit.price} ${expectedCurrency} ($${(hit.price * fxRate).toFixed(2)}) > max — skipping`,
			);
			return null;
		}
		return hit.price;
...
function extractPrice(result, expectedCurrency) {
	...
	if (summary && typeof summary === "string") {
		...
		if (hit && hit.currency !== expectedCurrency) {
			console.warn(
				`    [extractPrice] currency mismatch: got ${hit.currency}, expected ${expectedCurrency} — ${url}`,
			);
			return null;
		}
		if (hit) return hit;
...
async function fetchGroceryBasketPrices(prevSnapshot) {
	...
	const routeKeys = [
		...config.countries.flatMap((c) => config.items.map((i) => `${c.code}:${i.id}`)),
		OIL_MIGRATION_KEY,
		BAD_PRICES_KEY,
	];
	...
	if (!learnedRoutes.has(OIL_MIGRATION_KEY)) {
		const oilEvictions = new Set(
			config.countries.map((c) => `${c.code}:oil`).filter((k) => learnedRoutes.has(k)),
		);
		if (oilEvictions.size > 0) {
			console.log(
				`  [routes] one-time migration: evicting ${oilEvictions.size} stale oil routes (sunflower → canola)`,
			);
	...
	if (!learnedRoutes.has(BAD_PRICES_KEY)) {
		const knownBad = new Set(
			[
				...config.countries
					.filter((c) => c.code === "JP")
					.flatMap((c) => config.items.map((i) => `JP:${i.id}`)),
				"TR:sugar",
				"TR:eggs",
				"TR:milk",
				"TR:oil",
		...
		if (knownBad.size > 0) {
			console.log(
				`  [routes] one-time eviction: clearing ${knownBad.size} known-bad price routes (JP sites + TR/EG/IN sub-unit)`,
			);
	...
	for (const country of config.countries) {
		...
		const itemPrices = await Promise.all(
			config.items.map(async (item, idx) => {
								...
								for (const result of exaResult.results) {
									...
									if (fxRate && ITEM_USD_MAX[item.id]) {
										...
										if (usdEquiv > ITEM_USD_MAX[item.id]) {
											console.warn(
												`    [bulk] ${item.id}: ${extracted.price} ${country.currency} ($${usdEquiv.toFixed(2)}) > max $${ITEM_USD_MAX[item.id]} — skipping`,
											);
											continue;
										}
									}
						...
						if (exaPrice === null && exaUrls.length > 0) {
							for (const url of exaUrls.slice(0, 2)) {
								...
								if (fxRate && ITEM_USD_MAX[item.id]) {
									...
									if (usdEquiv > ITEM_USD_MAX[item.id]) {
										console.warn(
											`    [FC bulk] ${item.id}: ${fc.price} ${country.currency} ($${usdEquiv.toFixed(2)}) > max — skipping`,
										);
										continue;
									}
								}
				...
				const status =
					localPrice !== null ? `${localPrice} ${country.currency} = $${usdPrice}` : "N/A";
	...
	for (const itemId of itemIds) {
		...
		for (const country of countriesResult) {
			...
			console.warn(
				`  [outlier] ${country.code}/${itemId}: ${reason} — clearing + evicting learned route`,
			);
	...
	const cheapest = rankable.length
		? rankable.reduce((a, b) => (a.totalUsd < b.totalUsd ? a : b)).code
		: "";
	const mostExpensive = rankable.length
		? rankable.reduce((a, b) => (a.totalUsd > b.totalUsd ? a : b)).code
		: "";
	...
	const wowAvailable =
		prevSnapshot?.countries?.length > 0 && prevSnapshot.basketVersion === BASKET_VERSION;
	if (wowAvailable) {
		...
		for (const country of countriesResult) {
			if (country.totalUsd > 0 && prevMap[country.code] != null && prevMap[country.code] > 0) {
				country.wowPct = +(
					((country.totalUsd - prevMap[country.code]) / prevMap[country.code]) *
					100
				).toFixed(2);
			} else {
				country.wowPct = null;
...
await runSeed(
	...
	"grocery-basket",
	CANONICAL_KEY,
	() => fetchGroceryBasketPrices(prevSnapshot),
	{
		ttlSeconds: CACHE_TTL,
		validateFn: (data) => {
			if (!data?.countries?.length) return false;
			const minItems = Math.ceil(config.items.length * 0.4); // 40% item coverage per country
			const covered = data.countries.filter(
				(c) => c.items.filter((i) => i.available).length >= minItems,
			);
			if (covered.length < 5) {
				console.warn(
					`  [validate] only ${covered.length} countries with ≥40% item coverage — rejecting`,
				);
				return false;
			}
			return true;
