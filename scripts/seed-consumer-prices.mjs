...
if (!BASE_URL) {
	console.warn(
		"[consumer-prices] CONSUMER_PRICES_CORE_BASE_URL not set — writing empty placeholders",
	);
}
...
async function run() {
	...
	const [
		...
		fetchSnapshot(
			`/wm/consumer-prices/v1/basket-series?market=${MARKET}&basket=${BASKET}&range=30d`,
		),
		fetchSnapshot(
			`/wm/consumer-prices/v1/basket-series?market=${MARKET}&basket=${BASKET}&range=7d`,
		),
		fetchSnapshot(
			`/wm/consumer-prices/v1/basket-series?market=${MARKET}&basket=${BASKET}&range=90d`,
		),
