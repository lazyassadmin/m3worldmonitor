...
export async function main() {
	...
	try {
		...
		if (countries.size < MIN_COUNTRIES) {
			throw new Error(`OWID: only ${countries.size} countries parsed, expected >=${MIN_COUNTRIES}`);
		}

		const prevMeta = await redisGet(OWID_META_KEY).catch(() => null);
		if (prevMeta && typeof prevMeta === "object" && prevMeta.recordCount > 0) {
			const drop = ((prevMeta.recordCount - countries.size) / prevMeta.recordCount) * 100;
		...
		if (allCountriesCount < MIN_COUNTRIES) {
			throw new Error(`OWID _all: only ${allCountriesCount} entries, expected >=${MIN_COUNTRIES}`);
		}
		...
		commands.push(["SET", OWID_ALL_KEY, JSON.stringify(allCountriesMap), "EX", OWID_TTL_SECONDS]);
		commands.push([
			"SET",
			OWID_META_KEY,
