...
function mergeHistoricalBenchmarks(existingEntries, nextEntry, options = {}) {
	...
	if (index >= 0 && !replace) {
		throw new Error(
			`Benchmark entry "${nextEntry.name}" already exists. Re-run with --replace to overwrite it.`,
		);
	}
...
function renderUnifiedDiff(currentEntries, nextEntries, outputPath) {
	...
	try {
		try {
			const rawDiff = execFileSync("git", ["diff", "--no-index", "--", currentPath, nextPath], {
				encoding: "utf8",
			});
			return rawDiff
...
function parseArgs(argv) {
	...
	for (const arg of argv) {
		if (arg.startsWith("--limit=")) args.limit = Number(arg.split("=")[1] || 60);
			...
			args.maxCandidates = Number(arg.split("=")[1] || 10);
			...
			args.format = arg.split("=").slice(1).join("=") || "entry";
...
function pickCandidate(candidates, options = {}) {
	...
	if (!Number.isInteger(options.index) || options.index < 0 || options.index >= candidates.length) {
		throw new Error(
			`Candidate index ${options.index} is out of range for ${candidates.length} candidate(s).`,
		);
	}
