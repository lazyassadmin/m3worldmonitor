...
export function computeUsAqiFromPm25(pm25) {
	...
	const breakpoint =
		EPA_PM25_BREAKPOINTS.find(({ cHigh }) => concentration <= cHigh) ?? EPA_PM25_BREAKPOINTS.at(-1);
	const ratio = (breakpoint.iHigh - breakpoint.iLow) / (breakpoint.cHigh - breakpoint.cLow);
	return Math.max(
		0,
		Math.min(500, Math.round(ratio * (concentration - breakpoint.cLow) + breakpoint.iLow)),
	);
}
...
function isFreshMeasurement(measuredAt, nowMs = Date.now()) {
	return (
		...
		measuredAt >= nowMs - AIR_QUALITY_WINDOW_MS &&
		measuredAt <= nowMs + 5 * 60 * 1000
	);
}

function pickLocationName(location) {
	return (
		trimString(location?.locality) ||
...
function pickCoordinates(primary, fallback) {
	const lat =
		toFiniteNumber(primary?.latitude ?? primary?.lat) ??
		toFiniteNumber(fallback?.latitude ?? fallback?.lat);
	const lng =
		toFiniteNumber(primary?.longitude ?? primary?.lng) ??
		toFiniteNumber(fallback?.longitude ?? fallback?.lng);
...
function buildLocationMetadata(result, locationIndex) {
	const locationId = toFiniteNumber(
		result?.locationsId ?? result?.locationId ?? result?.location?.id,
	);
	...
	const coordinates = pickCoordinates(
		result?.coordinates,
		indexed?.coordinates ?? inlineLocation?.coordinates,
	);
...
export function buildOpenAqStations(locations = [], latestMeasurements = [], nowMs = Date.now()) {
	...
	for (const result of latestMeasurements) {
		...
		const measuredAt = toEpochMs(
			result?.datetime?.utc ?? result?.datetime?.local ?? result?.date?.utc ?? result?.date?.local,
		);
		...
		const pollutant =
			trimString(result?.parameter?.name) || trimString(result?.parameter) || "pm25";
		...
		if (
			!previous ||
			station.measuredAt > previous.measuredAt ||
			(station.measuredAt === previous.measuredAt && station.pm25 > previous.pm25)
		) {
	...
	return [...latestByLocation.values()].sort(
		(left, right) => right.aqi - left.aqi || right.measuredAt - left.measuredAt,
	);
}
...
export function buildWaqiStations(entries = [], nowMs = Date.now()) {
	...
	for (const entry of entries) {
		...
		if (
			...
			aqi == null ||
			!stationName ||
			!isFreshMeasurement(measuredAt, nowMs)
		)
			continue;
...
function normalizeSupplementalStations({
	waqiStations = [],
	waqiEntries = [],
	nowMs = Date.now(),
}) {
...
export function buildOpenAqHeaders(apiKey = process.env.OPENAQ_API_KEY) {
	...
	if (!trimmedKey) {
		throw new SeedConfigurationError(
			"Missing OPENAQ_API_KEY — OpenAQ v3 requests now require X-API-Key",
		);
	}
	return {
...
export function buildMirrorWriteCommands(
	payload,
	ttlSeconds,
	fetchedAt = Date.now(),
	sourceVersion = OPENAQ_SOURCE_VERSION,
) {
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			const cause = error?.cause
				...
				: "";
			console.warn(
				`  Retry ${attempt + 1}/${maxRetries} in ${wait}ms: ${error?.message ?? error}${cause}`,
			);
...
async function main() {
	...
	const lockResult = await acquireLockSafely(
		`${domain}:${resource}`,
		runId,
		AIR_QUALITY_LOCK_TTL_MS,
		{
	...
	try {
		...
		await extendExistingTtl(
			[HEALTH_AIR_QUALITY_KEY, CLIMATE_AIR_QUALITY_KEY, OPENAQ_META_KEY, CLIMATE_META_KEY],
			CACHE_TTL,
		).catch(() => {});
	...
	try {
		...
		if (verified) {
			...
			console.warn(
				`  WARNING: verification read returned null for one or more mirror keys (${HEALTH_AIR_QUALITY_KEY}, ${CLIMATE_AIR_QUALITY_KEY})`,
			);
		}
