...
const geojson = JSON.parse(
	fs.readFileSync(path.join(root, "public", "data", "countries.geojson"), "utf8"),
);
...
for (const f of geojson.features) {
	...
	if (!/^[A-Z]{2}$/.test(iso2)) {
		if (/^[A-Z]{3}$/.test(iso3)) {
			discrepancies.push(
				`Skipped ${iso3} (${props.name}): invalid ISO2 "${props["ISO3166-1-Alpha-2"]}"`,
			);
		}
		continue;
	}
	if (!/^[A-Z]{3}$/.test(iso3)) {
		discrepancies.push(
			`Skipped ${props.name} (${iso2}): invalid ISO3 "${props["ISO3166-1-Alpha-3"]}"`,
		);
		continue;
	}
