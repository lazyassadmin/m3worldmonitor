...
function processHexes(rawHexes) {
	...
	for (const hex of rawHexes) {
		if (typeof hex.h3Index !== "string") {
			console.error(
				`[gpsjam] WARN: skipping hex with non-string h3Index: ${JSON.stringify(hex).slice(0, 100)}`,
			);
			skipped++;
			continue;
		}
		...
		if (!Number.isInteger(hex.sampleCount) || hex.sampleCount < 0) {
			console.error(
				`[gpsjam] WARN: skipping hex ${hex.h3Index} — invalid sampleCount: ${hex.sampleCount}`,
			);
			skipped++;
			continue;
		}
		if (!Number.isInteger(hex.aircraftCount) || hex.aircraftCount < 0) {
			console.error(
				`[gpsjam] WARN: skipping hex ${hex.h3Index} — invalid aircraftCount: ${hex.aircraftCount}`,
			);
			skipped++;
			continue;
		}
	...
	if (h3Failures > rawHexes.length * 0.5) {
		throw new Error(
			`>50% of hexes failed h3 conversion (${h3Failures}/${rawHexes.length}) — aborting seed`,
		);
	}
	...
	console.error(
		`[gpsjam] Processed ${rawHexes.length} hexes → ${results.length} kept, ${skipped} invalid, ${h3Failures} h3 failures`,
	);
...
async function seedRedis(output) {
	...
	if (getResp.ok) {
		...
		if (getData.result) {
			...
			console.error(
				`[gpsjam] Verified: ${parsed.hexes?.length} hexes in Redis (source: ${parsed.source})`,
			);
		}
	}
...
async function main() {
	...
	try {
		...
		await extendExistingTtl(
			[REDIS_KEY_V2, REDIS_KEY_V1, "seed-meta:intelligence:gpsjam"],
			REDIS_TTL,
		);
		process.exit(0);
	}
	...
	console.error(
		`[gpsjam] ${body.hexes.length} total hexes → ${highCount} high, ${mediumCount} medium`,
	);
