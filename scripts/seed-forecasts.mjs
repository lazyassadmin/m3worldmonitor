...
const THEATER_EXPECTED_ACTORS = {
	...
	"south-china-sea": {
		...
		operators: ["plaaf", "plan", "usaf", "usn"],
	},
	"korea-theater": {
		countries: ["USA", "South Korea", "China", "Japan"],
		operators: ["usaf", "usn", "plaaf"],
	},
	"baltic-theater": {
		countries: ["NATO", "USA", "UK", "Germany"],
		operators: ["nato", "usaf", "raf", "gaf"],
	},
...
const MARKET_BUCKET_CONFIG = [
	...
	},
	{
		...
		signalWeights: {
			shipping_cost_shock: 1.2,
			inflation_impulse: 0.58,
			global_crude_spread_stress: 0.82,
			gas_supply_stress: 0.74,
		},
		edgeWeight: 1,
	},
	{
	...
	},
	{
		...
		signalWeights: {
			shipping_cost_shock: 0.84,
			cyber_cost_repricing: 1.02,
			infrastructure_capacity_loss: 0.9,
		},
		edgeWeight: 0.7,
	},
	{
	...
	},
	{
		...
		signalWeights: {
			fx_stress: 1.15,
			sovereign_stress: 0.82,
			risk_off_rotation: 0.8,
			volatility_shock: 0.88,
			policy_rate_pressure: 0.72,
		},
		edgeWeight: 0.72,
	},
	{
	...
	},
	{
		...
		signalWeights: {
			risk_off_rotation: 0.74,
			fx_stress: 0.84,
			liquidity_expansion: 0.86,
...
const IMPACT_COMMODITY_LEXICON = [
	...
	{ key: "natural_gas", pattern: /\b(gas|natgas|pipeline gas)\b/i },
	{
		...
		pattern: /\b(refined products|diesel|gasoline|jet fuel|fuel oil|naphtha|petrol)\b/i,
	},
	{
		key: "fertilizer",
		pattern: /\b(fertilizer|fertiliser|ammonia|urea|potash|nitrogen|phosphate|npk)\b/i,
	},
	{
		key: "petrochemicals",
		pattern: /\b(petrochemical|petrochemicals|ethylene|propylene|methanol)\b/i,
	},
	{
		key: "food_grains",
		pattern: /\b(wheat|grain|rice|corn|maize|food security|famine|cereal|bread|flour)\b/i,
	},
	{
		key: "shipping_freight",
		pattern:
			/\b(freight rate|charter rate|baltic dry|bulk carrier|dry bulk|tanker rate|hire rate)\b/i,
	},
];
const IMPACT_FACILITY_RE = /\b(lng|terminal|refinery|pipeline|port|field|depot)\b/i;
const SIMULATION_ENERGY_COMMODITY_KEYS = new Set([
	"crude_oil",
	"lng",
	"natural_gas",
	"refined_products",
...
const IMPACT_VARIABLE_CHANNELS = Object.fromEntries(
	Object.entries(IMPACT_VARIABLE_REGISTRY).map(([key, value]) => [
		key,
...
function __setRedisStoreForTests(store) {
...
async function warmPingChokepoints() {
	...
	if (!baseUrl) {
		console.log("  [Chokepoints] Warm-ping skipped (no WM_API_BASE_URL)");
		return;
	}
	try {
		...
		console.warn(`  [Chokepoints] Warm-ping error: ${err.message}`);
	}
}
...
async function readInputKeys() {
	...
	const parse = (i) => {
		try {
			return results[i]?.result ? JSON.parse(results[i].result) : null;
		} catch {
			return null;
		}
	};
	...
	const fredSeries = Object.fromEntries(
		FRED_MARKET_SERIES.map((seriesId) => [
			seriesId,
			parsedByKey[FRED_MARKET_INPUT_KEYS[seriesId]],
		]).filter(([, value]) => value),
	);
...
function selectPrimaryMilitarySurge(_theaterId, surges) {
	...
	return (
		surges.slice().sort((a, b) => {
			const aScore =
				(typePriority[a.surgeType] || 0) * 10 +
				(a.persistent ? 5 : 0) +
...
function canPromoteMilitarySurge(posture, surge) {
	...
	if (surge.strikeCapable || surge.fighters >= 4 || surge.awacs > 0 || surge.tankers > 0)
		return true;
	return false;
}
...
function normalizeCiiEntry(c) {
	...
	const trend = rawTrend.includes("rising")
		? "rising"
		: rawTrend.includes("falling")
			? "falling"
			: "stable";
	const level =
		score >= 81
	...
	return {
		...
		components: { ...c.components, unrest },
	};
}
...
function detectConflictScenarios(inputs, emaRiskScores) {
	...
	const iran = Array.isArray(inputs.iranEvents)
		? inputs.iranEvents
		: inputs.iranEvents?.events || [];
	const ucdp = Array.isArray(inputs.ucdpEvents)
		? inputs.ucdpEvents
		: inputs.ucdpEvents?.events || [];

	for (const c of scores) {
		if (!c.score || c.score <= 60) continue;
		if (c.level !== "high" && c.level !== "critical") continue;

		const signals = [{ type: "cii", value: `${c.name} CII ${c.score} (${c.level})`, weight: 0.4 }];
		let sourceCount = 1;

		if (c.change24h && Math.abs(c.change24h) > 2) {
			signals.push({
				type: "cii_delta",
				value: `24h change ${c.change24h > 0 ? "+" : ""}${c.change24h.toFixed(1)}`,
				weight: 0.2,
			});
			sourceCount++;
		}
		...
		const matchRegex = new RegExp(
			`\\b(${countryName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}|${countryCode.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})\\b`,
			"i",
		);
		...
		if (matchingIran.length > 0) {
			signals.push({
				...
				value: `${matchingIran.length} Iran-related events`,
				weight: 0.2,
			});
			sourceCount++;
		}
	...
	for (const t of theaters) {
		...
		predictions.push(
			makePrediction("conflict", region, `Theater escalation: ${region}`, prob, 0.5, "7d", signals),
		);
	}
		...
		predictions.push(
			makePrediction(
				...
				[
					...
					{
						type: "commodity",
						value: `${commodity.commodity} sensitivity: ${commodity.sensitivity}`,
						weight: 0.3,
					},
				],
			),
...
function detectSupplyChainScenarios(inputs) {
	const predictions = [];
	const chokepoints = inputs.chokepoints?.routes || inputs.chokepoints?.chokepoints || [];
	const anomalies = Array.isArray(inputs.temporalAnomalies)
		? inputs.temporalAnomalies
		: inputs.temporalAnomalies?.anomalies || [];
	const jamming = Array.isArray(inputs.gpsJamming)
		? inputs.gpsJamming
		: inputs.gpsJamming?.zones || [];
	...
	for (const cp of chokepoints) {
		...
		const signals = [{ type: "chokepoint", value: `${route} disruption detected`, weight: 0.5 }];
		let sourceCount = 1;

		const aisGaps = anomalies.filter(
			(a) =>
		...
		if (aisGaps.length > 0) {
			signals.push({
				type: "ais_gap",
				value: `${aisGaps.length} AIS gap anomalies near ${route}`,
				weight: 0.3,
			});
			sourceCount++;
		}
		...
		const prob = Math.min(
			0.85,
			riskNorm * 0.7 + (aisGaps.length > 0 ? 0.1 : 0) + (nearbyJam.length > 0 ? 0.05 : 0),
		);
		const confidence = Math.max(0.3, normalize(sourceCount, 0, 4));
...
function buildStateDerivedFeedSummary(domain, stateUnit, bucket, marketContext, channel = "") {
	const channelLabel = humanizeTransmissionChannel(channel || marketContext?.topChannel);
	const evidence = [];
	if ((marketContext?.transmissionEdgeCount || 0) > 0)
		evidence.push(`${marketContext.transmissionEdgeCount} transmission path(s)`);
	if ((marketContext?.criticalSignalCount || 0) > 0)
		evidence.push(`${marketContext.criticalSignalCount} urgent critical signal(s)`);
	const tail = evidence.length ? ` backed by ${evidence.join(" and ")}` : "";

	if (domain === "supply_chain") {
		if (bucket.id === "freight")
			return `${stateUnit.label} is carrying freight disruption through ${channelLabel}${tail}.`;
		if (bucket.id === "energy")
			return `${stateUnit.label} is carrying maritime energy-flow disruption through ${channelLabel}${tail}.`;
	}

	if (bucket.id === "energy")
		return `${stateUnit.label} is carrying energy repricing pressure through ${channelLabel}${tail}.`;
	if (bucket.id === "sovereign_risk")
		return `${stateUnit.label} is carrying sovereign-risk repricing through ${channelLabel}${tail}.`;
	if (bucket.id === "rates_inflation")
		return `${stateUnit.label} is carrying inflation-and-rates pressure through ${channelLabel}${tail}.`;
	if (bucket.id === "fx_stress")
		return `${stateUnit.label} is carrying FX-stress pressure through ${channelLabel}${tail}.`;
	return `${stateUnit.label} is carrying ${bucket.label.toLowerCase()} pressure through ${channelLabel}${tail}.`;
}

function computeStateDerivedBucketCandidate(
	domain,
	stateUnit,
	bucket,
	marketContext,
	...
	const stateDomainMatch = intersectAny(
		stateUnit?.domains || [],
		domain === "supply_chain"
	...
	const criticalAlignment = computeCriticalBucketAlignment(
		bucket.id,
		marketContext?.criticalSignalTypes || [],
	);
	...
	const requiresMaritimeStateKind =
		domain === "supply_chain" && ["freight", "energy"].includes(bucket.id);
	...
	const supplyChainFallbackEligible =
		domain === "supply_chain" &&
		stateDomainMatch &&
		directBucket &&
	...
	const score = clampUnitInterval(
		Number(marketContext?.confirmationScore || 0) * 0.28 +
			...
			Number(
				bucketContext?.topTransmissionStrength || marketContext?.topTransmissionStrength || 0,
			) *
				0.1 +
	...
	return {
		...
		fallbackScore: Math.max(
			0.3,
			getStateDerivedMinimumScore(domain, bucket.id) - 0.08 - (coverageScore < 0.45 ? 0.03 : 0),
		),
	};
}
...
function buildStateDerivedForecast(stateUnit, domain, bucket, candidate, marketContext) {
	...
	const signals = [
		{
			type:
				candidate.primarySignalType ||
				candidate.primaryChannel ||
				bucketContext?.topChannel ||
				marketContext?.topChannel ||
				"derived_transmission",
			value: buildStateDerivedLeadSignal(
				domain,
				stateUnit,
				bucket,
				marketContext,
...
function deriveStateDrivenForecasts({
	...
	for (const stateUnit of stateUnits) {
		...
		const linkedBuckets = uniqueSortedStrings(marketContext.linkedBucketIds || [])
			...
			.sort(
				(left, right) =>
					(right.id === marketContext.topBucketId ? 1 : 0) -
						(left.id === marketContext.topBucketId ? 1 : 0) ||
		...
		for (const domain of ["market", "supply_chain"]) {
			...
			for (const bucket of linkedBuckets) {
				...
				const record = {
					...
					marketContext,
					candidate,
					prediction: buildStateDerivedForecast(
						stateUnit,
						domain,
						bucket,
						candidate,
				...
				if (
					...
					(record.candidate.score === best.candidate.score &&
						bucket.id === marketContext.topBucketId &&
						best.bucket.id !== marketContext.topBucketId)
				) {
					best = record;
				}
			}
	...
	for (const pred of derived) {
		...
		if (!bucketId || !stateKind) {
			noDerivation.push(pred);
			continue;
		}
	...
	for (const group of crossGroups.values()) {
		...
		for (const pred of group) {
			...
			if (
				lastKeptProb !== null &&
				Math.abs(pred.probability - lastKeptProb) < MIN_CROSS_STATE_PROBABILITY_SPREAD
			)
				continue;
	...
	return crossDeduped.sort(
		(a, b) =>
			Number(a.stateDerivedBackfill) - Number(b.stateDerivedBackfill) ||
...
function detectPoliticalScenarios(inputs) {
	...
	const anomalies = Array.isArray(inputs.temporalAnomalies)
		? inputs.temporalAnomalies
		: inputs.temporalAnomalies?.anomalies || [];
	const unrestEvents = Array.isArray(inputs.unrestEvents)
		? inputs.unrestEvents
		: inputs.unrestEvents?.events || [];
	const unrestCounts = new Map();

	for (const event of unrestEvents) {
		const country = resolveCountryName(
			event.country || event.country_name || event.region || event.location || "",
		);
		if (!country) continue;
	...
	for (const c of scores) {
		...
		const signals = [
			{
				type: "unrest",
				value: `${c.name} unrest component: ${Math.max(unrestComp, unrestCount * 10)}`,
				weight: 0.4,
			},
		];
		let sourceCount = 1;
		...
		if (unrestCount > 0) {
			signals.push({
				type: "unrest_events",
				value: `${unrestCount} unrest events in ${c.name}`,
				weight: 0.3,
			});
			sourceCount++;
		}

		const protestAnomalies = anomalies.filter(
			(a) =>
		...
		if (protestAnomalies.length > 0) {
			...
			signals.push({
				type: "anomaly",
				value: `Protest anomaly z-score: ${maxZ.toFixed(1)}`,
				weight: 0.3,
			});
			sourceCount++;
		}
...
function detectMilitaryScenarios(inputs) {
	...
	const theatersById = new Map(
		theaters
			.map((theater) => [theater?.id || theater?.theater, theater])
	...
	for (const theaterId of theaterIds) {
		...
		const milFlights = anomalies.filter(
			(a) =>
				(a.type === "military_flights" || a.type === "military") &&
				[region, theaterLabel, theaterId].some(
					(part) =>
						part && (a.region || a.theater || "").toLowerCase().includes(part.toLowerCase()),
				),
		);
		if (milFlights.length > 0) {
			...
			signals.push({
				type: "mil_flights",
				value: `Military flight anomaly z-score: ${maxZ.toFixed(1)}`,
				weight: 0.3,
			});
			sourceCount++;
		}
		...
		if (t?.indicators && Array.isArray(t.indicators)) {
			...
			if (activeIndicators.length > 0) {
				signals.push({
					type: "indicators",
					value: `${activeIndicators.length} active posture indicators`,
					weight: 0.2,
				});
				sourceCount++;
			}
		...
		const supportBoost =
			highestSurge && (highestSurge.awacs > 0 || highestSurge.tankers > 0) ? 0.05 : 0;
		const strikeBoost =
			t?.activeOperations?.includes?.("strike_capable") || highestSurge?.strikeCapable ? 0.06 : 0;
		...
		predictions.push(makePrediction("military", region, title, prob, confidence, "7d", signals));
	}
...
function detectInfraScenarios(inputs) {
	...
	const cyber = Array.isArray(inputs.cyberThreats)
		? inputs.cyberThreats
		: inputs.cyberThreats?.threats || [];
	const jamming = Array.isArray(inputs.gpsJamming)
		? inputs.gpsJamming
		: inputs.gpsJamming?.zones || [];
	...
	for (const o of outages) {
		...
		const severity = rawSev.includes("critical")
			? "critical"
			: rawSev.includes("high")
				? "major"
				: rawSev.includes("total")
					? "total"
					: rawSev.includes("major")
						? "major"
						: rawSev;
		...
		const signals = [{ type: "outage", value: `${country} ${severity} outage`, weight: 0.4 }];
		let sourceCount = 1;
		...
		if (relatedCyber.length > 0) {
			signals.push({
				type: "cyber",
				value: `${relatedCyber.length} cyber threats targeting ${country}`,
				weight: 0.3,
			});
			sourceCount++;
		}
...
function detectCyberScenarios(inputs) {
	const predictions = [];
	const threats = Array.isArray(inputs.cyberThreats)
		? inputs.cyberThreats
		: inputs.cyberThreats?.threats || [];
	...
	for (const [country, items] of Object.entries(byCountry)) {
		...
		const criticalCount = items.filter((t) =>
			/ransomware|wiper|ddos|intrusion|exploit|botnet|malware/i.test(
				`${t.type || ""} ${t.category || ""}`,
			),
		).length;
		const score =
			items.length +
			types.size * CYBER_SCORE_TYPE_MULTIPLIER +
			criticalCount * CYBER_SCORE_CRITICAL_MULTIPLIER;
		const probability = Math.min(
			CYBER_PROB_MAX,
			normalize(items.length, 4, 50) * CYBER_PROB_VOLUME_WEIGHT +
				normalize(types.size, 1, 6) * CYBER_PROB_TYPE_WEIGHT,
		);
		candidates.push({
			country,
			items,
	...
	candidates
		.sort(
			(a, b) =>
				b.score - a.score || b.probability - a.probability || a.country.localeCompare(b.country),
		)
			...
			predictions.push(
				makePrediction(
					...
					[
						{
							type: "cyber",
							value: `${candidate.items.length} threats (${[...candidate.types].join(", ")})`,
							weight: 0.5,
						},
					],
				),
...
function detectGpsJammingScenarios(inputs) {
	const predictions = [];
	const zones = Array.isArray(inputs.gpsJamming)
		? inputs.gpsJamming
	...
	for (const [region, bounds] of Object.entries(MARITIME_REGIONS)) {
		const inRegion = zones.filter((h) => {
			...
			return (
				lat >= bounds.latRange[0] &&
				lat <= bounds.latRange[1] &&
				lon >= bounds.lonRange[0] &&
				lon <= bounds.lonRange[1]
			);
		});
		...
		predictions.push(
			makePrediction(
				...
				[
					{
						type: "gps_jamming",
						value: `${inRegion.length} jamming hexes in ${region}`,
						weight: 0.5,
					},
				],
			),
...
const DOMAIN_ACTOR_BLUEPRINTS = {
	...
	market: [
		...
		{
			key: "large_importers",
			name: "Large importers",
			category: "commercial",
			influenceScore: 0.68,
		},
		{
			key: "regional_producers",
			name: "Regional producers",
			category: "commercial",
			influenceScore: 0.62,
		},
	],
	supply_chain: [
		{
			key: "shipping_operators",
			name: "Shipping operators",
			category: "commercial",
			influenceScore: 0.84,
		},
		{
			key: "port_authorities",
	...
	political: [
		{
			key: "incumbent_leadership",
			name: "Incumbent leadership",
			category: "state",
			influenceScore: 0.86,
		},
		{
			key: "opposition_networks",
			name: "Opposition networks",
			category: "political",
			influenceScore: 0.69,
		},
		{
			key: "regional_diplomats",
			name: "Regional diplomats",
			category: "external",
			influenceScore: 0.57,
		},
	...
	military: [
		{
			key: "defense_planners",
			name: "Defense planners",
			category: "security",
			influenceScore: 0.86,
		},
		{
			key: "allied_observers",
			name: "Allied observers",
			category: "external",
			influenceScore: 0.68,
		},
		{
			key: "commercial_carriers",
			name: "Commercial carriers",
			category: "commercial",
			influenceScore: 0.51,
		},
		{
			key: "regional_command",
			name: "Regional command posts",
			category: "security",
			influenceScore: 0.74,
		},
	],
	...
	infrastructure: [
		...
		},
		{
			...
			name: "Civil protection authorities",
			category: "state",
			influenceScore: 0.72,
		},
		{
			key: "critical_providers",
			name: "Critical service providers",
			category: "commercial",
			influenceScore: 0.64,
		},
		{
			key: "cyber_responders",
...
const SIGNAL_TRIGGER_TEMPLATES = {
	cii: (pred, signal) =>
		`Watch for another deterioration in ${pred.region} risk indicators beyond ${signal.value}.`,
	cii_delta: (pred) =>
		`A further sharp 24h deterioration in ${pred.region} risk metrics would strengthen the base case.`,
	conflict_events: (pred) =>
		`A fresh cluster of reported conflict events in ${pred.region} would raise escalation pressure quickly.`,
	ucdp: (pred) =>
		`A sustained increase in verified conflict-event counts would confirm the escalation path in ${pred.region}.`,
	theater: (pred) =>
		`Any shift from elevated to critical theater posture in ${pred.region} would move this forecast higher.`,
	indicators: (pred) =>
		`More active posture indicators in ${pred.region} would support an escalatory revision.`,
	mil_flights: () =>
		"Another spike in military flight anomalies would strengthen the near-term risk path.",
	chokepoint: (_pred, signal) =>
		`${signal.value} persisting for another cycle would deepen downstream disruption risk.`,
	ais_gap: () =>
		"Further AIS gaps around the affected route would confirm operational disruption rather than noise.",
	gps_jamming: () =>
		"Wider GPS interference across adjacent zones would increase the chance of spillover effects.",
	unrest: (pred) =>
		`A higher unrest signal in ${pred.region} would raise the probability of instability broadening.`,
	anomaly: () =>
		"A new anomaly spike above the current protest baseline would strengthen the forecast.",
	outage: (pred) =>
		`A second major outage in ${pred.region} would turn a contained event into a cascade risk.`,
	cyber: (pred) =>
		`Additional cyber incidents tied to ${pred.region} infrastructure would materially worsen the case.`,
	prediction_market: () =>
		"A market repricing of 8-10 points would be a meaningful confirmation or rejection signal.",
	news_corroboration: (pred) =>
		`More directly matched reporting on ${pred.region} would improve confidence in the current path.`,
};
...
function uniqueLowerTerms(terms) {
	return [
		...new Set((terms || []).map((term) => (term || "").toLowerCase().trim()).filter(Boolean)),
	];
}
...
function extractMeaningfulTokens(text, exclude = []) {
	const excluded = new Set(
		uniqueLowerTerms(exclude)
			.flatMap((term) => term.split(/[^a-z0-9]+/g))
			.filter(Boolean),
	);
	return [
		...new Set(
			tokenizeText(text).filter(
				(token) => token.length >= 4 && !TEXT_STOPWORDS.has(token) && !excluded.has(token),
			),
		),
	];
}
...
function computeHeadlineRelevance(headline, terms, domain, options = {}) {
	...
	const expectedTags =
		options.expectedTags instanceof Set
...
function detectFromPredictionMarkets(inputs) {
	...
	for (const m of markets) {
		...
		const domain = titleLower.match(/war|strike|military|attack/)
			? "conflict"
			: titleLower.match(/tariff|recession|economy|gdp/)
				? "market"
				: "political";

		predictions.push(
			makePrediction(domain, region, m.title.slice(0, 100), yesPrice, 0.7, "30d", [
				{
					type: "prediction_market",
					value: `${m.source || "Polymarket"}: ${Math.round(yesPrice * 100)}%`,
					weight: 0.8,
				},
			]),
		);
...
function discoverGraphCascades(predictions, graph) {
	...
	for (const pred of predictions) {
		...
		for (const linkedId of node.links) {
			...
			if (!linked) continue;
			const linkedPred = predictions.find(
				(p) => p !== pred && p.domain !== pred.domain && graph.aliases[p.region] === linkedId,
			);
			if (!linkedPred) continue;

			const edge = graph.edges.find(
				(e) => (e.from === nodeId && e.to === linkedId) || (e.from === linkedId && e.to === nodeId),
			);
...
const DEFAULT_CASCADE_RULES = [
	{
		...
		to: "supply_chain",
		coupling: 0.6,
		mechanism: "chokepoint disruption",
		requiresChokepoint: true,
	},
	{
		from: "conflict",
		to: "market",
		coupling: 0.5,
		mechanism: "commodity price shock",
		requiresChokepoint: true,
	},
	{
		from: "political",
		to: "conflict",
		coupling: 0.4,
		mechanism: "instability escalation",
		minProbability: 0.6,
	},
	{
		from: "military",
		to: "conflict",
		coupling: 0.5,
		mechanism: "force deployment",
		requiresCriticalPosture: true,
	},
...
const PREDICATE_EVALUATORS = {
	...
	requiresCriticalPosture: (pred) =>
		pred.signals.some((s) => s.type === "theater" && s.value.includes("critical")),
	minProbability: (pred, val) => pred.probability >= val,
	requiresSeverity: (pred, val) =>
		pred.signals.some((s) => s.type === "outage" && s.value.toLowerCase().includes(val)),
};
...
function loadCascadeRules() {
	try {
		...
		const KNOWN_FIELDS = new Set([
			...
			"coupling",
			"mechanism",
			...Object.keys(PREDICATE_EVALUATORS),
		]);
		for (const r of raw) {
			...
			for (const key of Object.keys(r)) {
				if (!KNOWN_FIELDS.has(key))
					throw new Error(`unknown predicate '${key}' in rule: ${r.mechanism}`);
			}
		}
...
function resolveCascades(predictions, rules) {
	...
	for (const rule of rules) {
		...
		for (const src of sources) {
			...
			src.cascades.push({
				domain: rule.to,
				effect: rule.mechanism,
				probability: +cascadeProb.toFixed(3),
			});
		}
	}
}
	...
	for (const pred of predictions) {
		...
		const candidates = markets.geopolitical
			.map((m) => {
				...
				const match = computeMarketMatchScore(pred, m.title, regionTerms, {
					expectedTags,
					titleTokens,
				});
			...
			.filter((item) => {
				...
				if (pred.domain === "market") {
					return (
						hasSpecificRegionSignal &&
						item.titleHits > 0 &&
						(item.domainHits > 0 || item.score >= 7)
					);
				}
...
function computeTrends(predictions, prior) {
	if (!prior?.predictions) {
		for (const p of predictions) {
			p.trend = "stable";
			p.priorProbability = p.probability;
		}
		return;
	}
...
function getSearchTermsForRegion(region) {
	...
	if (!countryEntry) {
		...
		for (const [, entry] of Object.entries(codes)) {
			...
			if (
				nameLower === regionLower ||
				nameLower === regionBase ||
				regionLower.includes(nameLower)
			) {
...
function extractAllHeadlines(newsInsights, newsDigest) {
	...
	if (headlines.length === 0 && newsInsights?.topStories) {
		for (const s of newsInsights.topStories) {
			if (s?.primaryTitle && !seen.has(s.primaryTitle)) {
				seen.add(s.primaryTitle);
				headlines.push(s.primaryTitle);
			}
		}
	}
	return headlines;
...
const CRITICAL_NEWS_ROUTE_RE =
	/\b(hormuz|strait of hormuz|bab el[- ]mandeb|suez|red sea|black sea|baltic sea|kerch|shipping lane|shipping route|trade corridor|canal|port|terminal)\b/i;
const CRITICAL_NEWS_BLOCKAGE_RE =
	/\b(block(?:ade|ed|ing|s)?|clos(?:e|ed|ure|ing)|shut(?:ting)?|halt(?:ed|ing)?|suspend(?:ed|ing)?|interrupt(?:ed|ion)?|rerout(?:e|ed|ing)?|seiz(?:e|ed|ure)|interdict(?:ed|ion)?|mine(?:d|s)?)\b/i;
const CRITICAL_NEWS_ATTACK_RE =
	/\b(attack(?:ed|s)?|air ?strike(?:s)?|strike(?:s)?|struck|drone|missile|rocket|blast|explosion|fire|burn(?:ing)?|hit|damage(?:d)?|sabotage)\b/i;
const CRITICAL_NEWS_ENERGY_RE =
	/\b(oil|crude|gas|lng|liquefied natural gas|refiner(?:y|ies)|pipeline|terminal|export terminal|petrochemical|storage tank|tank farm|fuel depot|processing plant|tanker)\b/i;
const CRITICAL_NEWS_LNG_RE =
	/\b(lng|liquefied natural gas|ras laffan|north field|south pars|gas field|gas export|gas terminal)\b/i;
const CRITICAL_NEWS_REFINERY_RE =
	/\b(refiner(?:y|ies)|petrochemical|fuel depot|oil terminal|storage tank|tank farm|processing plant)\b/i;
const CRITICAL_NEWS_SANCTIONS_RE =
	/\b(sanction(?:s|ing|ed)?|embargo|export control|blacklist|freeze(?:d)? assets|price cap|trade ban|shipping ban)\b/i;
const CRITICAL_NEWS_ULTIMATUM_RE =
	/\b(ultimatum|deadline|final warning|48-hour|72-hour|must reopen|must withdraw|or face)\b/i;
const CRITICAL_NEWS_POWER_RE =
	/\b(power station|power plant|grid|substation|electricity|blackout)\b/i;
const CRITICAL_NEWS_SOURCE_TYPES = new Set([
	"critical_news",
	"critical_news_llm",
	"iran_events",
	"sanctions_pressure",
...
const CRITICAL_NEWS_GEO_HINTS = [
	{
		pattern:
			/\b(hormuz|strait of hormuz|persian gulf|gulf of oman|qatar|doha|ras laffan|south pars|north field|asaluyeh|bahrain|kuwait|uae|abu dhabi|dubai|fujairah|oman|saudi|riyadh|iraq|iran|israel|gaza|lebanon|syria|yemen)\b/i,
		region: "Middle East",
		macroRegion: "MENA",
	},
	...
	{
		pattern: /\b(taiwan|south china sea|china|japan|korea|philippines)\b/i,
		region: "South China Sea",
		macroRegion: "EAST_ASIA",
	},
	{
		pattern: /\b(united states|u\.s\.|washington|new york)\b/i,
...
function normalizeCriticalThreatLevel(value, text = "") {
	...
	if (
		CRITICAL_NEWS_ATTACK_RE.test(text) ||
		CRITICAL_NEWS_BLOCKAGE_RE.test(text) ||
		CRITICAL_NEWS_SANCTIONS_RE.test(text)
	)
		return "high";
	return "moderate";
}
...
function addCriticalSignalsFromTextItem(
	signals,
	item,
	...
	const corroborationBoost = Math.min(
		0.12,
		Math.max(0, (Number(item?.sourceCount || 1) - 1) * 0.03),
	);
	...
	const baseConfidence = clampUnitInterval(
		0.52 + threatWeight * 0.9 + corroborationBoost + (item?.isAlert ? 0.04 : 0),
	);
	...
	if (hasRoute && (hasBlockage || hasAttack || hasUltimatum)) {
		pushCriticalSignal(
			...
			"shipping_cost_shock",
			sourceType,
			`${region || "Critical route"} disruption pressure`,
			{
				...
				supportingEvidence: buildCriticalSignalSupport(item, [
					"Route disruption / closure terms are active",
				]),
			},
		);
		if (hasEnergy || /\b(hormuz|tanker|crude|oil|gulf)\b/i.test(text)) {
			pushCriticalSignal(
				...
				"energy_supply_shock",
				sourceType,
				`${region || "Critical route"} energy transit pressure`,
				{
					...
					supportingEvidence: buildCriticalSignalSupport(item, [
						"Energy transit exposure is directly referenced",
					]),
				},
			);
		}
	...
	if (hasAttack && hasEnergy) {
		pushCriticalSignal(
			...
			"energy_supply_shock",
			sourceType,
			`${region || "Critical asset"} energy infrastructure stress`,
			{
				...
				supportingEvidence: buildCriticalSignalSupport(item, [
					"Energy facility / export infrastructure is under direct threat",
				]),
			},
		);
		if (hasLng) {
			pushCriticalSignal(
				...
				"gas_supply_stress",
				sourceType,
				`${region || "Critical asset"} LNG and gas export stress`,
				{
					...
					domains: ["market", "supply_chain"],
					supportingEvidence: buildCriticalSignalSupport(item, [
						"Gas / LNG export capacity is directly implicated",
					]),
				},
			);
		}
		if (hasRefinery) {
			pushCriticalSignal(
				...
				"commodity_repricing",
				sourceType,
				`${region || "Critical asset"} refined-product repricing risk`,
				{
					...
					domains: ["market"],
					supportingEvidence: buildCriticalSignalSupport(item, [
						"Refinery / storage / petrochemical damage is referenced",
					]),
				},
			);
		}
	...
	if (hasSanctions) {
		pushCriticalSignal(
			...
			`${region || "Targeted region"} sanctions pressure is intensifying`,
			{
				...
				supportingEvidence: buildCriticalSignalSupport(item, [
					"Sanctions / export-control pressure is directly referenced",
				]),
			},
		);
		if (hasEnergy) {
			pushCriticalSignal(
				...
				"commodity_repricing",
				sourceType,
				`${region || "Targeted region"} sanctions are feeding commodity repricing`,
				{
					...
					domains: ["market"],
					supportingEvidence: buildCriticalSignalSupport(item, [
						"Energy / commodity sanctions are directly implicated",
					]),
				},
			);
		}
	...
	if (hasUltimatum && (hasRoute || hasEnergy || hasSanctions)) {
		pushCriticalSignal(
			...
			`${region || "Flashpoint"} deadline pressure is escalating`,
			{
				...
				supportingEvidence: buildCriticalSignalSupport(item, [
					"Deadline / ultimatum language indicates an acute state change",
				]),
			},
		);
	}
	...
	if (hasPower && (hasAttack || threatLevel === "critical")) {
		pushCriticalSignal(
			...
			"infrastructure_capacity_loss",
			sourceType,
			`${region || "Critical grid"} infrastructure capacity is under pressure`,
			{
				...
				supportingEvidence: buildCriticalSignalSupport(item, [
					"Grid / power infrastructure damage is referenced",
				]),
			},
		);
	}
...
function scoreCriticalNewsCandidate(item) {
	...
	const hasUltimatum = CRITICAL_NEWS_ULTIMATUM_RE.test(text);
	const hasPower = CRITICAL_NEWS_POWER_RE.test(text);
	const transmissionRelevant =
		hasRoute || hasEnergy || hasLng || hasRefinery || hasSanctions || hasUltimatum || hasPower;
	...
	return {
		...
		isUrgent:
			transmissionRelevant &&
			(clampUnitInterval(score) >= 0.58 ||
				(Boolean(item?.isAlert) &&
					(hasRoute || hasEnergy || hasSanctions || hasPower || hasAttack))),
	};
}
...
function validateCriticalSignalFrames(items, candidates = []) {
	...
	for (const item of items) {
		...
		const primaryKind = String(item?.primaryKind || "")
			.trim()
			.toLowerCase();
		...
		if ((strength <= 0 && confidence <= 0) || (impactHints.length === 0 && primaryKind === "other"))
			continue;
		valid.push({
			index,
			primaryKind,
			...
			evidence: Array.isArray(item?.evidence)
				? item.evidence
						.map((entry) => sanitizeForPrompt(String(entry || "")))
						.filter(Boolean)
						.slice(0, 3)
				: [],
...
function mapCriticalSignalFrameToSignals(frame, candidate) {
	...
	const macroRegion =
		frame.macroRegion ||
		candidate?.macroRegionHint ||
		inferredGeo.macroRegion ||
		getMacroRegion([region]) ||
		"";
	const baseStrength = clampUnitInterval(Number(frame.strength || candidate?.urgentScore || 0.6));
	const baseConfidence = clampUnitInterval(
		Number(frame.confidence || Math.max(0.58, candidate?.urgentScore || 0)),
	);
	...
	const support = buildCriticalSignalSupport(
		candidate,
		mergeSignalLists(
	...
	if (frame.primaryKind === "route_blockage" || impactHints.has("shipping")) {
		pushCriticalSignal(
			...
			"shipping_cost_shock",
			"critical_news_llm",
			`${routeLabel} disruption pressure`,
			{
	...
	if (
		...
		(frame.primaryKind === "route_blockage" &&
			/\b(oil|crude|tanker|gulf|energy)\b/i.test(commodity))
	) {
		pushCriticalSignal(
			signals,
			"energy_supply_shock",
			"critical_news_llm",
			`${facilityLabel} energy infrastructure stress`,
			{
	...
	if (
		impactHints.has("gas_lng") ||
		/\b(lng|gas|north field|south pars|ras laffan)\b/i.test(commodity)
	) {
		pushCriticalSignal(
			signals,
			"gas_supply_stress",
			"critical_news_llm",
			`${facilityLabel} LNG and gas export stress`,
			{
	...
	if (
		impactHints.has("refined_products") ||
		/\b(refinery|petrochemical|fuel depot|tank farm|storage tank)\b/i.test(commodity)
	) {
		pushCriticalSignal(
			signals,
			"commodity_repricing",
			"critical_news_llm",
			`${facilityLabel} refined-product repricing risk`,
			{
	...
	if (
		...
		pushCriticalSignal(signals, "sovereign_stress", "critical_news_llm", sovereignLabel, {
			...
			domains: [
				"market",
				"political",
				...(frame.primaryKind === "ultimatum_escalation" ? ["conflict"] : []),
			],
			supportingEvidence: support,
		});
	}
	...
	if (frame.primaryKind === "power_disruption" || impactHints.has("infrastructure")) {
		pushCriticalSignal(
			...
			"infrastructure_capacity_loss",
			"critical_news_llm",
			`${facilityLabel} infrastructure capacity is under pressure`,
			{
	...
	if (frame.primaryKind === "policy_intervention" || impactHints.has("rates_policy")) {
		pushCriticalSignal(
			...
			"policy_rate_pressure",
			"critical_news_llm",
			`${region || "Policy center"} emergency policy pressure is building`,
			{
...
function extractIranEventCriticalSignals(inputs) {
	const signals = [];
	const iranEvents = Array.isArray(inputs?.iranEvents)
		? inputs.iranEvents
		: inputs?.iranEvents?.events || [];
	for (const event of iranEvents.slice(0, 40)) {
		if (!["high", "critical"].includes(normalizeCriticalThreatLevel(event?.severity, event?.title)))
			continue;
		addCriticalSignalsFromTextItem(
			signals,
			{
				title: event?.title || "",
			...
			"iran_events",
			inferCriticalSignalGeo(String(event?.locationName || "")).region || "Middle East",
		);
	}
	return signals;
}
...
function extractSanctionsCountrySignals(inputs) {
	const signals = [];
	const sanctionsCountries = Array.isArray(inputs?.sanctionsPressure?.countries)
		? inputs.sanctionsPressure.countries
		: [];
	for (const country of sanctionsCountries.slice(0, 10)) {
		...
		pushCriticalSignal(
			...
			"sovereign_stress",
			"sanctions_pressure",
			`${region} sanctions pressure is rising`,
			{
				sourceKey: `sanctions_pressure:${country?.countryCode || region}:sovereign`,
				region,
				macroRegion,
				strength: normalizeSignalStrength(
					Math.max(
						Number(country?.newEntryCount || 0) * 0.18,
						Number(country?.entryCount || 0) * 0.05,
					),
					0.15,
					1,
				),
				confidence: clampUnitInterval(
					0.62 + Math.min(0.14, Number(country?.newEntryCount || 0) * 0.04),
				),
				domains: ["market", "political"],
				supportingEvidence: [
		...
		if (Number(country?.vesselCount || 0) > 0) {
			pushCriticalSignal(
				...
				"shipping_cost_shock",
				"sanctions_pressure",
				`${region} sanctions are tightening shipping pressure`,
				{
...
function extractSanctionsEntrySignals(inputs) {
	const signals = [];
	const sanctionsEntries = Array.isArray(inputs?.sanctionsPressure?.entries)
		? inputs.sanctionsPressure.entries
		: [];
	for (const entry of sanctionsEntries.slice(0, 12)) {
		...
		addCriticalSignalsFromTextItem(
			...
			"sanctions_pressure",
			entry?.countryNames?.[0] || entry?.countryCodes?.[0] || "",
		);
	}
	return signals;
}
...
function extractThermalCriticalSignals(inputs) {
	const signals = [];
	const thermalClusters = Array.isArray(inputs?.thermalEscalation?.clusters)
		? inputs.thermalEscalation.clusters
		: [];
	for (const cluster of thermalClusters.slice(0, 12)) {
		const highRelevance = cluster?.strategicRelevance === "THERMAL_RELEVANCE_HIGH";
		const acuteStatus =
			cluster?.status === "THERMAL_STATUS_SPIKE" || cluster?.status === "THERMAL_STATUS_PERSISTENT";
		if (!highRelevance || !acuteStatus || cluster?.context !== "THERMAL_CONTEXT_CONFLICT_ADJACENT")
			continue;
		...
		pushCriticalSignal(
			...
			"infrastructure_capacity_loss",
			"thermal_escalation",
			`${region || "Conflict-adjacent"} thermal escalation is threatening infrastructure`,
			{
				sourceKey: `thermal_escalation:${cluster?.id || region}:infrastructure`,
				region,
				macroRegion,
				strength: normalizeSignalStrength(
					Math.max(Number(cluster?.totalFrp || 0), Number(cluster?.observationCount || 0) * 15),
					60,
					220,
				),
		...
		if (
			/\b(qatar|iran|iraq|kuwait|saudi|united arab emirates|uae|oman|bahrain|libya)\b/i.test(region)
		) {
			pushCriticalSignal(
				signals,
				"energy_supply_shock",
				"thermal_escalation",
				`${region || "Conflict-adjacent"} thermal escalation is threatening energy throughput`,
				{
					sourceKey: `thermal_escalation:${cluster?.id || region}:energy`,
					region,
					macroRegion,
					strength: normalizeSignalStrength(
						Math.max(Number(cluster?.totalFrp || 0), Number(cluster?.persistenceHours || 0) * 10),
						80,
						260,
					),
...
function extractRegexCriticalNewsSignals(inputs, candidateItems = null) {
	...
	const items = Array.isArray(candidateItems)
		? candidateItems
		: extractNewsClusterItems(inputs?.newsInsights, inputs?.newsDigest);
	for (const item of items) {
...
async function extractCriticalSignalBundle(inputs) {
	...
	const fallbackSignalsFromCandidates = (coveredIndexes = new Set()) =>
		extractRegexCriticalNewsSignals(
			inputs,
			candidates.filter((item) => !coveredIndexes.has(item.candidateIndex)),
		);
...
function tryParseImpactExpansionCandidate(candidate) {
	try {
		...
		if (Array.isArray(parsed?.candidates))
			return { candidates: parsed.candidates, stage: "object_candidates" };
	...
	try {
		...
		if (Array.isArray(wrapped?.candidates))
			return { candidates: wrapped.candidates, stage: "wrapped_candidates" };
	} catch {
		// continue
	}
...
function normalizeImpactHypothesisDraft(item = {}) {
	const rawHypothesisKey = String(item?.hypothesisKey || "")
		.trim()
		.toLowerCase()
		.replace(/[^a-z0-9_]/g, "_")
		.slice(0, 80);
	const rawVariableKey = String(item?.variableKey || "")
		.trim()
		.toLowerCase();
	return {
		...
		affectedAssets: uniqueSortedStrings(
			(Array.isArray(item?.affectedAssets)
				? item.affectedAssets
				: Array.isArray(item?.assetsOrSectors)
					? item.assetsOrSectors
					: []
			)
				.map((value) => String(value || "").trim())
				.filter(Boolean),
		).slice(0, 6),
		marketImpact: String(item?.marketImpact || item?.channel || "")
			.trim()
			.toLowerCase()
			.slice(0, 40),
		...
		channel: String(item?.channel || item?.marketImpact || "")
			.trim()
			.toLowerCase(),
		targetBucket: String(item?.targetBucket || "")
			.trim()
			.toLowerCase(),
		region: String(item?.region || item?.geography || "").trim(),
		macroRegion: String(item?.macroRegion || "").trim(),
		countries: uniqueSortedStrings(
			(Array.isArray(item?.countries) ? item.countries : [])
				.map((value) => String(value || "").trim())
				.filter(Boolean),
		).slice(0, 6),
		assetsOrSectors: uniqueSortedStrings(
			(Array.isArray(item?.assetsOrSectors)
				? item.assetsOrSectors
				: Array.isArray(item?.affectedAssets)
					? item.affectedAssets
				...
				.map((value) => String(value || "").trim())
				.filter(Boolean),
		).slice(0, 6), // mirror of affectedAssets (see above)
		commodity: String(item?.commodity || "").trim(),
		dependsOnKey: String(item?.dependsOnKey || "")
			.trim()
			.toLowerCase()
			.replace(/[^a-z0-9_]/g, "_")
			.slice(0, 80),
		...
		analogTag: String(item?.analogTag || "")
			.trim()
			.toLowerCase(),
...
function sanitizeImpactExpansionDrafts(items, candidatePackets = []) {
	...
	for (const item of items) {
		...
		const directHypotheses = (Array.isArray(item?.directHypotheses) ? item.directHypotheses : [])
			.map(normalizeImpactHypothesisDraft)
			.slice(0, 3);
		const secondOrderHypotheses = (
			Array.isArray(item?.secondOrderHypotheses) ? item.secondOrderHypotheses : []
		)
			.map(normalizeImpactHypothesisDraft)
			.slice(0, 3);
		const thirdOrderHypotheses = (
			Array.isArray(item?.thirdOrderHypotheses) ? item.thirdOrderHypotheses : []
		)
			.map(normalizeImpactHypothesisDraft)
			.slice(0, 2);
		valid.push({
...
function buildImpactExpansionContinuityRecord(stateUnit, priorStateUnits = []) {
	...
	const probabilityDelta =
		Number(stateUnit.avgProbability || 0) - Number(prior.avgProbability || 0);
 ...
 */
function filterNewsHeadlinesByState(
	stateUnit,
	newsInsights,
	newsDigest,
	limit = 3,
	preExtractedItems = null,
) {
...
function buildImpactExpansionEvidenceTable(
	stateUnit,
	marketContext,
	continuityRecord,
	newsItems = [],
	...
	pushEvidence(
		"state_summary",
		`${stateUnit.label} (${stateUnit.stateKind || "state"}) is centered on ${stateUnit.dominantRegion || stateUnit.regions?.[0] || "the current theater"}.`,
	);
	...
	for (const signal of (stateUnit.topSignals || []).slice(0, 2)) {
		pushEvidence(
			"signal",
			`${String(signal.type || "").replace(/_/g, " ")} is active across ${signal.count || 0} linked forecasts.`,
		);
	}
	if (marketContext?.topBucketLabel) {
		pushEvidence(
			"market_context",
			`${marketContext.topBucketLabel} is the top linked bucket at ${roundPct(marketContext.topBucketPressure || 0)} pressure.`,
		);
	}
	if (marketContext?.consequenceSummary)
		pushEvidence("market_context", marketContext.consequenceSummary);
	if (continuityRecord?.summary) pushEvidence("continuity", continuityRecord.summary);
	if ((stateUnit.actors || []).length > 0)
		pushEvidence(
			"actor",
			`${stateUnit.actors.slice(0, 4).join(", ")} remain the lead actors in this state.`,
		);
...
function buildImpactExpansionSpecificity(stateUnit, marketContext) {
	...
	const geoCoherent =
		Boolean(regionMacro) &&
		((stateUnit.macroRegions || []).length === 0 ||
			(stateUnit.macroRegions || []).includes(regionMacro));
	return {
		dominantRegion,
		routeFacilityKey,
		commodityKey,
		specificityScore: +clampUnitInterval(
			(routeFacilityKey ? 0.5 : 0) + (commodityKey ? 0.3 : 0) + (geoCoherent ? 0.2 : 0),
		).toFixed(3),
	};
}

function isImpactExpansionCandidateEligible(
	stateUnit,
	marketContext,
	continuityRecord,
	specificity,
...
function computeImpactExpansionRankingScore(marketContext, continuityRecord, specificityScore) {
	...
	const transmissionEdgeScore = clampUnitInterval(
		Number(marketContext.transmissionEdgeCount || 0) / 4,
	);
...
function buildImpactExpansionCandidate(
	stateUnit,
	marketContext,
	priorStateUnits = [],
	newsInsights = null,
	newsDigest = null,
	preExtractedNewsItems = null,
) {
	...
	if (!isImpactExpansionCandidateEligible(stateUnit, marketContext, continuityRecord, specificity))
		return null;
	...
	const newsItems = filterNewsHeadlinesByState(
		stateUnitWithCommodity,
		newsInsights,
		newsDigest,
		3,
	...
	return {
		...
		marketBucketIds: uniqueSortedStrings(
			marketContext.linkedBucketIds || stateUnit.marketBucketIds || [],
		),
		transmissionChannels: uniqueSortedStrings(
			[
				marketContext.topChannel || "",
				...Object.values(marketContext.bucketContexts || {}).map(
					(context) => context.topChannel || "",
				),
				...(stateUnit.transmissionChannels || []),
			].filter(Boolean),
		),
		topSignalTypes: uniqueSortedStrings(
			(stateUnit.topSignals || []).map((signal) => signal.type).filter(Boolean),
		),
		...
		rankingScore: computeImpactExpansionRankingScore(
			marketContext,
			continuityRecord,
			specificity.specificityScore,
		),
		evidenceTable: buildImpactExpansionEvidenceTable(
			stateUnit,
			marketContext,
			continuityRecord,
			newsItems,
...
function selectImpactExpansionCandidates({
	...
	const preExtractedNewsItems =
		newsInsights || newsDigest ? extractNewsClusterItems(newsInsights, newsDigest) : null;
	return stateUnits
		.map((stateUnit) =>
			buildImpactExpansionCandidate(
				stateUnit,
		...
		.sort(
			(left, right) =>
				...
				Number(right.marketContext?.criticalSignalLift || 0) -
					Number(left.marketContext?.criticalSignalLift || 0) ||
				Number(right.marketContext?.topTransmissionStrength || 0) -
					Number(left.marketContext?.topTransmissionStrength || 0) ||
...
${candidatePackets
	.map((packet) =>
		[
			...
			"Evidence:",
			...(packet.evidenceTable || []).map(
				(entry) => `- ${entry.key} [${entry.kind}] ${sanitizeForPrompt(entry.text)}`,
			),
		].join("\n"),
	)
	.join("\n\n")}
...
async function recoverImpactExpansionDrafts(
	candidatePackets = [],
	invalidOutput = "",
	llmOptions = {},
) {
...
async function extractImpactExpansionBundle({
	...
	const priorStateUnits = Array.isArray(priorWorldState?.stateUnits)
		? priorWorldState.stateUnits
		: [];
	const selectedCandidatePackets =
		Array.isArray(candidatePackets) && candidatePackets.length
	...
	if (Array.isArray(cached?.candidates)) {
		const extractedCandidates = sanitizeImpactExpansionDrafts(
			cached.candidates,
			selectedCandidatePackets,
		);
		if (extractedCandidates.length > 0) {
			bundle.source = "cache";
			bundle.provider = "cache";
			...
			bundle.extractedHypothesisCount = extractedCandidates.reduce(
				(sum, item) =>
					sum +
	...
	for (let i = 0; i < selectedCandidatePackets.length; i += IMPACT_EXPANSION_CONCURRENCY) {
		...
		const batchResults = await Promise.all(
			batch.map(async (packet) => {
				...
				const single = await extractSingleImpactExpansionCandidate(
					packet,
					llmOptions,
					learnedSection,
				);
				if (single?.extractedCandidate) {
					await redisSet(
						...
						token,
						singleCacheKey,
						{ candidates: [single.extractedCandidate] },
	...
	bundle.extractedCandidates = extractedCandidates.sort(
	...
	bundle.extractedHypothesisCount = bundle.extractedCandidates.reduce(
		(sum, item) =>
			sum +
			(item.directHypotheses?.length || 0) +
			(item.secondOrderHypotheses?.length || 0) +
...
function extractCriticalNewsSignals(inputs) {
	if (Array.isArray(inputs?.criticalSignalBundle?.signals))
		return inputs.criticalSignalBundle.signals;
	return [...extractRegexCriticalNewsSignals(inputs), ...extractStructuredCriticalSignals(inputs)];
}
...
const SIGNAL_TO_SOURCE = {
	...
	ucdp: "ucdp",
	theater: "theater_posture",
	indicators: "theater_posture",
	mil_flights: "temporal_anomalies",
	anomaly: "temporal_anomalies",
...
function buildCounterEvidence(pred) {
	...
	if (!pred.newsContext || pred.newsContext.length === 0) {
		items.push({
			type: "coverage_gap",
			summary: `No directly matched headlines are currently attached to ${pred.region}.`,
			weight: 0.2,
		});
	}
	if (pred.confidence < 0.45) {
		items.push({
			type: "confidence",
			summary: `Confidence is only ${roundPct(pred.confidence)}, implying thin source diversity or mixed calibration.`,
			weight: 0.25,
		});
	}
	if (pred.trend === "falling") {
		items.push({
			type: "trend",
			summary: `The forecast is already trending down from its prior snapshot (${roundPct(pred.priorProbability)} to ${roundPct(pred.probability)}).`,
			weight: 0.35,
		});
	}
	if (pred.calibration) {
...
function buildCaseTriggers(pred) {
	...
	if (pred.calibration) {
		triggers.push(
			`If prediction markets move decisively away from ${roundPct(pred.calibration.marketPrice)}, revisit the probability baseline.`,
		);
	}
...
function buildForecastActors(pred) {
	const blueprints = DOMAIN_ACTOR_BLUEPRINTS[pred.domain] || [
		{
			key: "regional_watchers",
			name: "Regional watchers",
			category: "general",
			influenceScore: 0.6,
		},
		{
			key: "market_participants",
			name: "Market participants",
			category: "market",
			influenceScore: 0.52,
		},
		{
			key: "external_observers",
	...
	const topSupport =
		pred.signals?.[0]?.value || pred.caseFile?.supportingEvidence?.[0]?.summary || pred.title;
	...
	return blueprints.slice(0, 4).map((blueprint, index) => {
		...
		if (pred.domain === "conflict" || pred.domain === "military") {
			objectives.push(
				`Prevent the ${pred.region} situation from moving beyond the current ${pred.trend} path.`,
			);
			objectives.push(
				`Preserve decision freedom if ${topSupport} hardens into a broader escalation signal.`,
			);
			likelyActions.push(
				`Reposition attention and resources around ${pred.region} over the next ${pred.timeHorizon}.`,
			);
		} else if (pred.domain === "supply_chain") {
			objectives.push(
				`Keep critical flows through ${pred.region} functioning over the ${pred.timeHorizon}.`,
			);
			...
			objectives.push(
				`Price whether stress in ${pred.region} becomes durable over the ${pred.timeHorizon}.`,
			);
			objectives.push(`Protect against repricing if ${topSupport} intensifies.`);
			likelyActions.push(
				`Rebalance positions if the probability path moves away from ${roundPct(pred.probability)}.`,
			);
		} else if (pred.domain === "cyber") {
			objectives.push(
				`Contain hostile cyber activity affecting ${pred.region} before it spills into core services.`,
			);
			objectives.push(
				`Preserve resilience if ${topSupport} broadens into a sustained intrusion pattern.`,
			);
			likelyActions.push(`Harden exposed systems and triage incident response in ${pred.region}.`);
		} else if (pred.domain === "infrastructure") {
			objectives.push(
				`Contain service degradation in ${pred.region} before it becomes cross-system.`,
			);
			objectives.push(`Maintain continuity if ${topSupport} spreads across adjacent systems.`);
			likelyActions.push(
				`Prioritize mitigation and continuity measures around the most exposed nodes.`,
			);
		} else {
		...
		if ((pred.cascades || []).length > 0) {
			likelyActions.push(
				`Monitor spillover into ${(pred.cascades || [])
					.slice(0, 2)
					.map((c) => c.domain)
					.join(" and ")}.`,
			);
		...
		if (drift >= 0.08 && pred.calibration?.marketTitle) {
			constraints.push(
				`Market pricing in "${pred.calibration.marketTitle}" is not fully aligned with the internal estimate.`,
			);
		}
		if (pred.trend === "falling") {
			constraints.push(
				`Recent momentum is softening from ${roundPct(pred.priorProbability)} to ${roundPct(pred.probability)}.`,
			);
		}
		if (constraints.length === 0) {
			constraints.push(
				`Action remains bounded by the current ${roundPct(pred.confidence)} confidence level.`,
			);
		}
...
function buildForecastWorldState(pred, actors = [], triggers = [], counterEvidence = []) {
	const leadSupport =
		pred.caseFile?.supportingEvidence?.[0]?.summary || pred.signals?.[0]?.value || pred.title;
	...
	const activePressures = [
		...
		...(pred.cascades || [])
			.slice(0, 1)
			.map((cascade) => `Spillover pressure into ${cascade.domain} via ${cascade.effect}.`),
	]
		.filter(Boolean)
		.slice(0, 4);
	...
	const stabilizers = [
		...
		pred.trend === "falling"
			? `The observed trend is already easing from ${roundPct(pred.priorProbability)} to ${roundPct(pred.probability)}.`
			: "",
	...
	const keyUnknowns = [
		...
		!pred.newsContext?.length
			? `Whether directly matched reporting on ${pred.region} appears in the next run.`
			: "",
	]
		.filter(Boolean)
		.slice(0, 4);
...
function branchShift(kind, pred, context = {}) {
	...
	if (kind === "escalatory") {
		return Math.min(
			0.22,
			0.08 + pressureCount * 0.02 + triggerCount * 0.015 + cascadeFactor - stabilizerCount * 0.01,
		);
	}
	if (kind === "contrarian") {
		return -Math.min(
			0.22,
			0.08 + stabilizerCount * 0.025 + driftFactor + (pred.trend === "falling" ? 0.02 : 0),
		);
	}
...
function buildBranchRounds(kind, pred, context = {}) {
	const leadPressure =
		context.worldState?.activePressures?.[0] || pred.signals?.[0]?.value || pred.title;
	const leadTrigger =
		context.triggers?.[0] ||
		`The next ${pred.domain} update in ${pred.region} becomes the key threshold.`;
	const leadStabilizer =
		context.worldState?.stabilizers?.[0] ||
		context.counterEvidence?.[0]?.summary ||
		`The current ${roundPct(pred.confidence)} confidence level keeps this path from becoming fully settled.`;
	const actors = context.actors || [];

	const round1 = {
		round: 1,
		focus: kind === "contrarian" ? "Constraint absorption" : "Signal absorption",
		developments: [kind === "contrarian" ? leadStabilizer : leadPressure].filter(Boolean),
		actorMoves: actors
			.slice(0, 2)
			.map((actor) => actor.likelyActions?.[0])
			.filter(Boolean),
	...
	const round2 = {
		...
		actorMoves: actors
			.slice(0, 3)
			.map((actor) => actor.likelyActions?.[1] || actor.objectives?.[0])
			.filter(Boolean),
		probabilityShift: +(branchShift(kind, pred, context) / 3).toFixed(3),
	};
	...
	const round3 = {
		...
		developments: [
			kind === "escalatory" && (pred.cascades?.length || 0) > 0
				? `Spillover becomes visible in ${(pred.cascades || [])
						.slice(0, 2)
						.map((c) => c.domain)
						.join(" and ")}.`
				: kind === "contrarian"
		...
		actorMoves: actors
			.slice(0, 2)
			.map((actor) => actor.constraints?.[0])
			.filter(Boolean),
		probabilityShift: +(
			branchShift(kind, pred, context) -
			(branchShift(kind, pred, context) / 3) * 2
		).toFixed(3),
	};
...
function buildForecastBranches(pred, context = {}) {
	return ["base", "escalatory", "contrarian"].map((kind) => {
		...
		const leadStabilizer =
			context.worldState?.stabilizers?.[0] ||
			context.counterEvidence?.[0]?.summary ||
			`The current ${roundPct(pred.confidence)} confidence level still leaves room for reversal.`;
		const leadTrigger =
			context.triggers?.[0] || `The next evidence cycle in ${pred.region} becomes decisive.`;
...
function buildActorLenses(pred) {
	...
	const lenses = actors.map((actor) => {
		...
		const action =
			actor.likelyActions?.[0] || `Track ${pred.region} closely over the ${pred.timeHorizon}.`;
		return `${actor.name}: ${objective} ${action}`;
	});
	if (pred.cascades?.length > 0) {
		lenses.push(
			`Cross-domain watchers will track spillover into ${pred.cascades
				.slice(0, 2)
				.map((c) => c.domain)
				.join(" and ")} if this path hardens.`,
		);
	}
...
function buildForecastRunStatusPayload({
	...
	const statusSource =
		context.status || deepForecast?.status || (mode === "deep" ? "running" : "completed");
	...
	return {
		...
		updatedAt,
		completedAt,
		eligibleStateIds: Array.isArray(deepForecast?.selectedStateIds)
			? deepForecast.selectedStateIds
			: [],
		processedCandidateCount: Number(
			context.processedCandidateCount ?? worldState?.impactExpansion?.successfulCandidateCount ?? 0,
		),
		acceptedPathCount: Number(context.acceptedPathCount ?? deepForecast?.selectedPathCount ?? 0),
		failureReason:
			context.failureReason ||
			deepForecast?.failureReason ||
			worldState?.impactExpansion?.failureReason ||
			"",
		selectedDeepStateIds: Array.isArray(deepForecast?.selectedStateIds)
			? deepForecast.selectedStateIds
			: [],
...
function buildImpactExpansionDebugPayload(data = {}, worldState = null, runId = "") {
	...
	const convergence = {
		converged: qualityScore.composite >= 0.8,
		finalComposite: qualityScore.composite,
		predictedCritiqueIterations: qualityScore.composite < 0.8 ? 1 : 0,
	...
	return {
		...
		gateDetails: {
			...
			pathScoreThreshold: 0.5,
			acceptanceThreshold: 0.5,
			refinementQualityThreshold: 0.8,
		},
		selectedPaths: (data?.deepPathEvaluation?.selectedPaths || [])
			.map(summarizeImpactPathScore)
			.filter(Boolean),
		rejectedPaths: (data?.deepPathEvaluation?.rejectedPaths || [])
			.map(summarizeImpactPathScore)
			.filter(Boolean),
...
async function writeForecastRunStatusArtifact({
	...
	const keys = buildForecastTraceArtifactKeys(
		runId,
		generatedAt,
...
function logCanonicalPayloadDiagnostics(predictions) {
	...
	console.log(
		`  [Publish] Canonical payload ${(totalBytes / 1024 / 1024).toFixed(2)}MB total (${avgBytes}B avg per forecast)`,
	);
	if (totalBytes > CANONICAL_PAYLOAD_SOFT_LIMIT_BYTES) {
		const topHeaviest = entries
			.slice(0, 3)
			.map((item) => `${item.id}:${(item.bytes / 1024).toFixed(1)}KB`)
			.join(", ");
		console.warn(
			`  [Publish] Canonical payload above soft limit ${Math.round(CANONICAL_PAYLOAD_SOFT_LIMIT_BYTES / 1024 / 1024)}MB; heaviest forecasts: ${topHeaviest}`,
		);
	}
}
...
function buildActorContinuitySummary(currentActors, priorWorldState = null) {
	const priorActors = Array.isArray(priorWorldState?.actorRegistry)
		? priorWorldState.actorRegistry
		: [];
	...
	for (const actor of currentActors) {
		...
		const domainExpansion = actor.domains.filter(
			(domain) => !(prev.domains || []).includes(domain),
		);
		const regionExpansion = actor.regions.filter(
			(region) => !(prev.regions || []).includes(region),
		);
		const domainContraction = (prev.domains || []).filter(
			(domain) => !actor.domains.includes(domain),
		);
		const regionContraction = (prev.regions || []).filter(
			(region) => !actor.regions.includes(region),
		);
		...
		if (influenceDelta >= 0.05 || domainExpansion.length > 0 || regionExpansion.length > 0) {
			...
			influenceDelta <= -0.05 ||
			domainContraction.length > 0 ||
			regionContraction.length > 0
		) {
			weakened.push({
				id: actor.id,
				name: actor.name,
...
function buildForecastBranchStates(predictions) {
	...
	for (const pred of predictions) {
		const branchList =
			pred.caseFile?.branches ||
			buildForecastBranches(pred, {
		...
		for (const branch of branchList) {
			branches.push({
				...
				probabilityDelta: +((branch.projectedProbability || 0) - (pred.probability || 0)).toFixed(
					3,
				),
				summary: branch.summary,
				outcome: branch.outcome,
				...
				evidenceSample: (pred.caseFile?.supportingEvidence || [])
					.slice(0, 2)
					.map((item) => item.summary),
				counterEvidenceSample: (pred.caseFile?.counterEvidence || [])
					.slice(0, 2)
					.map((item) => item.summary),
			});
		}
	...
	return branches.sort(
		(a, b) => b.projectedProbability - a.projectedProbability || a.id.localeCompare(b.id),
	);
}

function buildBranchContinuitySummary(currentBranchStates, priorWorldState = null) {
	const priorBranchStates = Array.isArray(priorWorldState?.branchStates)
		? priorWorldState.branchStates
		: [];
	...
	for (const branch of currentBranchStates) {
		...
		const delta = +((branch.projectedProbability || 0) - (prev.projectedProbability || 0)).toFixed(
			3,
		);
		const actorDelta = branch.actorIds.filter((id) => !(prev.actorIds || []).includes(id));
		const triggerDelta = branch.triggerSample.filter(
			(item) => !(prev.triggerSample || []).includes(item),
		);
...
function formatSituationDomainLabel(domains = []) {
	const cleaned = uniqueSortedStrings(
		(domains || [])
			.map((value) =>
				String(value || "")
					.replace(/_/g, " ")
...
function formatSituationLabel(cluster) {
	const leadRegion =
		pickDominantSituationValue(cluster._regionCounts, cluster.regions) ||
		cluster.regions[0] ||
		"Cross-regional";
...
function filterSpecificSituationTokens(tokens = []) {
	return uniqueSortedStrings(
		(tokens || []).filter(
			(token) => token && token.length >= 4 && !FAMILY_GENERIC_TOKENS.has(token),
		),
	);
}
...
function buildSituationCandidate(prediction) {
	...
	const tokens = uniqueSortedStrings([
		...
		...(prediction.caseFile?.supportingEvidence || []).flatMap((item) =>
			normalizeSituationText(item?.summary),
		),
	...
	return {
		...
		actors: uniqueSortedStrings(
			(prediction.caseFile?.actors || []).map((actor) => actor.name || actor.id).filter(Boolean),
		),
		branchKinds: uniqueSortedStrings(
			(prediction.caseFile?.branches || []).map((branch) => branch.kind).filter(Boolean),
		),
		tokens,
		specificTokens,
		signalTypes: uniqueSortedStrings(
			(prediction.signals || []).map((signal) => signal?.type).filter(Boolean),
		),
...
function shouldMergeSituationCandidate(candidate, cluster, score) {
	...
	const specificTokenOverlap = intersectCount(
		candidate.specificTokens,
		cluster.specificTokens || [],
	);
	...
	const channelOverlap = intersectCount(
		candidate.transmissionChannels,
	...
	const isRegionalLogistics =
		MARKET_CLUSTER_DOMAINS.has(candidateDomain) || isMarketLikeDomains(cluster.domains);

	if (isRegionalLogistics) {
		if (sourceStateOverlap > 0) return true;
		if (
			candidate.sourceStateIds.length > 0 &&
			(cluster.sourceStateIds || []).length > 0 &&
			sourceStateOverlap === 0
		) {
			return false;
		}
		if (regionOverlap === 0 && macroOverlap === 0) return false;
		if (bucketOverlap === 0) return false;
		if (regionOverlap > 0 && (channelOverlap > 0 || signalOverlap > 0 || specificTokenOverlap >= 1))
			return true;
		if (
			macroOverlap > 0 &&
			bucketOverlap > 0 &&
			(channelOverlap > 0 || signalOverlap >= 2 || specificTokenOverlap >= 2)
		)
			return true;
	...
	const broadPressureDomain =
		isBroadNonMaritimePressureDomains(candidate.domains) ||
		isBroadNonMaritimePressureDomains(cluster.domains);
	if (
		broadPressureDomain &&
		regionOverlap === 0 &&
		!hasNonMaritimeMergeSpine({
			macroOverlap,
			actorOverlap,
			bucketOverlap,
			channelOverlap,
...
function buildSituationClusters(predictions) {
	...
	for (const prediction of predictions) {
		...
		bestCluster.macroRegions = uniqueSortedStrings([
			...bestCluster.macroRegions,
			...candidate.macroRegions,
		]);
		...
		bestCluster.branchKinds = uniqueSortedStrings([
			...bestCluster.branchKinds,
			...candidate.branchKinds,
		]);
		bestCluster.tokens = uniqueSortedStrings([...bestCluster.tokens, ...candidate.tokens]).slice(
			0,
			28,
		);
		bestCluster.specificTokens = uniqueSortedStrings([
			...bestCluster.specificTokens,
			...candidate.specificTokens,
		]).slice(0, 20);
		bestCluster.signalTypes = uniqueSortedStrings([
			...bestCluster.signalTypes,
			...candidate.signalTypes,
		]);
		bestCluster.marketBucketIds = uniqueSortedStrings([
			...bestCluster.marketBucketIds,
			...candidate.marketBucketIds,
		]);
		bestCluster.transmissionChannels = uniqueSortedStrings([
			...bestCluster.transmissionChannels,
			...candidate.transmissionChannels,
		]);
		bestCluster.sourceStateIds = uniqueSortedStrings([
			...bestCluster.sourceStateIds,
			...candidate.sourceStateIds,
		]);
...
function formatSituationFamilyLabel(family) {
	const regionEntries = Object.entries(family._regionCounts || {}).sort(
		(a, b) => b[1] - a[1] || a[0].localeCompare(b[0]),
	);
	...
	const hasClearLeadRegion =
		leadCount > 0 && leadCount >= Math.ceil(totalSituations * 0.5) && leadCount > secondCount;
...
function inferSituationFamilyArchetype(input = {}) {
	const domains = uniqueSortedStrings(
		[input.dominantDomain, ...(input.domains || [])].filter(Boolean),
	);
	const signals = uniqueSortedStrings((input.signalTypes || []).filter(Boolean));
	const tokens = uniqueSortedStrings(
		[...(input.tokens || []), ...(input.specificTokens || [])].filter(Boolean),
	);
	const hasMaritimeSignal = signals.some((item) => ["chokepoint", "gps_jamming"].includes(item));
	const hasStrongMaritimeToken = tokens.some((token) =>
		["shipping", "freight", "maritime", "logistics", "vessel", "rerouting"].includes(token),
	);
	const hasRouteToken = tokens.some((token) =>
		["port", "corridor", "transit", "route", "strait", "sea"].includes(token),
	);
	...
	if (
		domains.includes("supply_chain") ||
		hasMaritimeSignal ||
		(hasStrongMaritimeToken && hasRouteToken)
	) {
...
function buildSituationFamilyCandidate(cluster) {
	...
	return {
		cluster,
		regions: uniqueSortedStrings(
			[cluster.dominantRegion, ...(cluster.regions || [])].filter(Boolean),
		),
		macroRegions: uniqueSortedStrings(cluster.macroRegions || []),
		domains: uniqueSortedStrings(
			[cluster.dominantDomain, ...(cluster.domains || [])].filter(Boolean),
		),
		...
		regionTokens: extractRegionLinkTokens([
			cluster.dominantRegion,
			...(cluster.regions || []),
		]).slice(0, 8),
		signalTypes: uniqueSortedStrings(
			(cluster.topSignals || []).map((signal) => signal.type).filter(Boolean),
		),
...
function shouldMergeSituationFamilyCandidate(candidate, family, score) {
	...
	const archetypeMatch =
		candidate.archetype && family.archetype && candidate.archetype === family.archetype;
	...
	if (marketLike) {
		...
		if (
			candidate.sourceStateIds.length > 0 &&
			(family.sourceStateIds || []).length > 0 &&
			sourceStateOverlap === 0
		)
			return false;
		if (regionOverlap === 0 && macroOverlap === 0) return false;
		if (bucketOverlap === 0) return false;
		if (archetypeMatch && (channelOverlap > 0 || specificTokenOverlap > 0 || regionOverlap > 0))
			return true;
	...
	const broadPressureDomain =
		isBroadNonMaritimePressureDomains(candidate.domains) ||
		isBroadNonMaritimePressureDomains(family.domains);
	if (
		broadPressureDomain &&
		regionOverlap === 0 &&
		!hasNonMaritimeMergeSpine({
	...
	if (
		regionOverlap > 0 &&
		archetypeMatch &&
		(domainOverlap > 0 || signalOverlap > 0 || specificTokenOverlap > 0)
	)
		return true;
	if (actorOverlap > 0 && archetypeMatch && (domainOverlap > 0 || specificTokenOverlap > 0))
		return true;
	if (regionOverlap > 0 && actorOverlap > 0 && (specificTokenOverlap > 0 || signalOverlap > 0))
		return true;
	if (
		domainOverlap > 0 &&
		archetypeMatch &&
		signalOverlap >= 2 &&
		specificTokenOverlap >= 2 &&
		regionTokenOverlap > 0
	)
		return true;
	return false;
}
...
function finalizeSituationFamily(family) {
	...
	return {
		...
		avgProbability: +(family._probabilityTotal / Math.max(1, family.situationIds.length)).toFixed(
			3,
		),
	};
}
...
function buildSituationFamilies(situationClusters = []) {
	const families = [];
	const orderedClusters = [...(situationClusters || [])].sort(
		(a, b) =>
	...
	for (const cluster of orderedClusters) {
		...
		bestFamily.macroRegions = uniqueSortedStrings([
			...bestFamily.macroRegions,
			...candidate.macroRegions,
		]);
		...
		bestFamily.signalTypes = uniqueSortedStrings([
			...bestFamily.signalTypes,
			...candidate.signalTypes,
		]);
		bestFamily.tokens = uniqueSortedStrings([...bestFamily.tokens, ...candidate.tokens]).slice(
			0,
			32,
		);
		bestFamily.specificTokens = uniqueSortedStrings([
			...bestFamily.specificTokens,
			...(candidate.specificTokens || []),
		]).slice(0, 24);
		bestFamily.regionTokens = uniqueSortedStrings([
			...bestFamily.regionTokens,
			...(candidate.regionTokens || []),
		]).slice(0, 12);
		bestFamily.marketBucketIds = uniqueSortedStrings([
			...bestFamily.marketBucketIds,
			...(candidate.marketBucketIds || []),
		]);
		bestFamily.transmissionChannels = uniqueSortedStrings([
			...bestFamily.transmissionChannels,
			...(candidate.transmissionChannels || []),
		]);
		bestFamily.sourceStateIds = uniqueSortedStrings([
			...bestFamily.sourceStateIds,
			...(candidate.sourceStateIds || []),
		]);
...
function classifySimulationStateKind(cluster, family = null) {
	const dominantDomain =
		family?.dominantDomain || cluster?.dominantDomain || cluster?.domains?.[0] || "";
	const archetype = family?.archetype || "";
	const signalTypes = uniqueSortedStrings(
		(cluster?.topSignals || []).map((signal) => signal.type).filter(Boolean),
	);

	if (archetype === "war_theater" || ["conflict", "military"].includes(dominantDomain))
		return "security_escalation";
	if (archetype === "maritime_supply" || dominantDomain === "supply_chain")
		return "maritime_disruption";
	if (archetype === "cyber_pressure" || dominantDomain === "cyber") return "cyber_pressure";
	if (archetype === "infrastructure_fragility" || dominantDomain === "infrastructure")
		return "infrastructure_fragility";
	...
	if (archetype === "political_instability" || dominantDomain === "political") {
		if (
			signalTypes.some((type) =>
				["unrest", "unrest_events", "election", "sanctions"].includes(type),
			)
		) {
...
function formatStateUnitLabel(unit) {
	const regionEntries = Object.entries(unit._regionCounts || {}).sort(
		(a, b) => b[1] - a[1] || a[0].localeCompare(b[0]),
	);
	const leadRegion =
		regionEntries[0]?.[0] || unit.dominantRegion || unit.regions?.[0] || "Cross-regional";
	const label =
		SIMULATION_STATE_KIND_LABELS[unit.stateKind] ||
		SIMULATION_STATE_KIND_LABELS.cross_domain_pressure;
...
function computeStateUnitOverlap(candidate, unit) {
	return (
		...
		(candidate.familyArchetype &&
		unit.familyArchetype &&
		candidate.familyArchetype === unit.familyArchetype
			? 1.5
			: 0) +
...
function shouldMergeStateUnitCandidate(candidate, unit, score) {
	...
	if (marketLike) {
		...
		if (
			sameFamily &&
			sameKind &&
			(channelOverlap > 0 || signalOverlap > 0 || specificTokenOverlap > 0 || regionOverlap > 0)
		)
			return true;
		if (
			sameKind &&
			macroOverlap > 0 &&
			bucketOverlap > 0 &&
			(channelOverlap > 0 || signalOverlap > 0 || specificTokenOverlap >= 2)
		)
			return true;
		return false;
	}
	...
	const broadPressureDomain =
		isBroadNonMaritimePressureDomains(candidate.domains) ||
		isBroadNonMaritimePressureDomains(unit.domains);
	if (
		broadPressureDomain &&
		regionOverlap === 0 &&
		!hasNonMaritimeMergeSpine({
	...
	if (
		sameFamily &&
		sameKind &&
		(signalOverlap > 0 || actorOverlap > 0 || regionOverlap > 0 || specificTokenOverlap > 0)
	)
		return true;
	if (!sameKind) return false;
	if (regionOverlap > 0 && (actorOverlap > 0 || signalOverlap > 0 || specificTokenOverlap >= 2))
		return true;
	if (actorOverlap > 0 && domainOverlap > 0 && (signalOverlap > 0 || specificTokenOverlap >= 2))
		return true;
	return false;
}
...
function buildCanonicalStateUnits(situationClusters = [], situationFamilies = []) {
	...
	for (const cluster of orderedClusters) {
		...
		bestUnit.macroRegions = uniqueSortedStrings([
			...bestUnit.macroRegions,
			...candidate.macroRegions,
		]);
		...
		bestUnit.marketBucketIds = uniqueSortedStrings([
			...bestUnit.marketBucketIds,
			...candidate.marketBucketIds,
		]);
		bestUnit.transmissionChannels = uniqueSortedStrings([
			...bestUnit.transmissionChannels,
			...candidate.transmissionChannels,
		]);
		bestUnit.sourceStateIds = uniqueSortedStrings([
			...bestUnit.sourceStateIds,
			...candidate.sourceStateIds,
		]);
		bestUnit.tokens = uniqueSortedStrings([...bestUnit.tokens, ...candidate.tokens]).slice(0, 32);
		bestUnit.specificTokens = uniqueSortedStrings([
			...bestUnit.specificTokens,
			...candidate.specificTokens,
		]).slice(0, 24);
		bestUnit.sourceSituationIds = uniqueSortedStrings([
			...bestUnit.sourceSituationIds,
			...candidate.sourceSituationIds,
		]);
		...
		bestUnit._probabilityTotal +=
			Number(cluster.avgProbability || 0) * Math.max(1, cluster.forecastCount || 1);
		bestUnit._confidenceTotal +=
			Number(cluster.avgConfidence || 0) * Math.max(1, cluster.forecastCount || 1);
		...
		for (const signal of cluster.topSignals || []) {
			const type = signal?.type || "unknown";
			bestUnit._signalCounts[type] =
				(bestUnit._signalCounts[type] || 0) + Number(signal?.count || 0 || 1);
		}
	}
	...
	return units
		...
		.sort(
			(a, b) =>
				b.forecastCount - a.forecastCount ||
				b.avgProbability - a.avgProbability ||
				a.label.localeCompare(b.label),
		)
		.map((unit) => {
			...
			const label =
				domainLabel && !seenLabels.has(domainLabel)
					? domainLabel
					: `${unit.label} (${unit.id.slice(-4)})`;
...
function buildSituationContinuitySummary(currentSituationClusters, priorWorldState = null) {
	const priorSituationClusters = Array.isArray(priorWorldState?.situationClusters)
		? priorWorldState.situationClusters
		: [];
	...
	for (const cluster of currentSituationClusters) {
		...
		const probabilityDelta = +((cluster.avgProbability || 0) - (prev.avgProbability || 0)).toFixed(
			3,
		);
	...
	return {
		...
		strengthenedSituationPreview: strengthened
			.sort(
				(a, b) =>
					b.probabilityDelta - a.probabilityDelta ||
		...
		weakenedSituationPreview: weakened
			.sort(
				(a, b) =>
					a.probabilityDelta - b.probabilityDelta ||
...
function computeCriticalBucketAlignment(bucketId, criticalSignalTypes = []) {
	...
	if (!overlap) return 0;
	return +clampUnitInterval(
		overlap / Math.min(4, supportedTypes.length) +
			Math.min(0.16, criticalSignalTypes.length * 0.035),
	).toFixed(3);
}
...
function getPredictionTransmissionChannels(prediction = {}) {
	const derived = getPredictionDerivedStateMeta(prediction);
	return uniqueSortedStrings(
		[derived?.channel || "", prediction.marketSelectionContext?.topChannel || ""].filter(Boolean),
	);
}
...
function getPredictionSourceStateIds(prediction = {}) {
	const derived = getPredictionDerivedStateMeta(prediction);
	return uniqueSortedStrings(
		[derived?.sourceStateId || "", prediction.stateContext?.id || ""].filter(Boolean),
	);
}
...
const STABILIZING_ACTION_MARKERS = [
	"prevent",
	"preserve",
	"contain",
	"protect",
...
const GENERIC_ACTOR_NAME_MARKERS = [
	"regional",
	"participants",
	"observers",
	"operators",
...
function scoreActorSpecificity(actorLike = {}) {
	...
	const genericNameHitCount = GENERIC_ACTOR_NAME_MARKERS.filter((item) =>
		actorName.includes(item),
	).length;
	let score = 0.55;
	...
	if (
		actorName.includes("command") ||
		actorName.includes("desk") ||
		actorName.includes("authority")
	)
		score -= 0.14;
...
function summarizeBranchDynamics(branches = []) {
	...
	const avgScore = (items, scorer) =>
		items.length ? items.reduce((sum, item) => sum + scorer(item), 0) / items.length : 0;
	return {
		escalatoryWeight: clampUnitInterval(
			avgScore(
				escalatory,
				(branch) => (branch.projectedProbability || 0) + Math.max(0, branch.probabilityDelta || 0),
			),
		),
		contrarianWeight: clampUnitInterval(
			avgScore(
				contrarian,
				(branch) =>
					(branch.projectedProbability || 0) + Math.max(0, -(branch.probabilityDelta || 0)),
			),
		),
...
function scoreActorAction(summary, stage, dominantDomain, actor) {
	...
	if (
		stage === "round_1" &&
		["conflict", "military", "political", "cyber"].includes(dominantDomain)
	)
		pressureBias += 0.12;
	if (stage === "round_3") {
		stabilizationBias += 0.08;
	...
	const pressureContribution = +(
		...
		pressureBias *
		0.6 *
		(profile.actionPressureMultiplier || 1)
	).toFixed(3);
	const stabilizationContribution = +(
		influence *
		stabilizationBias *
		0.6 *
		(profile.actionStabilizationMultiplier || 1)
	).toFixed(3);
	let intent = "mixed";
...
function inferActionChannels(summary, intent, dominantDomain) {
	...
	if (dominantDomain === "conflict" || dominantDomain === "military")
		channels.add("security_escalation");
	...
	if (/(mobiliz|retaliat|escalat|strike|attack|deploy|coordination)/.test(text))
		channels.add("security_escalation");
	if (/(sanction|policy|cabinet|election|messaging|posture)/.test(text))
		channels.add("political_pressure");
	if (/(repric|price|commodity|oil|contract|risk premium)/.test(text))
		channels.add("market_repricing");
	if (/(rerout|shipping|port|throughput|logistics|corridor|freight)/.test(text))
		channels.add("logistics_disruption");
	if (/(outage|continuity|service|grid|facility|capacity|harden)/.test(text))
		channels.add("service_disruption");
...
function compareTransmissionEdgePriority(left, right) {
	return (
		Number(right?.strength || 0) +
			...
			(Number(left?.strength || 0) + Number(left?.confidence || 0)) ||
...
function buildActorRoundActions(stage, situation, actors = []) {
	return actors.slice(0, 6).map((actor) => {
		...
		if (stage === "round_1") {
			summary =
				actor.likelyActions?.[0] ||
				actor.objectives?.[0] ||
				`Adjust posture around ${situation.dominantRegion || situation.label}.`;
		} else if (stage === "round_2") {
			summary =
				actor.likelyActions?.[1] ||
				actor.objectives?.[1] ||
				actor.likelyActions?.[0] ||
				`Respond to the evolving ${situation.label}.`;
		} else {
			summary = actor.constraints?.[0]
				? `Operate within ${actor.constraints[0]}`
				: actor.likelyActions?.[2] ||
					actor.constraints?.[1] ||
					`Manage spillover from ${situation.label}.`;
		}
		const effect = scoreActorAction(
			summary,
			stage,
			situation.dominantDomain || situation.domains?.[0] || "",
			actor,
		);
		const channels = inferActionChannels(
			summary,
			effect.intent,
			situation.dominantDomain || situation.domains?.[0] || "",
		);
...
function buildSimulationRound(stage, situation, context) {
	const { actors, branches, counterEvidence, supportiveEvidence, priorSimulation, marketContext } =
		context;
	...
	const actionPressure = actorActions.reduce(
		(sum, action) => sum + (action.pressureContribution || 0),
		0,
	);
	const actionStabilization = actorActions.reduce(
		(sum, action) => sum + (action.stabilizationContribution || 0),
		0,
	);
	const effectChannels = pickTopCountEntries(
		summarizeTypeCounts(actorActions.flatMap((action) => action.channels || [])),
		5,
	);
	...
	const marketBias =
		MARKET_BUCKET_SIMULATION_BIAS[marketContext?.topBucketId || ""] ||
		MARKET_BUCKET_SIMULATION_BIAS.sovereign_risk;
	...
	const marketResistance = clampUnitInterval(marketContradiction * marketBias.contradiction);
	...
	if (stage === "round_1") {
		...
		lead = marketContext?.topBucketLabel
			...
			: (situation.domains || []).length > 1
				? `${formatSituationDomainLabel(situation.domains)} spillover`
				: `${situation.domains[0] || "regional"} effects`;
	}

	const rawPressureDelta = pressureDelta;
	const rawStabilizationDelta = stabilizationDelta;
	const netPressure = +clampUnitInterval(
		(situation.avgProbability || 0) * 0.78 + (pressureDelta - stabilizationDelta) * 0.36,
	).toFixed(3);
...
function summarizeSimulationOutcome(rounds = [], dominantDomain = "") {
	...
	const netPressureDelta = rounds.length
		? +rounds
				.reduce(
					(sum, round) => sum + ((round.pressureDelta || 0) - (round.stabilizationDelta || 0)),
					0,
				)
				.toFixed(3)
		: 0;
...
function inferSimulationActorRole(actor = {}) {
	...
	if (domains.has("military") || /\bbrigade|army|navy|air force|command\b/.test(name))
		return "military_actor";
	if (domains.has("cyber") || likelyActions.includes("cyber")) return "cyber_operator";
	if (
		domains.has("supply_chain") ||
		likelyActions.includes("reroute") ||
		likelyActions.includes("shipping")
	)
		return "logistics_actor";
	if (
		domains.has("infrastructure") ||
		likelyActions.includes("repair") ||
		likelyActions.includes("harden")
	)
		return "infrastructure_operator";
	if (domains.has("market") || likelyActions.includes("hedge") || likelyActions.includes("reprice"))
		return "market_actor";
	if (
		domains.has("political") ||
		likelyActions.includes("sanction") ||
		likelyActions.includes("negotiate")
	)
		return "political_actor";
...
function inferSimulationEnvironmentArchetype(simulation = {}) {
	...
	if (domain === "supply_chain" || topChannels.has("logistics_disruption"))
		return "logistics_corridor";
	if (domain === "cyber" || topChannels.has("cyber_disruption")) return "cyber_pressure_network";
	if (domain === "infrastructure" || topChannels.has("service_disruption"))
		return "infrastructure_fragility";
	if (domain === "market" || topChannels.has("market_repricing")) return "market_repricing_zone";
	if (domain === "political" || topChannels.has("political_pressure"))
		return "political_pressure_complex";
	return "mixed_pressure_zone";
}

function buildSimulationEnvironmentSpec(
	_worldState,
	situationSimulations = [],
	priorWorldState = null,
) {
	const priorEnvironment = priorWorldState?.simulationState?.environmentSpec;
	const priorBySituation = new Map(
		(priorEnvironment?.situations || []).map((item) => [item.situationId, item]),
	);
	const situations = (situationSimulations || []).map((simulation) => {
		const actionCount = (simulation.actionPlan || []).reduce(
		...
		const actorRoles = summarizeTypeCounts(
			(simulation.actorPostures || []).map((actor) => inferSimulationActorRole(actor)),
		);
	...
	const summary = situations.length
		? `${situations.length} simulation environments were configured with ${Object.keys(familyArchetypes).length} archetype classes and ${
				pickTopCountEntries(familyArchetypes, 2)
					.map((item) => item.type)
					.join(", ") || "mixed"
			} as the leading environment patterns.`
...
function buildSimulationMemoryMutations(_worldState, simulationState, priorWorldState = null) {
	...
	const priorSituationMemory = new Map(
		(priorMemory?.situations || []).map((item) => [item.situationId, item]),
	);
	const priorActorMemory = new Map(
		(priorMemory?.actors || []).map((item) => [item.actorId || item.actorName, item]),
	);
	...
	const situations = (simulationState?.situationSimulations || []).map((simulation) => {
		...
		const persistentChannels = uniqueSortedStrings([
			...
			...(simulationState?.reportableInteractionLedger || [])
				.filter(
					(item) =>
						item.sourceSituationId === simulation.situationId ||
						item.targetSituationId === simulation.situationId,
				)
		...
		const pressureMemory = +clamp01(
			(simulation.totalPressure || 0) * 0.45 +
				...
				((simulation.rounds || []).reduce((sum, round) => sum + (round.netPressure || 0), 0) /
					Math.max((simulation.rounds || []).length, 1)) *
					0.15,
		).toFixed(3);
	...
	for (const action of simulationState?.actionLedger || []) {
		...
		group.roles.add(
			action.category ||
				inferSimulationActorRole({
					domains: [action.dominantDomain],
					likelyActions: [action.summary],
				}),
		);
	...
	const actors = [...actorGroups.values()]
		.map((group) => {
		...
		.sort(
			(a, b) =>
				Math.abs(b.netPressure) - Math.abs(a.netPressure) ||
				b.actionCount - a.actionCount ||
				a.actorName.localeCompare(b.actorName),
		);

	const links = buildInteractionGroups(simulationState?.reportableInteractionLedger || [])
		.map((group) => ({
			...
			memoryStrength: +(
				(group.avgConfidence || 0) * 0.55 +
				clamp01((group.score || 0) / 10) * 0.45
			).toFixed(3),
...
function buildSimulationCausalReplayChains(simulationState) {
	const simulationsById = new Map(
		(simulationState?.situationSimulations || []).map((item) => [item.situationId, item]),
	);
	const actionLedger = simulationState?.actionLedger || [];
	const interactionGroups = buildInteractionGroups(
		simulationState?.reportableInteractionLedger || [],
	);
	...
	for (const edge of causalEdges) {
		...
		const interactionGroup =
			interactionGroups.find(
				(group) =>
					group.sourceSituationId === edge.sourceSituationId &&
					group.targetSituationId === edge.targetSituationId &&
					(group.strongestChannel === edge.primaryChannel ||
						(edge.supportingChannels || []).includes(group.strongestChannel)),
			) || null;
		const trigger =
			(source?.pressureSignals || [])[0]?.type ||
		...
		const leadAction =
			actionLedger.find(
				(action) =>
					...
					(action.channels || []).some((channel) =>
						(edge.supportingChannels || []).includes(channel),
					),
			) ||
			actionLedger.find((action) => action.situationId === edge.sourceSituationId) ||
			null;
		const stages = interactionGroup
			? [...(interactionGroup.stages || [])].sort()
	...
	for (const simulation of (simulationState?.situationSimulations || []).slice(0, 6)) {
		if (
			chains.some(
				(item) =>
					item.sourceSituationId === simulation.situationId && item.kind === "situation_resolution",
			)
		)
			continue;
		const trigger =
			(simulation.pressureSignals || [])[0]?.type ||
			simulation.branchSeeds?.[0]?.kind ||
			simulation.dominantDomain ||
			"pressure";
		const leadAction =
			actionLedger.find((action) => action.situationId === simulation.situationId) || null;
		chains.push({
	...
	return {
		...
		chains: chains
			.sort(
				(a, b) =>
					(b.confidence || 0) - (a.confidence || 0) || a.sourceLabel.localeCompare(b.sourceLabel),
			)
			.slice(0, 12),
	};
}
...
function buildSimulationMarketConsequences(simulationState, marketState, options = {}) {
	const simulations = Array.isArray(simulationState?.situationSimulations)
		? simulationState.situationSimulations
		: [];
	const bucketMap = new Map((marketState?.buckets || []).map((bucket) => [bucket.id, bucket]));
	...
	for (const simulation of simulations) {
		...
		for (const bucketId of primaryBucketIds) {
			...
			for (const [depth, candidateBucketId] of candidateBucketIds.entries()) {
				...
				const channel =
					bucketContext?.topChannel ||
					simulation.marketContext?.topChannel ||
					"derived_transmission";
				const supportingSignalTypes = uniqueSortedStrings([
					...(bucketContext?.supportingSignalTypes || []),
					...(simulation.marketContext?.criticalSignalTypes || []),
				]);
				const channelAllowed = isMarketBucketChannelAllowed(
					candidateBucketId,
					channel,
					consequenceType,
				);
				const bucketSupportSignalTypes =
					...
					[];
				const bucketSignalSupport = intersectCount(supportingSignalTypes, bucketSupportSignalTypes);
				...
				const coverageScore = computeMarketBucketCoverageScore(
					candidateBucketId,
					marketInputCoverage,
				);
				...
				const strength = clampUnitInterval(
					(simulation.marketContext?.confirmationScore || 0) * (direct ? 0.34 : 0.22) +
						(bucketContext?.topTransmissionStrength ||
							simulation.marketContext?.topTransmissionStrength ||
							0) *
							(direct ? 0.24 : 0.18) +
	...
	const deduped = [];
	const seen = new Set();
	for (const item of consequences.sort(
		(a, b) =>
			(b.reportableScore || 0) - (a.reportableScore || 0) ||
			b.strength + b.confidence - (a.strength + a.confidence) ||
			a.situationLabel.localeCompare(b.situationLabel),
	)) {
	...
	for (const item of internalItems) {
		...
		const criticalFloorRelief =
			item.consequenceType === "direct" && CORE_MARKET_BUCKET_IDS.includes(item.targetBucketId)
				? Math.min(
						0.08,
						Number(item.criticalAlignment || 0) * Number(item.criticalSignalLift || 0) * 0.14,
					)
				: 0;
		const lowCoverageRelief =
			item.consequenceType === "direct" &&
		...
		const minScore = Math.max(
			0.34,
			(MARKET_BUCKET_REPORTABLE_SCORE_FLOORS[item.targetBucketId] || 0.52) -
				criticalFloorRelief -
				lowCoverageRelief,
		);
		...
		if (item.targetBucketId === "defense") {
			const defenseEligible =
				...
				((item.strength || 0) >= 0.58 &&
					(item.confidence || 0) >= 0.5 &&
					["conflict", "military"].includes(item.dominantDomain));
			if (!defenseEligible) {
	...
	if (reportableItems.length === 0) {
		const fallback = internalItems.find(
			(item) =>
				["market", "supply_chain"].includes(item.dominantDomain) &&
				CORE_MARKET_BUCKET_IDS.includes(item.targetBucketId) &&
...
function buildSituationSimulationState(worldState, priorWorldState = null) {
	...
	const supporting = Array.isArray(worldState?.evidenceLedger?.supporting)
		? worldState.evidenceLedger.supporting
		: [];
	const counter = Array.isArray(worldState?.evidenceLedger?.counter)
		? worldState.evidenceLedger.counter
		: [];
	const familyIndex = buildSituationFamilyIndex(worldState?.situationFamilies || []);
	const simulationSources =
		Array.isArray(worldState?.stateUnits) && worldState.stateUnits.length
	...
	const observedMarketContextIndex =
		marketContextByRound?.observed ||
		buildSituationMarketContextIndex(
	...
	const compatiblePriorSimulations =
		priorSimulationState?.version === SIMULATION_STATE_VERSION
			? priorSimulationState?.situationSimulations || []
			: [];
	const priorSimulations = new Map(
		compatiblePriorSimulations.map((item) => [item.situationId, item]),
	);

	const situationSimulations = simulationSources.map((source) => {
		const sourceSituationIds = uniqueSortedStrings(
			source.sourceSituationIds || source.situationIds || [source.id],
		);
		const forecastIds = source.forecastIds || [];
		const actors = actorRegistry.filter((actor) =>
			intersectAny(actor.forecastIds || [], forecastIds),
		);
		const branches = branchStates.filter((branch) => forecastIds.includes(branch.forecastId));
		const supportingEvidence = supporting
			.filter((item) => forecastIds.includes(item.forecastId))
			.slice(0, 8);
		const counterEvidence = counter
			.filter((item) => forecastIds.includes(item.forecastId))
			.slice(0, 8);
		...
		const roundContexts = {
			round_1:
				(marketContextByRound?.round_1 || observedMarketContextIndex).bySituationId.get(
					source.id,
				) || observedMarketContext,
			round_2:
				(
					marketContextByRound?.round_2 ||
					marketContextByRound?.round_1 ||
					observedMarketContextIndex
				).bySituationId.get(source.id) || observedMarketContext,
			round_3:
				(
					marketContextByRound?.round_3 ||
					marketContextByRound?.round_2 ||
					observedMarketContextIndex
				).bySituationId.get(source.id) || observedMarketContext,
		};
		...
		const rounds = [
			buildSimulationRound("round_1", source, {
				actors,
				branches,
				counterEvidence,
				supportiveEvidence: supportingEvidence,
				priorSimulation,
				marketContext: roundContexts.round_1,
			}),
			buildSimulationRound("round_2", source, {
				actors,
				branches,
				counterEvidence,
				supportiveEvidence: supportingEvidence,
				priorSimulation,
				marketContext: roundContexts.round_2,
			}),
			buildSimulationRound("round_3", source, {
				actors,
				branches,
				counterEvidence,
				supportiveEvidence: supportingEvidence,
				priorSimulation,
				marketContext: roundContexts.round_3,
			}),
		];
		const outcome = summarizeSimulationOutcome(
			rounds,
			source.dominantDomain || source.domains?.[0] || "",
		);
		const effectChannelWeights = {};
		for (const round of rounds) {
			for (const item of round.effectChannels || []) {
				effectChannelWeights[item.type] =
					(effectChannelWeights[item.type] || 0) + (item.count || 0);
			}
		}
		...
		return {
			...
			stabilizers: uniqueSortedStrings(
	...
	const reportableInteractionLedger = buildReportableInteractionLedger(
		interactionLedger,
		situationSimulations,
		{
	...
	const blockedInteractions = Array.isArray(reportableInteractionLedger.blocked)
		? reportableInteractionLedger.blocked
		: [];
	const replayTimeline = buildSimulationReplayTimeline(
		situationSimulations,
		actionLedger,
		interactionLedger,
	);
	...
	const environmentSpec = buildSimulationEnvironmentSpec(
		worldState,
		situationSimulations,
		priorWorldState,
	);
	const memoryMutations = buildSimulationMemoryMutations(
		worldState,
		{
			situationSimulations,
			actionLedger,
	...
	const roundTransitions = ["round_1", "round_2", "round_3"].map((stage) => {
		...
		const avgNetPressure = roundSlice.length
			? +(
					roundSlice.reduce((sum, round) => sum + (round.netPressure || 0), 0) / roundSlice.length
				).toFixed(3)
			: 0;
		return {
			stage,
			...
			avgNetPressure,
			leadSignals: pickTopCountEntries(
				summarizeTypeCounts(roundSlice.flatMap((round) => round.signalTypes || [])),
				4,
			),
			leadActions: uniqueSortedStrings(
				roundSlice.flatMap((round) =>
					(round.actions || []).map((action) => action.summary).filter(Boolean),
				),
			).slice(0, 6),
		};
	});
...
function buildSimulationInteractionLedger(actionLedger = [], situationSimulations = []) {
	const simulationsById = new Map(
		(situationSimulations || []).map((item) => [item.situationId, item]),
	);
	const ledger = [];
	...
	function pickInteractionChannel(sharedChannels, sourceSimulation, targetSimulation) {
		const targetSensitivity = new Set(
			getTargetSensitivityChannels(targetSimulation?.dominantDomain),
		);
		const sourceChannelWeights = new Map(
			(sourceSimulation?.effectChannels || []).map((item) => [item.type, Number(item.count || 0)]),
		);
		return (
			uniqueSortedStrings(sharedChannels)
				.map((channel) => ({
					channel,
				...
				.sort(
					(a, b) =>
						b.usable - a.usable || b.weight - a.weight || a.channel.localeCompare(b.channel),
				)[0]?.channel || ""
		);
	}
	...
	function pushInteraction(source, target, stage) {
		...
		const sharedActor =
			...
			avgSpecificity >= 0.75;
		const sharedChannels = uniqueSortedStrings(
			(source.channels || []).filter((channel) => (target.channels || []).includes(channel)),
		);
		...
		const strongestChannel = pickInteractionChannel(
			sharedChannels,
			sourceSimulation,
			targetSimulation,
		);
		...
		ledger.push({
			...
			directLinkCount:
				(sharedActor ? 1 : 0) + (regionLink ? 1 : 0) + (sharedChannels.length > 0 ? 1 : 0),
			score: +score.toFixed(3),
			confidence: +(
				(sharedActor ? 0.38 : 0) +
				(regionLink ? 0.22 : 0) +
...
function buildSimulationReplayTimeline(
	situationSimulations = [],
	actionLedger = [],
	interactionLedger = [],
) {
	...
	return stages.map((stage) => {
		...
		return {
			...
			leadChannels: pickTopCountEntries(
				summarizeTypeCounts(actions.flatMap((action) => action.channels || [])),
				5,
			),
			leadActions: uniqueSortedStrings(
				actions.map((action) => action.summary).filter(Boolean),
			).slice(0, 6),
...
function buildReportableInteractionLedger(
	interactionLedger = [],
	situationSimulations = [],
	options = {},
) {
	const simulationIndex = new Map(
		(situationSimulations || []).map((item) => [item.situationId, item]),
	);
	const reportable = [];
	const blocked = [];
	...
	for (const item of interactionLedger || []) {
		...
		const sharedActor =
			Boolean(item.sharedActor) || intersectCount(source.actorIds || [], target.actorIds || []) > 0;
		const regionLink =
			Boolean(item.regionLink) || intersectCount(source.regions || [], target.regions || []) > 0;
		const crossTheater = isCrossTheaterPair(source.regions || [], target.regions || []);
		const bucketOverlap = intersectCount(
			source.marketContext?.linkedBucketIds || [],
			target.marketContext?.linkedBucketIds || [],
		);
		...
		const purelyPoliticalPair =
			source.dominantDomain === "political" && target.dominantDomain === "political";
		...
		if (politicalChannel) {
			...
			if (crossTheater) {
				const structuralPoliticalCarryover =
					purelyPoliticalPair &&
					sharedActor &&
					specificity >= 0.82 &&
					confidence >= 0.7 &&
				...
				if (
					!regionLink &&
					(!sharedActor || specificity < 0.82 || confidence < 0.68 || score < 5.4)
				) {
		...
		if (
			confidence >=
				(crossTheater && (politicalChannel || item.strongestChannel === "market_repricing")
					? crossTheaterConfidenceFloor
					: genericConfidenceFloor) &&
			score >=
				(crossTheater && (politicalChannel || item.strongestChannel === "market_repricing")
					? crossTheaterScoreFloor
					: genericScoreFloor) &&
		...
		if (
			directOverlap &&
			confidence >= (strictMode ? 0.64 : 0.58) &&
			score >= (strictMode ? 4.9 : 4.5)
		) {
			reportable.push(item);
			continue;
		}
		if (
			sharedActor &&
			specificity >= (strictMode ? 0.82 : 0.76) &&
			confidence >= (strictMode ? 0.66 : 0.6) &&
			(regionLink || marketLinked)
		) {
			reportable.push(item);
			continue;
		}
		if (
			!politicalChannel &&
			marketLinked &&
			confidence >= (strictMode ? 0.72 : 0.66) &&
			score >= (strictMode ? 5.2 : 4.8) &&
			(bucketOverlap > 0 || regionLink || sharedActor)
		) {
			reportable.push(item);
			continue;
		}
	...
	for (const item of reportable) {
		...
		const rightStrength = current
			? Number(current.score || 0) * Number(current.confidence || 0)
			: -1;
		if (
			!current ||
			...
			(leftStrength === rightStrength &&
				Number(item.score || 0) === Number(current.score || 0) &&
				Number(item.confidence || 0) > Number(current.confidence || 0)) ||
			(leftStrength === rightStrength &&
				Number(item.score || 0) === Number(current.score || 0) &&
				Number(item.confidence || 0) === Number(current.confidence || 0) &&
				String(item.sourceLabel || "").localeCompare(String(current.sourceLabel || "")) < 0) ||
			(leftStrength === rightStrength &&
				Number(item.score || 0) === Number(current.score || 0) &&
				Number(item.confidence || 0) === Number(current.confidence || 0) &&
				String(item.sourceLabel || "") === String(current.sourceLabel || "") &&
				String(item.targetLabel || "").localeCompare(String(current.targetLabel || "")) < 0)
		) {
			strongestByKey.set(key, item);
		}
	}

	const ordered = [...strongestByKey.values()].sort(
		(a, b) =>
			Number(b.score || 0) * Number(b.confidence || 0) -
				Number(a.score || 0) * Number(a.confidence || 0) ||
...
function buildInteractionGroups(interactions = []) {
	...
	for (const interaction of interactions || []) {
		...
		group.directLinkCount = Math.max(
			group.directLinkCount,
			Number(interaction.directLinkCount || 0),
		);
...
function buildSimulationCausalGraph(simulationState, priorWorldState = null) {
	const simulations = Array.isArray(simulationState?.situationSimulations)
		? simulationState.situationSimulations
		: [];
	const reportableEffects = Array.isArray(simulationState?.reportableEffects)
		? simulationState.reportableEffects
		: [];
	const groupedInteractions = buildInteractionGroups(
		simulationState?.reportableInteractionLedger || [],
	);
	...
	const priorCausalGraph =
		priorWorldState?.simulationState?.causalGraph?.version === SIMULATION_STATE_VERSION
			? priorWorldState.simulationState.causalGraph
			: null;
	const priorEdges = new Map(
		(priorCausalGraph?.edges || []).map((edge) => [
			buildCausalGraphKey(edge.sourceSituationId, edge.targetSituationId, edge.effectClass),
			edge,
		]),
	);
	...
	for (const effect of reportableEffects) {
		const key = buildCausalGraphKey(
			effect.sourceSituationId,
			effect.targetSituationId,
			effect.effectClass,
		);
		const interactionMatches = groupedInteractions.filter(
			(group) =>
				...
				(group.strongestChannel === effect.channel ||
					inferSystemEffectRelationFromChannel(
						group.strongestChannel,
						simulations.find((item) => item.situationId === effect.targetSituationId)
							?.dominantDomain,
					) === effect.relation),
		);
		const entry = groupedEdges.get(key) || {
			...
			directLinkCount: 0,
			sourcePosture:
				simulations.find((item) => item.situationId === effect.sourceSituationId)?.posture || "",
			sourcePostureScore:
				simulations.find((item) => item.situationId === effect.sourceSituationId)?.postureScore ||
				0,
		...
		for (const match of interactionMatches) {
			...
			const memory = linkMemory.get(
				`${match.sourceSituationId}:${match.targetSituationId}:${match.strongestChannel}`,
			);
	...
	const edges = [...groupedEdges.values()]
		.map((edge) => {
			const channels = uniqueSortedStrings([
				...(edge.channels || []),
				...(edge.supportingChannels || []),
			]);
			const primaryChannel =
				channels.slice().sort((left, right) => {
			...
			const continuityDelta = prior
				? +(
						edge.confidence +
						edge.memorySupport * 0.35 -
						((prior.confidence || 0) + (prior.memorySupport || 0) * 0.35)
					).toFixed(3)
	...
	const resolvedEdges = (priorCausalGraph?.edges || [])
		.filter(
			(prior) =>
				!groupedEdges.has(
					buildCausalGraphKey(prior.sourceSituationId, prior.targetSituationId, prior.effectClass),
				),
		)
		.slice(0, 12)
		.map((edge) => ({
...
function computeReportableEffectConfidence(group, source, target, strongestChannelWeight) {
	const structuralSharedActor =
		group.sharedActor || intersectCount(source?.actorIds || [], target?.actorIds || []) > 0;
	const structuralRegionLink =
		group.regionLink || intersectCount(source?.regions || [], target?.regions || []) > 0;
	const structuralDirectLinkCount = Math.max(
		Number(group.directLinkCount || 0),
		(structuralSharedActor ? 1 : 0) +
			(structuralRegionLink ? 1 : 0) +
			(strongestChannelWeight > 0 ? 1 : 0),
	);
	...
	const avgConfidence = clamp01(
		group.confidenceCount
			? Number(group.avgConfidence || 0)
			: Math.max(normalizedScore * 0.9, directLinkScore * 0.8),
	);
	const actorSpecificity = clamp01(
		group.actorSpecificityCount
			? Number(group.avgActorSpecificity || 0)
			: structuralSharedActor
				? 0.78
				: 0.62,
	);
	...
	if (
		(source?.dominantDomain || "") === "political" &&
		(target?.dominantDomain || "") !== "political"
	)
		confidence -= 0.05;
...
function buildSimulationReportInputs(worldState) {
	...
	const reportInputs = simulations.map((item) => ({
		...
		actionCount: (item.actionPlan || []).reduce(
			(sum, round) => sum + (round.actions || []).length,
			0,
		),
...
const MACRO_REGION_MAP = {
	...
	"Western Pacific": "EAST_ASIA",
	"South China Sea": "EAST_ASIA",
	"United States": "AMERICAS",
	Brazil: "AMERICAS",
	Mexico: "AMERICAS",
	Cuba: "AMERICAS",
...
const CROSS_THEATER_ACTOR_SPECIFICITY_MIN = 0.9;
...
function classifyEffectClass(channel, relation = "") {
	...
	if (channel === "logistics_disruption" || channel === "service_disruption")
		return "logistics_spillover";
	if (channel === "market_repricing") return "market_spillover";
	if (channel === "political_pressure" || relation === "regional pressure transfer")
		return "political_spillover";
...
function getEffectClassScoreThreshold(effectClass, context = {}) {
	...
	if (effectClass === "security_spillover")
		return sameMacroRegion && repeatedStages >= 2 ? 4.2 : 4.8;
...
function canEmitCrossSituationEffect(
	source,
	strongestChannel,
	strongestChannelWeight,
	hasDirectStructuralLink = false,
) {
	...
	if (
		...
		return false;
	if (
		(source?.posture || "") !== "escalatory" &&
		(source?.totalPressure || 0) <= (source?.totalStabilization || 0)
	)
		return false;
	return true;
}
...
function buildInteractionWatchlist(interactions = []) {
	...
	return [...groupedPairs.values()]
		...
		.map((item) => ({
			...
			summary: `${item.sourceLabel} interacted with ${item.targetLabel} across ${item.stages.size} round(s) via ${uniqueSortedStrings(
				[...item.channels],
			)
				.map((channel) => channel.replace(/_/g, " "))
				.join(
					", ",
				)}, with ${(item.confidence * 100).toFixed(0)}% report confidence and ${item.actorCount} named actors involved.`,
		}));
}
...
function buildCrossSituationEffects(simulationState, options = {}) {
	...
	const interactions =
		mode === "reportable"
			? Array.isArray(simulationState?.reportableInteractionLedger)
				? simulationState.reportableInteractionLedger
				: Array.isArray(simulationState?.interactionLedger)
					? simulationState.interactionLedger
					: []
			: Array.isArray(simulationState?.interactionLedger)
				? simulationState.interactionLedger
				: [];
	...
	if (interactionGroups.length > 0) {
		...
		for (const group of interactionGroups) {
			...
			const relation = inferSystemEffectRelationFromChannel(
				group.strongestChannel,
				target.dominantDomain,
			);
			if (!relation) continue;
			const strongestChannelWeight =
				(source.effectChannels || []).find((item) => item.type === group.strongestChannel)?.count ||
				0;
			const hasRegionLink =
				group.regionLink || intersectCount(source.regions || [], target.regions || []) > 0;
			const hasSharedActor =
				group.sharedActor || intersectCount(source.actorIds || [], target.actorIds || []) > 0;
			...
			if (
				...
				(!hasSharedActor ||
					Number(group.avgActorSpecificity || 0) < CROSS_THEATER_ACTOR_SPECIFICITY_MIN)
			) {
				blockedEffects.push({
					sourceSituationId: source.situationId,
					sourceLabel: source.label,
			...
			const confidence = computeReportableEffectConfidence(
				group,
				source,
				target,
				strongestChannelWeight,
			...
			const score = +(
				...
				(repeatedStages >= 2 && ["cyber_spillover", "logistics_spillover"].includes(effectClass)
					? 0.4
					: 0)
			).toFixed(3);
	...
	for (let i = 0; i < simulations.length; i++) {
		...
		for (let j = 0; j < simulations.length; j++) {
			...
			const labelTokenOverlap = intersectCount(
				normalizeSituationText(source.label).filter(
					(token) =>
						![
							"situation",
							"conflict",
				...
				normalizeSituationText(target.label).filter(
					(token) =>
						![
							"situation",
							"conflict",
			...
			const hasDirectObservableLink =
				regionOverlap > 0 || actorOverlap > 0 || labelTokenOverlap > 0;
			if (channelOverlap === 0) continue;
			if (!hasDirectObservableLink) continue;

			const strongestChannelEntry =
				(source.effectChannels || [])
					.slice()
					.sort((a, b) => b.count - a.count || a.type.localeCompare(b.type))
					.find((item) => targetSensitivity.includes(item.type)) || null;
			...
			const fallbackThreshold =
				mode === "reportable" && effectClass === "political_spillover" && hasDirectStructuralLink
					? Math.max(0.5, confidenceThreshold - 0.08)
					: confidenceThreshold;
			if (fallbackConfidence < fallbackThreshold) continue;

			const score =
				(source.posture === "escalatory" ? 2 : source.posture === "contested" ? 1 : 0) +
				channelOverlap * 2.5 +
				(familyLink ? 0.5 : 0) +
				regionOverlap * 2 +
				actorOverlap * 1.5 +
...
function attachSituationContext(
	predictions,
	situationClusters = buildSituationClusters(predictions),
) {
...
function projectSituationClusters(situationClusters, predictions) {
	...
	for (const cluster of situationClusters) {
		...
		const topSignals = Object.entries(signalCounts)
			...
			.slice(0, 4)
			.map(([type, count]) => ({ type, count }));
		const avgProbability = clusterPredictions.length
			? probabilityTotal / clusterPredictions.length
			: 0;
		const avgConfidence = clusterPredictions.length
			? confidenceTotal / clusterPredictions.length
			: 0;
	...
	return projected.sort(
		(a, b) => b.forecastCount - a.forecastCount || b.avgProbability - a.avgProbability,
	);
}
...
function buildWorldStateReport(worldState) {
	...
	const leadRegions = (worldState.regionalStates || []).slice(0, 4).map((item) => ({
		region: item.region,
	...
	const simulationSummary =
		worldState.simulationState?.summary || "No simulation-state summary is available.";
	...
	const reportableInteractionLedger = Array.isArray(
		worldState.simulationState?.reportableInteractionLedger,
	)
		? worldState.simulationState.reportableInteractionLedger
		: [];
	const blockedInteractionSummary =
		worldState.simulationState?.blockedInteractionSummary || summarizeBlockedInteractions([]);
	...
	const replayTimeline = Array.isArray(worldState.simulationState?.replayTimeline)
		? worldState.simulationState.replayTimeline
		: [];
	...
	const replayWatchlist = replayTimeline.slice().map((round) => ({
	...
	const environmentWatchlist = (worldState.simulationState?.environmentSpec?.situations || [])
		...
		.sort(
			(a, b) =>
				(b.activityIntensity || 0) - (a.activityIntensity || 0) || a.label.localeCompare(b.label),
		)
		.slice(0, 6)
		.map((item) => ({
	...
	const memoryWatchlist = (worldState.simulationState?.memoryMutations?.situations || [])
		...
		.sort(
			(a, b) =>
				Math.abs(b.memoryDelta || 0) - Math.abs(a.memoryDelta || 0) ||
				a.label.localeCompare(b.label),
		)
		.slice(0, 6)
		.map((item) => ({
	...
	const blockedEffectWatchlist = (worldState.simulationState?.blockedEffectSummary?.preview || [])
		.slice(0, 4)
		.map((item) => ({
	...
	const marketBuckets = Array.isArray(worldState.marketState?.buckets)
		? worldState.marketState.buckets
		: [];
	const transmissionEdges = Array.isArray(worldState.marketTransmission?.edges)
		? worldState.marketTransmission.edges
		: [];
	...
	const marketWatchlist = marketBuckets
		.slice()
		.sort(
			(a, b) => (b.pressureScore || 0) - (a.pressureScore || 0) || a.label.localeCompare(b.label),
		)
		.slice(0, 6)
		.map((bucket) => ({
	...
	const marketConsequenceWatchlist = marketConsequences.slice(0, 6).map((item) => ({
		type: `market_consequence_${item.targetBucketId}`,
		label: `${item.situationLabel} -> ${item.targetBucketLabel}`,
		summary: item.summary,
	}));
	const blockedMarketConsequenceWatchlist = (
		worldState.simulationState?.marketConsequences?.blockedSummary?.preview || []
	)
		.slice(0, 4)
		.map((item) => ({
	...
	const familyWatchlist = (worldState.situationFamilies || []).slice(0, 6).map((family) => ({
	...
	return {
		...
		replayTimeline,
		blockedEffectSummary:
			worldState.simulationState?.blockedEffectSummary || summarizeBlockedEffects([]),
		keyUncertainties: (worldState.uncertainties || [])
			.slice(0, 6)
...
function buildForecastRunContinuity(predictions) {
	...
	for (const pred of predictions) {
		if (
			pred.priorProbability == null ||
			pred.caseFile?.changeSummary?.startsWith("This forecast is new")
		) {
			newForecasts++;
		}
		...
		const delta = Math.abs(
			(pred.probability || 0) - (pred.priorProbability ?? pred.probability ?? 0),
		);
		changed.push({
			id: pred.id,
			title: pred.title,
...
function buildWorldSignals(inputs, predictions = [], _situationClusters = []) {
	...
	const stablecoins = Array.isArray(inputs?.stablecoinMarkets?.stablecoins)
		? inputs.stablecoinMarkets.stablecoins
		: [];
	...
	for (const cp of chokepoints) {
		...
		const riskScore = Number(
			cp.riskScore || cp.disruptionScore || (cp.riskLevel === "critical" ? 85 : 70),
		);
		...
		signals.push(
			buildWorldSignal(
				"shipping_cost_shock",
				"chokepoint",
				`${cp.name || region} disruption pressure`,
				{
		...
		if (commodity && /oil|gas|energy/i.test(commodity.commodity)) {
			signals.push(
				buildWorldSignal(
					"energy_supply_shock",
					"chokepoint",
					`${cp.name || region} energy exposure`,
					{
						sourceKey: cp.id || cp.name || region,
						region,
		...
		if (commodity) {
			signals.push(
				buildWorldSignal(
					"commodity_repricing",
					"chokepoint",
					`${commodity.commodity} repricing risk from ${cp.name || region}`,
					{
						sourceKey: cp.id || cp.name || region,
						region,
	...
	for (const index of shippingIndices) {
		...
		signals.push(
			buildWorldSignal("shipping_cost_shock", "shipping_rates", `${index.name} freight repricing`, {
	...
	for (const quote of commodityQuotes) {
		...
		signals.push(
			buildWorldSignal(
				type,
				"commodity_quotes",
				`${quote.name || quote.symbol} moved ${Number(quote.change || 0).toFixed(1)}%`,
				{
		...
		if (energyClass === "gas") {
			signals.push(
				buildWorldSignal(
					"gas_supply_stress",
					"commodity_quotes",
					`${quote.name || quote.symbol} is signalling gas-market stress`,
					{
						sourceKey: quote.symbol || quote.name,
						region: "Global",
						...
						domains: ["market", "supply_chain"],
						supportingEvidence: [
							`${quote.symbol || quote.name} moved ${Number(quote.change || 0).toFixed(1)}%`,
						],
					},
				),
			);
		...
		if (energyClass === "gold" && Number(quote.change || 0) >= 1.2) {
			signals.push(
				buildWorldSignal(
					"safe_haven_bid",
					"commodity_quotes",
					`${quote.name || quote.symbol} is catching a safe-haven bid`,
					{
						sourceKey: quote.symbol || quote.name,
						region: "Global",
						...
						domains: ["market", "political"],
						supportingEvidence: [
							`${quote.name || quote.symbol} rose ${Number(quote.change || 0).toFixed(1)}%`,
						],
					},
				),
			);
	...
	if (negativeStocks >= 4) {
		signals.push(
			buildWorldSignal("risk_off_rotation", "market_quotes", "Broad equity risk-off rotation", {
	...
	const energySector = sectorItems.find((item) =>
		/xle|energy/i.test(`${item.symbol || ""} ${item.name || ""}`),
	);
	if (energySector && Number(energySector.change || 0) >= 1.5) {
		signals.push(
			buildWorldSignal("commodity_repricing", "sector_summary", "Energy sector repricing", {
				...
				domains: ["market"],
				supportingEvidence: [
					`${energySector.symbol || energySector.name} ${Number(energySector.change || 0).toFixed(1)}%`,
				],
			}),
		);
	}

	const defenseSector = sectorItems.find((item) =>
		/ita|xar|defen|aerospace/i.test(`${item.symbol || ""} ${item.name || ""}`),
	);
	if (defenseSector && Number(defenseSector.change || 0) >= 1.2) {
		signals.push(
			buildWorldSignal("defense_repricing", "sector_summary", "Defense sector repricing", {
				...
				supportingEvidence: [
					`${defenseSector.symbol || defenseSector.name} ${Number(defenseSector.change || 0).toFixed(1)}%`,
				],
			}),
		);
	}
	...
	for (const quote of gulfQuotes) {
		...
		if (
			(quote.type === "oil" ||
				energyClass === "wti" ||
				energyClass === "brent" ||
				energyClass === "energy_generic") &&
			Math.abs(Number(quote.change || 0)) >= 1.5
		) {
			signals.push(
				buildWorldSignal(
					"energy_supply_shock",
					"gulf_quotes",
					`${quote.name} moved ${Number(quote.change || 0).toFixed(1)}%`,
					{
						sourceKey: quote.symbol || quote.name,
						region: "Middle East",
		...
		if (
			(quote.type === "gas" || energyClass === "gas") &&
			Math.abs(Number(quote.change || 0)) >= 1.8
		) {
			signals.push(
				buildWorldSignal(
					"gas_supply_stress",
		...
		if (spread >= 3) {
			signals.push(
				buildWorldSignal(
					"global_crude_spread_stress",
					"commodity_quotes",
					`Brent-WTI spread widened to ${spread.toFixed(1)}`,
					{
	...
	if (Math.abs(totalEtfFlow) > 100_000_000) {
		signals.push(
			buildWorldSignal(
				"risk_off_rotation",
				"etf_flows",
				totalEtfFlow > 0 ? "ETF inflow impulse" : "ETF outflow impulse",
				{
	...
	if (etfItems.filter((item) => item.direction === "outflow").length >= 5) {
		signals.push(
			buildWorldSignal("risk_off_rotation", "etf_flows", "ETF outflow breadth", {
				sourceKey: "market:etf-flows:v1",
				region: "Global",
	...
	if (cryptoWeakness >= 2) {
		signals.push(
			buildWorldSignal("risk_off_rotation", "crypto_quotes", "Crypto risk reduction", {
	...
	if (
		stablecoins.some(
			(coin) =>
				Number(coin.deviation || 0) >= 0.5 || /warning|depegged/i.test(coin.pegStatus || ""),
		)
	) {
		const stressed = stablecoins.filter(
			(coin) =>
				Number(coin.deviation || 0) >= 0.5 || /warning|depegged/i.test(coin.pegStatus || ""),
		);
		signals.push(
			buildWorldSignal("fx_stress", "stablecoins", "Stablecoin peg stress", {
				sourceKey: "market:stablecoins:v1",
				region: "Global",
				strength: normalize(
					Math.max(...stressed.map((coin) => Number(coin.deviation || 0))),
					0.5,
					3,
				),
				confidence: 0.62,
				domains: ["market", "cyber"],
				supportingEvidence: stressed
					.slice(0, 2)
					.map((coin) => `${coin.symbol} deviation ${coin.deviation}%`),
			}),
		);
	}
	...
	for (const rate of bisExchange) {
		...
		signals.push(
			buildWorldSignal("fx_stress", "bis_exchange", `${rate.countryName} exchange-rate stress`, {
	...
	for (const rate of bisPolicy) {
		...
		signals.push(
			buildWorldSignal(
				"policy_rate_pressure",
				"bis_policy",
				`${rate.countryName} policy-rate shift`,
				{
					sourceKey: rate.countryCode || rate.countryName,
					region: rate.countryName || "",
	...
	const economicCards = correlationCards.filter((item) =>
		/economic|market|sanctions/i.test(`${item.domain || ""} ${item.title || ""}`),
	);
	if (economicCards.length > 0) {
		signals.push(
			buildWorldSignal(
				"risk_off_rotation",
				"correlation_cards",
				"Economic stress correlations are active",
				{
					...
					confidence: 0.48,
					domains: ["market"],
					supportingEvidence: economicCards
						.slice(0, 2)
						.map((item) => item.title || item.summary || item.label || "economic correlation"),
				},
			),
		);
	}
	...
	if (Number.isFinite(vixLatest) && vixLatest >= 18) {
		signals.push(
			buildWorldSignal("volatility_shock", "fred", `VIX moved to ${vixLatest.toFixed(1)}`, {
				sourceKey: "VIXCLS",
				region: "Global",
	...
	if (Number.isFinite(curveLatest) && curveLatest <= 0.4) {
		signals.push(
			buildWorldSignal(
				"yield_curve_stress",
				"fred",
				`Yield-curve stress at ${curveLatest.toFixed(2)}`,
				{
					sourceKey: "T10Y2Y",
					region: "United States",
					macroRegion: "Americas",
	...
	if (Number.isFinite(fedFundsLatest) && fedFundsLatest >= 3.25) {
		signals.push(
			buildWorldSignal(
				"policy_rate_pressure",
				"fred",
				`Fed policy remains restrictive at ${fedFundsLatest.toFixed(2)}%`,
				{
					sourceKey: "FEDFUNDS",
					region: "United States",
					macroRegion: "Americas",
	...
	if (Number.isFinite(dgs10Latest) && dgs10Latest >= 4) {
		signals.push(
			buildWorldSignal(
				"policy_rate_pressure",
				"fred",
				`10Y Treasury yield at ${dgs10Latest.toFixed(2)}%`,
				{
					sourceKey: "DGS10",
					region: "United States",
					macroRegion: "Americas",
	...
	if (Number.isFinite(cpiYoY) && cpiYoY >= 2.4) {
		signals.push(
			buildWorldSignal(
				"inflation_impulse",
				"fred",
				`Consumer inflation is running near ${cpiYoY.toFixed(1)}% year-on-year`,
				{
					sourceKey: "CPIAUCSL",
					region: "United States",
					macroRegion: "Americas",
	...
	if (
		(Number.isFinite(unemploymentLatest) && unemploymentLatest >= 4.1) ||
		(Number.isFinite(unemploymentDelta) && unemploymentDelta >= 0.2)
	) {
		signals.push(
			buildWorldSignal(
				"labor_softness",
				"fred",
				`Labor-market softness at ${Number(unemploymentLatest || 0).toFixed(1)}% unemployment`,
				{
					sourceKey: "UNRATE",
					region: "United States",
					macroRegion: "Americas",
					strength: normalizeSignalStrength(
						Math.max(Number(unemploymentLatest || 0), Number(unemploymentDelta || 0) * 10),
						4,
						6.2,
					),
					confidence: 0.66,
					...
					supportingEvidence: [
						...
						Number.isFinite(unemploymentDelta)
							? `3-month change ${unemploymentDelta.toFixed(1)} points`
							: "",
					].filter(Boolean),
				},
			),
	...
	if (Number.isFinite(walclChange) && Math.abs(walclChange) >= 1.5) {
		...
		signals.push(
			buildWorldSignal(
				liquidityType,
				"fred",
				walclChange > 0 ? "Fed balance sheet expansion" : "Fed balance sheet contraction",
				{
					sourceKey: "WALCL",
					region: "United States",
					macroRegion: "Americas",
	...
	if (Number.isFinite(m2Change) && Math.abs(m2Change) >= 1.5) {
		...
		signals.push(
			buildWorldSignal(
				liquidityType,
				"fred",
				m2Change > 0 ? "Money supply expansion" : "Money supply contraction",
				{
					sourceKey: "M2SL",
					region: "United States",
					macroRegion: "Americas",
	...
	if (
		(Number.isFinite(oilLatest) && oilLatest >= 80) ||
		(Number.isFinite(oilChange) && oilChange >= 8)
	) {
		signals.push(
			buildWorldSignal(
				"oil_macro_shock",
				"fred",
				`WTI oil pressure at ${Number(oilLatest || 0).toFixed(1)}`,
				{
					sourceKey: "DCOILWTICO",
					region: "Middle East",
					macroRegion: "EMEA",
					strength: normalizeSignalStrength(
						Math.max(Number(oilLatest || 0), Math.abs(Number(oilChange || 0)) * 4),
						75,
						110,
					),
					confidence: 0.8,
		...
		signals.push(
			buildWorldSignal(
				"energy_supply_shock",
				"fred",
				`WTI crude is reinforcing energy stress at ${Number(oilLatest || 0).toFixed(1)}`,
				{
					sourceKey: "DCOILWTICO",
					region: "Middle East",
					macroRegion: "EMEA",
		...
		if (oilDivergence >= 2.5) {
			signals.push(
				buildWorldSignal(
					...
					`Spot and FRED WTI diverged by ${oilDivergence.toFixed(1)}`,
					{
	...
	if (Number.isFinite(gdpChange) && gdpChange <= 0.2) {
		signals.push(
			buildWorldSignal(
				"sovereign_stress",
				"fred",
				"Growth is slowing into sovereign-risk conditions",
				{
					sourceKey: "GDP",
					region: "United States",
					macroRegion: "Americas",
	...
	for (const pred of predictions) {
		if (
			(pred.domain === "conflict" || pred.domain === "military") &&
			(pred.probability || 0) >= 0.55
		) {
			signals.push(
				buildWorldSignal("security_escalation", "forecast", pred.title, {
					sourceKey: pred.id,
					region: pred.region,
		...
		if (pred.domain === "cyber" && (pred.probability || 0) >= 0.45) {
			signals.push(
				buildWorldSignal("cyber_cost_repricing", "forecast", pred.title, {
					sourceKey: pred.id,
					region: pred.region,
					strength: pred.probability || 0,
					confidence: pred.confidence || 0,
		...
		if (pred.domain === "infrastructure" && (pred.probability || 0) >= 0.45) {
			signals.push(
				buildWorldSignal("infrastructure_capacity_loss", "forecast", pred.title, {
					sourceKey: pred.id,
					region: pred.region,
					strength: pred.probability || 0,
					confidence: pred.confidence || 0,
	...
	for (const signal of signals) {
		...
		if (existingIndex != null) {
			...
			existing.countries = uniqueSortedStrings([
				...(existing.countries || []),
				...(signal.countries || []),
			]);
			existing.actors = uniqueSortedStrings([...(existing.actors || []), ...(signal.actors || [])]);
			existing.domains = uniqueSortedStrings([
				...(existing.domains || []),
				...(signal.domains || []),
			]);
			existing.supportingEvidence = mergeSignalLists(
	...
	const criticalSignals = dedupedSignals
		...
		.sort(
			(a, b) =>
				b.strength + b.confidence - (a.strength + a.confidence) || a.label.localeCompare(b.label),
		);
	...
	return {
		...
		criticalExtraction: criticalSignalBundle
			? {
					...
					candidates: Array.isArray(criticalSignalBundle.candidates)
						? criticalSignalBundle.candidates.slice(0, 8)
						: [],
				}
			: null,
...
function inferSituationMarketBuckets(situation) {
	...
	const domains = uniqueSortedStrings(
		[situation?.dominantDomain, ...(situation?.domains || [])].filter(Boolean),
	);
	...
	if (domains.includes("conflict") || domains.includes("military")) {
		...
		buckets.add("defense");
		if (/middle east|red sea|black sea|eastern mediterranean|israel|gaza/i.test(region))
			buckets.add("energy");
		if (/red sea|black sea|eastern mediterranean|south china sea|western pacific/i.test(region))
			buckets.add("freight");
	}
	if (domains.includes("supply_chain")) {
		...
		if (
			/middle east|red sea|black sea/i.test(region) ||
			/oil|gas|grain|shipping|freight/i.test(label)
		)
			buckets.add("energy");
		if (/western pacific|south china sea|taiwan/i.test(region) || /semiconductor|chip/i.test(label))
			buckets.add("semis");
	}
...
function buildMarketTransmissionGraph(worldSignals, situationClusters = []) {
	...
	for (const situation of situationClusters) {
		...
		const regionSet = new Set(
			uniqueSortedStrings([situation.dominantRegion, ...(situation.regions || [])].filter(Boolean)),
		);
		...
		for (const bucketId of bucketIds) {
			...
			const bucketSignals = supportingSignals.filter((signal) =>
				bucketConfig?.signalTypes.includes(signal.type),
			);
			const baseStrength =
				Number(situation.avgProbability || 0) * 0.55 +
			...
			edges.push({
				...
				confidence: normalizeSignalStrength(
					(situation.avgConfidence || 0) * 0.6 + Math.min(0.35, bucketSignals.length * 0.08),
					0,
					1,
				),
...
function buildMarketState(worldSignals, transmissionGraph) {
	...
	const buckets = MARKET_BUCKET_CONFIG.map((config) => {
		...
		const macroSignals = weightedSignals.filter(
			(signal) =>
				signal.sourceType === "fred" ||
				signal.sourceType === "bis_policy" ||
				signal.sourceType === "bis_exchange",
		);
		const pressureNumerator =
			weightedSignals.reduce((sum, signal) => sum + signal.strength * signal.bucketWeight, 0) +
			bucketEdges.reduce((sum, edge) => sum + edge.strength * Number(config.edgeWeight || 1), 0);
		const confidenceNumerator =
			weightedSignals.reduce((sum, signal) => sum + signal.confidence * signal.bucketWeight, 0) +
			bucketEdges.reduce((sum, edge) => sum + edge.confidence * Number(config.edgeWeight || 1), 0);
		const divisor = Math.max(
			1,
			weightedSignals.reduce((sum, signal) => sum + signal.bucketWeight, 0) +
				bucketEdges.length * Number(config.edgeWeight || 1),
		);
		const macroConfirmation = macroSignals.length
			? clampUnitInterval(
					macroSignals.reduce((sum, signal) => sum + signal.strength, 0) / macroSignals.length,
				)
			: 0;
		const calibration = MARKET_BUCKET_STATE_CALIBRATION[config.id] || {};
		const defenseSignalConfirmation =
			config.id === "defense" && bucketSignals.length
				? clampUnitInterval(
						bucketSignals
		...
		const edgeDensity = bucketEdges.length
			? clampUnitInterval(
					bucketEdges.reduce((sum, edge) => sum + Number(edge.strength || 0), 0) /
						bucketEdges.length,
				)
			: 0;
		const calibratedPressure =
			pressureNumerator / divisor +
			macroConfirmation * Number(calibration.macroLift || 0) +
			edgeDensity * Number(calibration.edgeLift || 0) +
			defenseSignalConfirmation * 0.12 -
			(!defenseSignalConfirmation && config.id === "defense"
				? Number(calibration.dampener || 0)
				: 0);
		const calibratedConfidence =
			confidenceNumerator / divisor +
		...
		return {
			...
			topSignals: weightedSignals
				...
				.sort(
					(a, b) =>
						b.strength * b.bucketWeight +
							b.confidence -
							(a.strength * a.bucketWeight + a.confidence) || a.label.localeCompare(b.label),
				)
				.slice(0, 3)
				.map((signal) => ({
					id: signal.id,
			...
			topSituations: bucketEdges
				...
				.sort(
					(a, b) =>
						(b.strength + b.confidence) * Number(config.edgeWeight || 1) -
							(a.strength + a.confidence) * Number(config.edgeWeight || 1) ||
						a.sourceLabel.localeCompare(b.sourceLabel),
				)
				.slice(0, 3)
				.map((edge) => ({
		...
		(bucket) =>
			bucket.pressureScore > 0 || bucket.topSignals.length > 0 || bucket.topSituations.length > 0,
	);
...
function buildSituationMarketContextIndex(
	worldSignals,
	marketTransmission,
	marketState,
	sourceItems = [],
	...
	for (const source of sourceItems || []) {
		const sourceSituationIds = uniqueSortedStrings(
			[...(source?.sourceSituationIds || source?.situationIds || []), source?.id].filter(Boolean),
		);
		const situationEdges = edges.filter((edge) =>
			sourceSituationIds.includes(edge.sourceSituationId),
		);
		...
		for (const bucketId of uniqueSortedStrings(situationEdges.map((edge) => edge.targetBucketId))) {
			...
			const supportingSignalIds = uniqueSortedStrings(
				bucketEdges.flatMap((edge) => edge.supportingSignalIds || []),
			);
			const supportingSignals = supportingSignalIds
				.map((signalId) => signals.find((signal) => signal.id === signalId))
				.filter(Boolean);
			const topEdge = bucketEdges.slice().sort(compareTransmissionEdgePriority)[0] || null;
		...
		const linkedBuckets = uniqueSortedStrings(situationEdges.map((edge) => edge.targetBucketId))
			...
			.sort((left, right) => {
				...
				return (
					right.pressureScore +
						right.confidence +
						Number(rightContext.topTransmissionStrength || 0) -
						(left.pressureScore +
							left.confidence +
							Number(leftContext.topTransmissionStrength || 0)) ||
					left.label.localeCompare(right.label)
				);
			});
		const linkedSignalIds = uniqueSortedStrings(
			situationEdges.flatMap((edge) => edge.supportingSignalIds || []),
		);
		const linkedSignals = linkedSignalIds
			.map((signalId) => signals.find((signal) => signal.id === signalId))
			.filter(Boolean);
		const criticalSignals = linkedSignals.filter((signal) =>
			CRITICAL_NEWS_SOURCE_TYPES.has(signal.sourceType),
		);
		const avgEdgeStrength = situationEdges.length
			? situationEdges.reduce((sum, edge) => sum + Number(edge.strength || 0), 0) /
				situationEdges.length
			: 0;
		const avgEdgeConfidence = situationEdges.length
			? situationEdges.reduce((sum, edge) => sum + Number(edge.confidence || 0), 0) /
				situationEdges.length
			: 0;
		const avgBucketPressure = linkedBuckets.length
			? linkedBuckets.reduce((sum, bucket) => sum + Number(bucket.pressureScore || 0), 0) /
				linkedBuckets.length
			: 0;
		const alignedSignalStrength = linkedSignals.length
			? linkedSignals.reduce((sum, signal) => sum + Number(signal.strength || 0), 0) /
				linkedSignals.length
			: 0;
		const criticalSignalStrength = criticalSignals.length
			? criticalSignals.reduce(
					(sum, signal) =>
						sum + (Number(signal.strength || 0) * 0.62 + Number(signal.confidence || 0) * 0.38),
					0,
				) / criticalSignals.length
			: 0;
		const criticalSignalLift = clampUnitInterval(
			criticalSignalStrength * 0.78 + Math.min(0.18, criticalSignals.length * 0.05),
		);
		...
		const contradictionScore = clampUnitInterval(
			...
			["market", "supply_chain", "conflict", "political", "military"].includes(
				source.dominantDomain || "",
			)
				? 0.18
				: 0) +
		...
		const topBucket = linkedBuckets
			...
			.sort(
				(a, b) =>
					b.pressureScore + b.confidence - (a.pressureScore + a.confidence) ||
					a.label.localeCompare(b.label),
			)[0];
		const topEdge = situationEdges.slice().sort(compareTransmissionEdgePriority)[0];
		const topBucketCoverageScore = topBucket
			? computeMarketBucketCoverageScore(topBucket.id, marketInputCoverage)
			: 0;
		...
		contexts.set(source.id, {
			...
			topChannel:
				(topBucket ? bucketContexts[topBucket.id]?.topChannel : "") || topEdge?.channel || "",
			topTransmissionStrength: Number(topEdge?.strength || 0),
			topTransmissionConfidence: Number(topEdge?.confidence || 0),
...
function summarizeMarketInputCoverage(inputs = {}) {
	const coverage = {
		...
		stablecoins: Array.isArray(inputs?.stablecoinMarkets?.stablecoins)
			? inputs.stablecoinMarkets.stablecoins.length
			: 0,
		...
		militaryTheaters: Array.isArray(inputs?.militaryForecastInputs?.theaters)
			? inputs.militaryForecastInputs.theaters.length
			: 0,
	};
...
function flattenImpactExpansionHypotheses(bundle = null) {
	...
	const extractedCandidates = Array.isArray(bundle?.extractedCandidates)
...
function getImpactValidationFloors(order = "direct") {
	if (order === "third_order") {
		return { internal: 0.66, mapped: 0.7, multiplier: 0.72 };
	}
	if (order === "second_order") {
		return { internal: 0.5, mapped: 0.58, multiplier: 0.88 };
	}
...
function evaluateImpactHypothesisRejection(hypothesis, context = {}) {
	...
	if (!hypothesis.hypothesisKey && hypothesis.variableKey) {
		...
		if (!registry || !(registry.allowedChannels || []).includes(hypothesis.channel))
			return "unsupported_variable_channel";
		...
		if (!targetBucketAllowed || !bucketSignalTypes.includes(hypothesis.channel))
			return "weak_bucket_coherence";
	...
	if (
		...
		(candidateSalience < 0.58 || transmissionEdgeCount < 2)
	) {
		return "over_speculative_order";
	}

	const contradictionScore = clampUnitInterval(
		Number(candidate?.marketContext?.contradictionScore || 0),
	);
...
function validateImpactHypotheses(bundle = null) {
	...
	for (const [candidateIndex, items] of byCandidate.entries()) {
		...
		const ordered = items
			...
			.sort(
				(left, right) =>
					IMPACT_EXPANSION_ORDERS.indexOf(left.order) -
						IMPACT_EXPANSION_ORDERS.indexOf(right.order) ||
					(left.hypothesisKey || left.variableKey || "").localeCompare(
						right.hypothesisKey || right.variableKey || "",
					) ||
					left.targetBucket.localeCompare(right.targetBucket),
			);

		for (const hypothesis of ordered) {
			const lowerOrderKeys =
				hypothesis.order === "second_order"
					? validatedDirectKeys
					: hypothesis.order === "third_order"
			...
			const analogAdjustedSupport =
				hypothesis.analogTag && IMPACT_ANALOG_PRIORS[hypothesis.analogTag]
			...
			const contradictionPenalty = clampUnitInterval(
				Number(candidate.marketContext?.contradictionScore || 0),
			);
			// Free-form semantic scoring: reward geographic specificity, commodity precision, causal reasoning
			const geographyScore =
				!rejectionReason && hypothesis.geography && hypothesis.geography.trim().length >= 4 ? 1 : 0;
			const commodityScore =
				!rejectionReason && hypothesis.commodity && hypothesis.commodity.trim().length >= 2 ? 1 : 0;
			const causalLinkScore = rejectionReason
				? 0
				: hypothesis.order === "direct"
					? 1
					: hypothesis.causalLink && hypothesis.causalLink.trim().length >= 10
						? 1
						: 0;
			const assetScore =
				!rejectionReason &&
				(hypothesis.affectedAssets || hypothesis.assetsOrSectors || []).length > 0
					? 1
					: 0;
			// Legacy coherence terms for old cached responses without hypothesisKey
			const channelCoherence =
				!rejectionReason && !hypothesis.hypothesisKey && hypothesis.variableKey ? 1 : 0;
			const bucketCoherence =
				!rejectionReason && !hypothesis.hypothesisKey && hypothesis.variableKey ? 1 : 0;
			...
			else if (!rejectionReason && validationScore >= floors.internal)
				validationStatus = "trace_only";
			...
			results.push({
				...
				targetBucketLabel:
					MARKET_BUCKET_CONFIG.find((bucket) => bucket.id === hypothesis.targetBucket)?.label ||
	...
	const mappedDirectKeySet = new Set(
		results
			.filter((r) => r.order === "direct" && r.validationStatus === "mapped")
			.map((r) => r.hypothesisKey || r.variableKey)
			.filter(Boolean),
	);
	for (const item of results) {
		if (
			item.order === "second_order" &&
			item.validationStatus === "mapped" &&
			item.dependsOnKey &&
			!mappedDirectKeySet.has(item.dependsOnKey)
		) {
	...
	const mappedSecondKeySet = new Set(
		results
			.filter((r) => r.order === "second_order" && r.validationStatus === "mapped")
			.map((r) => r.hypothesisKey || r.variableKey)
			.filter(Boolean),
	);
	for (const item of results) {
		if (
			item.order === "third_order" &&
			item.validationStatus === "mapped" &&
			item.dependsOnKey &&
			!mappedSecondKeySet.has(item.dependsOnKey)
		) {
			item.validationStatus = "trace_only";
		}
	}
	...
	const validated = results.filter(
		(item) => item.validationStatus === "mapped" || item.validationStatus === "trace_only",
	);
	return {
		hypotheses: results,
		validated,
		...
		rejectionReasonCounts: summarizeTypeCounts(
			results.filter((item) => item.rejectionReason).map((item) => item.rejectionReason),
		),
...
function mapImpactHypothesesToWorldSignals(validation = null) {
	...
	for (const hypothesis of validation?.mapped || []) {
		...
		const key = [hypothesis.candidateStateId, hypothesis.order, effectiveHypKey].join(":");
		if (seen.has(key)) continue;
		seen.add(key);
		const registry = IMPACT_VARIABLE_REGISTRY[hypothesis.variableKey];
		const candidate = hypothesis.candidate || {};
		const evidenceTextByKey = new Map(
			(candidate.evidenceTable || []).map((entry) => [entry.key, entry.text]),
		);
		const signalLabel =
			hypothesis.description ||
			hypothesis.summary ||
			`${candidate.candidateStateLabel || "State"} -> ${hypothesis.geography || hypothesis.marketImpact || "market"}`;
		const rawChannel = hypothesis.channel || hypothesis.marketImpact || "";
		const signalChannel = IMPACT_SIGNAL_CHANNELS.has(rawChannel)
			? rawChannel
			: resolveImpactChannel(rawChannel);
		mappedSignals.push(
			buildWorldSignal(signalChannel, IMPACT_EXPANSION_SOURCE_TYPE, signalLabel, {
				...
				confidence: clampUnitInterval(
					Number(hypothesis.confidence || 0) * 0.64 +
						Number(hypothesis.validationScore || 0) * 0.36,
				),
				supportingEvidence: (hypothesis.evidenceRefs || [])
					.map((ref) => evidenceTextByKey.get(ref))
					.filter(Boolean)
					.slice(0, 3),
...
function buildWorldSignalLayer(observedWorldSignals, extraSignals = []) {
	const baseSignals = Array.isArray(observedWorldSignals?.signals)
		? observedWorldSignals.signals
		: [];
...
function buildImpactExpansionSimulationLayers({
	...
	const layer1 =
		mappedDirect.length > 0 ? buildWorldSignalLayer(observedWorldSignals, mappedDirect) : layer0;
	...
	const contextObserved = buildSituationMarketContextIndex(
		layer0,
	...
	const transmissionRound1 =
		layer1 === layer0
			? transmissionObserved
			: buildMarketTransmissionGraph(layer1, situationClusters);
	const stateRound1 =
		layer1 === layer0 ? stateObserved : buildMarketState(layer1, transmissionRound1);
	const contextRound1 =
		layer1 === layer0
			? contextObserved
			: buildSituationMarketContextIndex(
	...
	const transmissionRound2 =
		layer2 === layer1
			? transmissionRound1
			: buildMarketTransmissionGraph(layer2, situationClusters);
	const stateRound2 =
		layer2 === layer1 ? stateRound1 : buildMarketState(layer2, transmissionRound2);
	const contextRound2 =
		layer2 === layer1
			? contextRound1
			: buildSituationMarketContextIndex(
	...
	const transmissionRound3 =
		layer3 === layer2
			? transmissionRound2
			: buildMarketTransmissionGraph(layer3, situationClusters);
	const stateRound3 =
		layer3 === layer2 ? stateRound2 : buildMarketState(layer3, transmissionRound3);
	const contextRound3 =
		layer3 === layer2
			? contextRound2
			: buildSituationMarketContextIndex(
...
function materializeImpactExpansion({
	...
	const filteredBundle = bundle
		? {
				...bundle,
				candidatePackets: (Array.isArray(bundle?.candidatePackets)
					? bundle.candidatePackets
					: []
	...
	if (filteredBundle) {
		const allowedIndexes = new Set(
			filteredBundle.candidatePackets.map((packet) => packet.candidateIndex),
		);
		filteredBundle.candidates = (Array.isArray(bundle?.candidates) ? bundle.candidates : []).filter(
			(packet) => allowedIndexes.has(packet.candidateIndex),
		);
		filteredBundle.extractedCandidates = (
			Array.isArray(bundle?.extractedCandidates) ? bundle.extractedCandidates : []
	...
	return {
		...
		topHypotheses,
		candidatePreview: Array.isArray(filteredBundle?.candidates)
			? filteredBundle.candidates.slice(0, 6)
			: [],
		candidatePackets: Array.isArray(filteredBundle?.candidatePackets)
			? filteredBundle.candidatePackets
			: [],
...
function buildForecastRunWorldState(data) {
	...
	const stateUnits =
		data?.stateUnits || buildCanonicalStateUnits(situationClusters, situationFamilies);
	...
	const impactExpansionBundle =
		data?.impactExpansionBundle || inputs?.impactExpansionBundle || null;
	...
	const activeDomains = domainStates
		.filter((item) => item.forecastCount > 0)
...
function summarizeWorldStateSurface(worldState) {
	...
	return {
		...
		forecastCount: Array.isArray(worldState.branchStates)
			? new Set(worldState.branchStates.map((branch) => branch.forecastId)).size
			: 0,
		...
		observedWorldSignalCount:
			worldState.impactExpansion?.observedWorldSignalCount ||
			worldState.worldSignals?.signals?.length ||
			0,
		expandedWorldSignalCount:
			worldState.impactExpansion?.expandedWorldSignalCount ||
			worldState.worldSignals?.signals?.length ||
			0,
		...
		impactExpansionValidatedHypothesisCount:
			worldState.impactExpansion?.validatedHypothesisCount || 0,
		...
		expandedTransmissionEdgeCount:
			worldState.impactExpansion?.expandedTransmissionEdgeCount ||
			worldState.marketTransmission?.edges?.length ||
			0,
		marketConsequenceCount: worldState.simulationState?.marketConsequences?.items?.length || 0,
		blockedMarketConsequenceCount:
			worldState.simulationState?.marketConsequences?.blockedCount || 0,
		...
		reportableInteractionCount:
			worldState.simulationState?.reportableInteractionLedger?.length || 0,
		...
		simulationEnvironmentCount:
			worldState.simulationState?.environmentSpec?.situations?.length || 0,
...
function buildImpactPathScore(candidatePacket, direct, second, third) {
	return +clampUnitInterval(
		Number(direct?.validationScore || 0) * 0.45 +
			...
			Number(candidatePacket?.rankingScore || 0) * 0.1 +
...
function buildImpactPathsForCandidate(candidatePacket, validation = null) {
	...
	const candidateMapped = (validation?.mapped || []).filter(
		(item) => item.candidateIndex === candidatePacket.candidateIndex,
	);
	...
	for (const second of secondItems) {
		...
		const direct = directItems.find(
			(item) => (item.hypothesisKey || item.variableKey) === second.dependsOnKey,
		);
		if (!direct) continue;
		...
		for (const third of thirdMatches) {
			...
			if (seen.has(key) || pathScore < 0.5) continue;
			expanded.push({
	...
	for (const path of expanded.sort(
		(a, b) => b.pathScore - a.pathScore || a.pathId.localeCompare(b.pathId),
	)) {
		const pathDirectKey = path.direct
			? path.direct.hypothesisKey || path.direct.variableKey || ""
			: "";
...
function buildImpactExpansionBundleFromPaths(paths = [], candidatePackets = [], meta = {}) {
	...
	for (const path of paths || []) {
		...
		if (path.second)
			entry.secondOrderHypotheses.push({
		...
		if (path.third)
			entry.thirdOrderHypotheses.push({
	...
	const extractedCandidates = [...byCandidate.values()].sort(
		(a, b) => a.candidateIndex - b.candidateIndex,
	);
	return {
		...
		extractedHypothesisCount: extractedCandidates.reduce(
			(sum, item) =>
				sum +
				item.directHypotheses.length +
				item.secondOrderHypotheses.length +
				item.thirdOrderHypotheses.length,
			0,
		),
		partialFailureCount: 0,
...
function computeDeepReportableQualityScore(pathWorldState, candidateStateId) {
	...
	const reportableInteractionLedger =
		pathWorldState?.simulationState?.reportableInteractionLedger || [];
	const blockedInteractions = Array.isArray(reportableInteractionLedger?.blocked)
		? reportableInteractionLedger.blocked
		: [];
	...
	const pathReportable = filterCandidateTouchingItems(
		reportableInteractionLedger,
		candidateStateId,
	);
	...
	const avgReportableConfidence = pathReportable.length
		? clampUnitInterval(
				pathReportable.reduce((sum, item) => sum + Number(item.confidence || 0), 0) /
					pathReportable.length,
			)
		: 0;
	const pathEffectSupport = clampUnitInterval(
		pathEffects.length / Math.max(pathReportable.length, 1),
	);
	const pathBlockedRate = clampUnitInterval(
		pathBlocked.length / Math.max(pathReportable.length + pathBlocked.length, 1),
	);
	return +clampUnitInterval(
		pathSelectivityScore * 0.45 +
			avgReportableConfidence * 0.35 +
			pathEffectSupport * 0.2 -
			pathBlockedRate * 0.2,
	).toFixed(3);
}
...
function computeDeepMarketCoherenceScore(pathWorldState, candidatePacket, path) {
	...
	const mappedHypothesisAvg = mappedHypotheses.length
		? clampUnitInterval(
				mappedHypotheses.reduce((sum, item) => sum + Number(item.validationScore || 0), 0) /
					mappedHypotheses.length,
			)
		: 0;
	const marketConsequences = pathWorldState?.simulationState?.marketConsequences?.items || [];
	const blockedMarketConsequences =
		pathWorldState?.simulationState?.marketConsequences?.blocked || [];
	const admissibleConsequenceCount = marketConsequences.filter(
		(item) => item.situationId === candidatePacket.candidateStateId,
	).length;
	const blockedAdmissibilityCount = blockedMarketConsequences.filter(
		(item) =>
	...
	const admissibleRate = clampUnitInterval(
		admissibleConsequenceCount /
			Math.max(admissibleConsequenceCount + blockedAdmissibilityCount, 1),
	);
	...
	const roleSeparationScore =
		!hasFreightRole || !hasMarketRole || hypothesisChannels.size > 1 ? 1 : 0;
	return +clampUnitInterval(
		mappedHypothesisAvg * 0.4 +
			admissibleRate * 0.35 +
			specificityBonus * 0.15 +
			roleSeparationScore * 0.1,
	).toFixed(3);
}

function computeDeepPathAcceptanceScore(candidatePacket, path, pathWorldState) {
	const contradictionPenalty = clampUnitInterval(
		Number(candidatePacket?.marketContext?.contradictionScore || 0),
	);
	const reportableQualityScore = computeDeepReportableQualityScore(
		pathWorldState,
		candidatePacket.candidateStateId,
	);
	const marketCoherenceScore = computeDeepMarketCoherenceScore(
		pathWorldState,
		candidatePacket,
		path,
	);
...
function annotateDeepForecastOrigins(worldState, acceptedPaths = []) {
	...
	const tagItems = (items = []) => {
		for (const item of items) {
			...
			item.originStateId = sourcePathId
				? item.sourceSituationId
				: targetPathId
					? item.targetSituationId
					: "";
		}
	};
...
const SIMULATION_MERGE_ACCEPT_THRESHOLD = 0.5;
const SIMULATION_ELIGIBILITY_RANK_THRESHOLD = 0.4;
...
function contradictsPremise(invalidator, expandedPath, fromSimulation = false) {
	...
	const subjectKeywords = [
		...stateKind.toLowerCase().split("_"),
		...bucket.toLowerCase().split("_"),
	].filter((w) => w.length >= 4);
...
function computeSimulationAdjustment(expandedPath, simTheaterResult, candidatePacket) {
	let adjustment = 0;
	const details = {
		bucketChannelMatch: false,
		actorOverlapCount: 0,
		roleOverlapCount: 0,
		keyActorsOverlapCount: 0,
	...
	if (bucketChannelMatch) {
		...
		if (actorSrc === "stateSummary") {
			const simRoles = new Set(
				(Array.isArray(bucketChannelMatch.keyActorRoles) ? bucketChannelMatch.keyActorRoles : [])
		...
		const simEntities = new Set(
			(Array.isArray(bucketChannelMatch.keyActors) ? bucketChannelMatch.keyActors : [])
				.map(normalizeActorName)
				.filter(Boolean),
		);
...
function applySimulationMerge(
	...
	for (const path of allPaths) {
		...
		const candidatePacket = (candidatePackets || []).find(
			(c) => c.candidateStateId === path.candidateStateId,
		);
		...
		if (wasAccepted && mergedAcceptanceScore < SIMULATION_MERGE_ACCEPT_THRESHOLD) {
			...
			!wasAccepted &&
			mergedAcceptanceScore >= SIMULATION_MERGE_ACCEPT_THRESHOLD &&
	...
	if (anyPathChanged) {
		...
		if (newSelectedExpanded.length > 0 && evaluation.status === "completed_no_material_change") {
			...
			const acceptedBundle = buildImpactExpansionBundleFromPaths(
				newSelectedExpanded,
				candidatePackets,
				{
...
function validateDeepForecastSnapshot(snapshot = {}) {
	const fullRunStateUnits = Array.isArray(snapshot?.fullRunStateUnits)
		? snapshot.fullRunStateUnits
		: [];
...
function buildDeepWorldStateFromSnapshot(
	snapshot,
	priorWorldState,
...
async function evaluateDeepForecastPaths(snapshot, priorWorldState, candidatePackets, bundle) {
	...
	for (const candidatePacket of candidatePackets || []) {
		...
		for (const path of expandedPaths) {
			...
			const pathWorldState = buildDeepWorldStateFromSnapshot(
				snapshot,
				priorWorldState,
				pathBundle,
				{
		...
		evaluated.sort(
			(a, b) =>
				b.acceptanceScore - a.acceptanceScore ||
				b.pathScore - a.pathScore ||
				a.pathId.localeCompare(b.pathId),
		);
		const accepted = evaluated.find((item) => item.acceptanceScore >= 0.5) || null;
		if (accepted) {
...
function summarizeForecastPopulation(predictions) {
	...
	const legacyDetectorDomainCounts = Object.fromEntries(
		FORECAST_DOMAINS.map((domain) => [domain, 0]),
	);
	const stateDerivedDomainCounts = Object.fromEntries(
		FORECAST_DOMAINS.map((domain) => [domain, 0]),
	);
	...
	return {
		...
		generationOriginCounts: summarizeTypeCounts(
			Object.entries(generationOriginCounts).flatMap(([origin, count]) =>
				Array(count).fill(origin),
			),
		),
...
function summarizeForecastTraceQuality(
	predictions,
	tracedPredictions,
	enrichmentMeta = null,
	...
	const promotionSignalCounts = summarizeTypeCounts(
		tracedPredictions.flatMap((item) =>
			(item.signals || []).slice(0, 3).map((signal) => signal.type),
		),
	);

	const suppressionSignalCounts = summarizeTypeCounts(
		tracedPredictions.flatMap((item) =>
			(item.caseFile?.counterEvidence || []).map((counter) => counter.type),
		),
	);
	...
	const avgProbability = tracedPredictions.length
		? +(
				tracedPredictions.reduce((sum, item) => sum + (item.probability || 0), 0) /
				tracedPredictions.length
			).toFixed(3)
		: 0;
	const avgConfidence = tracedPredictions.length
		? +(
				tracedPredictions.reduce((sum, item) => sum + (item.confidence || 0), 0) /
				tracedPredictions.length
			).toFixed(3)
		: 0;
	...
	const llmCombinedCount =
		(narrativeSourceCounts.llm_combined || 0) + (narrativeSourceCounts.llm_combined_cache || 0);
	const llmScenarioCount =
		(narrativeSourceCounts.llm_scenario || 0) + (narrativeSourceCounts.llm_scenario_cache || 0);
	...
	return {
		...
		traced: {
			...
			fallbackCount,
			fallbackRate: tracedPredictions.length
				? +(fallbackCount / tracedPredictions.length).toFixed(3)
				: 0,
			enrichedCount,
			enrichedRate: tracedPredictions.length
				? +(enrichedCount / tracedPredictions.length).toFixed(3)
				: 0,
			llmCombinedCount,
			llmScenarioCount,
			avgReadiness,
		...
		candidateRun:
			Array.isArray(candidatePredictions) && candidatePredictions.length > predictions.length
...
function buildForecastTraceArtifacts(data, context = {}, config = {}) {
	const generatedAt = data?.generatedAt || Date.now();
	const predictions = Array.isArray(data?.predictions) ? data.predictions : [];
	const fullRunPredictions = Array.isArray(data?.fullRunPredictions)
		? data.fullRunPredictions
		: predictions;
	const maxForecasts = config.maxForecasts || getTraceMaxForecasts(predictions.length);
	const worldState =
		data?.worldStateOverride ||
	...
	const tracedPredictions = predictions
		.slice(0, maxForecasts)
		.map((pred, index) => buildForecastTraceRecord(pred, index + 1, simulationByForecastId));
	...
	const candidateWorldState =
		data?.candidateWorldStateOverride ||
		(fullRunPredictions !== predictions || data?.fullRunSituationClusters
			? buildForecastRunWorldState({
					generatedAt,
	...
	const summary = {
		...
		worldStateSummary: {
			...
			criticalSignalCandidateCount:
				worldState.worldSignals?.criticalExtraction?.candidateCount || 0,
			criticalSignalFrameCount:
				worldState.worldSignals?.criticalExtraction?.extractedFrameCount || 0,
			criticalSignalFallbackCount:
				worldState.worldSignals?.criticalExtraction?.fallbackNewsSignalCount || 0,
			...
			simulationEffectCount: worldState.report?.crossSituationEffects?.length || 0,
			blockedEffectCount: worldState.simulationState?.blockedEffects?.length || 0,
			blockedMarketConsequenceReasons:
				worldState.simulationState?.marketConsequences?.blockedSummary?.byReason || {},
			simulationEnvironmentSummary: worldState.simulationState?.environmentSpec?.summary || "",
			simulationEnvironmentCount:
				worldState.simulationState?.environmentSpec?.situations?.length || 0,
			memoryMutationSummary: worldState.simulationState?.memoryMutations?.summary || "",
	...
	const impactExpansionDebug = buildImpactExpansionDebugPayload(data, worldState, manifest.runId);
...
async function readForecastWorldStateHistory(storageConfig, limit = WORLD_STATE_HISTORY_LIMIT) {
	try {
		...
		const states = await Promise.all(
			keys.map((key) => getR2JsonObject(storageConfig, key).catch(() => null)),
		);
...
async function writeForecastTraceArtifacts(data, context = {}) {
	...
	console.log(
		`  [Trace] Storage mode=${storageConfig.mode} bucket=${storageConfig.bucket} prefix=${storageConfig.basePrefix}`,
	);
	console.log(
		`  Trace cap: raw=${traceCap.raw ?? "default"} resolved=${traceCap.resolved} total=${traceCap.totalForecasts}`,
	);
	...
	const [priorWorldStates, priorWorldStateFallback] =
		data?.priorWorldStates?.length || data?.priorWorldState
	...
	if (artifacts.impactExpansionDebug) {
		await putR2JsonObject(
			storageConfig,
			artifacts.impactExpansionDebugKey,
			artifacts.impactExpansionDebug,
			{
				runid: String(artifacts.manifest.runId || ""),
	...
	await Promise.all(
		artifacts.forecasts.map((item, index) =>
			putR2JsonObject(storageConfig, item.key, item.payload, {
				runid: String(artifacts.manifest.runId || ""),
				kind: "forecast",
				rank: String(index + 1),
			}),
...
function mapActorCategoryToEntityClass(category, domains = []) {
	...
	if (category === "commercial")
		return domains.includes("supply_chain") ? "logistics_operator" : "exporter_or_importer";
	return "state_actor";
}
...
function inferEntityClassFromName(name) {
	...
	if (
		/\b(military|army|navy|air\s+force|national\s+guard|houthi|irgc|revolutionary\s+guard|armed\s+forces?)\b/.test(
			s,
		)
	)
		return "military_or_security_actor";
	if (/central bank|fed |ecb |boe |opec|regulator|reserve bank/.test(s))
		return "regulator_or_central_bank";
	if (/shipping|tanker|port|logistics|freight|carrier|maersk|cosco/.test(s))
		return "logistics_operator";
	if (/exporter|importer|producer|supplier|aramco|national oil/.test(s))
		return "exporter_or_importer";
	if (/media|press|public bloc|civil society/.test(s)) return "media_or_public_bloc";
	if (/trader|hedge fund|market participant|investor|commodity/.test(s))
...
function buildSimulationRequirementText(theater, candidate) {
	...
	const critTypes = (candidate.criticalSignalTypes || [])
		.slice(0, 3)
		.map((t) => sanitizeForPrompt(t).replace(/_/g, " "))
		.join(", ");
	...
	for (const theater of selectedTheaters) {
		...
		for (const entry of candidate.evidenceTable || []) {
			if (entry.kind === "headline") {
				...
				entry.kind === "signal" &&
				/disruption|blockage|attack|strike|closure|incident/i.test(entry.text)
			) {
				seeds.push({
					seedId: `seed-${++idx}`,
					theaterId: theater.theaterId,
					...
					strength: +Math.min(
						0.9,
						Number(candidate.marketContext?.criticalSignalLift || 0) + 0.3,
					).toFixed(3),
				});
			}
		}
...
function buildSimulationPackageConstraints(selectedTheaters, candidates) {
	...
	for (const theater of selectedTheaters) {
		...
		if (
			MACRO_FIN_BUCKETS.some((b) => bucketArr.includes(b) || topBucket === b) &&
			!theater.routeFacilityKey
		) {
			theaterConstraints.push({
				constraintId: `c-${++idx}`,
				theaterId: theater.theaterId,
		...
		if (
			!theater.routeFacilityKey &&
			!theater.commodityKey &&
			theater.stateKind !== "market_repricing"
		) {
			theaterConstraints.push({
				constraintId: `c-${++idx}`,
				theaterId: theater.theaterId,
...
function buildEvalTargetQuestions(theater, macroRegion) {
	...
	const commodity = theater.commodityKey
		? ` and ${theater.commodityKey.replace(/_/g, " ")} flows`
		: "";
	...
	if (stateKind === "market_repricing") {
		return [
			{
				pathType: "escalation",
				question: `How does ${label} escalate through ${bucket} market conditions, and which institutional actors accelerate the repricing?`,
			},
			{
				pathType: "containment",
				question: `What policy interventions or market signals contain the ${bucket} repricing before second-order spillovers materialize?`,
			},
			{
				pathType: "market_cascade",
				question: `Model 2nd and 3rd order economic consequences: ${bucket.replace(/_/g, " ")} direction, FX stress on import-dependent economies in ${macroRegion}, sovereign spread widening, and downstream sector demand compression.`,
			},
		];
	}
	if (stateKind === "political_instability" || stateKind === "governance_pressure") {
		return [
			{
				pathType: "escalation",
				question: `How does ${label} in ${macroRegion} escalate through government dysfunction, trade posture shifts, and investor confidence into ${bucket} pressure?`,
			},
			{
				pathType: "containment",
				question: `What institutional stabilizers (coalition formation, international mediation, credible commitment signals) contain the political instability before market spillover?`,
			},
			{
				pathType: "market_cascade",
				question: `Model 2nd and 3rd order economic consequences: capital flight from ${macroRegion}, FX stress, sovereign risk premium widening, and downstream trade disruption.`,
			},
		];
	}
	if (stateKind === "security_escalation") {
		return [
			{
				pathType: "escalation",
				question: `How does ${label} in ${macroRegion} escalate through military posture changes, logistics disruption, and regional alliance dynamics into ${bucket} pressure?`,
			},
			{
				pathType: "containment",
				question: `What deterrence signals, diplomatic channels, or de-escalation steps contain ${label} before it triggers broader market repricing?`,
			},
			{
				pathType: "market_cascade",
				question: `Model 2nd and 3rd order economic consequences: regional risk premium, defense spending signals, logistics cost increases in ${macroRegion}, and ${bucket} market sentiment.`,
			},
		];
	}
	if (stateKind === "infrastructure_fragility") {
		return [
			{
				pathType: "escalation",
				question: `How does ${label}${commodity} at ${route} escalate through supply chain capacity loss, logistics rerouting, and production continuity gaps into ${bucket} pressure?`,
			},
			{
				pathType: "containment",
				question: `What restoration timelines or alternative routing contain the infrastructure disruption before ${bucket} markets reprice?`,
			},
			{
				pathType: "market_cascade",
				question: `Model 2nd and 3rd order economic consequences: production shortfall in ${macroRegion}, logistics cost increases, downstream manufacturing disruptions, and ${bucket} supply gap.`,
			},
		];
	}
	if (stateKind === "cyber_pressure") {
		return [
			{
				pathType: "escalation",
				question: `How does ${label} escalate through systems availability failures, financial network disruption, and institutional confidence loss into ${bucket} pressure?`,
			},
			{
				pathType: "containment",
				question: `What technical containment, incident response timelines, or redundancy activation limits the ${label} impact on ${bucket} conditions?`,
			},
			{
				pathType: "market_cascade",
				question: `Model 2nd and 3rd order economic consequences: financial settlement disruption in ${macroRegion}, confidence shock on ${bucket} participants, and downstream sector operational losses.`,
			},
		];
	}
	// fallback
	return [
		{
			pathType: "escalation",
			question: `How does ${label} in ${macroRegion} escalate through actor behavior and institutional response into broader ${bucket} pressure, and who accelerates it?`,
		},
		{
			pathType: "containment",
			question: `What conditions or interventions contain ${label} before it crosses into sustained ${bucket} repricing?`,
		},
		{
			pathType: "market_cascade",
			question: `Model 2nd and 3rd order economic consequences of ${label}: ${bucket.replace(/_/g, " ")} conditions, FX stress in ${macroRegion}, downstream sector impacts, and actor-driven amplification.`,
		},
	];
}
...
function buildSimulationPackageEvaluationTargets(selectedTheaters, candidates) {
	...
	for (const theater of selectedTheaters) {
		...
		if (!candidate) {
			console.warn(
				`[SimulationPackage] No candidate for theaterId=${theater.theaterId} (evaluationTargets)`,
			);
		}
		const bucket = theater.topBucketId || "market";
		const macroRegion = theater.macroRegions?.[0] || theater.dominantRegion;
		const actors = (candidate?.stateSummary?.actors || []).slice(0, 3).join(", ") || "key actors";
		const isMaritimeOrInfra =
			theater.routeFacilityKey &&
			(theater.stateKind === "maritime_disruption" ||
				theater.stateKind === "infrastructure_fragility");
		result[theater.theaterId] = {
			...
			timingMarkers: [
				{
					label: "T+24h",
					description: `Initial state and ${isMaritimeOrInfra ? "logistics " : ""}actor response to ${theater.label}`,
				},
				{
					label: "T+48h",
					description: `${bucket} market repricing and policy signals emerging from ${macroRegion}`,
				},
...
function buildSimulationStructuralWorld(
	selectedTheaters,
	{
		stateUnits,
		worldSignals,
	...
	const theaterRegions = new Set(
		selectedTheaters.flatMap((t) => [t.dominantRegion, ...(t.macroRegions || [])]).filter(Boolean),
	);
	...
	const touchingSignals = (worldSignals?.signals || [])
		.filter(
			(s) =>
				theaterRegions.has(s.region) ||
				theaterRegions.has(s.macroRegion) ||
				theaterStateIds.has(s.situationId),
		)
		.slice(0, 20);
	const touchingTransmissionEdges = (marketTransmission?.edges || [])
		.filter(
			(e) => theaterStateIds.has(e.sourceSituationId) || theaterStateIds.has(e.targetSituationId),
		)
		.slice(0, 15);
	const touchingMarketBuckets = (marketState?.buckets || [])
		.filter((b) => theaterBucketIds.has(b.id))
		.slice(0, 5);
	const relevantClusters = (situationClusters || [])
		.filter(
			(c) => (c.regions || []).some((r) => theaterRegions.has(r)) || theaterStateIds.has(c.id),
		)
		.slice(0, 5);
	...
	const simulationRequirement = Object.fromEntries(
		selectedTheaters.map((theater) => [
			...
			buildSimulationRequirementText(
				theater,
				top.find((c) => c.candidateStateId === theater.candidateStateId),
			),
		]),
	);
...
async function enqueueDeepForecastTask(task) {
	...
	await redisCommand(url, token, [
		...
		String(Number(task.createdAt || Date.now())),
		task.runId,
	]);
	await redisCommand(url, token, [
		"EXPIRE",
...
function buildChangeItems(pred, prev) {
	...
	if (!prev) {
		items.push(
			`New forecast surfaced in this run at ${roundPct(pred.probability)} over the ${pred.timeHorizon}.`,
		);
		...
		if (pred.calibration?.marketTitle) {
			items.push(
				`Initial market check: ${pred.calibration.marketTitle} at ${roundPct(pred.calibration.marketPrice)}.`,
			);
		}
		return items.slice(0, 4);
	}
	...
	const newHeadlines = (pred.newsContext || []).filter(
		(headline) => !previousHeadlines.has(headline),
	);
	...
	if (pred.calibration) {
		...
		if (!prevMarket || prevMarket.marketTitle !== pred.calibration.marketTitle) {
			items.push(
				`New market anchor: ${pred.calibration.marketTitle} at ${roundPct(pred.calibration.marketPrice)}.`,
			);
		} else if (
			Math.abs((pred.calibration.marketPrice || 0) - (prevMarket.marketPrice || 0)) >= 0.05
		) {
			items.push(
				`Market moved from ${roundPct(prevMarket.marketPrice)} to ${roundPct(pred.calibration.marketPrice)} in ${pred.calibration.marketTitle}.`,
			);
		}
	}
	...
	if (items.length === 0) {
		if (
			Math.abs(pred.probability - (prev.probability || pred.priorProbability || pred.probability)) <
			0.05
		) {
...
function scoreForecastReadiness(pred) {
	...
	const sourceCount = new Set((pred.signals || []).map((s) => SIGNAL_TO_SOURCE[s.type] || s.type))
		.size;

	const evidenceScore = clamp01(
		normalize(supportCount, 0, 6) * 0.55 + normalize(sourceCount, 1, 4) * 0.45,
	);
	...
	const alternativeScore = clamp01(
		(pred.caseFile?.baseCase || supportCount > 0 ? 1 : 0) * (1 / 3) +
			(pred.caseFile?.escalatoryCase || triggerCount > 0 || (pred.cascades?.length || 0) > 0
				? 1
				: 0) *
				(1 / 3) +
			(pred.caseFile?.contrarianCase || counterCount > 0 || pred.trend === "falling" ? 1 : 0) *
				(1 / 3),
	);
	const actorScore = actorCount > 0 ? Math.min(1, actorCount / 3) : 0;
	const driftPenalty = pred.calibration
		? Math.min(0.18, Math.abs(pred.calibration.drift || 0) * 0.6)
		: 0;
...
function computeAnalysisPriority(pred) {
	...
	const counterEvidenceTypes = new Set(
		(pred.caseFile?.counterEvidence || []).map((item) => item.type),
	);
	const hasNewsCorroboration = (pred.signals || []).some(
		(signal) => signal.type === "news_corroboration",
	);
	...
	const priorityDomainBonus =
		ENRICHMENT_PRIORITY_DOMAINS.includes(pred.domain) && readiness.overall >= 0.45 ? 0.012 : 0;
	...
	const cyberThinSignalPenalty =
		pred.domain === "cyber" && counterEvidenceTypes.has("coverage_gap") ? 0.01 : 0;
	return +(
...
function prepareForecastMetrics(predictions) {
	for (const pred of predictions) {
		...
		pred.analysisPriority =
			typeof pred.analysisPriority === "number"
				? pred.analysisPriority
...
function getForecastSituationTokens(pred) {
	return uniqueSortedStrings([
		...
		...(pred.caseFile?.supportingEvidence || []).flatMap((item) =>
			extractMeaningfulTokens(item.summary, [pred.region]),
		),
	]).slice(0, 12);
}
...
function computeSituationDuplicateScore(current, kept) {
	const currentActors = uniqueSortedStrings(
		(current.caseFile?.actors || []).map((actor) => actor.name || actor.id),
	);
	const keptActors = uniqueSortedStrings(
		(kept.caseFile?.actors || []).map((actor) => actor.name || actor.id),
	);
	const currentBranches = uniqueSortedStrings(
		(current.caseFile?.branches || []).map((branch) => branch.kind),
	);
	const keptBranches = uniqueSortedStrings(
		(kept.caseFile?.branches || []).map((branch) => branch.kind),
	);
	const currentState = getForecastSelectionStateContext(current);
	const keptState = getForecastSelectionStateContext(kept);
	const currentSignals = uniqueSortedStrings(
		(currentState?.topSignals || []).map((signal) => signal.type),
	);
	const keptSignals = uniqueSortedStrings(
		(keptState?.topSignals || []).map((signal) => signal.type),
	);
	...
	if ((currentState?.familyId || "") && currentState?.familyId === keptState?.familyId)
		score += 0.45;
	if (
		(currentState?.dominantRegion || current.region || "") ===
		(keptState?.dominantRegion || kept.region || "")
	)
		score += 1.5;
...
function summarizePublishFiltering(
	predictions,
	selectedPredictions = [],
	...
	const reasonCounts = summarizeTypeCounts(
		predictions.map((pred) => pred.publishDiagnostics?.reason).filter(Boolean),
	);
	const situationCounts = summarizeTypeCounts(
		predictions.map((pred) => pred.situationContext?.id).filter(Boolean),
	);
	const familyCounts = summarizeTypeCounts(
		predictions.map((pred) => pred.familyContext?.id).filter(Boolean),
	);
	const cappedSituationIds = new Set(
		predictions
			.filter(
				(pred) =>
					pred.publishDiagnostics?.reason === "situation_cap" &&
					pred.publishDiagnostics?.situationId,
			)
	...
	const cappedFamilyIds = new Set(
		predictions
			.filter(
				(pred) =>
					pred.publishDiagnostics?.reason === "situation_family_cap" &&
					pred.publishDiagnostics?.familyId,
			)
	...
	return {
		...
		selectedSupplyChainCount: selectedPredictions.filter((pred) => pred.domain === "supply_chain")
			.length,
		publishedSupplyChainCount: publishedPredictions.filter((pred) => pred.domain === "supply_chain")
			.length,
...
function getPublishSelectionTarget(predictions = []) {
	const familyCount = new Set(predictions.map((pred) => pred.familyContext?.id).filter(Boolean))
		.size;
	const stateCount = new Set(
		predictions.map((pred) => getForecastSelectionStateContext(pred)?.id).filter(Boolean),
	).size;
	...
	return Math.max(
		...
		Math.min(
			predictions.length,
			MAX_TARGET_PUBLISHED_FORECASTS,
			dynamicTarget || MIN_TARGET_PUBLISHED_FORECASTS,
		),
	);
}
...
function buildPublishSelectionMemoryIndex(priorWorldState = null) {
	...
	for (const item of situationMemory) {
		...
		const regionKey = String(item.dominantRegion || "")
			.trim()
			.toLowerCase();
...
function getPublishSelectionMemoryHint(pred, memoryIndex = null) {
	...
	const labelKey = String(stateContext?.label || "")
		.trim()
		.toLowerCase();
	...
	const regionDomainKey = `${String(
		stateContext?.dominantRegion || pred?.region || stateContext?.regions?.[0] || "",
	)
		.trim()
		.toLowerCase()}:${stateContext?.dominantDomain || pred?.domain || ""}`;
...
function computePublishSelectionScore(pred, memoryIndex = null) {
	...
	const domainLift = ["market", "military", "supply_chain", "infrastructure"].includes(pred.domain)
		? 0.02
		: 0;
	...
	const marketTransmissionLift = Math.min(
		0.07,
	...
	const criticalLift = Math.min(
		0.05,
		criticalSignalLift * 0.035 + Math.min(0.015, criticalSignalCount * 0.004),
	);
	...
	const coreBucketLift = CORE_MARKET_BUCKET_IDS.includes(topBucketId)
		? Math.min(
				0.035,
				marketConfirmation * 0.025 +
					Number(pred.marketSelectionContext?.topBucketPressure || 0) * 0.02,
			)
		: 0;
	const defensePenalty =
		topBucketId === "defense" && pred.marketSelectionContext?.topChannel !== "defense_repricing"
			? 0.018
			: 0;
...
function isHighLeverageStateFollowOn(pred) {
	...
	if (
		coreBucket &&
		["market", "supply_chain", "military", "infrastructure"].includes(pred.domain)
	) {
		if (marketConfirmation >= 0.56 || criticalSignalLift >= 0.54) return true;
	}
	if (transmissionEdgeCount >= 2 && (marketConfirmation >= 0.52 || criticalSignalLift >= 0.48))
		return true;
	return false;
}
...
function classifyForecastStrategicRole(pred) {
	...
	const text =
		`${pred?.title || ""} ${pred?.feedSummary || ""} ${pred?.caseFile?.baseCase || ""}`.toLowerCase();
	const logisticsText =
		/\b(shipping|freight|logistics|port|route|corridor|rerouting|throughput|transit|container|maritime|tanker|canal|strait)\b/;
	if (domain === "supply_chain") return "logistics";
	if (domain === "market") {
		if (
			topBucketId === "freight" ||
			topChannel === "shipping_cost_shock" ||
			logisticsText.test(text)
		)
...
function isStrategicSupplyChainCandidate(pred) {
	...
	return (
		...
		/\b(shipping|freight|logistics|port|route|corridor|rerouting|throughput|transit|container|maritime|tanker|canal|strait)\b/.test(
			text,
		)
	);
}
...
function canCoexistAsDistinctStrategicFollowOn(pred, selected = []) {
	...
	return (selected || []).some((item) => {
		if (!item || item.id === pred.id || !["market", "supply_chain"].includes(item.domain))
			return false;
		if (item.domain === pred.domain) return false;
		const existingRole = classifyForecastStrategicRole(item);
		return (
			(pred.domain === "supply_chain" &&
				predRole === "logistics" &&
				existingRole === "repricing") ||
...
function selectPublishedForecastPool(predictions, options = {}) {
	const eligible = (predictions || []).filter(
		(pred) => (pred?.probability || 0) > (options.minProbability ?? PUBLISH_MIN_PROBABILITY),
	);
	...
	for (const pred of predictions || [])
		pred.publishSelectionScore = computePublishSelectionScore(pred, memoryIndex);
	...
	for (const pred of ranked) {
		const familyId =
			pred.familyContext?.id || `solo:${getForecastSelectionStateContext(pred)?.id || pred.id}`;
	...
	const orderedFamilyIds = [...familyBuckets.keys()].sort((leftId, rightId) => {
		...
		const leftScore =
			(leftTop?.publishSelectionScore || 0) +
			Math.min(0.05, ((leftTop?.familyContext?.forecastCount || 1) - 1) * 0.005);
		const rightScore =
			(rightTop?.publishSelectionScore || 0) +
			Math.min(0.05, ((rightTop?.familyContext?.forecastCount || 1) - 1) * 0.005);
	...
	function canSelect(pred, mode = "fill") {
		...
		const selectedForSituation = selected.filter(
			(item) => (getForecastSelectionStateContext(item)?.id || item.id) === situationId,
		);
		const distinctStrategicFollowOn = canCoexistAsDistinctStrategicFollowOn(
			pred,
			selectedForSituation,
		);
		if (
			familyTotal >=
			Math.min(MAX_PUBLISHED_FORECASTS_PER_FAMILY, MAX_PRESELECTED_FORECASTS_PER_FAMILY)
		)
			return false;
		...
		if (
			(mode === "state_anchor" || mode === "diversity") &&
			situationTotal >= 1 &&
			!distinctStrategicFollowOn
		)
			return false;
		if (
			mode === "fill" &&
			situationTotal >= 1 &&
			!distinctStrategicFollowOn &&
			!isHighLeverageStateFollowOn(pred)
		)
			return false;
		if (mode === "diversity") {
			...
			if (
				domainTotal >= 2 &&
				!["market", "military", "supply_chain", "infrastructure"].includes(pred.domain)
			)
				return false;
		}
		return true;
	...
	const memoryAnchors = ranked.filter(
		(pred) =>
	...
	const stateAnchors = [...stateAnchorMap.values()].sort(
		(a, b) =>
			(b.publishSelectionScore || 0) - (a.publishSelectionScore || 0) ||
			(b.analysisPriority || 0) - (a.analysisPriority || 0) ||
			(b.probability || 0) - (a.probability || 0),
	);
	const marketAnchors = ranked.filter(
		(pred) =>
	...
	const transmissionAnchors = ranked.filter(
		(pred) =>
	...
	for (const familyId of orderedFamilyIds) {
		...
		const selectedDomains = new Set(
			selected
				.filter(
					(pred) =>
						(pred.familyContext?.id ||
							`solo:${getForecastSelectionStateContext(pred)?.id || pred.id}`) === familyId,
				)
				.map((pred) => pred.domain),
		);
		const choice = bucket.find(
			(pred) => !selectedDomains.has(pred.domain) && canSelect(pred, "diversity"),
		);
		if (choice) take(choice);
	}
	...
	if (selected.length < MAX_TARGET_PUBLISHED_FORECASTS) {
		...
		if (!selected.some((p) => p.domain === "supply_chain")) {
			const candidate = ranked.find(
				(p) => isStrategicSupplyChainCandidate(p) && canSelect(p, "backfill"),
			);
			if (candidate) take(candidate);
		}
	}
	...
	if (deferredCandidates.length > 0) {
		console.log(
			`  [filterPublished] Deferred ${deferredCandidates.length} forecast(s) in family selection`,
		);
	}
...
function buildPublishedForecastArtifacts(candidatePool, fullRunSituationClusters) {
	...
	const filteredSituationClusters = projectSituationClusters(
		fullRunSituationClusters,
		filteredPredictions,
	);
	attachSituationContext(filteredPredictions, filteredSituationClusters);
	const filteredSituationFamilies = attachSituationFamilyContext(
		filteredPredictions,
		buildSituationFamilies(filteredSituationClusters),
	);
	const filteredStateUnits = attachStateContext(
		filteredPredictions,
		buildCanonicalStateUnits(filteredSituationClusters, filteredSituationFamilies),
	);
	const publishedPredictions = applySituationFamilyCaps(
		filteredPredictions,
		filteredSituationFamilies,
	);
	const publishedSituationClusters = projectSituationClusters(
		fullRunSituationClusters,
		publishedPredictions,
	);
	attachSituationContext(publishedPredictions, publishedSituationClusters);
	const publishedSituationFamilies = attachSituationFamilyContext(
		publishedPredictions,
		buildSituationFamilies(publishedSituationClusters),
	);
...
function filterPublishedForecasts(predictions, minProbability = PUBLISH_MIN_PROBABILITY) {
	...
	for (const pred of predictions) {
		...
		const bestDuplicate = kept.find((item) => {
			if (item.domain !== pred.domain) return false;
			if (
				item.familyContext?.id &&
				pred.familyContext?.id &&
				item.familyContext.id !== pred.familyContext.id
			)
				return false;
	...
	if (overlapSuppressedCount > 0) {
		console.log(
			`  [filterPublished] Suppressed ${overlapSuppressedCount} situation-overlap forecast(s)`,
		);
	}
	if (situationDomainCapSuppressedCount > 0) {
		console.log(
			`  [filterPublished] Suppressed ${situationDomainCapSuppressedCount} situation-domain-cap forecast(s)`,
		);
	}
	if (situationCapSuppressedCount > 0) {
		console.log(
			`  [filterPublished] Suppressed ${situationCapSuppressedCount} situation-cap forecast(s)`,
		);
	}
	return published;
}
...
function applySituationFamilyCaps(predictions, situationFamilies = []) {
	...
	if (familyCapSuppressedCount > 0) {
		console.log(
			`  [filterPublished] Suppressed ${familyCapSuppressedCount} situation-family-cap forecast(s)`,
		);
	}
...
function selectForecastsForEnrichment(predictions, options = {}) {
	...
	for (const domain of ENRICHMENT_PRIORITY_DOMAINS) {
		...
		const candidate = ranked.find(
			(item) => item.pred.domain === domain && !selectedIds.has(item.pred.id),
		);
		if (candidate && trySelect(scenarioOnly, candidate)) reservedScenarioDomains.push(domain);
	}

	for (const item of ranked) {
		if (combined.length + scenarioOnly.length >= maxTotal || scenarioOnly.length >= maxScenario)
			break;
...
const FORECAST_LLM_PROVIDER_NAMES = new Set(
...
function getForecastLlmCallOptions(stage = "default") {
	...
	const combinedProviderOrder = parseForecastProviderOrder(
		process.env.FORECAST_LLM_COMBINED_PROVIDER_ORDER,
	);
	const criticalProviderOrder = parseForecastProviderOrder(
		process.env.FORECAST_LLM_CRITICAL_PROVIDER_ORDER,
	);
	const impactProviderOrder = parseForecastProviderOrder(
		process.env.FORECAST_LLM_IMPACT_PROVIDER_ORDER,
	);
	const marketImplicationsProviderOrder = parseForecastProviderOrder(
	...
	const openrouterModel =
		stage === "combined"
			? process.env.FORECAST_LLM_COMBINED_MODEL_OPENROUTER ||
				process.env.FORECAST_LLM_MODEL_OPENROUTER
			: stage === "critical_signals"
				? process.env.FORECAST_LLM_CRITICAL_MODEL_OPENROUTER ||
					process.env.FORECAST_LLM_MODEL_OPENROUTER
				: stage === "impact_expansion"
					? process.env.FORECAST_LLM_IMPACT_MODEL_OPENROUTER ||
						process.env.FORECAST_LLM_MODEL_OPENROUTER
					: stage === "market_implications"
						? process.env.FORECAST_LLM_MARKET_IMPLICATIONS_MODEL_OPENROUTER ||
							process.env.FORECAST_LLM_MODEL_OPENROUTER
						: process.env.FORECAST_LLM_MODEL_OPENROUTER;
...
function resolveForecastLlmProviders(options = {}) {
	const requestedOrder =
		Array.isArray(options.providerOrder) && options.providerOrder.length > 0
...
function validatePerspectives(items, predictions) {
	...
	return items.filter((item) => {
		if (typeof item.index !== "number" || item.index < 0 || item.index >= predictions.length)
			return false;
...
function extractStructuredLlmPayload(text) {
	...
	for (const candidate of candidates) {
		...
		if (direct.items)
			return {
				items: direct.items,
				diagnostics: { stage: direct.stage, preview: sanitizeForPrompt(trimmed).slice(0, 220) },
			};
		const firstArray = extractFirstJsonArray(trimmed);
		if (firstArray) {
			const arrayParsed = tryParseStructuredCandidate(firstArray);
			if (arrayParsed.items)
				return {
					items: arrayParsed.items,
					diagnostics: {
						stage: arrayParsed.stage,
						preview: sanitizeForPrompt(firstArray).slice(0, 220),
					},
				};
		}
	}
...
function tryParseStructuredCandidate(candidate) {
	try {
		...
		if (Array.isArray(parsed?.scenarios))
			return { items: parsed.scenarios, stage: "object_scenarios" };
		if (Array.isArray(parsed?.predictions))
			return { items: parsed.predictions, stage: "object_predictions" };
	} catch {
...
function buildScenarioEvidenceCandidates(pred) {
	return [
		...
		...(pred.caseFile?.branches || []).flatMap((branch) => [
			branch.summary || "",
			branch.outcome || "",
		]),
	].filter(Boolean);
...
function validateScenarios(scenarios, predictions) {
	...
	return scenarios.filter((s) => {
		...
		const hasEvidenceRef = evidenceCandidates.some((candidate) =>
			hasEvidenceReference(scenarioLower, candidate),
		);
		if (!hasEvidenceRef) {
...
async function callForecastLLM(systemPrompt, userPrompt, options = {}) {
	...
	console.log(
		`  [LLM:${stage}] providerOrder=${requestedOrder} modelOverrides=${JSON.stringify(options.modelOverrides || {})}`,
	);
			...
			const resp = await fetch(provider.apiUrl, {
				...
				headers: {
					...
					...(provider.name === "openrouter"
						? { "HTTP-Referer": "https://worldmonitor.app", "X-Title": "World Monitor" }
						: {}),
				},
				body: JSON.stringify({
...
async function redisSet(url, token, key, data, ttlSeconds) {
	if (_testRedisStore) {
		_testRedisStore[key] = JSON.parse(JSON.stringify(data));
		return;
	}
	try {
		await fetch(url, {
		...
		.update(
			JSON.stringify(
				preds.map((p) => ({
					...
					g: (p.cascades || [])
						.map((cascade) => `${cascade.domain}:${cascade.effect}:${cascade.probability}`)
						.join(","),
				})),
			),
		)
	...
	const predsText = preds
		.map((p, i) => {
			...
			const support = (caseFile.supportingEvidence || [])
				...
				.map(
					(item) =>
						`- ${sanitizeForPrompt(item.summary)} (${Math.round((item.weight || 0) * 100)}%)`,
				)
				.join("\n");
			const counter = (caseFile.counterEvidence || [])
				...
				.join("\n");
			const triggers = (caseFile.triggers || [])
				.slice(0, 3)
			...
			const actors = (caseFile.actors || [])
				...
				.map(
					(actor) =>
						`- ${sanitizeForPrompt(actor.name)} [${sanitizeForPrompt(actor.category)}]: ${sanitizeForPrompt(actor.role)} | objective: ${sanitizeForPrompt(actor.objectives?.[0] || "")} | likely action: ${sanitizeForPrompt(actor.likelyActions?.[0] || "")}`,
				)
				.join("\n");
			const worldSummary = caseFile.worldState?.summary
				? sanitizeForPrompt(caseFile.worldState.summary)
				: "";
			const worldPressures = (caseFile.worldState?.activePressures || [])
				.slice(0, 3)
				.map((item) => `- ${sanitizeForPrompt(item)}`)
				.join("\n");
			const worldStabilizers = (caseFile.worldState?.stabilizers || [])
				.slice(0, 2)
				.map((item) => `- ${sanitizeForPrompt(item)}`)
				.join("\n");
			const worldUnknowns = (caseFile.worldState?.keyUnknowns || [])
				.slice(0, 3)
				.map((item) => `- ${sanitizeForPrompt(item)}`)
				.join("\n");
			const branches = (caseFile.branches || [])
				...
				.map(
					(branch) =>
						`- ${sanitizeForPrompt(branch.kind)}: ${sanitizeForPrompt(branch.summary)} | outcome: ${sanitizeForPrompt(branch.outcome)} | projected: ${Math.round((branch.projectedProbability || 0) * 100)}%`,
				)
				.join("\n");
...
function buildFallbackBaseCase(pred) {
	...
	if (branch?.summary && branch?.outcome) {
		...
		if (
			situation?.forecastCount > 1 &&
			!branchNarrative.toLowerCase().includes((situation.label || "").toLowerCase())
		) {
...
function buildFeedSummary(pred) {
	...
	const shouldPreferScenario =
		...
		(isLlmNarrativeSource(narrativeSource) &&
			scenario &&
			(!baseCase ||
				sanitizeForOutput(baseCase, 500) === sanitizeForOutput(deterministicBaseCase, 500)));
...
function populateFallbackNarratives(predictions) {
	...
	for (const pred of predictions) {
		...
		if (!pred.caseFile.escalatoryCase)
			pred.caseFile.escalatoryCase = buildFallbackEscalatoryCase(pred);
		if (!pred.caseFile.contrarianCase)
	...
	if (fallbackCount > 0) {
		console.log(
			`  [fallbackNarratives] Applied fallback narratives to ${fallbackCount} forecast(s)`,
		);
	}
}
...
function refreshPublishedNarratives(predictions) {
	for (const pred of predictions || []) {
		...
		if (
			!preserveNarratives ||
			!pred.feedSummary ||
			sanitizeForOutput(pred.feedSummary, 500) === deterministicFeedSummary
		) {
...
async function recoverScenarioNarratives(
	predictions,
	llmOptions = {},
	stage = "scenario_recovery",
) {
	if (!Array.isArray(predictions) || predictions.length === 0) return null;
	const result = await callForecastLLM(SCENARIO_SYSTEM_PROMPT, buildUserPrompt(predictions), {
		...llmOptions,
		stage,
	});
	if (!result) return null;
...
async function enrichScenariosWithLLM(predictions) {
	...
	console.log(
		`  [LLM] selected combined=${topWithPerspectives.length} scenario=${scenarioOnly.length}`,
	);
	...
	if (topWithPerspectives.length > 0) {
		...
		console.log(
			`  [LLM:combined] start selected=${topWithPerspectives.length} cacheKey=${cacheKey}`,
		);
		const cached = await redisGet(url, token, cacheKey);
		...
		if (cached?.items) {
			...
			enrichmentMeta.combined.perspectives = cached.items.filter(
				(item) => item.strategic || item.regional || item.contrarian,
			).length;
			enrichmentMeta.combined.cases = cached.items.filter(
				(item) => item.baseCase || item.escalatoryCase || item.contrarianCase,
			).length;
				...
				if (item.index >= 0 && item.index < topWithPerspectives.length) {
					...
					if (item.baseCase || item.escalatoryCase || item.contrarianCase) {
						topWithPerspectives[item.index].caseFile = {
							...(topWithPerspectives[item.index].caseFile ||
								buildForecastCase(topWithPerspectives[item.index])),
							baseCase: item.baseCase || topWithPerspectives[item.index].caseFile?.baseCase || "",
							escalatoryCase:
								item.escalatoryCase ||
								topWithPerspectives[item.index].caseFile?.escalatoryCase ||
								"",
							contrarianCase:
								item.contrarianCase ||
								topWithPerspectives[item.index].caseFile?.contrarianCase ||
								"",
						};
					}
					if (
			...
			console.log(
				JSON.stringify({ event: "llm_combined", cached: true, count: cached.items.length, hash }),
			);
		} else {
			...
			const result = await callForecastLLM(
				...
				buildUserPrompt(topWithPerspectives),
				{ ...combinedLlmOptions, stage: "combined" },
			);
			if (result) {
				const parsed = extractStructuredLlmPayload(result.text);
				const raw = parsed.items;
				...
				if (validScenarios.length === 0 && validCases.length === 0) {
					const recovery = await recoverScenarioNarratives(
						topWithPerspectives,
						scenarioLlmOptions,
						"combined_recovery",
					);
					if (recovery && (recovery.validScenarios.length > 0 || recovery.validCases.length > 0)) {
						...
						enrichmentMeta.combined.rawPreview =
							recovery.parsed.diagnostics?.preview || enrichmentMeta.combined.rawPreview;
					}
				}

				for (const s of validScenarios) topWithPerspectives[s.index].scenario = s.scenario;
				for (const p of validPerspectives) {
					topWithPerspectives[p.index].perspectives = {
				...
				for (const c of validCases) {
					topWithPerspectives[c.index].caseFile = {
						...(topWithPerspectives[c.index].caseFile ||
							buildForecastCase(topWithPerspectives[c.index])),
						baseCase: c.baseCase || topWithPerspectives[c.index].caseFile?.baseCase || "",
						escalatoryCase:
							c.escalatoryCase || topWithPerspectives[c.index].caseFile?.escalatoryCase || "",
						contrarianCase:
							c.contrarianCase || topWithPerspectives[c.index].caseFile?.contrarianCase || "",
					};
				}
				const touchedCombinedIndexes = new Set([
				...
				applyLlmTraceMeta(
					topWithPerspectives,
					[...touchedCombinedIndexes],
					combinedNarrativeSource,
					enrichmentMeta.combined.provider,
					enrichmentMeta.combined.model,
					false,
				);
				...
				for (const index of [...touchedCombinedIndexes].sort((a, b) => a - b)) {
					...
					if (p) {
						entry.strategic = p.strategic;
						entry.regional = p.regional;
						entry.contrarian = p.contrarian;
					}
				...
				console.log(
					JSON.stringify({
						...
						model: result.model,
						hash,
						count: topWithPerspectives.length,
						...
						latencyMs: Math.round(Date.now() - t0),
						cached: false,
					}),
				);
	...
	if (scenarioOnly.length > 0) {
		...
		if (cached?.scenarios) {
			...
			enrichmentMeta.scenario.cases = cached.scenarios.filter(
				(item) => item.baseCase || item.escalatoryCase || item.contrarianCase,
			).length;
			...
			for (const s of cached.scenarios) {
				...
				if (
					s.index >= 0 &&
					s.index < scenarioOnly.length &&
					(s.baseCase || s.escalatoryCase || s.contrarianCase)
				) {
					scenarioOnly[s.index].caseFile = {
						...
						escalatoryCase:
							s.escalatoryCase || scenarioOnly[s.index].caseFile?.escalatoryCase || "",
						contrarianCase:
							s.contrarianCase || scenarioOnly[s.index].caseFile?.contrarianCase || "",
					};
				}
				if (
					s.index >= 0 &&
					s.index < scenarioOnly.length &&
					(s.scenario || s.baseCase || s.escalatoryCase || s.contrarianCase)
				) {
			...
			if (result) {
				...
				for (const c of validCases) {
					scenarioOnly[c.index].caseFile = {
						...
						escalatoryCase:
							c.escalatoryCase || scenarioOnly[c.index].caseFile?.escalatoryCase || "",
						contrarianCase:
							c.contrarianCase || scenarioOnly[c.index].caseFile?.contrarianCase || "",
					};
				}
				const touchedScenarioIndexes = new Set([
				...
				console.log(
					JSON.stringify({
						...
						model: result.model,
						hash,
						count: scenarioOnly.length,
						rawItems: Array.isArray(raw) ? raw.length : 0,
						parseStage: enrichmentMeta.scenario.parseStage || "",
						scenarios: valid.length,
						cases: validCases.length,
...
async function updateEmaWindows(inputs, url, token) {
	...
	const ucdpEvents = Array.isArray(inputs.ucdpEvents)
		? inputs.ucdpEvents
		: (inputs.ucdpEvents?.events ?? []);
	...
	await redisCommand(url, token, [
		"SET",
		"conflict:ema-windows:v1",
		JSON.stringify(windowsObj),
		"EX",
		ttl,
	]).catch((err) => console.warn(`  [EMA] Failed to persist windows: ${err.message}`));
	await redisCommand(url, token, [
		"SET",
		"seed-meta:conflict:ema-windows:v1",
		JSON.stringify({ fetchedAt: new Date().toISOString(), recordCount: updatedWindows.size }),
		"EX",
		ttl,
	...
	if (spikeCount > 0) {
		console.log(
			`  [EMA] ${spikeCount} velocity spike(s) detected:`,
			[...riskScores.entries()]
				.filter(([, v]) => v.velocitySpike)
				.map(([k, v]) => `${k}(${v.risk24h})`)
				.join(", "),
		);
	}
...
async function fetchForecasts() {
	...
	console.log(`  Generated ${predictions.length} predictions`);
	{
		...
		console.log(
			`  Forecast trace config: raw=${traceCap.raw ?? "default"} resolved=${traceCap.resolved} total=${traceCap.totalForecasts}`,
		);
	}
	...
	let fullRunSituationFamilies = attachSituationFamilyContext(
		predictions,
		buildSituationFamilies(fullRunSituationClusters),
	);
	...
	let selectionMarketTransmission = buildMarketTransmissionGraph(
	...
	if (stateDerivedPredictions.length > 0) {
		const stateDerivedDomainCounts = summarizeTypeCounts(
			stateDerivedPredictions.map((pred) => pred.domain),
		);
		console.log(
			`  [stateDerived] Added ${stateDerivedPredictions.length} forecast(s) from canonical state units (${Object.entries(
				stateDerivedDomainCounts,
			)
		...
		fullRunSituationFamilies = attachSituationFamilyContext(
			predictions,
			buildSituationFamilies(fullRunSituationClusters),
		);
		...
		selectionMarketTransmission = buildMarketTransmissionGraph(
			selectionWorldSignals,
			fullRunSituationClusters,
		);
	...
	let publishArtifacts = buildPublishedForecastArtifacts(
		finalSelectionPool,
		fullRunSituationClusters,
	);
	while (
		publishArtifacts.publishedPredictions.length < (finalSelectionPool.targetCount || 0) &&
		deferredCandidates.length > 0
	) {
		finalSelectionPool.push(deferredCandidates.shift());
		publishArtifacts = buildPublishedForecastArtifacts(
			finalSelectionPool,
			fullRunSituationClusters,
		);
	}
	...
	const publishTelemetry = summarizePublishFiltering(
		predictions,
		finalSelectionPool,
		publishedPredictions,
	);
	...
	if (publishedPredictions.length !== predictions.length) {
		console.log(
			`  Filtered ${predictions.length - publishedPredictions.length} forecasts at publish floor > ${PUBLISH_MIN_PROBABILITY}`,
		);
	}
...
function buildDeepForecastRejectedPreview(paths = []) {
	return (paths || [])
		...
		.sort(
			(a, b) =>
				Number(b.acceptanceScore || 0) - Number(a.acceptanceScore || 0) ||
...
async function processDeepForecastTask(task = {}) {
	...
	if (!snapshotValidation.pass) {
		...
		if (snapshotValidation.unresolvedSelectedStateIds.length > 0) {
			errors.push(
				`unresolved_selected_state_ids:${snapshotValidation.unresolvedSelectedStateIds.join(",")}`,
			);
		}
		if (snapshotValidation.duplicateStateLabels.length > 0) {
			errors.push(
				`duplicate_canonical_state_labels:${snapshotValidation.duplicateStateLabels.map((item) => item.label).join(",")}`,
			);
		}
	...
	const learnedSection =
		(await redisGet(redisUrl, redisToken, PROMPT_LEARNED_KEY).catch(() => null)) || "";
	...
	try {
		const simulationOutcome = await fetchSimulationOutcomeForMerge(
			storageConfig,
			snapshot.runId || "",
		);
	...
	const baseDeepForecast = {
		...
		selectedPathCount: (evaluation.selectedPaths || []).filter((path) => path.type === "expanded")
			.length,
...
async function writeFailedDeepForecastArtifacts(task = {}, failureReason = "") {
	...
	const deepForecast = {
		...
		completedAt: new Date().toISOString(),
		replacedFastRun: false,
		rejectedPathsPreview: Array.isArray(snapshot.deepForecast?.rejectedPathsPreview)
			? snapshot.deepForecast.rejectedPathsPreview
			: [],
		selectedPathCount: 0,
	};
	await writeForecastTraceArtifacts(
...
function scoreImpactExpansionQuality(validation, candidatePackets = []) {
	...
	const commodityRate =
		mapped.filter((h) => h.commodity && h.commodity !== "").length / Math.max(mapped.length, 1);

	// asset coverage: % of mapped with at least 1 affectedAssets entry
	const assetRate =
		mapped.filter((h) => (h.affectedAssets || h.assetsOrSectors || []).length > 0).length /
	...
	const chainCoverage =
		Object.values(byCandidate).filter((c) => c.direct > 0 && c.second > 0).length / nCandidates;
...
function buildImpactPromptCritiqueUserPrompt(qualityMetrics, mapped, candidatePackets) {
	const sample = mapped
		...
		.join("\n");
	const candidates = candidatePackets
		.slice(0, 3)
		.map(
			(p) =>
	...
	const uniqueDirectGeos = [
		...new Set(
			directMapped.map((h) => (h.geography || h.region || "").split(",")[0].trim()).filter(Boolean),
		),
	];
...
async function runImpactExpansionPromptRefinement({
	candidatePackets,
	validation,
	priorWorldState,
}) {
	try {
		...
		if (lastAttemptRaw && Date.now() - Number(lastAttemptRaw) < PROMPT_MIN_REFINEMENT_INTERVAL_MS)
			return { iterationCount: 0, committed: false, exitReason: "rate_limited" };
		...
		console.log(
			`  [PromptRefinement] Quality breakdown — composite=${currentScore.composite.toFixed(3)} comDiversity=${directCommodityDiversity.toFixed(2)} geoDiversity=${directGeoDiversity.toFixed(2)} spread=${candidateSpreadScore.toFixed(2)} chain=${chainCoverage.toFixed(2)} comRate=${commodityRate.toFixed(2)} mapped=${mappedCount}`,
		);
		for (const h of validation?.mapped || []) {
			console.log(
				`    [${h.order}] key=${h.hypothesisKey || h.variableKey || "?"} geo="${h.geography || h.region || ""}" com="${h.commodity || ""}" assets=${(h.affectedAssets || h.assetsOrSectors || []).length} score=${h.validationScore?.toFixed(3) || "?"}`,
			);
		}
		...
		console.log(
			`  [PromptRefinement] Quality ${currentScore.composite.toFixed(3)} below 0.80 — running critique (comDiversity=${currentScore.directCommodityDiversity.toFixed(2)})`,
		);
		...
		try {
			...
			console.warn(
				`  [PromptRefinement] Raw response (first 400 chars): ${critiqueResult.text?.slice(0, 400)}`,
			);
		...
		if (didCommit) {
			...
			console.log(
				`  [PromptRefinement] Committed: ${currentBaseline.toFixed(3)} → ${testScore.composite.toFixed(3)} | ${critique.diagnosis}`,
			);
		} else {
			console.log(
				`  [PromptRefinement] Reverted: test ${testScore.composite.toFixed(3)} <= baseline ${currentBaseline.toFixed(3)}`,
			);
		}
		return {
			iterationCount: 1,
			committed: didCommit,
			exitReason: didCommit ? "committed" : "reverted",
		};
	} catch (err) {
...
async function processNextDeepForecastTask(options = {}) {
	...
	console.log(
		`  [DeepForecast] Queue check: ${queuedRunIds.length} task(s) in ${FORECAST_DEEP_TASK_QUEUE_KEY}`,
	);
	for (const runId of queuedRunIds) {
		const task = await claimDeepForecastTask(runId, workerId);
		if (!task) {
			console.log(`  [DeepForecast] ${runId}: already claimed or completed`);
			continue;
		}
		try {
			...
			await writeFailedDeepForecastArtifacts(task, err.message).catch((writeErr) => {
				console.warn(
					`  [DeepForecast] Failed to write failed-task artifacts for ${runId}: ${writeErr.message}`,
				);
			});
			await completeDeepForecastTask(runId);
...
function buildMarketImplicationsContext(inputs) {
	...
	if (Array.isArray(criticalSignals) && criticalSignals.length > 0) {
		...
		const top = criticalSignals.slice(0, 8).map((s) => {
			...
			const domains =
				Array.isArray(s.domains) && s.domains.length ? ` [${s.domains.join(",")}]` : "";
			const evidence =
				Array.isArray(s.supportingEvidence) && s.supportingEvidence.length
	...
	if (Array.isArray(commodities) && commodities.length > 0) {
		const top = commodities
			...
			.map(
				(q) =>
					`${q.display || q.symbol}: ${q.price != null ? q.price.toFixed(2) : "N/A"} (${q.change != null ? (q.change >= 0 ? "+" : "") + q.change.toFixed(2) + "%" : "N/A"})`,
			);
	...
	if (Array.isArray(sectors) && sectors.length > 0) {
		const top = sectors
			...
			.map(
				(s) =>
					`${s.name}: ${s.change != null ? (s.change >= 0 ? "+" : "") + s.change.toFixed(2) + "%" : "N/A"}`,
			);
		parts.push(`[SECTORS]\n${top.join("\n")}`);
	}
	...
	if (etfItems.length > 0) {
		const sorted = [...etfItems].sort(
			(a, b) => Math.abs(b.flowPct ?? b.changePct ?? 0) - Math.abs(a.flowPct ?? a.changePct ?? 0),
		);
	...
	if (policyRates.length > 0) {
		const rateLines = policyRates
			.slice(0, 8)
			.map(
				(r) =>
					`${r.country || r.code || r.name}: ${r.rate != null ? r.rate.toFixed(2) + "%" : "N/A"}`,
			);
	...
	const chokepointList = Array.isArray(chokepoints)
		? chokepoints
		: Array.isArray(chokepoints?.routes)
			? chokepoints.routes
			: Array.isArray(chokepoints?.chokepoints)
				? chokepoints.chokepoints
				: [];
	if (chokepointList.length > 0) {
		const atRisk = chokepointList
	...
	if (fredSeries && typeof fredSeries === "object") {
		...
		if (fredSeries.T10Y2Y?.value != null)
			fredParts.push(`10Y-2Y Spread: ${fredSeries.T10Y2Y.value}`);
		if (fredSeries.FEDFUNDS?.value != null)
			fredParts.push(`Fed Funds: ${fredSeries.FEDFUNDS.value}`);
		if (fredSeries.DCOILWTICO?.value != null)
			fredParts.push(`WTI Crude (FRED): ${fredSeries.DCOILWTICO.value}`);
		if (fredSeries.UNRATE?.value != null)
			fredParts.push(`Unemployment Rate: ${fredSeries.UNRATE.value}%`);
	...
	if (Array.isArray(geoMarkets) && geoMarkets.length > 0) {
		const top = geoMarkets
			...
			.map(
				(m) =>
					`- ${sanitizeForPrompt(m.title.slice(0, 100))}: ${Math.round(m.yesPrice)}% YES (${m.source || "Polymarket"})`,
			);
	...
	if (Array.isArray(sanctionedCountries) && sanctionedCountries.length > 0) {
		const high = sanctionedCountries
			...
			.slice(0, 5)
			.map(
				(c) => `${c.name || c.country || c.code}: pressure=${c.score ?? c.pressureScore ?? "N/A"}`,
			);
	...
	if (Array.isArray(insights) && insights.length > 0) {
		const top = insights
			.slice(0, 5)
			.map((s) => `- ${sanitizeForPrompt(s.title || s.summary || "")}`);
...
function validateMarketImplications(cards, allowedTickers = ALL_ALLOWED_TICKERS) {
	...
	for (const card of cards) {
		...
		const confidence =
			typeof card.confidence === "string" ? card.confidence.trim().toUpperCase() : "";
		...
		if (Array.isArray(card.transmission_chain)) {
			for (const step of card.transmission_chain.slice(0, 4)) {
				...
				const impactType =
					typeof step.impact_type === "string" ? step.impact_type.trim().toLowerCase() : "";
		...
		valid.push({
			...
			narrative,
			risk_caveat:
				typeof card.risk_caveat === "string" ? card.risk_caveat.trim().slice(0, 300) : "",
...
async function buildAndSeedMarketImplications(inputs) {
	...
	if (!Array.isArray(rawCards) || rawCards.length === 0) {
		console.warn(
			`  [MarketImplications] No parseable cards in LLM response (diagnostics: ${JSON.stringify(parsed.diagnostics)})`,
		);
		return;
	}
	...
	if (liveTickerSet.size > 0) {
		...
		console.log(
			`  [MarketImplications] Extended allowlist: ${ALL_ALLOWED_TICKERS.size} static + ${tradeableLive.size} live equity symbols`,
		);
	} else {
	...
	console.log(
		`  [MarketImplications] Published ${cards.length} cards to ${MARKET_IMPLICATIONS_KEY} (${Math.round(durationMs)}ms, model=${result.model || "unknown"})`,
	);
}
...
if (_isDirectRun) {
	...
	console.log(
		`  [Trigger] source=${triggerContext.triggerSource}${triggerContext.triggerRequest?.requester ? ` requester=${triggerContext.triggerRequest.requester}` : ""}`,
	);
		...
		},
		{
			...
			afterPublish: async (data, meta) => {
				...
				try {
					...
					console.log(
						`  History appended: ${snapshot.predictions.length} forecasts -> ${HISTORY_KEY}`,
					);
				} catch (err) {
				...
				try {
					...
					if (snapshotWrite?.storageConfig && (data.impactExpansionCandidates || []).length > 0) {
						writeSimulationPackage(snapshotPayload, {
							...
							.catch((err) =>
								console.warn(`  [SimulationPackage] Write/enqueue failed: ${err.message}`),
							);
					}
					if (
						deepForecast.status === "queued" &&
						(data.impactExpansionCandidates || []).length > 0
					) {
						if (snapshotWrite?.snapshotKey) {
							const queueResult = await enqueueDeepForecastTask({
								runId,
								snapshotKey: snapshotWrite.snapshotKey,
								fastPrefix: buildTraceRunPrefix(
									runId,
									data.generatedAt,
									snapshotWrite.storageConfig?.basePrefix || FORECAST_DEEP_RUN_PREFIX,
								),
								priorWorldStateKey: data.priorWorldStateKey || "",
							...
							if (!queueResult.queued) {
								deepForecast = {
									...
									failureReason:
										queueResult.reason === "duplicate" ? "" : queueResult.reason || "queue_failed",
								};
							}
						} else {
							deepForecast = {
						...
						console.warn(
							"  [DeepForecast] Snapshot write skipped or failed; replay will not be available for this run",
						);
					}
					...
					if (pointer) {
						console.log(
							`  [Trace] Written: ${pointer.summaryKey} (${pointer.tracedForecastCount} forecasts)`,
						);
					} else {
					...
					if (err.stack)
						console.warn(`  [Trace] Stack: ${err.stack.split("\n").slice(0, 3).join(" | ")}`);
				}
...
function buildSimulationRound1SystemPrompt(theater, pkg) {
	...
	const entityList = theaterEntities
		.slice(0, 10)
		.map(
			(e) =>
				`- ${sanitizeForPrompt(e.entityId)} | ${sanitizeForPrompt(e.name)} | class=${sanitizeForPrompt(e.class)} | stance=${sanitizeForPrompt(e.stance || "unknown")}`,
		)
		.join("\n");

	const theaterSeeds = (pkg.eventSeeds || []).filter((s) => s.theaterId === theater.theaterId);
	const seedList = theaterSeeds
		.slice(0, 8)
		.map(
			(s) =>
				`- ${sanitizeForPrompt(s.seedId)} [${sanitizeForPrompt(s.type)}] ${sanitizeForPrompt(s.summary)} (${sanitizeForPrompt(s.timing)})`,
		)
		.join("\n");

	const constraints =
		(pkg.constraints?.[theater.theaterId] || [])
			.map(
				(c) =>
					`- [${c.hard ? "hard" : "soft"}] ${sanitizeForPrompt(c.class)}: ${sanitizeForPrompt(c.statement)}`,
			)
			.join("\n") || "- No explicit constraints";
	const theaterEvalTargets = pkg.evaluationTargets?.[theater.theaterId];
	const evalTargets =
		(theaterEvalTargets?.requiredPaths || [])
			.map((p) => `- ${sanitizeForPrompt(p.pathType)}: ${sanitizeForPrompt(p.question)}`)
...
function buildSimulationRound2SystemPrompt(theater, pkg, round1) {
	...
	const pathSummaries =
		r1Paths
			.map(
				(p) =>
					`- ${p.pathId}: ${sanitizeForPrompt(p.summary || "")} — actors: ${(
						p.initialReactions || []
					)
						.slice(0, 3)
						.map((r) => sanitizeForPrompt(r.actorId || ""))
						.join(", ")}`,
			)
	...
	const entityIds = theaterEntities
		.slice(0, 10)
		.map((e) => sanitizeForPrompt(e.entityId || ""))
		.join(", ");

	const r2EvalTargets = pkg.evaluationTargets?.[theater.theaterId];
	const evalTargets =
		(r2EvalTargets?.requiredPaths || [])
			.map((p) => `- ${sanitizeForPrompt(p.pathType)}: ${sanitizeForPrompt(p.question)}`)
			.join("\n") || "- General market and security dynamics";
	...
	try {
		...
		if (round === 2) {
			return {
				...
				stabilizers: Array.isArray(parsed.stabilizers)
					? parsed.stabilizers.map(String).slice(0, 6)
					: [],
				invalidators: Array.isArray(parsed.invalidators)
					? parsed.invalidators.map(String).slice(0, 6)
					: [],
		...
		return {
			paths,
			dominantReactions: Array.isArray(parsed.dominantReactions)
				? parsed.dominantReactions.map(String).slice(0, 6)
				: [],
...
function extractSimulationRoundPayload(text, round) {
	...
	for (const candidate of candidates) {
		...
		if (direct.paths)
			return {
				...direct,
				diagnostics: { stage: "direct", preview: sanitizeForPrompt(trimmed).slice(0, 160) },
			};
		const firstObject = extractFirstJsonObject(trimmed);
		if (firstObject) {
			...
			if (parsed.paths)
				return {
					...
					diagnostics: {
						stage: "extracted",
						preview: sanitizeForPrompt(firstObject).slice(0, 160),
					},
				};
		}
	}
	return {
		paths: null,
		diagnostics: { stage: "no_json", preview: sanitizeForPrompt(cleaned).slice(0, 160) },
	};
}
...
async function runTheaterSimulation(theater, pkg) {
	...
	const r1Raw = await callForecastLLM(
		...
		userPrompt1,
		{
			...getForecastLlmCallOptions("simulation_round_1"),
			stage: "simulation_round_1",
			maxTokens: SIMULATION_ROUND1_MAX_TOKENS,
			temperature: 0,
		},
	);
	...
	if (!r1.paths)
		return { failed: true, reason: "round1_parse_failed", diagnostics: r1.diagnostics };
	...
	const r2Raw = await callForecastLLM(
		...
		userPrompt2,
		{
			...getForecastLlmCallOptions("simulation_round_2"),
			stage: "simulation_round_2",
			maxTokens: SIMULATION_ROUND2_MAX_TOKENS,
			temperature: 0,
		},
	);
...
async function writeSimulationDecorations(mergeResult, snapshot) {
	...
	for (const [candidateStateId, adjs] of byCandidateId) {
		...
		for (const fid of forecastIds) {
			...
			if (
				!existing ||
				Math.abs(best.simulationAdjustment) > Math.abs(existing.simulationAdjustment)
			) {
	...
	try {
		const { url, token } = getRedisCredentials();
		const sideKeyStatus = await redisAtomicWriteSimDecorations(
			url,
			token,
		...
		if (sideKeyStatus.startsWith("SKIPPED:")) {
			console.log(
				`  [SimulationDecorations] Skipping side key write — existing is from a newer run (existing=${sideKeyStatus.slice(8)}, this_run=${runGeneratedAt})`,
			);
			return;
		}
		await redisSet(
			url,
			token,
			SIMULATION_DECORATIONS_META_KEY,
			{
				fetchedAt: Date.now(),
		...
		console.log(
			`  [SimulationDecorations] Written ${decorationCount} decorations from ${byCandidateId.size} candidates (runId=${snapshot?.runId || "unknown"})`,
		);
 ...
 * @returns {Promise<string>}
 */
async function redisAtomicPatchSimDecorations(
	url,
	token,
	canonicalKey,
	byForecastId,
...
async function patchPublishedForecastsWithSimDecorations(byForecastId, runGeneratedAt) {
	try {
		const { url, token } = getRedisCredentials();
		const status = await redisAtomicPatchSimDecorations(
			url,
			token,
			CANONICAL_KEY,
			byForecastId,
		...
		if (status.startsWith("PATCHED:")) {
			console.log(
				`  [SimulationDecorations] Patched ${status.slice(8)} forecasts in ${CANONICAL_KEY} (atomic)`,
			);
		} else if (status.startsWith("SKIPPED:")) {
			console.log(
				`  [SimulationDecorations] Skipping patch — canonical key is from a newer run (published=${status.slice(8)}, sim_run=${runGeneratedAt})`,
			);
		} else if (status === "MISSING") {
			console.warn(
				"  [SimulationDecorations] Cannot patch canonical key — predictions missing or not an array",
			);
		}
		// UNCHANGED: no-op, no log needed
	} catch (err) {
		console.warn(
			`  [SimulationDecorations] Canonical key patch failed (non-fatal): ${err.message}`,
		);
	}
}
...
async function applySimulationDecorationsToForecasts(predictions) {
	try {
		...
		const ageMs =
			typeof decorations.generatedAt === "number" ? Date.now() - decorations.generatedAt : Infinity;
		if (ageMs > SIMULATION_DECORATIONS_MAX_AGE_MS) {
			console.warn(
				`  [SimulationDecorations] Skipping stale decorations (age=${Math.round(ageMs / 3600000)}h, run=${decorations.runId || "unknown"})`,
			);
			return;
		}
		...
		if (applied > 0) {
			console.log(
				`  [SimulationDecorations] Applied to ${applied}/${predictions.length} forecasts (run ${decorations.runId || "unknown"})`,
			);
		}
	} catch (err) {
...
async function applyPostSimulationRescore(runId, freshOutcome, storageConfig) {
	...
	try {
		...
		const hasPromotionOpportunity = (evalData.rejectedPaths || []).some(
			(p) =>
				p.type === "expanded" &&
				p.acceptanceScore >= 0.38 &&
				p.acceptanceScore < SIMULATION_MERGE_ACCEPT_THRESHOLD,
		);
		if (
			evalData.status !== "completed_no_material_change" &&
			!hasDemotionRisk &&
		...
		const deepForecast = {
			...
			failureReason: "",
			rejectedPathsPreview: buildDeepForecastRejectedPreview(evaluation.rejectedPaths || []),
			selectedPathCount: (evaluation.selectedPaths || []).filter((p) => p.type === "expanded")
				.length,
			replacedFastRun: evaluation.status === "completed",
			status: evaluation.status || "completed_no_material_change",
			selectedStateIds: (evaluation.selectedPaths || [])
				.filter((p) => p.type === "expanded")
				.map((p) => p.candidateStateId),
...
function buildSimulationTaskKey(runId) {
	return `${SIMULATION_TASK_KEY_PREFIX}:${runId}`;
}
function buildSimulationLockKey(runId) {
	return `${SIMULATION_LOCK_KEY_PREFIX}:${runId}`;
}
...
async function enqueueSimulationTask(runId) {
	...
	const queued = await redisCommand(url, token, [
		"SET",
		buildSimulationTaskKey(runId),
		JSON.stringify({ runId, createdAt: Date.now() }),
		"EX",
		String(SIMULATION_TASK_TTL_SECONDS),
		"NX",
	]);
...
async function claimSimulationTask(runId, workerId) {
	...
	const claim = await redisCommand(url, token, [
		...
		workerId,
		"EX",
		String(SIMULATION_LOCK_TTL_SECONDS),
		"NX",
	]);
	if (claim?.result !== "OK") return null;
...
function sanitizeKeyActorRoles(rawRoles, allowedRoles) {
	...
	const sanitized = (Array.isArray(rawRoles) ? rawRoles : []).map((s) =>
		sanitizeForPrompt(String(s)).slice(0, 80),
	);
...
async function processNextSimulationTask(options = {}) {
	...
	console.log(
		`  [Simulation] Queue check: ${queuedRunIds.length} task(s) in ${SIMULATION_TASK_QUEUE_KEY}`,
	);
	...
	for (const runId of queuedRunIds) {
		...
		if (!task) {
			console.log(`  [Simulation] ${runId}: already claimed or completed`);
			continue;
		}
		...
		try {
			...
			if (pkgPointer.runId && pkgPointer.runId !== runId) {
				console.warn(
					`  [Simulation] Package runId mismatch: task=${runId} pkg=${pkgPointer.runId} — using latest package (Phase 2 behaviour)`,
				);
			}
			...
			console.log(
				`  [Simulation] ${runId}: ${eligibleTheaters.length}/${pkgData.selectedTheaters.length} theaters eligible`,
			);
			...
			for (const theater of eligibleTheaters) {
				...
				const mergedPaths = (r2Paths.length ? r2Paths : r1Paths).map((p) => {
					...
					return {
						...
						roundByRoundEvolution: Array.isArray(p.roundByRoundEvolution)
							? p.roundByRoundEvolution.map((r) => ({
									round: r.round,
									summary: sanitizeForPrompt(r.summary || "").slice(0, 160),
								}))
							: [
									{
										round: 1,
										summary: sanitizeForPrompt(r1Path?.summary || p.summary || "").slice(0, 160),
									},
								],
						confidence:
							typeof p.confidence === "number" ? Math.max(0, Math.min(1, p.confidence)) : 0.5,
						timingMarkers: Array.isArray(p.timingMarkers)
							? p.timingMarkers.slice(0, 6).map((m) => ({
									event: sanitizeForPrompt(m.event || "").slice(0, 80),
									timing: String(m.timing || "T+0h").slice(0, 10),
								}))
							: [],
					};
				});
				...
				theaterResults.push({
					...
					dominantReactions: (result.round1?.dominantReactions || [])
						.map((s) => sanitizeForPrompt(String(s)).slice(0, 120))
						.slice(0, 6),
					...
					invalidators: (result.round2?.invalidators || [])
						.map((s) => sanitizeForPrompt(String(s)).slice(0, 120))
						.slice(0, 6),
					timingMarkers: (result.round2?.paths?.[0]?.timingMarkers || []).slice(0, 4).map((m) => ({
						event: sanitizeForPrompt(m.event || "").slice(0, 80),
						timing: String(m.timing || "T+0h").slice(0, 10),
					})),
				});
			...
			await completeSimulationTask(runId);
			console.log(
				`  [Simulation] Completed ${runId}: ${theaterResults.length} theaters → ${writeResult?.outcomeKey}`,
			);
			...
			const rescoreResult = await applyPostSimulationRescore(runId, outcome, storageConfig).catch(
				(err) => {
					console.warn(`  [SimulationRescore] Error for ${runId}: ${err.message}`);
					return null;
				},
			);
			if (rescoreResult && !rescoreResult.skipped)
				console.log(`  [SimulationRescore] ${runId}: ${JSON.stringify(rescoreResult)}`);
			return {
				status: "completed",
				runId,
