...
export function parseRecord(record, seededAt) {
	...
	if (raw > 500) {
		return {
			...
			abroadDays:
				(parseIntOrNull(record.abroadIndustry) ?? 0) + (parseIntOrNull(record.abroadPublic) ?? 0),
			belowObligation: false,
			obligationThreshold: IEA_90_DAY_OBLIGATION,
			anomaly: true,
			seededAt: ts,
...
export function buildOilStocksAnalysis(members, dataMonth, updatedAt) {
	...
	const ieaMembers = ranked.map((m) => {
		/** @type {number | null} */
		const vsObligation = m.netExporter
			? null
			: m.daysOfCover !== null
				? m.daysOfCover - IEA_90_DAY_OBLIGATION
				: null;
		const obligationMet =
			m.netExporter || (m.daysOfCover !== null && m.daysOfCover >= IEA_90_DAY_OBLIGATION);
		const entry = {
			iso2: m.iso2,
			daysOfCover: m.daysOfCover,
			netExporter: m.netExporter,
	...
	function regionStats(regionSet) {
		const regionMembers = eligible.filter(
			(m) => regionSet.has(m.iso2) && !m.netExporter && m.daysOfCover !== null,
		);
	...
	const naAvgDays =
		naImporters.length > 0
			? Math.round(
					naImporters.reduce((s, m) => s + /** @type {number} */ (m.daysOfCover), 0) /
						naImporters.length,
				)
			: null;
	...
	return {
		...
		regionalSummary: {
			europe: {
				avgDays: euStats.avgDays,
				minDays: euStats.minDays,
				countBelowObligation: euStats.countBelowObligation,
			},
			asiaPacific: {
				avgDays: apStats.avgDays,
				minDays: apStats.minDays,
				countBelowObligation: apStats.countBelowObligation,
			},
			northAmerica: {
				netExporters: naNetExporters,
				...(naAvgDays !== null ? { avgDays: naAvgDays } : {}),
			},
		},
		shockScenario: null,
	};
