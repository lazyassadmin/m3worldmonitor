...
async function withR2Retry(operation, context = {}) {
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		try {
			...
			console.warn(
				`  [R2] Retry ${attempt}/${maxAttempts - 1} for ${context.op || "operation"} key=${context.key || ""}: ${summarizeError(err)}`,
			);
...
function resolveR2StorageConfig(env = process.env, options = {}) {
	...
	const bucket = getEnvValue(env, [
		options.bucketEnv || "CLOUDFLARE_R2_TRACE_BUCKET",
	...
	const endpoint =
		getEnvValue(env, ["CLOUDFLARE_R2_ENDPOINT"]) ||
		(accountId ? `https://${accountId}.r2.cloudflarestorage.com` : "");
	const apiBaseUrl =
		getEnvValue(env, ["CLOUDFLARE_API_BASE_URL"]) || "https://api.cloudflare.com/client/v4";
	const region = getEnvValue(env, ["CLOUDFLARE_R2_REGION"]) || "auto";
	const basePrefix = (
		getEnvValue(env, [options.prefixEnv || "CLOUDFLARE_R2_TRACE_PREFIX"]) ||
	...
	if (!bucket || !accountId) {
		console.log(
			`  [R2] Config: accountId=${accountId ? "set" : "MISSING"}, bucket=${bucket ? "set" : "MISSING"}`,
		);
		return null;
	}
		...
		return withR2Retry(
			async () => {
				...
				const resp = await fetch(
					`${config.apiBaseUrl}/accounts/${config.accountId}/r2/buckets/${config.bucket}/objects/${encodedKey}`,
					{
						method: "PUT",
						headers: {
				...
				if (!resp.ok) {
					...
					const error = new Error(
						`Cloudflare R2 API upload failed: HTTP ${resp.status} — ${text.slice(0, 200)}`,
					);
		...
		return withR2Retry(
			async () => {
				...
				if (!resp.ok) {
					const text = await resp.text().catch(() => "");
					const error = new Error(
						`Cloudflare R2 API download failed: HTTP ${resp.status} — ${text.slice(0, 200)}`,
					);
					error.status = resp.status;
					throw error;
				}
...
export { resolveR2StorageConfig, getR2StorageClient, getR2JsonObject, putR2JsonObject };
