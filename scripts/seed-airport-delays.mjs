...
async function seedNotamClosures() {
	...
	console.log(
		`[NOTAM] Fetching closures for ${MONITORED_AIRPORTS_ICAO.length} monitored airports...`,
	);
...
async function main() {
	...
	const lockResult = await acquireLockSafely("aviation:delays", runId, 120_000, {
		label: "aviation:delays",
	});
	...
	try {
		...
		await extendExistingTtl(
			[FAA_CACHE_KEY, NOTAM_CACHE_KEY, "seed-meta:aviation:faa", "seed-meta:aviation:notam"],
			CACHE_TTL,
		);
