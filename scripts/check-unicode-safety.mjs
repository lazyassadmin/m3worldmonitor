...
const SCAN_ROOTS = ["src", "server", "api", "scripts", "tests", "e2e", ".github", ".husky"];
...
function main() {
	...
	if (files.length === 0) {
		console.log(
			stagedOnly
				? "Unicode safety: no staged executable files to scan."
				: "Unicode safety: no files matched scan scope.",
		);
		return;
	}
	...
	if (findings.length === 0) {
		console.log(
			`Unicode safety: scanned ${files.length} file(s), no suspicious hidden Unicode found.`,
		);
		return;
	}
