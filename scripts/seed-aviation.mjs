...
async function fetchAviationStackFlights(airports) {
	...
	for (const iata of airports) {
		try {
			...
			if (!resp.ok) {
				console.warn(`  AviationStack ${iata}: HTTP ${resp.status}`);
				continue;
			}
			const json = await resp.json();
			if (json.error) {
				console.warn(`  AviationStack ${iata}: ${json.error.message}`);
				continue;
			}
...
async function fetchAirportOpsSummary() {
	...
	console.log(
		`  Airport ops: ${summaries.length} airports, ${avResult.alerts.length} with live data`,
	);
	return { summaries };
}
		...
		while ((match = itemRegex.exec(xml)) !== null) {
			...
			const desc =
				block
					...
					.trim() || "";
			if (title && link)
				items.push({ title, link, pubDate, description: desc, _source: sourceName });
		}
...
async function fetchAll() {
	const [ops, news] = await Promise.allSettled([fetchAirportOpsSummary(), fetchAviationNews()]);
	...
	if (ops.status === "rejected")
		console.warn(`  AirportOps failed: ${ops.reason?.message || ops.reason}`);
	if (news.status === "rejected")
		console.warn(`  AviationNews failed: ${news.reason?.message || news.reason}`);
	...
	if (newsData?.items?.length > 0)
		await writeExtraKeyWithMeta(NEWS_CACHE_KEY, newsData, NEWS_TTL, newsData.items.length);
