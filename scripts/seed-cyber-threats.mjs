...
async function hydrateCoordinates(threats) {
	...
	return threats.map((t) => {
		...
		if (lookup)
			return { ...t, lat: lookup.lat, lon: lookup.lon, country: t.country || lookup.country };
...
async function fetchFeodo(cutoffMs) {
	try {
		...
		for (const r of records) {
			...
			const sev =
				status === "online" && /emotet|qakbot|trickbot|dridex|ransom/i.test(mf)
					? "critical"
					: status === "online"
						? "high"
						: "medium";
			const t = sanitize({
				...
				indicator: ip,
				indicatorType: "ip",
				lat: toNum(r?.latitude ?? r?.lat),
				lon: toNum(r?.longitude ?? r?.lon),
				country: normCountry(r?.country || r?.country_code),
				severity: sev,
				malwareFamily: mf,
				tags: normTags(["botnet", "c2", ...normTags(r?.tags)]),
				firstSeen,
				lastSeen,
			});
			if (t) threats.push(t);
...
async function fetchUrlhaus(cutoffMs) {
	...
	if (!authKey) {
		console.log("  URLhaus: skipped (no URLHAUS_AUTH_KEY)");
		return { ok: false, threats: [] };
	}
	try {
		...
		for (const r of rows) {
			...
			if (rawUrl) {
				try {
					hostname = clean(new URL(rawUrl).hostname, 255).toLowerCase();
				} catch {}
			}
			...
			const t = sanitize({
				...
				country: normCountry(r?.country || r?.country_code),
				severity: sev,
				malwareFamily: clean(r?.threat, 80),
				tags,
				firstSeen,
				lastSeen,
			});
...
async function fetchOtx(days) {
	...
	if (!apiKey) {
		console.log("  OTX: skipped (no OTX_API_KEY)");
		return { ok: false, threats: [] };
	}
	try {
		...
		for (const r of results) {
			...
			const t = sanitize({
				...
				malwareFamily: title,
				tags,
				firstSeen: toEpochMs(r?.created),
				lastSeen: toEpochMs(r?.modified || r?.created),
			});
			if (t) threats.push(t);
			if (threats.length >= MAX_LIMIT) break;
		}
...
async function fetchAbuseIpDb() {
	...
	if (!apiKey) {
		console.log("  AbuseIPDB: skipped (no ABUSEIPDB_API_KEY)");
		return { ok: false, threats: [] };
	}
	...
	try {
		...
		if (Date.now() - lastTs < ABUSEIPDB_MIN_INTERVAL_MS) {
			...
			if (Array.isArray(cached) && cached.length > 0) {
				console.log(
					`  AbuseIPDB: ${cached.length} threats (cached, called ${Math.round((Date.now() - lastTs) / 60000)}m ago)`,
				);
		...
		console.warn(
			"  AbuseIPDB: rate-limit check failed (Redis) — proceeding with caution:",
			e?.message || e,
		);
	...
	try {
		...
		for (const r of records) {
			...
			const t = sanitize({
				...
				lat: toNum(r?.latitude ?? r?.lat),
				lon: toNum(r?.longitude ?? r?.lon),
				country: normCountry(r?.countryCode || r?.country),
				severity: sev,
				malwareFamily: "",
				tags: normTags([`score:${score}`]),
				firstSeen: 0,
				lastSeen: toEpochMs(r?.lastReportedAt),
			});
			if (t) threats.push(t);
			if (threats.length >= MAX_LIMIT) break;
		}
