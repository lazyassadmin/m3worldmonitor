...
for (const file of srcFiles) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const patterns = [line.match(/from\s+['"]@\/(\w+)/), line.match(/import\(['"]@\/(\w+)/)];
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.match(/from\s+['"].*\/(components|app)\//)) {
			violations.push({
				...
				text: line.trim(),
				remedy:
					"Server code must not import browser UI code. Extract shared logic into server/_shared/ or src/types/.",
			});
		}
	}
}
		...
		if (isTs) {
			...
			if (line.match(/from\s+['"]\.\..*\/src\//) && !line.match(/\/src\/generated\//)) {
				violations.push({
					...
					text: line.trim(),
					remedy:
						"RPC Edge Functions may import from server/ and src/generated/, but not from src/ application code (components, services, config).",
				});
			}
		} else {
			...
			if (line.match(/from\s+['"]\.\.\/(?:src|server)\//)) {
				violations.push({
					...
					text: line.trim(),
					remedy:
						"Legacy Edge Functions must be self-contained. Only same-directory _*.js helpers and npm packages are allowed.",
				});
			}
		}
	}
