		...
		return [
			{
				...
				gasoline:
					ron95 != null
						? { localPrice: ron95, grade: "RON95", source: "data.gov.my", observedAt }
						: null,
				diesel:
					diesel != null
						? { localPrice: diesel, grade: "Euro5", source: "data.gov.my", observedAt }
						: null,
			},
		];
	} catch (err) {
...
async function fetchSpain() {
	try {
		...
		console.log(
			`  [ES] Gasoline=${avgGasoline} EUR/L, Diesel=${avgDiesel} EUR/L (${stations.length} stations)`,
		);
		return [
			{
				code: "ES",
...
async function fetchMexico() {
	try {
		...
		const regularPrices = latest
			.map((r) => parseFloat(r.precio_gasolina_regular))
			.filter((v) => !isNaN(v) && v > 0);
		const dieselPrices = latest
			.map((r) => parseFloat(r.precio_diesel))
			.filter((v) => !isNaN(v) && v > 0);
		...
		console.log(
			`  [MX] Regular=${avgRegular} MXN/L, Diesel=${avgDiesel} MXN/L (${latest.length} entries, date=${maxDate})`,
		);
		return [
			{
				code: "MX",
...
async function fetchUS_EIA() {
	try {
		...
		const gasolineUSDPerL =
			gasolineUSDPerGal != null ? +(gasolineUSDPerGal / GALLONS_TO_LITERS).toFixed(4) : null;
		const dieselUSDPerL =
			dieselUSDPerGal != null ? +(dieselUSDPerGal / GALLONS_TO_LITERS).toFixed(4) : null;
		const observedAt = rows[0]?.period ?? new Date().toISOString().slice(0, 10);

		console.log(
			`  [US] Gasoline=${gasolineUSDPerL} USD/L, Diesel=${dieselUSDPerL} USD/L (period=${observedAt})`,
		);
		return [
			{
				code: "US",
				...
				flag: "🇺🇸",
				gasoline:
					gasolineUSDPerL != null
						? {
								localPrice: gasolineUSDPerL,
								usdPrice: gasolineUSDPerL,
								grade: "Regular",
								source: "eia.gov",
								observedAt,
							}
				...
				diesel:
					dieselUSDPerL != null
						? {
								localPrice: dieselUSDPerL,
								usdPrice: dieselUSDPerL,
								grade: "Diesel",
								source: "eia.gov",
								observedAt,
							}
...
async function fetchEU_CSV() {
	try {
		...
		const sheetName =
			sheetNames.find((n) => /with.tax/i.test(n)) ??
		...
		if (headerRowIdx < 0) {
			console.warn(
				`  [EU] XLSX: no Euro-super column found. First 3 rows: ${rows
					.slice(0, 3)
		...
		for (let i = headerRowIdx + 2; i < rows.length; i++) {
			...
			euResults.push({
				...
				gasoline:
					gasPrice != null
						? { localPrice: gasPrice, grade: "E5", source: "energy.ec.europa.eu", observedAt }
						: null,
				diesel:
					dslPrice != null
						? { localPrice: dslPrice, grade: "Diesel", source: "energy.ec.europa.eu", observedAt }
						: null,
			});
		}
...
async function fetchBrazil() {
	...
	try {
		...
		if (gasResult.status === "rejected")
			console.warn(`  [BR] gas CSV failed: ${gasResult.reason.message}`);
		if (dslResult.status === "rejected")
			console.warn(`  [BR] dsl CSV failed: ${dslResult.reason.message}`);

		const gas =
			gasResult.status === "fulfilled"
				? nationalMean(gasResult.value, "GASOLINA", "valor de venda")
				: null;
		const dsl =
			dslResult.status === "fulfilled"
				? nationalMean(dslResult.value, "DIESEL", "valor de venda")
				: null;
		if (!gas && !dsl) return [];

		console.log(
			`  [BR] Gasoline=${gas?.avg} BRL/L (${gas?.date}), Diesel=${dsl?.avg} BRL/L (${dsl?.date})`,
		);
		return [
			{
				code: "BR",
				...
				gasoline:
					gas?.avg != null
						? { localPrice: gas.avg, grade: "Regular", source: "gov.br/anp", observedAt: gas.date }
						: null,
				diesel:
					dsl?.avg != null
						? { localPrice: dsl.avg, grade: "Diesel", source: "gov.br/anp", observedAt: dsl.date }
						: null,
			},
		];
	} catch (err) {
...
async function fetchNewZealand() {
	...
	try {
		...
		const gasPrice = gasRow
			? (parseFloat(gasRow[valIdx]) || null) && +(parseFloat(gasRow[valIdx]) / 100).toFixed(4)
			: null;
		const dslPrice = dslRow
			? (parseFloat(dslRow[valIdx]) || null) && +(parseFloat(dslRow[valIdx]) / 100).toFixed(4)
			: null;
		...
		return [
			{
				...
				diesel:
					dslPrice != null
						? { localPrice: dslPrice, grade: "Diesel", source: "mbie.govt.nz", observedAt: obsDate }
						: null,
			},
		];
	} catch (err) {
...
async function fetchUK_DESNZ() {
	...
	try {
		...
		const csvAttach = apiData?.details?.attachments?.find(
			(a) => a.content_type?.includes("csv") && a.title?.includes("2018"),
		);
		...
		return [
			{
				...
				gasoline:
					gasPrice != null
						? { localPrice: gasPrice, grade: "E10", source: "gov.uk/desnz", observedAt }
						: null,
				diesel:
					dslPrice != null
						? { localPrice: dslPrice, grade: "B7", source: "gov.uk/desnz", observedAt }
						: null,
			},
		];
	} catch (err) {
...
if (countries.length < MIN_COUNTRIES) {
	console.warn(
		`  [COVERAGE] Only ${countries.length} countries (min=${MIN_COUNTRIES}) — publishing anyway`,
	);
}
if (prevSnapshot?.countries?.length) {
	...
	if (dropPct > MAX_DROP_PCT) {
		console.warn(
			`  [COVERAGE] Drop: was ${prevCount}, now ${countries.length} (${dropPct.toFixed(1)}% drop) — publishing anyway`,
		);
	}
}

// Compute WoW per fuel entry
const prevAge = prevSnapshot?.fetchedAt
	? Date.now() - new Date(prevSnapshot.fetchedAt).getTime()
	: 0;
...
if (hasPrevData && prevTooRecent) {
	console.warn(
		`  [WoW] Skipping WoW — previous snapshot is only ${Math.round(prevAge / 3600000)}h old (need 144h+)`,
	);
}
...
if (wowAvailable) {
	...
	for (const country of countries) {
		...
		if (country.gasoline && prev.gasoline?.usdPrice > 0 && country.gasoline.usdPrice > 0) {
			const raw = +(
				((country.gasoline.usdPrice - prev.gasoline.usdPrice) / prev.gasoline.usdPrice) *
				100
			).toFixed(2);
			if (Math.abs(raw) > WOW_ANOMALY_THRESHOLD) {
				console.warn(
					`  [WoW] ANOMALY ${country.flag} ${country.name} gasoline: ${raw}% — omitting`,
				);
			} else {
		...
		if (country.diesel && prev.diesel?.usdPrice > 0 && country.diesel.usdPrice > 0) {
			const raw = +(
				((country.diesel.usdPrice - prev.diesel.usdPrice) / prev.diesel.usdPrice) *
				100
			).toFixed(2);
			if (Math.abs(raw) > WOW_ANOMALY_THRESHOLD) {
...
console.log(
	`  Most expensive gasoline: ${mostExpensiveGasoline}, Most expensive diesel: ${mostExpensiveDiesel}`,
);
