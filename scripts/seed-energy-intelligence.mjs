...
async function fetchFeed(feed) {
	try {
		...
		try {
			...
			console.warn(
				`[EnergyIntel] ${feed.label} direct failed (${directErr.message}), retrying via proxy`,
			);
...
async function fetchEnergyIntelligence() {
	...
	console.log(
		`[EnergyIntel] ${allItems.length} raw → ${recent.length} recent → ${relevant.length} relevant → ${deduped.length} deduped → ${limited.length} final`,
	);
