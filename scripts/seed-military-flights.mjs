...
import {
	summarizeMilitaryTheaters,
	buildMilitarySurges,
	appendMilitaryHistory,
} from "./_military-surges.mjs";
...
const TRUSTED_HEX_OPERATORS = new Set([
	"usaf",
	"raf",
	"faf",
	"gaf",
...
function summarizeRawSourcePreview(sourceMeta = {}) {
	...
	return Object.fromEntries(Object.entries(preview).filter(([, value]) => Boolean(value)));
}
...
function recordSourceCoverage(
	...
	if (
		stageCounters.sourceShapeSamples.length < 5 &&
		((sourceMeta.rawKeys || []).length > 0 || Object.keys(rawPreview).length > 0)
	) {
...
function deriveSourceHints(sourceMeta = {}) {
	...
	return {
		hintText,
		militaryHint:
			/(AIR FORCE|AIR ?SELF ?DEFEN[CS]E|MILIT|NAVY|MARINE|ARMY|DEFEN[CS]E|SQUADRON|\bUSAF\b|\bUSN\b|\bUSMC\b|\bRAF\b|\bRCAF\b|\bRAAF\b|NATO|\bPLAAF\b|\bPLAN\b|\bVKS\b|RECON|AWACS|TANKER|AIRLIFT|FIGHTER|BOMBER|DRONE)/.test(
				hintText,
			),
		militaryOperatorHint:
			/(AIR FORCE|AIR ?SELF ?DEFEN[CS]E|NAVY|MARINE|ARMY|DEFEN[CS]E|SQUADRON|EMIRI AIR FORCE|ROYAL .* AIR FORCE|AEROSPACE FORCES|\bPLAAF\b|\bPLAN\b|NATO)/.test(
				hintText,
			),
		commercialHint:
			/(AIRLINES|AIRWAYS|LOGISTICS|EXPRESS|CARGOLUX|TURKISH AIRLINES|ETHIOPIAN AIRLINES|QATAR AIRWAYS|EMIRATES SKYCARGO|SAUDIA)/.test(
				hintText,
			),
	};
}
...
function detectAircraftTypeFromSourceMeta(sourceMeta = {}) {
	...
	if (
		/(C-17|C17|C-130|C130|C-2|C2|C-27|C27|A400M|IL-76|IL76|Y-20|Y20|TRANSPORT|AIRLIFT|CARGO)/.test(
			hintText,
		)
	)
		return "transport";
	if (/(RC-135|RC135|RECON|SURVEILLANCE|SIGINT|ELINT|ISR|U-2|P-8|P8|P-3|P3|PATROL)/.test(hintText))
	...
	if (
		/(F-16|F16|F-15|F15|F-18|F18|F-22|F22|F-35|F35|J-10|J10|J-11|J11|J-16|J16|SU-27|SU27|SU-30|SU30|SU-35|SU35|MIG-29|MIG29|FIGHTER)/.test(
			hintText,
		)
	)
		return "fighter";
...
function deriveOperatorFromSourceMeta(sourceMeta = {}) {
	...
	if (/PEOPLE'?S LIBERATION ARMY AIR FORCE|\bPLAAF\b|CHINESE AIR FORCE/.test(hintText))
		return {
			operator: "plaaf",
	...
	if (/PEOPLE'?S LIBERATION ARMY NAVY|\bPLAN\b/.test(hintText))
		return {
			operator: "plan",
			operatorCountry: "China",
			reason: "source_operator",
	...
	if (/UNITED STATES AIR FORCE|US AIR FORCE|\bUSAF\b/.test(hintText))
		return {
			operator: "usaf",
			operatorCountry: "USA",
			reason: "source_operator",
	...
	if (/UNITED STATES NAVY|US NAVY|\bUSN\b/.test(hintText))
		return {
			operator: "usn",
			operatorCountry: "USA",
			reason: "source_operator",
	...
	if (/UNITED STATES MARINE CORPS|US MARINE|\bUSMC\b/.test(hintText))
		return {
			operator: "usmc",
			operatorCountry: "USA",
			reason: "source_operator",
	...
	if (/UNITED STATES ARMY|US ARMY/.test(hintText))
		return {
			operator: "usa",
			operatorCountry: "USA",
			reason: "source_operator",
	...
	if (/ROYAL AIR FORCE|\bRAF\b/.test(hintText))
		return {
			operator: "raf",
			operatorCountry: "UK",
			reason: "source_operator",
	...
	if (/ROYAL NAVY/.test(hintText))
		return { operator: "rn", operatorCountry: "UK", reason: "source_operator", confidence: "high" };
	if (/FRENCH AIR FORCE|ARMEE DE L'?AIR|ARMÉE DE L'?AIR|\bFAF\b/.test(hintText))
		return {
			operator: "faf",
	...
	if (/GERMAN AIR FORCE|LUFTWAFFE|\bGAF\b/.test(hintText))
		return {
			operator: "gaf",
	...
	if (/ISRAELI AIR FORCE|\bIAF\b/.test(hintText))
		return {
			operator: "iaf",
	...
	if (/NATO/.test(hintText))
		return {
			operator: "nato",
			operatorCountry: "NATO",
			reason: "source_operator",
			confidence: "high",
		};
	if (/QATAR EMIRI AIR FORCE|\bQEAF\b/.test(hintText))
		return {
			operator: "qeaf",
	...
	if (/ROYAL SAUDI AIR FORCE|\bRSAF\b/.test(hintText))
		return {
			operator: "rsaf",
			operatorCountry: "Saudi Arabia",
			reason: "source_operator",
			confidence: "high",
		};
	if (/TURKISH AIR FORCE|\bTURAF\b|\bTRKAF\b/.test(hintText))
		return {
			operator: "turaf",
	...
	if (/UNITED ARAB EMIRATES AIR FORCE|UAE AIR FORCE|EMIRATI AIR FORCE/.test(hintText))
		return {
			operator: "uaeaf",
			operatorCountry: "UAE",
			reason: "source_operator",
	...
	if (/KUWAIT AIR FORCE/.test(hintText))
		return {
			operator: "kuwaf",
			operatorCountry: "Kuwait",
			reason: "source_operator",
			confidence: "high",
		};
	if (/EGYPTIAN AIR FORCE/.test(hintText))
		return {
			operator: "egyaf",
	...
	if (/PAKISTAN AIR FORCE|\bPAF\b/.test(hintText))
		return {
			operator: "paf",
			operatorCountry: "Pakistan",
			reason: "source_operator",
			confidence: "high",
		};
	if (/\bJASDF\b|JAPAN AIR SELF DEFENSE FORCE/.test(hintText))
		return {
			operator: "jasdf",
			operatorCountry: "Japan",
			reason: "source_operator",
			confidence: "high",
		};
	if (/\bROKAF\b|REPUBLIC OF KOREA AIR FORCE/.test(hintText))
		return {
			operator: "rokaf",
			operatorCountry: "South Korea",
			reason: "source_operator",
			confidence: "high",
		};
	if (/RUSSIAN AEROSPACE FORCES|\bVKS\b/.test(hintText))
		return {
			operator: "vks",
	...
	if (/ROYAL AUSTRALIAN AIR FORCE|\bRAAF\b/.test(hintText))
		return {
			operator: "raaf",
	...
	if (/ROYAL CANADIAN AIR FORCE|\bRCAF\b|CANADIAN ARMED FORCES/.test(hintText))
		return {
			operator: "rcaf",
...
async function proxyFetchJson(
	url,
	{ headers = {}, timeout = 15000, method = "GET", body = null } = {},
) {
...
async function fetchJsonDirect(
	url,
	{ headers = {}, method = "GET", body = null, timeout = 15_000 } = {},
) {
			...
			try {
				...
				try {
					...
					try {
						...
						throw new Error(
							`direct=${redactProxy(directError.message)} | proxy=${redactProxy(proxyError.message)}`,
						);
					}
				}
...
async function fetchOpenSkyAnonymous(region) {
	...
	try {
		...
		try {
			...
			throw new Error(
				`error:direct=${redactProxy(directError.message)} | proxy=${redactProxy(proxyError.message)}`,
			);
		}
	}
}
...
async function fetchOpenSkyRegion(region, { source, fetchSources, seenIds, allStates }) {
	...
	if (states) {
		...
		if (added > 0)
			console.log(`  [OpenSky] +${added} new from ${region.name} (total: ${allStates.length})`);
	}
...
async function fetchWingbits() {
	...
	console.log(
		`  [Wingbits] POST ${WINGBITS_BASE} with ${areas.length} areas: ${areas.map((a) => `${a.alias}(${a.w}x${a.h}nm)`).join(", ")}`,
	);
	...
	if (!Array.isArray(data)) {
		console.warn(
			`  [Wingbits] Unexpected response shape: ${typeof data}, keys: ${Object.keys(data || {}).join(",")}`,
		);
		return [];
	}
	...
	for (let i = 0; i < data.length; i++) {
		const ar = data[i];
		const flightList = Array.isArray(ar.data)
			? ar.data
			: Array.isArray(ar.flights)
				? ar.flights
				: Array.isArray(ar)
					? ar
					: [];
		console.log(
			`  [Wingbits]   area[${i}] ${ar.alias || areas[i]?.alias || "?"}: ${flightList.length} flights, keys: ${Object.keys(ar || {}).join(",")}`,
		);
...
function summarizeClassificationAudit(rawStates, flights, rejected, stageCounters) {
	...
	for (const flight of flights) {
		...
		if (
			flight.aircraftTypeInferenceReason === "callsign_pattern" ||
			flight.classificationReason === "callsign_pattern"
		)
			typedByCallsign += 1;
		if (
			flight.aircraftTypeInferenceReason === "source_metadata" ||
			flight.operatorInferenceReason === "source_metadata" ||
			flight.classificationReason === "source_metadata"
		)
			typedBySource += 1;
	...
	return {
		...
		sourceCoverage: {
			...Object.fromEntries(
				Object.entries(stageCounters.sourceFieldCoverage).map(([field, count]) => [
					`${field}Present`,
					count,
				]),
			),
...
function classifyCallsignMatchedFlight({ csMatch, hexMatch, callsign, sourceMeta }) {
	...
	const operator =
		csMatch.operator === "other" && sourceOperator?.operator
			? sourceOperator.operator
			: csMatch.operator;
	const operatorCountry =
		csMatch.operator === "other" && sourceOperator?.operatorCountry
	...
	const operatorInferenceReason =
		operator !== csMatch.operator ? "source_metadata" : "callsign_pattern";
...
function classifyHexMatchedFlight({
	...
	if (
		!trustedHex &&
		(!sourceHints.militaryHint || (sourceHints.commercialHint && !sourceHints.militaryOperatorHint))
	) {
...
function buildMilitaryFlightRecord(state, classified, sourceHints) {
	...
	const isInteresting =
		...
		classified.aircraftType === "bomber" ||
		classified.aircraftType === "reconnaissance" ||
		classified.aircraftType === "awacs";
...
function filterMilitaryFlights(allStates) {
	...
	for (const state of allStates) {
		...
		const flight = buildMilitaryFlightRecord(
			state,
			{
				...classified,
...
async function main() {
	...
	const lockResult = await acquireLockSafely("military:flights", runId, 120_000, {
	...
	try {
		...
		console.log(
			`  Military: ${flights.length} (${Object.entries(byType)
				.map(([t, n]) => `${t}:${n}`)
				.join(", ")})`,
		);
		if (classificationAudit) {
			console.log(
				`  [Audit] unknownRate=${classificationAudit.unknownTypeRate} hexOnly=${classificationAudit.hexOnlyAdmissions} rejected=${classificationAudit.rejectedFlights}`,
			);
			console.log(
	...
	try {
		...
		await redisSet(
			...
			token,
			MILITARY_CLASSIFICATION_AUDIT_LIVE_KEY,
			{ fetchedAt: assessedAt, sourceVersion: source || "", ...classificationAudit },
			MILITARY_CLASSIFICATION_AUDIT_LIVE_TTL,
		);
		await redisSet(
			url,
			token,
			MILITARY_CLASSIFICATION_AUDIT_STALE_KEY,
			{ fetchedAt: assessedAt, sourceVersion: source || "", ...classificationAudit },
			MILITARY_CLASSIFICATION_AUDIT_STALE_TTL,
		);
		...
		await redisSet(
			...
			THEATER_POSTURE_STALE_KEY,
			posturePayload,
			THEATER_POSTURE_STALE_TTL,
		);
		await redisSet(
			url,
			token,
			THEATER_POSTURE_BACKUP_KEY,
			posturePayload,
			THEATER_POSTURE_BACKUP_TTL,
		);
		await redisSet(
			url,
			token,
			"seed-meta:theater-posture",
			{ fetchedAt: assessedAt, recordCount: theaterFlights.length, sourceVersion: source || "" },
			604800,
		);
		...
		const priorSurgeHistory =
			(await redisGet(url, token, MILITARY_SURGES_HISTORY_KEY))?.history || [];
		const theaterActivity = summarizeMilitaryTheaters(flights, POSTURE_THEATERS, assessedAt);
		const surges = buildMilitarySurges(theaterActivity, priorSurgeHistory, {
			sourceVersion: source || "",
		});
		const surgePayload = {
			surges,
		...
		const surgeHistory = appendMilitaryHistory(
			priorSurgeHistory,
			{
				assessedAt,
				sourceVersion: source || "",
		...
		await redisSet(
			...
			token,
			MILITARY_FORECAST_INPUTS_LIVE_KEY,
			forecastInputsPayload,
			MILITARY_FORECAST_INPUTS_LIVE_TTL,
		);
		await redisSet(
			url,
			token,
			MILITARY_FORECAST_INPUTS_STALE_KEY,
			forecastInputsPayload,
			MILITARY_FORECAST_INPUTS_STALE_TTL,
		);
		...
		await redisSet(
			url,
			token,
			MILITARY_SURGES_HISTORY_KEY,
			{ history: surgeHistory },
			MILITARY_SURGES_HISTORY_TTL,
		);
		await redisSet(
			url,
			token,
		...
		console.log(
			`  Military surges: ${surges.length} detected (history: ${surgeHistory.length} runs)`,
		);
		await releaseLock("military:flights", runId);
		lockReleased = true;
		try {
			...
			console.warn(
				`  Forecast refresh request failed after military publish: ${err.message || err}`,
			);
		}
