...
async function fetchEuGasStorage() {
	...
	if (!apiKey) {
		console.warn(
			"  WARNING: GIE_API_KEY / AGSI_API_KEY not set — attempting unauthenticated request",
		);
	}
	...
	const gasDaysConsumption = current.gwh > 0 ? +(current.gwh / 18).toFixed(1) : 0;
	...
	console.log(
		`  EU gas storage: fill=${result.fillPct}%, change1d=${result.fillPctChange1d}, trend=${result.trend}`,
	);
	return result;
}
