...
function normalizeTheater(raw) {
	...
	return (
		String(raw)
			.trim()
			.replace(/\b\w/g, (c) => c.toUpperCase()) || "Global"
	);
}
...
async function readAllSourceKeys() {
	...
	for (let i = 0; i < SOURCE_KEYS.length; i++) {
		...
		if (!raw) continue;
		try {
			data[SOURCE_KEYS[i]] = JSON.parse(raw);
		} catch {
			/* skip malformed */
		}
	}
...
function extractThermalSpike(d) {
	...
	for (const c of spikes.slice(0, 5)) {
		...
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_THERMAL_SPIKE"] *
			Math.min(3, safeNum(c.anomalyScore) || 1.5);
		signals.push({
...
function extractGpsJamming(d) {
	...
	const highHexes = hexes.filter(
		(h) => h.level === "high" || (safeNum(h.npAvg) < 0.5 && safeNum(h.npAvg) > 0),
	);
...
function extractMilitaryFlightSurge(d) {
	...
	for (const [theater, count] of regionMap) {
		if (count < 3) continue;
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_MILITARY_FLIGHT_SURGE"] * Math.min(2, 1 + count / 20);
		signals.push({
...
function extractUnrestSurge(d) {
	...
	const recent = events.filter(
		(e) => safeNum(e.date || e.timestamp || e.created_at) > cutoff || !e.date,
	);
	const regionMap = new Map();
	...
	for (const [theater, count] of regionMap) {
		if (count < 3) continue;
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_UNREST_SURGE"] * Math.min(2, 1 + count / 10);
		signals.push({
...
function extractOrefAlertCluster(d) {
	...
	const critical = advisories.filter(
		(a) => String(a.level || "").toLowerCase() === "do not travel",
	);
...
function extractVixSpike(d) {
	...
	const score =
		BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_VIX_SPIKE"] * (vixVal > 40 ? 2 : vixVal > 30 ? 1.5 : 1.2);
	return [
		{
...
function extractCommodityShock(d) {
	...
	for (const q of quotes) {
		...
		const theater =
			q.symbol === "OIL" || q.symbol === "CL=F" || q.display?.includes("Oil")
				? "Persian Gulf"
				: "Global Markets";
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_COMMODITY_SHOCK"] * Math.min(2, Math.abs(change) / 5);
		signals.push({
...
function extractCyberEscalation(d) {
	...
	if (!payload) return [];
	const threats = Array.isArray(payload.threats)
		? payload.threats
		: Array.isArray(payload)
			? payload
			: [];
	...
	for (const [theater, count] of regionMap) {
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_CYBER_ESCALATION"] * Math.min(2, 1 + count / 5);
		signals.push({
...
function extractShippingDisruption(d) {
	...
	const disrupted = routes.filter(
		(r) => r.disrupted || r.status === "disrupted" || safeNum(r.rerouting) > 10,
	);
	if (disrupted.length === 0) return [];
	const theater = disrupted.some(
		(r) =>
			String(r.name || "")
				.toLowerCase()
				.includes("red sea") ||
		...
		: "Global";
	const score =
		BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_SHIPPING_DISRUPTION"] *
		Math.min(2, 1 + disrupted.length / 3);
	return [
		{
...
function extractSanctionsSurge(d) {
	...
	const score =
		BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_SANCTIONS_SURGE"] * Math.min(2, 1 + newCount / 20);
	return [
		{
...
function extractEarthquakeSignificant(d) {
	...
	if (!payload) return [];
	const quakes = Array.isArray(payload.earthquakes)
		? payload.earthquakes
		: Array.isArray(payload)
			? payload
			: [];
	...
	return significant.slice(0, 2).map((q) => {
		...
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_EARTHQUAKE_SIGNIFICANT"] *
			(mag >= 7.5 ? 2 : mag >= 7.0 ? 1.5 : 1.2);
		return {
...
function extractRadiationAnomaly(d) {
	...
	const anomalies = observations.filter(
		(o) => o.alert || o.status === "alert" || safeNum(o.value) > safeNum(o.threshold) * 1.5,
	);
	if (anomalies.length === 0) return [];
	return anomalies.slice(0, 2).map((a) => {
		const locationStr =
			a.locationName || a.stationName || a.country || a.region || "unknown station";
...
function extractInfrastructureOutage(d) {
	...
	if (!payload) return [];
	const outages = Array.isArray(payload.outages)
		? payload.outages
		: Array.isArray(payload)
			? payload
			: [];
	const major = outages.filter(
		(o) => o.severity === "major" || o.severity === "critical" || safeNum(o.affectedUsers) > 100000,
	);
	...
	for (const [theater, count] of regionMap) {
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_INFRASTRUCTURE_OUTAGE"] * Math.min(2, 1 + count / 3);
		signals.push({
...
function extractWildfireEscalation(d) {
	...
	if (!payload) return [];
	const fires = Array.isArray(payload.fires)
		? payload.fires
		: Array.isArray(payload)
			? payload
			: [];
	const extreme = fires.filter(
		(f) => f.radiativePower > 5000 || f.severity === "extreme" || safeNum(f.brightness) > 400,
	);
	...
	for (const [theater, count] of regionMap) {
		if (count < 5) continue;
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_WILDFIRE_ESCALATION"] * Math.min(2, 1 + count / 50);
		signals.push({
...
function extractDisplacementSurge(d) {
	...
	if (!payload) return [];
	const crises = Array.isArray(payload.crises)
		? payload.crises
		: Array.isArray(payload)
			? payload
			: [];
	...
	return surges.slice(0, 2).map((c) => {
		...
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_DISPLACEMENT_SURGE"] *
			Math.min(2, 1 + safeNum(c.newDisplacements) / 100000);
		return {
...
function extractForecastDeterioration(d) {
	...
	const predictions = Array.isArray(payload.predictions)
		...
		: Array.isArray(payload)
			? payload
			: [];
	const deteriorating = predictions.filter(
		(p) =>
			p.trend === "deteriorating" || p.direction === "negative" || safeNum(p.probability) > 0.65,
	);
	...
	return deteriorating.slice(0, 2).map((p) => {
		...
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_FORECAST_DETERIORATION"] *
			Math.min(2, 1 + safeNum(p.probability));
		return {
...
function extractMarketStress(d) {
	...
	if (!payload) return [];
	const quotes = Array.isArray(payload.quotes) ? payload.quotes : [];
	const spx = quotes.find(
		(q) => q.symbol === "^GSPC" || q.symbol === "SPX" || q.display === "S&P 500",
	);
	if (!spx) return [];
	const change = safeNum(spx.change);
	if (Math.abs(change) < 2) return [];
	const score =
		BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_MARKET_STRESS"] * Math.min(2, Math.abs(change) / 2);
	return [
		{
...
function extractWeatherExtreme(d) {
	...
	for (const [theater, count] of regionMap) {
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_WEATHER_EXTREME"] * Math.min(2, 1 + count / 5);
		signals.push({
...
function extractMediaToneDeterioration(d) {
	...
	for (const topic of GDELT_TONE_TOPICS) {
		...
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_MEDIA_TONE_DETERIORATION"] *
			Math.min(2, Math.abs(finalVal) / 3);
		signals.push({
			id: `gdelt-tone:${topic}`,
...
function extractRiskScoreSpike(d) {
	...
	const spiking = ciiScores.filter(
		(s) => safeNum(s.combinedScore) > 80 || s.trend === "TREND_DIRECTION_RISING",
	);
	if (spiking.length === 0) return [];
	return spiking.slice(0, 3).map((s) => {
		const theater = normalizeTheater(s.region || "");
		const score =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_RISK_SCORE_SPIKE"] *
			Math.min(2, safeNum(s.combinedScore) / 60);
		return {
...
function extractRegulatoryAction(d) {
	...
	const recent = actions
		...
		.filter(
			({ action, publishedAtTs }) =>
				(action.tier === "high" || action.tier === "medium") && publishedAtTs > cutoff,
		)
		.sort((a, b) => {
	...
	for (const [theater, categoryMap] of theaterMap) {
		...
		for (const [, categorySigs] of categoryMap) {
			for (const s of categorySigs) {
				contributingTypes.push(
					s.type.replace("CROSS_SOURCE_SIGNAL_TYPE_", "").replace(/_/g, " ").toLowerCase(),
				);
		...
		const compositeScore =
			BASE_WEIGHT["CROSS_SOURCE_SIGNAL_TYPE_COMPOSITE_ESCALATION"] *
				Math.min(3, 1 + categoryMap.size / 3) +
			totalScore * 0.2;
		composites.push({
...
async function aggregateCrossSourceSignals() {
	...
	const sortedSignals = allSignals.sort(
		(a, b) => b.severityScore - a.severityScore || b.detectedAt - a.detectedAt,
	);
