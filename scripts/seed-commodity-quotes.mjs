	...
	if (avKey) {
		...
		for (const sym of physicalSymbols) {
			...
			if (q) {
				...
				console.log(
					`  [AV:physical] ${sym}: $${q.price} (${q.change > 0 ? "+" : ""}${q.change.toFixed(2)}%)`,
				);
			}
		}

		// REALTIME_BULK_QUOTES for ETF-style symbols (URA, LIT)
		const bulkCandidates = COMMODITY_SYMBOLS.filter(
			(s) =>
				!AV_PHYSICAL_MAP[s] &&
		...
		for (const [sym, q] of bulkResults) {
			...
			quotes.push({
				symbol: sym,
				name: meta?.name || sym,
				display: meta?.display || sym,
				price: q.price,
				change: q.change,
				sparkline: [],
			});
			console.log(
				`  [AV:bulk] ${sym}: $${q.price} (${q.change > 0 ? "+" : ""}${q.change.toFixed(2)}%)`,
			);
		}
	}
	...
	for (let i = 0; i < COMMODITY_SYMBOLS.length; i++) {
		...
		try {
			...
			if (parsed) {
				...
				covered.add(symbol);
				console.log(
					`  [Yahoo] ${symbol}: $${parsed.price} (${parsed.change > 0 ? "+" : ""}${parsed.change}%)`,
				);
			} else {
				misses++;
			}
