...
function validate(data) {
	...
	const countriesWithMetrics = Object.values(countries).filter((c) => Object.keys(c).length >= 1);
	if (countriesWithMetrics.length < 5) {
		console.warn(
			`  Validation failed: only ${countriesWithMetrics.length} countries with data (need ≥5)`,
		);
		return false;
	}
	return true;
