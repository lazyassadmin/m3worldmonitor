...
export function extractCountryData(allRows, iso2) {
	...
	for (const month of sortedMonths) {
		...
		const hasValidCode = monthRows.some(
			(r) => r.ASSESSMENT_CODE === "1" || r.ASSESSMENT_CODE === "2",
		);
		...
		const hasSecondaryData = monthRows.some(
			(r) =>
				(r.ASSESSMENT_CODE === "1" || r.ASSESSMENT_CODE === "2") &&
				r.ENERGY_PRODUCT in SECONDARY_PRODUCTS,
		);
		if (hasSecondaryData) {
			dataMonth = month;
			break;
	...
	function pickVal(product, flow, isAnomalyCapped) {
		const r = monthRows.find(
			(row) => row.ENERGY_PRODUCT === product && row.FLOW_BREAKDOWN === flow,
		);
		if (!r) return null;
		...
		if (isAnomalyCapped && iso2 !== "US" && flow === "TOTDEMO" && val > ANOMALY_DEMAND_KBD)
			return null;
		return val;
	}
...
export function buildAllCountries(allRows) {
	const countries = new Set(
		allRows.filter((r) => r.REF_AREA && r.UNIT_MEASURE === "KBD").map((r) => r.REF_AREA),
	);
	const results = [];
...
export function mergeSourceRows(primaryCurrent, primaryPrior, secondaryCurrent, secondaryPrior) {
	if (!secondaryCurrent && !secondaryPrior) {
		throw new Error(
			"Both secondary JODI CSV files failed to download; product-level data unavailable",
		);
	}
	const allRows = [
...
async function fetchAllRows() {
	...
	const [primaryCurrent, primaryPrior, secondaryCurrent, secondaryPrior] = await Promise.all([
		withRetry(() => fetchCsv(`${JODI_BASE}primary/${currentYear}.csv`), 2, 2000).catch((e) => {
			console.warn(`  primary/${currentYear}.csv failed: ${e.message}`);
			return "";
		}),
		withRetry(() => fetchCsv(`${JODI_BASE}primary/${priorYear}.csv`), 2, 2000).catch((e) => {
			console.warn(`  primary/${priorYear}.csv failed: ${e.message}`);
			return "";
		}),
		withRetry(() => fetchCsv(`${JODI_BASE}secondary/${currentYear}.csv`), 2, 2000).catch((e) => {
			console.warn(`  secondary/${currentYear}.csv failed: ${e.message}`);
			return "";
		}),
		withRetry(() => fetchCsv(`${JODI_BASE}secondary/${priorYear}.csv`), 2, 2000).catch((e) => {
			console.warn(`  secondary/${priorYear}.csv failed: ${e.message}`);
			return "";
		}),
	]);
...
async function main() {
	...
	try {
		...
		if (!validateCoverage(countries)) {
			console.error(
				`  COVERAGE GATE FAILED: only ${countries.length} countries, need >=${MIN_VALID_COUNTRIES}`,
			);
		...
		for (const payload of countries) {
			commands.push([
				"SET",
				`${COUNTRY_KEY_PREFIX}${payload.iso2}`,
