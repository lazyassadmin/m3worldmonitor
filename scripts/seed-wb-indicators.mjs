...
async function fetchWithRetry(url, attempt = 1) {
	try {
		...
		if (attempt < MAX_RETRIES) {
			...
			console.warn(
				`  Retry ${attempt}/${MAX_RETRIES} for ${url} in ${delay}ms... (${err.message})`,
			);
			await sleep(delay);
...
async function main() {
	...
	console.log(
		`  Top 5: ${rankings
			.slice(0, 5)
			.map((r) => `${r.rank}. ${r.countryName} (${r.score})`)
			.join(", ")}\n`,
	);
	...
	try {
		...
		if (priorMeta && typeof priorMeta.recordCount === "number" && priorMeta.recordCount > 0) {
			if (rankings.length < priorMeta.recordCount * 0.5) {
				console.warn(
					`Rankings dropped >50%: ${rankings.length} vs prior ${priorMeta.recordCount} — extending TTLs instead of overwriting.`,
				);
	...
	const pipeline = [
		...
		[
			"SET",
			`seed-meta:${BOOTSTRAP_KEY}`,
			JSON.stringify({ fetchedAt: Date.now(), recordCount: rankings.length }),
			"EX",
			metaTtl,
		],
	];
	if (progressWithData.length > 0) {
		...
		pipeline.push([
			"SET",
			`seed-meta:${PROGRESS_KEY}`,
			JSON.stringify({ fetchedAt: Date.now(), recordCount: progressWithData.length }),
			"EX",
			metaTtl,
		]);
	}
	if (renewableData.historicalData.length > 0) {
		...
		pipeline.push([
			"SET",
			`seed-meta:${RENEWABLE_KEY}`,
			JSON.stringify({ fetchedAt: Date.now(), recordCount: renewableData.historicalData.length }),
			"EX",
			metaTtl,
		]);
	}
	...
	if (verifyResp[2]?.result) {
		...
		console.log(
			`  ✓ renewableEnergy: ${r.regions?.length || 0} regions, global=${r.globalPercentage}%`,
		);
	}
