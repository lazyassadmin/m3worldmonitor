	...
	return parseNoaaRows(text)
		...
		.filter(
			(row) =>
				...
				isValidMeasurement(row.average),
		)
...
function findClosestPriorYearValue(rows, latest) {
	const exact = rows.find(
		(row) => row.year === latest.year - 1 && row.month === latest.month && row.day === latest.day,
	);
	...
	const closest = candidates.reduce((best, row) => {
		...
		if (
			rowDelta === bestDelta &&
			toEpochMs(row.year, row.month, row.day) < toEpochMs(best.year, best.month, best.day)
		) {
			return row;
		}
		return best;
...
export function buildTrend12m(monthlyRows) {
	const byMonth = new Map(
		monthlyRows.map((row) => [formatMonth(row.year, row.month), row.average]),
	);
...
function findMonthlyAverageForLatestDaily(monthlyRows, latestDaily) {
	const exact = monthlyRows.findLast(
		(row) => row.year === latestDaily.year && row.month === latestDaily.month,
	);
	if (exact) return exact.average;
...
export function buildCo2MonitoringPayload({
	dailyRows,
	monthlyRows,
	annualRows,
	methaneRows,
	...
	const monthlyAverage = latestDaily
		? findMonthlyAverageForLatestDaily(monthlyRows, latestDaily)
		: 0;
