		...
		for (const meta of GULF_SYMBOLS) {
			if (meta.type === "oil") {
				...
				if (q) {
					...
					console.log(
						`  [AV:physical] ${meta.symbol}: $${q.price} (${q.change > 0 ? "+" : ""}${q.change.toFixed(2)}%)`,
					);
				}
				...
				if (q) {
					...
					covered.add(meta.symbol);
					console.log(
						`  [AV:fx] ${meta.symbol}: ${q.price} (${q.change > 0 ? "+" : ""}${q.change.toFixed(2)}%)`,
					);
				}
			}
	...
	for (let i = 0; i < GULF_SYMBOLS.length; i++) {
		...
		try {
			...
			if (parsed) {
				...
				covered.add(meta.symbol);
				console.log(
					`  [Yahoo] ${meta.symbol}: $${parsed.price} (${parsed.change > 0 ? "+" : ""}${parsed.change}%)`,
				);
			} else {
				misses++;
			}
