...
const EUROSTAT_ENERGY_URL =
	"https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nrg_ind_id?freq=A";
...
async function fetchText(
	url,
	{ accept = "text/plain, text/html, application/json", timeoutMs = 30_000 } = {},
) {
	try {
...
async function fetchWhoIndicatorRows(indicatorCode) {
	...
	while (nextUrl && pageCount < MAX_WHO_PAGES) {
		...
		if (!Array.isArray(payload?.value))
			throw new Error(`Unexpected WHO response shape for ${indicatorCode}`);
...
export async function fetchRsfDataset() {
	...
	if (parsed.size < 100)
		throw new Error(`RSF ranking page returned only ${parsed.size} countries (expected 180+)`);
	return parsed;
}
...
function parseLatestEurostatValue(data, geoCode) {
	...
	for (const key of Object.keys(values).sort((left, right) => Number(right) - Number(left))) {
		...
		const matchedTime = Object.entries(timeIndexObj || {}).find(
			([, position]) => position === coords.time,
		);
...
export async function fetchEnergyDependencyDataset() {
	...
	if (merged.size === 0)
		throw new Error("Energy dependency: both Eurostat and World Bank fallback failed");
	return merged;
}
...
export function parseGpiRows(csvText, resolvedYear) {
	...
	if (parsed.size < 50)
		throw new Error(`GPI CSV returned only ${parsed.size} countries (expected 163+)`);
	return parsed;
}
...
export function parseFsinRows(csvText) {
	...
	const parsed = new Map();
	for (const row of rows) {
		const rawIso3 = String(row["Country (ISO3)"] || row["Country"] || "")
			.trim()
			.toUpperCase();
...
async function fetchFsinDataset() {
	...
	const { text: csvText } = await withRetry(
		() => fetchText(hdxUrl, { accept: "text/csv" }),
		2,
		750,
	);
...
export function finalizeCountryPayloads(
	datasetMaps,
	seedYear = nowSeedYear(),
...
async function publishSuccess(countryPayloads, manifest, meta) {
	...
	for (const [iso2, payload] of countryPayloads.entries()) {
		commands.push([
			"SET",
			countryRedisKey(iso2),
			JSON.stringify(payload),
			"EX",
			RESILIENCE_STATIC_TTL_SECONDS,
		]);
	}
	commands.push([
		"SET",
...
async function fetchAllDatasetMaps() {
	...
	for (let idx = 0; idx < adapters.length; idx += 1) {
		...
		if (result.status === "fulfilled") {
			...
			console.warn(
				`  ${adapter.key}: ${result.reason?.message || result.reason || "unknown error"}`,
			);
		}
	}
...
export async function recoverFailedDatasets(
	datasetMaps,
	failedDatasets,
	{ readIndex, readPipeline },
) {
	...
	try {
		...
		throw new Error(
			`Dataset(s) (${failedDatasets.join(", ")}) failed and Redis index read also failed: ${err.message}`,
		);
	}
	...
	if (existingCountries.length === 0) {
		console.warn(
			`  [fallback] dataset(s) failed (${failedDatasets.join(", ")}) — no prior snapshot to recover from`,
		);
		return;
	}
	...
	let pipelineResults;
	try {
		pipelineResults = await readPipeline(
			existingCountries.map((iso2) => ["GET", countryRedisKey(iso2)]),
		);
	} catch (err) {
		throw new Error(
			`Dataset(s) (${failedDatasets.join(", ")}) failed and Redis pipeline read also failed: ${err.message}`,
		);
	}
	...
	for (let i = 0; i < existingCountries.length; i++) {
		...
		let existing = null;
		try {
			existing = JSON.parse(pipelineResults[i]?.result ?? "null");
		} catch {
			/* skip */
		}
	...
	for (const key of failedDatasets) {
		console.warn(
			`  [fallback] dataset '${key}' failed — preserved ${datasetMaps[key].size} existing Redis records from prior snapshot`,
		);
	}
}
