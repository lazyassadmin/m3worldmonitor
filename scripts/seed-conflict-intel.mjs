	...
	for (const r of records) {
		...
		if (month === agg.month) {
			...
			if (eventType.includes("political_violence")) {
				agg.eventsPV += events;
				agg.fatPV += fatalities;
			}
			if (eventType.includes("civilian_targeting")) {
				agg.eventsCT += events;
				agg.fatCT += fatalities;
			}
...
async function fetchAllHumanitarianSummaries() {
	...
	console.log(
		`  Humanitarian: ${Object.keys(results).length}/${CONFLICT_COUNTRIES.length} countries`,
	);
	return results;
}
...
async function fetchPizzintStatus() {
	...
	const avgPop =
		open.length > 0 ? open.reduce((s, l) => s + l.currentPopularity, 0) / open.length : 0;
...
async function fetchGdeltTensions() {
	...
	const resp = await fetch(
		`https://www.pizzint.watch/api/gdelt/batch?pairs=${encodeURIComponent(pairs)}&method=gpr`,
		{
			headers: { Accept: "application/json", "User-Agent": CHROME_UA },
			signal: AbortSignal.timeout(10_000),
		},
...
async function fetchAll() {
	...
	if (acled.status === "rejected")
		console.warn(`  ACLED failed: ${acled.reason?.message || acled.reason}`);
	if (hapi.status === "rejected")
		console.warn(`  HAPI failed: ${hapi.reason?.message || hapi.reason}`);
	if (pizzint.status === "rejected")
		console.warn(`  PizzINT failed: ${pizzint.reason?.message || pizzint.reason}`);
	if (gdelt.status === "rejected")
		console.warn(`  GDELT failed: ${gdelt.reason?.message || gdelt.reason}`);
	...
	if (ha) {
		for (const [cc, data] of Object.entries(ha))
			await writeExtraKeyWithMeta(`${HAPI_CACHE_KEY_PREFIX}:${cc}`, data, HAPI_TTL, 1);
	}
	if (pi)
		await writeExtraKeyWithMeta(
			"intel:pizzint:v1:base",
			{ pizzint: pi, tensionPairs: [] },
			PIZZINT_TTL,
			pi.locationsMonitored ?? 0,
		);
	if (pi && gd)
		await writeExtraKeyWithMeta(
			"intel:pizzint:v1:gdelt",
			{ pizzint: pi, tensionPairs: gd },
			PIZZINT_TTL,
			gd.length ?? 0,
		);
