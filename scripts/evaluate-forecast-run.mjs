...
function hasHighValueDeepCandidate(snapshot = null) {
	const candidates = Array.isArray(snapshot?.impactExpansionCandidates)
	...
	return candidates.some((packet) => {
		...
		return (
			Boolean(packet?.routeFacilityKey) ||
...
function evaluateForecastRunArtifacts(artifacts = {}) {
	...
	const simulationInteractionCount = Number(
		worldState?.simulationState?.interactionLedger?.length ||
			summary?.worldStateSummary?.simulationInteractionCount ||
			0,
	);
	const reportableInteractionCount = Number(
		worldState?.simulationState?.reportableInteractionLedger?.length ||
			summary?.worldStateSummary?.reportableInteractionCount ||
			0,
	);
	const candidateSupplyChainCount = Number(
		summary?.quality?.candidateRun?.domainCounts?.supply_chain || 0,
	);
	const publishedSupplyChainCount = Number(
		summary?.quality?.traced?.domainCounts?.supply_chain || 0,
	);
	const mappedSignalCount = Number(
		worldState?.impactExpansion?.mappedSignalCount ||
			summary?.worldStateSummary?.impactExpansionMappedSignalCount ||
			0,
	);
	const eligibleStateCount = Number(
		summary?.deepForecast?.eligibleStateCount || runStatus?.eligibleStateIds?.length || 0,
	);
	...
	const checks = [
		...
		buildCheck(
			"reportable_interactions_are_subset",
			reportableInteractionCount < simulationInteractionCount || simulationInteractionCount === 0,
			"error",
			{
		...
		buildCheck(
			"supply_chain_survives_when_candidate_present",
			candidateSupplyChainCount === 0 || publishedSupplyChainCount > 0,
			"warn",
			{
		...
		buildCheck(
			"eligible_high_value_deep_run_materializes_mapped_signals",
			eligibleStateCount === 0 || !hasHighValueDeepCandidate(snapshot) || mappedSignalCount > 0,
			"error",
			{
				eligibleStateCount,
				mappedSignalCount,
...
export { parseArgs, hasHighValueDeepCandidate, evaluateForecastRunArtifacts, evaluateForecastRun };
