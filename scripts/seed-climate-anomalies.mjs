...
import {
	CLIMATE_ZONES,
	MIN_CLIMATE_ZONE_COUNT,
	hasRequiredClimateZones,
} from "./_climate-zones.mjs";
...
export function buildClimateAnomalyFromResponse(zone, payload, normalsIndex) {
	...
	if (!monthlyNormal) {
		console.warn(
			`  [CLIMATE] Missing monthly normal for ${zone.name} month ${month}; skipping zone`,
		);
		return null;
	}
...
export async function fetchClimateAnomalies() {
	...
	if (!normalsPayload?.normals?.length) {
		throw new Error(
			`Missing ${CLIMATE_ZONE_NORMALS_KEY} baseline; run node scripts/seed-climate-zone-normals.mjs before enabling the anomaly cron`,
		);
	}
	...
	if (anomalies.length < MIN_CLIMATE_ZONE_COUNT) {
		throw new Error(
			`Only ${anomalies.length}/${CLIMATE_ZONES.length} zones returned data (${failures} errors) — skipping write to preserve previous Redis data`,
		);
	}
