...
async function fetchCotData() {
	...
	try {
		...
		commodityRows = await fetchSocrata(
			"rxbv-e226",
			`%20AND%20cftc_contract_market_code%20IN%28${codeList}%29`,
		);
	} catch (e) {
	...
	const pushInstrument = (
		...
		console.log(
			`  ${target.code}: AM net ${netPct.toFixed(1)}% (${amLong}L / ${amShort}S), date=${reportDate}`,
		);
	};
	...
	for (const target of FINANCIAL_INSTRUMENTS) {
		...
		if (!row) {
			console.warn(`  CFTC: no row for ${target.name}`);
			continue;
		}
		pushInstrument(
			target,
			row,
			toNum(row.asset_mgr_positions_long),
			toNum(row.asset_mgr_positions_short),
			toNum(row.lev_money_positions_long),
			toNum(row.lev_money_positions_short),
			toNum(row.dealer_positions_long_all),
			toNum(row.dealer_positions_short_all),
		);
	}
	...
	for (const target of COMMODITY_INSTRUMENTS) {
		...
		pushInstrument(
			target,
			row,
			toNum(row.m_money_positions_long_all),
			toNum(row.m_money_positions_short_all),
			0,
			0,
			toNum(row.swap_positions_long_all),
			toNum(row.swap__positions_short_all),
		);
	}
