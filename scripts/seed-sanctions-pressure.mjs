...
function buildCountryPressure(entries) {
	...
	return [...map.values()]
		.sort(
			(a, b) =>
				b.newEntryCount - a.newEntryCount ||
	...
	return new Promise((resolve, reject) => {
		...
		function finalizeParty() {
			...
			if (profileSubTypeId === "1") entityType = "SANCTIONS_ENTITY_TYPE_VESSEL";
				...
				profileFeatures?.some((f) =>
					/birth|citizenship|nationality/i.test(featureTypes.get(f.featureTypeId) || ""),
				)
			) {
		...
		function finalizeEntry() {
			...
			const legalNote =
				(entryLegalIds ?? []).map((id) => legalBasis.get(id) || "").find((n) => n) || "";
		...
		parser.onopentag = (node) => {
			...
			switch (name) {
				...
				case "FeatureType":
					...
					break;
				case "LegalBasis":
					if (inLegalBasisValues) {
						refId = attrs.ID || "";
						refShortRef = attrs.LegalBasisShortRef || "";
					}
					break;
				...
				case "Alias":
					if (inDistinctParties)
						curAlias = {
							...
							typeId: attrs.AliasTypeID || "",
							nameParts: [],
						};
					break;
				case "DocumentedName":
					if (curAlias) {
						inDocumentedName = true;
						namePartsBuf = [];
					}
					break;
				case "Feature":
					if (inDistinctParties)
						curFeature = { featureTypeId: attrs.FeatureTypeID || "", locationIds: [] };
					break;
		...
		parser.onclosetag = (rawName) => {
			...
			switch (name) {
				...
				case "DateOfIssue":
					inDateOfIssue = false;
					datasetDate = epoch(doiYear, doiMonth, doiDay);
					break;
				...
				case "NamePartValue":
					...
					break;
				case "DocumentedName":
					if (curAlias && namePartsBuf !== null) {
						curAlias.nameParts = namePartsBuf;
						namePartsBuf = null;
				...
				case "Comment":
					...
					if (entryNoteComments !== null && t && !PROGRAM_CODE_RE.test(t))
						entryNoteComments.push(t);
					break;
				...
				case "SanctionsEntry":
					...
					entryProfileId = "";
					entryDates = null;
					entryMeasureDates = null;
					entryPrograms = null;
					entryNoteComments = null;
					entryLegalIds = null;
					break;
			}
		};
		...
		parser.onend = () => {
			console.log(
				`  ${source.label}: ${(bytesReceived / 1024).toFixed(0)}KB streamed, ${entries.length} entries parsed (${Date.now() - t0}ms)`,
			);
...
async function fetchSanctionsPressure() {
	...
	const previousIds = new Set(
		Array.isArray(previousState?.entryIds) ? previousState.entryIds.map((id) => String(id)) : [],
	);
	const hasPrevious = previousIds.size > 0;
	console.log(
		`  Previous state: ${hasPrevious ? `${previousIds.size} known IDs` : "none (first run or expired)"}`,
	);
	...
	const vesselCount = entries.filter(
		(entry) => entry.entityType === "SANCTIONS_ENTITY_TYPE_VESSEL",
	).length;
	const aircraftCount = entries.filter(
		(entry) => entry.entityType === "SANCTIONS_ENTITY_TYPE_AIRCRAFT",
	).length;
	console.log(
		`  Merged: ${totalCount} total (${results[0]?.entries.length ?? 0} SDN + ${results[1]?.entries.length ?? 0} consolidated), ${newEntryCount} new, ${vesselCount} vessels, ${aircraftCount} aircraft`,
	);
	...
	console.log(
		`  Entity index: ${_entityIndex.length} records (~${Math.round(JSON.stringify(_entityIndex).length / 1024)}KB)`,
	);
