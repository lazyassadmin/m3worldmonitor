...
export async function fetchAvPhysicalCommodity(yahooSymbol, apiKey) {
	...
	try {
		...
		if (json.Information) {
			console.warn(`  [AV] Rate limit: ${String(json.Information).slice(0, 100)}`);
			return null;
		}
	...
	for (let i = 0; i < symbols.length; i += BATCH) {
		...
		try {
			...
			if (json.Information) {
				...
				console.warn(
					`  [AV] Rate limit hit${remaining > 0 ? ` — dropping ${remaining} remaining symbols` : ""}: ${String(json.Information).slice(0, 80)}`,
				);
				break;
			}
