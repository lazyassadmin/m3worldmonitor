...
const CHART_CONFIGS = [
	{
		label: "crude_oil_outbound",
		title: "Crude Oil Outbound Shipments",
		containerIndex: 0,
		projections: [1, 2, 3, 0],
	},
	{
		label: "lng_outbound",
		title: "LNG Outbound Shipments",
		containerIndex: 1,
		projections: [0, 1, 2, 3],
	},
	{
		label: "fertilizer_outbound",
		title: "Fertilizer Outbound Shipments",
		containerIndex: 2,
		projections: [1, 2, 3, 0],
	},
	{
		label: "agriculture_inbound",
		title: "Agriculture Inbound Shipments",
		containerIndex: 3,
		projections: [0, 1, 2, 3],
	},
];
...
async function fetchPbiCharts() {
	...
	for (const cfg of CHART_CONFIGS) {
		...
		try {
			const configObj =
				typeof container.config === "string" ? JSON.parse(container.config) : container.config;
		...
		let result;
		try {
			result = await pbiJson(`${WABI_BASE}/public/reports/querydata?synchronous=true`, {
				method: "POST",
...
await runSeed("supply_chain", "hormuz_tracker", CANONICAL_KEY, buildPayload, {
	...
	validateFn: (d) =>
		!!(d?.updatedDate || d?.summary || d?.title) &&
