...
import {
	BUDGET_LAB_TARIFFS_URL,
	htmlToPlainText,
	toIsoDate,
	parseBudgetLabEffectiveTariffHtml,
} from "./_trade-parse-utils.mjs";
...
const SHIPPING_SERIES = [
	{
		seriesId: "PCU483111483111",
		name: "Deep Sea Freight Producer Price Index",
		unit: "index",
		frequency: "m",
	},
	{
...
async function fetchShippingRates() {
	...
	for (const cfg of SHIPPING_SERIES) {
		try {
			...
			const previous =
				observations.length > 1 ? observations[observations.length - 2].value : current;
			...
			indices.push({
				...
				spikeAlert: detectSpike(observations),
			});
			await sleep(200);
		} catch (e) {
			console.warn(`  FRED ${cfg.seriesId}: ${e.message}`);
...
async function fetchSSEIndex(indexName, indexId, dataItemType, displayName, unit) {
	try {
		...
		if (!resp.ok) {
			console.warn(`  SSE ${indexName}: HTTP ${resp.status}`);
			return [];
		}
		...
		if (!composite) {
			console.warn(`  SSE ${indexName}: ${dataItemType} not found`);
			return [];
		}
		...
		const changePct =
			typeof composite.percentage === "number"
...
async function fetchBDI() {
	try {
		...
		if (contentLength > 1_000_000) {
			console.warn("  BDI: response too large");
			return [];
		}
		const html = await resp.text();
		if (html.length > 1_000_000) {
			console.warn("  BDI: body too large");
			return [];
		}
		...
		for (const cfg of BDI_INDEX_MAP) {
			const patterns = [
				new RegExp(
					`Baltic ${cfg.label} Index \\(${cfg.id}\\)[^.]*?(?:reach|to|at)\\s+([\\d,]+)\\s*points`,
					"i",
				),
			...
			for (const re of patterns) {
				...
				if (m) {
					currentValue = parseFloat(m[1].replace(/,/g, ""));
					break;
				}
			}
			...
			const deltaRe = new RegExp(
				`${cfg.id}\\)?[^.]*?(increased|decreased|gained|lost|dropped|rose)\\s+by\\s+([\\d,]+)\\s+points`,
				"i",
			);
...
async function wtoFetch(path, params) {
	...
	if (!apiKey) {
		console.warn("[WTO] WTO_API_KEY not set");
		return null;
	}
	...
	if (!resp.ok) {
		console.warn(`[WTO] HTTP ${resp.status} for ${path}`);
		return null;
	}
	return resp.json();
}
...
async function fetchBudgetLabEffectiveTariffRate() {
	try {
		...
		console.log(
			`  Budget Lab effective tariff: ${parsed.tariffRate.toFixed(1)}%${parsed.observationPeriod ? ` (${parsed.observationPeriod})` : ""}`,
		);
		return parsed;
	} catch (e) {
...
function buildFlowRecords(rows, reporterCode, partnerCode) {
	...
	for (const row of rows) {
		...
		if (row.indicator === "ITS_MTV_AX") e.exports = row.value;
		else e.imports = row.value;
	}
	...
	return sortedYears.map((year, i) => {
		...
		return {
			...
			partnerCountry:
				partnerCode === "000" ? "World" : (WTO_MEMBER_CODES[partnerCode] ?? partnerCode),
			year,
			exportValueUsd: cur.exports,
			importValueUsd: cur.imports,
			yoyExportChange:
				prev?.exports > 0
					? Math.round(((cur.exports - prev.exports) / prev.exports) * 10000) / 100
					: 0,
			yoyImportChange:
				prev?.imports > 0
					? Math.round(((cur.imports - prev.imports) / prev.imports) * 10000) / 100
					: 0,
...
async function fetchFlowPair(reporter, partner, years, flows) {
	...
	const rows = [
		...(exportsData ? parseFlowRows(exportsData, "ITS_MTV_AX") : []),
		...(importsData ? parseFlowRows(importsData, "ITS_MTV_AM") : []),
	];
...
async function fetchTradeFlows() {
	...
	console.log(
		`  Trade flows: ${Object.keys(flows).length} pairs (${MAJOR_REPORTERS.length} world + ${BILATERAL_PAIRS.length} bilateral)`,
	);
	return flows;
}
...
async function fetchTradeBarriers() {
	...
	const parseRows = (data) => {
		...
		return dataset
			.map((row) => {
				...
				return !Number.isNaN(year) && !Number.isNaN(value) && cc
					? { country: WTO_MEMBER_CODES[cc] ?? "", countryCode: cc, year, value }
					: null;
			})
			.filter(Boolean);
	};
	...
	const latestByCountry = (rows) => {
		const m = new Map();
		for (const r of rows) {
			const e = m.get(r.countryCode);
			if (!e || r.year > e.year) m.set(r.countryCode, r);
		}
		return m;
	};
...
async function fetchTradeRestrictions() {
	...
	const restrictions = [...latestByCountry.values()]
		.map((row) => {
			const value = parseFloat(row.Value ?? row.value ?? "");
			...
			return {
				...
				description: `WTO MFN baseline: ${value.toFixed(1)}%`,
...
async function fetchTariffTrends() {
	...
	for (const reporter of MAJOR_REPORTERS) {
		...
		const data = await wtoFetch("/data", {
			i: "TP_A_0010",
			r: reporter,
			ps: `${currentYear - years}-${currentYear}`,
			fmt: "json",
			mode: "full",
			max: "500",
		});
		...
		if (datapoints.length > 0) {
			...
			trends[cacheKey] = {
				datapoints,
				...(reporter === "840" && usEffectiveTariffRate
					? { effectiveTariffRate: usEffectiveTariffRate }
					: {}),
				fetchedAt: new Date().toISOString(),
				upstreamUnavailable: false,
			};
...
async function fetchCustomsRevenue() {
	...
	const fields =
		"record_date,current_month_rcpt_outly_amt,current_fytd_rcpt_outly_amt,record_fiscal_year,record_calendar_year,record_calendar_month";
	...
	if (rows.length > 100)
		throw new Error(`Treasury MTS returned unexpected row count: ${rows.length}`);
	...
	console.log(
		`  Treasury customs revenue: ${months.length} months (${months[0]?.recordDate} to ${months[months.length - 1]?.recordDate})`,
	);
...
async function fetchAll() {
	const [shipping, scfi, ccfi, bdi, barriers, restrictions, flows, tariffs, customs] =
	...
	if (shipping.status === "rejected")
		console.warn(`  Shipping failed: ${shipping.reason?.message || shipping.reason}`);
	if (scfi.status === "rejected")
		console.warn(`  SCFI failed: ${scfi.reason?.message || scfi.reason}`);
	if (ccfi.status === "rejected")
		console.warn(`  CCFI failed: ${ccfi.reason?.message || ccfi.reason}`);
	if (bdi.status === "rejected") console.warn(`  BDI failed: ${bdi.reason?.message || bdi.reason}`);
	if (barriers.status === "rejected")
		console.warn(`  Barriers failed: ${barriers.reason?.message || barriers.reason}`);
	if (restrictions.status === "rejected")
		console.warn(`  Restrictions failed: ${restrictions.reason?.message || restrictions.reason}`);
	if (flows.status === "rejected")
		console.warn(`  Flows failed: ${flows.reason?.message || flows.reason}`);
	if (tariffs.status === "rejected")
		console.warn(`  Tariffs failed: ${tariffs.reason?.message || tariffs.reason}`);
	if (customs.status === "rejected")
		console.warn(`  Treasury customs failed: ${customs.reason?.message || customs.reason}`);

	const allIndices = [...(sh?.indices || []), ...scfiResult, ...ccfiResult, ...bdiResult];

	if (allIndices.length === 0 && !ba && !re)
		throw new Error("All supply-chain/trade fetches failed");

	// History accumulation: read previous payload, merge history
	let previousPayload = null;
	try {
		previousPayload = await verifySeedKey(KEYS.shipping);
	} catch {
		/* ignore */
	}
	const mergedIndices = accumulateHistory(allIndices, previousPayload);
	console.log(
		`  Merged shipping indices: ${mergedIndices.length} (FRED: ${sh?.indices?.length ?? 0}, SCFI: ${scfiResult.length}, CCFI: ${ccfiResult.length}, BDI: ${bdiResult.length})`,
	);
	...
	if (re)
		await writeExtraKeyWithMeta(KEYS.restrictions, re, TRADE_TTL, re.restrictions?.length ?? 0);
	if (fl) {
		for (const [key, data] of Object.entries(fl))
			await writeExtraKeyWithMeta(key, data, TRADE_TTL, data.flows?.length ?? 0);
	}
	if (ta) {
		for (const [key, data] of Object.entries(ta))
			await writeExtraKeyWithMeta(key, data, TARIFF_TTL, data.datapoints?.length ?? 0);
	}
