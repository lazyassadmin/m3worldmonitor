...
async function fetchUS_EIA_prev() {
	try {
		...
		if (!apiKey) {
			console.warn("  [US-prev] EIA_API_KEY not set, skipping");
			return [];
		}
		...
		console.log(
			`  [US-prev] period=${prevPeriod} Gasoline=${gasolineUSDPerL} USD/L, Diesel=${dieselUSDPerL} USD/L`,
		);
		return [
			{
				code: "US",
...
async function fetchEU_CSV() {
	...
	try {
		...
		for (let i = 0; i < Math.min(rows.length, 10); i++) {
			if (rows[i].some((c) => String(c).includes("Euro-super"))) {
				headerRowIdx = i;
				break;
			}
		}
		if (headerRowIdx === -1) {
			console.warn("  [EU] Could not find header row");
			return [];
		}
		...
		if (dmatch)
			observedAt = `${dmatch[3]}-${dmatch[2].padStart(2, "0")}-${dmatch[1].padStart(2, "0")}`;

		const e95ColIdx = headerRow.findIndex((c) => String(c).includes("Euro-super"));
		const dieselColIdx = headerRow.findIndex(
			(c, i) => i > e95ColIdx && /gas.oil|diesel/i.test(String(c)),
		);
		...
		for (let r = dataStartIdx; r < rows.length; r++) {
			...
			results.push({
				...
				gasoline:
					gasPrice != null
						? { localPrice: gasPrice, grade: "Euro95", source: "energy.ec.europa.eu", observedAt }
						: null,
				diesel:
					dslPrice != null
...
async function fetchSpain() {
	try {
		...
		for (const s of stations) {
			const gv = parseP(s["Precio Gasolina 95 E5"]);
			if (gv) g.push(gv);
			const dv = parseP(s["Precio Gasoleo A"]);
			if (dv) d.push(dv);
		}
		...
		return [
			{
				...
				gasoline:
					avgG != null
						? { localPrice: avgG, grade: "E5", source: "minetur.gob.es", observedAt: today }
						: null,
				diesel:
					avgD != null
						? { localPrice: avgD, grade: "Diesel A", source: "minetur.gob.es", observedAt: today }
						: null,
			},
		];
	} catch (err) {
		console.warn(`  [ES] error: ${err.message}`);
		return [];
	}
}
		...
		return [
			{
				...
				gasoline:
					avgR != null
						? { localPrice: avgR, grade: "Regular", source: "datos.gob.mx", observedAt: maxDate }
						: null,
				diesel:
					avgD != null
						? { localPrice: avgD, grade: "Diesel", source: "datos.gob.mx", observedAt: maxDate }
						: null,
			},
		];
	} catch (err) {
		console.warn(`  [MX] error: ${err.message}`);
		return [];
	}
}
...
for (let i = 0; i < fetchResults.length; i++) {
	...
	if (result.status === "fulfilled" && result.value.length > 0) {
		for (const entry of result.value) {
			...
			const fxRate =
				currency === "USD" ? 1 : (fxRates[currency] ?? SHARED_FX_FALLBACKS[currency] ?? 0);
