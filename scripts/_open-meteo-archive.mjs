	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		...
		try {
			...
			if (attempt < maxRetries) {
				...
				console.log(
					`  [OPEN_METEO] ${err?.message ?? err} for ${label}; retrying batch in ${Math.round(retryMs / 1000)}s`,
				);
				await sleep(retryMs);
				continue;
			}
		...
		if (resp.ok) {
			...
			if (data.length !== zones.length) {
				throw new Error(
					`Open-Meteo batch size mismatch for ${label}: expected ${zones.length}, got ${data.length}`,
				);
			}
			return data;
		}
		...
		if (RETRYABLE_STATUSES.has(resp.status) && attempt < maxRetries) {
			const retryMs =
				parseRetryAfterMs(resp.headers.get("retry-after")) ?? retryBaseMs * 2 ** attempt;
			console.log(
				`  [OPEN_METEO] ${resp.status} for ${label}; retrying batch in ${Math.round(retryMs / 1000)}s`,
			);
			await sleep(retryMs);
			continue;
		}
