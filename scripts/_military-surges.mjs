...
export function summarizeMilitaryTheaters(flights, theaters, assessedAt = Date.now()) {
	return theaters.map((theater) => {
		...
		const counts = {
			...
			reconnaissance: theaterFlights.filter((flight) => flight.aircraftType === "reconnaissance")
				.length,
		...
		for (const flight of theaterFlights) {
			...
			if (flight.operatorCountry)
				byCountry[flight.operatorCountry] = (byCountry[flight.operatorCountry] || 0) + 1;
		}

		const totalFlights = theaterFlights.length;
		const postureLevel =
			totalFlights >= theater.thresholds.critical
				? "critical"
...
export function buildMilitarySurges(theaterSummaries, history, opts = {}) {
	...
	for (const summary of theaterSummaries) {
		...
		const maybePush = (surgeType, currentCount, baselineCount, minCount) => {
			...
			const persistenceCount = countPersistentSnapshots(
				priorSnapshots,
				field,
				effectiveBaseline,
				minCount,
		...
		const totalChangePct =
			((summary.totalFlights - effectiveTotalBaseline) / effectiveTotalBaseline) * 100;
		if (
			summary.totalFlights >=
				Math.max(6, Math.ceil(effectiveTotalBaseline * totalSurgeThreshold)) &&
			totalChangePct >= 40
		) {
			const persistenceCount = countPersistentSnapshots(
				priorSnapshots,
				"totalFlights",
				effectiveTotalBaseline,
				6,
				totalSurgeThreshold,
			);
			surges.push({
