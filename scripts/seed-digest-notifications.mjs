...
async function buildDigest(rule, windowStartMs) {
	...
	const trackResults = await upstashPipeline(hashes.map((h) => ["HGETALL", `story:track:v1:${h}`]));
...
function formatDigestHtml(stories, nowMs) {
	...
	function storyCard(s) {
		...
		const meta = [
			phaseCap
				? `<span style="font-size: 10px; color: ${phaseColor}; text-transform: uppercase; letter-spacing: 1px; font-weight: 700;">${phaseCap}</span>`
				: "",
			srcText ? `<span style="font-size: 11px; color: #555;">${escapeHtml(srcText)}</span>` : "",
		]
			.filter(Boolean)
			.join('<span style="color: #333; margin: 0 6px;">&bull;</span>');
...
async function sendSlack(userId, webhookEnvelope, text) {
	...
	try {
		...
		console.warn(`[digest] Slack decrypt failed for ${userId}:`, err.message);
		return false;
	}
	if (!SLACK_RE.test(webhookUrl)) {
		console.warn(`[digest] Slack URL invalid for ${userId}`);
		return false;
	}
	try {
		...
		if (addrs.some(isPrivateIP)) {
			console.warn(`[digest] Slack SSRF blocked for ${userId}`);
			return false;
		}
	} catch {
		return false;
...
async function sendDiscord(userId, webhookEnvelope, text) {
	...
	try {
		...
		console.warn(`[digest] Discord decrypt failed for ${userId}:`, err.message);
		return false;
	}
	if (!DISCORD_RE.test(webhookUrl)) {
		console.warn(`[digest] Discord URL invalid for ${userId}`);
		return false;
	}
	try {
		const hostname = new URL(webhookUrl).hostname;
		const addrs = await dns.resolve4(hostname).catch(() => []);
		if (addrs.some(isPrivateIP)) {
			console.warn(`[digest] Discord SSRF blocked for ${userId}`);
			return false;
		}
	} catch {
		return false;
...
async function main() {
	...
	for (const rule of rules) {
		...
		const shortDate = new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(
			new Date(nowMs),
		);
