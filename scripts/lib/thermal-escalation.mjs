...
function deriveConfidence(observationCount, uniqueSourceCount, baselineSamples) {
	if (observationCount >= 8 && uniqueSourceCount >= 2 && baselineSamples >= 4)
		return "THERMAL_CONFIDENCE_HIGH";
...
function deriveStatus({
	...
	if (persistenceHours >= 12 && (countDelta >= 3 || totalFrp >= 80))
		return "THERMAL_STATUS_PERSISTENT";
	if (
		zScore >= 2.5 ||
		countDelta >= 6 ||
		frpDelta >= 120 ||
		(observationCount >= 8 && totalFrp >= 150)
	) {
	...
	if (
		...
		countDelta >= 3 ||
		frpDelta >= 50 ||
		(baselineSamples === 0 && observationCount >= 5)
	) {
...
function deriveRelevance(status, context, totalFrp, persistenceHours) {
	...
	if (status === "THERMAL_STATUS_PERSISTENT" || totalFrp >= 120 || persistenceHours >= 12) {
		return "THERMAL_RELEVANCE_MEDIUM";
	}
	return "THERMAL_RELEVANCE_LOW";
}

function buildNarrativeFlags({
	context,
	status,
	uniqueSourceCount,
	persistenceHours,
...
function buildSummary(clusters) {
	return {
		...
		elevatedCount: clusters.filter((cluster) => cluster.status === "THERMAL_STATUS_ELEVATED")
			.length,
		spikeCount: clusters.filter((cluster) => cluster.status === "THERMAL_STATUS_SPIKE").length,
		persistentCount: clusters.filter((cluster) => cluster.status === "THERMAL_STATUS_PERSISTENT")
			.length,
		conflictAdjacentCount: clusters.filter(
			(cluster) => cluster.context === "THERMAL_CONTEXT_CONFLICT_ADJACENT",
		).length,
		highRelevanceCount: clusters.filter(
			(cluster) => cluster.strategicRelevance === "THERMAL_RELEVANCE_HIGH",
		).length,
	};
}

export function computeThermalEscalationWatch(
	detections,
	previousHistory = { cells: {} },
	options = {},
) {
					...
					key,
					{
						entries: Array.isArray(value?.entries)
							? value.entries.filter(
									(entry) => nowMs - Date.parse(entry.observedAt || 0) <= HISTORY_RETENTION_MS,
								)
							: [],
					},
				])
	...
	for (const cluster of clusters) {
		...
		const prevEntries = Array.isArray(previousCells[key]?.entries)
			? previousCells[key].entries.filter(
					(entry) => nowMs - Date.parse(entry.observedAt || 0) <= HISTORY_RETENTION_MS,
				)
			: [];
		const baselineEntries = prevEntries.filter(
			(entry) => nowMs - Date.parse(entry.observedAt || 0) <= BASELINE_WINDOW_MS,
		);
		const baselineCounts = baselineEntries
			.map((entry) => Number(entry.observationCount || 0))
			.filter(Number.isFinite);
		const baselineFrps = baselineEntries
			.map((entry) => Number(entry.totalFrp || 0))
			.filter(Number.isFinite);
		...
		const totalFrp = round(
			sorted.reduce((sum, detection) => sum + (Number(detection.frp) || 0), 0),
			1,
		);
		const maxFrp = round(
			sorted.reduce((max, detection) => Math.max(max, Number(detection.frp) || 0), 0),
			1,
		);
		const maxBrightness = round(
			sorted.reduce((max, detection) => Math.max(max, Number(detection.brightness) || 0), 0),
			1,
		);
		const avgBrightness = round(
			average(sorted.map((detection) => Number(detection.brightness) || 0)),
			1,
		);
		const countDelta = round(observationCount - baselineExpectedCount, 1);
		const frpDelta = round(totalFrp - baselineExpectedFrp, 1);
		const countSigma =
			baselineCounts.length >= 2 ? stdDev(baselineCounts, baselineExpectedCount) : 0;
		const zScore = round(
			countSigma > 0 ? (observationCount - baselineExpectedCount) / countSigma : 0,
			2,
		);
		const uniqueSourceCount = new Set(sorted.map((detection) => detection.satellite || "unknown"))
			.size;
		const nightDetectionShare = round(
			sorted.filter((detection) => String(detection.dayNight || "").toUpperCase() === "N").length /
				observationCount,
			2,
		);
		const context = deriveContext(cluster.regionLabel);
		const lastPrevObservationMs =
			prevEntries.length > 0
				? Math.max(
						...prevEntries
							.map((entry) => Date.parse(entry.observedAt || 0))
							.filter(Number.isFinite),
					)
				: 0;
		const persistenceHours = round(
			lastPrevObservationMs > 0 && nowMs - lastPrevObservationMs <= RECENT_PERSISTENCE_MS
				? (nowMs - Math.min(Number(first.detectedAt) || nowMs, lastPrevObservationMs)) /
						(60 * 60 * 1000)
				: (Number(last.detectedAt) - Number(first.detectedAt)) / (60 * 60 * 1000),
			1,
		);
