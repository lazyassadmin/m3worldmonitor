...
function proxyConnectTunnel(
	targetHostname,
	proxyConfig,
	{ timeoutMs = 20_000, targetPort = 443 } = {},
) {
...
function proxyFetch(
	url,
	proxyConfig,
	{ accept = "*/*", headers = {}, method = "GET", body = null, timeoutMs = 20_000 } = {},
) {
	const targetUrl = new URL(url);

	return proxyConnectTunnel(targetUrl.hostname, proxyConfig, { timeoutMs }).then(
		({ socket: tlsSocket, destroy }) => {
			return new Promise((resolve, reject) => {
				const timer = setTimeout(() => {
					destroy();
