	...
	while (offset < buf.length - 30) {
		...
		if (!filename || entryName === filename || entryName.endsWith("/" + filename)) {
			if (flags & 0x08 && compSize === 0) {
				throw new Error(
					`JODI Gas ZIP: entry uses data-descriptor (bit 3) — compSize unknown in local header`,
				);
			}
...
export function buildLngVulnerabilityIndex(members, dataMonth, updatedAt) {
	const withLng = members.filter(
		(r) =>
			r.lngShareOfImports !== null &&
			typeof r.lngShareOfImports === "number" &&
			(r.lngImportsTj ?? 0) > 0,
	);
	...
	const top20LngDependent = withLng
		.sort(
			(a, b) =>
				/** @type {number} */ (b.lngShareOfImports) - /** @type {number} */ (a.lngShareOfImports),
		)
		.slice(0, 20)
		.map((r) => ({
			iso2: r.iso2,
	...
	const top20PipelineDependent = withPipe
		.sort(
			(a, b) =>
				/** @type {number} */ (a.lngShareOfImports) - /** @type {number} */ (b.lngShareOfImports),
		)
		.slice(0, 20)
		.map((r) => ({
			iso2: r.iso2,
