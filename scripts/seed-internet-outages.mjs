...
async function fetchDdosData(token) {
	...
	const [protocolResp, vectorResp, targetResp] = await Promise.all([
		fetch(`${CF_RADAR_BASE}/radar/attacks/layer3/summary/protocol?dateRange=7d`, {
			headers,
			signal: AbortSignal.timeout(15_000),
		}),
		fetch(`${CF_RADAR_BASE}/radar/attacks/layer3/summary/vector?dateRange=7d`, {
			headers,
			signal: AbortSignal.timeout(15_000),
		}),
		fetch(`${CF_RADAR_BASE}/radar/attacks/layer3/top/locations/target?dateRange=7d`, {
			headers,
			signal: AbortSignal.timeout(15_000),
		}),
	]);
	...
	if (!protocolResp.ok || !vectorResp.ok) {
		throw new Error(
			`CF Radar DDoS API error: protocol=${protocolResp.status} vector=${vectorResp.status}`,
		);
	}
	...
	function toEntries(summary) {
		return Object.entries(summary || {})
			.map(([label, pct]) => ({ label, percentage: parseFloat(pct) || 0 }))
			.sort((a, b) => b.percentage - a.percentage);
	}

	const topTargetLocations = (targetData?.result?.top_0 || [])
		.map((item) => {
...
async function fetchAll() {
	...
	if (ddosResult.status === "rejected")
		console.warn(
			`  CF Radar DDoS fetch failed: ${ddosResult.reason?.message || ddosResult.reason}`,
		);
	if (anomaliesResult.status === "rejected")
		console.warn(
			`  CF Radar traffic anomalies fetch failed: ${anomaliesResult.reason?.message || anomaliesResult.reason}`,
		);
	...
	if (ddos && (ddos.protocol.length > 0 || ddos.vector.length > 0)) {
		await writeExtraKeyWithMeta(
			...
			ddos,
			DDOS_TTL,
			ddos.protocol.length + ddos.vector.length,
		);
	}
	if (anomalies && anomalies.anomalies.length > 0) {
		await writeExtraKeyWithMeta(
			TRAFFIC_ANOMALIES_KEY,
			anomalies,
			ANOMALIES_TTL,
