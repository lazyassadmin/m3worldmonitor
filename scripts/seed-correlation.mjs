...
async function fetchInputData() {
	...
	for (let i = 0; i < INPUT_KEYS.length; i++) {
		...
		if (raw) {
			try {
				data[INPUT_KEYS[i]] = JSON.parse(raw);
			} catch {
				/* skip */
			}
		}
...
function collectEscalationSignals(protests, outages, newsClusters) {
	...
	for (const p of protests) {
		...
		if (!code) continue;
		const severityMap = {
			SEVERITY_LEVEL_HIGH: 85,
			SEVERITY_LEVEL_MEDIUM: 55,
			SEVERITY_LEVEL_LOW: 30,
			high: 85,
			medium: 55,
			low: 30,
		};
	...
	for (const o of outages) {
		...
		if (!code) continue;
		const severityMap = {
			OUTAGE_SEVERITY_TOTAL: 90,
			OUTAGE_SEVERITY_MAJOR: 70,
			OUTAGE_SEVERITY_PARTIAL: 40,
			total: 90,
			major: 70,
			partial: 40,
		};
	...
	const conflictCountries = new Set(
		signals
			.filter((s) => s.type === "conflict_event")
			.map((s) => s.country)
			.filter(Boolean),
	);
...
const KNOWN_ENTITIES =
	/\b(Iran|Russia|China|North Korea|Venezuela|Cuba|Syria|Myanmar|Belarus|Turkey|Saudi|OPEC|EU|USA?|United States|India)\b(?![A-Za-z])/i;
...
function generateEconomicTitle(cluster, entityKey) {
	...
	if (types.has("sanctions_news")) {
		...
		if (types.has("market_move")) {
			...
			const moverNames = movers
				.map((s) => s.display ?? s.symbol ?? s.label.split(" ")[0])
				.slice(0, 2);
	...
	const fallback =
		entityKey && !GENERIC_ENTITY_KEYS.has(entityKey)
...
function generateDisasterTitle(cluster) {
	...
	if (types.has("earthquake")) {
		const maxMag = Math.max(
			...cluster.filter((s) => s.type === "earthquake").map((s) => s.magnitude ?? 0),
		);
		parts.push(`M${maxMag.toFixed(1)} seismic`);
	}
	if (types.has("infra_outage")) parts.push("infra disruption");
	const quakePlace = cluster
		.find((s) => s.type === "earthquake")
		?.label?.split("\u2014")[1]
		?.trim();
	return parts.length > 0
...
function clusterByProximity(signals, radiusKm) {
	...
	for (const i of validIndices) {
		...
		if (list) list.push(signals[i]);
		else clusterMap.set(root, [signals[i]]);
	}
	const clusters = [];
	...
	return clusters
		.map((cluster) => {
			...
			const key =
				cluster.country ??
				cluster.entityKey ??
				`${centroidLat?.toFixed(1)},${centroidLon?.toFixed(1)}`;
...
function toCard(scored, domain, titleFn) {
	...
	const location =
		scored.centroidLat != null && scored.centroidLon != null
...
const DOMAINS = {
	military: {
		weights: { military_flight: 0.4, ais_gap: 0.3, military_vessel: 0.3 },
	...
	escalation: {
		weights: { conflict_event: 0.45, escalation_outage: 0.25, news_severity: 0.3 },
		clusterMode: "country",
		threshold: 20,
		titleFn: generateEscalationTitle,
	},
	economic: {
		weights: { market_move: 0.35, sanctions_news: 0.3, commodity_spike: 0.35 },
		clusterMode: "entity",
		threshold: 20,
...
async function computeCorrelation() {
	...
	const flights =
		data["military:flights:v1"]?.flights ??
		data["military:flights:stale:v1"]?.flights ??
		data["military:flights:v1"] ??
		data["military:flights:stale:v1"] ??
		[];
	...
	const newsClusters = (insights?.topStories ?? []).map((s) => ({
		...
		lastUpdated: s.pubDate
			? new Date(s.pubDate).getTime()
			: insights?.generatedAt
				? new Date(insights.generatedAt).getTime()
				: Date.now(),
		lat: s.lat,
		lon: s.lon,
	}));
	...
	const milScored = scoreClusters(
		milClusters,
		DOMAINS.military.weights,
		DOMAINS.military.threshold,
	);
	result.military = milScored
		.map((s) => toCard(s, "military", generateMilitaryTitle))
		.sort((a, b) => b.score - a.score);

	// Escalation
	const escSignals = collectEscalationSignals(protests, outages, newsClusters);
	const escClusters = clusterByCountry(escSignals);
	const escScored = scoreClusters(
		escClusters,
		DOMAINS.escalation.weights,
		DOMAINS.escalation.threshold,
	);
	result.escalation = escScored
		.map((s) => toCard(s, "escalation", generateEscalationTitle))
		.sort((a, b) => b.score - a.score);

	// Economic
	const ecoSignals = collectEconomicSignals(allMarkets, newsClusters);
	const ecoClusters = clusterByEntity(ecoSignals);
	const ecoScored = scoreClusters(
		ecoClusters,
		DOMAINS.economic.weights,
		DOMAINS.economic.threshold,
	);
	result.economic = ecoScored
		.map((s) => toCard(s, "economic", generateEconomicTitle))
		.sort((a, b) => b.score - a.score);

	// Disaster
	const disSignals = collectDisasterSignals(earthquakes, outages, protests);
	const disClusters = clusterByProximity(disSignals, 500);
	const disScored = scoreClusters(
		disClusters,
		DOMAINS.disaster.weights,
		DOMAINS.disaster.threshold,
	);
	result.disaster = disScored
		.map((s) => toCard(s, "disaster", generateDisasterTitle))
		.sort((a, b) => b.score - a.score);
