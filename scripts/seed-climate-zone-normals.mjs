...
export function computeMonthlyNormals(daily) {
	...
	return Array.from(dailyBucketByYearMonth.entries())
		...
		.filter(
			(entry) =>
				entry != null && Number.isFinite(entry.tempMean) && Number.isFinite(entry.precipMean),
		);
}
...
export function buildZoneNormalsFromBatch(zones, batchPayloads) {
	return zones.flatMap((zone, index) => {
		...
		if (months.length !== 12) {
			console.warn(
				`  [CLIMATE_NORMALS] Open-Meteo normals incomplete for ${zone.name}: expected 12 months, got ${months.length}`,
			);
			return [];
		}
