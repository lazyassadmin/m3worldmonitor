	...
	try {
		...
		if (!test.validate(data)) {
			return {
				name: test.name,
				pass: false,
				reason: "Validation failed — unexpected shape",
				elapsed,
				data,
			};
		}
...
async function main() {
	...
	if (health.pass && health.seeds) {
		const icon = health.overall === "healthy" ? PASS : health.overall === "warning" ? WARN : FAIL;
		console.log(`  ${icon} Overall: ${health.overall}`);
		for (const [domain, info] of Object.entries(health.seeds)) {
			const dIcon = info.status === "ok" ? PASS : info.status === "stale" ? WARN : FAIL;
	...
	if (health.seeds) {
		...
		for (const [domain, testName] of Object.entries(seedDomainToTest)) {
			...
			if (seedInfo.status === "ok" && rpcResult.pass) {
				...
				console.log(
					`  ${WARN} ${domain}: seed ${seedInfo.status} but RPC still returns data (fallback working)`,
				);
			} else {
