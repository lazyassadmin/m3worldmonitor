...
async function fetchArxivPapers() {
	...
	for (const cat of categories) {
		...
		if (!resp.ok) {
			console.warn(`  arXiv ${cat}: HTTP ${resp.status}`);
			continue;
		}
...
async function fetchHackerNews() {
	...
	for (const feed of feeds) {
		...
		if (!idsResp.ok) {
			console.warn(`  HN ${feed}: HTTP ${idsResp.status}`);
			continue;
		}
...
async function fetchTechEvents() {
	...
	try {
		...
		console.warn(`  Techmeme ICS: ${e.message}`);
	}
	...
	try {
		...
		if (resp.ok) {
			...
			for (const m of items) {
				...
				if (dateMatch) {
					...
					if (!Number.isNaN(p.getTime())) startDate = p.toISOString().split("T")[0];
				}
				if (!startDate || startDate < today) continue;
				events.push({
					id: guid || `dev-${title.slice(0, 20)}`,
					title,
					type: "conference",
					location: "",
					startDate,
		...
		console.warn(`  dev.events RSS: ${e.message}`);
	}
		...
		.filter((e) => {
			const key =
				e.title
					.toLowerCase()
					.replace(/[^a-z0-9]/g, "")
					.slice(0, 30) + e.startDate.slice(0, 4);
...
async function fetchTrendingFromOSSInsight(lang) {
	...
	return rows.slice(0, 50).map((r) => ({
		...
		language: r.primary_language || lang,
		stars: r.stars || 0,
		starsToday: 0,
		forks: r.forks || 0,
	...
	for (const lang of languages) {
		try {
			...
			if (!repos || repos.length === 0) {
				console.warn(`  Trending ${lang}: no data from any source`);
				continue;
			}
...
async function fetchAll() {
	...
	if (arxiv.status === "rejected")
		console.warn(`  arXiv failed: ${arxiv.reason?.message || arxiv.reason}`);
	if (hn.status === "rejected") console.warn(`  HN failed: ${hn.reason?.message || hn.reason}`);
	if (techEvents.status === "rejected")
		console.warn(`  TechEvents failed: ${techEvents.reason?.message || techEvents.reason}`);
	if (trending.status === "rejected")
		console.warn(`  Trending failed: ${trending.reason?.message || trending.reason}`);

	if (!allData.arxiv && !allData.hn && !allData.trending)
		throw new Error("All research fetches failed");
	...
	if (allData.hn) {
		for (const [key, data] of Object.entries(allData.hn))
			await writeExtraKeyWithMeta(key, data, HN_TTL, data.items?.length ?? 0);
	}
	if (allData.techEvents?.events?.length > 0)
		await writeExtraKeyWithMeta(
			...
			allData.techEvents,
			TECH_EVENTS_TTL,
			allData.techEvents.events.length,
		);
	if (allData.trending) {
		for (const [key, data] of Object.entries(allData.trending))
			await writeExtraKeyWithMeta(key, data, TRENDING_TTL, data.repos?.length ?? 0);
	}
