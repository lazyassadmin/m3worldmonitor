...
const WHO_DON_API =
	"https://www.who.int/api/emergencies/diseaseoutbreaknews?sf_provider=dynamicProvider372&sf_culture=en&$orderby=PublicationDateAndTime%20desc&$select=Title,ItemDefaultUrl,PublicationDateAndTime&$top=30";
...
const THINKGLOBALHEALTH_BUNDLE =
	"https://raw.githubusercontent.com/thinkglobalhealth/disease_tracker/main/index_bundle.js";
...
function detectAlertLevel(title, desc) {
	...
	if (
		...
		return "alert";
	if (text.includes("warning") || text.includes("spread") || text.includes("cases increasing"))
		return "warning";
	return "watch";
}
...
async function fetchWhoDonApi() {
	try {
		...
		if (!resp.ok) {
			console.warn(`[Disease] WHO DON API HTTP ${resp.status}`);
			return [];
		}
		const data = await resp.json();
		const items = data?.value;
		if (!Array.isArray(items)) {
			console.warn("[Disease] WHO DON API: unexpected response shape");
			return [];
		}
		return items
			.map((item) => ({
				...
				desc: "",
				publishedMs: item.PublicationDateAndTime
...
async function fetchRssItems(url, sourceName) {
	try {
		...
		if (!resp.ok) {
			console.warn(`[Disease] ${sourceName} HTTP ${resp.status}`);
			return [];
		}
...
async function fetchThinkGlobalHealth() {
	try {
		...
		if (!resp.ok) {
			console.warn(`[Disease] ThinkGlobalHealth HTTP ${resp.status}`);
			return [];
		}
		...
		if (startIdx === -1) {
			console.warn("[Disease] ThinkGlobalHealth: data marker not found");
			return [];
		}
		...
		console.log(
			`[Disease] ThinkGlobalHealth: ${records.length} total, ${items.length} in last ${TGH_LOOKBACK_DAYS}d`,
		);
		return items;
	} catch (e) {
...
async function fetchDiseaseOutbreaks() {
	...
	console.log(
		`[Disease] Sources: WHO=${whoItems.length} CDC=${cdcItems.length} ONT=${outbreakNewsItems.length} TGH=${tghItems.length}`,
	);
	...
	const dedupedOthers = otherOutbreaks.filter((o) => {
		const key =
			o.disease === "Unknown Disease" ? o.id : `${o.disease}:${o.countryCode || o.location}`;
