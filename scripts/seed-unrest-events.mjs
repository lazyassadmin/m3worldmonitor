...
async function fetchUnrestEvents() {
	...
	if (results[0].status === "rejected")
		console.log(`  ACLED failed: ${results[0].reason?.message || results[0].reason}`);
	if (results[1].status === "rejected")
		console.log(`  GDELT failed: ${results[1].reason?.message || results[1].reason}`);

	const merged = deduplicateEvents([...acledEvents, ...gdeltEvents]);
	const sorted = sortBySeverityAndRecency(merged);

	console.log(
		`  Merged: ${acledEvents.length} ACLED + ${gdeltEvents.length} GDELT = ${sorted.length} deduplicated`,
	);
