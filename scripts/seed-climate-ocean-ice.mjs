...
const NOAA_GLOBAL_OCEAN_V6_INDEX_URL =
	"https://www.ncei.noaa.gov/data/noaa-global-surface-temperature/v6/access/timeseries/";
const NOAA_GLOBAL_OCEAN_V51_URL =
	"https://www.ncei.noaa.gov/data/noaa-global-surface-temperature/v5.1/access/timeseries/aravg.mon.ocean.90S.90N.v5.1.0.202312.asc";
...
function dayOfYear(year, month, day) {
	return Math.floor(
		(Date.UTC(year, month - 1, day) - Date.UTC(year, 0, 0)) / (24 * 60 * 60 * 1000),
	);
}
	...
	return text
		...
		.map((line) => {
			...
			return {
				...
				extent:
					Number.isFinite(primaryExtent) && primaryExtent > 0 ? primaryExtent : fallbackExtent,
			};
		})
...
export function parseSeaIceClimatologyRows(text) {
	return text
		...
		.filter(
			(row) =>
				Number.isInteger(row.doy) &&
				row.doy >= 1 &&
				row.doy <= 366 &&
				Number.isFinite(row.medianExtent) &&
				row.medianExtent > 0,
		);
}
...
function classifyArcticTrend(current, monthlyMedian, dailyRows) {
	const sameDayHistory = dailyRows.filter(
		(row) => row.month === current.month && row.day === current.day,
	);
...
async function fetchSeaIceSection() {
	const [dailyText, climatologyResult] = await Promise.all([
		...
		fetchText(NSIDC_CLIMATOLOGY_URL, "NSIDC sea ice climatology")
			.then((text) => ({ text }))
			.catch((err) => {
	...
	return {
		data: {
			...
			...(Number.isFinite(currentMedian)
				? { arctic_extent_anomaly_mkm2: round(latest.extent - currentMedian, 2) }
				: {}),
...
export function parseSeaLevelOverlay(html) {
	const normalized = html
		...
		.trim();
	const riseMatch =
		normalized.match(/RISE SINCE 1993\s+([0-9]+(?:\.[0-9]+)?)\s+millimeters/i) ??
		normalized.match(/since 1993[^0-9]*([0-9]+(?:\.[0-9]+)?)\s*(?:mm|millimeters)/i);
	const rateMatch =
		normalized.match(
			/current yearly rate of\s+[0-9.]+\s+inches\/year\s+\(([0-9.]+)\s+centimeters\/year\)/i,
...
async function fetchSeaLevelSection() {
	...
	if (
		!Number.isFinite(parsed.seaLevelMmAbove1993) &&
		!Number.isFinite(parsed.seaLevelAnnualRiseMm)
	) {
	...
	return {
		data: {
			...(Number.isFinite(parsed.seaLevelMmAbove1993)
				? { sea_level_mm_above_1993: parsed.seaLevelMmAbove1993 }
				: {}),
			...(Number.isFinite(parsed.seaLevelAnnualRiseMm)
				? { sea_level_annual_rise_mm: parsed.seaLevelAnnualRiseMm }
				: {}),
		},
	};
}
...
async function fetchSstSection() {
	...
	const currentText = await fetchText(
		new URL(latestPath, NOAA_GLOBAL_OCEAN_V6_INDEX_URL).toString(),
		"NOAA global ocean temperature series",
	);
...
if (isMain) {
	runSeed("climate", "ocean-ice", CLIMATE_OCEAN_ICE_KEY, fetchOceanIceData, {
		...
		sourceVersion:
			"nsidc-sea-ice_v4-climatology-noaa-ohc-nasa-gmsl-noaa-global-ocean-v6-v51-baseline-v3",
	}).catch((err) => {
