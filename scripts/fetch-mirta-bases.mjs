...
const BASE =
	"https://services7.arcgis.com/n1YM8pTrFmm7L4hs/arcgis/rest/services/mirta/FeatureServer";
...
async function fetchAllFeatures(layerIndex) {
	...
	while (exceeded) {
		...
		console.log(
			`  Page ${page}: got ${features.length} features (total so far: ${allFeatures.length})`,
		);
...
function processFeature(feature) {
	...
	return {
		...
		lon,
		kind:
			p.FEATUREDESCRIPTION && p.FEATUREDESCRIPTION !== "na" ? p.FEATUREDESCRIPTION : "Installation",
...
async function main() {
	...
	const combinedRaw = {
		...
		features: [...pointsGeoJson.features, ...boundaryGeoJson.features],
	};
	...
	const samples = [
		processed[0],
		processed[Math.floor(processed.length / 3)],
		processed[Math.floor((processed.length * 2) / 3)],
