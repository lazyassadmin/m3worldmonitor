...
export function buildCountriesPayload(rawEntries) {
	...
	for (const { iso2, entries } of rawEntries) {
		...
		const countryName = sorted[0].name ?? sorted[0].country ?? COUNTRY_NAMES[iso2] ?? iso2;
...
async function preservePreviousSnapshot(errorMsg) {
	...
	await extendExistingTtl([...perCountryKeys, GAS_STORAGE_COUNTRIES_KEY], GAS_STORAGE_TTL_SECONDS);
...
export async function main() {
	...
	if (!apiKey) {
		console.warn(
			"  WARNING: GIE_API_KEY / AGSI_API_KEY not set — attempting unauthenticated requests",
		);
	}
		...
		for (let i = 0; i < EU_COUNTRIES.length; i += BATCH_SIZE) {
			...
			for (const result of results) {
				if (result.status === "fulfilled") {
					...
					console.warn(
						`  [gas-storage-countries] Failed to fetch country data:`,
