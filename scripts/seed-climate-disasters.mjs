...
function getNaturalSourceMeta(event) {
	...
	if (name === "nasa firms" || name.startsWith("firms") || url.includes("firms.modaps."))
		return { source: "NASA FIRMS" };
	if (
		name === "gdacs" ||
		name.startsWith("gdacs") ||
		url.includes("gdacs.org") ||
		id.startsWith("gdacs-")
	)
		return { source: "GDACS" };
	if (url.includes("eonet.") || id.startsWith("EONET_") || name.startsWith("eonet"))
		return { source: "EONET" };
...
function isClimateNaturalEvent(event) {
	...
	const text =
		`${event.categoryTitle || ""} ${event.classification || ""} ${event.title || ""}`.toLowerCase();
...
function getReliefWebAppname() {
	const appname = String(
		process.env.RELIEFWEB_APPNAME || process.env.RELIEFWEB_APP_NAME || "",
	).trim();
...
function mapReliefItem(item) {
	...
	const countries = [...asArray(fields.primary_country), ...asArray(fields.country)];
	...
	const startedAt = parseTimestamp(
		fields?.date?.event || fields?.date?.created || fields?.date?.changed,
	);
	...
	for (const endpoint of RELIEFWEB_ENDPOINTS) {
		for (const body of requestBodies) {
			try {
				...
				if (rows.length > 0)
					console.log(`  [ReliefWeb] ${rows.length} rows returned but all mapped to null`);
			} catch (err) {
				lastError = err;
				const message = String(err?.message || err);
				if (/approved appname/i.test(message) || /HTTP 40[13]/.test(message)) {
					const cfgErr = new Error(
						`ReliefWeb rejected RELIEFWEB_APPNAME="${appname}" — configure an approved appname`,
					);
...
function mapNaturalEvent(event) {
	...
	return {
		...
		type,
		name: normalizeDisasterName(
			event.title || event.stormName || event.categoryTitle || "Untitled disaster",
		),
		country,
		countryCode,
...
async function fetchClimateDisasters() {
	const results = await Promise.allSettled([fetchReliefWeb(), fetchNaturalClimateDisasters()]);
