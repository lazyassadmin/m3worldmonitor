...
async function fetchWithRetry(topic, maxRetries = 3) {
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			console.log(
				`    429 rate-limited, waiting ${backoff / 1000}s... (attempt ${attempt + 1}/${maxRetries + 1})`,
			);
			await sleep(backoff);
		}
	}
...
async function fetchAllTopics() {
	...
	for (let i = 0; i < INTEL_TOPICS.length; i++) {
		...
		if (result.exhausted && i < INTEL_TOPICS.length - 1) {
			console.log(
				`    Rate-limit cooldown: waiting ${POST_EXHAUST_DELAY_MS / 1000}s before next topic...`,
			);
			...
			for (const topic of topics) {
				if (topic.articles.length === 0 && prevMap.has(topic.id)) {
					...
					if (prev.articles?.length > 0) {
						console.log(
							`    ${topic.id}: rate-limited — using ${prev.articles.length} cached articles from previous snapshot`,
						);
