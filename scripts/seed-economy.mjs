...
const STRESS_COMPONENTS = [
	{
		...
		label: "Yield Curve",
		weight: 0.2,
		/** @param {number} v */ score: (v) => clamp(((0.5 - v) / (0.5 - -1.5)) * 100),
	},
	{
		id: "T10Y3M",
		label: "Bank Spread",
		weight: 0.15,
		/** @param {number} v */ score: (v) => clamp(((0.5 - v) / (0.5 - -1.0)) * 100),
	},
	{
		id: "VIXCLS",
		label: "Volatility",
		weight: 0.2,
		/** @param {number} v */ score: (v) => clamp(((v - 15) / (80 - 15)) * 100),
	},
	{
		id: "STLFSI4",
		label: "Financial Stress",
		weight: 0.2,
		/** @param {number} v */ score: (v) => clamp(((v - -1) / (5 - -1)) * 100),
	},
	{
		id: "GSCPI",
		label: "Supply Chain",
		weight: 0.15,
		/** @param {number} v */ score: (v) => clamp(((v - -2) / (4 - -2)) * 100),
	},
	{
		id: "ICSA",
		label: "Job Claims",
		weight: 0.1,
		/** @param {number} v */ score: (v) => clamp(((v - 180000) / (500000 - 180000)) * 100),
	},
];
...
function computeStressIndex(fr) {
	...
	for (const comp of STRESS_COMPONENTS) {
		...
		if (rawValue === null) {
			missingCount++;
			if (comp.id !== "GSCPI")
				console.warn(`  [StressIndex] ${comp.id} missing from FRED — excluding`);
			components.push({
				id: comp.id,
				label: comp.label,
				rawValue: null,
	...
	console.log(
		`  [StressIndex] Composite: ${compositeScore} (${label}) — ${STRESS_COMPONENTS.length - missingCount}/${STRESS_COMPONENTS.length} components`,
	);
	return {
		compositeScore,
		label,
...
async function fetchEnergyPrices() {
	...
	for (const c of EIA_COMMODITIES) {
		...
		if (!resp.ok) {
			console.warn(`  EIA ${c.commodity}: HTTP ${resp.status}`);
			continue;
		}
...
async function fetchFredSeries() {
	...
	for (const seriesId of FRED_SERIES) {
		try {
			...
			const [obsResp, metaResp] = await Promise.allSettled([
				fredFetchJson(
					`https://api.stlouisfed.org/fred/series/observations?${obsParams}`,
					_proxyAuth,
				),
			...
			if (obsResp.status === "rejected") {
				console.warn(
					`  FRED ${seriesId}: fetch failed — ${obsResp.reason?.message || obsResp.reason}`,
				);
				continue;
			}
			...
			const observations = (obsData.observations || [])
				.map((o) => {
					...
					return Number.isNaN(v) || o.value === "." ? null : { date: o.date, value: v };
				})
				.filter(Boolean)
				.reverse();
	...
	if (fredCount === 0)
		console.warn(
			"  [WARN] FRED series: 0 fetched — all series failed. Check FRED_API_KEY and PROXY_URL. FRED-dependent panels will go stale.",
		);
	return results;
}
...
async function fetchFredJpyFallback() {
	...
	try {
		...
		const data = await fredFetchJson(
		...
		return (data.observations || [])
			.map((o) => {
				const v = parseFloat(o.value);
				return Number.isNaN(v) || o.value === "." ? null : v;
			})
			.filter(Boolean)
			.reverse();
	} catch {
...
async function fetchMacroSignals(proxyAuth = null) {
	...
	const jpyChart = await fetchJsonSafe(
		`${yahooBase}/JPY=X?range=1y&interval=1d`,
		8000,
		proxyAuth,
	).catch(() => null);
	await sleep(150);
	const btcChart = await fetchJsonSafe(
		`${yahooBase}/BTC-USD?range=1y&interval=1d`,
		8000,
		proxyAuth,
	).catch(() => null);
	await sleep(150);
	const qqqChart = await fetchJsonSafe(
		`${yahooBase}/QQQ?range=1y&interval=1d`,
		8000,
		proxyAuth,
	).catch(() => null);
	await sleep(150);
	const xlpChart = await fetchJsonSafe(
		`${yahooBase}/XLP?range=1y&interval=1d`,
		8000,
		proxyAuth,
	).catch(() => null);
...
async function fetchCrudeInventories() {
	...
	for (let i = 0; i < Math.min(rows.length, 9); i++) {
		...
		const period =
			typeof row.period === "string" && /^\d{4}-\d{2}-\d{2}$/.test(row.period) ? row.period : "";
	...
	if (weeks.length < CRUDE_MIN_WEEKS)
		throw new Error(`EIA WCRSTUS1: only ${weeks.length} valid rows (need >= ${CRUDE_MIN_WEEKS})`);
...
async function fetchNatGasStorage() {
	...
	if (weeks.length < NAT_GAS_MIN_WEEKS)
		throw new Error(
			`EIA NW2_EPG0_SWO_R48_BCF: only ${weeks.length} valid rows (need >= ${NAT_GAS_MIN_WEEKS})`,
		);
	const latestPeriod = weeks[0]?.period ?? "";
...
async function fetchSprLevels() {
	...
	if (weeks.length < SPR_MIN_WEEKS)
		throw new Error(`EIA WCSSTUS1: only ${weeks.length} valid rows (need >= ${SPR_MIN_WEEKS})`);
	...
	console.log(
		`  SPR levels: ${weeks.length} weeks, latest=${latestPeriod}, barrels=${latest.barrels}M`,
	);
...
async function fetchRefineryInputs() {
	...
	if (weeks.length < REFINERY_MIN_WEEKS)
		throw new Error(
			`EIA WCRRIUS2: only ${weeks.length} valid rows (need >= ${REFINERY_MIN_WEEKS})`,
		);
	...
	const latestPeriod = latest.period;
	console.log(
		`  Refinery inputs: ${weeks.length} weeks, latest=${latestPeriod}, inputs=${latest.inputsMbblpd} MBBL/D`,
	);
...
async function fetchAll() {
	...
	if (energyPrices.status === "rejected")
		console.warn(`  EnergyPrices failed: ${energyPrices.reason?.message || energyPrices.reason}`);
	if (energyCapacity.status === "rejected")
		console.warn(
			`  EnergyCapacity failed: ${energyCapacity.reason?.message || energyCapacity.reason}`,
		);
	if (fredResults.status === "rejected")
		console.warn(`  FRED failed: ${fredResults.reason?.message || fredResults.reason}`);
	if (macroSignals.status === "rejected")
		console.warn(`  MacroSignals failed: ${macroSignals.reason?.message || macroSignals.reason}`);
	if (crudeInventories.status === "rejected")
		console.warn(
			`  CrudeInventories failed: ${crudeInventories.reason?.message || crudeInventories.reason}`,
		);
	if (natGasStorage.status === "rejected")
		console.warn(
			`  NatGasStorage failed: ${natGasStorage.reason?.message || natGasStorage.reason}`,
		);
	if (sprLevels.status === "rejected")
		console.warn(`  SPRLevels failed: ${sprLevels.reason?.message || sprLevels.reason}`);
	if (refineryInputs.status === "rejected")
		console.warn(
			`  RefineryInputs failed: ${refineryInputs.reason?.message || refineryInputs.reason}`,
		);
	...
	if (ec?.series?.length > 0)
		await writeExtraKeyWithMeta(KEYS.energyCapacity, ec, CAPACITY_TTL, ec.series.length);
	...
	if (ms && !ms.unavailable && ms.totalCount > 0)
		await writeExtraKeyWithMeta(KEYS.macroSignals, ms, MACRO_TTL, ms.totalCount ?? 0);

	const isValidWeek = (w) =>
		typeof w.period === "string" && typeof w.stocksMb === "number" && Number.isFinite(w.stocksMb);
	if (ci?.weeks?.length >= CRUDE_MIN_WEEKS && ci.weeks.every(isValidWeek)) {
		...
		console.warn(
			`  CrudeInventories: skipped write — ${ci.weeks?.length ?? 0} weeks or schema invalid`,
		);
	}

	const isValidNgWeek = (w) =>
		typeof w.period === "string" && typeof w.storBcf === "number" && Number.isFinite(w.storBcf);
	if (ng?.weeks?.length >= NAT_GAS_MIN_WEEKS && ng.weeks.every(isValidNgWeek)) {
		...
		console.warn(
			`  NatGasStorage: skipped write — ${ng.weeks?.length ?? 0} weeks or schema invalid`,
		);
	}

	const isValidSprWeek = (w) =>
		typeof w.period === "string" && typeof w.barrels === "number" && Number.isFinite(w.barrels);
	...
	const isValidRuWeek = (w) =>
		typeof w.period === "string" &&
		typeof w.inputsMbblpd === "number" &&
		Number.isFinite(w.inputsMbblpd);
	if (ru?.weeks?.length >= REFINERY_MIN_WEEKS && ru.weeks.every(isValidRuWeek)) {
		...
		console.warn(
			`  RefineryInputs: skipped write — ${ru.weeks?.length ?? 0} weeks or schema invalid`,
		);
	}
	...
	if (frHasData) {
		...
		if (gscpi) {
			...
			console.warn(
				"  [StressIndex] GSCPI not in Redis yet (ais-relay lag or first run) — excluding",
			);
		}
