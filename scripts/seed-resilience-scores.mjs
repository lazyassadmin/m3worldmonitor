...
async function seedResilienceScores() {
	...
	if (countryCodes.length === 0) {
		console.warn(
			"[resilience-scores] Static index is empty — has seed-resilience-static run this year?",
		);
	...
	for (let i = 0; i < countryCodes.length; i++) {
		...
		try {
			scoreMap.set(countryCodes[i], JSON.parse(raw));
		} catch {
			/* skip malformed */
		}
	}
	...
	if (scored < countryCodes.length) {
		...
		console.warn(
			`[resilience-scores] ${missing} countries missing scores — skipping ranking cache write to avoid pinning incomplete data`,
		);
