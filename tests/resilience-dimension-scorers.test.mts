...
describe("resilience dimension scorers", () => {
	...
	it("produce plausible country ordering for infrastructure and energy", async () => {
		...
		assertOrdered(
			"infrastructure",
			infrastructure.no.score,
			infrastructure.us.score,
			infrastructure.ye.score,
		);
	...
	it("produce plausible country ordering for social, governance, health, and food dimensions", async () => {
		...
		assertOrdered(
			...
			information.no.score,
			information.us.score,
			information.ye.score,
		);
	...
	it("returns all 13 dimensions with bounded scores and coverage", async () => {
		...
		for (const dimensionId of RESILIENCE_DIMENSION_ORDER) {
			...
			assert.ok(
				...
				`${dimensionId} score out of bounds: ${result.score}`,
			);
			assert.ok(
				result.coverage >= 0 && result.coverage <= 1,
				`${dimensionId} coverage out of bounds: ${result.coverage}`,
			);
		}
	});
	...
	it("scoreEnergy with full data uses 7-metric blend and high coverage", async () => {
		...
		assert.ok(
			no.coverage >= 0.85,
			`NO coverage should be >=0.85 with full data, got ${no.coverage}`,
		);
		assert.ok(
			no.score > 50,
			`NO score should be >50 (high renewables, low dependency), got ${no.score}`,
		);
	});
	...
	it("scoreEnergy without OWID mix data degrades gracefully to 4-metric blend", async () => {
		...
		assert.ok(
			no.coverage < 0.75,
			`Coverage should be <0.75 without mix data (3 of 7 metrics), got ${no.coverage}`,
		);
		assert.ok(
			no.score > 0,
			`Score should be non-zero with only iea + electricity data, got ${no.score}`,
		);
	});
	...
	it("Lebanon-like profile: null IEA (Eurostat EU-only gap) + crisis-level electricity → energy < 50", async () => {
		...
		assert.ok(
			score.score < 50,
			`Lebanon energy should be < 50 with crisis-level consumption (null IEA), got ${score.score}`,
		);
	...
	it("scoreTradeSanctions: heavily sanctioned country scores low", async () => {
		...
		assert.ok(
			score.score < 55,
			`RU with 500 designations should score below midpoint, got ${score.score}`,
		);
	});
	...
	it("scoreCurrencyExternal: non-BIS country with no IMF data falls back to curated_list_absent (score 50)", async () => {
		...
		const reader = async (key: string): Promise<unknown | null> => {
			if (key === "economic:bis:eer:v1")
				return { rates: [{ countryCode: "US", realChange: 1.2, realEer: 101, date: "2025-09" }] };
	...
	it("scoreCurrencyExternal: non-BIS country with IMF inflation uses inflation proxy (coverage 0.45)", async () => {
		...
		const reader = async (key: string): Promise<unknown | null> => {
			...
			if (key === "economic:imf:macro:v2")
				return { countries: { MZ: { inflationPct: 8, currentAccountPct: -5, year: 2024 } } };
			return null;
		};
		...
		assert.equal(
			score.coverage,
			0.45,
			"IMF inflation proxy coverage=0.45 (better than pure imputation)",
		);
	});
	...
	it("scoreCurrencyExternal: non-BIS country with hyperinflation is capped at score 0", async () => {
		const reader = async (key: string): Promise<unknown | null> => {
			if (key === "economic:bis:eer:v1")
				return { rates: [{ countryCode: "US", realChange: 1.2, realEer: 101, date: "2025-09" }] };
			if (key === "economic:imf:macro:v2")
				return { countries: { ZW: { inflationPct: 250, currentAccountPct: -8, year: 2024 } } };
			return null;
		};
	...
	it("scoreCurrencyExternal: BIS outage + IMF inflation present → uses proxy with coverage=0.35", async () => {
		...
		const reader = async (key: string): Promise<unknown | null> => {
			if (key === "economic:imf:macro:v2")
				return { countries: { MZ: { inflationPct: 6, currentAccountPct: -2, year: 2024 } } };
		...
		assert.equal(
			score.coverage,
			0.35,
			"BIS outage reduces proxy coverage to 0.35 (primary source unavailable)",
		);
	});
	...
	it("scoreMacroFiscal: IMF current account loaded, surplus country scores higher than deficit", async () => {
		const makeReader =
			...
			async (key: string): Promise<unknown | null> => {
				if (key === "economic:national-debt:v1")
					return { entries: [{ iso3: "HRV", debtToGdp: 70, annualGrowth: 1.5 }] };
				if (key === "economic:imf:macro:v2")
					return {
						countries: {
							HR: { inflationPct: 3.0, currentAccountPct: caPct, govRevenuePct: 40, year: 2024 },
						},
					};
				return null;
			};
		...
		assert.ok(
			surplus.score > deficit.score,
			`surplus (${surplus.score}) must score higher than deficit (${deficit.score})`,
		);
	...
	it("scoreMacroFiscal: IMF macro seed outage does not impute — debt growth still scores", async () => {
		...
		assert.ok(
			score.coverage > 0.15 && score.coverage < 0.25,
	...
	it("scoreFoodWater: country absent from FAO/IPC DB gets crisis_monitoring_absent imputation (not WGI proxy)", async () => {
		...
		assert.equal(
			withWgi.score,
			88,
			"imputed score must be 88 (crisis_monitoring_absent for IPC food)",
		);
		assert.ok(
			withWgi.coverage > 0.3 && withWgi.coverage < 0.6,
		...
		assert.equal(
			withWgi.score,
			withoutWgi.score,
			"score must not change based on WGI presence (imputation is absence-type, not proxy)",
		);
		assert.equal(
			withWgi.coverage,
			withoutWgi.coverage,
			"coverage must not change based on WGI presence",
		);
	});
	...
	it("scoreFoodWater: missing static bundle (seed outage) does not impute as crisis-free", async () => {
		...
		assert.equal(
			score.coverage,
			0,
			`missing static bundle must give coverage=0, got ${score.coverage}`,
		);
	...
	it("scoreBorderSecurity: displacement source loaded but country absent → crisis_monitoring_absent imputation", async () => {
		...
		const reader = async (key: string): Promise<unknown | null> => {
			...
			if (key.startsWith("displacement:summary:v1:"))
				return { summary: { countries: [{ code: "SY", totalDisplaced: 1e6, hostTotal: 5e5 }] } };
			return null;
		};
		...
		assert.ok(
			score.coverage > 0.8,
			`expected coverage >0.8 with source loaded, got ${score.coverage}`,
		);
	});
	...
	it("scoreBorderSecurity: displacement seed outage does not impute", async () => {
		...
		assert.ok(
			score.coverage > 0.6 && score.coverage < 0.7,
