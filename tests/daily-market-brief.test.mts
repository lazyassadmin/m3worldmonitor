		...
		const brief = await buildDailyMarketBrief({
			...
			newsByCategory: {
				markets: [
					...
					makeNewsItem(
						"Microsoft slides as cloud guidance softens",
						"Bloomberg",
				...
				economic: [
					makeNewsItem(
						"Treasury yields steady ahead of inflation data",
						"WSJ",
			...
			summarize: async () => ({
				summary:
					"Risk appetite is mixed, with Apple leading while Microsoft weakens into macro headlines.",
				provider: "openrouter",
				model: "test-model",
				cached: false,
			}),
