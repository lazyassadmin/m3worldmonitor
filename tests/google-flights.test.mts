...
const { searchGoogleFlights } = await import(
	"../server/worldmonitor/aviation/v1/search-google-flights.ts"
);
const { searchGoogleDates } = await import(
...
describe("searchGoogleFlights — multi-airline filtering", () => {
	...
	it("forwards multiple airlines as repeated query params to the relay", async () => {
		...
		assert.deepEqual(
			airlines.sort(),
			["AA", "BA"],
			"each airline should be a separate airlines= param",
		);
	...
	it("handles comma-joined string from codegen (parseStringArray path)", async () => {
		...
		assert.deepEqual(
			airlines.sort(),
			["CX", "UA"],
			"comma-joined string should be split into separate params",
		);
	});
});
	...
	it("sets degraded: true when relay returns partial: true", async () => {
		mockFetch(
			async () =>
				new Response(
					JSON.stringify({
						dates: [{ date: "2026-05-01", return_date: "", price: 450 }],
						partial: true,
					}),
					{ status: 200 },
				),
	...
	it("sets degraded: false when relay returns complete results", async () => {
		mockFetch(
			async () =>
				new Response(
					JSON.stringify({
						dates: [{ date: "2026-06-01", return_date: "", price: 380 }],
						partial: false,
					}),
					{ status: 200 },
				),
