...
describe("oref-locations.ts", () => {
	...
	it("contains known cities", () => {
		...
		assert.ok(
			SRC.includes("'באר שבע'") || SRC.includes("Be'er Sheva") || SRC.includes("Beer Sheva"),
			"should include Beer Sheva",
		);
	});
	...
	it("returns original string when no match found", () => {
		assert.ok(
			SRC.includes("?? hebrew") || SRC.includes("|| hebrew"),
			"should return original on miss",
		);
	});
	...
	it("includes zone translations", () => {
		assert.ok(
			SRC.includes("'גליל עליון'") || SRC.includes("Upper Galilee"),
