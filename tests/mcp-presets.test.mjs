...
describe("MCP Presets — static validation", () => {
	...
	it("all presets have required string fields: name, serverUrl, defaultTool", () => {
		...
		assert.deepEqual(
			missing,
			[],
			`Presets missing required fields: ${missing.map((p) => p.name).join(", ")}`,
		);
	});
	...
	it("all serverUrls use https:// protocol", () => {
		const nonHttps = presets.filter((p) => {
			try {
				return new URL(p.serverUrl).protocol !== "https:";
			} catch {
				return true;
			}
		});
		assert.deepEqual(
			nonHttps,
			[],
			`Presets with non-https serverUrl: ${nonHttps.map((p) => p.name).join(", ")}`,
		);
	});
	...
	it("no serverUrls point to private/RFC1918 hosts", () => {
		...
		assert.deepEqual(
			ssrf,
			[],
			`Presets with private-host serverUrls (SSRF risk): ${ssrf.map((p) => p.name).join(", ")}`,
		);
	});
	...
	it("no known-dead or banned serverUrls", () => {
		...
		assert.deepEqual(
			dead,
			[],
			`Presets using known-dead URLs: ${dead.map((p) => p.name).join(", ")}`,
		);
	});

	it("all serverUrls are parseable URLs", () => {
		const broken = presets.filter((p) => {
			try {
				new URL(p.serverUrl);
				return false;
			} catch {
		...
		assert.deepEqual(
			broken,
			[],
			`Presets with unparseable serverUrls: ${broken.map((p) => p.name).join(", ")}`,
		);
	});
	...
	it("Datadog serverUrl includes /api/unstable/mcp-server/mcp", () => {
		...
		assert.ok(
			dd.serverUrl.includes("/api/unstable/mcp-server/mcp"),
			`Datadog URL is outdated: ${dd.serverUrl}`,
		);
	});
...
describe(`MCP Presets — live connectivity (${LIVE ? "ENABLED" : "SKIPPED — set LIVE_MCP_TESTS=1"})`, {
	skip: !LIVE,
}, () => {
