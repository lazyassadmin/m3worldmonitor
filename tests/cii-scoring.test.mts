...
function emptyAux() {
	return {
		...
		advisories: null as {
			byCountry: Record<string, "do-not-travel" | "reconsider" | "caution">;
		} | null,
		displacedByIso3: {} as Record<string, number>,
		newsTopStories: [] as Array<{
			countryCode: string | null;
			threatLevel: string;
			primaryTitle: string;
		}>,
		threatSummaryByCountry: null as Record<
			string,
...
describe("CII scoring", () => {
	...
	it("UCDP minor conflict floor: composite >= 50", () => {
		...
		assert.ok(
			pk.combinedScore >= 50,
			`PK score ${pk.combinedScore} should be >= 50 with UCDP minor`,
		);
	});
	...
	it("advisory do-not-travel floor: composite >= 60", () => {
		...
		for (const code of ["UA", "SY", "YE", "MM"]) {
			...
			assert.ok(
				s.combinedScore >= 60,
				`${code} score ${s.combinedScore} should be >= 60 (do-not-travel)`,
			);
		}
	});
	...
	it("advisory reconsider floor: composite >= 50", () => {
		...
		for (const code of ["MX", "IR", "PK", "VE", "CU"]) {
			...
			assert.ok(
				s.combinedScore >= 50,
				`${code} score ${s.combinedScore} should be >= 50 (reconsider)`,
			);
		}
	});
	...
	it("OREF active alerts boost IL conflict score", () => {
		...
		assert.ok(
			withOref.combinedScore > withoutOref.combinedScore,
	...
	it("outage TOTAL severity gives higher unrest component than PARTIAL", () => {
		...
		assert.ok(
			total.components!.ciiContribution > partial.components!.ciiContribution,
	...
	it("GPS high level gives higher weight than medium", () => {
		...
		assert.ok(
			high.components!.militaryActivity >= med.components!.militaryActivity,
	...
	it("conflict fatalities use sqrt scaling", () => {
		...
		assert.ok(
			diff < (s400.combinedScore - s100.staticBaseline) * 0.5,
	...
	it("log2 scaling dampens high-volume low-multiplier countries vs linear", () => {
		...
		assert.ok(
			ratio < 5,
			`10x events should produce < 5x unrest ratio (got ${ratio.toFixed(2)}), log2 dampens`,
		);
	});
	...
	it("iran high severity strikes boost conflict", () => {
		...
		assert.ok(
			highSev.combinedScore >= lowSev.combinedScore,
	...
	it("IL scores higher than MX with active conflict signals", () => {
		...
		assert.ok(
			il.combinedScore > mx.combinedScore,
	...
	it("scores capped at 100", () => {
		...
		aux.iranEvents = Array.from({ length: 50 }, () => ({
			lat: 35.0,
			lon: 38.0,
			severity: "critical",
		}));
	...
	it("empty data returns baseline-derived scores with floors", () => {
		...
		assert.ok(
			us.combinedScore >= 2 && us.combinedScore <= 10,
			`US baseline score ${us.combinedScore} should be ~2-10`,
		);
	});
	...
	it("newsTopStories critical threat boosts newsActivity for attributed country", () => {
		...
		assert.ok(
			withNews.components!.newsActivity > 0,
			"newsActivity should be > 0 with critical story",
		);
		assert.ok(
			withNews.combinedScore > withoutNews.combinedScore,
	...
	it("threatSummaryByCountry boosts newsActivity for target country", () => {
		...
		assert.ok(
			withThreat.components!.newsActivity > 0,
			"newsActivity should be > 0 with threat summary",
		);
		assert.ok(
			withThreat.combinedScore > withoutThreat.combinedScore,
	...
	it("newsTopStories newsActivity capped at 20", () => {
		...
		aux.newsTopStories = Array.from({ length: 20 }, () => ({
			...
			primaryTitle: "Syria conflict escalates",
		}));
		const scores = computeCIIScores([], aux);
		const sy = scoreFor(scores, "SY")!;
		assert.ok(
			sy.components!.newsActivity <= 20,
			`newsActivity ${sy.components!.newsActivity} should be capped at 20`,
		);
	});

	it("threatSummaryByCountry newsActivity capped at 20", () => {
		const aux = emptyAux();
		aux.threatSummaryByCountry = {
			SY: { critical: 100, high: 100, medium: 100, low: 100, info: 100 },
		};
		const scores = computeCIIScores([], aux);
		const sy = scoreFor(scores, "SY")!;
		assert.ok(
	...
	it("newsTopStories moderate threat contributes (not silently dropped)", () => {
		...
		assert.ok(
			withNews.components!.newsActivity > 0,
			"moderate threat should produce non-zero newsActivity",
		);
		assert.ok(
			withNews.combinedScore >= withoutNews.combinedScore,
	...
	it("newsTopStories null countryCode falls back to title keyword match", () => {
		...
		aux.newsTopStories = [
			{
				countryCode: null,
				threatLevel: "high",
				primaryTitle: "Iran launches ballistic missile test",
			},
		];
		...
		assert.ok(
			withNews.components!.newsActivity > 0,
			"null countryCode with Iran keyword should attribute to IR",
		);
		assert.ok(
			withNews.components!.newsActivity > withoutNews.components!.newsActivity,
	...
	it("newsTopStories info threat is not counted", () => {
		...
		assert.equal(
			withInfo.components!.newsActivity,
			withoutNews.components!.newsActivity,
	...
	it("null threatSummaryByCountry produces zero newsActivity", () => {
		...
		for (const s of scores) {
			assert.equal(
				s.components!.newsActivity,
				0,
				`${s.region} should have zero newsActivity with null threatSummary`,
			);
		}
	});
});
