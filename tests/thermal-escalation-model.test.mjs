...
describe("thermal escalation model", () => {
	...
	it("builds an elevated or stronger conflict-adjacent cluster from raw detections", () => {
		...
		const detections = [
			...
			makeDetection("b", 50.46, 30.53, nowMs - 80 * 60 * 1000, {
				frp: 42,
		...
		assert.ok(
			["THERMAL_STATUS_ELEVATED", "THERMAL_STATUS_SPIKE", "THERMAL_STATUS_PERSISTENT"].includes(
				cluster.status,
			),
		);
