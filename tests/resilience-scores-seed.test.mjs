...
describe("sortRankingItems", () => {
	it("sorts by overallScore descending", () => {
		...
		assert.deepEqual(
			sorted.map((i) => i.countryCode),
			["NO", "DE", "YE"],
		);
		assert.deepEqual(
			sorted.map((i) => i.overallScore),
			[80, 55, 20],
		);
	});
...
describe("exported constants", () => {
	it("RESILIENCE_RANKING_CACHE_KEY matches server-side key", () => {
		assert.equal(RESILIENCE_RANKING_CACHE_KEY, "resilience:ranking");
	});

	it("RESILIENCE_SCORE_CACHE_PREFIX matches server-side prefix", () => {
		assert.equal(RESILIENCE_SCORE_CACHE_PREFIX, "resilience:score:");
	});
