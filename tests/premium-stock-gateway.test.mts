...
describe("premium gateway API key enforcement", () => {
	it("requires credentials for premium endpoints regardless of origin", async () => {
		...
		const browserNoKey = await handler(
		...
		const resilienceScoreNoKey = await handler(
			new Request(
				"https://worldmonitor.app/api/resilience/v1/get-resilience-score?countryCode=US",
				{
					headers: { Origin: "https://worldmonitor.app" },
				},
			),
		...
		const resilienceRankingNoKey = await handler(
		...
		const browserWithKey = await handler(
			new Request("https://worldmonitor.app/api/market/v1/analyze-stock?symbol=AAPL", {
				headers: {
		...
		const resilienceScoreWithKey = await handler(
			new Request(
				"https://worldmonitor.app/api/resilience/v1/get-resilience-score?countryCode=US",
				{
					headers: {
		...
		const resilienceRankingWithKey = await handler(
			new Request("https://worldmonitor.app/api/resilience/v1/get-resilience-ranking", {
				headers: {
					Origin: "https://worldmonitor.app",
					"X-WorldMonitor-Key": "real-key-123",
		...
		const unknownNoKey = await handler(
			new Request("https://external.example.com/api/market/v1/analyze-stock?symbol=AAPL", {
		...
		const publicAllowed = await handler(
...
describe("premium gateway bearer token auth", () => {
	...
	function signToken(
		...
		opts?: { key?: CryptoKey; audience?: string },
	) {
	...
	it("rejects free bearer token on resilience premium endpoints → 403", async () => {
		...
		const rankingRes = await handler(
			new Request("https://worldmonitor.app/api/resilience/v1/get-resilience-ranking", {
				headers: {
					Origin: "https://worldmonitor.app",
					Authorization: `Bearer ${token}`,
