			...
			if (parsed.pathname === "/pipeline") {
				...
				const result = commands.map((command) => {
					...
					if (verb === "ZREM") {
						...
						sortedSets.set(
							key,
							(sortedSets.get(key) ?? []).filter((item) => !removals.has(item.member)),
						);
...
describe("stock analysis history helpers", () => {
	it("merges snapshots per symbol, dedupes identical runs, and caps retained history", () => {
		...
		assert.deepEqual(
			...
			["2026-03-09T10:00:00.000Z", "2026-03-08T10:00:00.000Z", "2026-03-07T10:00:00.000Z"],
		);
	...
	it("returns the latest snapshot per symbol ordered by recency", () => {
		const history = {
			...
			AAPL: [makeSnapshot("AAPL", "2026-03-08T09:00:00.000Z", 74)],
			MSFT: [makeSnapshot("MSFT", "2026-03-07T09:00:00.000Z", 69)],
		};
