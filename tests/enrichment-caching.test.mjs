...
describe("cachedFetchJson — null path (fetch failure)", () => {
	...
	it("returns null when fetcher returns null", async () => {
		const result = await cachedFetchJson("intel:enrichment:gh-org:testco", 3600, async () => null);
	...
	it("subsequent call returns null from NEG_SENTINEL (no fetcher call)", async () => {
		...
		const result = await cachedFetchJson("intel:enrichment:gh-org:testco", 3600, async () => {
			fetcherCalled = true;
			return { name: "should not get here" };
		});
		assert.equal(result, null);
...
describe("cachedFetchJson — empty array (successful empty result)", () => {
	...
	it("returns [] when fetcher returns []", async () => {
		const result = await cachedFetchJson("intel:enrichment:hn:emptyco", 1800, async () => []);
...
describe("cachedFetchJson — cache hit skips upstream fetch", () => {
	...
	it("returns cached data without calling fetcher", async () => {
		let fetcherCalled = false;
		const result = await cachedFetchJson("intel:enrichment:gh-org:stripe", 3600, async () => {
			fetcherCalled = true;
			return { name: "WRONG" };
		});
...
describe("cachedFetchJson — cache key encoding", () => {
	...
	it("encodes special chars in company names", async () => {
		await cachedFetchJson(`intel:enrichment:hn:${encodeURIComponent("at&t")}`, 1800, async () => [
			{ title: "AT&T news", url: "https://example.com", points: 10, comments: 5, createdAtMs: 0 },
		]);
	...
	it("lowercased names produce consistent keys", async () => {
		...
		await cf2(`intel:enrichment:gh-org:${encodeURIComponent("stripe")}`, 3600, async () => ({
			name: "Stripe",
		}));
		await cf2(
			`intel:enrichment:gh-org:${encodeURIComponent("STRIPE".toLowerCase())}`,
			3600,
			async () => ({ name: "WRONG" }),
		);
...
describe("cachedFetchJson — import verification", () => {
	it("get-company-enrichment.ts imports cachedFetchJson", async () => {
		...
		const src = readFileSync(
			resolve("server/worldmonitor/intelligence/v1/get-company-enrichment.ts"),
			"utf-8",
		);
	...
	it("list-company-signals.ts imports cachedFetchJson", async () => {
		...
		const { resolve } = await import("node:path");
		const src = readFileSync(
			resolve("server/worldmonitor/intelligence/v1/list-company-signals.ts"),
			"utf-8",
		);
		assert.ok(src.includes("from '../../../_shared/redis'"), "must import from _shared/redis");
		assert.ok(src.includes("cachedFetchJson"), "must use cachedFetchJson");
