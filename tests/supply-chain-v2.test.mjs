...
describe("Chokepoint handler v2 changes", () => {
	...
	it("uses 5-minute Redis TTL", () => {
		assert.match(src, /REDIS_CACHE_TTL\s*=\s*300/, "Chokepoint Redis TTL should be 300s (5 min)");
	});
	...
	it("description does not duplicate warning/disruption counts (use structured fields)", () => {
		assert.doesNotMatch(
			src,
			/descriptions\.push\(`Navigational warnings:/,
			"Warning counts should not be in description text (use activeWarnings field)",
		);
		assert.doesNotMatch(
			src,
			/descriptions\.push\(`AIS vessel disruptions:/,
	...
	it('description does not use vague "AIS congestion detected" phrasing', () => {
		assert.doesNotMatch(src, /AIS congestion detected/, "Old vague description removed");
	});
...
describe("Minerals handler v2 changes", () => {
	...
	it("slices to top 3 producers (not 5)", () => {
		assert.match(handlerSrc, /\.slice\(0,\s*3\)/, "Should slice top producers to 3");
		assert.doesNotMatch(handlerSrc, /\.slice\(0,\s*5\)/, "Old slice(0,5) should be removed");
	});
...
describe("Gateway daily cache tier", () => {
	...
	it("daily tier has 86400s s-maxage", () => {
		assert.match(src, /daily.*s-maxage=86400/, "daily tier should have s-maxage=86400");
	});
...
describe("Chokepoint threat level config", () => {
	...
	it("every entry has required fields", () => {
		for (const cp of CHOKEPOINTS) {
			...
			assert.ok(
				["war_zone", "critical", "high", "elevated", "normal"].includes(cp.threatLevel),
	...
	it("THREAT_CONFIG_LAST_REVIEWED is a valid ISO date string", () => {
		...
		assert.ok(
			!Number.isNaN(Date.parse(THREAT_CONFIG_LAST_REVIEWED)),
