...
function loadRelayFunctions() {
	...
	assert.ok(
		src.includes("replace(/^ws(s?):\\/\\//"),
		"relay.ts must use single-regex wss:// transform",
	);
	assert.ok(src.includes("...extra"), "relay.ts must spread extra before auth headers");
	assert.ok(
		src.includes("relayHeader !== 'authorization'"),
		"relay.ts must guard against Authorization header collision",
	);
...
describe("getRelayHeaders — with auth", () => {
	...
	it("respects custom RELAY_AUTH_HEADER", () => {
		withEnv({ RELAY_AUTH_HEADER: "x-wm-relay" }, () => {
			...
			assert.equal(
				h["x-relay-key"],
				undefined,
				"default header should not be set when custom header used",
			);
		});
	});
	...
	it("auth headers cannot be overridden via extra (auth set after spread)", () => {
		...
		assert.equal(
			...
			"auth should not be overridable via extra",
		);
	});
	...
	it("RELAY_AUTH_HEADER=Authorization: sets only the direct secret, no Bearer duplicate", () => {
		withEnv({ RELAY_AUTH_HEADER: "Authorization" }, () => {
			...
			assert.equal(
				h["Authorization"],
				undefined,
				"Bearer duplicate must NOT be set when relayHeader === authorization",
			);
		});
	});
});
...
describe("relay.ts — consumer import verification", () => {
	const consumers = [
		...
		{
			file: "server/worldmonitor/research/v1/list-tech-events.ts",
			importPath: "../../../_shared/relay",
		},
		{
			file: "server/worldmonitor/maritime/v1/get-vessel-snapshot.ts",
			importPath: "../../../_shared/relay",
		},
		...
		{
			file: "server/worldmonitor/military/v1/list-military-flights.ts",
			importPath: "../../../_shared/relay",
		},
	];
	...
	it("military list-military-flights.ts uses getRelayBaseUrl() not raw WS_RELAY_URL + /opensky", () => {
		const src = readFileSync(
			resolve("server/worldmonitor/military/v1/list-military-flights.ts"),
			"utf-8",
		);
		assert.ok(
