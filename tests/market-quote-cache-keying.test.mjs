...
describe("CircuitBreaker keyed cache — market quote isolation", () => {
	it("caches different symbol sets independently within one breaker", async () => {
		...
		try {
			...
			const cachedTech = await breaker.execute(async () => fallback, fallback, {
				cacheKey: "AAPL,MSFT,NVDA",
			});
			const cachedMetals = await breaker.execute(async () => fallback, fallback, {
				cacheKey: "GLD,SLV",
			});
	...
	it("evicts least-recently-used entries when maxCacheEntries is reached", async () => {
		...
		try {
			...
			assert.equal(
				(await breaker.execute(async () => fallback, fallback, { cacheKey: "B" })).quotes[0]
					?.symbol,
				"B",
			);
	...
	it("stale SWR hits still count as used for LRU before refresh completes", async () => {
		...
		try {
			...
			assert.equal(
				keysBeforeRefresh.includes("A"),
				true,
				"served stale key A should stay resident",
			);
			assert.equal(
				keysBeforeRefresh.includes("B"),
				false,
				"B should be evicted after A is promoted by the stale hit",
			);
	...
	it("SWR background refresh is per-key (does not block other keys)", async () => {
		...
		try {
			...
			await breaker.execute(async () => quoteResponse("GLD", 300), fallback, {
				cacheKey: "METALS",
			});
			...
			const techResult = await breaker.execute(
				async () => {
					techRefreshCalled = true;
					return quoteResponse("AAPL", 155);
				},
				fallback,
				{ cacheKey: "TECH" },
			);
			const metalsResult = await breaker.execute(
				async () => {
					metalsRefreshCalled = true;
					return quoteResponse("GLD", 305);
				},
				fallback,
				{ cacheKey: "METALS" },
			);
	...
	it("SWR background refresh respects shouldCache predicate", async () => {
		...
		try {
			...
			await breaker.execute(async () => quoteResponse("GC=F", 2800), fallback, {
				cacheKey: "COMMODITY",
			...
			await breaker.execute(async () => emptyMarketFallback(), fallback, {
				cacheKey: "COMMODITY",
				shouldCache: (r) => r.quotes.length > 0,
			});
	...
	it("empty responses are not cached when shouldCache rejects them (P1)", async () => {
		...
		try {
			...
			const result = await breaker.execute(async () => emptyMarketFallback(), fallback, {
				cacheKey: "GC=F,CL=F",
				shouldCache: (r) => r.quotes.length > 0,
			});
			...
			await breaker.execute(
				async () => {
					...
					return quoteResponse("GC=F", 2880);
				},
				fallback,
	...
	it("non-cacheable successes still reset failures (P2)", async () => {
		...
		try {
			...
			assert.ok(
				...
				"two consecutive failures after reset should trip cooldown",
			);
		} finally {
			clearAllCircuitBreakers();
		}
