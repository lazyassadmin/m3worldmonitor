...
describe("resolveCascades", () => {
	it("conflict near chokepoint creates supply_chain and market cascades", () => {
		const pred = makePrediction(
			...
			0.6,
			"7d",
			[{ type: "cii", value: "Iran CII 85", weight: 0.4 }],
		);
	...
	it("cascade probabilities capped at 0.8", () => {
		const pred = makePrediction(
			...
			0.9,
			"7d",
			[{ type: "cii", value: "high", weight: 0.4 }],
		);
	...
	it("no self-edges: cascade domain differs from source domain", () => {
		...
		for (const c of pred.cascades) {
			assert.notEqual(
				c.domain,
				pred.domain,
				`cascade domain ${c.domain} should differ from source ${pred.domain}`,
			);
		}
	});

	it("political > 0.6 creates conflict cascade", () => {
		const pred = makePrediction("political", "Iran", "Political instability", 0.65, 0.5, "30d", [
			{ type: "unrest", value: "unrest", weight: 0.4 },
		]);
		resolveCascades([pred], DEFAULT_CASCADE_RULES);
		const domains = pred.cascades.map((c) => c.domain);
	...
	it("political <= 0.6 does not cascade to conflict", () => {
		const pred = makePrediction("political", "Iran", "Political instability", 0.5, 0.5, "30d", [
			{ type: "unrest", value: "unrest", weight: 0.4 },
		]);
		resolveCascades([pred], DEFAULT_CASCADE_RULES);
...
describe("calibrateWithMarkets", () => {
	it("matching market adjusts probability with 40/60 blend", () => {
		...
		const markets = {
			geopolitical: [
				{ title: "Will Iran conflict escalate in MENA?", yesPrice: 30, source: "polymarket" },
			],
		};
	...
	it("does not calibrate from unrelated same-region macro market", () => {
		...
		const markets = {
			geopolitical: [
				{
					title: "Will Netanyahu remain prime minister through 2026?",
					yesPrice: 20,
					source: "polymarket",
					volume: 100000,
				},
	...
	it("does not calibrate commodity forecasts from loosely related regional conflict markets", () => {
		...
		const markets = {
			geopolitical: [
				{
					title: "Will Israel launch a major ground offensive in Lebanon by March 31?",
					yesPrice: 57,
					source: "polymarket",
					volume: 100000,
				},
...
describe("detectConflictScenarios", () => {
	...
	it("critical theater posture produces prediction", () => {
		const inputs = {
			ciiScores: [],
			theaterPosture: {
				theaters: [{ id: "iran-theater", name: "Iran Theater", postureLevel: "critical" }],
			},
			iranEvents: [],
			ucdpEvents: [],
		};
	...
	it("accepts theater posture entries that use theater instead of id", () => {
		const inputs = {
			ciiScores: [],
			theaterPosture: {
				theaters: [{ theater: "taiwan-theater", name: "Taiwan Theater", postureLevel: "elevated" }],
			},
			iranEvents: [],
			ucdpEvents: [],
		};
...
describe("detectInfraScenarios", () => {
	...
	it("cyber threats boost probability", () => {
		...
		assert.ok(
			cyberResult[0].probability > baseResult[0].probability,
	...
	it("accepts live theater entries that use theater instead of id", () => {
		const inputs = {
			militaryForecastInputs: {
				fetchedAt: Date.now(),
				theaters: [{ theater: "baltic-theater", postureLevel: "critical", activeFlights: 12 }],
			},
			temporalAnomalies: { anomalies: [] },
		};
...
describe("attachNewsContext", () => {
	...
	it("reads headlines from digest categories (primary path)", () => {
		...
		const digest = {
			categories: {
				middleeast: {
					items: [{ title: "Iran launches missile test" }, { title: "Saudi oil output stable" }],
				},
	...
	it("prefers region-relevant headlines over generic domain-only matches", () => {
		const preds = [
			makePrediction("supply_chain", "Red Sea", "Shipping disruption: Red Sea", 0.6, 0.4, "7d", []),
		];
		const news = {
			topStories: [
	...
	it("rejects domain-only headlines with no geographic grounding", () => {
		const preds = [
			makePrediction(
				"military",
				"Northern Europe",
				"Military posture escalation: Northern Europe",
				0.6,
				0.4,
				"7d",
				[],
...
describe("headline and market relevance helpers", () => {
	it("scores region-specific headlines above generic domain headlines", () => {
		...
		const specific = computeHeadlineRelevance(
			"Red Sea shipping disruption worsens after new attacks",
			terms,
			"supply_chain",
		);
		const generic = computeHeadlineRelevance(
			"Global shipping shares rise in New York trading",
			terms,
			"supply_chain",
		);
	...
	it("scores semantically aligned markets above broad regional ones", () => {
		...
		const targeted = computeMarketMatchScore(pred, "Will Iran conflict escalate before July?", [
			"Iran",
			"Middle East",
		]);
		const broad = computeMarketMatchScore(
			pred,
			"Will Netanyahu remain prime minister through 2026?",
			["Iran", "Middle East"],
		);
...
describe("forecast case assembly", () => {
	...
	it("helper functions return structured case ingredients", () => {
		const pred = makePrediction(
			"supply_chain",
			"Red Sea",
			"Supply chain disruption: Red Sea",
			0.64,
			0.35,
			"7d",
			[
...
describe("forecast evaluation and ranking", () => {
	...
	it("penalizes thin forecasts with weak grounding even at similar base probability", () => {
		const grounded = makePrediction(
			"political",
			"France",
			"Political instability: France",
			0.64,
			0.57,
			"7d",
			[
	...
	it("filters non-positive forecasts before publish while keeping positive probabilities", () => {
		const dropped = makePrediction(
			"market",
			"Red Sea",
			"Shipping/Oil price impact from Suez Canal disruption",
			0,
			0.58,
			"30d",
			[],
	...
	it("selects enrichment targets from a broader, domain-balanced top slice", () => {
		...
		const cyberA = makePrediction("cyber", "China", "Cyber A", 0.69, 0.58, "7d", [
			{ type: "cyber", value: "Hostile malware hosting remains elevated", weight: 0.4 },
			{
				...
				value: "Security firms warn of sustained activity",
				weight: 0.2,
			},
		]);
		...
		const cyberB = makePrediction("cyber", "Russia", "Cyber B", 0.67, 0.56, "7d", [
			...
			{
				type: "news_corroboration",
				value: "Government agencies issue new advisories",
				weight: 0.2,
			},
		]);
...
describe("forecast change tracking", () => {
	it("builds prior snapshots with enough context for evidence diffs", () => {
		...
		assert.deepEqual(snapshot.newsContext, [
	...
	it("buildPublishedSeedPayload strips simulation-only forecast bulk from the canonical payload", () => {
		...
		pred.caseFile.worldState = {
			...
			simulationSummary:
				"Heavy simulation linkage that should stay out of the canonical Redis payload.",
	...
	it("annotates what changed versus the prior run", () => {
		...
		assert.ok(
			pred.caseFile.changeItems.some((item) => item.includes("New signal: 3 UCDP conflict events")),
		);
		assert.ok(
			pred.caseFile.changeItems.some((item) =>
				item.includes("New reporting: Regional officials warn of retaliation risk"),
			),
		);
		assert.ok(
			pred.caseFile.changeItems.some((item) => item.includes("Market moved from 53% to 64%")),
		);
	});
...
describe("forecast llm overrides", () => {
	it("parses provider order safely", () => {
		...
		assert.deepEqual(parseForecastProviderOrder("openrouter, groq, openrouter, invalid"), [
			"openrouter",
			"groq",
		]);
	});
...
describe("forecast narrative fallbacks", () => {
	...
	it("fallback scenario stays concise and evidence-led", () => {
		const pred = makePrediction(
			"infrastructure",
			"France",
			"Infrastructure cascade risk: France",
			0.48,
			0.4,
			"24h",
	...
	it("buildFeedSummary preserves the full narrative without server-side clipping", () => {
		...
		buildForecastCase(pred);
		pred.caseFile.baseCase =
			"Iran CII 87 (critical) and 3 UCDP conflict events keep the base path elevated over the next 7d with persistent force pressure and increasingly visible cross-border signaling, while regional actors still avoid a decisive break into a wider confrontation.";
		const summary = buildFeedSummary(pred);
	...
	it("refreshPublishedNarratives preserves validated llm narratives and only fills gaps", () => {
		const pred = makePrediction(
			...
			[
				{
					type: "shipping_cost_shock",
					value: "Strait of Hormuz shipping costs remain elevated",
					weight: 0.42,
				},
			],
		);
		...
		pred.traceMeta = { narrativeSource: "llm_combined", llmProvider: "openrouter" };
		pred.caseFile.baseCase =
			"LLM base case keeps Hormuz freight and energy repricing tied to persistent shipping disruption over the next 30d.";
		pred.caseFile.escalatoryCase =
			"LLM escalatory case sees a sharper repricing if maritime insurance and rerouting costs jump again.";
		pred.caseFile.contrarianCase =
			"LLM contrarian case assumes corridor access stabilizes before the freight shock spreads further.";
		pred.scenario =
			"LLM scenario keeps Hormuz inflation pressure elevated while the corridor remains contested.";
		pred.feedSummary = "";
	...
	it("rebuilds deterministic feed summaries from enriched scenarios instead of leaving fallback phrasing in place", () => {
		const pred = makePrediction(
			"market",
			"Strait of Hormuz",
			"Energy repricing risk from Strait of Hormuz maritime disruption state",
			0.68,
			0.63,
			"30d",
			[
				{
					type: "shipping_cost_shock",
					value: "Hormuz freight costs remain elevated.",
					weight: 0.4,
				},
			],
		);
		...
		pred.scenario =
			"LLM scenario keeps Hormuz energy and freight stress elevated as the corridor stays contested and downstream importers continue to hedge against extended rerouting pressure.";
			...
			[
				{
					...
					escalatoryCase:
						"A further rise in Iran CII 87 and added conflict-event reporting would move risk materially higher.",
					contrarianCase:
						"If no new corroborating headlines appear, the current path would lose support and flatten out.",
				},
			],
			[pred],
		);
	...
	it("accepts partial case narratives when at least one branch is substantive", () => {
		const pred = makePrediction(
			...
			[
				{
					type: "fx_stress",
					value: "India cyber pressure state is keeping FX stress active",
					weight: 0.42,
				},
			],
		);
			...
			[
				{
					index: 0,
					baseCase:
						"India cyber pressure state remains the clearest anchor for the current FX stress base case over the next 30d.",
				},
			],
			[pred],
		);
...
describe("sanitizeForPrompt", () => {
	it("strips HTML tags", () => {
		assert.equal(
			sanitizeForPrompt('<script>alert("xss")</script>hello'),
...
describe("parseLLMScenarios", () => {
	...
	it("strips thinking tags before parsing", () => {
		const result = parseLLMScenarios(
			'<think>reasoning here</think>[{"index": 0, "scenario": "Test"}]',
		);
		assert.equal(result.length, 1);
	});
...
describe("validateScenarios", () => {
		...
		const scenarios = [
			{
				index: 0,
				scenario:
					"The Iran CII score of 87 indicates critical instability in the region, driven by ongoing military activity.",
			},
		];
		...
		const scenarios = [
			{
				index: 0,
				scenario:
					"Iran military drills intensify after border incident, keeping escalation pressure elevated over the next 7d.",
			},
		];
		const valid = validateScenarios(scenarios, preds);
		assert.equal(valid.length, 1);
		...
		const scenarios = [
			{
				index: 0,
				scenario:
					"Will oil close above $90? remains a live market cue, and a market repricing of 8-10 points would confirm the current path.",
			},
		];
		const valid = validateScenarios(scenarios, preds);
		assert.equal(valid.length, 1);
		...
		const scenarios = [
			{
				index: 0,
				scenario:
					"India cyber pressure state remains the clearest anchor for the current FX stress path over the next 30d.",
			},
		];
		const valid = validateScenarios(scenarios, preds);
		assert.equal(valid.length, 1);
		...
		const scenarios = [
			{
				index: 0,
				scenario:
					"Tensions continue to rise in the region due to various geopolitical factors and ongoing disputes.",
			},
		];
		const valid = validateScenarios(scenarios, preds);
	...
	it("rejects out-of-bounds index", () => {
		const scenarios = [
			{ index: 5, scenario: "Iran CII 87 indicates critical instability in the region." },
		];
		const valid = validateScenarios(scenarios, preds);
		assert.equal(valid.length, 0);
	});
		...
		const scenarios = [
			{
				index: 0,
				scenario:
					"The Iran CII score of 87 <b>critical</b> indicates instability in the conflict zone region.",
			},
		];
		const valid = validateScenarios(scenarios, preds);
		assert.equal(valid.length, 1);
		...
		const items = [
			{
				index: 0,
				strategic:
					"The CII data shows critical instability with a score of 87 in the conflict region.",
				regional: "Regional actors face mounting pressure from the elevated CII threat level.",
				contrarian:
					"Despite CII readings, diplomatic channels remain open and could defuse tensions.",
			},
		];
...
describe("detectFromPredictionMarkets", () => {
	it("generates from 60-90% markets with region", () => {
		const markets = {
			geopolitical: [{ title: "Will Iran strike Israel?", yesPrice: 70, source: "polymarket" }],
		};
	...
	it("caps at 5 predictions", () => {
		const markets = {
			geopolitical: Array.from({ length: 10 }, (_, i) => ({
				title: `Will Europe face crisis ${i}?`,
				yesPrice: 70,
			})),
		};
...
describe("lowered CII conflict threshold", () => {
	it("CII score 67 (high level) now triggers conflict", () => {
		const result = detectConflictScenarios({
			ciiScores: {
				ciiScores: [
					{ region: "IL", combinedScore: 67, trend: "TREND_DIRECTION_STABLE", components: {} },
				],
			},
			theaterPosture: { theaters: [] },
			iranEvents: { events: [] },
	...
	it("CII score 62 (elevated level) does NOT trigger conflict", () => {
		const result = detectConflictScenarios({
			ciiScores: {
				ciiScores: [
					{ region: "JO", combinedScore: 62, trend: "TREND_DIRECTION_RISING", components: {} },
				],
			},
			theaterPosture: { theaters: [] },
			iranEvents: { events: [] },
...
describe("discoverGraphCascades", () => {
	it("finds linked predictions via graph", () => {
		...
		assert.ok(
			irCascades.length > 0 || preds[1].cascades.length > 0,
			"should find graph cascade between Iran and Middle East",
		);
	});
...
describe("forecast quality gating", () => {
	it("reserves scenario enrichment slots for scarce market and military forecasts", () => {
		const predictions = [
			makePrediction("cyber", "A", "Cyber A", 0.7, 0.55, "7d", [
				{ type: "cyber", value: "8 threats", weight: 0.5 },
			]),
			makePrediction("cyber", "B", "Cyber B", 0.68, 0.55, "7d", [
				{ type: "cyber", value: "7 threats", weight: 0.5 },
			]),
			makePrediction("conflict", "C", "Conflict C", 0.66, 0.6, "7d", [
				{ type: "ucdp", value: "12 events", weight: 0.5 },
			]),
			makePrediction("market", "Middle East", "Oil price impact", 0.4, 0.5, "30d", [
				{ type: "news_corroboration", value: "Oil traders react", weight: 0.3 },
			]),
			makePrediction(
				"military",
				"Korean Peninsula",
				"Elevated military air activity",
				0.34,
				0.5,
				"7d",
				[{ type: "mil_surge", value: "fighter surge", weight: 0.4 }],
			),
		];
		buildForecastCases(predictions);
		const selected = selectForecastsForEnrichment(predictions, {
			maxCombined: 2,
			maxScenario: 2,
			maxPerDomain: 2,
			minReadiness: 0,
	...
	it("filters only the weakest fallback forecasts from publish output", () => {
		const weak = makePrediction(
			"cyber",
			"Thinland",
			"Cyber threat concentration: Thinland",
			0.11,
			0.32,
			"7d",
	...
	it("suppresses weaker duplicate-like conflict forecasts while preserving distinct consequences", () => {
		...
		const duplicate = makePrediction(
			"conflict",
			"Iran",
			"Active armed conflict: Iran",
			0.52,
			0.42,
			"7d",
			[
		...
		const consequence = makePrediction(
			"market",
			"Middle East",
			"Oil price impact from Strait of Hormuz disruption",
			0.41,
		...
		const distinctConflict = makePrediction(
			"conflict",
			"Gulf",
			"Spillover conflict risk: Gulf shipping corridor",
			0.47,
			0.53,
			"14d",
			[
				{
					type: "news_corroboration",
					value: "Gulf states prepare for possible spillover",
					weight: 0.35,
				},
			],
		);
		...
		primary.caseFile.situationContext = {
			id: "sit-1",
			label: "Iran conflict pressure",
			forecastCount: 3,
			topSignals: [{ type: "ucdp", count: 2 }],
		};
		duplicate.caseFile.situationContext = {
			id: "sit-1",
			label: "Iran conflict pressure",
			forecastCount: 3,
			topSignals: [{ type: "ucdp", count: 2 }],
		};
		consequence.caseFile.situationContext = {
			id: "sit-1",
			label: "Iran conflict pressure",
			forecastCount: 3,
			topSignals: [{ type: "ucdp", count: 2 }],
		};
		distinctConflict.caseFile.situationContext = {
			id: "sit-2",
			label: "Gulf spillover pressure",
			forecastCount: 1,
		...
		const telemetry = summarizePublishFiltering([
			primary,
			duplicate,
			consequence,
			distinctConflict,
	...
	it("caps dominant same-domain situation output while preserving cross-domain consequences", () => {
		const conflictA = makePrediction(
			"conflict",
			"Iran",
			"Escalation risk: Iran",
			0.66,
		...
		const conflictB = makePrediction(
			"conflict",
			"Gulf",
			"Spillover conflict risk: Gulf shipping corridor",
			0.61,
		...
		const conflictC = makePrediction(
			"conflict",
			"Israel",
			"Retaliatory conflict risk: Israel",
			0.58,
			0.53,
			"14d",
			[
				{
					type: "news_corroboration",
					value: "Retaliatory pressure remains elevated around Israel",
					weight: 0.42,
				},
			],
		);
	...
	it("does not suppress same-domain forecasts as duplicates when they belong to different situation families", () => {
		const iranConflict = makePrediction(
			"conflict",
			"Iran",
			"Escalation risk: Iran",
			0.67,
		...
		iranConflict.situationContext = {
			id: "sit-iran",
			label: "Iran conflict situation",
			forecastCount: 1,
			topSignals: [{ type: "ucdp", count: 1 }],
		};
		brazilConflict.situationContext = {
			id: "sit-brazil",
		...
		iranConflict.familyContext = {
			id: "fam-middle-east",
			label: "Middle East conflict pressure family",
			situationCount: 1,
			forecastCount: 1,
		};
		brazilConflict.familyContext = {
			id: "fam-brazil",
			label: "Brazil conflict pressure family",
			situationCount: 1,
			forecastCount: 1,
		};
	...
	it("caps dominant family output while preserving family diversity", () => {
		const preds = [
			makePrediction("conflict", "Iran", "Escalation risk: Iran", 0.69, 0.62, "7d", [
				{ type: "ucdp", value: "Iran events remain elevated", weight: 0.4 },
			]),
			makePrediction("political", "Iran", "Political instability: Iran", 0.56, 0.56, "14d", [
				{ type: "news_corroboration", value: "Emergency cabinet talks continue", weight: 0.35 },
			]),
			makePrediction("market", "Middle East", "Oil price impact: Middle East", 0.53, 0.55, "30d", [
				{ type: "prediction_market", value: "Oil repricing persists", weight: 0.3 },
			]),
			makePrediction(
				"supply_chain",
				"Persian Gulf",
				"Shipping disruption: Persian Gulf",
				0.51,
				0.54,
				"14d",
				[{ type: "chokepoint", value: "Shipping reroutes persist", weight: 0.35 }],
			),
			makePrediction("infrastructure", "Iran", "Infrastructure strain: Iran", 0.49, 0.53, "14d", [
				{
					type: "news_corroboration",
					value: "Grid strain and outages remain elevated",
					weight: 0.32,
				},
			]),
			makePrediction("conflict", "Brazil", "Escalation risk: Brazil", 0.63, 0.58, "7d", [
				{ type: "ucdp", value: "Brazil conflict remains active", weight: 0.42 },
			]),
		];
		...
		preds[0].situationContext = {
			id: "sit-iran-conflict",
			label: "Iran conflict situation",
			forecastCount: 1,
			topSignals: [{ type: "ucdp", count: 1 }],
		};
		preds[1].situationContext = {
			id: "sit-iran-political",
			label: "Iran political situation",
			forecastCount: 1,
			topSignals: [{ type: "news_corroboration", count: 1 }],
		};
		preds[2].situationContext = {
			id: "sit-middleeast-market",
			label: "Middle East market situation",
			forecastCount: 1,
			topSignals: [{ type: "prediction_market", count: 1 }],
		};
		preds[3].situationContext = {
			id: "sit-gulf-shipping",
			label: "Persian Gulf supply chain situation",
			forecastCount: 1,
			topSignals: [{ type: "chokepoint", count: 1 }],
		};
		preds[4].situationContext = {
			id: "sit-iran-infra",
			label: "Iran infrastructure situation",
			forecastCount: 1,
			topSignals: [{ type: "news_corroboration", count: 1 }],
		};
		preds[5].situationContext = {
			id: "sit-brazil-conflict",
			label: "Brazil conflict situation",
			forecastCount: 1,
			topSignals: [{ type: "ucdp", count: 1 }],
		};
	...
	it("preselects published forecasts across families before overlap suppression", () => {
		const preds = [
			makePrediction("conflict", "Iran", "Escalation risk: Iran", 0.72, 0.65, "7d", [
				{ type: "ucdp", value: "Iran events elevated", weight: 0.4 },
			]),
			makePrediction("political", "Iran", "Political instability: Iran", 0.58, 0.59, "14d", [
				{ type: "news_corroboration", value: "Emergency meetings continue", weight: 0.35 },
			]),
			makePrediction("market", "Middle East", "Oil repricing risk: Gulf", 0.55, 0.57, "30d", [
				{ type: "prediction_market", value: "Oil reprices higher", weight: 0.3 },
			]),
			makePrediction(
				"supply_chain",
				"Persian Gulf",
				...
				0.56,
				"14d",
				[{ type: "chokepoint", value: "Routing delays persist", weight: 0.35 }],
			),
			makePrediction("conflict", "Ukraine", "Escalation risk: Ukraine", 0.64, 0.61, "7d", [
				{ type: "ucdp", value: "Ukraine conflict remains active", weight: 0.42 },
			]),
			makePrediction("market", "Black Sea", "Grain pricing pressure: Black Sea", 0.5, 0.54, "30d", [
				{ type: "prediction_market", value: "Grain risk premium widens", weight: 0.28 },
			]),
		];
		...
		const familyB = {
			id: "fam-black-sea",
			label: "Black Sea pressure family",
			forecastCount: 2,
			situationCount: 2,
			situationIds: ["sit-ukraine-conflict", "sit-blacksea-market"],
		};
		const contexts = [
	...
	it("backfills deferred forecasts when filtering drops a preselected duplicate", () => {
		const primary = makePrediction("conflict", "Iran", "Escalation risk: Iran", 0.74, 0.66, "7d", [
			{ type: "ucdp", value: "Iran events elevated", weight: 0.4 },
		]);
		const duplicate = makePrediction(
			"conflict",
			"Iran",
			"Retaliatory conflict risk: Iran",
			0.69,
			0.58,
			"7d",
			[{ type: "ucdp", value: "Iran events elevated", weight: 0.36 }],
		);
		const political = makePrediction(
			"political",
			"Iran",
			"Political instability: Iran",
			0.59,
			0.57,
			"14d",
		...
		const supply = makePrediction(
			...
			0.56,
			"14d",
			[{ type: "chokepoint", value: "Routing delays persist", weight: 0.34 }],
		);
		...
		const fullRunSituationClusters = [
			{
				...
				actors: ["Iran"],
				branchKinds: ["base"],
				forecastIds: [primary.id, duplicate.id],
				forecastCount: 2,
				avgProbability: 0.715,
				avgConfidence: 0.62,
				topSignals: [{ type: "ucdp", count: 2 }],
				sampleTitles: [primary.title, duplicate.title],
			},
			{
				id: "sit-iran-political",
				label: "Iran political situation",
				...
				actors: ["Iran"],
				branchKinds: ["base"],
				forecastIds: [political.id],
				forecastCount: 1,
				avgProbability: 0.59,
				avgConfidence: 0.57,
				topSignals: [{ type: "news_corroboration", count: 1 }],
				sampleTitles: [political.title],
			},
			{
				id: "sit-gulf-shipping",
				label: "Persian Gulf shipping situation",
				dominantRegion: "Persian Gulf",
				dominantDomain: "supply_chain",
				regions: ["Persian Gulf"],
				domains: ["supply_chain"],
				actors: ["Shipping"],
				branchKinds: ["base"],
				forecastIds: [supply.id],
				forecastCount: 1,
				avgProbability: 0.54,
				avgConfidence: 0.56,
				topSignals: [{ type: "chokepoint", count: 1 }],
				sampleTitles: [supply.title],
			},
		];
		...
		const familyA = {
			...
			forecastCount: 3,
			situationCount: 2,
			situationIds: ["sit-iran-conflict", "sit-iran-political"],
		};
		const familyB = {
			id: "fam-gulf",
			label: "Persian Gulf pressure family",
			forecastCount: 1,
			situationCount: 1,
			situationIds: ["sit-gulf-shipping"],
		};
		...
		const expandedPool = selectPublishedForecastPool([primary, duplicate, political, supply], {
			targetCount: 3,
		});
		...
		while (
			artifacts.publishedPredictions.length < expandedPool.targetCount &&
			deferred.length > 0
		) {
	...
	it("boosts memory-backed situations during publish selection", () => {
		const persistent = makePrediction(
			"political",
			"Iran",
			...
			0.5,
			"14d",
			[{ type: "news_corroboration", value: "Iran unrest persists", weight: 0.34 }],
		);
		const fresh = makePrediction(
			"political",
			"India",
			"Political pressure: India",
			0.54,
			0.5,
			"14d",
			[{ type: "news_corroboration", value: "India coalition talks continue", weight: 0.34 }],
		);
		...
		persistent.situationContext = {
			id: "sit-iran-political",
			label: "Iran political situation",
			forecastCount: 1,
			topSignals: [{ type: "news_corroboration", count: 1 }],
		};
		fresh.situationContext = {
			id: "sit-india-political",
		...
		persistent.familyContext = {
			id: "fam-mena-political",
			label: "MENA political family",
			forecastCount: 1,
			situationCount: 1,
			situationIds: ["sit-iran-political"],
		};
		fresh.familyContext = {
			id: "fam-asia-political",
			label: "Asia political family",
			forecastCount: 1,
			situationCount: 1,
			situationIds: ["sit-india-political"],
		};
	...
	it("boosts market-confirmed situations during publish selection", () => {
		const confirmed = makePrediction(
			"market",
			"Middle East",
			"Oil repricing: Strait of Hormuz",
			0.51,
			0.48,
			"30d",
			[{ type: "prediction_market", value: "Oil contracts reprice on Hormuz stress", weight: 0.3 }],
		);
		const unconfirmed = makePrediction(
			"political",
			"India",
			"Political pressure: India",
			0.54,
		...
		for (const pred of [confirmed, unconfirmed]) {
			...
			pred.situationContext = {
				id: `sit-${pred.region}`,
				label: `${pred.region} situation`,
				forecastCount: 1,
				topSignals: [{ type: "news_corroboration", count: 1 }],
			};
			pred.familyContext = {
				id: `fam-${pred.region}`,
				label: `${pred.region} family`,
				forecastCount: 1,
				situationCount: 1,
				situationIds: [pred.situationContext.id],
			};
			pred.caseFile.situationContext = pred.situationContext;
	...
	it("keeps strategic supply-chain forecasts alive alongside same-state market repricing and reports survival telemetry", () => {
		const market = makePrediction(
			...
			[
				{
					type: "energy_supply_shock",
					value: "Energy repricing persists around Hormuz shipping stress.",
					weight: 0.36,
				},
			],
		);
		const supply = makePrediction(
			...
			[
				{
					type: "shipping_cost_shock",
					value: "Shipping reroutes persist through the Hormuz corridor.",
					weight: 0.35,
				},
			],
		);
		...
		const hormuzSituation = {
			...
			avgConfidence: 0.59,
			topSignals: [
				{ type: "shipping_cost_shock", count: 1 },
				{ type: "energy_supply_shock", count: 1 },
			],
		...
		const hormuzFamily = {
			...
			forecastCount: 2,
			situationCount: 1,
			situationIds: ["sit-hormuz"],
		};
		const brazilFamily = {
			id: "fam-brazil",
			label: "Brazil escalation family",
			forecastCount: 1,
			situationCount: 1,
			situationIds: ["sit-brazil"],
		};
	...
	it("keeps unrelated forecasts in separate situations instead of token-only over-merging", () => {
		...
		const cyber = makePrediction(
			...
			[
				{
					type: "news_corroboration",
					value: "Estonia reports sustained cyber probing",
					weight: 0.35,
				},
			],
		);
		...
		assert.ok(
			worldState.situationClusters.every((cluster) => !/fc-[a-z]+-[0-9a-f]{8}/.test(cluster.label)),
		);
	});
});
