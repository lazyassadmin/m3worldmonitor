...
describe("resilience handlers", () => {
	it("computes and caches a country score with domains, trend metadata, and history writes", async () => {
		...
		assert.equal(
			response.level,
			response.overallScore >= 70 ? "high" : response.overallScore >= 40 ? "medium" : "low",
		);
		...
		assert.ok(
			history.some((entry) => entry.member.startsWith(today + ":")),
			"expected today history member to be written",
		);
		...
		assert.equal(
			(sortedSets.get("resilience:history:US") ?? []).length,
			history.length,
			"cache hit must not append history",
		);
	});
});
