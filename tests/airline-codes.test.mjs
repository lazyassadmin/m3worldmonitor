...
describe("getWingbitsLiveFlight — IATA schedule fallback", () => {
	...
	it("falls back to IATA callsign when ICAO schedule returns 404, populates airline fields", async () => {
			...
			if (u === `${ECS_BASE}/schedule/EK528`) {
				return jsonResp({
					schedule: {
						depIata: "DXB",
						arrIata: "LHR",
						depTimeUtc: "2024-01-01T10:00:00Z",
						arrTimeUtc: "2024-01-01T18:00:00Z",
						status: "en-route",
						duration: 480,
					},
				});
		...
		assert.ok(
			fetchedUrls.some((u) => u.includes("/schedule/EK528")),
			"should have fetched IATA schedule",
		);
		// Cache key uses ICAO callsign (uppercase), not IATA
		assert.ok(
			fetchedUrls.some((u) => u.includes("/schedule/UAE528")),
			"should have tried ICAO schedule first",
		);
	});
	...
	it("uses ICAO schedule when available, skips IATA fallback", async () => {
		...
		globalThis.fetch = async (url) => {
			...
			if (u === `${ECS_BASE}/schedule/UAE528`) {
				return jsonResp({
					schedule: { depIata: "DXB", arrIata: "LHR", status: "en-route", duration: 480 },
				});
			}
		...
		assert.ok(
			!fetchedUrls.some((u) => u.includes("/schedule/EK528")),
			"should not fetch IATA schedule when ICAO succeeds",
		);
	});
