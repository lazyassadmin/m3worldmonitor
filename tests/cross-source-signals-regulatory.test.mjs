...
const pureSrc = seedSrc
	...
	.replace(/loadEnvFile\([^)]+\);\n/, "")
	.replace(
		/async function readAllSourceKeys[\s\S]*?\n}\n\n\/\/ ── Signal extractors/m,
		"// readAllSourceKeys removed for unit test\n\n// ── Signal extractors",
	)
	.replace(/runSeed\('intelligence'[\s\S]*$/m, "");

const ctx = vm.createContext({ console, Date, Math, Number, Array, Map, Set, String, RegExp });
vm.runInContext(
	`${pureSrc}\n;globalThis.__exports = { SOURCE_KEYS, TYPE_CATEGORY, BASE_WEIGHT, scoreTier, extractRegulatoryAction, detectCompositeEscalation };`,
	ctx,
);
...
describe("extractRegulatoryAction", () => {
	...
	it("returns an empty array when payload.actions is not an array", () => {
		assert.deepEqual(
			normalize(extractRegulatoryAction({ "regulatory:actions:v1": { actions: "corrupted" } })),
			[],
		);
		assert.deepEqual(
			normalize(extractRegulatoryAction({ "regulatory:actions:v1": { actions: null } })),
			[],
		);
		assert.deepEqual(normalize(extractRegulatoryAction({ "regulatory:actions:v1": {} })), []);
		assert.deepEqual(
			normalize(extractRegulatoryAction({ "regulatory:actions:v1": { actions: 42 } })),
			[],
		);
	});
	...
	it("emits only high and medium signals, prioritizes high before fresher medium, and limits output to 3", () => {
		...
		assert.deepEqual(
			...
			["regulatory:sec-a", "regulatory:sec-c", "regulatory:fdic-a"],
		);
