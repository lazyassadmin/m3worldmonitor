...
describe("cache-only handlers read from seed keys", () => {
	it("climate anomalies is pure cache read (seed controls TTL)", () => {
		...
		assert.match(src, /getCachedJson/, "Climate handler should use getCachedJson (seed-only)");
		assert.doesNotMatch(src, /cachedFetchJson/, "Climate handler should not call external APIs");
	});
	...
	it("fire detections is pure cache read (seed controls TTL)", () => {
		...
		assert.match(src, /getCachedJson/, "Fire handler should use getCachedJson (seed-only)");
		assert.doesNotMatch(src, /cachedFetchJson/, "Fire handler should not call external APIs");
	});
});
...
describe("ACLED shared cache layer", () => {
	...
	it("exports fetchAcledCached function", () => {
		assert.match(
			src,
			/export async function fetchAcledCached/,
	...
	it("derives cache key from query parameters", () => {
		assert.match(
			src,
			/acled:shared:\$\{opts\.eventTypes\}:\$\{opts\.startDate\}:\$\{opts\.endDate\}/,
	...
	it("uses cachedFetchJson to check Redis cache before upstream API call", () => {
		assert.match(
			src,
			/cachedFetchJson\s*<.*>\s*\(cacheKey/,
			"Should use cachedFetchJson which handles cache check + coalescing",
		);
		assert.ok(src.includes("fetch(`${ACLED_API_URL}"), "Should call ACLED API inside the fetcher");
	});

	it("uses 15-minute cache TTL", () => {
		assert.match(src, /ACLED_CACHE_TTL = 900/, "ACLED cache TTL should be 900s (15 minutes)");
	});
	...
	it("caches empty successful responses to avoid repeated cache misses", () => {
		assert.doesNotMatch(
			src,
			/if\s*\(events\.length\s*>\s*0\)\s*\{[\s\S]*setCachedJson\(cacheKey, events, ACLED_CACHE_TTL\)/,
...
describe("ACLED consumers use shared cache layer", () => {
	it("conflict handler imports fetchAcledCached", () => {
		...
		assert.match(src, /fetchAcledCached/, "Conflict handler should use shared ACLED fetch");
	});
	...
	it("unrest handler is pure cache read (seed-only, no ACLED calls)", () => {
		...
		assert.match(src, /getCachedJson/, "Unrest handler should use getCachedJson (seed-only)");
		assert.doesNotMatch(src, /cachedFetchJson/, "Unrest handler should not call external APIs");
	});

	it("risk scores handler imports fetchAcledCached", () => {
		const src = readSrc("server/worldmonitor/intelligence/v1/get-risk-scores.ts");
		assert.match(src, /fetchAcledCached/, "Risk scores handler should use shared ACLED fetch");
	});
...
describe("maritime AIS visibility guard (SmartPollLoop)", () => {
	...
	it("uses startSmartPollLoop for polling", () => {
		assert.match(src, /startSmartPollLoop/, "Should use startSmartPollLoop for AIS polling");
	});

	it("pauses entirely when hidden via pauseWhenHidden option", () => {
		assert.match(
			src,
			/pauseWhenHidden:\s*true/,
	...
	it("passes AbortSignal through to pollSnapshot", () => {
		...
		assert.match(pollFn, /signal\?\.aborted/, "pollSnapshot should check signal.aborted");
	});
