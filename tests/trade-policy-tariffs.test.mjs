...
} from "../scripts/_trade-parse-utils.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, "..");

const protoSrc = readFileSync(
	join(root, "proto/worldmonitor/trade/v1/get_tariff_trends.proto"),
	"utf-8",
);
const tradeDataProtoSrc = readFileSync(
	join(root, "proto/worldmonitor/trade/v1/trade_data.proto"),
	"utf-8",
);
...
const clientGeneratedSrc = readFileSync(
	join(root, "src/generated/client/worldmonitor/trade/v1/service_client.ts"),
	"utf-8",
);
const serverGeneratedSrc = readFileSync(
	join(root, "src/generated/server/worldmonitor/trade/v1/service_server.ts"),
	"utf-8",
);
...
describe("parseBudgetLabEffectiveTariffHtml — edge cases", () => {
	it("returns null when page contains no recognizable rate", () => {
		assert.equal(
			parseBudgetLabEffectiveTariffHtml("<html><body><p>No tariff data here.</p></body></html>"),
			null,
		);
	});
	...
	it("strips HTML tags before matching", () => {
		const html =
			"<p>effective tariff rate reaching <strong>7.5%</strong> in <em>March 2026</em></p>";
		const result = parseBudgetLabEffectiveTariffHtml(html);
		assert.ok(result);
