...
async function runTests() {
	if (!OREF_PROXY_AUTH) {
		...
		console.log(
			'  Example: OREF_PROXY_AUTH="user:pass;il;;;@proxy.froxy.com:9000" node tests/oref-proxy.test.mjs',
		);
		process.exit(0);
	}
	...
	try {
		assert.throws(() => orefCurlFetch("baduser:badpass@127.0.0.1:1", OREF_ALERTS_URL), /./);
