...
describe("LiveNewsPanel instantiation guard", () => {
	...
	it("DEFAULT_LIVE_CHANNELS is [] on happy variant (crash source)", () => {
		...
		const match = liveNews.match(
			/DEFAULT_LIVE_CHANNELS\s*=\s*SITE_VARIANT\s*===\s*['"]tech['"]\s*\?[^:]+:\s*SITE_VARIANT\s*===\s*['"]happy['"]\s*\?\s*(\[[^\]]*\])/,
		);
		assert.ok(match, "DEFAULT_LIVE_CHANNELS assignment not found");
		assert.equal(
			match[1].replace(/\s/g, ""),
			"[]",
			"happy variant DEFAULT_LIVE_CHANNELS must be []",
		);
	});
	...
	it("constructor does not fall back to FULL_LIVE_CHANNELS after getDefaultLiveChannels()", () => {
		...
		const afterDefaults = liveNews.slice(
			liveNews.indexOf("if (this.channels.length === 0) this.channels = getDefaultLiveChannels();"),
		);
	...
	it("refreshChannelsFromStorage does not fall back to FULL_LIVE_CHANNELS", () => {
		const liveNews = src("src/components/LiveNewsPanel.ts");
		const refreshBlock = liveNews.match(
			/refreshChannelsFromStorage[^}]+loadChannelsFromStorage[^}]+getDefaultLiveChannels[^}]+(this\.channels\s*=\s*\[\.\.\.FULL_LIVE_CHANNELS\])?/s,
		);
	...
	it("panel-layout.ts live-news guard checks getDefaultLiveChannels()", () => {
		...
		const guardBlock = layout.match(
			/shouldCreatePanel\('live-news'\)[^}]*getDefaultLiveChannels\(\)\.length/s,
		);
		assert.ok(
			guardBlock,
	...
	it("panel-layout.ts live-news guard also checks loadChannelsFromStorage()", () => {
		const layout = src("src/app/panel-layout.ts");
		const guardBlock = layout.match(
			/shouldCreatePanel\('live-news'\)[^}]*loadChannelsFromStorage\(\)\.length/s,
		);
		assert.ok(
			guardBlock,
