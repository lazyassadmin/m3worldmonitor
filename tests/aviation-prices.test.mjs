...
describe("AirlineIntelPanel — prices tab never auto-fetches", () => {
	...
	it("refresh() skips loading the prices tab", () => {
		...
		assert.ok(body.includes("'prices'"), "refresh() must guard against loading the prices tab");
		assert.ok(
	...
	it("prices tab state uses googleFlightsData and datesData, not pricesData", () => {
		...
		assert.ok(panel.includes("datesData"), "AirlineIntelPanel must use datesData state field");
		assert.ok(!panel.includes("pricesData"), "AirlineIntelPanel must NOT use old pricesData field");
	});
	...
	it("all server-derived strings pass through escapeHtml", () => {
		...
		assert.ok(
			panel.includes("escapeHtml(leg.airlineCode)") &&
				panel.includes("escapeHtml(leg.flightNumber)"),
