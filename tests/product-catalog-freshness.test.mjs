...
describe("Product catalog freshness", () => {
	...
	const tiersJson = JSON.parse(
		readFileSync(join(ROOT, "pro-test/src/generated/tiers.json"), "utf8"),
	);
	...
	it("generated products.ts contains valid product IDs", () => {
		assert.ok(
			generatedProductIds.length >= 4,
			`Expected at least 4 product IDs, got ${generatedProductIds.length}`,
		);
	...
	it("generated files are fresh (re-running generator produces same output)", () => {
		...
		assert.equal(
			currentProducts,
			freshProducts,
			"products.generated.ts is stale — run: npx tsx scripts/generate-product-config.mjs",
		);
		assert.equal(
			currentTiers,
			freshTiers,
			"tiers.json is stale — run: npx tsx scripts/generate-product-config.mjs",
		);
