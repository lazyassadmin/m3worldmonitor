...
describe("gdelt-intel.ts — dedicated circuit breakers per GDELT query type", () => {
	...
	assert.ok(
		posStart !== -1,
		"fetchPositiveGdeltArticles not found in gdelt-intel.ts — was it renamed?",
	);
	...
	it("GDELT breakers have distinct names", () => {
		assert.match(src, /GDELT Intelligence/, 'gdeltBreaker must have name "GDELT Intelligence"');
		assert.match(src, /GDELT Positive/, 'positiveGdeltBreaker must have name "GDELT Positive"');
	});
...
describe("CircuitBreaker isolation — HAPI per-country independence", () => {
	...
	it("HAPI: failure in one country does not trip another", async () => {
		...
		try {
			...
			const alwaysFail = () => {
				throw new Error("HDX HAPI unavailable");
			};
			...
			assert.equal(
				breakerUS.isOnCooldown(),
				true,
				"breakerUS should be on cooldown after 2 failures",
			);

			// breakerRU must NOT be affected
			assert.equal(
				breakerRU.isOnCooldown(),
				false,
				"breakerRU must not be on cooldown when breakerUS fails",
			);

			// breakerRU should still call through successfully
			const goodData = {
				summary: { countryCode: "RU", conflictEvents: 12, displacedPersons: 5000 },
			};
			const result = await breakerRU.execute(async () => goodData, fallback);
			assert.deepEqual(
				result,
				goodData,
				"breakerRU should return live data unaffected by breakerUS cooldown",
			);
		} finally {
			clearAllCircuitBreakers();
		}
	...
	it("HAPI: different countries cache independently (no cross-country poisoning)", async () => {
		...
		try {
			...
			const ruData = {
				summary: { countryCode: "RU", conflictEvents: 47, displacedPersons: 120000 },
			};
			...
			assert.equal(
				cachedUS.summary?.countryCode,
				"US",
				"breakerUS cache must return US data, not RU data",
			);
			assert.equal(
				cachedRU.summary?.countryCode,
				"RU",
				"breakerRU cache must return RU data, not US data",
			);
			assert.notEqual(
				cachedUS.summary?.conflictEvents,
				cachedRU.summary?.conflictEvents,
...
describe("CircuitBreaker isolation — GDELT split breaker independence", () => {
	...
	it("GDELT: positive breaker failure does not trip regular breaker", async () => {
		...
		try {
			const gdelt = createCircuitBreaker({
			...
			const alwaysFail = () => {
				throw new Error("GDELT API unavailable");
			};
			...
			assert.equal(
				positive.isOnCooldown(),
				true,
				"positive breaker should be on cooldown after 2 failures",
			);
			...
			assert.equal(
				gdelt.isOnCooldown(),
				false,
				"gdelt breaker must not be on cooldown when positive fails",
			);

			// gdelt should still call through successfully
			const realArticles = {
				articles: [{ url: "https://news.example/military", title: "Conflict update" }],
				totalArticles: 1,
			};
			...
			assert.deepEqual(
				result,
				realArticles,
				"gdelt breaker should return live data unaffected by positive cooldown",
			);
		} finally {
			clearAllCircuitBreakers();
		}
	...
	it("GDELT: regular and positive breakers cache different data independently", async () => {
		...
		try {
			...
			const fallback = { articles: [], totalArticles: 0 };
			const militaryData = {
				articles: [{ url: "https://news.example/military", title: "Military operations" }],
				totalArticles: 1,
			};
			const peaceData = {
				articles: [{ url: "https://good.example/peace", title: "Peace agreement" }],
				totalArticles: 1,
			};
