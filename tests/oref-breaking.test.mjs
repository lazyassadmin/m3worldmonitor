...
const SRC = readFileSync(
	join(__dirname, "..", "src", "services", "breaking-news-alerts.ts"),
	"utf8",
);
...
describe("breaking-news-alerts oref_siren integration", () => {
	...
	it("exports dispatchOrefBreakingAlert function", () => {
		assert.ok(
			SRC.includes("export function dispatchOrefBreakingAlert("),
			"should export dispatchOrefBreakingAlert",
		);
	});

	it("imports OrefAlert type", () => {
		assert.ok(
			SRC.includes("import type { OrefAlert }") || SRC.includes("import { OrefAlert }"),
			"should import OrefAlert",
		);
	});
	...
	it("bypasses global cooldown (no isGlobalCooldown check)", () => {
		const fnBody = SRC.slice(
			SRC.indexOf("function dispatchOrefBreakingAlert"),
			SRC.indexOf("export function initBreakingNewsAlerts"),
		);
	...
	it("returns early when settings disabled or no alerts", () => {
		...
		assert.ok(
			fnBody.includes("!settings.enabled") && fnBody.includes("!alerts.length"),
			"should guard settings and empty alerts",
		);
	});
});
...
describe("data-loader oref breaking news wiring", () => {
	...
	it("imports dispatchOrefBreakingAlert", () => {
		assert.ok(
			DL.includes("dispatchOrefBreakingAlert"),
			"data-loader should import dispatchOrefBreakingAlert",
		);
	});
	...
	it("calls dispatchOrefBreakingAlert in onOrefAlertsUpdate callback", () => {
		...
		assert.ok(
			callbackSection.includes("dispatchOrefBreakingAlert"),
			"should call in update callback",
		);
	});
});
