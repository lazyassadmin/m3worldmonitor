...
describe("api/mcp-proxy", () => {
	...
	describe("CORS and method handling", () => {
		it("returns 403 for disallowed origin", async () => {
			const res = await handler(
				makeGetRequest({ serverUrl: "https://mcp.example.com/mcp" }, "https://evil.com"),
			);
		...
		it("returns 405 for DELETE", async () => {
			const res = await handler(
				new Request("https://worldmonitor.app/api/mcp-proxy", {
					method: "DELETE",
	...
	describe("GET /api/mcp-proxy (list tools)", () => {
		...
		it("returns 400 for link-local 169.254.x.x (cloud metadata)", async () => {
			const res = await handler(
				makeGetRequest({ serverUrl: "http://169.254.169.254/latest/meta-data/" }),
			);
			assert.equal(res.status, 400);
		});
