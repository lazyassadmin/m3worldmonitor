...
describe("getLocalizedNameExpression", () => {
	it("returns simplified English coalesce expression", () => {
		assert.deepEqual(getLocalizedNameExpression("en"), [
			"coalesce",
			["get", "name:en"],
			["get", "name"],
		]);
	...
	it("returns localized-first coalesce expression for non-English language", () => {
		assert.deepEqual(getLocalizedNameExpression("fr"), [
			"coalesce",
			["get", "name:fr"],
			["get", "name:en"],
			["get", "name"],
	...
	it("Vietnamese falls back to English expression (no tile field)", () => {
		assert.deepEqual(getLocalizedNameExpression("vi"), [
			"coalesce",
			["get", "name:en"],
			["get", "name"],
		]);
	...
	it("unknown language falls back to English expression", () => {
		assert.deepEqual(getLocalizedNameExpression("xx"), [
			"coalesce",
			["get", "name:en"],
			["get", "name"],
		]);
...
describe("isLocalizableTextField", () => {
	...
	describe("expression arrays", () => {
		it("accepts expression referencing name", () => {
			assert.equal(isLocalizableTextField(["coalesce", ["get", "name:en"], ["get", "name"]]), true);
		});
...
describe("RTL text plugin", () => {
	it("self-hosted mapbox-gl-rtl-text.min.js exists in public/", () => {
		...
		assert.ok(
			content.includes("mapboxgl") ||
				content.includes("RTLTextPlugin") ||
				content.includes("applyArabicShaping"),
