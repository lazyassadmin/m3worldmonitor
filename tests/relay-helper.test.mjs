...
const { getRelayBaseUrl, getRelayHeaders, fetchWithTimeout, createRelayHandler } = await import(
	"../api/_relay.js"
);
...
describe("createRelayHandler", () => {
	...
	it("preserves JSON error with uppercase APPLICATION/JSON content-type", async () => {
		mockFetch(
			async () =>
				new Response('{"detail":"bad request"}', {
					status: 400,
					headers: { "Content-Type": "APPLICATION/JSON" },
				}),
		);
	...
	it("passes application/vnd.api+json error through unchanged (JSON-compatible type)", async () => {
		...
		mockFetch(
			async () =>
				new Response('{"errors":[{"status":"500"}]}', {
					status: 500,
	...
	it("wraps 401 text/html error in JSON envelope", async () => {
		mockFetch(
			async () =>
				new Response("Unauthorized", { status: 401, headers: { "Content-Type": "text/html" } }),
		);
		const handler = createRelayHandler({ relayPath: "/test" });
		const res = await handler(makeRequest("https://worldmonitor.app/api/test"));
		assert.equal(res.status, 401);
	...
	it("wraps 499 text/plain error in JSON envelope", async () => {
		mockFetch(
			async () =>
				new Response("Client Closed Request", {
					status: 499,
					headers: { "Content-Type": "text/plain" },
				}),
		);
	...
	it("does NOT wrap 200 non-JSON response (success passthrough)", async () => {
		mockFetch(
			async () =>
				new Response("<html>OK page</html>", {
					status: 200,
					headers: { "Content-Type": "text/html" },
				}),
		);
	...
	it("wraps empty body with non-JSON error content-type", async () => {
		mockFetch(
			async () => new Response("", { status: 502, headers: { "Content-Type": "text/html" } }),
		);
		const handler = createRelayHandler({ relayPath: "/test" });
		const res = await handler(makeRequest("https://worldmonitor.app/api/test"));
		assert.equal(res.status, 502);
	...
	it("wraps very large HTML body and still returns parseable JSON", async () => {
		...
		mockFetch(
			async () =>
				new Response(largeHtml, { status: 502, headers: { "Content-Type": "text/html" } }),
		);
		const handler = createRelayHandler({ relayPath: "/test" });
		const res = await handler(makeRequest("https://worldmonitor.app/api/test"));
		assert.equal(res.status, 502);
	...
	it("wraps null body with error status", async () => {
		mockFetch(
			async () => new Response(null, { status: 503, headers: { "Content-Type": "text/plain" } }),
		);
		const handler = createRelayHandler({ relayPath: "/test" });
		const res = await handler(makeRequest("https://worldmonitor.app/api/test"));
		assert.equal(res.status, 503);
	...
	it("produces parseable JSON envelope for every non-2xx non-JSON status", async () => {
		...
		for (const status of statuses) {
			mockFetch(
				async () =>
					new Response(`<html>Error ${status}</html>`, {
						status,
						headers: { "Content-Type": "text/html" },
					}),
			);
			...
			}, `Body not valid JSON for status ${status}`);
			assert.ok(
				parsed.error.startsWith("Upstream error"),
	...
	it("passes through application/xml success response unchanged", async () => {
		mockFetch(
			async () =>
				new Response('<?xml version="1.0"?><data/>', {
					status: 200,
					headers: { "Content-Type": "application/xml" },
				}),
		);
	...
	it("passes through text/plain success response unchanged", async () => {
		mockFetch(
			async () =>
				new Response("just plain text", { status: 200, headers: { "Content-Type": "text/plain" } }),
		);
		const handler = createRelayHandler({ relayPath: "/test" });
		const res = await handler(makeRequest("https://worldmonitor.app/api/test"));
		assert.equal(res.status, 200);
	...
	it("passes through application/vnd.api+json success response unchanged", async () => {
		mockFetch(
			async () =>
				new Response('{"data":{"type":"articles","id":"1"}}', {
					status: 200,
					headers: { "Content-Type": "application/vnd.api+json" },
				}),
		);
	...
	it("returns 502 when upstream produces an invalid status code (triggers catch)", async () => {
		mockFetch(
			async () =>
				new Response("informational-ish", {
					status: 199,
					headers: { "Content-Type": "text/plain" },
				}),
		);
	...
	it("wraps non-JSON 599 error (upper bound of valid HTTP status)", async () => {
		mockFetch(
			async () =>
				new Response("custom error", { status: 599, headers: { "Content-Type": "text/plain" } }),
		);
		const handler = createRelayHandler({ relayPath: "/test" });
		const res = await handler(makeRequest("https://worldmonitor.app/api/test"));
