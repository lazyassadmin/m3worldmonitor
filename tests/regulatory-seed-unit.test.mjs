...
describe("extractAtomLink + parseAtomEntries", () => {
	it("prefers alternate href and extracts summary/content with normalized publishedAt", () => {
      ...
      <feed xmlns="http://www.w3.org/2005/Atom">
		...
		assert.equal(
			extractAtomLink(
				'<entry><link rel="self" href="https://example.test/self" /><link rel="alternate" href="/press/notice-a" /></entry>',
			),
			"/press/notice-a",
		);

		assert.deepEqual(
			normalize(parseAtomEntries(xml, "https://www.federalreserve.gov/feeds/press_all.xml")),
			[
				{
...
describe("parseFeed", () => {
	it("detects Atom feeds automatically", () => {
		const atom =
			'<feed><entry><title>A</title><link href="https://example.test/a" /><updated>2026-03-28T00:00:00Z</updated></entry></feed>';
				...
				[
					{
						...
						link: "https://example.test/sec",
						publishedAt: "2026-03-29T14:00:00.000Z",
					},
					{
						title: "SEC Summary",
						description: "extra context",
						link: "https://example.test/sec-2",
		...
		const fetchStub = async (url, options) => {
			...
			if (url.endsWith("/sec")) {
				return {
					ok: true,
					text: async () =>
						`<rss><channel><item><title>SEC Charges Bank</title><link>https://sec.test/a</link><pubDate>Mon, 30 Mar 2026 18:00:00 GMT</pubDate></item></channel></rss>`,
				};
			}
...
describe("fetchRegulatoryActionPayload", () => {
	it("returns classified payload from fetched actions", async () => {
		const payload = normalize(
			await fetchRegulatoryActionPayload(async (url) => ({
				ok: true,
				text: async () =>
					`<rss><channel><item><title>FDIC update</title><description>FDIC resolves action through a remedial action plan.</description><link>${url}/item</link><pubDate>Mon, 30 Mar 2026 18:00:00 GMT</pubDate></item></channel></rss>`,
			})),
		);
...
describe("main", () => {
	it("wires runSeed with the regulatory key, TTL, and validateFn", async () => {
		...
		const fetchStub = async (url) => ({
			ok: true,
			text: async () =>
				`<rss><channel><item><title>CFTC Issues Advisory</title><link>${url}/item</link><pubDate>Mon, 30 Mar 2026 18:00:00 GMT</pubDate></item></channel></rss>`,
		});
