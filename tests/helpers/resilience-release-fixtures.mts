...
function buildStaticRecord(descriptor: CountryDescriptor) {
	...
	return {
		...
		gpi: {
			score: round(clamp(4.1 - quality * 0.03, 1.2, 4.2), 2),
			rank: Math.round(190 - quality * 1.5),
			year: 2025,
		},
		rsf: {
			score: round(clamp(8 + quality * 0.92, 10, 95), 1),
			rank: Math.round(180 - quality * 1.6),
			year: 2025,
		},
		who: {
			indicators: {
		...
		aquastat:
			descriptor.profile === "fragile"
				? {
						indicator: "Water stress",
						value: round(clamp(100 - quality * 0.5, 45, 98)),
						year: 2024,
					}
				: {
						indicator: "Renewable water availability",
						value: round(clamp(300 + quality * 42, 300, 5000)),
						year: 2024,
					},
		iea: {
...
export function buildReleaseGateFixtures(): ReleaseGateFixtureMap {
	...
	for (const descriptor of descriptors) {
		...
		bisExchangeRates.push(
			{
				countryCode: descriptor.code,
				realChange: amplitude * 0.7,
				realEer: exchangeBase,
				date: "2025-08",
			},
			{
				countryCode: descriptor.code,
				realChange: -amplitude,
				realEer: exchangeBase + amplitude,
				date: "2025-09",
			},
			{
				countryCode: descriptor.code,
				realChange: amplitude * 0.6,
				realEer: exchangeBase - amplitude * 0.5,
				date: "2025-10",
			},
			{
				countryCode: descriptor.code,
				realChange: -amplitude * 0.8,
				realEer: exchangeBase + amplitude * 0.4,
				date: "2025-11",
			},
		);
		...
		for (let index = 0; index < conflictEvents; index += 1) {
			ucdpEvents.push({
				...
				violenceType:
					index % 2 === 0 ? "UCDP_VIOLENCE_TYPE_STATE_BASED" : "UCDP_VIOLENCE_TYPE_ONE_SIDED",
			});
		}
		...
		socialPosts.push(
			{
				title: `${descriptor.name} resilience watch`,
				velocityScore: round(clamp(stressed / 2.5, 1, 80), 1),
			},
			{
				title: `${descriptor.name} infrastructure stability update`,
				velocityScore: round(clamp(stressed / 3.2, 1, 65), 1),
			},
		);
	...
	fixtures[`displacement:summary:v1:${new Date().getFullYear()}`] = {
		summary: { countries: displacementCountries },
	};
