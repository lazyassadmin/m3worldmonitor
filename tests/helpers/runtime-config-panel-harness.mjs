...
function matchesSelector(element, selector) {
	return splitSelectorList(selector).some((part) => {
		...
		if (
			parsed.attributes.some(({ name, value }) => {
			...
			return false;
		if (
			parsed.notAttributes.some(({ name, value }) => {
...
async function loadRuntimeConfigPanel() {
    ...
    `,
		...
		[
			"analytics-stub",
			`export function trackPanelResized() {} export function trackFeatureToggle() {}`,
		],
		[
...
export async function createRuntimeConfigPanelHarness() {
	...
	function setRuntimeState({ totalFeatures, availableFeatures, configuredCount }) {
