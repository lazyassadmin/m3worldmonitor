...
const panelSrc = readFileSync(
	resolve(__dirname, "..", "src", "components", "SupplyChainPanel.ts"),
	"utf-8",
);
...
describe("SupplyChainPanel transit chart mount contract", () => {
	...
	it("clearTransitChart() cancels timer, disconnects observer, and destroys chart", () => {
		...
		assert.ok(
			body.includes("chartMountTimer") && body.includes("null"),
			"must null the timer handle",
		);
	...
	it("fallback timer clears itself and disconnects observer after mounting", () => {
		...
		const timerBody = panelSrc.match(
			/chartMountTimer\s*=\s*setTimeout\(\(\)\s*=>\s*\{([\s\S]*?)\},\s*\d+\)/,
		);
		assert.ok(timerBody, "fallback timer callback should exist");
		const body = timerBody[1];
		assert.ok(
			body.includes("chartObserver") && body.includes("disconnect"),
			"timer callback must disconnect observer",
		);
	...
	it("MutationObserver callback clears timer and disconnects itself after mounting", () => {
		...
		assert.ok(
			body.includes("clearTimeout") || body.includes("chartMountTimer"),
			"observer callback must cancel fallback timer",
		);
	...
	it("tab switch clears transit chart before re-rendering", () => {
		...
		assert.ok(
			body.indexOf("clearTransitChart") < body.indexOf("render"),
			"clearTransitChart must come before render()",
		);
	});
...
describe("SupplyChainPanel restructure contract", () => {
	...
	it("renderShipping delegates to renderDisruptionSnapshot", () => {
		const shippingMethod = panelSrc.match(
			/private\s+renderShipping\(\)[\s\S]*?\{([\s\S]*?)\n\s{2}\}/,
		);
	...
	it("renderDisruptionSnapshot handles null chokepointData as loading state", () => {
		const method = panelSrc.match(
			/private\s+renderDisruptionSnapshot\(\)[\s\S]*?\{([\s\S]*?)\n\s{2}\}/,
		);
	...
	it("server description no longer contains riskSummary or warning count text", () => {
		const descBlock = serverSrc.match(
			/const descriptions:\s*string\[\]\s*=\s*\[\];([\s\S]*?)description:\s*descriptions\.join/,
		);
