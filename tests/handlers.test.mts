...
describe("Cyber enum maps", () => {
	...
	it("SEVERITY_RANK orders critical > high > medium > low > unspecified", () => {
		assert.ok(
			SEVERITY_RANK["CRITICALITY_LEVEL_CRITICAL"]! > SEVERITY_RANK["CRITICALITY_LEVEL_HIGH"]!,
		);
		assert.ok(
			SEVERITY_RANK["CRITICALITY_LEVEL_HIGH"]! > SEVERITY_RANK["CRITICALITY_LEVEL_MEDIUM"]!,
		);
		assert.ok(SEVERITY_RANK["CRITICALITY_LEVEL_MEDIUM"]! > SEVERITY_RANK["CRITICALITY_LEVEL_LOW"]!);
		assert.ok(
			SEVERITY_RANK["CRITICALITY_LEVEL_LOW"]! > SEVERITY_RANK["CRITICALITY_LEVEL_UNSPECIFIED"]!,
		);
	});
});
...
describe("deduplicateHeadlines", () => {
	...
	it("returns single headline unchanged", () => {
		assert.deepEqual(deduplicateHeadlines(["Breaking: earthquake hits Japan"]), [
	...
	it("filters short words (< 4 chars) from similarity comparison", () => {
		...
		const headlines = ["The art of the deal is dead", "The end of an era is here"];
		const result = deduplicateHeadlines(headlines);
