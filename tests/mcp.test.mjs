...
describe("api/mcp.ts — PRO MCP Server", () => {
	...
	it("returns HTTP 401 + WWW-Authenticate when no credentials provided", async () => {
		...
		assert.ok(
			res.headers.get("www-authenticate")?.includes('Bearer realm="worldmonitor"'),
	...
	it("unknown method returns JSON-RPC -32601", async () => {
		const res = await handler(
			makeReq("POST", { jsonrpc: "2.0", id: 5, method: "nonexistent/method", params: {} }),
		);
		const body = await res.json();
	...
	it("tools/list returns 28 tools with name, description, inputSchema", async () => {
		const res = await handler(
			makeReq("POST", { jsonrpc: "2.0", id: 2, method: "tools/list", params: {} }),
		);
		assert.equal(res.status, 200);
		const body = await res.json();
		...
		assert.equal(
			...
			28,
			`Expected 28 tools, got ${body.result.tools.length}`,
		);
	...
	it("returns JSON-RPC -32029 when rate limited", async () => {
		...
		assert.ok(
			body.error?.code === -32029 || body.result?.protocolVersion,
			"Handler must return valid JSON-RPC (either rate limited or initialized)",
		);
	});
	...
	it("get_airspace returns partial:true + warning when military source fails", async () => {
		globalThis.fetch = async (url) => {
			const u = url.toString();
			if (u.includes("/api/aviation/v1/track-aircraft")) {
				return new Response(JSON.stringify({ positions: [], source: "opensky" }), {
					status: 200,
					headers: { "Content-Type": "application/json" },
				});
			}
		...
		assert.ok(
			data.warnings?.some((w) => w.includes("military")),
			"warnings must mention military",
		);
	...
	it("get_airspace type=civilian skips military fetch", async () => {
		...
		assert.equal(
			...
			undefined,
			"military_flights must be absent for type=civilian",
		);
							...
							disruptions: [
								{
									...
									vessel_count: 12,
									region: "Persian Gulf",
									description: "Elevated dark-ship activity",
								},
							],
						},
					}),
