...
describe("buildExposureIndex", () => {
	...
	it("each bucket includes only countries with a non-null value for that metric", () => {
		...
		assert.ok(
			!idx.gas.some((e) => e.iso2 === "MT"),
			"MT has null gasShare — should not appear in gas bucket",
		);
		assert.ok(
			!idx.coal.some((e) => e.iso2 === "MT"),
			"MT has null coalShare — should not appear in coal bucket",
		);
		assert.ok(
			idx.oil.some((e) => e.iso2 === "MT"),
			"MT has oilShare=3 — must appear in oil bucket",
		);
		assert.ok(
			idx.imported.some((e) => e.iso2 === "MT"),
			"MT has importShare=93 — must appear in imported bucket",
		);
	});
	...
	it("countries with only oil/import/renewables data are not excluded from those buckets", () => {
		...
		assert.ok(
			idx.oil.some((e) => e.iso2 === "XX"),
			"oil-only country must appear in oil bucket",
		);
		assert.ok(
			idx.imported.some((e) => e.iso2 === "XX"),
			"oil-only country must appear in imported bucket",
		);
		assert.ok(
			!idx.gas.some((e) => e.iso2 === "XX"),
			"oil-only country must not appear in gas bucket",
		);
		assert.ok(
			!idx.coal.some((e) => e.iso2 === "XX"),
			"oil-only country must not appear in coal bucket",
		);
	});
	...
	it("each bucket is sorted descending by share", () => {
		...
		for (const bucket of [idx.gas, idx.coal, idx.oil, idx.imported, idx.renewable]) {
			for (let i = 1; i < bucket.length; i++) {
				assert.ok(
					bucket[i - 1].share >= bucket[i].share,
