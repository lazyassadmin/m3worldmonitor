...
} from "../src/utils/signal-quality.ts";
...
function isq(
	...
	focalFn = noFocalFn,
	ciiFn: (c: string) => number | null = () => null,
	ready = noFocalReady,
) {
	...
	it("composite is always in [0, 1]", () => {
		const inputs: SignalQualityInput[] = [
			{ sourceCount: 0, isAlert: false },
			{
				...
				velocity: { sourcesPerHour: 10, level: "spike", trend: "rising" },
			},
		...
		for (const input of inputs) {
			...
			assert.ok(
				result.composite >= 0 && result.composite <= 1,
				`composite out of range: ${result.composite}`,
			);
		}
	});
});
...
describe("computeISQ — expectation gap tri-state", () => {
	...
	it("gap is 0.8 when focal data available but country absent (novel)", () => {
		const result = isq(
			{ sourceCount: 2, isAlert: false, countryCode: "DE" },
			noFocalFn,
			() => null,
			focalReady,
		);
...
describe("computeISQ — CII not warmed up", () => {
	it("intensity falls back to threatLevel only when ciiScoreFn returns null", () => {
		const result = isq(
			{ sourceCount: 1, isAlert: false, threatLevel: "high", countryCode: "IR" },
			noFocalFn,
			() => null,
		);
...
describe("computeISQ — tiers", () => {
		...
		const result = isq(
			{
				...
				isAlert: true,
				threatLevel: "critical",
				velocity: { sourcesPerHour: 5, level: "spike", trend: "rising" },
				countryCode: "IR",
			},
			focalFn,
			() => 85,
	...
	it("weak tier for low-signal story (single source, info threat)", () => {
		...
		assert.ok(
			result.tier === "weak" || result.tier === "noise",
			`expected weak/noise, got ${result.tier}`,
		);
...
describe("computeISQ — weight profiles sum to 1", () => {
	const WEIGHTS: Record<string, [number, number, number, number]> = {
		default: [0.35, 0.3, 0.2, 0.15],
		risk: [0.45, 0.25, 0.2, 0.1],
		macro: [0.25, 0.4, 0.2, 0.15],
		shortTerm: [0.3, 0.25, 0.2, 0.25],
	};
