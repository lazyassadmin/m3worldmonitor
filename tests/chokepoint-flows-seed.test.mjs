...
describe("seed-chokepoint-flows.mjs exports", () => {
	...
	it("has 7 chokepoints with EIA baselines", () => {
		...
		assert.ok(
			...
			`expected 7 canonicalId entries, got ${matches?.length ?? 0}`,
		);
	});
...
describe("flow ratio computation", () => {
	...
	it("Hormuz disruption: 5/day recent vs 60/day baseline ≈ ratio 0.083", () => {
		const history = [...makeDays(7, 5, 0, 0), ...makeDays(90, 60, 0, 7)].sort((a, b) =>
	...
	it("caps at 1.5 for surge scenarios", () => {
		const history = [...makeDays(7, 120, 0, 0), ...makeDays(90, 60, 0, 7)].sort((a, b) =>
			a.date.localeCompare(b.date),
		);
		const last7 = history.slice(-7);
		const prev90 = history.slice(-97, -7);
	...
	it("stays on DWT path when recent capTanker collapses to zero (disruption)", () => {
		...
		assert.ok(
			ratioDwt < ratioCount,
			"DWT ratio should be lower than count ratio during DWT-collapse disruption",
		);
	});
	...
	it("does NOT activate DWT mode on sparse baseline (< 50% days with DWT)", () => {
		...
		assert.equal(
			resolveUseDwt(sparseBaseline),
			false,
			"should not use DWT with <50% baseline coverage",
		);
	});
	...
	it("DWT variant uses capTanker instead of tanker", () => {
		...
		const history = [...makeDays(7, 10, 50000, 0), ...makeDays(90, 60, 50000, 7)].sort((a, b) =>
			a.date.localeCompare(b.date),
		);
		const last7 = history.slice(-7);
		const prev90 = history.slice(-97, -7);
		...
		assert.ok(
			Math.abs(ratioDwt - 1.0) < 0.01,
			`DWT ratio should be ~1.0 (no DWT disruption), got ${ratioDwt}`,
		);
	});
});
