...
describe("Customs revenue handler", () => {
	const handlerSrc = readFileSync(
		join(root, "server/worldmonitor/trade/v1/get-customs-revenue.ts"),
		"utf-8",
	);
...
describe("Customs revenue panel (WTO gate fix)", () => {
	...
	it("does NOT have panel-wide early return for missing WTO key", () => {
		assert.doesNotMatch(
			panelSrc,
			/if \(isDesktopRuntime\(\) && !isFeatureAvailable\('wtoTrade'\)\)\s*\{[\s\S]*?return;\s*\}/,
		);
	});
	...
	it("shows localized Treasury source for revenue tab", () => {
		assert.match(
			panelSrc,
			/activeTab === 'revenue' \? t\('components\.tradePolicy\.sourceTreasury'\)/,
		);
	});
