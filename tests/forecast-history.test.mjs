...
describe("forecast history snapshot", () => {
	it("buildHistorySnapshot stores a compact rolling snapshot", () => {
		...
		const snapshot = buildHistorySnapshot(
			{ generatedAt: 1234, predictions: [rich, thin] },
			{ maxForecasts: 1 },
		);
		...
		assert.deepEqual(snapshot.predictions[0].signals[0], {
			type: "cii",
			value: "Iran CII 87 (critical)",
			weight: 0.4,
		});
	});
});
...
describe("forecast history candidate extraction", () => {
	it("summarizes observed change across consecutive snapshots", () => {
		...
		const current = {
			...
			probability: 0.68,
			trend: "rising",
			signals: [...prior.signals, { type: "ucdp", value: "3 UCDP conflict events", weight: 0.3 }],
			...
			predictions: [
				{
					...
					newsContext: ["Earlier commentary on Iranian posture appears"],
					calibration: {
						marketTitle: "Will Iran conflict escalate before July?",
						marketPrice: 0.455,
					},
...
describe("forecast benchmark promotion", () => {
	it("builds a historical benchmark entry with derived thresholds", () => {
		...
		assert.ok(
			entry.thresholds.changeItemsInclude.some((item) =>
				item.includes("New signal: 3 UCDP conflict events"),
			),
		);
	});

	it("merges a promoted historical entry by append or replace", () => {
		const existing = [{ name: "red_sea_shipping_disruption_2024_01_15", eventDate: "2024-01-15" }];
		const nextEntry = {
		...
		const replaced = mergeHistoricalBenchmarks(
			appended,
			{ ...nextEntry, description: "updated" },
			{ replace: true },
		);
