...
describe("seed Redis key strings", () => {
	it("SPR payload shape matches expected consumer contract", () => {
		...
		assert.equal(
			typeof result.barrels,
			"number",
			"barrels must be a number (already in M bbl — do NOT divide again)",
		);
	});
	...
	it("refinery key follows economic:refinery-inputs:v1 convention", () => {
		...
		assert.ok(result !== null);
		assert.ok(
			"inputsMbblpd" in result,
			"Refinery payload must have inputsMbblpd field (not utilization %)",
		);
...
describe("TTL constants", () => {
	...
	it("REFINERY_INPUTS_TTL is at least 21 days in seconds", () => {
		assert.ok(
			REFINERY_INPUTS_TTL >= 21 * 24 * 3600,
			`REFINERY_INPUTS_TTL ${REFINERY_INPUTS_TTL} < 21 days`,
		);
	});
});
