...
describe("GlobeMap AIS ship traffic markers", () => {
	...
	it("imports AisDisruptionEvent and AisDisruptionType from @/types", () => {
		assert.match(src, /AisDisruptionEvent.*AisDisruptionType/, "GlobeMap should import AIS types");
	});
	...
	it("defines AisDisruptionMarker interface with required fields", () => {
		...
		assert.match(src, /type: AisDisruptionType/, "AisDisruptionMarker must carry type field");
		assert.match(
			src,
	...
	it("includes AisDisruptionMarker in GlobeMarker union", () => {
		...
		assert.ok(
			unionMatch[0].includes("AisDisruptionMarker"),
	...
	it("setAisData maps disruptions to aisMarkers with correct fields", () => {
		...
		assert.match(src, /type: d\.type/, "setAisData must copy type field");
		assert.match(src, /severity: d\.severity/, "setAisData must copy severity field");
		assert.match(src, /description: d\.description/, "setAisData must copy description field");
	});

	it("buildMarkerElement renders aisDisruption with severity-appropriate color", () => {
		assert.match(
			src,
			/d\._kind === 'aisDisruption'/,
			"buildMarkerElement must handle aisDisruption case",
		);
		// Color is severity-based
		assert.match(
			src,
			/d\.severity === 'high'.*#ff2020.*d\.severity === 'elevated'.*#ff8800/s,
