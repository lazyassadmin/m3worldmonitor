...
describe("SCFI parser (functional)", () => {
	...
	it("handles missing percentage field by computing from values", () => {
		const fixture = {
			data: {
				lineDataList: [{ dataItemTypeName: "SCFI_T", currentContent: 110, lastContent: 100 }],
			},
		};
		const result = parseSSEResponse(fixture, "SCFI", "SCFI_T", "test", "index");
		assert.equal(result.length, 1);
		assert.ok(
			Math.abs(result[0].changePct - 10) < 0.01,
			`Expected ~10%, got ${result[0].changePct}`,
		);
	});
});
...
describe("History accumulation (functional)", () => {
	...
	it("deduplicates same-date entries using _observationDate", () => {
		...
		const newIndices = [
			{ indexId: "SCFI", currentValue: 1600, history: [], _observationDate: "2026-03-13" },
		];
	...
	it("uses _observationDate instead of today for history entries", () => {
		...
		const newIndices = [
			{ indexId: "SCFI", currentValue: 1710, history: [], _observationDate: "2026-03-13" },
		];

		const result = accumulateHistory(newIndices, prevPayload);
		assert.equal(result[0].history.length, 2);
		assert.equal(
			result[0].history[1].date,
			"2026-03-13",
			"Should use SSE observation date, not today",
		);
	...
	it("strips _observationDate from output", () => {
		const prevPayload = { indices: [{ indexId: "BDI", history: [] }] };
		const newIndices = [
			{ indexId: "BDI", currentValue: 2000, history: [], _observationDate: "2026-03-14" },
		];
	...
	it("preserves existing history for indices with their own history (FRED)", () => {
		...
		const prevPayload = {
			indices: [{ indexId: "PCU483111483111", history: [{ date: "2025-12-01", value: 95 }] }],
		};
		...
		assert.deepEqual(
			result[0].history,
			fredHistory,
			"Should not overwrite FRED indices that already have history",
		);
	});

	it("handles null/empty previous payload and strips _observationDate", () => {
		const newIndices = [
			{ indexId: "BDI", currentValue: 1900, history: [], _observationDate: "2026-03-14" },
		];
		const result1 = accumulateHistory(newIndices, null);
		assert.equal(result1[0].history.length, 0, "Null payload: history stays empty");
		assert.equal(
			result1[0]._observationDate,
			undefined,
			"_observationDate stripped on null payload",
		);

		const result2 = accumulateHistory(
			[{ indexId: "BDI", currentValue: 1900, history: [], _observationDate: "2026-03-14" }],
			{ indices: [] },
		);
	...
	it("merges history for new index not in previous payload", () => {
		const prevPayload = {
			indices: [{ indexId: "SCFI", history: [{ date: "2026-03-01", value: 1500 }] }],
		};
...
describe("Handler cache-only (get-shipping-rates.ts)", () => {
	const handlerSrc = readFileSync(
		resolve(root, "server/worldmonitor/supply-chain/v1/get-shipping-rates.ts"),
		"utf-8",
	);
