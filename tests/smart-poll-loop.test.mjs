...
describe("startSmartPollLoop", () => {
	...
	describe("scheduling", () => {
		it("fires first tick after intervalMs", async () => {
			...
			startSmartPollLoop(
				...
				{ intervalMs: 5_000, jitterFraction: 0 },
			);
	...
	describe("backoff", () => {
		...
		it("caps at maxBackoffMultiplier", async () => {
			...
			timers.advanceBy(1_000);
			await Promise.resolve(); // 1x
			timers.advanceBy(2_000);
			await Promise.resolve(); // 2x
			timers.advanceBy(4_000);
			await Promise.resolve(); // 4x (cap)
			assert.equal(calls, 3);

			timers.advanceBy(4_000);
			await Promise.resolve(); // still 4x
			assert.equal(calls, 4);
		});
	});
	...
	describe("hiddenIntervalMs", () => {
		it("explicit hidden interval overrides multiplier", async () => {
			...
			startSmartPollLoop(
				...
				{
					intervalMs: 1_000,
					hiddenMultiplier: 100,
					hiddenIntervalMs: 3_000,
					jitterFraction: 0,
					pauseWhenHidden: false,
				},
			);
	...
	describe("visibilityHub integration", () => {
		...
		it("uses hub callbacks for visibility changes instead of direct DOM listener", () => {
			...
			assert.equal(
				doc._listenerCount("visibilitychange"),
				0,
				"no direct DOM listener added when hub provided",
			);
...
describe("VisibilityHub", () => {
	...
	it("removes the DOM listener when the last subscriber unsubscribes", () => {
		...
		assert.equal(
			doc._listenerCount("visibilitychange"),
			0,
			"listener removed when subscriber count reaches 0",
		);
		hub.destroy();
	});
	...
	it("multiple subscribers share one DOM listener", () => {
		...
		assert.equal(
			doc._listenerCount("visibilitychange"),
			1,
			"N subscribers → exactly 1 DOM listener",
		);
		hub.destroy();
	});
});
