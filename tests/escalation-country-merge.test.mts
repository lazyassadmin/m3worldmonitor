...
describe("escalation adapter — country normalization structure", () => {
	...
	it("normalizeToCode is NOT exported", () => {
		...
		assert.match(src, /function\s+normalizeToCode/, "normalizeToCode function must exist");
	});

	it("nameToCountryCode runs before the 2-char fast path", () => {
		const fnBody = src.slice(
			src.indexOf("function normalizeToCode"),
			src.indexOf("const ESCALATION_KEYWORDS"),
		);
		...
		assert.ok(
			nameIdx < twoCharIdx,
			"nameToCountryCode must run BEFORE the 2-char fast path to resolve aliases like UK->GB",
		);
	});
...
describe("escalation adapter — behavioral country normalization", () => {
		...
		mock.method(globalThis, "fetch", (url: string | URL | Request, init?: RequestInit) => {
			...
			if (urlStr.includes("countries.geojson")) {
				return Promise.resolve(
					new Response(JSON.stringify(MOCK_GEOJSON), {
						status: 200,
	...
	it('generateTitle shows full name "Iran" not code "IR"', async () => {
		...
		assert.ok(
			title.includes("news escalation"),
			`title should contain "news escalation", got "${title}"`,
		);
	});
	...
	it('protest "Iran" and news "IR" normalize to same code for clustering', async () => {
		...
		assert.ok(
			iranSignals.length >= 2,
			`expected at least 2 signals with country "IR", got ${iranSignals.length}`,
		);
		assert.equal(
			nonIrSignals.length,
			0,
			`no signals should have country other than "IR", found: ${nonIrSignals.map((s) => s.country)}`,
		);
	});
