...
import { resolveR2StorageConfig } from "../scripts/_r2-storage.mjs";
import { evaluateForecastRunArtifacts } from "../scripts/evaluate-forecast-run.mjs";
import { diffForecastRuns } from "../scripts/diff-forecast-runs.mjs";
...
describe("forecast trace artifact builder", () => {
	it("builds manifest, summary, and per-forecast trace artifacts", () => {
		...
		const b = makePrediction(
			"supply_chain",
			"Red Sea",
			"Shipping disruption: Red Sea",
			0.68,
			0.59,
			"7d",
			[
		...
		assert.match(
			artifacts.worldStateKey,
			/forecast-runs\/2026\/03\/15\/run-123\/world-state\.json/,
		);
		...
		assert.match(
			artifacts.fastSummaryKey,
			/forecast-runs\/2026\/03\/15\/run-123\/fast-summary\.json/,
		);
		assert.match(
			artifacts.fastWorldStateKey,
			/forecast-runs\/2026\/03\/15\/run-123\/fast-world-state\.json/,
		);
		...
		assert.ok(
			artifacts.summary.worldStateSummary.candidateStateSummary.situationCount >=
				artifacts.summary.worldStateSummary.situationCount,
		);
		...
		assert.equal(
			artifacts.summary.triggerContext.triggerRequest.requester,
			"seed-military-flights",
		);
		...
		assert.ok(
			typeof artifacts.summary.worldStateSummary.marketInputCoverage?.loadedSourceCount ===
				"number",
		);
		...
		if (simulations.length > 0) {
			assert.ok(
				typeof forecastWorldState.situationId === "string" &&
					forecastWorldState.situationId.length > 0,
				"worldState.situationId should be set from simulation",
			);
			assert.ok(
				typeof forecastWorldState.simulationSummary === "string" &&
					forecastWorldState.simulationSummary.length > 0,
				"worldState.simulationSummary should be set from simulation",
			);
			assert.ok(
				["escalatory", "contested", "constrained"].includes(forecastWorldState.simulationPosture),
				"worldState.simulationPosture should be a valid posture",
			);
			assert.ok(
				typeof forecastWorldState.simulationPostureScore === "number",
				"worldState.simulationPostureScore should be a number",
			);
		}
	});
	...
	it("derives sidecar artifact keys for fast and deep lifecycle files", () => {
		const keys = buildForecastTraceArtifactKeys(
			"1774288939672-9bvvqa",
			Date.parse("2026-03-23T18:02:19.672Z"),
			"seed-data/forecast-traces",
		);
		assert.match(
			keys.summaryKey,
			/seed-data\/forecast-traces\/2026\/03\/23\/1774288939672-9bvvqa\/summary\.json/,
		);
	...
	it("stores full canonical narrative fields alongside compact short fields in trace artifacts", () => {
		const pred = makePrediction(
			"market",
			"Strait of Hormuz",
			"Energy repricing risk: Strait of Hormuz",
			0.71,
			0.64,
			"30d",
			[
				{
					...
					value: "Strait of Hormuz rerouting is keeping freight costs elevated.",
					weight: 0.38,
				},
			],
		);
		...
		pred.scenario =
			"Strait of Hormuz shipping disruption keeps freight and energy repricing active across the Gulf over the next 30d while LNG routes, tanker insurance costs, and importer hedging behavior continue to amplify the base path across multiple downstream markets and policy-sensitive sectors.";
		pred.feedSummary =
			"Strait of Hormuz disruption is still anchoring the main market path through higher freight, wider energy premia, and persistent rerouting pressure across Gulf-linked trade flows, even as participants avoid assuming a full corridor closure.";
	...
	it("summarizes fallback, enrichment, and domain quality across traced forecasts", () => {
		...
		const b = makePrediction(
			"cyber",
			"China",
			"Cyber threat concentration: China",
			0.6,
			0.52,
			"7d",
			[
				{ type: "cyber", value: "Malware-hosting concentration remains elevated", weight: 0.4 },
				{
					...
					value: "Security researchers warn of renewed activity",
					weight: 0.2,
				},
			],
		);
		...
		const artifacts = buildForecastTraceArtifacts(
			{
				...
				enrichmentMeta: {
					selection: {
						candidateCount: 2,
						readinessEligibleCount: 2,
						selectedCombinedCount: 1,
						selectedScenarioCount: 1,
						reservedScenarioDomains: ["market"],
					},
					combined: {
						requested: 1,
						source: "live",
	...
	it("projects published situations from the original full-run clusters without re-clustering ranked subsets", () => {
		...
		const d = makePrediction(
			"supply_chain",
			"Red Sea",
			"Container rerouting risk: Red Sea",
			0.68,
			0.57,
			"7d",
		...
		const projectedClusters = projectSituationClusters(
		...
		assert.equal(
			projectedClusters.reduce((sum, cluster) => sum + cluster.forecastCount, 0),
	...
	it("preserves llm narratives after projected situation context refresh", () => {
		const a = makePrediction(
			"market",
			"Strait of Hormuz",
			"Inflation and rates pressure from Strait of Hormuz maritime disruption state",
			0.72,
			0.66,
			"30d",
			[
				{
					type: "shipping_cost_shock",
					value: "Hormuz shipping costs are feeding inflation pressure",
					weight: 0.42,
				},
			],
		);
			...
			[
				{
					...
					value: "Oil pricing remains sensitive to Strait of Hormuz disruption",
					weight: 0.38,
				},
			],
		);
		...
		a.caseFile.baseCase =
			"LLM base case keeps Hormuz inflation pressure elevated while freight rerouting and insurance costs remain sticky.";
		a.caseFile.escalatoryCase =
			"LLM escalatory case sees a broader price shock if corridor access worsens again.";
		a.caseFile.contrarianCase =
			"LLM contrarian case assumes shipping normalization starts before downstream pass-through broadens.";
		a.scenario =
			"LLM scenario keeps Hormuz repricing elevated without a full break in corridor access.";
		a.feedSummary =
			"LLM base case keeps Hormuz inflation pressure elevated while freight rerouting and insurance costs remain sticky.";
...
describe("market transmission macro state", () => {
	it("uses live-shaped macro and market payloads to form energy-aware world signals and keep market consequences selective", () => {
		...
		const political = makePrediction(
			"political",
			"United States",
			"US sovereign risk repricing",
			0.58,
			0.56,
			"30d",
		...
		const worldState = buildForecastRunWorldState({
			...
			inputs: {
				...
				bisExchangeRates: {
					exchange: {
						rates: [{ countryCode: "TRY", countryName: "Turkey", realChange: 3.2 }],
					},
				},
				bisPolicyRates: {
					policy: {
						rates: [{ countryCode: "BR", countryName: "Brazil", rate: 11.25, previousRate: 10.75 }],
					},
					exchange: {
						rates: [{ countryCode: "MX", countryName: "Mexico", realChange: 2.7 }],
					},
				},
				correlationCards: {
					military: [{ domain: "military", title: "Carrier posture and sanctions correlation" }],
					markets: [{ domain: "market", title: "Economic sanctions and commodity correlation" }],
				},
				fredSeries: {
					VIXCLS: fredSeries("VIXCLS", [
						["2026-02-01", 18.2],
						["2026-03-01", 23.4],
					]),
					...
					DCOILWTICO: fredSeries("DCOILWTICO", [
						["2026-01-20", 74.8],
						["2026-03-01", 86.6],
					]),
				},
		...
		const buckets = new Map(
			(worldState.marketState?.buckets || []).map((bucket) => [bucket.id, bucket]),
		);
		...
		assert.ok(
			(buckets.get("energy")?.pressureScore || 0) >= (buckets.get("defense")?.pressureScore || 0),
		);
...
describe("publish selection", () => {
	it("prefers unique state anchors before taking same-state follow-ons", () => {
		...
		a.stateContext = {
			...
			forecastCount: 3,
			topSignals: [{ type: "sovereign_stress" }],
		};
		b.stateContext = {
			id: "state-a",
			label: "State A",
...
describe("state-driven domain derivation", () => {
	it("derives market and supply-chain forecasts from strong state transmission when legacy detectors miss", () => {
		const base = makePrediction(
			"conflict",
			"Red Sea",
			"Escalation risk: Red Sea maritime pressure",
			0.72,
			0.61,
			"7d",
			[
				{
					type: "shipping_cost_shock",
					value: "Shipping costs are surging around the Red Sea corridor",
					weight: 0.4,
				},
				{
					type: "energy_supply_shock",
					value: "Energy flows remain exposed to Red Sea disruption",
					weight: 0.35,
				},
			],
		);
			...
			stateUnits: [
				{
					...
					avgConfidence: 0.61,
					topSignals: [
						{ type: "shipping_cost_shock", count: 3 },
						{ type: "energy_supply_shock", count: 2 },
					],
	...
	it("uses a state-derived backfill only when scores miss the main threshold but clear the fallback floor", () => {
		const base = makePrediction(
			"conflict",
			"Red Sea",
			"Escalation risk: constrained maritime pressure",
			0.5,
			0.45,
			"7d",
			[
		...
		const legacySupplyChain = makePrediction(
			"supply_chain",
			"Red Sea",
			"Supply chain disruption: Red Sea corridor",
			0.41,
			0.39,
			"7d",
			[
				{
					type: "shipping_cost_shock",
					value: "Shipping costs remain elevated around the corridor",
					weight: 0.22,
				},
			],
		);
	...
	it("does not derive a market forecast when the direct bucket only has an allowed but semantically mismatched channel", () => {
		const base = makePrediction(
			...
			[
				{
					type: "shipping_cost_shock",
					value: "Shipping routes remain under pressure",
					weight: 0.35,
				},
			],
		);
		...
		assert.equal(
			derived.some((pred) => pred.domain === "market"),
			false,
		);
	});

	it("keeps state-derived market clustering coherent across source states and buckets", () => {
		const indiaFx = makePrediction(
			"market",
			"India",
			"FX stress from India cyber pressure state",
			0.58,
			0.56,
			"14d",
			[
				{
					...
					value: "Risk-off pricing is pressuring India FX",
					weight: 0.36,
				},
			],
		);
		...
		const redSeaEnergy = makePrediction(
			"market",
			"Red Sea",
			"Energy repricing risk from Red Sea maritime disruption state",
			0.66,
			0.59,
			"14d",
			[
				{
					type: "energy_supply_shock",
					value: "Red Sea disruption is pressuring energy flows",
					weight: 0.4,
				},
			],
		);
		...
		const redSeaFreight = makePrediction(
			"supply_chain",
			"Red Sea",
			"Maritime energy flow disruption from Red Sea maritime disruption state",
			0.64,
			0.58,
			"14d",
			[
				{
					type: "shipping_cost_shock",
					value: "Freight routes are rerouting around the Red Sea corridor",
					weight: 0.39,
				},
			],
		);
		...
		const indiaCluster = marketLikeClusters.find((cluster) =>
			cluster.forecastIds.includes(indiaFx.id),
		);
		const redSeaCluster = marketLikeClusters.find(
			(cluster) =>
				cluster.forecastIds.includes(redSeaEnergy.id) ||
				cluster.forecastIds.includes(redSeaFreight.id),
		);
...
describe("forecast run world state", () => {
	it("builds a canonical run-level world state artifact", () => {
		const a = makePrediction("conflict", "Iran", "Escalation risk: Iran", 0.74, 0.64, "7d", [
			...
			{
				type: "news_corroboration",
				value: "Regional officials warn of retaliation risk",
				weight: 0.3,
			},
		]);
		a.newsContext = ["Regional officials warn of retaliation risk"];
		...
		assert.ok(
			worldState.actorContinuity.noLongerActivePreview.some((actor) => actor.id === "legacy:state"),
		);
		...
		assert.ok(
			worldState.simulationState.situationSimulations.every((unit) => unit.rounds.length === 3),
		);
	...
	it("keeps broad non-maritime pressure states from merging across regions without a real spine", () => {
		const germanyCyber = makePrediction(
			"cyber",
			"Germany",
			"Cyber pressure: Germany telecom networks",
			0.58,
			0.56,
			"14d",
			[
				{
					type: "cyber",
					value: "Germany telecom intrusion pressure remains elevated.",
					weight: 0.38,
				},
			],
		);
		const usCyber = makePrediction(
			"cyber",
			"United States",
			"Cyber pressure: United States grid networks",
			0.57,
			0.55,
			"14d",
			[
				{
					type: "cyber",
					value: "United States grid intrusion pressure remains elevated.",
					weight: 0.37,
				},
			],
		);
		...
		germanyCyber.caseFile.actors = [
			{ id: "actor-germany-cert", name: "German CERT", role: "defender" },
		];
		usCyber.caseFile.actors = [
			{ id: "actor-us-grid", name: "US Grid Operators", role: "defender" },
		];
		...
		assert.equal(
			worldState.situationClusters.some(
				(cluster) =>
					cluster.regions.includes("Germany") && cluster.regions.includes("United States"),
			),
			false,
		);
		assert.equal(
			worldState.stateUnits.some(
				(unit) => unit.regions.includes("Germany") && unit.regions.includes("United States"),
			),
			false,
		);
	});
			...
			[
				{
					...
					value: "Oil contracts reprice on Strait of Hormuz risk",
					weight: 0.4,
				},
	...
	it("marks fading pressures for situations present in prior state but absent from current run", () => {
		...
		assert.ok(
			worldState.reportContinuity.fadingPressurePreview.every(
	...
	it("does not collapse unrelated cross-country conflict and political forecasts into one giant situation", () => {
		const conflictIran = makePrediction(
			"conflict",
			"Iran",
			"Escalation risk: Iran",
			0.74,
			0.64,
			"7d",
		...
		const conflictBrazil = makePrediction(
			"conflict",
			"Brazil",
			"Active armed conflict: Brazil",
			0.68,
			0.44,
			"7d",
		...
		const politicalTurkey = makePrediction(
			"political",
			"Turkey",
			"Political instability: Turkey",
			0.43,
			0.52,
			"14d",
	...
	it("does not describe a lower-probability situation as strengthened just because it expanded", () => {
		...
		assert.ok(
			worldState.report.situationWatchlist.every((item) => item.type !== "strengthened_situation"),
		);
	});
	...
	it("builds deterministic simulation units and round transitions from clustered situations", () => {
		...
		const supply = makePrediction(
			...
			"Shipping disruption: Eastern Mediterranean",
			0.59,
			0.55,
			"14d",
			[
				{
					type: "chokepoint",
					value: "Shipping reroutes through the Eastern Mediterranean",
					weight: 0.4,
				},
			],
		);
		...
		assert.ok(
			worldState.simulationState.roundTransitions.every((round) => round.situationCount >= 1),
		);
		...
		assert.ok(
			worldState.simulationState.situationSimulations.every((unit) =>
				["escalatory", "contested", "constrained"].includes(unit.posture),
			),
		);
		assert.ok(
			worldState.simulationState.situationSimulations.every((unit) =>
				unit.rounds.every((round) => typeof round.netPressure === "number"),
			),
		);
		assert.ok(
			worldState.simulationState.situationSimulations.every(
				(unit) => Array.isArray(unit.actionPlan) && unit.actionPlan.length === 3,
			),
		);
		assert.ok(
			worldState.simulationState.situationSimulations.every((unit) =>
				unit.actionPlan.every((round) => Array.isArray(round.actions)),
			),
		);
	});

	it("derives differentiated simulation postures from actor actions, branches, and counter-evidence", () => {
		const escalatory = makePrediction(
			"conflict",
			"Israel",
			"Active armed conflict: Israel",
			0.88,
		...
		const constrained = makePrediction(
			"infrastructure",
			"Cuba",
			"Infrastructure cascade risk: Cuba",
			0.28,
			0.44,
			"14d",
		...
		constrained.caseFile.counterEvidence = [
			{
				type: "confidence",
				summary: "Confidence remains limited and the pattern is not yet broad.",
				weight: 0.3,
			},
		...
		const escalatoryUnit = worldState.simulationState.situationSimulations.find((unit) =>
			unit.label.includes("Israel"),
		);
		const constrainedUnit = worldState.simulationState.situationSimulations.find((unit) =>
			unit.label.includes("Cuba"),
		);
		assert.equal(escalatoryUnit?.posture, "escalatory");
		assert.equal(constrainedUnit?.posture, "constrained");
		assert.ok(
			escalatoryUnit?.rounds.some(
				(round) => (round.actionMix?.pressure || 0) > (round.actionMix?.stabilizing || 0),
			),
		);
		assert.ok(
			constrainedUnit?.rounds.some(
				(round) => (round.actionMix?.stabilizing || 0) >= (round.actionMix?.pressure || 0),
			),
		);
	});
	...
	it("keeps moderate market and supply-chain situations contested unless pressure compounds strongly", () => {
		const market = makePrediction("market", "Japan", "Oil price impact: Japan", 0.58, 0.56, "30d", [
			{
				type: "prediction_market",
				value: "Oil contracts reprice on Japan energy risk",
				weight: 0.3,
			},
		...
		const marketUnit = worldState.simulationState.situationSimulations.find((unit) =>
			unit.label.includes("Japan"),
		);
		const supplyUnit = worldState.simulationState.situationSimulations.find((unit) =>
			unit.label.includes("Red Sea"),
		);
	...
	it("builds report outputs from simulation outcomes and cross-situation effects", () => {
		...
		assert.ok(
			worldState.report.simulationOutcomeSummaries.every((item) => item.rounds.length === 3),
		);
		assert.ok(
			worldState.report.simulationOutcomeSummaries.every((item) =>
				["escalatory", "contested", "constrained"].includes(item.posture),
			),
		);
		...
		assert.ok(
			worldState.report.crossSituationEffects.some((item) => item.summary.includes("Japan")),
		);
	...
	it("does not synthesize cross-situation effects for unrelated theaters with no overlap", () => {
		const brazilConflict = makePrediction(
			"conflict",
			"Brazil",
			"Active armed conflict: Brazil",
			0.77,
		...
		const japanMarket = makePrediction(
			"market",
			"Japan",
			"Market repricing: Japan",
			0.58,
			0.54,
			"30d",
	...
	it("uses the true dominant domain when deriving simulation report inputs and effects", () => {
		const supplyA = makePrediction(
			"supply_chain",
			"Middle East",
			"Shipping disruption: Middle East",
			0.66,
			0.57,
			"14d",
		...
		const supplyB = makePrediction(
			"supply_chain",
			"Middle East",
			"Logistics delay: Middle East",
			0.62,
			0.55,
			"14d",
		...
		const market = makePrediction(
			...
			[
				{
					type: "prediction_market",
					value: "Oil contracts reprice on logistics risk",
					weight: 0.3,
				},
			],
		);
		...
		const dominantInput = worldState.report.simulationOutcomeSummaries.find((item) =>
			item.label.includes("Middle East"),
		);
		const dominantSimulation = worldState.simulationState.situationSimulations.find((item) =>
			item.label.includes("Middle East"),
		);
	...
	it("does not emit cross-situation effects from constrained low-energy infrastructure situations", () => {
		const cuba = makePrediction(
			"infrastructure",
			"Cuba",
			"Infrastructure degradation: Cuba",
			0.29,
			0.45,
			"14d",
			[
				{
					type: "outage",
					value: "Localized infrastructure outages remain contained in Cuba",
					weight: 0.25,
				},
			],
		);
		...
		const iran = makePrediction(
			"infrastructure",
			"Iran",
			"Infrastructure degradation: Iran",
			0.31,
			0.46,
			"14d",
			[
				{
					type: "outage",
					value: "Localized infrastructure outages remain contained in Iran",
					weight: 0.25,
				},
			],
		);
		...
		iran.caseFile.counterEvidence = [
			{
				type: "containment",
				summary: "Containment actions are limiting broader spread.",
				weight: 0.35,
			},
		];
	...
	it("allows cyber sources above the domain constrained threshold to emit direct effects", () => {
		const cyber = makePrediction(
			"cyber",
			"Poland",
			"Cyber disruption risk: Poland",
			0.46,
			0.54,
			"14d",
			[
				{
					type: "cyber",
					value: "Cyber disruption pressure remains elevated across Poland",
					weight: 0.35,
				},
			],
		);
		...
		const infrastructure = makePrediction(
			"infrastructure",
			"Baltic States",
			"Infrastructure disruption risk: Baltic States",
			0.41,
			0.52,
			"14d",
			[
				{
					type: "outage",
					value: "Infrastructure resilience is under pressure in the Baltic States",
					weight: 0.3,
				},
			],
		);
		...
		const patchedSimulationState = structuredClone(worldState.simulationState);
		const cyberUnit = patchedSimulationState.situationSimulations.find((item) =>
			item.label.includes("Poland"),
		);
		assert.ok(cyberUnit);
	...
	it("keeps direct regional spillovers when a source only contributes one matching channel but has direct overlap", () => {
		const cyber = makePrediction("cyber", "Estonia", "Cyber pressure: Estonia", 0.47, 0.53, "14d", [
			{
				type: "cyber",
				value: "Regional cyber pressure remains elevated around Estonia",
				weight: 0.32,
			},
		]);
		buildForecastCase(cyber);
		...
		patchedSimulationState.interactionLedger = (patchedSimulationState.interactionLedger || []).map(
			(item) => ({
				...item,
				confidence: 0.94,
		...
		assert.ok(
			effects.some(
				(item) =>
					item.channel === "regional_spillover" && item.relation === "regional pressure transfer",
			),
		);
	});
	...
	it("emits reverse-direction effects when only the later-listed situation can drive the target", () => {
		const worldState = buildForecastRunWorldState({
			...
			predictions: [
				makePrediction(
					"infrastructure",
					"Romania",
					"Infrastructure pressure: Romania",
					0.34,
					0.48,
					"14d",
				...
				makePrediction("market", "Black Sea", "Market repricing: Black Sea", 0.57, 0.56, "14d", [
					{
						type: "prediction_market",
						value: "Black Sea pricing reacts to service disruption risk",
						weight: 0.36,
					},
				]),
			],
		...
		const patchedSimulationState = structuredClone(worldState.simulationState);
		const infraUnit = patchedSimulationState.situationSimulations.find(
			(item) => item.dominantDomain === "infrastructure",
		);
		const marketUnit = patchedSimulationState.situationSimulations.find(
			(item) => item.dominantDomain === "market",
		);
		assert.ok(infraUnit);
		...
		patchedSimulationState.reportableInteractionLedger = [
			...patchedSimulationState.interactionLedger,
		];
		...
		assert.ok(
			effects.some(
				(item) =>
					item.sourceSituationId === marketUnit.situationId &&
					item.targetSituationId === infraUnit.situationId,
			),
		);
	});
		...
		const worldState = buildForecastRunWorldState({
			...
			predictions: [
				makePrediction("market", "Black Sea", "Market repricing: Black Sea", 0.56, 0.55, "14d", [
					{
						type: "prediction_market",
						value: "Black Sea pricing reflects service disruption risk",
						weight: 0.36,
					},
				]),
				makePrediction(
					...
					[
						{
							type: "outage",
							value: "Romania infrastructure remains exposed to service disruption",
							weight: 0.3,
						},
					],
				),
	...
	it("uses a cross-regional family label when no single region clearly dominates a family", () => {
		const iranPolitical = makePrediction(
			"political",
			"Iran",
			"Political pressure: Iran",
			0.62,
			0.56,
			"14d",
		...
		const germanyPolitical = makePrediction(
			"political",
			"Germany",
			"Political pressure: Germany",
			0.6,
			0.55,
			"14d",
		...
		assert.ok(worldState.situationFamilies.length >= 1);
		assert.ok(
			worldState.situationFamilies.some((family) => family.label.startsWith("Cross-regional ")),
		);
	});
	...
	it("assigns archetype-aware family labels for maritime supply situations", () => {
		const supplyA = makePrediction(
			...
			[
				{
					type: "chokepoint",
					value: "Shipping disruption persists in the Red Sea corridor",
					weight: 0.4,
				},
			],
		);
		...
		const supplyB = makePrediction(
			"supply_chain",
			"Bab el-Mandeb",
			"Freight rerouting: Bab el-Mandeb",
			0.64,
			0.56,
			"14d",
			[
				{
					type: "gps_jamming",
					value: "Maritime routing disruption persists near Bab el-Mandeb",
					weight: 0.32,
				},
			],
		);
		...
		assert.ok(
			worldState.situationFamilies.some((family) => family.archetype === "maritime_supply"),
		);
		assert.ok(
			worldState.situationFamilies.some((family) => family.label.includes("maritime supply")),
		);
	});

	it("does not infer maritime families from generic port labor talk tokens", () => {
		const portTalks = makePrediction(
			"political",
			"Spain",
			"Port labor talks: Spain",
			0.58,
			0.55,
			"14d",
		...
		const dockStrikePolitics = makePrediction(
			"political",
			"Portugal",
			"Port labor pressure: Portugal",
			0.56,
			0.53,
			"14d",
			[
				{
					type: "policy_change",
					value: "Dockworker negotiations are shaping coalition pressure in Portugal",
					weight: 0.26,
				},
			],
		);
		...
		assert.ok(worldState.situationFamilies.length >= 1);
		assert.ok(
			worldState.situationFamilies.every((family) => family.archetype !== "maritime_supply"),
		);
		assert.ok(
			worldState.situationFamilies.every((family) => !family.label.includes("maritime supply")),
		);
	});

	it("keeps weak generic interactions out of the reportable interaction surface", () => {
		const source = makePrediction(
			"political",
			"Brazil",
			"Political pressure: Brazil",
			0.56,
			0.53,
			"14d",
		...
		const target = makePrediction(
			"political",
			"Israel",
			"Political pressure: Israel",
			0.58,
			0.54,
			"14d",
	...
	it("dedupes the interaction watchlist by source target and channel before report surfacing", () => {
		...
		assert.equal(
			watchlist[0].label,
			"Brazil cyber situation -> United States cyber and political situation",
		);
	...
	it("blocks cross-theater political effects even with shared-actor when actorSpec below 0.90", () => {
		...
		assert.equal(
			effects.length,
			0,
			"US → Japan cross-theater political_pressure at actorSpec 0.87 should be blocked",
		);
	});
	...
	it("ignores incompatible prior simulation momentum when the simulation version changes", () => {
		...
		priorWorldState.simulationState = {
			...priorWorldState.simulationState,
			version: 1,
			situationSimulations: (priorWorldState.simulationState?.situationSimulations || []).map(
				(item) => ({
					...item,
					postureScore: 0.99,
...
describe("cross-theater gate", () => {
	...
	it("blocks Israel → Taiwan via generic Incumbent Leadership (regional_spillover, spec 0.68)", () => {
		...
		assert.equal(
			effects.length,
			0,
			"Israel → Taiwan via generic actor should be blocked by cross-theater gate",
		);
	});
	...
	it("allows China → US via Threat Actors (cyber_disruption, exempt channel)", () => {
		const effects = buildCrossSituationEffects({
			situationSimulations: [
				...
				makeSimulation(
					"sit-us",
					"United States infrastructure situation",
					"infrastructure",
					["United States"],
					"contested",
					0.62,
		...
		assert.equal(
			effects.length,
			1,
			"China (EAST_ASIA) → US (AMERICAS) via cyber_disruption should pass (exempt channel)",
		);
	...
	it("blocks Brazil → Israel conflict via External Power Broker (security_escalation, spec 0.85 < 0.90)", () => {
		...
		assert.equal(
			effects.length,
			0,
			"Brazil → Israel via generic external actor should be blocked (actorSpec 0.85 < 0.90)",
		);
	});
	...
	it("allows Brazil → Mexico (same macro-region, security_escalation → infrastructure)", () => {
		const effects = buildCrossSituationEffects({
			situationSimulations: [
				...
				makeSimulation(
					"sit-mexico",
					"Mexico infrastructure situation",
					"infrastructure",
					["Mexico"],
					"escalatory",
					0.72,
		...
		assert.equal(
			effects.length,
			1,
			"Brazil → Mexico should pass (both AMERICAS, cross-theater gate does not apply)",
		);
		assert.equal(effects[0].channel, "security_escalation");
	});
	...
	it("blocks Cuba → Iran infrastructure (cross-theater, service_disruption, spec 0.73 < 0.90)", () => {
		...
		assert.equal(
			effects.length,
			0,
			"Cuba → Iran via generic civil-protection actor should be blocked",
		);
	});
});
...
describe("impact expansion layer", () => {
	function makeImpactCandidatePacket(
		stateId = "state-1",
		...
		return {
			...
			evidenceTable: [
				{ key: "E1", kind: "state_summary", text: "Strait of Hormuz shipping pressure is active." },
				{
					key: "E2",
					kind: "headline",
					text: "Qatar LNG export risk is rising as route security deteriorates.",
				},
			],
			marketContext: {
				topBucketId: "energy",
				...
				consequenceSummary:
					"Strait of Hormuz is transmitting into Energy through gas supply stress.",
			},
			stateSummary: {
				avgProbability: 0.71,
				avgConfidence: 0.63,
	...
	function makeImpactExpansionBundle(
		stateId = "state-1",
		label = "Strait of Hormuz maritime disruption state",
		packetOverrides = {},
	) {
					...
					thirdOrderHypotheses: [
						{
							...
							summary:
								"Funding stress follows if the inflation shock broadens into sovereign repricing.",
							evidenceRefs: ["E2"],
						},
					],
				},
	...
	it("keeps impact-expansion cache hashes stable when source situation ids churn", () => {
		const left = makeImpactCandidatePacket(
			"state-1",
			"Strait of Hormuz maritime disruption state",
			{
				sourceSituationIds: ["sit-a"],
			},
		);
		const right = makeImpactCandidatePacket(
			"state-1",
			"Strait of Hormuz maritime disruption state",
			{
	...
	it("validates exact evidence refs and maps only strong hypotheses", () => {
		...
		const direct = validation.hypotheses.find(
			(item) => item.order === "direct" && item.variableKey === "lng_export_stress",
		);
		const secondOrder = validation.hypotheses.find(
			(item) => item.order === "second_order" && item.variableKey === "inflation_pass_through",
		);
		const thirdOrder = validation.hypotheses.find(
			(item) => item.order === "third_order" && item.variableKey === "sovereign_funding_stress",
		);
	...
	it("only builds expanded paths from chains that include a second-order hypothesis", () => {
		...
		const directOnlyPaths = buildImpactPathsForCandidate(
			directOnlyBundle.candidatePackets[0],
			directOnlyValidation,
		);
		...
		const chainedPaths = buildImpactPathsForCandidate(
			makeImpactExpansionBundle().candidatePackets[0],
			chainedValidation,
		);
	...
	it("selects only high-salience deep candidates from the ranked impact-expansion packets", () => {
		const eligible = makeImpactCandidatePacket(
			"state-eligible",
			"Strait of Hormuz maritime disruption state",
			{
				rankingScore: 0.71,
		...
		const ineligible = makeImpactCandidatePacket(
			"state-ineligible",
			"Low-salience market repricing state",
			{
				rankingScore: 0.59,
				routeFacilityKey: "",
				commodityKey: "",
	...
	it("preserves deep forecast metadata when trace artifacts are rebuilt from a deep world-state override", () => {
		const prediction = makePrediction(
			...
			[
				{
					type: "energy_supply_shock",
					value: "Transit stress is building across the Strait of Hormuz.",
					weight: 0.4,
				},
			],
		);
		...
		const bundle = makeImpactExpansionBundle(
			baseState.stateUnits[0].id,
			baseState.stateUnits[0].label,
		);
		const validation = validateImpactHypotheses(bundle);
		const paths = buildImpactPathsForCandidate(bundle.candidatePackets[0], validation).filter(
			(path) => path.type === "expanded",
		);
		const deepBundle = buildImpactExpansionBundleFromPaths(
			paths.slice(0, 1),
	...
	it("computes deep reportable quality score from candidate-touching interactions and effects", () => {
		...
		const reportableLedger = [
			{
				sourceSituationId: candidateStateId,
				targetSituationId: "state-2",
				confidence: 0.78,
				score: 5.6,
			},
			{
				sourceSituationId: "state-3",
				targetSituationId: candidateStateId,
				confidence: 0.82,
				score: 5.9,
			},
		];
	...
	it("computes deep market coherence score from mapped hypotheses and admissibility", () => {
		const candidatePacket = makeImpactCandidatePacket(
			"state-1",
			"Strait of Hormuz maritime disruption state",
		);
		const path = {
			...
			simulationState: {
				marketConsequences: {
					...
					blocked: [
						{
							situationId: "state-1",
							bucketId: "rates_inflation",
							channel: "inflation_impulse",
							reason: "inadmissible_bucket_channel",
						},
					],
				},
			},
	...
	it("accepts expanded path and builds deep world state when acceptance score clears 0.50 floor", async () => {
		const prediction = makePrediction(
			...
			[
				{
					type: "shipping_cost_shock",
					value: "Shipping costs are rising around Strait of Hormuz rerouting.",
					weight: 0.5,
				},
				{
					type: "energy_supply_shock",
					value: "Energy transit pressure is building around Qatar LNG flows.",
					weight: 0.32,
				},
			],
		);
		prediction.newsContext = [
			"Tanker rerouting is amplifying LNG and freight pressure around the Gulf.",
		];
		buildForecastCase(prediction);
		populateFallbackNarratives([prediction]);
		...
		const bundle = makeImpactExpansionBundle(stateUnit.id, stateUnit.label, {
			...
			transmissionChannels: stateUnit.transmissionChannels || [
				"shipping_cost_shock",
				"gas_supply_stress",
			],
		...
		assert.ok(
			...
			"deep world state must be built when expanded path accepted",
		);
	});
	...
	it("evaluateDeepForecastPaths includes validation on the mapped=0 early-return path", async () => {
		...
		const evaluation = await evaluateDeepForecastPaths(
			...
			null,
			invalidBundle.candidatePackets,
			invalidBundle,
		);
		...
		assert.ok(
			evaluation.validation.hypotheses.every(
				(h) => typeof h.candidateIndex === "number" && typeof h.candidateStateId === "string",
			),
		);
	});
	...
	it("evaluateDeepForecastPaths includes validation on paths beyond the mapped=0 early return", async () => {
		...
		const prediction = makePrediction(
			...
			[
				{
					type: "shipping_cost_shock",
					value: "Shipping costs rising around Red Sea.",
					weight: 0.5,
				},
			],
		);
		...
		assert.ok(
			(evaluation.validation.mapped || []).length > 0,
			"fixture must produce mapped hypotheses (past mapped=0 early return)",
		);
	...
	it("buildForecastTraceArtifacts surfaces hypothesisValidation in impactExpansionDebug", () => {
		...
		const validationWithRejection = {
			...
			rejectionReasonCounts: {
				...rawValidation.rejectionReasonCounts,
				unsupported_variable_channel: 1,
			},
		};
		...
		assert.ok(
			artifacts.impactExpansionDebug.hypothesisValidation,
			"hypothesisValidation must be present",
		);
		assert.ok(
			typeof artifacts.impactExpansionDebug.hypothesisValidation.totalHypotheses === "number",
		);
		assert.ok(
			typeof artifacts.impactExpansionDebug.hypothesisValidation.validatedCount === "number",
		);
		assert.ok(typeof artifacts.impactExpansionDebug.hypothesisValidation.mappedCount === "number");
		assert.ok(
			typeof artifacts.impactExpansionDebug.hypothesisValidation.rejectionReasonCounts === "object",
		);
		assert.ok(
			artifacts.impactExpansionDebug.hypothesisValidation.rejectionReasonCounts
				.unsupported_variable_channel >= 1,
		);
...
describe("critical news signal extraction", () => {
			...
			},
			{
				categories: {
					geopolitics: {
						items: [
							{
								title: "US issues fresh sanctions on Iran shipping network",
								isAlert: true,
								link: "https://example.com/sanctions",
								pubDate: "2026-03-22T11:35:00.000Z",
							},
						],
					},
				},
		...
		const signals = extractCriticalNewsSignals({
			...
			sanctionsPressure: {
				...
				entries: [
					{
						id: "sp-1",
						name: "Iran tanker network",
						countryCodes: ["IR"],
						countryNames: ["Iran"],
						programs: ["IRAN"],
						isNew: true,
						note: "New sanctions target oil tanker exports",
					},
				],
			},
			thermalEscalation: {
	...
	it("recognizes plural sanctions, airstrike, and blocks phrasing in critical headlines", () => {
		...
		assert.ok(
			signals.some(
				(signal) => signal.sourceType === "critical_news" && signal.type === "sovereign_stress",
			),
		);
		assert.ok(
			signals.some(
				(signal) => signal.sourceType === "critical_news" && signal.type === "energy_supply_shock",
			),
		);
		assert.ok(
			signals.some(
				(signal) => signal.sourceType === "critical_news" && signal.type === "shipping_cost_shock",
			),
		);
	});
	...
	it("extends thermal energy sensitivity to Oman and does not force unknown thermal regions into MENA", () => {
		...
		const omanEnergy = signals.find(
			(signal) =>
				signal.sourceType === "thermal_escalation" &&
				signal.type === "energy_supply_shock" &&
				signal.region === "Oman",
		);
		const unknownInfra = signals.find(
			(signal) =>
				signal.sourceType === "thermal_escalation" &&
				signal.type === "infrastructure_capacity_loss" &&
				signal.region === "Unknown Energy Province",
		);

		assert.ok(omanEnergy, "Oman thermal escalation should now be treated as energy-sensitive");
		assert.equal(
			unknownInfra?.macroRegion || "",
			"",
			"unknown thermal regions should not be forced into MENA",
		);
	});
	...
	it("dedupes corroborated critical events across news and iran event sources in world signals", () => {
		...
		const lngSignals = criticalSignals.filter(
			(signal) =>
				signal.type === "gas_supply_stress" &&
				signal.label === "Middle East LNG and gas export stress",
		);
		const energySignals = criticalSignals.filter(
			(signal) =>
				signal.type === "energy_supply_shock" &&
				signal.label === "Middle East energy infrastructure stress",
		);
	...
	it("triages only urgent free-form critical-news candidates for structured extraction", () => {
		...
		assert.equal(
			candidates[0].title,
			"Iran threatens closure of the Strait of Hormuz after tanker strike",
		);
...
describe("military domain guarantee in publish selection", () => {
	...
	it("injects military forecast when buried below high-scoring non-military forecasts", () => {
		...
		const military = makeMinimalPred("mil-baltic", "military", 0.41, 0.3);
	...
	it("does not double-inject military when it already ranks into selection naturally", () => {
		const forecasts = [
			makeMinimalPred("mil-1", "military", 0.8, 0.75),
			makeMinimalPred("conflict-1", "conflict", 0.6, 0.6),
...
describe("forecast replay lifecycle helpers", () => {
	...
	it("buildCanonicalStateUnits disambiguates label collisions without dropping units", () => {
		...
		const clusterA = {
			...
			avgConfidence: 0.7,
			topSignals: [{ type: "shipping_cost_shock", count: 3 }],
			sampleTitles: ["Red Sea shipping delay"],
			sourceStateIds: [],
			macroRegions: ["EMEA"],
			marketBucketIds: ["freight"],
			transmissionChannels: ["shipping_cost_shock"],
			branchKinds: [],
		};
		const clusterB = {
			id: "cluster-label-b",
			label: "Red Sea oil export disruption",
			dominantRegion: "Red Sea",
			dominantDomain: "supply_chain",
			regions: ["Red Sea"],
			domains: ["supply_chain"],
			...
			avgConfidence: 0.6,
			topSignals: [{ type: "energy_supply_shock", count: 2 }],
			sampleTitles: ["Iranian oil blockade"],
			sourceStateIds: [],
			macroRegions: ["EMEA"],
			marketBucketIds: ["energy"],
			transmissionChannels: ["oil_macro_shock"],
			branchKinds: [],
		};
		...
		const snapValidation = validateDeepForecastSnapshot({
			fullRunStateUnits: units,
			deepForecast: { selectedStateIds: [] },
		});
		assert.equal(
			snapValidation.duplicateStateLabels.length,
			0,
			"validator must see no duplicate labels after disambiguation",
		);

		// Higher-priority unit (higher avgProbability) keeps original label, lower one gets suffix
		const sortedByPriority = [...units].sort(
			(a, b) => b.forecastCount - a.forecastCount || b.avgProbability - a.avgProbability,
		);
		assert.ok(
			sortedByPriority[0].label === "Red Sea maritime disruption state",
			"highest-priority unit keeps clean label",
		);
		assert.ok(
			sortedByPriority[1].label !== "Red Sea maritime disruption state",
			"collision unit gets disambiguated label",
		);
		assert.ok(
			sortedByPriority[1].label.startsWith("Red Sea maritime disruption state"),
			"disambiguated label keeps base",
		);
	});
	...
	it("evaluates a run artifact set against deep lifecycle checks", () => {
		const evaluation = evaluateForecastRunArtifacts({
			...
			snapshot: {
				fullRunStateUnits: [{ id: "state-1", label: "Strait of Hormuz maritime disruption state" }],
		...
		assert.ok(
			evaluation.checks.some(
				(check) => check.name === "reportable_interactions_are_subset" && check.pass === false,
			),
		);
		assert.ok(
			evaluation.checks.some(
				(check) =>
					check.name === "eligible_high_value_deep_run_materializes_mapped_signals" &&
					check.pass === false,
			),
		);
	});
	...
	it("diffs two forecast runs by lifecycle and publication metrics", () => {
		...
		assert.ok(
			diff.addedTopForecastTitles.includes(
				"Supply chain stress from Strait of Hormuz disruption state",
			),
		);
		assert.ok(
			diff.removedTopForecastTitles.includes("FX stress from Germany cyber pressure state"),
		);
	});
});
...
describe("phase 2 scoring recalibration + prompt excellence", () => {
		...
		const packet = {
			...
			evidenceTable: [
				{ key: "E1", kind: "state_summary", text: "Baltic Sea shipping pressure is active." },
				{
					key: "E2",
					kind: "headline",
					text: "Baltic freight rates are climbing on route uncertainty.",
				},
			],
			marketContext: {
				topBucketId: "freight",
	...
	it("T4: expanded path is generated when both direct and second_order are mapped", () => {
		...
		assert.ok(expanded[0].pathScore >= 0.5, `pathScore ${expanded[0].pathScore} must be >= 0.50`);
	});
	...
	it("T5: scoringBreakdown in debug artifact includes ALL hypotheses with scoring factors", () => {
		...
		assert.ok(
			hv.scoringBreakdown.length === enrichedValidation.hypotheses.length,
	...
	it("T6: gateDetails in debug artifact records active thresholds", () => {
		...
		assert.equal(gd.pathScoreThreshold, 0.5);
		assert.equal(gd.acceptanceThreshold, 0.5);
	});

	it("T7: prompt v4 contains all required guidance strings", () => {
		const prompt = buildImpactExpansionSystemPrompt();
		assert.ok(
			prompt.includes("at least 2 evidence keys"),
			"prompt must mention 2-evidence requirement",
		);
		assert.ok(
			prompt.includes("MUST be the exact hypothesisKey of one of your direct"),
			"prompt must have dependsOnKey exactness rule",
		);
		assert.ok(
			prompt.includes("strength 0.82-0.95"),
			"prompt must include confidence calibration guidance",
		);
		assert.ok(
			prompt.includes("direct+second_order pair is the core unit"),
	...
	it("T8: new chokepoints are detected by extractImpactRouteFacilityKey", () => {
		...
		assert.equal(
			extractImpactRouteFacilityKey(["Danish Straits closure impacts Scandinavian trade"]),
			"Danish Straits",
		);
		assert.equal(
			extractImpactRouteFacilityKey(["Strait of Gibraltar blockade scenario"]),
			"Strait of Gibraltar",
		);
		assert.equal(
			extractImpactRouteFacilityKey(["Panama Canal drought cuts transit"]),
			"Panama Canal",
		);
		assert.equal(
			extractImpactRouteFacilityKey(["Lombok Strait alternative route pressure"]),
			"Lombok Strait",
		);
		assert.equal(
			extractImpactRouteFacilityKey(["Cape of Good Hope rerouting surge"]),
			"Cape of Good Hope",
		);
		// Original chokepoints must still work
		assert.equal(
			extractImpactRouteFacilityKey(["Strait of Hormuz tanker attack"]),
			"Strait of Hormuz",
		);
		...
		assert.equal(
			extractImpactRouteFacilityKey(["South China Sea naval tensions"]),
			"South China Sea",
		);
	});
	...
	it("T9: scoreImpactExpansionQuality — high commodity rate + chain coverage yields high composite", () => {
		...
		const validation = {
			hypotheses: [
				...
				{
					order: "second_order",
					variableKey: "shipping_cost_spike",
					targetBucket: "equity",
					validationStatus: "mapped",
					commodity: "crude_oil",
					candidateIndex: 1,
		...
		assert.ok(
			result.directCommodityDiversity === 1.0,
			"2 candidates × 2 unique direct commodities (LNG, crude_oil) → directCommodityDiversity 1.0",
		);
		assert.ok(
			result.candidateSpreadScore === 1.0,
			"2 candidates × 2 hypotheses each → perfectly even spread",
		);
		assert.ok(
			result.chainCoverage === 1.0,
			"both candidates have direct+second → chainCoverage 1.0",
		);
	...
	it("T-conv-1: buildImpactExpansionDebugPayload — converged=true when composite >= 0.80", () => {
		...
		const mapped = [
			{
				order: "direct",
				hypothesisKey: "hormuz_crude_disruption",
				commodity: "crude_oil",
			...
			{
				order: "second_order",
				hypothesisKey: "crude_inflation_pass_through",
				commodity: "crude_oil",
			...
			{
				order: "direct",
				hypothesisKey: "baltic_shipping_cost_spike",
				commodity: "LNG",
			...
			{
				order: "second_order",
				hypothesisKey: "lng_inflation_europe",
				commodity: "LNG",
		...
		const payload = buildImpactExpansionDebugPayload(
			makeConvTestData(mapped, candidatePackets),
			null,
			"run-conv-test",
		);
		...
		assert.ok(
			payload.convergence.converged === true,
			`converged should be true (composite=${payload.convergence.finalComposite})`,
		);
		assert.equal(
			payload.convergence.predictedCritiqueIterations,
			0,
			"no predicted critique iterations when quality good",
		);
		assert.ok(typeof payload.convergence.finalComposite === "number");
		assert.ok(
			payload.convergence.finalComposite >= 0.8,
			`finalComposite should be >= 0.80 (got ${payload.convergence.finalComposite})`,
		);
	});
	...
	it("T-conv-2: buildImpactExpansionDebugPayload — converged=false when composite < 0.80, predictedCritiqueIterations=1", () => {
		...
		const mapped = [
			{
				order: "direct",
				hypothesisKey: "route_disruption",
				commodity: "",
				geography: "",
				affectedAssets: [],
				candidateStateId: "state-A",
		...
		assert.ok(
			...
			`converged should be false (composite=${payload.convergence.finalComposite})`,
		);
		// predictedCritiqueIterations is derived from quality score (refinement is fire-and-forget)
		assert.equal(
			payload.convergence.predictedCritiqueIterations,
			1,
			"predictedCritiqueIterations=1 when composite < 0.80",
		);
		assert.ok(
			payload.convergence.finalComposite < 0.8,
			`finalComposite should be < 0.80 (got ${payload.convergence.finalComposite})`,
...
describe("filterNewsHeadlinesByState", () => {
	...
	it("T-news-2: LNG alert headline scores above threshold and is returned for Hormuz state", () => {
		const insights = makeInsights([
			{
				title: "Qatar LNG tankers rerouted away from Hormuz strait",
				isAlert: true,
				sourceCount: 3,
			},
		]);
		...
		assert.ok(
			result[0].includes("LNG") || result[0].includes("Qatar"),
			`expected LNG headline, got: ${result[0]}`,
		);
	});
	...
	it("T-news-4: returns at most 3 headlines even when more qualify", () => {
		const stories = [
			...
			{
				title: "Gas export terminal shut by Iran sanctions embargo",
				isAlert: true,
				sourceCount: 4,
			},
			{
				title: "LNG prices spike as Strait of Hormuz route blocked",
				isAlert: true,
				sourceCount: 3,
			},
...
describe("buildImpactExpansionEvidenceTable — live_news injection", () => {
	...
	const makeContinuity = () => ({
		summary: "Disruption ongoing for 4 weeks.",
		continuityScore: 0.5,
		continuityMode: "sustained",
	});

	it("T-news-4b: with 3 newsItems, evidence table has 11 entries and last 3 have kind=live_news", () => {
		const newsItems = [
			"Dire fertiliser shortage worsens",
			"LNG tankers rerouted via Cape",
			"Wheat prices hit 2-year high",
		];
		const table = buildImpactExpansionEvidenceTable(
			makeMinimalState(),
			makeMarket(),
			makeContinuity(),
			newsItems,
		...
		assert.ok(
			table.every((e, i) => e.key === `E${i + 1}`),
			"keys should be E1..EN sequentially",
		);
	});
	...
	it("T-news-4c: with no newsItems, evidence table behaves identically to before (cap 8)", () => {
		...
		assert.ok(
			table.every((e) => e.kind !== "live_news"),
			"no live_news entries when newsItems is empty",
		);
	});
});
...
describe("IMPACT_COMMODITY_LEXICON — extended entries", () => {
	...
	it("T-lex-3: extractImpactCommodityKey returns fertilizer for fertiliser/nitrogen/ammonia text", () => {
		assert.equal(
			extractImpactCommodityKey(["fertiliser shortage due to Hormuz crisis"]),
			"fertilizer",
		);
...
describe("simulation package export", () => {
			...
			evidenceTable: [
				{
					key: "E1",
					kind: "state_summary",
					text: "Strait of Hormuz (route_blockage) is centered on Strait of Hormuz.",
				},
				{ key: "E2", kind: "headline", text: "Tanker attack near Strait of Hormuz" },
				{
					key: "E3",
					kind: "signal",
					text: "shipping_cost_shock active across 4 linked forecasts.",
				},
				{
					key: "E4",
					kind: "actor",
					text: "Iran IRGC, US Navy Fifth Fleet, Saudi Aramco remain the lead actors in this state.",
				},
			],
			...overrides,
		};
	...
	it("T-E8: SIMULATION_ELIGIBILITY_RANK_THRESHOLD is exported and equals 0.40", () => {
		assert.equal(SIMULATION_ELIGIBILITY_RANK_THRESHOLD, 0.4);
	});

	it("buildSimulationPackageFromDeepSnapshot returns null when no qualifying candidates", () => {
		const pkg = buildSimulationPackageFromDeepSnapshot(
			makeSnapshot([
				makeCandidate({
					rankingScore: 0.1,
					marketBucketIds: [],
	...
	it("buildSimulationPackageFromDeepSnapshot produces v1 schema with all required top-level fields", () => {
		...
		assert.ok(
			pkg.constraints && typeof pkg.constraints === "object" && !Array.isArray(pkg.constraints),
		);
		assert.ok(
			pkg.evaluationTargets &&
				typeof pkg.evaluationTargets === "object" &&
				!Array.isArray(pkg.evaluationTargets),
		);
	});
	...
	it("evaluationTargets is keyed by theaterId with escalation, containment, market_cascade paths", () => {
		...
		assert.deepEqual(target.requiredOutputs, [
			"key_invalidators",
			"timing_markers",
			"actor_response_summary",
		]);
	...
	it("geo-dedup: Strait of Hormuz (MENA_Gulf) and Red Sea (MENA_RedSea) are distinct groups — both selected", () => {
		...
		assert.equal(
			pkg.selectedTheaters.length,
			3,
			"all 3 should be selected — Hormuz (MENA_Gulf), Red Sea (MENA_RedSea), Malacca (AsiaPacific)",
		);
		...
		assert.ok(
			routeKeys.includes("Red Sea"),
			"Red Sea must be selected — it is a distinct geo group from Hormuz",
		);
	...
	it("label cleanup: (stateKind) suffix is stripped from theater label", () => {
		...
		assert.ok(
			!label.includes("(supply_chain)"),
			`label must not contain stateKind suffix, got: "${label}"`,
		);
		assert.equal(
			label,
			"Black Sea maritime disruption state",
			`label should be stripped, got: "${label}"`,
		);
	});
	...
	it('inferEntityClassFromName does not classify "Salesforce Inc" as military', () => {
		...
		assert.notEqual(
			cls,
			"military_or_security_actor",
			`"Salesforce Inc" must not be military — got ${cls}`,
		);
	});
	...
	it('inferEntityClassFromName classifies "workforce solutions" as non-military', () => {
		...
		assert.notEqual(
			cls,
			"military_or_security_actor",
			`"workforce solutions" must not be military — got ${cls}`,
		);
	});
	...
	it("entities from two candidates from different geo-groups but same actor name are both present", () => {
		...
		const candidateB = makeCandidate({
			candidateStateId: "state-malacca-1",
			dominantRegion: "South China Sea",
			routeFacilityKey: "Strait of Malacca",
			rankingScore: 0.78,
		});
		...
		assert.equal(
			pkg.selectedTheaters.length,
			2,
			"both theaters from different geo-groups should be selected",
		);
		const irgcEntities = pkg.entities.filter((e) => e.name === "IRGC Naval Forces");
		assert.ok(
			irgcEntities.length >= 2,
			`Expected 2 IRGC entities (one per candidate), got ${irgcEntities.length}`,
		);
	});
	...
	it('theater.label is never "undefined" when candidateStateLabel is missing', () => {
		...
		assert.ok(
			...
			`simulationRequirement must not contain "undefined": ${text}`,
		);
	});
...
describe("buildSimulationRequirementText — stateKind branching", () => {
	...
	it('T-R1: maritime_disruption contains "shipping behavior" and "importer response" (regression)', () => {
		const text = buildSimulationRequirementText(
			makeTheater({ stateKind: "maritime_disruption" }),
			makeMinCand(),
		);
	...
	it('T-R2: market_repricing does NOT contain "shipping behavior" — contains "credit conditions" and "FX stress"', () => {
		...
		assert.ok(
			!text.includes("shipping behavior"),
			`"shipping behavior" should not appear, got: ${text}`,
		);
	...
	it('T-R4: security_escalation contains "military posture" and "logistics disruption"', () => {
		...
		assert.ok(
			text.includes("logistics disruption"),
			`expected "logistics disruption", got: ${text}`,
		);
	});
	...
	it('T-R5: infrastructure_fragility contains "supply chain capacity"', () => {
		...
		assert.ok(
			text.includes("supply chain capacity"),
			`expected "supply chain capacity", got: ${text}`,
		);
	});
	...
	it('T-R6: cyber_pressure contains "systems availability" and "financial network continuity"', () => {
		...
		assert.ok(
			text.includes("systems availability"),
			`expected "systems availability", got: ${text}`,
		);
		assert.ok(
			text.includes("financial network continuity"),
			`expected "financial network continuity", got: ${text}`,
		);
	});
...
describe("buildSimulationPackageConstraints — non-maritime constraint classes", () => {
	...
	it("T-C1: rates_inflation bucket + no routeFacilityKey generates macro_financial_posture constraint", () => {
		...
		assert.ok(
			c,
			`expected macro_financial_posture constraint, got: ${JSON.stringify(constraints.map((x) => x.class))}`,
		);
	...
	it("T-C2: political instability theater (no routeFacilityKey, no commodityKey, stateKind != market_repricing) generates structural_event_premise constraint with hard:true", () => {
		...
		assert.ok(
			c,
			`expected structural_event_premise, got: ${JSON.stringify(constraints.map((x) => x.class))}`,
		);
	...
	it("T-C3: maritime theater regression — generates route_chokepoint_status + commodity_exposure, NOT structural_event_premise", () => {
		...
		assert.ok(
			classes.includes("route_chokepoint_status"),
			`expected route_chokepoint_status, got: ${classes}`,
		);
		assert.ok(
			classes.includes("commodity_exposure"),
			`expected commodity_exposure, got: ${classes}`,
		);
		assert.ok(
			!classes.includes("structural_event_premise"),
			`unexpected structural_event_premise, got: ${classes}`,
		);
	});
	...
	it("T-C4: maritime theater does NOT generate macro_financial_posture (has routeFacilityKey)", () => {
		...
		const cand = makeCandObj({
			marketBucketIds: ["energy"],
			marketContext: { topBucketId: "energy", criticalSignalLift: 0.2, contradictionScore: 0 },
		});
		const result = buildSimulationPackageConstraints([theater], [cand]);
		const classes = (result["th-test"] || []).map((x) => x.class);
		assert.ok(
			!classes.includes("macro_financial_posture"),
			`unexpected macro_financial_posture, got: ${classes}`,
		);
	});
	...
	it("T-C5: political_instability + sovereign_risk bucket (in MACRO_FIN_BUCKETS) generates both macro_financial_posture and structural_event_premise", () => {
		...
		assert.ok(
			classes.includes("macro_financial_posture"),
			`expected macro_financial_posture, got: ${classes}`,
		);
		assert.ok(
			classes.includes("structural_event_premise"),
			`expected structural_event_premise, got: ${classes}`,
		);
...
const minimalPkg = {
	...
	entities: [
		{
			entityId: "houthi-forces",
			name: "Houthi Forces",
			class: "military_or_security_actor",
			region: "Yemen",
			stance: "active",
			objectives: [],
			constraints: [],
			relevanceToTheater: "test-theater-1",
		},
		{
			entityId: "aramco-exports",
			name: "Saudi Aramco",
			class: "exporter_or_importer",
			region: "Saudi Arabia",
			stance: "stressed",
			objectives: [],
			constraints: [],
	...
	eventSeeds: [
		{
			...
			theaterId: "test-theater-1",
			type: "live_news",
			summary: "Houthi missile attack on Red Sea shipping",
			evidenceRefs: ["E1"],
			timing: "T+0h",
		},
		{
			...
			theaterId: "test-theater-1",
			type: "state_signal",
			summary: "Oil tanker rerouting Cape of Good Hope",
			evidenceRefs: ["E2"],
			timing: "T+12h",
		},
	],
		...
		"test-theater-1": [
			{
				...
				statement: "Red Sea is under elevated risk per current world signals.",
				hard: false,
				source: "test",
			},
			{
				...
				statement: "crude oil is the primary exposed commodity.",
				hard: true,
				source: "test",
			},
		],
	...
	evaluationTargets: {
		"test-theater-1": {
			...
			requiredPaths: [
				{
					pathType: "escalation",
					question: "How does disruption at Red Sea escalate into a broader energy shock?",
				},
				{
					pathType: "containment",
					question: "What conditions contain the Red Sea disruption before energy repricing?",
				},
				{
					pathType: "market_cascade",
					question:
						"What are the 2nd and 3rd order economic consequences? Model $/bbl direction and freight rate delta.",
				},
			],
			requiredOutputs: ["key_invalidators", "timing_markers", "actor_response_summary"],
			timingMarkers: [
				{ label: "T+24h", description: "Initial response" },
				{ label: "T+48h", description: "Repricing signals" },
				{ label: "T+72h", description: "Bifurcation point" },
			],
	...
	simulationRequirement: {
		"test-theater-1":
			"Simulate how a Red Sea disruption propagates through energy and logistics markets",
	},
};
...
describe("simulation runner — prompt builders", () => {
	...
	it("Round 1 prompt: market_cascade path includes 2nd/3rd order economic framing", () => {
		...
		assert.ok(
			prompt.includes("$/bbl") || prompt.includes("freight rate"),
			"should include economic cascade language ($/bbl or freight rate)",
		);
	...
	it("Round 1 prompt renders evaluationTargets questions from requiredPaths (not fallback)", () => {
		...
		assert.ok(
			prompt.includes("market_cascade:"),
			"evalTargets market_cascade question must appear",
		);
		assert.ok(
			!prompt.includes("General market and security dynamics"),
			"fallback text must not appear when evalTargets are present",
		);
	});
	...
	it("Round 1 prompt renders constraints with hard/soft labels (not fallback)", () => {
		...
		assert.ok(
			!prompt.includes("No explicit constraints"),
			"fallback text must not appear when constraints are present",
		);
	});

	it("Round 2 prompt renders evaluationTargets questions from requiredPaths (not fallback)", () => {
		const round1 = {
			paths: [{ pathId: "escalation", summary: "Escalation summary", initialReactions: [] }],
		};
		const prompt = buildSimulationRound2SystemPrompt(minimalTheater, minimalPkg, round1);
		assert.ok(
			prompt.includes("escalation:"),
			"evalTargets escalation question must appear in round 2",
		);
		assert.ok(
			!prompt.includes("General market and security dynamics"),
			"fallback text must not appear in round 2",
		);
	});
	...
	it("Round 2 prompt contains Round 1 path summaries", () => {
		const round1 = {
			paths: [
				{
					pathId: "escalation",
					summary: "Escalation path summary",
					initialReactions: [{ actorId: "houthi-forces" }],
				},
		...
		const prompt = buildSimulationRound2SystemPrompt(minimalTheater, minimalPkg, round1);
		assert.ok(
			prompt.includes("Escalation path summary"),
			"should include round 1 escalation summary",
		);
		assert.ok(
			prompt.includes("Containment path summary"),
			"should include round 1 containment summary",
		);
	...
	it("T-P1: Round 2 prompt includes CANDIDATE ACTOR ROLES section when theater has actorRoles", () => {
		...
		const round1 = {
			paths: [{ pathId: "escalation", summary: "Oil supply shock", initialReactions: [] }],
		};
		const prompt = buildSimulationRound2SystemPrompt(theaterWithRoles, minimalPkg, round1);
		assert.ok(
			prompt.includes("copy these EXACT strings into keyActorRoles"),
			"prompt should include role section header with copy instruction",
		);
		...
		assert.ok(
			prompt.includes("return [] if none apply"),
			"prompt should include fallback instruction",
		);
	});
	...
	it("T-P2: Round 2 prompt omits CANDIDATE ACTOR ROLES list section when theater has no actorRoles", () => {
		...
		assert.ok(
			!prompt.includes("copy these EXACT strings into keyActorRoles"),
			"role list section should NOT be injected when actorRoles absent",
		);
	});
});
...
describe("simulation runner — extractSimulationRoundPayload", () => {
	const r1Payload = JSON.stringify({
		paths: [
			{ pathId: "escalation", label: "Escalate", summary: "Forces escalate", initialReactions: [] },
			{
				pathId: "containment",
				label: "Contain",
				summary: "Forces contained",
				initialReactions: [],
			},
			{
				pathId: "market_cascade",
				label: "Cascade",
				summary: "Oil +$18/bbl, freight +22%, Asian importers face FX stress",
				initialReactions: [],
			},
		],
		...
		paths: [
			{
				...
				summary: "Escalated 72h",
				keyActors: ["houthi-forces"],
				roundByRoundEvolution: [
					{ round: 1, summary: "Round 1" },
					{ round: 2, summary: "Round 2" },
				],
				confidence: 0.75,
				timingMarkers: [{ event: "First strike", timing: "T+6h" }],
			},
			{
				pathId: "containment",
				label: "Contained",
				summary: "Contained 72h",
				keyActors: [],
				roundByRoundEvolution: [],
				confidence: 0.6,
				timingMarkers: [],
			...
			{
				pathId: "market_cascade",
				label: "Economic Cascade",
				summary: "Energy repricing 72h",
				keyActors: [],
				roundByRoundEvolution: [],
				confidence: 0.4,
				timingMarkers: [],
	...
	it("rejects spillover pathId (replaced by market_cascade)", () => {
		...
		assert.equal(
			result.paths.length,
			2,
			"spillover path should be filtered out, only 2 valid paths remain",
		);
		assert.ok(
			!result.paths.some((p) => p.pathId === "spillover"),
			"spillover must not appear in parsed paths",
		);
	});
			...
			paths: [
				{
					pathId: "escalation",
					label: "Escalation",
					summary: "Oil disruption",
					keyActors: ["Iran"],
					keyActorRoles: ["Commodity traders", "Shipping operators"],
					roundByRoundEvolution: [],
					confidence: 0.6,
					timingMarkers: [],
				},
...
describe("simulation runner — writeSimulationOutcome", () => {
	...
	it("returns null when pkg has no runId", async () => {
		...
		const result = await writeSimulationOutcome({ generatedAt: Date.now() }, outcome, {
			storageConfig: null,
		});
		assert.equal(result, null);
	});
...
describe("phase 3 simulation re-ingestion — computeSimulationAdjustment", () => {
	const makePath = (targetBucket, channel, affectedAssets = []) => ({
		...
		candidate: {
			routeFacilityKey: "Strait of Hormuz",
			commodityKey: "crude_oil",
			topBucketId: targetBucket,
			topChannel: channel,
		},
	});

	const makeCandidatePacket = (
		routeFacilityKey = "Strait of Hormuz",
		commodityKey = "crude_oil",
	) => ({
		candidateStateId: "state-1",
	...
	it("T2: bucket+channel match + 2 actor overlap gives +0.12", () => {
		...
		const simResult = {
			...
			topPaths: [
				{
					label: "Oil energy supply shock via Hormuz",
					summary: "Crude supply disruption",
					keyActors: ["Iran", "Houthi", "US Navy"],
					keyActorRoles: ["Iran", "Houthi movement", "US Navy"],
				},
			],
			invalidators: [],
			stabilizers: [],
	...
	it("T5: bucket+channel match (+0.08) plus invalidator (-0.12) gives net -0.04", () => {
		...
		const simResult = {
			...
			topPaths: [
				{
					label: "Oil supply shock escalation",
					summary: "Crude oil supply disruption energy",
					keyActors: [],
				},
			],
	...
	it("T6b: bucket+channel nested under marketContext (production packet shape) gives +0.08", () => {
		...
		const path = {
			...
			pathScore: 0.6,
			acceptanceScore: 0.55,
			candidate: { routeFacilityKey: "Strait of Hormuz", commodityKey: "crude_oil" },
		};
		const simResult = {
			theaterId: "state-1",
			topPaths: [
		...
		const { adjustment, details } = computeSimulationAdjustment(
			path,
			simResult,
			packetWithMarketContext,
		);
		assert.equal(adjustment, 0.08);
	...
	const riskOffSimResult = {
		...
		topPaths: [
			{
				label: "Sustained Conflict",
				summary: "risk aversion and capital flight amid oil supply concerns",
				keyActors: [],
			},
		],
		invalidators: [],
			...
			topPaths: [
				{
					...
					summary: "energy supply disruption from Red Sea",
					keyActors: ["Iran", "Saudi_Arabia", "US"],
					keyActorRoles: ["Iran", "Saudi Arabia"],
				},
			],
			invalidators: [],
			stabilizers: [],
	...
	it("T-G: entity ID format keyActors resolve to names via normalization (keyActorsOverlapCount telemetry)", () => {
		...
		const simResult = {
			...
			topPaths: [
				{
					label: "Oil supply shock",
					summary: "energy supply disruption",
					keyActors: ["state-aa3f41bf4f:iran", "state-aa3f41bf4f:saudi_arabia"],
					keyActorRoles: ["Iran", "Saudi Arabia"],
				},
			],
			invalidators: [],
	...
	it("T-K: two paths for same candidate get identical actor result even when affectedAssets differ (strict precedence)", () => {
		...
		const path2 = makePath("energy", "energy_supply_shock", [
			"TTF gas futures",
			"European utility stocks",
		]); // different affectedAssets — irrelevant
		const simResult = {
			topPaths: [
				{
					label: "Supply shock",
					summary: "energy supply disruption",
					keyActors: ["Iran", "Saudi_Arabia"],
					keyActorRoles: ["Iran", "Saudi Arabia"],
				},
			],
			invalidators: [],
		...
		const { adjustment: adj1, details: d1 } = computeSimulationAdjustment(
			path1,
			simResult,
			candidatePacket,
		);
		const { adjustment: adj2, details: d2 } = computeSimulationAdjustment(
			path2,
			simResult,
			candidatePacket,
		);
			...
			topPaths: [
				{
					...
					summary: "energy supply disruption",
					confidence: 0.72,
					keyActors: ["Iran", "US_Navy"],
					keyActorRoles: ["Iran", "US Navy"],
				},
			],
			invalidators: [],
			stabilizers: [],
	...
	it("T-N4: missing confidence (undefined) falls back to 1.0 → full +0.08", () => {
		...
		const simResult = {
			topPaths: [
				{ label: "Oil supply disruption", summary: "energy supply disruption", keyActors: [] },
			], // no confidence field
			invalidators: [],
			stabilizers: [],
		};
	...
	it("T-RO2: absent keyActorRoles with stateSummary source gives roleOverlapCount=0 (old sim output gracefully degrades)", () => {
		...
		const simResult = {
			topPaths: [
				{ label: "Oil supply shock", summary: "energy supply disruption", keyActors: ["Iran"] },
			], // no keyActorRoles
			invalidators: [],
			stabilizers: [],
		};
...
describe("phase 3 simulation re-ingestion — applySimulationMerge", () => {
	...
	it("T8: null simulation outcome returns unchanged evaluation and null simulationEvidence", () => {
		const evaluation = makeEval("completed", [makeExpandedPath("state-1", 0.6)]);
	...
	it("T9: demotion — accepted path drops below 0.50 when invalidator hits", () => {
		...
		const simOutcome = makeSimOutcome(
			"state-1",
			[],
			["Red Sea reopened after diplomatic ceasefire"],
		);
		...
		const { simulationEvidence } = applySimulationMerge(
			evaluation,
			simOutcome,
			candidatePackets,
			{ generatedAt: Date.now(), impactExpansionCandidates: candidatePackets },
			null,
		);
	...
	it("T10: promotion — rejected path rises above 0.50 when match hits", () => {
		...
		const simOutcome = makeSimOutcome("state-1", [
			{
				label: "Oil energy supply shock escalation",
				summary: "Crude supply disruption energy",
				keyActors: ["Iran", "Houthi"],
			},
		]);
		const candidatePackets = [
	...
	it("T11: no adjustment when sim theater not found for path candidateStateId", () => {
		const path = makeExpandedPath("state-999", 0.6);
		const evaluation = makeEval("completed", [path]);
		const simOutcome = makeSimOutcome("state-1", [
			{ label: "energy shock", summary: "energy supply disruption", keyActors: [] },
		]);
		const candidatePackets = [
			{
				candidateStateId: "state-999",
				routeFacilityKey: "",
				commodityKey: "",
				marketContext: { topBucketId: "energy", topChannel: "energy_supply_shock" },
			},
	...
	it("T12: simulationEvidence contains outcomeRunId, theaterCount, adjustments", () => {
		const path = makeExpandedPath("state-1", 0.6);
		const evaluation = makeEval("completed", [path]);
		const simOutcome = makeSimOutcome("state-1", [
			{ label: "Oil energy supply shock", summary: "Crude energy disruption", keyActors: [] },
		]);
		const candidatePackets = [
			{
				candidateStateId: "state-1",
	...
	it("T13: candidateStateId-keyed lookup works when theaterId is a positional ID (production scenario)", () => {
		...
		assert.equal(
			simulationEvidence.pathsDemoted,
			1,
			"path should be demoted via candidateStateId lookup",
		);
	...
	it("T-J: applySimulationMerge promotes 0.39 path to 0.51 via stateSummary.actors alone (empty affectedAssets)", () => {
		...
		const { evaluation: result } = applySimulationMerge(
			evaluation,
			simOutcome,
		...
		const promoted = (result.selectedPaths || []).find(
			(p) => p.candidateStateId === stateA && p.type === "expanded",
		);
		assert.ok(promoted, "path should be promoted to selected");
		assert.ok(
			promoted.mergedAcceptanceScore >= 0.5,
			`score should be >= 0.50, got ${promoted.mergedAcceptanceScore}`,
		);
		assert.ok(
			promoted.simulationAdjustment >= 0.12,
			"should have +0.12 (bucket+channel + actor overlap)",
		);
	});
	...
	it("T-O2: negative adjustment that does not cross threshold → simulationSignal.backed=false, demoted=false", () => {
		...
		const path = makeExpandedPath(stateId, 0.7); // accepted; 0.70 - 0.12 = 0.58 ≥ 0.50 → NOT demoted
		const candidatePacket = {
			candidateStateId: stateId,
			routeFacilityKey: "Red Sea",
			commodityKey: "crude_oil",
	...
	it("T-O3: zero adjustment (no matching theater) → simulationSignal is undefined", () => {
		...
		const path = makeExpandedPath(stateId, 0.6);
		const candidatePacket = {
			candidateStateId: stateId,
			routeFacilityKey: "Red Sea",
			commodityKey: "crude_oil",
		...
		const simOutcome = {
			...
			theaterResults: [
				{
					theaterId: "other-state",
					candidateStateId: "other-state",
					topPaths: [],
					invalidators: [],
					stabilizers: [],
				},
	...
	it("T-O5: summarizeImpactPathScore includes simulationSignal when present (path-scorecards / impact-expansion-debug coverage)", () => {
		const signal = {
			backed: true,
			adjustmentDelta: 0.08,
			channelSource: "market",
			demoted: false,
			simPathConfidence: 0.9,
		};
		const path = {
			pathId: "p-o5",
			type: "expanded",
		...
		assert.deepStrictEqual(
			summary.simulationSignal,
			signal,
			"simulationSignal must be forwarded into scorecard summary",
		);
	});
	...
	it("T-O7: applySimulationMerge clears stale sim fields on zero-adjustment paths from prior cycles", () => {
		...
		assert.equal(
			stalePath.simulationAdjustment,
			undefined,
			"stale simulationAdjustment must be cleared",
		);
		assert.equal(stalePath.simulationSignal, undefined, "stale simulationSignal must be cleared");
		assert.equal(
			stalePath.mergedAcceptanceScore,
			undefined,
			"stale mergedAcceptanceScore must be cleared",
		);
		assert.equal(
			stalePath.demotedBySimulation,
			undefined,
			"stale demotedBySimulation must be cleared",
		);
		assert.equal(
			stalePath.promotedBySimulation,
			undefined,
			"stale promotedBySimulation must be cleared",
		);
	});
	...
	it("T-O8: applySimulationMerge clears stale sim fields when theater has no matching result in fresh simulation", () => {
		...
		const simOutcome = {
			...
			theaterResults: [
				{
					theaterId: "theater-1",
					candidateStateId: "state-different",
					topPaths: [
						{ label: "Other path", summary: "different theater", confidence: 0.8, keyActors: [] },
					],
					invalidators: [],
					stabilizers: [],
				},
		...
		assert.equal(
			stalePath.simulationAdjustment,
			undefined,
			"stale simulationAdjustment must be cleared when no theater matches",
		);
		assert.equal(
			stalePath.simulationSignal,
			undefined,
			"stale simulationSignal must be cleared when no theater matches",
		);
		assert.equal(
			stalePath.mergedAcceptanceScore,
			undefined,
			"stale mergedAcceptanceScore must be cleared when no theater matches",
		);
	});
	...
	it("T-SC-1: simDetail written when simulationAdjustment and simulationAdjustmentDetail are set", () => {
		...
		assert.strictEqual(
			result.simDetail.simPathConfidence,
			undefined,
			"simPathConfidence must not be duplicated in simDetail",
		);
	});
	...
	it("T-SC-4: computeSimulationAdjustment details flow through to summarizeImpactPathScore simDetail", () => {
		...
		assert.ok(
			scorecard.simDetail.actorOverlapCount >= 2,
			`actorOverlapCount should be >=2, got ${scorecard.simDetail.actorOverlapCount}`,
		);
...
describe("phase 3 simulation re-ingestion — matching helpers", () => {
	it("matchesBucket matches energy path to energy bucket via label", () => {
		assert.ok(
			matchesBucket(
				{ label: "Oil price escalation via crude supply shock", summary: "" },
				"energy",
			),
		);
	});
	...
	it("matchesBucket matches freight path to freight bucket via summary", () => {
		assert.ok(
			matchesBucket(
				{ label: "", summary: "Shipping cost increase from route disruption" },
				"freight",
			),
		);
	});
	...
	it("matchesBucket returns false for unrelated text", () => {
		assert.ok(
			!matchesBucket(
				{ label: "Diplomatic meeting held", summary: "Political talks resume" },
				"energy",
			),
		);
	});
	...
	it("matchesChannel matches energy_supply_shock via label", () => {
		assert.ok(
			matchesChannel(
				{ label: "Crude supply disruption energy", summary: "" },
				"energy_supply_shock",
			),
		);
	});
	...
	it("matchesChannel energy_supply_shock matches oil infrastructure scenario (bridge keyword)", () => {
		assert.ok(
			matchesChannel(
				{ label: "Oil infrastructure damage leads to supply disruption", summary: "" },
				"energy_supply_shock",
			),
		);
		assert.ok(
			matchesChannel(
				{ label: "Crude price spike from Persian Gulf closure", summary: "" },
				"energy_supply_shock",
			),
		);
	});
	...
	it("matchesChannel shipping_cost_shock matches maritime rerouting scenario (bridge keyword)", () => {
		assert.ok(
			matchesChannel(
				{ label: "Tanker rerouting via Cape of Good Hope raises freight costs", summary: "" },
				"shipping_cost_shock",
			),
		);
		assert.ok(
			matchesChannel(
				{ label: "Vessel traffic diverted from Suez Canal shipping lane", summary: "" },
				"shipping_cost_shock",
			),
		);
	});
	...
	it("matchesChannel risk_off_rotation matches geopolitical sovereign risk scenario (bridge keyword)", () => {
		assert.ok(
			matchesChannel(
				{
					label: "Regional Conflict & Sovereign Risk Spiral",
					summary: "rapid repricing of sovereign risk",
				},
				"risk_off_rotation",
			),
		);
		assert.ok(
			matchesChannel({ label: "Global Economic Shockwave", summary: "" }, "risk_off_rotation"),
		);
		assert.ok(
			matchesChannel(
				{ label: "Market contagion from India FX crisis", summary: "" },
				"risk_off_rotation",
			),
		);
	});
	...
	it("matchesChannel returns false for unrelated text", () => {
		assert.ok(
			!matchesChannel({ label: "Political talks", summary: "Ceasefire" }, "shipping_cost_shock"),
		);
	});
	...
	it("negatesDisruption detects commodity restoration", () => {
		...
		assert.ok(
			!negatesDisruption("crude_oil supply chain restored to normal operations", candidatePacket),
		);
	});
	...
	it("negatesDisruption — non-maritime: rates_inflation bucket + negation term matches", () => {
		const candidatePacket = {
			...
			commodityKey: "",
			stateKind: "market_repricing",
			marketContext: { topBucketId: "rates_inflation", topChannel: "policy_rate_pressure" },
		};
		assert.ok(
			negatesDisruption(
				"inflation pressures stabilized as Fed signals rate normalization",
				candidatePacket,
			),
		);
	});
	...
	it("negatesDisruption — non-maritime: unrelated stateKind text does not match", () => {
		const candidatePacket = {
			routeFacilityKey: "",
			commodityKey: "",
			stateKind: "cyber_pressure",
			marketContext: { topBucketId: "rates_inflation", topChannel: "" },
		};
	...
	it("buildSimulationPackageEvaluationTargets — market_repricing does NOT contain maritime framing", () => {
		...
		assert.ok(
			!allText.includes("freight rate delta"),
			`"freight rate delta" must not appear for non-maritime theater, got: ${allText}`,
		);
		assert.ok(
			!allText.includes("$/bbl"),
			`"$/bbl" must not appear for non-maritime theater, got: ${allText}`,
		);
		assert.ok(
			allText.includes("inflation") || allText.includes("rates"),
			`expected bucket-related text, got: ${allText}`,
		);
	});
});
	...
	it("buildImpactExpansionDebugPayload includes simulationEvidence when provided in data", () => {
		const simEvidence = {
			...
			adjustments: [
				{
					pathId: "p1",
					originalAcceptanceScore: 0.55,
					simulationAdjustment: 0.08,
					mergedAcceptanceScore: 0.63,
					wasAccepted: true,
					nowAccepted: true,
				},
			],
		...
		const data = {
			...
			deepPathEvaluation: {
				selectedPaths: [],
				rejectedPaths: [],
				validation: { mapped: [], hypotheses: [], validated: [], rejectionReasonCounts: {} },
			},
			impactExpansionCandidates: [],
...
describe("phase 3 simulation re-ingestion — applyPostSimulationRescore", () => {
	...
	const makeRescorePath = (stateId, acceptanceScore, channel = "supply_disruption") => ({
		...
		direct: {
			...
			targetBucket: "",
			channel,
			affectedAssets: ["Red Sea shipping"],
		},
		second: null,
		third: null,
				...
				topPaths: [
					{
						...
						summary: "oil supply shock from Red Sea disruption",
						keyActors: ["Red Sea shipping"],
					},
				],
				invalidators: [],
				stabilizers: [],
	...
	it("R-1: skips when storageConfig is null", async () => {
		const result = await applyPostSimulationRescore(
			"1774800000000-test01",
			makeRescoreSimOutcome("state-x"),
			null,
		);
	...
	it("R-4: applySimulationMerge promotes near-threshold path to selected when bucketChannelMatch fires", () => {
			...
			theaterResults: [
				{
					...
					topPaths: [
						{
							label: "Oil Supply Shock",
							summary: "oil supply disruption from Red Sea energy shock",
							keyActors: [],
						},
					],
					invalidators: [],
		...
		assert.ok(
			simulationEvidence.adjustments[0].details.channelSource === "market",
			"channel resolved via marketContext fallback",
		);
	...
	it("R-6: applySimulationMerge persists partial demotion in completed run without status change (CASE 3)", () => {
		...
		assert.equal(
			updatedEval.status,
			"completed",
			"status stays completed — stateB is still selected",
		);
	...
	it("R-7: applySimulationMerge promotes near-threshold rejected path in completed run (CASE 3 promotion)", () => {
		...
		assert.equal(
			updatedEval.status,
			"completed",
			"status stays completed — existing selected path remains",
		);
		const selectedIds = updatedEval.selectedPaths.map((p) => p.candidateStateId);
	...
	it("R-8: applySimulationMerge promotes path at 0.39 via +0.12 (bucketChannelMatch + actor overlap)", () => {
		...
		const pathA = {
			...
			direct: {
				...
				targetBucket: "",
				channel: "supply_disruption",
				affectedAssets: ["Red Sea Oil Tankers", "Saudi Aramco"],
			},
		};
		const pathB = { ...makeRescorePath(stateB, 0.75), candidate: candidateB };
		const evaluation = {
					...
					topPaths: [
						{
							label: "Oil Supply Shock",
							summary: "oil supply disruption from Red Sea energy shock",
							keyActors: ["Red Sea Oil Tankers", "Saudi Aramco"],
						},
					],
					invalidators: [],
					stabilizers: [],
		...
		assert.ok(
			updatedEval.selectedPaths.some((p) => p.candidateStateId === stateA),
			"promoted path in selectedPaths",
		);
	});
	...
	it("R-9: applySimulationMerge demotes path at 0.64 via -0.15 stabilizer (boundary above old 0.62 guard)", () => {
		...
		assert.ok(
			!updatedEval.selectedPaths.some((p) => p.candidateStateId === stateA),
			"demoted path removed",
		);
		assert.ok(
			updatedEval.selectedPaths.some((p) => p.candidateStateId === stateB),
			"safe path remains",
		);
	});
	...
	it("R-5: simulation adjustments use marketContext fallback when direct.channel is an LLM-generated invalid key", () => {
		...
		const simResult = {
			topPaths: [
				{
					label: "Energy Crisis",
					summary: "oil supply disruption drives energy price spike",
					keyActors: [],
				},
			],
			invalidators: [],
		...
		const { adjustment, details } = computeSimulationAdjustment(path, simResult, candidatePacket);
		assert.equal(
			details.channelSource,
			"market",
			"supply_disruption is invalid — falls back to marketContext",
		);
...
describe("writeSimulationDecorations and applySimulationDecorationsToForecasts", () => {
	...
	it("WD-2b: writeSimulationDecorations skips when simulationEvidence.adjustments is not an array", async () => {
		const store = {};
		__setRedisStoreForTests(store);
		await writeSimulationDecorations(
			{ simulationEvidence: { adjustments: null } },
			makeSnapshot([]),
		);
		assert.deepEqual(
			Object.keys(store),
			[],
			"null adjustments field — do not clear old decorations",
		);
	});
	...
	it("WD-3: writeSimulationDecorations writes empty decoration when adjustments present but no stateUnit has matching forecastIds", async () => {
		...
		await writeSimulationDecorations(
			makeMergeResult([adj]),
			makeSnapshot(stateUnits, "run-nomatch"),
		);
		const written = store[SIMULATION_DECORATIONS_KEY];
	...
	it("WD-5: writeSimulationDecorations picks strongest adjustment when multiple paths share candidateStateId", async () => {
		...
		const adjs = [
			makeAdj("state-b", 0.08, 0.6),
			makeAdj("state-b", 0.12, 0.88), // strongest — should win
			makeAdj("state-b", -0.04, 0.5),
		];
	...
	it("WD-6: writeSimulationDecorations resolves conflict when two candidates map to the same forecastId — highest absolute value wins", async () => {
		...
		const adjs = [
			makeAdj("state-c1", 0.08, 0.7),
			makeAdj("state-c2", -0.15, 0.9), // higher absolute value
		];
		await writeSimulationDecorations(makeMergeResult(adjs), makeSnapshot(stateUnits));
		...
		const store = {
			[SIMULATION_DECORATIONS_KEY]: {
				...
				byForecastId: {
					"fc-aa": {
						simulationAdjustment: 0.12,
						simPathConfidence: 0.85,
						demotedBySimulation: false,
					},
					"fc-bb": {
						simulationAdjustment: -0.15,
						simPathConfidence: 0.95,
	...
	it("WD-10: applySimulationDecorationsToForecasts is non-fatal when Redis credentials are missing", async () => {
		__setRedisStoreForTests(null);
		const predictions = [
			{ id: "fc-xx", simulationAdjustment: 0, simPathConfidence: 0, demotedBySimulation: false },
		];
	...
	it("WD-11: applySimulationDecorationsToForecasts is non-fatal when decoration value has wrong shape", async () => {
		const store = {
			[SIMULATION_DECORATIONS_KEY]: {
				runId: "run-bad",
				generatedAt: Date.now(),
				byForecastId: "not-an-object",
			},
		};
		__setRedisStoreForTests(store);
		const predictions = [
			{ id: "fc-yy", simulationAdjustment: 0, simPathConfidence: 0, demotedBySimulation: false },
		];
		await assert.doesNotReject(() => applySimulationDecorationsToForecasts(predictions));
		assert.equal(
			predictions[0].simulationAdjustment,
			0,
			"prediction unchanged when byForecastId is not an object",
		);
	});
	...
	it("WD-13: applySimulationDecorationsToForecasts skips decorations older than SIMULATION_DECORATIONS_MAX_AGE_MS", async () => {
		...
		__setRedisStoreForTests(store);
		const predictions = [
			{ id: "fc-stale", simulationAdjustment: 0, simPathConfidence: 0, demotedBySimulation: false },
		];
		await applySimulationDecorationsToForecasts(predictions);
	...
	it("WD-14: empty-adjustments write clears stale decorations — subsequent apply sees no flags", async () => {
		...
		assert.deepEqual(
			store[SIMULATION_DECORATIONS_KEY].byForecastId,
			{},
			"empty map written, stale entry gone",
		);
		...
		assert.equal(
			predictions[0].simulationAdjustment,
			0,
			"stale flag cleared — sub-bar correctly shows nothing",
		);
	});
	...
	it("WD-16: writeSimulationDecorations patches forecast:predictions:v2 in-place so same-run consumers see sim fields", async () => {
		...
		const stateUnits = [makeStateUnit("state-canon-a", ["fc-canon-01", "fc-canon-02"])];
		...
		assert.equal(
			canon[0].simulationAdjustment,
			0.12,
			"fc-canon-01 sim field updated in canonical key",
		);
		assert.equal(canon[0].simPathConfidence, 0.88);
		assert.equal(
			canon[1].simulationAdjustment,
			0.12,
			"fc-canon-02 sim field updated in canonical key",
		);
		...
		assert.ok(
			store[CANONICAL_KEY].generatedAt < Date.now() - 4000,
			"original generatedAt preserved",
		);
	});
	...
	it("WD-18: patchPublishedForecastsWithSimDecorations skips when canonical key is from a newer run", async () => {
		...
		const olderSnapshot = {
			runId: "run-older",
			generatedAt: olderRunGeneratedAt,
			fullRunStateUnits: stateUnits,
		};
		...
		assert.equal(
			canon[0].simulationAdjustment,
			0.08,
			"run B sim adjustment preserved — run A must not overwrite",
		);
		assert.equal(canon[0].simPathConfidence, 0.9, "run B sim confidence preserved");
		assert.equal(
			store[CANONICAL_KEY].generatedAt,
			newerRunGeneratedAt,
			"canonical generatedAt unchanged",
		);
	});
	...
	it("WD-20: redisAtomicPatchSimDecorations — interleaving race: newer fast-path payload wins even when it arrives after the check", async () => {
		...
		const store = {
			[CANONICAL_KEY]: {
				...
				predictions: [
					{
						id: "fc-race-01",
						simulationAdjustment: 0.05,
						simPathConfidence: 0.75,
						demotedBySimulation: false,
						title: "Race 01",
					},
				],
		...
		assert.equal(
			store[CANONICAL_KEY].predictions[0].simulationAdjustment,
			0.05,
			"newer payload untouched by older run",
		);
		assert.equal(
			store[CANONICAL_KEY].predictions[0].simPathConfidence,
			0.75,
			"newer confidence untouched",
		);
	...
	it("WD-21: writeSimulationDecorations skips side key and canonical patch when existing side key is from a newer run", async () => {
		...
		assert.equal(
			store[SIMULATION_DECORATIONS_KEY].generatedAt,
			runBTs,
			"side key generatedAt preserved (not poisoned with Date.now())",
		);
		assert.deepStrictEqual(
	...
	it("WD-22: writeSimulationDecorations stores snapshot.generatedAt (not Date.now()) so age check uses run origin time", async () => {
		...
		assert.strictEqual(
			storedTs,
			runOriginTs,
			"stored generatedAt equals snapshot.generatedAt, not Date.now()",
		);
	});
	...
	it("WD-23: redisAtomicWriteSimDecorations skips when existing has newer generatedAt", async () => {
		...
		const status = await redisAtomicWriteSimDecorations(
			"http://test",
			"test",
			SIMULATION_DECORATIONS_KEY,
			{
				...
				generatedAt: olderTs,
				byForecastId: {
					"fc-x": { simulationAdjustment: 0.12, simPathConfidence: 1, demotedBySimulation: false },
				},
			},
			259200,
		);
	...
	it("WD-15: inline deep path — applySimulationMerge result + snapshot.fullRunStateUnits writes decorations (covers processDeepForecastTask call site)", async () => {
			...
			fullRunStateUnits: [
				{
					...
					forecastIds: ["fc-inline-001", "fc-inline-002"],
				},
			],
			impactExpansionCandidates: [candidatePacket],
		};
			...
			theaterResults: [
				{
					...
					topPaths: [
						{
							label: "Hormuz energy shock",
							summary: "oil supply disruption from energy supply shock",
							keyActors: [],
						},
					],
					invalidators: [],
		...
		assert.ok(
			"fc-inline-001" in written.byForecastId,
			"fc-inline-001 decorated via fullRunStateUnits",
		);
		assert.ok(
			"fc-inline-002" in written.byForecastId,
			"fc-inline-002 decorated via fullRunStateUnits",
		);
