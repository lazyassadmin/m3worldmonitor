...
describe("CSP violation filter (shouldSuppressCspViolation)", () => {
		...
		it("suppresses HTTPS connect-src when CSP allows https:", () => {
			assert.ok(
				suppress(
					"enforce",
					"connect-src",
					"https://api.worldmonitor.app/api/oref-alerts",
					"",
					true,
				),
			);
		...
		it("suppresses HTTPS connect-src for tilecache.rainviewer.com", () => {
			assert.ok(
				suppress(
					"enforce",
					"connect-src",
					"https://tilecache.rainviewer.com/v2/radar/abc/256/4/3/4/6/1_1.png",
					"",
					true,
				),
			);
		...
		it("suppresses HTTPS connect-src for Sentry ingest (with port and path)", () => {
			assert.ok(
				suppress(
					"enforce",
					"connect-src",
					"https://o450.ingest.us.sentry.io:443/api/12345/envelope/",
					"",
					true,
				),
			);
		...
		it("suppresses HTTPS connect-src for foxnews HLS", () => {
			assert.ok(
				suppress(
					"enforce",
					"connect-src",
					"https://247preview.foxnews.com/hls/live/stream.m3u8",
					"",
					true,
				),
			);
	...
	describe("scheme-only and special values", () => {
		...
		it("suppresses blob: URI", () => {
			assert.ok(
				suppress("enforce", "worker-src", "blob:https://www.worldmonitor.app/abc", "", false),
			);
		});
		...
		it("suppresses Google Translate", () => {
			assert.ok(
				suppress(
					"enforce",
					"connect-src",
					"https://translate.gstatic.com/_/translate_http",
					"",
					false,
				),
			);
		...
		it("suppresses manifest.webmanifest", () => {
			assert.ok(
				suppress(
					"enforce",
					"default-src",
					"https://www.worldmonitor.app/manifest.webmanifest",
					"",
					false,
				),
			);
	...
	describe("localhost/loopback", () => {
		it("suppresses http://localhost:9009 (Smart TV tuner service)", () => {
			assert.ok(
				suppress("enforce", "connect-src", "http://localhost:9009/service/tvinfo", "", false),
			);
		});
