...
function makeGpiCsv(count = 55) {
	...
	const rows = GPI_ISO3_POOL.slice(0, count).map(
		(iso3, i) => `${iso3},${i + 1},${(1.1 + i * 0.03).toFixed(3)},2025`,
	);
...
describe("resilience static seed CSV parsers", () => {
	describe("parseGpiRows", () => {
		...
		it("falls back to resolvedYear when row.year is absent", () => {
			...
			const rows = GPI_ISO3_POOL.slice(0, 55).map(
				(iso3, i) => `${iso3},${i + 1},${(1.1 + i * 0.03).toFixed(3)}`,
			);
			...
			assert.equal(
				result.get("NO")?.year,
				2024,
				"should use resolvedYear when row.year is missing",
			);
		});
		...
		it("resolveGpiCsv falls back to prior year on 404, without proxying the 404 URL", async () => {
			...
			assert.ok(
				proxiedUrls.every((url) => !url.includes("2026")),
				"2026 URL must never be sent to proxy/retry — only 2025 fallback may use it",
			);
		});
	...
	describe("parseFsinRows", () => {
		it("parses new-schema column names (Phase 3+ #) and outputs peopleInCrisis + phase", () => {
			const csv = csvRows("Country (ISO3),Phase 3+ #,Phase 4 #,Phase 5 #,Period", [
	...
	describe("buildAquastatWbMap", () => {
		...
		it("output indicator keyword matches scoreAquastatValue stress branch", () => {
			...
			assert.ok(
				result.get("DE")?.indicator?.includes("stress"),
				'indicator must include "stress" to route correctly in scoreAquastatValue()',
			);
		});
	});
});
...
describe("resilience static seed payload assembly", () => {
		...
		const payloads = finalizeCountryPayloads(
			{
				...
				infrastructure: new Map([
					[
						"NO",
						{
							...
							indicators: { "EG.ELC.ACCS.ZS": { value: 100, year: 2024 } },
						},
					],
				]),
				gpi: new Map(),
				...
				who: new Map([
					[
						...
						{
							source: "who-gho",
							indicators: { uhcIndex: { indicator: "UHC_INDEX_REPORTED", value: 81, year: 2021 } },
						},
					],
				]),
				fao: new Map(),
				...
				iea: new Map([
					[
						...
						{
							source: "eurostat-nrg_ind_id",
							energyImportDependency: { value: -13.3, year: 2024, source: "eurostat" },
						},
					],
				]),
			},
	...
	it("skips reruns only after a successful snapshot for the same seed year", () => {
		assert.equal(
			shouldSkipSeedYear({ status: "ok", seedYear: 2026, recordCount: 150 }, 2026),
			true,
		);
		assert.equal(
			shouldSkipSeedYear({ status: "error", seedYear: 2026, recordCount: 150 }, 2026),
			false,
		);
		assert.equal(
			shouldSkipSeedYear({ status: "ok", seedYear: 2025, recordCount: 150 }, 2026),
			false,
		);
	});
});
...
describe("recoverFailedDatasets", () => {
	...
	it("injects prior fao values when FSIN fails and a prior snapshot exists", async () => {
		...
		assert.ok(
			typeof maps.fao.get("YE").peopleInCrisis === "number",
			"recovered fao must have peopleInCrisis for scoreFoodWater()",
		);
		assert.ok(
			typeof maps.fao.get("YE").phase === "string",
			"recovered fao must have phase string for scoreFoodWater()",
		);
	});
	...
	it("recovers aquastat with WB shape { value, indicator, year } that scoreAquastatValue() reads", async () => {
		...
		await recoverFailedDatasets(maps, ["aquastat"], {
			...
			readPipeline: async () => [{ result: JSON.stringify({ aquastat: existingAquastat }) }],
		});
		const de = maps.aquastat.get("DE");
		assert.deepEqual(de, existingAquastat, "DE aquastat should be recovered");
		assert.ok(
			typeof de.value === "number",
			"recovered aquastat must have numeric value for scoreAquastatValue()",
		);
		assert.ok(
			typeof de.indicator === "string",
			"recovered aquastat must have indicator string for scoreAquastatValue()",
		);
	});
});
...
describe("resilience static health registrations", () => {
	...
	it("registers annual seed-health monitoring for resilience static", () => {
		assert.match(
			seedHealthSrc,
			/'resilience:static':\s+\{ key: 'seed-meta:resilience:static',\s+intervalMin: 288000 \}/,
		);
	});
});
