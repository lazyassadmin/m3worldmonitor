...
describe("validateBearerToken (with JWKS)", () => {
	...
	function signToken(
		...
		opts?: { expiresIn?: string; key?: CryptoKey },
	) {
	...
	it("reuses the JWKS resolver across calls (not per-request)", async () => {
		...
		const [r1, r2] = await Promise.all([validateBearerToken(token1), validateBearerToken(token2)]);
...
describe("CORS origin matching (convex/http.ts)", () => {
	...
	const TRUSTED = ["https://worldmonitor.app", "*.worldmonitor.app", "http://localhost:3000"];
