...
describe("runtime env guards", () => {
	...
	it("reuses the guarded ENV wrapper for runtime env lookups", () => {
		assert.ok(
			runtimeSrc.includes("const WS_API_URL = ENV.VITE_WS_API_URL || ''"),
			"WS API URL should read from ENV",
		);
		assert.ok(
			runtimeSrc.includes("const FORCE_DESKTOP_RUNTIME = ENV.VITE_DESKTOP_RUNTIME === '1'"),
			"Desktop runtime flag should read from ENV",
		);
		assert.ok(
			runtimeSrc.includes("const configuredBaseUrl = ENV.VITE_TAURI_API_BASE_URL;"),
			"Tauri API base should read from ENV",
		);
		assert.ok(
			runtimeSrc.includes("const configuredRemoteBase = ENV.VITE_TAURI_REMOTE_API_BASE_URL;"),
			"Remote API base should read from ENV",
		);
		assert.ok(
			runtimeSrc.includes("...extractHostnames(WS_API_URL, ENV.VITE_WS_RELAY_URL)"),
			"Relay host extraction should read from ENV",
		);
	});
});
...
describe("variant env guards", () => {
	...
	it("reuses buildVariant for SSR, Tauri, and localhost fallback paths", () => {
		...
		assert.equal(
			buildVariantUses.length,
			3,
			`Expected three buildVariant fallbacks, got ${buildVariantUses.length}`,
		);
		assert.ok(
			variantSrc.includes("if (typeof window === 'undefined') return buildVariant;"),
