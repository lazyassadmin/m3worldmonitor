	...
	it("persistentStaleCeilingMs does not affect in-memory cache TTL", async () => {
		...
		try {
			...
			assert.deepEqual(
				result,
				{ data: "first" },
				"In-memory cache TTL must be independent of persistentStaleCeilingMs",
			);
		} finally {
			clearAllCircuitBreakers();
		}
...
describe("cached-risk-scores.ts — uses persistentStaleCeilingMs", () => {
	...
	it("Risk Scores persistentStaleCeilingMs matches localStorage staleness (1h)", () => {
		...
		const uses1hConstant = /persistentStaleCeilingMs\s*:\s*LS_MAX_STALENESS_MS/.test(
			breakerCreation,
		);
		const uses1hLiteral = /persistentStaleCeilingMs\s*:\s*60\s*\*\s*60\s*\*\s*1000/.test(
			breakerCreation,
		);
	...
	it("updating one breaker cache does not pollute another with a different ceiling", async () => {
		...
		try {
			...
			assert.deepEqual(
				breakerB.getCached(),
				{ x: 99 },
				"breakerB must not be affected by breakerA recordSuccess",
			);
	...
	it("cacheTtlMs=0 disables persistence even if persistentStaleCeilingMs is set", async () => {
		...
		try {
			...
			assert.deepEqual(
				r2,
				{ status: "live2" },
				"cacheTtlMs=0 means no caching — must call fn each time",
			);
		} finally {
			clearAllCircuitBreakers();
		}
	...
	it("persistCache=false means ceiling is irrelevant (no hydration attempted)", async () => {
		...
		try {
			...
			assert.deepEqual(
				r2,
				{ data: "live" },
				"In-memory cache should still work with persistCache=false",
			);
		} finally {
			clearAllCircuitBreakers();
		}
