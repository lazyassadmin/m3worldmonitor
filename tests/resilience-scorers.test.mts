...
describe("resilience scorer contracts", () => {
		...
		for (const countryCode of ["NO", "US", "YE"]) {
			for (const [dimensionId, scorer] of Object.entries(RESILIENCE_DIMENSION_SCORERS)) {
				...
				assert.ok(
					...
					`${countryCode}/${dimensionId} score out of bounds: ${result.score}`,
				);
				assert.ok(
					result.coverage >= 0 && result.coverage <= 1,
					`${countryCode}/${dimensionId} coverage out of bounds: ${result.coverage}`,
				);
			}
		}
	});
	...
	it("returns coverage=0 when all backing seeds are missing (source outage must not impute)", async () => {
		...
		for (const [dimensionId, scorer] of Object.entries(RESILIENCE_DIMENSION_SCORERS)) {
			...
			assert.ok(
				result.score >= 0 && result.score <= 100,
				`${dimensionId} fallback score out of bounds: ${result.score}`,
			);
			assert.equal(
				result.coverage,
				0,
				`${dimensionId} must have coverage=0 when all seeds missing (source outage ≠ country absence)`,
			);
		}
	});
	...
	it("produces the expected weighted overall score from the known fixture dimensions", async () => {
		...
		const domainAverages = Object.fromEntries(
			RESILIENCE_DOMAIN_ORDER.map((domainId) => {
				const dimensionScores = RESILIENCE_DIMENSION_ORDER.filter(
					(dimensionId) => RESILIENCE_DIMENSION_DOMAINS[dimensionId] === domainId,
				).map((dimensionId) => scoreMap[dimensionId].score);
				const average = Number(
					(dimensionScores.reduce((sum, value) => sum + value, 0) / dimensionScores.length).toFixed(
						2,
					),
				);
		...
		assert.equal(overallScore, 68.26);
	});
});
...
describe("scoreEnergy storageBuffer metric", () => {
	...
	it("EU country with low storage (20% fill) scores lower than with high storage", async () => {
		...
		assert.ok(
			...
			`low storage (${resultLow.score}) should score lower than high storage (${resultHigh.score})`,
		);
	});
	...
	it("EU country with null fillPct falls back gracefully (excludes storageBuffer from weighted avg)", async () => {
		...
		assert.ok(
			resultNull.score >= 0 && resultNull.score <= 100,
			`score out of bounds: ${resultNull.score}`,
		);
		assert.equal(
			resultNull.score,
			resultMissing.score,
			"null fillPct should behave identically to missing key",
		);
	});
});
