...
describe("disruption event parsing", () => {
	it("parses comma-separated affectedPorts correctly", () => {
		assert.deepEqual(parseAffectedPorts("port137,port138, port139"), [
			"port137",
			"port138",
			"port139",
		]);
...
describe("hazard matching", () => {
	...
	it("matches RED event within 500km of chokepoint", () => {
		const events = [
			{ alertLevel: "RED", active: true, lat: 25.0, lon: 57.0, eventName: "CYCLONE-X" },
		];
	...
	it("does NOT match event beyond 500km", () => {
		...
		const events = [
			{ alertLevel: "RED", active: true, lat: 19.0, lon: 72.8, eventName: "FAR-FLOOD" },
		];
	...
	it("does NOT match YELLOW or GREEN alerts", () => {
		const events = [
			{ alertLevel: "YELLOW", active: true, lat: 25.0, lon: 57.0, eventName: "YELLOW-EV" },
		];
		assert.equal(findNearestHazard(events, hormuzLat, hormuzLon), null);
	});
	...
	it("does NOT match inactive (ended) events", () => {
		const events = [
			{ alertLevel: "RED", active: false, lat: 25.0, lon: 57.0, eventName: "OLD-EV" },
		];
		assert.equal(findNearestHazard(events, hormuzLat, hormuzLon), null);
	});
