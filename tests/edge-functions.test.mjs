...
describe("scripts/shared/ stays in sync with shared/", () => {
	const sharedFiles = readdirSync(sharedDir).filter(
		(f) => f.endsWith(".json") || f.endsWith(".cjs"),
	);
	for (const file of sharedFiles) {
		it(`scripts/shared/${file} matches shared/${file}`, () => {
			...
			assert.ok(
				existsSync(srcPath),
				`scripts/shared/${file} is missing — run: cp shared/${file} scripts/shared/`,
			);
			...
			assert.strictEqual(
				copy,
				original,
				`scripts/shared/${file} is out of sync with shared/${file} — run: cp shared/${file} scripts/shared/`,
			);
		});
	}
});
...
describe("vercel.json CSP: Slack OAuth callback has unsafe-inline override", () => {
	...
	it("vercel.json has a CSP override for /api/slack/oauth/callback allowing unsafe-inline scripts", () => {
		...
		assert.ok(
			rule,
			"vercel.json: missing header rule for /api/slack/oauth/callback — the callback page serves inline JS (postMessage + window.close) which is blocked by the global CSP",
		);
		const csp = rule.headers?.find((h) => h.key === "Content-Security-Policy");
		assert.ok(
			csp,
			"vercel.json: /api/slack/oauth/callback rule must include a Content-Security-Policy header",
		);
		assert.ok(
