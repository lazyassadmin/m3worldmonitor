...
describe("deploy/cache configuration guardrails", () => {
	it("disables caching for HTML entry routes on Vercel", () => {
		const spaNoCache = getCacheHeaderValue(
			"/((?!api|mcp|oauth|assets|blog|docs|favico|map-styles|data|textures|pro|sw\\.js|workbox-[a-f0-9]+\\.js|manifest\\.webmanifest|offline\\.html|robots\\.txt|sitemap\\.xml|llms\\.txt|llms-full\\.txt|\\.well-known|wm-widget-sandbox\\.html).*)",
		);
	...
	it("contains variant-specific metadata fields used by html replacement and manifest", () => {
		const variantMetaSource = readFileSync(
			resolve(__dirname, "../src/config/variant-meta.ts"),
			"utf-8",
		);
...
describe("security header guardrails", () => {
	...
	it("Permissions-Policy delegates media APIs to allowed origins", () => {
		...
		for (const api of ["autoplay", "encrypted-media"]) {
			assert.match(
				policy,
				new RegExp(
					`${api}=\\(self "https://www\\.youtube\\.com" "https://www\\.youtube-nocookie\\.com"\\)`,
				),
	...
	it("CSP connect-src does not allow unencrypted WebSocket (ws:)", () => {
		...
		assert.ok(
			...
			"CSP connect-src must not contain ws: (unencrypted WebSocket)",
		);
	...
	it("CSP connect-src does not contain localhost in production", () => {
		...
		const connectSrc = csp.match(/connect-src\s+([^;]+)/)?.[1] ?? "";
		assert.ok(
			!connectSrc.includes("http://localhost"),
			"CSP connect-src must not contain http://localhost in production",
		);
	});
	...
	it("CSP script-src includes wasm-unsafe-eval for WebAssembly support", () => {
		...
		assert.ok(
			...
			"CSP script-src must include wasm-unsafe-eval for WASM support",
		);
