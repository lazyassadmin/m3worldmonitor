...
} from "../scripts/seed-climate-zone-normals.mjs";
...
describe("climate zone normals", () => {
	...
	it("requires the new climate-specific zones to be present", () => {
		...
		assert.equal(
			hasRequiredClimateZones([{ zone: "Arctic" }, { zone: "Greenland" }], (zone) => zone.zone),
			false,
		);
	});
});
...
describe("climate anomalies", () => {
	it("uses stored monthly normals instead of a rolling 30-day baseline", () => {
		const normalsIndex = indexZoneNormals({
			normals: [
				{
					zone: "Test Zone",
					months: [{ month: 3, tempMean: 10, precipMean: 2 }],
				},
			],
		});
	...
	it("skips zones missing monthly normals without failing the whole batch", () => {
		...
		const normalsIndex = indexZoneNormals({
			normals: [{ zone: "Zone A", months: [{ month: 3, tempMean: 10, precipMean: 2 }] }],
		});
		const payloads = [
			{
				daily: {
...
`);
		...
		for (const [month, value] of monthlyValues) {
			...
			monthlyLines.push(
				`${year} ${monthNum} ${year}.${monthNum} ${value.toFixed(2)} ${value.toFixed(2)} 30 0.12 0.08`,
			);
		}
...
`);
		...
		const payload = buildCo2MonitoringPayload({
			dailyRows,
			monthlyRows,
			annualRows,
			methaneRows,
...
`);
		const medians = computeSeaIceMonthlyMedians(
			new Map([
				[
					3,
					parseSeaIceMonthlyRows(
						"1981,3,NSIDC-0051,N,14.80,13.20\n1990,3,NSIDC-0051,N,14.70,13.10\n2010,3,NSIDC-0051,N,14.65,13.05",
						3,
					),
				],
				[
					5,
					parseSeaIceMonthlyRows(
						"1981,5,NSIDC-0051,N,12.60,10.90\n1990,5,NSIDC-0051,N,12.40,10.70\n2010,5,NSIDC-0051,N,12.50,10.80",
						5,
					),
				],
				[
					6,
					parseSeaIceMonthlyRows(
						"1981,6,NSIDC-0051,N,10.90,9.50\n1990,6,NSIDC-0051,N,10.80,9.40\n2010,6,NSIDC-0051,N,10.70,9.30",
						6,
					),
				],
			]),
...
`);
	...
	it("counts partial scalar sections so validation does not discard useful partial data", () => {
		...
		assert.equal(
			countIndicators({
				ice_trend_12m: [{ month: "2026-03", extent_mkm2: 13.95, anomaly_mkm2: -0.75 }],
			}),
			1,
		);
	});
	...
	it("preserves prior cache for failed source groups only", () => {
		...
		assert.equal(
			...
			undefined,
			"sea-ice section omitted anomaly — must not bleed from prior",
		);
		assert.equal(
			payload.arctic_trend,
			undefined,
			"sea-ice section omitted trend — must not bleed from prior",
		);
	...
	it("sea-ice climatology unavailable + unrelated failure does not reintroduce stale anomaly/trend", () => {
		...
		assert.equal(
			payload.arctic_extent_anomaly_mkm2,
			undefined,
			"must not reintroduce stale anomaly",
		);
...
describe("open-meteo archive helper", () => {
	...
	it("retries transient fetch errors", async () => {
		...
		try {
			...
			const result = await fetchOpenMeteoArchiveBatch([{ name: "Retry Zone", lat: 1, lon: 2 }], {
