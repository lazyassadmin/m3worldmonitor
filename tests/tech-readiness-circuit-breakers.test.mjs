...
function getTechIndicatorKeys(sourceFile) {
	...
	assert.ok(
		...
		"TECH_INDICATORS object must exist",
	);
...
function getCreateCircuitBreakerNameInitializer(fn) {
	const createCall = collectCallExpressions(fn).find((call) =>
		isIdentifierNamed(call.expression, "createCircuitBreaker"),
	);
	...
	assert.ok(
		...
		"createCircuitBreaker must receive an options object",
	);
...
describe("economic/index.ts — per-indicator World Bank circuit breakers", () => {
	...
	it("has a wbBreakers Map for per-indicator instances", () => {
		...
		assert.ok(
			...
			"wbBreakers declaration must exist",
		);
		assert.ok(
			isIdentifierNamed(decl.initializer.expression, "Map"),
			"wbBreakers must be initialized with new Map(...)",
		);
	});
	...
	it("has a getWbBreaker(indicatorCode) factory function", () => {
		...
		assert.ok(
			collectCallExpressions(fn).some((call) =>
				isIdentifierNamed(call.expression, "createCircuitBreaker"),
			),
	...
	it("mirrors fredBatchBreaker pattern (consistency check)", () => {
		...
		assert.ok(
			fredDecl?.initializer && ts.isCallExpression(fredDecl.initializer),
			"fredBatchBreaker must exist",
		);
		assert.ok(isIdentifierNamed(fredDecl.initializer.expression, "createCircuitBreaker"));
		assert.ok(
			findFunctionDeclaration(sourceFile, "getWbBreaker"),
			"getWbBreaker implementation should be present",
		);
	});
});
...
describe("CircuitBreaker isolation — independent per-indicator instances", () => {
	...
	it("two breakers with different names are independent (failure in one does not trip the other)", async () => {
		...
		const breakerA = createCircuitBreaker({
			name: "WB:IT.NET.USER.ZS",
			cacheTtlMs: 30 * 60 * 1000,
		});
		const breakerB = createCircuitBreaker({
			name: "WB:IT.CEL.SETS.P2",
			cacheTtlMs: 30 * 60 * 1000,
		});
		...
		const alwaysFail = () => {
			callCount++;
			throw new Error("World Bank unavailable");
		};
		...
		assert.equal(
			breakerB.isOnCooldown(),
			false,
			"breakerB must not be on cooldown when breakerA fails",
		);
		...
		assert.deepEqual(
			result,
			goodData,
			"breakerB should return live data unaffected by breakerA cooldown",
		);
	...
	it("two breakers with different names cache independently (no cross-indicator cache poisoning)", async () => {
		...
		const fallback = { data: [], pagination: undefined };
		const internetData = {
			data: [{ countryCode: "USA", indicatorCode: "IT.NET.USER.ZS", year: 2023, value: 90 }],
			pagination: undefined,
		};
		const mobileData = {
			data: [{ countryCode: "USA", indicatorCode: "IT.CEL.SETS.P2", year: 2023, value: 120 }],
			pagination: undefined,
		};
	...
	it("empty server response does not poison the cache for other indicators", async () => {
		...
		const goodData = {
			data: [{ countryCode: "DEU", indicatorCode: "IT.CEL.SETS.P2", year: 2023, value: 130 }],
			pagination: undefined,
		};
		...
		assert.equal(
			resultB.data.length,
			1,
			"breakerB returns real data unaffected by breakerA empty cache",
		);
