...
describe("isAllowedRouteHost", () => {
	it("accepts URL matching a listed site exactly", () => {
		assert.equal(
			isAllowedRouteHost("https://carrefouruae.com/product/sugar", [
				"carrefouruae.com",
				"noon.com",
			]),
			true,
	...
	it("accepts URL with www. prefix", () => {
		assert.equal(
			isAllowedRouteHost("https://www.carrefouruae.com/product/sugar", ["carrefouruae.com"]),
			true,
		);
	});
	...
	it("accepts subdomain of listed site", () => {
		assert.equal(
			isAllowedRouteHost("https://shop.luluhypermarket.com/en/sugar", ["luluhypermarket.com"]),
			true,
		);
	});
	...
	it("rejects URL from unlisted hostname", () => {
		assert.equal(
			isAllowedRouteHost("https://numbeo.com/cost-of-living", ["carrefouruae.com"]),
			false,
		);
	});
...
describe("bulkReadLearnedRoutes", () => {
	...
	it("parses valid pipeline responses into Map", async () => {
		const route = {
			url: "https://carrefouruae.com/sugar",
			lastSuccessAt: 1000,
			hits: 3,
			failsSinceSuccess: 0,
			currency: "AED",
		...
		const restore = mockFetch(async () => ({
			ok: true,
			json: async () => [{ result: JSON.stringify(route) }, { result: null }],
		}));
...
describe("bulkWriteLearnedRoutes", () => {
	...
	it("sends DELs before SETs in pipeline", async () => {
		...
		const route = {
			url: "https://carrefouruae.com/salt",
			lastSuccessAt: 1000,
			hits: 1,
			failsSinceSuccess: 0,
			currency: "AED",
...
describe("processItemRoute", () => {
	...
	it("learned-hit success: fetchViaExa not called", async () => {
		...
		assert.equal(result.localPrice, 5.5);
	...
	it("learned-hit fail + EXA success: routeUpdate has new URL, hits=1", async () => {
		const result = await processItemRoute({
			...
			fetchViaExa: async () => ({
				localPrice: 6.0,
				sourceSite: "https://carrefouruae.com/new-sugar",
			}),
		});
		assert.equal(result.localPrice, 6.0);
	...
	it("EXA success but host not in allowlist: price returned, route NOT saved", async () => {
		const result = await processItemRoute({
			...baseOpts,
			learned: undefined,
			fetchViaExa: async () => ({ localPrice: 5.5, sourceSite: "https://evil.com/sugar" }),
		});
		assert.equal(result.localPrice, 5.5);
		assert.equal(result.routeUpdate, null);
		assert.equal(result.routeDelete, false);
