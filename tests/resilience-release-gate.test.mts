...
const REQUIRED_DIMENSION_COUNTRIES = [
	"US",
	"GB",
	"DE",
	"FR",
...
describe("resilience release gate", () => {
		...
		for (const countryCode of REQUIRED_DIMENSION_COUNTRIES) {
			...
			for (const [dimensionId, score] of entries) {
				assert.ok(
					...
					`${countryCode} ${dimensionId} should produce a numeric score`,
				);
				assert.ok(
					score.coverage > 0,
					`${countryCode} ${dimensionId} should not fall back to zero-coverage placeholder scoring`,
				);
			}
		}
	});
	...
	it("keeps Cronbach alpha above 0.6 for at least 10 G20 countries and preserves score sanity anchors", async () => {
		...
		assert.ok(
			alphaPassing.length >= 10,
			`expected at least 10 G20 countries with alpha > 0.6, got ${alphaPassing.length}`,
		);
		...
		for (const response of highAnchors) {
			assert.ok(
				response.overallScore >= 75,
				`${response.countryCode} should remain in the high-resilience band`,
			);
		}
		...
		for (const response of lowAnchors) {
			assert.ok(
				response.overallScore <= 30,
				`${response.countryCode} should remain in the low-resilience band`,
			);
		}
	});
	...
	it("marks sparse WHO/FAO countries as low confidence", async () => {
		...
		for (const countryCode of SPARSE_CONFIDENCE_COUNTRIES) {
			...
			assert.equal(
				...
				true,
				`${countryCode} should be flagged as low confidence`,
			);
		}
	});
	...
	it("Lebanon (fragile) scores lower than South Africa (stressed)", async () => {
		...
		const [lb, za] = await Promise.all([
			getResilienceScore({ request: new Request("https://example.com?countryCode=LB") } as never, {
				countryCode: "LB",
			}),
			getResilienceScore({ request: new Request("https://example.com?countryCode=ZA") } as never, {
				countryCode: "ZA",
			}),
		]);
	...
	it("US is not low-confidence with full 8/8 dataset coverage", async () => {
		...
		assert.equal(
			us.lowConfidence,
			false,
			`US has full 8/8 dataset coverage in fixtures and should not be flagged low-confidence`,
		);
	});
	...
	it("produces complete ranking and choropleth entries for the full G20 + EU27 release set", async () => {
		...
		const ranking = await getResilienceRanking(
			{ request: new Request("https://example.com") } as never,
			{},
		);
		const relevantItems = ranking.items.filter((item) =>
			CHOROPLETH_TARGET_COUNTRIES.includes(
				item.countryCode as (typeof CHOROPLETH_TARGET_COUNTRIES)[number],
			),
		);
		assert.equal(relevantItems.length, CHOROPLETH_TARGET_COUNTRIES.length);
		assert.ok(
			relevantItems.every((item) => item.overallScore >= 0),
			"release-gate countries should not fall back to blank ranking placeholders",
		);
