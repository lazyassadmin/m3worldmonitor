...
describe("summary circuit breaker configuration", () => {
	...
	it("persistCache is enabled on summaryResultBreaker", () => {
		assert.match(
			src,
			/summaryResultBreaker[\s\S]*?persistCache:\s*true/,
			"persistCache should be true on summaryResultBreaker",
		);
	});
	...
	it("cacheTtlMs is 2 hours on summaryResultBreaker", () => {
		...
		assert.match(
			block![1].trim(),
			/^2\s*\*\s*60\s*\*\s*60\s*\*\s*1000$/,
			`cacheTtlMs should be 2 * 60 * 60 * 1000, got "${block![1].trim()}"`,
		);
	});
});
