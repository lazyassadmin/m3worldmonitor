...
describe("redis caching behavior", { concurrency: 1 }, () => {
	it("coalesces concurrent misses into one upstream fetcher execution", async () => {
		...
		try {
			...
			assert.ok(
				setCalls >= 1,
				"at least one cache write should happen after coalesced fetch (data + optional seed-meta)",
			);
		} finally {
	...
	it("cachedFetchJsonWithMeta returns data:null source:cache on sentinel hit", async () => {
		...
		try {
			...
			assert.equal(
				second.data,
				null,
				"sentinel should resolve to null data, not the sentinel string",
			);
...
describe("country intel brief caching behavior", { concurrency: 1 }, () => {
	...
	it("uses distinct cache keys for distinct context snapshots", async () => {
		...
		try {
			...
			const alpha = await module.getCountryIntelBrief(
				makeCtx(
					"https://example.com/api/intelligence/v1/get-country-intel-brief?country_code=IL&context=alpha",
				),
				req,
			);
			const beta = await module.getCountryIntelBrief(
				makeCtx(
					"https://example.com/api/intelligence/v1/get-country-intel-brief?country_code=IL&context=beta",
				),
				req,
			);
			const alphaCached = await module.getCountryIntelBrief(
				makeCtx(
					"https://example.com/api/intelligence/v1/get-country-intel-brief?country_code=IL&context=alpha",
				),
				req,
			...
			assert.ok(
				setKeys[0]?.startsWith("ci-sebuf:v3:IL:"),
				"cache key should use v3 country-intel namespace",
			);
			assert.ok(
				setKeys[1]?.startsWith("ci-sebuf:v3:IL:"),
				"cache key should use v3 country-intel namespace",
			);
	...
	it("uses base cache key and prompt when context is missing or blank", async () => {
		...
		try {
			...
			const first = await module.getCountryIntelBrief(
				makeCtx("https://example.com/api/intelligence/v1/get-country-intel-brief?country_code=US"),
				req,
			);
			const second = await module.getCountryIntelBrief(
				makeCtx(
					"https://example.com/api/intelligence/v1/get-country-intel-brief?country_code=US&context=%20%20%20",
				),
				req,
			);
			...
			assert.ok(
				!userPrompts[0]?.includes("Context snapshot:"),
				"prompt should omit context block when absent",
			);
