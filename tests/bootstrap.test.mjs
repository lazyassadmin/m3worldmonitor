...
describe("Bootstrap cache key registry", () => {
	...
	it("api/bootstrap.js inlined keys match server/_shared/cache-keys.ts", () => {
		...
		for (const [name, key] of Object.entries(canonical)) {
			assert.equal(
				inlined[name],
				key,
				`Key '${name}' mismatch: canonical='${key}', inlined='${inlined[name]}'`,
			);
		}
	...
	it("every cache key matches a handler cache key pattern", () => {
		...
		for (const key of keys) {
			assert.match(
				key,
				/^[a-z0-9_-]+(?::[a-z0-9_-]+)+:v\d+(?::[a-z0-9_-]+)*$/,
				`Cache key "${key}" does not match expected pattern`,
			);
		}
	});
	...
	it("has no duplicate cache keys", () => {
		...
		assert.equal(
			unique.size,
			keys.length,
			`Found duplicate cache keys: ${keys.filter((k, i) => keys.indexOf(k) !== i)}`,
		);
	});
	...
	it("has no duplicate logical names", () => {
		...
		assert.equal(
			unique.size,
			names.length,
			`Found duplicate names: ${names.filter((n, i) => names.indexOf(n) !== i)}`,
		);
	});
		...
		function walk(dir) {
			for (const entry of readdirSync(dir)) {
				...
				if (statSync(full).isDirectory()) walk(full);
					...
					entry.endsWith(".ts") &&
					!entry.includes("service_server") &&
					!entry.includes("service_client")
				) {
...
describe("Bootstrap endpoint (api/bootstrap.js)", () => {
	...
	it("defines getCachedJsonBatch inline (self-contained, no server imports)", () => {
		...
		assert.ok(
			...
			"Should not import from server/ — Edge Functions cannot resolve cross-directory TS imports",
		);
	});
...
describe("Frontend hydration (src/services/bootstrap.ts)", () => {
	...
	it("exports fetchBootstrapData function", () => {
		assert.ok(
			src.includes("export async function fetchBootstrapData"),
			"Missing fetchBootstrapData export",
		);
	});
	...
	it("uses consume-once pattern (deletes after read)", () => {
		assert.ok(
			src.includes(".delete("),
			"Missing delete in getHydratedData — consume-once pattern not implemented",
		);
	});

	it("has a fast timeout cap to avoid regressing startup", () => {
		const timeoutMatches = [
			...src.matchAll(/setTimeout\([^,]+,\s*(?:desktop\s*\?\s*[\d_]+\s*:\s*)?(\d[\d_]*)\)/g),
		];
		...
		for (const m of timeoutMatches) {
			...
			assert.ok(
				ms <= 5000,
				`Timeout ${ms}ms too high — should be ≤5000ms to avoid regressing startup`,
			);
		}
	});
	...
	it("keeps web bootstrap tier timeouts under 2 seconds", () => {
		...
		assert.deepEqual(
			timeouts,
			[1200, 1800],
			`Expected aggressive web bootstrap timeouts (1200, 1800)`,
		);
	});

	it("allows longer bootstrap timeouts for desktop runtime", () => {
		assert.ok(
			src.includes("isDesktopRuntime"),
			"Bootstrap should branch on desktop for longer timeouts",
		);
	});
	...
	it("handles fetch failure silently", () => {
		assert.ok(
			src.includes("catch"),
			"Missing error handling — panels should fall through to individual calls",
		);
	});
...
describe("Panel hydration consumers", () => {
	const panels = [
		...
		{
			name: "ServiceStatusPanel (via infrastructure)",
	...
	for (const panel of panels) {
		it(`${panel.name} checks getHydratedData('${panel.key}')`, () => {
			...
			assert.ok(
				src.includes("getHydratedData"),
				`${panel.name} missing getHydratedData import/usage`,
			);
			assert.ok(
				src.includes(`'${panel.key}'`),
				`${panel.name} missing hydration key '${panel.key}'`,
			);
		});
	}
});
...
describe("Bootstrap tier definitions", () => {
	...
	it("SLOW_KEYS + FAST_KEYS cover all BOOTSTRAP_CACHE_KEYS with no overlap", () => {
		...
		assert.deepEqual(
			[...union].sort(),
			[...all].sort(),
			"SLOW_KEYS ∪ FAST_KEYS must equal BOOTSTRAP_CACHE_KEYS",
		);
	...
	it("tier sets in bootstrap.js match BOOTSTRAP_TIERS in cache-keys.ts", () => {
		...
		assert.deepEqual(
			[...tierKeys].sort(),
			[...setKeys].sort(),
			"BOOTSTRAP_TIERS keys must match SLOW_KEYS ∪ FAST_KEYS",
		);
	});
});
...
describe("Adaptive backoff adopters", () => {
	it("ServiceStatusPanel.fetchStatus returns Promise<boolean>", () => {
		...
		assert.ok(
			src.includes("fetchStatus(): Promise<boolean>"),
			"fetchStatus should return Promise<boolean> for adaptive backoff",
		);
	...
	it("MacroSignalsPanel.fetchData returns Promise<boolean>", () => {
		...
		assert.ok(
			src.includes("fetchData(): Promise<boolean>"),
			"fetchData should return Promise<boolean> for adaptive backoff",
		);
	...
	it("StrategicRiskPanel.refresh returns Promise<boolean>", () => {
		...
		assert.ok(
			src.includes("refresh(): Promise<boolean>"),
			"refresh should return Promise<boolean> for adaptive backoff",
		);
		assert.ok(
			src.includes("lastRiskFingerprint"),
			"Missing lastRiskFingerprint for change detection",
		);
	});
});
