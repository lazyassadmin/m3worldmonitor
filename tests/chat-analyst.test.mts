...
import {
	buildActionEvents,
	VISUAL_INTENT_RE,
} from "../server/worldmonitor/intelligence/v1/chat-analyst-actions.ts";
...
describe("buildAnalystSystemPrompt — domain filtering", () => {
	...
	it("empty context produces no-live-data fallback", () => {
		...
		assert.ok(
			prompt.includes("No live context available"),
			"should include fallback text when no context",
		);
	});
	...
	it("unknown domain falls back to all-inclusive behavior", () => {
		...
		assert.ok(
			prompt.includes("Global tensions elevated"),
			"should include worldBrief for unknown domain",
		);
...
describe("buildActionEvents — visual intent detection", () => {
	...
	it("returns empty for non-visual market summary query", () => {
		assert.deepEqual(
			buildActionEvents("Key market moves, macro signals, and commodity moves today"),
			[],
		);
	});
...
describe("extractKeywords", () => {
	...
	it("preserves uk, eu, un, ai regardless of case", () => {
		for (const acronym of ["uk", "eu", "un", "ai"]) {
			...
			assert.ok(
				upper.includes(acronym),
				`"${acronym.toUpperCase()}" (uppercase) should be preserved and lowercased`,
			);
		}
	});
...
describe("extractKeywords — retrieval priority (current turn first)", () => {
	...
	it("prior-turn keywords backfill remaining slots after current-turn fills first", () => {
		...
		assert.ok(
			kw.includes("countries") || kw.includes("electricity") || kw.includes("consumption"),
	...
	it("prior-turn does not crowd out current-turn pivot when prior is long", () => {
		...
		const currentQuery = "What about Germany?";
		const longPrior =
			"global shipping routes disrupted supply chains ports containers freight logistics maritime";
