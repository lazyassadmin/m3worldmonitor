...
const fnBody = mainSrc
	.slice(bsStart + "beforeSend(event) ".length, bsEnd)
...
describe("first-party file detection", () => {
	...
	for (const [filename, label] of testPatterns) {
		it(`treats ${label} (${filename}) as first-party`, () => {
			...
			assert.ok(
				result !== null,
				`${filename} should be detected as first-party, event should NOT be suppressed`,
			);
		});
	}
	...
	it("does NOT treat blob: URLs as first-party", () => {
		const event = makeEvent(".trim is not a function", "TypeError", [extensionFrame()]);
...
describe("empty-stack network/timeout errors are NOT suppressed", () => {
	...
	for (const msg of networkErrors) {
		it(`lets through "${msg.slice(0, 60)}..." with empty stack`, () => {
			...
			assert.ok(
				result !== null,
				`"${msg}" with empty stack should NOT be suppressed (could be our code)`,
			);
		});
	}
	...
	for (const msg of networkErrors) {
		it(`suppresses "${msg.slice(0, 50)}..." with confirmed third-party stack`, () => {
			...
			assert.equal(
				beforeSend(event),
				null,
				`"${msg}" with extension-only stack should be suppressed`,
			);
		});
	}
