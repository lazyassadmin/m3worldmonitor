...
describe("computeEntries formula", () => {
	...
	it("calculates per_second_rate from deficit %", () => {
		...
		assert.ok(
			Math.abs(jpn.perSecondRate - expectedPerSec) < 1,
			`perSecondRate=${jpn.perSecondRate}`,
		);
	});
...
describe("US Treasury override", () => {
	it("uses Treasury debtUsd for USA when provided", () => {
		...
		const entries = computeEntries(
			...
			gdp,
			{},
			{ debtUsd: treasuryDebtUsd, date: "2024-12-31" },
		);
		assert.equal(entries.length, 1);
		assert.ok(
			Math.abs(entries[0].debtUsd - treasuryDebtUsd) < 1e6,
			`debtUsd should be Treasury value`,
		);
