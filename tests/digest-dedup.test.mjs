...
describe("deduplicateStories", () => {
	...
	it("merges near-duplicate Reuters headlines about downed jet", () => {
		const stories = [
			story(
				"US fighter jet shot down over Iran, search underway for crew, US official says - reuters.com",
				90,
			),
			story(
				"US fighter jet shot down over Iran, search underway for crew, US officials say - reuters.com",
				85,
			),
			story(
				"US fighter jet shot down over Iran, search under way for crew member, US officials say - reuters.com",
				80,
			),
			story(
		...
		assert.equal(
			result.length,
			1,
			`Expected 1 cluster, got ${result.length}: ${result.map((r) => r.title).join(" | ")}`,
		);
	...
	it("merges same story reported by different outlets with different suffixes", () => {
		const stories = [
			...
			story(
				"Downed planes spell new peril for Trump as Tehran hunts missing US pilot - reuters.com",
				85,
			),
		];
		const result = mod.deduplicateStories(stories);
	...
	it("merges stories with minor wording differences", () => {
		const stories = [
			...
			story(
				"Trump, Israel pressure Iran ahead of deadline as search continues for missing US airman - Reuters",
				70,
			),
		];
		const result = mod.deduplicateStories(stories);
