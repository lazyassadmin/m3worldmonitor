...
describe("parseRssItems", () => {
	it("extracts title, url, publishedAt from a minimal RSS XML fixture", () => {
		...
		assert.ok(
			typeof first.publishedAt === "number" && first.publishedAt > 0,
			"publishedAt should be a positive number",
		);
...
describe("filterEnergyRelevant", () => {
	it("keeps items with 'oil' in title, drops items with no energy keywords", () => {
		const items = [
			...
			{
				id: "2",
				title: "Latest sports results from the weekend",
				url: "https://example.com/2",
				source: "IEA",
				publishedAt: Date.now(),
				summary: "Football match highlights and scores.",
			},
			{
				id: "3",
				title: "Tech startup raises funding round",
				url: "https://example.com/3",
				source: "IEA",
				publishedAt: Date.now(),
				summary: "Silicon Valley venture capital news.",
			},
		];
	...
	it("is case-insensitive — 'LNG' in title matches 'lng' keyword", () => {
		const items = [
			{
				id: "1",
				title: "LNG exports hit record highs in Q1 2026",
				url: "https://example.com/1",
				source: "IEA",
				publishedAt: Date.now(),
				summary: "",
	...
	it("matches keyword in summary when title has no keyword", () => {
		const items = [
			{
				id: "1",
				title: "Market update for April",
				url: "https://example.com/1",
				source: "IEA",
				publishedAt: Date.now(),
				summary: "Crude oil inventories fell sharply last week.",
			},
		];
		const filtered = filterEnergyRelevant(items);
		assert.equal(filtered.length, 1);
