		...
		if (keyCheck.required && !keyCheck.valid) {
			if (needsLegacyProBearerGate) {
				...
				if (authHeader?.startsWith("Bearer ")) {
					...
					return new Response(
						JSON.stringify({ error: keyCheck.error, _debug: (keyCheck as any)._debug }),
						{
							status: 401,
							headers: { "Content-Type": "application/json", ...corsHeaders },
						},
		...
		if (!matchedHandler && request.method === "POST") {
			...
			if (contentLen < 1_048_576) {
				...
				try {
					...
					/* non-JSON body — skip POST→GET conversion */
				}
		...
		if (response.status === 200 && request.method === "GET" && response.body) {
			...
			if (mergedHeaders.get("X-No-Cache") || isUpstreamUnavailable) {
				...
				const envOverride = process.env[
					`CACHE_TIER_OVERRIDE_${rpcName.replace(/-/g, "_").toUpperCase()}`
				] as CacheTier | undefined;
				const tier =
					(envOverride && envOverride in TIER_HEADERS ? envOverride : null) ??
