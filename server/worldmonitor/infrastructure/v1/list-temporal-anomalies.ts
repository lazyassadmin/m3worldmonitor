...

		{
			...
			const baselines = await Promise.all(
				typesWithCounts.map(
					(t) =>
						getCachedJson(
							makeBaselineKeyV2(t, "global", weekday, month),
						) as Promise<BaselineEntry | null>,
				),
			);
			...
			for (let i = 0; i < typesWithCounts.length; i++) {
				...
				try {
					await setCachedJson(
						makeBaselineKeyV2(type, "global", weekday, month),
						{
							mean: newMean,
							m2: newM2,
							sampleCount: n,
			...
			if (writeFailures > 0) {
				console.warn(
					`[TemporalBaseline] ${writeFailures}/${typesWithCounts.length} baseline writes failed`,
				);
			}
