...
const CABLE_LANDINGS: Record<string, [number, number][]> = {
	...
	southern_cross_cable_network_sccn: [
		[-33.87, 151.21],
		[-36.85, 174.76],
		[33.74, -118.27],
	],
	...
	eastern_africa_submarine_system_eassy: [
		[-29.85, 31.02],
		[-25.97, 32.58],
		[-6.8, 39.28],
		[-4.04, 39.67],
	...
	southeast_asia_japan_cable_sjc: [
		[35.69, 139.69],
		[36.07, 120.32],
		[1.35, 103.82],
		[22.29, 114.17],
...
export async function getCableHealth(
	...
	try {
		const result = await cachedFetchJson<GetCableHealthResponse>(CACHE_KEY, CACHE_TTL, async () => {
			...
			const ngaData = await cachedFetchJson<NgaWarning[]>(
				NGA_CACHE_KEY,
				NGA_CACHE_TTL,
				fetchNgaWarnings,
			);
		...
		if (result) {
			...
			setCachedJson(
				...
				{ fetchedAt: Date.now(), recordCount: Math.max(count, 1) },
				604800,
			).catch(() => {});
