	...
	try {
		...
		if (service.customParser === "statusio") {
			...
			if (code >= 500) {
				return withStatus(
					...
					overall.status || "Service disruption",
					latencyMs,
				);
			}
		...
		if (service.customParser === "stripe") {
			...
			if (data.largestatus === "degraded") {
				return withStatus(
					"SERVICE_OPERATIONAL_STATUS_DEGRADED",
					data.message || "Degraded performance",
					latencyMs,
				);
			}
			if (data.largestatus === "down") {
				return withStatus(
					"SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE",
					data.message || "Service disruption",
					latencyMs,
				);
			}
		...
		if (data.status?.indicator !== undefined) {
			return withStatus(
				normalizeToProtoStatus(data.status.indicator),
...
export async function listServiceStatuses(
	...
	try {
		const { data: results, source } = await cachedFetchJsonWithMeta<ServiceStatus[]>(
			INFRA_CACHE_KEY,
			INFRA_CACHE_TTL,
			async () => {
		...
		if (results) {
			...
			if (source === "fresh") {
				setCachedJson(
					"seed-meta:infra:service-statuses",
