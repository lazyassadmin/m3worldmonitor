...
async function trySeededData(
	req: GetDisplacementSummaryRequest,
): Promise<GetDisplacementSummaryResponse | null> {
	try {
		...
		const [seedData, seedMeta] = await Promise.all([
			...
			getCachedJson("seed-meta:displacement:summary", true) as Promise<{
				fetchedAt?: number;
			} | null>,
		]);
...
export async function getDisplacementSummary(
	...
	try {
		...
		const result = await cachedFetchJson<GetDisplacementSummaryResponse>(
			cacheKey,
			REDIS_CACHE_TTL,
			async () => {
