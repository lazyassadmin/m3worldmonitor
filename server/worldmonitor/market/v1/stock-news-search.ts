...
async function searchWithBrave(
	...
	return dedupeHeadlines(
		(payload.web?.results || []).map((item) => ({
			title: String(item.title || "").trim(),
			source:
				String(item.meta_url?.hostname || "").replace(/^www\./, "") ||
...
async function searchWithSerpApi(
	...
	const rawResults =
		(payload.news_results?.length ? payload.news_results : payload.organic_results) || [];
...
async function searchViaProviders(
	...
	maxResults: number,
	days: number,
): Promise<StockNewsSearchResult | null> {
...
export async function searchRecentStockHeadlines(
	...
	const cached = await cachedFetchJson<StockNewsSearchResult>(
		cacheKey,
