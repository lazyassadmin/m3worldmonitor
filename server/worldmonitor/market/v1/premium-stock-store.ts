...
function compareAnalysisDesc<T extends { analysisAt: number; generatedAt: string }>(
	a: T,
	b: T,
): number {
...
async function zrevrange(key: string, limit: number): Promise<string[]> {
	if (limit <= 0) return [];
	const data = await runRedisPipeline([["ZREVRANGE", key, 0, Math.max(0, limit - 1)]]);
...
export async function storeStockBacktestSnapshot(snapshot: BacktestStockResponse): Promise<void> {
