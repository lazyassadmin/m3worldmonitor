	...
	for (let i = 0; i < symbols.length; i += BATCH) {
		...
		for (let attempt = 0; attempt < 2; attempt++) {
			try {
				...
				console.warn(
					`[AV] Bulk quotes fetch error (attempt ${attempt + 1}):`,
		...
		try {
			...
			if (json.Information) {
				...
				console.warn(
					`[AV] Rate limit hit${remaining > 0 ? ` — dropping ${remaining} remaining symbols` : ""}: ${json.Information.slice(0, 80)}`,
				);
				break;
			}
...
export async function fetchCoinPaprikaMarkets(geckoIds: string[]): Promise<CoinGeckoMarketItem[]> {
...
export async function fetchCryptoMarkets(ids: string[]): Promise<CoinGeckoMarketItem[]> {
	try {
