...
export const listThermalEscalations: ThermalServiceHandler["listThermalEscalations"] = async (
	...
	const summary = {
		...
		conflictAdjacentCount: sliced.filter((c) => c.context === "THERMAL_CONTEXT_CONFLICT_ADJACENT")
			.length,
		highRelevanceCount: sliced.filter((c) => c.strategicRelevance === "THERMAL_RELEVANCE_HIGH")
			.length,
	};
