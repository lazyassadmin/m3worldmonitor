...
async function readHistory(countryCode: string): Promise<ResilienceHistoryPoint[]> {
	const result = await runRedisPipeline([["ZRANGE", historyKey(countryCode), 0, -1, "WITHSCORES"]]);
...
export async function ensureResilienceScoreCached(
	countryCode: string,
	...
	return (
		(await cachedFetchJson<GetResilienceScoreResponse>(
			...
			async () => {
				...
				const history = (await readHistory(normalizedCountryCode)).filter(
...
export async function listScorableCountries(): Promise<string[]> {
	const manifest = (await getCachedJson(
		RESILIENCE_STATIC_INDEX_KEY,
		true,
	)) as ResilienceStaticIndex | null;
	return (manifest?.countries ?? [])
		.map((countryCode) => normalizeCountryCode(String(countryCode || "")))
		.filter(Boolean);
}

export async function getCachedResilienceScores(
	countryCodes: string[],
	...
	const results = await runRedisPipeline(
		normalized.map((countryCode) => ["GET", scoreCacheKey(countryCode)]),
	);
...
export const GREY_OUT_COVERAGE_THRESHOLD = 0.4;
...
function computeOverallCoverage(response: GetResilienceScoreResponse): number {
	const coverages = response.domains.flatMap((domain) =>
		domain.dimensions.map((dimension) => dimension.coverage),
	);
...
export async function warmMissingResilienceScores(countryCodes: string[]): Promise<void> {
	const uniqueCodes = [
		...new Set(
			countryCodes.map((countryCode) => normalizeCountryCode(countryCode)).filter(Boolean),
		),
	];
