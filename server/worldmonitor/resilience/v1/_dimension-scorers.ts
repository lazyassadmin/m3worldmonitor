...
interface ResilienceStaticCountryRecord {
	...
	iea?: {
		energyImportDependency?: { value?: number; year?: number | null; source?: string } | null;
	} | null;
}
...
function weightedBlend(metrics: WeightedMetric[]): ResilienceDimensionScore {
	...
	const weightedScore =
		available.reduce((sum, metric) => sum + (metric.score || 0) * metric.weight, 0) /
		availableWeight;
...
async function readStaticCountry(
	countryCode: string,
	reader: ResilienceSeedReader,
): Promise<ResilienceStaticCountryRecord | null> {
...
function getLatestDebtEntry(raw: unknown, countryCode: string): NationalDebtEntry | null {
	...
	const entries: NationalDebtEntry[] = Array.isArray(
...
function countTradeRestrictions(raw: unknown, countryCode: string): number {
	const restrictions: TradeRestriction[] = Array.isArray(
		(raw as { restrictions?: unknown[] } | null)?.restrictions,
	)
		? ((raw as { restrictions?: TradeRestriction[] }).restrictions ?? [])
		: [];
	return restrictions.reduce((count, item) => {
		const matches =
			matchesCountryIdentifier(item.reportingCountry, countryCode) ||
...
function countTradeBarriers(raw: unknown, countryCode: string): number {
	const barriers: TradeBarrier[] = Array.isArray((raw as { barriers?: unknown[] } | null)?.barriers)
		...
		: [];
	return barriers.reduce(
		(count, item) => count + (matchesCountryIdentifier(item.notifyingCountry, countryCode) ? 1 : 0),
		0,
	);
}

function summarizeOutages(
	raw: unknown,
	countryCode: string,
): { total: number; major: number; partial: number } {
...
function summarizeUnrest(
	raw: unknown,
	countryCode: string,
): { unrestCount: number; fatalities: number } {
	const events: UnrestEvent[] = Array.isArray((raw as { events?: unknown[] } | null)?.events)
		? ((raw as { events?: UnrestEvent[] }).events ?? [])
		: [];
	return events.reduce<{ unrestCount: number; fatalities: number }>(
		(summary, item) => {
			if (
				!matchesCountryText(item.country, countryCode) &&
				!matchesCountryIdentifier(item.country, countryCode)
...
function summarizeUcdp(
	raw: unknown,
	countryCode: string,
): { eventCount: number; deaths: number; typeWeight: number } {
...
function summarizeSocialVelocity(raw: unknown, countryCode: string): number {
	...
	return posts.reduce(
		(sum, post) =>
			sum + (matchesCountryText(post.title, countryCode) ? (safeNum(post.velocityScore) ?? 0) : 0),
		0,
	);
}
...
function getTransitDisruptionScore(raw: unknown): number | null {
	const summaries = (
		raw as {
			summaries?: Record<string, { disruptionPct?: number; incidentCount7d?: number }>;
		} | null
	)?.summaries;
...
function getEnergyPriceStress(raw: unknown): number | null {
	const prices: Array<{ change?: number }> = Array.isArray(
		(raw as { prices?: Array<{ change?: number }> } | null)?.prices,
	)
...
function scoreAquastatValue(record: ResilienceStaticCountryRecord | null): number | null {
	...
	if (
		indicator.includes("stress") ||
		indicator.includes("withdrawal") ||
		indicator.includes("dependency")
	) {
		return normalizeLowerBetter(value, 0, 100);
	}
	if (
		indicator.includes("availability") ||
		indicator.includes("renewable") ||
		indicator.includes("access")
	) {
		return value <= 100
	...
	console.warn(
		`[Resilience] AQUASTAT indicator "${record?.aquastat?.indicator}" did not match known keywords, using value-range heuristic`,
	);
	return value <= 100 ? normalizeHigherBetter(value, 0, 100) : normalizeLowerBetter(value, 0, 5000);
}
...
export async function scoreMacroFiscal(
	...
	return weightedBlend([
		...
		imfMacroRaw == null
			...
			: {
					score:
						imfEntry?.govRevenuePct == null
							? null
							: normalizeHigherBetter(imfEntry.govRevenuePct, 5, 45),
					weight: 0.5,
				},
		// Debt growth rate: rapid debt accumulation = fiscal stress even at moderate levels.
		{
			score: extractMetric(debtEntry, (entry) =>
				normalizeLowerBetter(Math.max(0, safeNum(entry.annualGrowth) ?? 0), 0, 20),
			),
			weight: 0.2,
		},
		...
		imfMacroRaw == null
			...
			: {
					score:
						imfEntry?.currentAccountPct == null
							? null
							: normalizeHigherBetter(
									Math.max(-20, Math.min(imfEntry.currentAccountPct, 20)),
									-20,
									20,
								),
					weight: 0.3,
	...
	return weightedBlend([
		{ score: vol == null ? null : normalizeLowerBetter(vol, 0, 50), weight: 0.7 },
		{
			score:
				latest == null
					? null
					: normalizeLowerBetter(Math.abs((safeNum(latest.realEer) ?? 100) - 100), 0, 35),
			weight: 0.3,
		},
	]);
}
...
export async function scoreCyberDigital(
	...
	return weightedBlend([
		{
			score: cyberRaw != null ? normalizeLowerBetter(cyber.weightedCount, 0, 25) : null,
			weight: 0.45,
		},
...
export async function scoreLogisticsSupply(
	...
	return weightedBlend([
		...
		{
			score: shippingStress == null ? null : normalizeLowerBetter(shippingStress, 0, 100),
			weight: 0.25,
		},
		{
			score: transitStress == null ? null : normalizeLowerBetter(transitStress, 0, 30),
			weight: 0.25,
		},
	]);
}
...
export async function scoreInfrastructure(
	...
	const electricityAccess = getStaticIndicatorValue(
		staticRecord,
		"infrastructure",
		"EG.ELC.ACCS.ZS",
	);
	...
	return weightedBlend([
		{
			score: electricityAccess == null ? null : normalizeHigherBetter(electricityAccess, 40, 100),
			weight: 0.4,
		},
...
export async function scoreEnergy(
	...
	const mix =
		energyMixRaw != null && typeof energyMixRaw === "object"
	...
	const electricityConsumption = getStaticIndicatorValue(
		staticRecord,
		"infrastructure",
		"EG.USE.ELEC.KH.PC",
	);

	const storageFillPct =
		storageRaw != null && typeof storageRaw === "object"
			? (() => {
			...
			: null;
	const storageStress =
		storageFillPct != null ? Math.min(1, Math.max(0, (80 - storageFillPct) / 80)) : null;
	...
	return weightedBlend([
		...
		{
			score: storageStress == null ? null : normalizeLowerBetter(storageStress * 100, 0, 100),
			weight: 0.1,
		},
		{ score: energyStress == null ? null : normalizeLowerBetter(energyStress, 0, 25), weight: 0.1 },
		{
			score:
				electricityConsumption == null
					? null
					: normalizeHigherBetter(electricityConsumption, 200, 8000),
			weight: 0.3,
		},
	]);
}
...
export async function scoreGovernanceInstitutional(
	...
	const wgiScores = getStaticWgiValues(staticRecord).map((value) =>
		normalizeHigherBetter(value, -2.5, 2.5),
	);
...
export async function scoreBorderSecurity(
	...
	const displacementMetric =
		safeNum(displacement?.hostTotal) ?? safeNum(displacement?.totalDisplaced);
...
export async function scoreInformationCognitive(
	...
	return weightedBlend([
		...
		{
			score: velocity > 0 ? normalizeLowerBetter(Math.log10(velocity + 1), 0, 3) : null,
			weight: 0.15,
		},
...
export async function scoreHealthPublicService(
	...
	return weightedBlend([
		...
		{
			score: measlesCoverage == null ? null : normalizeHigherBetter(measlesCoverage, 50, 99),
			weight: 0.35,
		},
	...
	if (fao == null) {
		return weightedBlend([
			staticRecord == null
				...
				: {
						score: IMPUTE.ipcFood.score,
						weight: 0.6,
						certaintyCoverage: IMPUTE.ipcFood.certaintyCoverage,
					},
...
export async function scoreAllDimensions(
	...
	const entries = await Promise.all(
		RESILIENCE_DIMENSION_ORDER.map(
			async (dimensionId) =>
				[
					dimensionId,
