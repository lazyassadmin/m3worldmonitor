...
function isOutcomePointer(v: unknown): v is OutcomePointer {
	...
	return (
		...
		typeof o["outcomeKey"] === "string" &&
		typeof o["schemaVersion"] === "string" &&
		typeof o["theaterCount"] === "number" &&
...
export const getSimulationOutcome: ForecastServiceHandler["getSimulationOutcome"] = async (
	...
	try {
		...
		const note =
			req.runId && req.runId !== pointer.runId
				? "runId filter not yet active; returned outcome may differ from requested run"
				: "";
		const theaterSummariesJson =
			Array.isArray(pointer.uiTheaters) && pointer.uiTheaters.length > 0
		...
		console.warn(
			"[getSimulationOutcome] Redis error:",
