...
async function fetchTextWithRelay(url: string): Promise<string | null> {
	...
	try {
		...
		if (resp.ok) {
			...
			console.warn(
				`[tech-events] Direct fetch ${url} returned short response (${text.length} chars)`,
			);
		} else {
...
async function fetchTechEvents(req: ListTechEventsRequest): Promise<ListTechEventsResponse> {
	...
	if (externalSourcesFailed > 0) {
		console.warn(
			`[tech-events] ${externalSourcesFailed}/2 external sources failed, returning ${events.length} events (curated fallback)`,
		);
	}
...
function filterEvents(events: TechEvent[], req: ListTechEventsRequest): ListTechEventsResponse {
	const { type, mappable } = req;
	const limit = clampInt(req.limit, 50, 1, 200);
	const days = clampInt(req.days, 90, 1, 365);
