...
function fromMonthMatrix(
	t: TpMonthMatrixTicket,
	origin: string,
	destination: string,
	currency: string,
	now: number,
...
export async function searchPricesTravelpayouts(opts: {
	...
	if (isDayPrecision) {
		...
		const data = await cachedFetchJson<TpV3Ticket[]>(cacheKey, 3600, () =>
			fetchTp<TpV3Ticket[]>(`${BASE_V3}/prices_for_dates?${params}`, token).then((d) => d ?? []),
		);

		quotes = (data ?? [])
			.slice(0, maxResults)
			.map((t) => fromV3(t, origin, destination, currency_, cabin, now));
		...
		const data = await cachedFetchJson<TpMonthMatrixTicket[]>(cacheKey, 7200, () =>
			fetchTp<TpMonthMatrixTicket[]>(`${BASE_V2}/month-matrix?${params}`, token).then(
				(d) => d ?? [],
			),
		);
		...
		quotes = rows
			.slice(0, maxResults)
			.map((t) => fromMonthMatrix(t, origin, destination, currency_, now));
	} else {
		// v2: latest
		const params = new URLSearchParams({
			currency: currency_,
		...
		const data = await cachedFetchJson<TpLatestTicket[]>(cacheKey, 3600, () =>
			fetchTp<TpLatestTicket[]>(`${BASE_V2}/latest?${params}`, token).then((d) => d ?? []),
		);
		...
		quotes = rows
			.slice(0, maxResults)
			.map((t) => fromLatest(t, origin, destination, currency_, now));
	}
