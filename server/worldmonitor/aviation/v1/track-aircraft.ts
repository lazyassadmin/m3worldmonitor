...
export async function trackAircraft(
			...
			async () => {
				...
				if (isCallsignOnly && relayBase) {
					try {
						...
						console.warn(
							`[Aviation] Wingbits callsign relay failed: ${err instanceof Error ? err.message : err}`,
						);
					}
				}
				...
				if (!isCallsignOnly && relayBase && req.swLat != null && req.neLat != null) {
					...
					const [osResult, wbResult] = await Promise.allSettled([
						...
						fetch(wbUrl, { headers: getRelayHeaders({}), signal: AbortSignal.timeout(10_000) })
							.then((r) =>
								r.ok ? (r.json() as Promise<WingbitsRelayResponse>) : Promise.resolve(null),
							)
							.then((d) => d?.positions ?? [])
							.catch(() => [] as PositionSample[]),
					]);
					...
					if (merged.length > 0) {
						const source =
							wbPositions.length > 0 && osPositions.length > 0
								? "wingbits"
								: wbPositions.length > 0
									? "wingbits"
									: "opensky";
					...
					try {
						...
						console.warn(
							`[Aviation] OpenSky anonymous failed: ${err instanceof Error ? err.message : err}`,
						);
					}
				}
				...
				if (!isCallsignOnly && relayBase && req.icao24) {
					try {
						...
						console.warn(
							`[Aviation] Relay icao24 failed: ${err instanceof Error ? err.message : err}`,
						);
					}
				}
	...
	if (result) {
		...
		if (req.callsign)
			positions = positions.filter((p) => p.callsign.includes(req.callsign.toUpperCase()));
