...
interface AVSFlight {
	flight?: {
		iata?: string;
		icao?: string;
		codeshared?: { flight_iata?: string; airline_iata?: string }[];
	};
...
export async function listAirportFlights(
	...
	try {
		const result = await cachedFetchJson<{ flights: FlightInstance[]; source: string }>(
			...
			async () => {
				...
				try {
					...
					console.warn(
						`[Aviation] Flights relay fetch failed for ${airport}: ${err instanceof Error ? err.message : err}`,
					);
		...
		console.warn(
			`[Aviation] ListAirportFlights error: ${err instanceof Error ? err.message : err}`,
		);
