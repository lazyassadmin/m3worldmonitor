...
export async function getAirportOpsSummary(
	...
	const requested =
		rawAirports.length > 0 ? rawAirports.map((a) => a.toUpperCase()) : DEFAULT_WATCHED_AIRPORTS;
	...
	try {
		...
		let healthy = false;
		try {
			const seedData = (await getCachedJson(SEED_CACHE_KEY, true)) as {
		...
		console.warn(
			`[Aviation] GetAirportOpsSummary failed: ${err instanceof Error ? err.message : err}`,
		);
		return { summaries: [], cacheHit: false };
	}
}
