		...
		const result = await cachedFetchJson<{ items: AviationNewsItem[] }>(
			...
			async () => {
				...
				await Promise.allSettled(
					AVIATION_RSS_FEEDS.map((f) =>
						fetchFeed(f.url, f.name).then((items) => allItems.push(...items)),
					),
				);
				...
				for (const item of allItems) {
					...
					if (item.pubDate) {
						try {
							publishedAt = new Date(item.pubDate as string).getTime();
						} catch {
							/* skip */
						}
					}
					...
					const snippet = ((item.description as string | undefined) ?? "")
						.replace(/<[^>]+>/g, "")
						.slice(0, 200);
