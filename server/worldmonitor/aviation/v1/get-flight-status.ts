...
function normalizeFlight(f: AVSFlight, now: number): FlightInstance {
	...
	return {
		...
		estimatedDeparture: f.departure?.estimated
			? new Date(f.departure.estimated).getTime()
			: schedDep + delayMs,
		...
		status: (() => {
			const m: Record<string, FlightInstance["status"]> = {
		...
		codeshareFlightNumbers: (f.flight?.codeshared ?? [])
			.map((c) => c.flight_iata ?? "")
			.filter(Boolean),
...
export async function getFlightStatus(
	...
	const flightNumber = (req.flightNumber?.toUpperCase().replace(/\s/g, "") || "").replace(
		/^([A-Z]{2,3})0+(\d+)$/,
		"$1$2",
	);
	...
	try {
		...
		console.warn(
			`[Aviation] GetFlightStatus failed for ${flightNumber}: ${err instanceof Error ? err.message : err}`,
		);
		return { flights: [], source: "error", cacheHit: false };
	}
}
