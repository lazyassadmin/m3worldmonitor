...
export async function getCarrierOps(
	...
	const airports =
		rawAirports.length > 0
			? rawAirports.map((a) => a.toUpperCase())
			: DEFAULT_WATCHED_AIRPORTS.slice(0, 3);
	...
	try {
		const result = await cachedFetchJson<{ carriers: CarrierOpsSummary[] }>(
			...
			async () => {
				...
				type FI =
					import("../../../../src/generated/server/worldmonitor/aviation/v1/service_server").FlightInstance;
				const allFlights: FI[] = [];
				...
				for (const f of allFlights) {
					...
					if (!groups.has(key)) {
						groups.set(key, {
							carrier: f.operatingCarrier ?? { iataCode: iata, icaoCode: "", name: iata },
							airport,
							flights: [],
						});
					}
