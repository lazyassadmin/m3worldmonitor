...
export async function listAirportDelays(
	...
	try {
		const seedData = (await getCachedJson(FAA_CACHE_KEY, true)) as {
	...
	if (notamResult) {
		...
		const applyNotam = (
			...
			if (existingIatas.has(airport.iata)) {
				...
				if (idx >= 0) {
					allAlerts[idx] = mergeNotamWithExistingAlert(
						airport,
						reason,
						allAlerts[idx] ?? null,
						severity,
		...
		const total =
			(notamResult.closedIcaos?.length ?? 0) + (notamResult.restrictedIcaos?.length ?? 0);
		if (total > 0) {
			console.warn(
				`[Aviation] NOTAM: ${notamResult.closedIcaos?.length ?? 0} closures, ${notamResult.restrictedIcaos?.length ?? 0} restrictions applied`,
			);
		}
	}
