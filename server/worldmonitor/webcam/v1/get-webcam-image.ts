import type {
	...
	ServerContext,
} from "../../../../src/generated/server/worldmonitor/webcam/v1/service_server";
...
export async function getWebcamImage(
	_ctx: ServerContext,
	req: GetWebcamImageRequest,
): Promise<GetWebcamImageResponse> {
	const { webcamId } = req;
	...
	const result = await cachedFetchJson<GetWebcamImageResponse>(
		...
		async () => {
			const resp = await fetch(
				`${WINDY_BASE}/${encodeURIComponent(webcamId)}?include=images,urls`,
				{
