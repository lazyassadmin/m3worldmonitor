...
function normalizeTitle(title: string): string {
	...
	return (
		title
			...
			.replace(
				/\s*[-\u2013\u2014]\s*(?:reuters|ap news|bbc|cnn|al jazeera|france 24|dw news|pbs newshour|cbs news|nbc|abc|associated press|the guardian|nos nieuws|tagesschau|cnbc|the national)\s*$/,
				"",
			)
...
function derivePhase(track: StoryTrack): ProtoStoryPhase {
	...
	if (track.currentScore > 0 && track.peakScore > 0 && track.currentScore < track.peakScore * 0.5)
		return "STORY_PHASE_FADING";
...
async function readStoryTracks(titleHashes: string[]): Promise<Map<string, StoryTrack>> {
	...
	const commands = titleHashes.map((h) => ["HMGET", `${STORY_TRACK_KEY_PREFIX}${h}`, ...fields]);
...
export async function listFeedDigest(
	...
	try {
		...
		const fresh = await cachedFetchJson<ListFeedDigestResponse>(digestCacheKey, 900, async () => {
...
async function buildDigest(variant: string, lang: string): Promise<ListFeedDigestResponse> {
	...
	try {
		...
		for (const [category, items] of results) {
			items.sort((a, b) => b.importanceScore - a.importanceScore || b.publishedAt - a.publishedAt);
		...
		const storyTracks = await readStoryTracks(uniqueHashes).catch(
			() => new Map<string, StoryTrack>(),
		);
