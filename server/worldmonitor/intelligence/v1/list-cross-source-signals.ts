...
function normalizeSignal(s: CachedSignal, index: number): CrossSourceSignal {
	return {
		...
		severityScore:
			typeof s.severityScore === "number" && Number.isFinite(s.severityScore) ? s.severityScore : 0,
		detectedAt:
			typeof s.detectedAt === "number" && Number.isFinite(s.detectedAt) ? s.detectedAt : Date.now(),
...
export const listCrossSourceSignals: IntelligenceServiceHandler["listCrossSourceSignals"] = async (
	...
	const signals = Array.isArray(payload.signals) ? payload.signals.map(normalizeSignal) : [];
