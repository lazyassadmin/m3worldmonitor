...
async function getGulfCrudeShare(
	countryCode: string,
): Promise<{ share: number; hasData: boolean }> {
...
export async function computeEnergyShockScenario(
	...
	const { share: rawGulfShare, hasData: comtradeHasData } =
		gulfShareResult.status === "fulfilled" ? gulfShareResult.value : { share: 0, hasData: false };
	...
	const effectiveCoverDays = computeEffectiveCoverDays(
		daysOfCover,
		netExporter,
		crudeLossKbd,
		crudeImportsKbd,
