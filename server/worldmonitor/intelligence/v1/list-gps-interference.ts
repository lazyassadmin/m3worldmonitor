...
export const listGpsInterference: IntelligenceServiceHandler["listGpsInterference"] = async (
	...
	const stats = regionFilter
		...
		: {
				...
				highCount: toNumber(
					data.stats?.highCount ||
						hexes.filter((hex) => hex.level === "INTERFERENCE_LEVEL_HIGH").length,
				),
				mediumCount: toNumber(
					data.stats?.mediumCount ||
						hexes.filter((hex) => hex.level === "INTERFERENCE_LEVEL_MEDIUM").length,
				),
			};
	return {
		hexes,
