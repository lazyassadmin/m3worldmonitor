...
export async function getCountryIntelBrief(
	...
	try {
		...
		contextSnapshot = sanitizeForPrompt(
			(url.searchParams.get("context") || "").trim().slice(0, 4000),
		);
	...
	const frameworkRaw =
		isPremium && typeof req.framework === "string" ? req.framework.slice(0, 2000) : "";
...
- No speculation beyond what data supports.${lang === "fr" ? "\n- IMPORTANT: You MUST respond ENTIRELY in French language." : ""}`;
	...
	try {
		result = await cachedFetchJson<GetCountryIntelBriefResponse>(
			cacheKey,
			INTEL_CACHE_TTL,
			async () => {
