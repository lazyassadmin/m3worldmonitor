...
function buildWorldBrief(data: unknown): string {
	...
	const stories = Array.isArray(d.topStories)
		? d.topStories
	...
	if (stories.length > 0) {
		...
		for (const s of stories.slice(0, 12)) {
			const title = sanitizeHeadline(
				safeStr(
					(s as Record<string, unknown>).headline || (s as Record<string, unknown>).title || s,
				),
			);
...
function buildRiskScores(data: unknown): string {
	...
	const top15 = scores
		...
		.sort((a: unknown, b: unknown) => {
			const sa = safeNum(
				(a as Record<string, unknown>)?.score ?? (a as Record<string, unknown>)?.cii,
			);
			const sb = safeNum(
				(b as Record<string, unknown>)?.score ?? (b as Record<string, unknown>)?.cii,
			);
			return sb - sa;
		})
		.slice(0, 15);
...
function buildMacroSignals(data: unknown): string {
	...
	const active = Array.isArray(d.activeSignals)
		? d.activeSignals
...
function buildPredictionMarkets(data: unknown): string {
	...
	const all = [
		...
		.sort((a: unknown, b: unknown) => {
			return (
				safeNum((b as Record<string, unknown>)?.volume) -
				safeNum((a as Record<string, unknown>)?.volume)
			);
		})
		.slice(0, 8);
...
function extractCommodityQuote(
	commodities: unknown,
	symbol: string,
...
function buildSpotCommodityLine(
	commodities: unknown,
	symbol: string,
	label: string,
	unit: string,
...
async function buildGasStorage(): Promise<string | undefined> {
	try {
				...
				try {
					...
					if (data && typeof data === "object") {
						...
						if (typeof d.fillPct === "number") {
							entries.push({
								iso2: safeStr(d.iso2) || iso2,
								fillPct: d.fillPct,
								trend: safeStr(d.trend) || undefined,
							});
						}
					}
				} catch {
		...
		const parts = sorted.map(
			(e) => `${e.iso2}: ${e.fillPct.toFixed(1)}%${e.trend ? ` (${e.trend})` : ""}`,
		);
		return parts.join(" | ");
	} catch {
		return undefined;
...
async function buildProductSupply(iso2: string): Promise<string | undefined> {
	try {
		...
		const fmt = (v: unknown) =>
			typeof v === "number" && Number.isFinite(v as number) ? Math.round(v as number) : null;
		const parts: string[] = [];
		for (const [key, label] of [
			...
			parts.push(
				`${label} ${demand} kbd demand${details.length ? ` (${details.join(", ")})` : ""}`,
			);
		}
...
async function buildGasFlows(iso2: string): Promise<string | undefined> {
	try {
		...
		const lngShare =
			typeof d.lngShareOfImports === "number"
				? Math.round((d.lngShareOfImports as number) * 100)
				: null;
...
async function buildOilStocksCover(iso2: string): Promise<string | undefined> {
	try {
		...
		const threshold =
			typeof d.obligationThreshold === "number" ? (d.obligationThreshold as number) : 90;
...
const SOURCE_LABELS: Array<
	[keyof Omit<AnalystContext, "timestamp" | "degraded" | "activeSources">, string]
> = [
...
export async function assembleAnalystContext(
	...
	const countryKey =
		geoContext && /^[A-Z]{2}$/.test(geoContext.toUpperCase())
	...
	const needsProductSupply =
		iso2 != null && new Set(["economic", "geo", "all"]).has(resolvedDomain);
	...
	const ctx: AnalystContext = {
		...
		coalSpotPrice: needsSpotEnergy
			? buildSpotCommodityLine(get(commoditiesResult), "MTF=F", "Newcastle coal", "$", "/t")
			: "",
		gasSpotTtf: needsSpotEnergy
			? buildSpotCommodityLine(get(commoditiesResult), "TTF=F", "TTF gas", "€")
			: "",
	...
	ctx.activeSources = SOURCE_LABELS.filter(([field]) => Boolean(ctx[field])).map(
		([, label]) => label,
	);
