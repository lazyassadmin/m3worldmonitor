...
export const listMarketImplications: IntelligenceServiceHandler["listMarketImplications"] = async (
	...
	const fwId =
		req.frameworkId && ALLOWED_FRAMEWORK_IDS.has(req.frameworkId) ? req.frameworkId : null;
	...
	const data = ((await getCachedJson(redisKey, true).catch(() => null)) ??
		(fwId
			? await getCachedJson(BASE_KEY, true).catch(() => null)
			: null)) as CachedMarketImplicationsData | null;
