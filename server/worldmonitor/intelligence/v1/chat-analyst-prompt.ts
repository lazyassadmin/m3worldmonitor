...
const DOMAIN_EMPHASIS: Record<string, string> = {
	...
	military:
		"Emphasise force posture, conflict escalation, weapons systems, and military operations.",
	economic:
		"Emphasise macroeconomic signals, monetary policy, inflation, supply chains, and fiscal trends.",
};
...
export function buildAnalystSystemPrompt(ctx: AnalystContext, domainFocus?: string): string {
	...
	const allowed =
		domainFocus && domainFocus !== "all" && DOMAIN_SECTIONS[domainFocus]
	...
	if (ctx.riskScores && include("riskScores")) contextSections.push(`## ${ctx.riskScores}`);
	if (ctx.marketImplications && include("marketImplications"))
		contextSections.push(`## ${ctx.marketImplications}`);
	if (ctx.forecasts && include("forecasts")) contextSections.push(`## ${ctx.forecasts}`);
	if (ctx.marketData && include("marketData")) contextSections.push(`## ${ctx.marketData}`);
	if (ctx.macroSignals && include("macroSignals")) contextSections.push(`## ${ctx.macroSignals}`);
	if (ctx.energyExposure && include("energyExposure"))
		contextSections.push(`## Energy Exposure\n${ctx.energyExposure}`);
	if (
		(ctx.gasSpotTtf || ctx.coalSpotPrice) &&
		(include("gasSpotTtf") || include("coalSpotPrice"))
	) {
		const parts: string[] = [];
		...
		contextSections.push(`## Energy Intelligence\n${ctx.energyIntelligence}`);
	{
		...
		if (energyDataParts.length)
			contextSections.push(`## Country Energy Data\n${energyDataParts.join("\n")}`);
	}
	if (ctx.predictionMarkets && include("predictionMarkets"))
		contextSections.push(`## ${ctx.predictionMarkets}`);
	if (ctx.countryBrief && include("countryBrief")) contextSections.push(`## ${ctx.countryBrief}`);
