...
export async function getCountryEnergyProfile(
	...
	const gasStorage =
		gasStorageResult.status === "fulfilled" ? (gasStorageResult.value as GasStorage | null) : null;
	const electricity =
		electricityResult.status === "fulfilled"
			? (electricityResult.value as ElectricityEntry | null)
			: null;
	const jodiOil =
		jodiOilResult.status === "fulfilled" ? (jodiOilResult.value as JodiOil | null) : null;
	const jodiGas =
		jodiGasResult.status === "fulfilled" ? (jodiGasResult.value as JodiGas | null) : null;
	const ieaStocks =
		ieaStocksResult.status === "fulfilled" ? (ieaStocksResult.value as IeaStocks | null) : null;
