...
async function fetchACLEDEvents(): Promise<
	Array<{ country: string; event_type: string; fatalities: number; daysAgo: number }>
> {
	const now = Date.now();
...
async function fetchAuxiliarySources(): Promise<AuxiliarySources> {
	...
	const [
		...
		getCachedJson(`displacement:summary:v1:${currentYear}`, true)
			...
			.then(
				(d) =>
					d ?? getCachedJson(`displacement:summary:v1:${currentYear - 1}`, true).catch(() => null),
			),
	...
	const rawStories: any[] =
		insightsRaw && Array.isArray((insightsRaw as any).topStories)
	...
	return {
		...
		orefData,
		advisories:
			advisoriesRaw && typeof advisoriesRaw === "object" && (advisoriesRaw as any).byCountry
...
export function computeCIIScores(
	...
	for (const o of aux.outages) {
		...
		else if (sev.includes("PARTIAL") || sev.includes("LOCAL") || sev.includes("MINOR"))
	...
	for (const story of aux.newsTopStories) {
		...
		let code: string | null =
			story.countryCode && data[story.countryCode] ? story.countryCode : null;
	...
	if (aux.threatSummaryByCountry) {
		const SUMMARY_WEIGHT: Record<string, number> = {
			critical: 4,
			high: 2,
			medium: 1,
			low: 0.5,
	...
	for (const code of Object.keys(TIER1_COUNTRIES)) {
		...
		const adjustedCount =
			multiplier < 0.7 ? Math.log2(unrestCount + 1) * multiplier * 5 : unrestCount * multiplier;
		...
		const outageBoost = Math.min(
			50,
			d.outageTotalCount * 30 + d.outageMajorCount * 15 + d.outagePartialCount * 5,
		);
		...
		const acledScore = Math.min(
			50,
			Math.round((d.battles * 3 + d.explosions * 4 + d.civilianViolence * 5) * multiplier),
		);
		...
		const orefBoost =
			code === "IL" && d.orefAlertCount > 0 ? 25 + Math.min(25, d.orefAlertCount * 5) : 0;
		...
		const eventScore = unrest * 0.25 + conflict * 0.3 + security * 0.2 + information * 0.25;
		...
		const orefBlendBoost =
			code === "IL"
				? (d.orefAlertCount > 0 ? 15 : 0) +
					(d.orefHistoryCount24h >= 10 ? 10 : d.orefHistoryCount24h >= 3 ? 5 : 0)
				: 0;
		...
		const advisoryFloor =
			d.advisoryLevel === "do-not-travel" ? 60 : d.advisoryLevel === "reconsider" ? 50 : 0;
...
export async function getRiskScores(
	...
	try {
		const { data: result } = await cachedFetchJsonWithMeta<GetRiskScoresResponse>(
			...
			RISK_CACHE_TTL,
			async () => {
				const [acled, aux] = await Promise.all([fetchACLEDEvents(), fetchAuxiliarySources()]);
