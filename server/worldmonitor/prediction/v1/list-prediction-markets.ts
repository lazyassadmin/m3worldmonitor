...
function toProtoMarket(m: BootstrapMarket, category: string): PredictionMarket {
	return {
		...
		source:
			m.source === "kalshi"
				? ("MARKET_SOURCE_KALSHI" as MarketSource)
				: ("MARKET_SOURCE_POLYMARKET" as MarketSource),
	};
}
...
export const listPredictionMarkets: PredictionServiceHandler["listPredictionMarkets"] = async (
	...
	try {
		...
		const variant = isTech
			? bootstrap.tech
			: isFinance
				? (bootstrap.finance ?? bootstrap.geopolitical)
