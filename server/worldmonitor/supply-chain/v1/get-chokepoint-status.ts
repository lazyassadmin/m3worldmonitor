...
export const CHOKEPOINTS: ChokepointConfig[] = [
	...
	},
	{
		...
		lat: -34.36,
		lon: 18.49,
		primaryKeywords: ["cape of good hope", "good hope"],
		areaKeywords: ["cape of good hope", "good hope", "cape town", "south africa", "cape agulhas"],
	...
	},
	{
		...
		lat: 35.96,
		lon: -5.35,
		primaryKeywords: ["strait of gibraltar", "gibraltar"],
		areaKeywords: ["strait of gibraltar", "gibraltar", "mediterranean", "algeciras", "tangier"],
	...
	},
	{
		...
		lat: 41.12,
		lon: 29.05,
		primaryKeywords: ["bosphorus", "bosporus", "dardanelles", "canakkale", "turkish straits"],
		areaKeywords: [
			"bosphorus",
			"bosporus",
			"dardanelles",
	...
	},
	{
		...
		lat: 34.0,
		lon: 129.0,
		primaryKeywords: ["korea strait", "tsushima strait"],
		areaKeywords: ["korea strait", "tsushima", "busan", "shimonoseki", "sea of japan", "east sea"],
	...
	},
	{
		...
		lat: 51.05,
		lon: 1.45,
		primaryKeywords: ["dover strait", "strait of dover", "english channel"],
		areaKeywords: ["dover", "calais", "english channel", "north sea", "pas-de-calais"],
	...
	},
	{
		...
		lat: 45.33,
		lon: 36.6,
		primaryKeywords: ["kerch strait", "kerch bridge"],
		areaKeywords: ["kerch", "crimea", "azov", "sea of azov", "black sea"],
	...
	},
	{
		...
		lat: -8.47,
		lon: 115.72,
		primaryKeywords: ["lombok strait"],
		areaKeywords: ["lombok", "bali", "indonesia", "nusa tenggara"],
...
async function fetchChokepointData(): Promise<ChokepointFetchResult> {
	...
	const [navResult, vesselResult, transitSummariesData, flowsData] = await Promise.all([
		...
		getCachedJson(TRANSIT_SUMMARIES_KEY, true).catch(
			() => null,
		) as Promise<TransitSummariesPayload | null>,
		getCachedJson(FLOWS_KEY, true).catch(() => null) as Promise<Record<
			string,
			FlowEstimateEntry
		> | null>,
	]);
	...
	if (Object.keys(summaries).length === 0) {
		const [portwatch, corridorRisk, transitCounts] = await Promise.all([
			getCachedJson(PORTWATCH_FALLBACK_KEY, true).catch(
				() => null,
			) as Promise<PortWatchData | null>,
			getCachedJson(CORRIDORRISK_FALLBACK_KEY, true).catch(() => null) as Promise<Record<
				string,
				CorridorRiskEntry
			> | null>,
			getCachedJson(TRANSIT_COUNTS_FALLBACK_KEY, true).catch(
				() => null,
			) as Promise<RelayTransitPayload | null>,
		]);
	...
	const chokepoints = CHOKEPOINTS.map((cp): ChokepointInfo => {
		...
		const disruptionScore = Math.min(
			100,
			computeDisruptionScore(threatScore, matchedWarnings.length, maxSeverity) + anomalyBonus,
		);
		...
		if (anomaly.signal) {
			descriptions.push(
				`Traffic down ${anomaly.dropPct}% vs 30-day baseline, vessels may be transiting dark (AIS off)`,
			);
		}
		...
		const result = await cachedFetchJson<GetChokepointStatusResponse>(
			...
			async () => {
				...
				setCachedJson(
					...
					{ fetchedAt: Date.now(), recordCount: chokepoints.length },
					604800,
				).catch(() => {});
				return response;
			},
