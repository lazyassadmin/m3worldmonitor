...
export function cronbachAlpha(items: number[][]): number {
	...
	for (let column = 0; column < itemCount; column += 1) {
		...
		const variance =
			sample.reduce((sum, value) => sum + (value - mean) ** 2, 0) / (observationCount - 1);
	...
	const totalVariance =
		totalScores.reduce((sum, value) => sum + (value - totalMean) ** 2, 0) / (observationCount - 1);
...
export function detectChangepoints(
	values: number[],
	threshold = CHANGEPOINT_DEFAULT_THRESHOLD,
): number[] {
