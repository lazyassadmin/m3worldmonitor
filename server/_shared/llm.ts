...
function callLlmProfile(
	...
	const provider = (
		envProvider && PROVIDER_SET.has(envProvider)
			? envProvider
			: (() => {
					if (envProvider)
						console.warn(
							`[llm] ${providerEnv}="${envProvider}" is not a known provider; falling back to "${defaultProvider}"`,
						);
	...
	return callLlm({
		...
		modelOverrides: model
			? ({ [provider]: model } as Partial<Record<LlmProviderName, string>>)
			: undefined,
	});
}
...
export type LlmStreamOptions = Omit<
	LlmCallOptions,
	"stripThinkingTags" | "validate" | "providerOrder" | "modelOverrides" | "provider"
> & {
...
export function callLlmReasoningStream(opts: LlmStreamOptions): ReadableStream<Uint8Array> {
	...
	const provider = (
		envProvider && PROVIDER_SET.has(envProvider) ? envProvider : "openrouter"
	) as LlmProviderName;
	const model = process.env.LLM_REASONING_MODEL;
	const remaining = PROVIDER_CHAIN.filter((p) => p !== provider);
	const providerOrder = [provider, ...remaining];
	const modelOverrides = model
		? ({ [provider]: model } as Partial<Record<LlmProviderName, string>>)
		: undefined;
			...
			for (const providerName of providerOrder) {
				...
				try {
					...
					if (!resp.ok || !resp.body) {
						...
						console.warn(
							`[llm-stream:${providerName}] HTTP ${resp.status} model=${creds.model} body=${errBody.slice(0, 300)}`,
						);
						continue;
					}
